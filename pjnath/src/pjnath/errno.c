...
static const struct
{
    ...
    PJ_BUILD_ERR(PJNATH_EINSTUNMSGTYPE,
    ...
    PJ_BUILD_ERR(PJNATH_ESTUNFINGERPRINT,
                 "STUN FINGERPRINT verification failed"),
    PJ_BUILD_ERR(PJNATH_ESTUNMSGINTPOS,
                 "Invalid STUN attribute after MESSAGE-INTEGRITY"),
    PJ_BUILD_ERR(PJNATH_ESTUNFINGERPOS,
                 "Invalid STUN attribute after FINGERPRINT"),

    PJ_BUILD_ERR(PJNATH_ESTUNNOMAPPEDADDR,
    ...
    PJ_BUILD_ERR(PJNATH_ESTUNINSERVER,
    ...
    PJ_BUILD_ERR(PJNATH_EICEMISMATCH,
                 "Default target doesn't match any ICE candidates"),
    ...
    PJ_BUILD_ERR(PJNATH_EICENOHOSTCAND,
                 "No host candidate associated with srflx"),
    PJ_BUILD_ERR(PJNATH_EICENOMTIMEOUT,
                 "Controlled agent timed out waiting for nomination"),
 ...
 * pjnath_strerror()
 */
static pj_str_t pjnath_strerror(pj_status_t statcode, char* buf,
                                pj_size_t bufsize)
{
    pj_str_t errstr;
    ...
    errstr.ptr = buf;
    errstr.slen =
      pj_ansi_snprintf(buf, bufsize, "Unknown pjnath error %d", statcode);
...
static pj_str_t pjnath_strerror2(pj_status_t statcode, char* buf,
                                 pj_size_t bufsize)
{
    ...
    if (cmsg.slen == 0) {
        ...
        errstr.ptr = buf;
        errstr.slen =
          pj_ansi_snprintf(buf, bufsize, "Unknown STUN err-code %d", stun_code);
    } else {
        errstr.ptr = buf;
...
PJ_DEF(pj_status_t) pjnath_init(void)
{
    ...
    status = pj_register_strerror(PJNATH_ERRNO_START, 299, &pjnath_strerror);
    pj_assert(status == PJ_SUCCESS);

    status = pj_register_strerror(PJ_STATUS_FROM_STUN_CODE(300), 699 - 300,
                                  &pjnath_strerror2);
    pj_assert(status == PJ_SUCCESS);
