...
static const char* state_names[] = { "Null",        "Resolving", "Resolved",
                                     "Allocating",  "Ready",     "Deallocating",
                                     "Deallocated", "Destroying" };
 ...
 * Prototypes.
 */
static void sess_shutdown(pj_turn_session* sess, pj_status_t status);
...
static pj_status_t stun_on_send_msg(pj_stun_session* sess, void* token,
                                    const void* pkt, pj_size_t pkt_size,
                                    const pj_sockaddr_t* dst_addr,
                                    unsigned addr_len);
static void stun_on_request_complete(pj_stun_session* sess, pj_status_t status,
...
static pj_status_t stun_on_rx_indication(pj_stun_session* sess,
                                         ...
                                         const pj_sockaddr_t* src_addr,
                                         unsigned src_addr_len);
static void dns_srv_resolver_cb(void* user_data, pj_status_t status,
                                const pj_dns_srv_record* rec);
static struct ch_t* lookup_ch_by_addr(pj_turn_session* sess,
                                      const pj_sockaddr_t* addr,
                                      unsigned addr_len, pj_bool_t update,
...
static struct perm_t* lookup_perm(pj_turn_session* sess,
                                  ...
                                  pj_bool_t update);
static void invalidate_perm(pj_turn_session* sess, struct perm_t* perm);
 ...
 */
PJ_DEF(void)
pj_turn_alloc_param_copy(pj_pool_t* pool, pj_turn_alloc_param* dst,
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_create(const pj_stun_config* cfg, const char* name, int af,
                       pj_turn_tp_type conn_type, pj_grp_lock_t* grp_lock,
                       const pj_turn_session_cb* cb, unsigned options,
                       void* user_data, pj_turn_session** p_sess)
{
    pj_pool_t* pool;
    ...
    pj_grp_lock_add_handler(sess->grp_lock, pool, sess, &turn_sess_on_destroy);
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_destroy(pj_turn_session* sess, pj_status_t last_err)
{
    PJ_ASSERT_RETURN(sess, PJ_EINVAL);
 ...
 */
PJ_DEF(void) pj_turn_session_set_log(pj_turn_session* sess, unsigned flags)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_set_software_name(pj_turn_session* sess, const pj_str_t* sw)
{
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_set_server(pj_turn_session* sess, const pj_str_t* domain,
                           int default_port, pj_dns_resolver* resolver)
{
    ...
    status = pj_inet_pton(sess->af, domain, pj_sockaddr_get_addr(&tmp_addr));
    ...
    if (!is_ip_addr && resolver) {
        ...
        PJ_LOG(5, (sess->obj_name, "Resolving %.*s%.*s with DNS SRV",
                   (int)res_name.slen, res_name.ptr, (int)domain->slen,
                   domain->ptr));
        ...
        status =
          pj_dns_srv_resolve(domain, &res_name, default_port, sess->pool,
                             resolver, opt, sess, &dns_srv_resolver_cb, NULL);
        if (status != PJ_SUCCESS) {
        ...
        ai = (pj_addrinfo*)pj_pool_calloc(sess->pool, cnt, sizeof(pj_addrinfo));
        ...
        sess->srv_addr_list =
          (pj_sockaddr*)pj_pool_calloc(sess->pool, cnt, sizeof(pj_sockaddr));
        for (i = 0; i < cnt; ++i) {
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_set_credential(pj_turn_session* sess,
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_alloc(pj_turn_session* sess, const pj_turn_alloc_param* param)
{
    ...
    PJ_ASSERT_RETURN(sess, PJ_EINVAL);
    PJ_ASSERT_RETURN(
      sess->state > PJ_TURN_STATE_NULL && sess->state <= PJ_TURN_STATE_RESOLVED,
      PJ_EINVALIDOP);
    ...
    if (param && param->af) {
        PJ_ASSERT_RETURN(
          param->af == pj_AF_INET() || param->af == pj_AF_INET6(), PJ_EINVAL);
    }
    ...
    status = pj_stun_session_send_msg(
      sess->stun, NULL, PJ_FALSE, retransmit, sess->srv_addr,
      pj_sockaddr_get_len(sess->srv_addr), tdata);
    if (status != PJ_SUCCESS) {
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_set_perm(pj_turn_session* sess, unsigned addr_cnt,
                         const pj_sockaddr addr[], unsigned options)
{
    ...
    status = pj_stun_session_create_req(sess->stun, PJ_STUN_CREATE_PERM_REQUEST,
                                        PJ_STUN_MAGIC, NULL, &tdata);
    if (status != PJ_SUCCESS) {
        pj_grp_lock_release(sess->grp_lock);
        return status;
    ...
    for (i = 0; i < addr_cnt; ++i) {
        ...
        perm =
          lookup_perm(sess, &addr[i], pj_sockaddr_get_len(&addr[i]), PJ_TRUE);
        ...
        if (perm->req_token != req_token) {
            ...
            status = pj_stun_msg_add_sockaddr_attr(
              tdata->pool, tdata->msg, PJ_STUN_ATTR_XOR_PEER_ADDR, PJ_TRUE,
...
on_error:
    ...
    while (it) {
        struct perm_t* perm =
          (struct perm_t*)pj_hash_this(sess->perm_table, it);
...
static void send_refresh(pj_turn_session* sess, int lifetime)
{
    ...
    status = pj_stun_session_send_msg(
      sess->stun, NULL, PJ_FALSE, (sess->conn_type == PJ_TURN_TP_UDP),
      sess->srv_addr, pj_sockaddr_get_len(sess->srv_addr), tdata);
    if (status != PJ_SUCCESS)
        goto on_error;
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_sendto(pj_turn_session* sess, const pj_uint8_t* pkt,
                       unsigned pkt_len, const pj_sockaddr_t* addr,
                       unsigned addr_len)
{
    struct ch_t* ch;
    struct perm_t* perm;
    ...
    PJ_ASSERT_RETURN(sess && pkt && pkt_len && addr && addr_len, PJ_EINVAL);
    ...
    if (perm == NULL) {
        ...
        status = pj_turn_session_set_perm(sess, 1, (const pj_sockaddr*)addr, 0);
        if (status != PJ_SUCCESS) {
            pj_grp_lock_release(sess->grp_lock);
            return status;
        }
    ...
    ch = lookup_ch_by_addr(sess, addr, pj_sockaddr_get_len(addr), PJ_FALSE,
                           PJ_FALSE);
    if (ch && ch->num != PJ_TURN_INVALID_CHANNEL && ch->bound) {
        ...
        status =
          sess->cb.on_send_pkt(sess, sess->tx_pkt, total_len, sess->srv_addr,
        ...
        status =
          pj_stun_msg_init(&send_ind, PJ_STUN_SEND_INDICATION, PJ_STUN_MAGIC,
        ...
        /* Encode the message */
        status =
          pj_stun_msg_encode(&send_ind, sess->tx_pkt, sizeof(sess->tx_pkt), 0,
                             NULL, &send_ind_len);
        if (status != PJ_SUCCESS)
            goto on_return;
        ...
        status = sess->cb.on_send_pkt(sess, sess->tx_pkt,
                                      (unsigned)send_ind_len, sess->srv_addr,
                                      pj_sockaddr_get_len(sess->srv_addr));
    }
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_bind_channel(pj_turn_session* sess,
                             const pj_sockaddr_t* peer_adr, unsigned addr_len)
{
    struct ch_t* ch;
    pj_stun_tx_data* tdata;
    pj_uint16_t ch_num;
    ...
    status = pj_stun_session_create_req(
      sess->stun, PJ_STUN_CHANNEL_BIND_REQUEST, PJ_STUN_MAGIC, NULL, &tdata);
    if (status != PJ_SUCCESS)
        goto on_return;
    ...
    pj_stun_msg_add_sockaddr_attr(tdata->pool, tdata->msg,
                                  PJ_STUN_ATTR_XOR_PEER_ADDR, PJ_TRUE, peer_adr,
                                  addr_len);
     ...
     */
    status = pj_stun_session_send_msg(
      sess->stun, ch, PJ_FALSE, (sess->conn_type == PJ_TURN_TP_UDP),
      sess->srv_addr, pj_sockaddr_get_len(sess->srv_addr), tdata);
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_connection_bind(pj_turn_session* sess, pj_pool_t* pool,
                                pj_uint32_t conn_id,
                                ...
                                unsigned addr_len)
{
    ...
    status = pj_stun_session_create_req(
      sess->stun, PJ_STUN_CONNECTION_BIND_REQUEST, PJ_STUN_MAGIC, NULL, &tdata);
    if (status != PJ_SUCCESS)
        goto on_return;

    /* Add CONNECTION_ID attribute */
    pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg,
                              PJ_STUN_ATTR_CONNECTION_ID, conn_id);
     ...
     */
    status = pj_stun_session_send_msg(sess->stun, conn_bind, PJ_FALSE, PJ_FALSE,
                                      peer_addr, addr_len, tdata);
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_connect(pj_turn_session* sess, const pj_sockaddr_t* peer_addr,
                        unsigned addr_len)
{
    pj_stun_tx_data* tdata;
    pj_status_t status;
    ...
    if (status != PJ_SUCCESS)
        goto on_return;
    status = pj_stun_session_send_msg(
      sess->stun, (void*)peer_addr, PJ_FALSE, PJ_FALSE, sess->srv_addr,
      pj_sockaddr_get_len(sess->srv_addr), tdata);
...
PJ_DEF(pj_status_t)
pj_turn_session_on_rx_pkt(pj_turn_session* sess, void* pkt, pj_size_t pkt_len,
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_on_rx_pkt2(pj_turn_session* sess,
                           pj_turn_session_on_rx_pkt_param* prm)
{
    pj_bool_t is_stun;
    pj_status_t status;
    ...
    if (is_stun) {
        ...
        unsigned options;
        const pj_sockaddr_t* src_addr =
          prm->src_addr ? prm->src_addr : sess->srv_addr;
        unsigned src_addr_len = prm->src_addr_len ?
                                  prm->src_addr_len :
                                  pj_sockaddr_get_len(sess->srv_addr);
 ...
 */
static pj_status_t stun_on_send_msg(pj_stun_session* stun, void* token,
                                    const void* pkt, pj_size_t pkt_size,
                                    const pj_sockaddr_t* dst_addr,
                                    unsigned addr_len)
{
    ...
    if (sess->cb.on_stun_send_pkt) {
        return (*sess->cb.on_stun_send_pkt)(
          sess, (const pj_uint8_t*)pkt, (unsigned)pkt_size, dst_addr, addr_len);
    } else {
        return (*sess->cb.on_send_pkt)(sess, (const pj_uint8_t*)pkt,
                                       (unsigned)pkt_size, dst_addr, addr_len);
    }
}
 ...
 */
static void on_session_fail(pj_turn_session* sess, enum pj_stun_method_e method,
                            pj_status_t status, const pj_str_t* reason)
{
    sess->last_status = status;
    ...
    do {
        ...
        PJ_LOG(
          4, (sess->obj_name, "%s error: %.*s", pj_stun_get_method_name(method),
                                ...
                                const pj_stun_msg* msg)
{
    ...
    lf_attr = (const pj_stun_lifetime_attr*)pj_stun_msg_find_attr(
      msg, PJ_STUN_ATTR_LIFETIME, 0);
    if (lf_attr == NULL) {
     ...
     * address family.
     */
    raddr_attr = (const pj_stun_xor_relayed_addr_attr*)pj_stun_msg_find_attr(
      msg, PJ_STUN_ATTR_XOR_RELAYED_ADDR, 0);
    if (raddr_attr == NULL && method == PJ_STUN_ALLOCATE_METHOD) {
        on_session_fail(sess, method, PJNATH_EINSTUNMSG,
                        pj_cstr(&s,
                                "Error: Received ALLOCATE without "
          ...
          raddr_attr->sockaddr.addr.sa_family != sess->af)))
    {
        on_session_fail(sess, method, PJNATH_EINSTUNMSG,
                        pj_cstr(&s,
                                "Error: Mismatched RELAY-ADDRESS "
                                "address family"));
        return;
    }
    ...
    if (raddr_attr) {
        ...
        if (pj_sockaddr_has_addr(&sess->relay_addr)) {
            if (pj_sockaddr_cmp(&sess->relay_addr, &raddr_attr->sockaddr)) {
                on_session_fail(sess, method, PJNATH_EINSTUNMSG,
                                pj_cstr(&s,
                                        "Error: different RELAY-ADDRESS is"
                                        "returned by server"));
                return;
            }
        } else {
    ...
    /* Get mapped address */
    mapped_attr = (const pj_stun_sockaddr_attr*)pj_stun_msg_find_attr(
 ...
 */
static void stun_on_request_complete(pj_stun_session* stun, pj_status_t status,
                                     void* token, pj_stun_tx_data* tdata,
                                     const pj_stun_msg* response,
                                     const pj_sockaddr_t* src_addr,
                                     unsigned src_addr_len)
{
    pj_turn_session* sess;
    enum pj_stun_method_e method =
      (enum pj_stun_method_e)PJ_STUN_GET_METHOD(tdata->msg->hdr.type);
    ...
    if (method == PJ_STUN_ALLOCATE_METHOD) {
        ...
        if (status == PJ_SUCCESS &&
            ...
            if (status == PJ_SUCCESS) {
                ...
                err_attr = (const pj_stun_errcode_attr*)pj_stun_msg_find_attr(
        ...
        if (status == PJ_SUCCESS &&
            ...
            /* Update hash table */
            lookup_ch_by_addr(sess, &ch->addr, pj_sockaddr_get_len(&ch->addr),
                              PJ_TRUE, PJ_TRUE);
            ...
            PJ_LOG(1, (sess->obj_name, "ChannelBind failed: %d/%.*s", err_code,
                       (int)reason.slen, reason.ptr));

            if (err_code == PJ_STUN_SC_ALLOCATION_MISMATCH) {
                /* Allocation mismatch means allocation no longer exists */
                on_session_fail(sess, PJ_STUN_CHANNEL_BIND_METHOD, status,
                                &reason);
                return;
            }
        }
        ...
        if (status == PJ_SUCCESS &&
            ...
            if (response) {
                ...
                eattr = (const pj_stun_errcode_attr*)pj_stun_msg_find_attr(
                  response, PJ_STUN_ATTR_ERROR_CODE, 0);
                if (eattr) {
            ...
            if (err_code == PJ_STUN_SC_ALLOCATION_MISMATCH) {
                /* Allocation mismatch means allocation no longer exists */
                on_session_fail(sess, PJ_STUN_CREATE_PERM_METHOD, status,
                                &reason);
                return;
            }
        }
        ...
        if (sess->cb.on_connection_bind_status) {
            (*sess->cb.on_connection_bind_status)(sess, status, conn_bind->id,
                                                  &conn_bind->peer_addr,
                                                  conn_bind->peer_addr_len);
        }
        ...
        if (status != PJ_SUCCESS ||
            ...
            conn_id_attr = (const pj_stun_uint_attr*)pj_stun_msg_find_attr(
              response, PJ_STUN_ATTR_CONNECTION_ID, 0);
        ...
        if (sess->cb.on_connect_complete) {
            (*sess->cb.on_connect_complete)(sess, status, conn_id, peer_addr,
                                         ...
                                         unsigned src_addr_len)
{
    ...
    if (msg->hdr.type == PJ_STUN_CONNECTION_ATTEMPT_INDICATION) {
        ...
        connection_id_attr = (pj_stun_uint_attr*)pj_stun_msg_find_attr(
          msg, PJ_STUN_ATTR_CONNECTION_ID, 0);
        ...
        if (sess->cb.on_connection_attempt) {
            (*sess->cb.on_connection_attempt)(
              sess, connection_id_attr->value, &peer_attr->sockaddr,
    ...
    icmp = (pj_stun_icmp_attr*)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_ICMP, 0);
    if (icmp != NULL) {
    ...
    /* Get DATA attribute */
    data_attr =
      (pj_stun_data_attr*)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_DATA, 0);
                                ...
                                const pj_dns_srv_record* rec)
{
    ...
    sess->srv_addr_list =
      (pj_sockaddr*)pj_pool_calloc(sess->pool, tot_cnt, sizeof(pj_sockaddr));
                                      ...
                                      pj_bool_t bind_channel)
{
    ...
    ch = (struct ch_t*)pj_hash_get(sess->ch_table, addr, addr_len, &hval);
    if (ch == NULL && update) {
        ...
        pj_hash_set(sess->pool, sess->ch_table, &ch->addr, addr_len, hval, ch);
    }
    ...
    if (ch && update) {
        ...
        if (bind_channel) {
            ...
            if (pj_hash_get(sess->ch_table, &ch->num, sizeof(ch->num),
                            &hval2) == 0) {
 ...
 */
static struct ch_t* lookup_ch_by_chnum(pj_turn_session* sess, pj_uint16_t chnum)
{
    return (struct ch_t*)pj_hash_get(sess->ch_table, &chnum, sizeof(chnum),
                                     NULL);
}
                                  ...
                                  pj_bool_t update)
{
    ...
    perm = (struct perm_t*)pj_hash_get(sess->perm_table, addr, addr_len, &hval);
    if (perm == NULL && update) {
        ...
        if (perm->expiry.sec - 1 <= now->sec) {
            if (perm->renew) {
                ...
                if (tdata == NULL) {
                    /* Create a bare CreatePermission request */
                    status = pj_stun_session_create_req(
                      sess->stun, PJ_STUN_CREATE_PERM_REQUEST, PJ_STUN_MAGIC,
                      NULL, &tdata);
                    if (status != PJ_SUCCESS) {
                        PJ_PERROR(1,
                                  (sess->obj_name, status,
                ...
                perm->expiry = *now;
                perm->expiry.sec +=
                  PJ_TURN_PERM_TIMEOUT - sess->ka_interval - 1;
                perm->req_token = req_token;
                ++count;
...
static void on_timer_event(pj_timer_heap_t* th, pj_timer_entry* e)
{
    ...
    if (eid == TIMER_KEEP_ALIVE) {
        ...
        while (it) {
            struct ch_t* ch = (struct ch_t*)pj_hash_this(sess->ch_table, it);
        ...
        if (!pkt_sent && sess->alloc_param.ka_interval > 0) {
            ...
            /* Create blank SEND-INDICATION */
            rc = pj_stun_session_create_ind(sess->stun, PJ_STUN_SEND_INDICATION,
                                            &tdata);
            if (rc == PJ_SUCCESS) {
                ...
                pj_stun_session_send_msg(
                  sess->stun, NULL, PJ_FALSE, PJ_FALSE, sess->srv_addr,
                  pj_sockaddr_get_len(sess->srv_addr), tdata);
            }
        }
