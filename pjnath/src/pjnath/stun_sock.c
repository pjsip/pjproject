...
static pj_status_t sess_on_send_msg(pj_stun_session* sess, void* token,
 ...
 * is complete
 */
static void sess_on_request_complete(pj_stun_session* sess, pj_status_t status,
...
{
    const char* names[] = { "?", "DNS resolution", "STUN Binding request",
                            "Keep-alive", "Mapped addr. changed" };
...
pj_stun_sock_create(pj_stun_config* stun_cfg, const char* name, int af,
                    const pj_stun_sock_cb* cb, const pj_stun_sock_cfg* cfg,
                    void* user_data, pj_stun_sock** p_stun_sock)
{
    pj_pool_t* pool;
    ...
    status = pj_sock_apply_qos2(stun_sock->sock_fd, cfg->qos_type,
                                &cfg->qos_params, 2, stun_sock->obj_name, NULL);
    ...
    if (cfg->so_rcvbuf_size > 0) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(
              3, (stun_sock->obj_name, status, "Failed setting SO_RCVBUF"));
        } else {
            if (sobuf_size < cfg->so_rcvbuf_size) {
                ...
                PJ_LOG(
                  5, (stun_sock->obj_name, "SO_RCVBUF set to %d", sobuf_size));
            }
        }
    }
    if (cfg->so_sndbuf_size > 0) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(
              3, (stun_sock->obj_name, status, "Failed setting SO_SNDBUF"));
        } else {
            if (sobuf_size < cfg->so_sndbuf_size) {
                ...
                PJ_LOG(
                  5, (stun_sock->obj_name, "SO_SNDBUF set to %d", sobuf_size));
            }
        }
    }
    ...
    /* Init active socket configuration */
    {
        ...
        status = pj_activesock_create(pool, stun_sock->sock_fd, pj_SOCK_DGRAM(),
    ...
    /* Create STUN session */
    {
        ...
        status = pj_stun_session_create(
          &stun_sock->stun_cfg, stun_sock->obj_name, &sess_cb, PJ_FALSE,
          stun_sock->grp_lock, &stun_sock->stun_sess);
        if (status != PJ_SUCCESS)
            goto on_error;
    }
...
/* Start socket. */
PJ_DEF(pj_status_t)
pj_stun_sock_start(pj_stun_sock* stun_sock, const pj_str_t* domain,
                   pj_uint16_t default_port, pj_dns_resolver* resolver)
{
    pj_status_t status;
    ...
    if (status != PJ_SUCCESS && resolver) {
        ...
        status = pj_dns_srv_resolve(domain, &res_name, default_port,
                                    stun_sock->pool, resolver, opt, stun_sock,
                                    &dns_srv_resolver_cb, &stun_sock->q);
        if (status != PJ_SUCCESS) {
...
{
    TRACE_((stun_sock->obj_name, "STUN sock %p request, ref_cnt=%d", stun_sock,
            pj_grp_lock_get_ref(stun_sock->grp_lock)));
                           ...
                           pj_status_t status)
{
    ...
    PJ_PERROR(4,
              (stun_sock->obj_name, status, "Session failed because %s failed",
                                ...
                                const pj_dns_srv_record* rec)
{
    ...
    pj_sockaddr_init(stun_sock->af, &stun_sock->srv_addr, NULL,
                     rec->entry[0].port);
    if (stun_sock->af == pj_AF_INET6()) {
        stun_sock->srv_addr.ipv6.sin6_addr = rec->entry[0].server.addr[0].ip.v6;
    } else {
        stun_sock->srv_addr.ipv4.sin_addr = rec->entry[0].server.addr[0].ip.v4;
    }
...
static pj_status_t get_mapped_addr(pj_stun_sock* stun_sock)
{
    ...
    status = pj_stun_session_create_req(
      stun_sock->stun_sess, PJ_STUN_BINDING_REQUEST, PJ_STUN_MAGIC,
      (const pj_uint8_t*)stun_sock->tsx_id, &tdata);
    if (status != PJ_SUCCESS)
        goto on_error;

    /* Send request */
    status = pj_stun_session_send_msg(
      stun_sock->stun_sess, INTERNAL_MSG_TOKEN, PJ_FALSE, PJ_TRUE,
      &stun_sock->srv_addr, pj_sockaddr_get_len(&stun_sock->srv_addr), tdata);
    if (status != PJ_SUCCESS)
        goto on_error;
...
pj_stun_sock_get_info(pj_stun_sock* stun_sock, pj_stun_sock_info* info)
{
    ...
    pj_memcpy(&info->srv_addr, &stun_sock->srv_addr, sizeof(pj_sockaddr));
    pj_memcpy(&info->mapped_addr, &stun_sock->mapped_addr, sizeof(pj_sockaddr));

    /* Retrieve bound address */
    addr_len = sizeof(info->bound_addr);
    status =
      pj_sock_getsockname(stun_sock->sock_fd, &info->bound_addr, &addr_len);
    if (status != PJ_SUCCESS) {
        pj_grp_lock_release(stun_sock->grp_lock);
        return status;
    }
    ...
    if (pj_sockaddr_has_addr(&info->bound_addr)) {
        ...
        status =
          pj_enum_ip_interface2(&enum_opt, &info->alias_cnt, info->aliases);
...
PJ_DEF(pj_status_t)
pj_stun_sock_sendto(pj_stun_sock* stun_sock, pj_ioqueue_op_key_t* send_key,
                    const void* pkt, unsigned pkt_len, unsigned flag,
                    const pj_sockaddr_t* dst_addr, unsigned addr_len)
{
    pj_ssize_t size;
    pj_status_t status;
    ...
    size = pkt_len;
    status = pj_activesock_sendto(stun_sock->active_sock, send_key, pkt, &size,
                                  flag, dst_addr, addr_len);
                                     ...
                                     unsigned src_addr_len)
{
    ...
    if (mapped_attr == NULL) {
        mapped_attr = (const pj_stun_sockaddr_attr*)pj_stun_msg_find_attr(
          response, PJ_STUN_ATTR_MAPPED_ADDR, 0);
    }
     ...
     */
    mapped_changed =
      !pj_sockaddr_has_addr(&stun_sock->mapped_addr) ||
      pj_sockaddr_cmp(&stun_sock->mapped_addr, &mapped_attr->sockaddr) != 0;
    if (mapped_changed) {
        /* Print mapped adress */
        {
            char addrinfo[PJ_INET6_ADDRSTRLEN + 10];
            PJ_LOG(
              4, (stun_sock->obj_name, "STUN mapped address found/changed: %s",
                  pj_sockaddr_print(&mapped_attr->sockaddr, addrinfo,
                                    sizeof(addrinfo), 3)));
        }
...
static void start_ka_timer(pj_stun_sock* stun_sock)
{
    ...
    if (stun_sock->ka_interval > 0 && !stun_sock->is_destroying) {
        ...
        pj_timer_heap_schedule_w_grp_lock(stun_sock->stun_cfg.timer_heap,
                                          &stun_sock->ka_timer, &delay, PJ_TRUE,
                                          stun_sock->grp_lock);
    }
}
