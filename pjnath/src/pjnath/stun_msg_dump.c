...
static int print_binary(char* buffer, unsigned length, const pj_uint8_t* data,
                        unsigned data_len)
{
    unsigned i;
                      ...
                      const pj_stun_attr_hdr* ahdr)
{
    ...
    if (*attr_name == '?') {
        pj_ansi_snprintf(attr_buf, sizeof(attr_buf), "Attr 0x%x", ahdr->type);
        attr_name = attr_buf;
    }

    len = pj_ansi_snprintf(p, end - p, "  %s: length=%d", attr_name,
                           (int)ahdr->length);
    APPLY();
    ...
    case PJ_STUN_ATTR_ALTERNATE_SERVER: {
        ...
        if ((af == pj_AF_INET()) || (af == pj_AF_INET6())) {
            len = pj_ansi_snprintf(
              ...
              pj_sockaddr_print(&attr->sockaddr, addr, sizeof(addr), 3));
        } else {
            len = pj_ansi_snprintf(p, end - p, ", INVALID ADDRESS FAMILY!\n");
        }
        APPLY();
    } break;
    ...
    case PJ_STUN_ATTR_CHANNEL_NUMBER: {
        ...
        len = pj_ansi_snprintf(p, end - p, ", chnum=%u (0x%x)\n",
    ...
    case PJ_STUN_ATTR_ICMP: {
        ...
        attr = (const pj_stun_uint_attr*)ahdr;
        len =
          pj_ansi_snprintf(p, end - p, ", value=%u (0x%x)\n",
                           (pj_uint32_t)attr->value, (pj_uint32_t)attr->value);
        APPLY();
    } break;
    ...
    case PJ_STUN_ATTR_SOFTWARE: {
        ...
        len = pj_ansi_snprintf(p, end - p, ", value=\"%.*s\"\n",
    ...
    case PJ_STUN_ATTR_ERROR_CODE: {
        ...
        len = pj_ansi_snprintf(p, end - p, ", err_code=%d, reason=\"%.*s\"\n",
                               attr->err_code, (int)attr->reason.slen,
                               attr->reason.ptr);
        APPLY();
    } break;
    ...
    case PJ_STUN_ATTR_UNKNOWN_ATTRIBUTES: {
        ...
        len = pj_ansi_snprintf(p, end - p, ", unknown list:");
        APPLY();

        for (j = 0; j < attr->attr_count; ++j) {
            len = pj_ansi_snprintf(p, end - p, " %d", (int)attr->attrs[j]);
            APPLY();
        }
    } break;
                 ...
                 unsigned* printed_len)
{
    ...
    len = pj_ansi_snprintf(p, end - p,
                           ...
                           " Attributes:\n",
                           msg->hdr.length, msg->hdr.magic, tsx_id[0],
                           tsx_id[1], tsx_id[2]);
    APPLY();
