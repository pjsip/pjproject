...
static const char* nat_type_names[] = {
    "Unknown",   "ErrUnknown", "Open",       "Blocked",        "Symmetric UDP",
    "Full Cone", "Symmetric",  "Restricted", "Port Restricted"
};
...
static void on_request_complete(pj_stun_session* sess, pj_status_t status,
                                void* token, pj_stun_tx_data* tdata,
                                const pj_stun_msg* response,
                                const pj_sockaddr_t* src_addr,
                                unsigned src_addr_len);
static pj_status_t on_send_msg(pj_stun_session* sess, void* token,
                               const void* pkt, pj_size_t pkt_size,
                               const pj_sockaddr_t* dst_addr,
                               unsigned addr_len);

static pj_status_t send_test(nat_detect_session* sess, enum test_type test_id,
                             const pj_sockaddr* alt_addr,
                             pj_uint32_t change_flag);
static void on_sess_timer(pj_timer_heap_t* th, pj_timer_entry* te);
                                       ...
                                       pj_sockaddr* local_addr)
{
    ...
    status = pj_sock_socket(server->addr.sa_family, pj_SOCK_DGRAM(), 0, &sock);
                         ...
                         void* user_data, pj_stun_nat_detect_cb* cb)
{
    ...
    status =
      pj_ioqueue_register_sock2(sess->pool, stun_cfg->ioqueue, sess->sock,
                                sess->grp_lock, sess, &ioqueue_cb, &sess->key);
    if (status != PJ_SUCCESS)
        goto on_error;
...
static void end_session(nat_detect_session* sess, pj_status_t status,
                             ...
                             pj_ssize_t bytes_read)
{
    ...
    status = pj_ioqueue_recvfrom(key, op_key, sess->rx_pkt, &sess->rx_pkt_len,
                                 PJ_IOQUEUE_ALWAYS_ASYNC, &sess->src_addr,
                                 &sess->src_addr_len);
 ...
 */
static pj_status_t on_send_msg(pj_stun_session* stun_sess, void* token,
                               const void* pkt, pj_size_t pkt_size,
 ...
 */
static void on_request_complete(pj_stun_session* stun_sess, pj_status_t status,
                                void* token, pj_stun_tx_data* tdata,
                                const pj_stun_msg* response,
                                const pj_sockaddr_t* src_addr,
                                unsigned src_addr_len)
{
    ...
    if (status == PJ_SUCCESS) {
        ...
        if (PJ_STUN_IS_ERROR_RESPONSE(response->hdr.type)) {
            ...
            eattr = (pj_stun_errcode_attr*)pj_stun_msg_find_attr(
            ...
            mattr = (pj_stun_sockaddr_attr*)pj_stun_msg_find_attr(
            ...
            ca = (pj_stun_changed_addr_attr*)pj_stun_msg_find_attr(
              response, PJ_STUN_ATTR_CHANGED_ADDR, 0);
    ...
    if (test_id >= ST_MAX) {
        PJ_LOG(4, (sess->pool->obj_name,
                   "Invalid transaction ID %u in response", test_id));
    ...
    PJ_PERROR(
      5, (sess->pool->obj_name, status, "Completed %s", test_names[test_id]));
                             ...
                             pj_uint32_t change_flag)
{
    ...
    status = pj_stun_session_create_req(
      sess->stun_sess, PJ_STUN_BINDING_REQUEST, magic, (pj_uint8_t*)tsx_id,
    ...
    status =
      pj_stun_msg_add_uint_attr(sess->pool, sess->result[test_id].tdata->msg,
                                PJ_STUN_ATTR_CHANGE_REQUEST, change_flag);
    if (status != PJ_SUCCESS)
        goto on_error;
