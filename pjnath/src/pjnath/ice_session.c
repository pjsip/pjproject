...
static const char* check_state_name[] = { "Frozen", "Waiting", "In Progress",
                                          "Succeeded", "Failed" };

static const char* clist_state_name[] = { "Idle", "Running", "Completed" };
#endif /* PJ_LOG_MAX_LEVEL >= 4 */

static const char* role_names[] = { "Unknown", "Controlled", "Controlling" };
...
static void destroy_ice(pj_ice_sess* ice, pj_status_t reason);
static pj_status_t start_periodic_check(pj_timer_heap_t* th,
                                        pj_timer_entry* te);
static void start_nominated_check(pj_ice_sess* ice);
static void periodic_timer(pj_timer_heap_t* th, pj_timer_entry* te);
static void handle_incoming_check(pj_ice_sess* ice,
                                  const pj_ice_rx_check* rcheck);
static void end_of_cand_ind_timer(pj_timer_heap_t* th, pj_timer_entry* te);

/* These are the callbacks registered to the STUN sessions */
static pj_status_t on_stun_send_msg(pj_stun_session* sess, void* token,
...
static pj_status_t stun_auth_get_auth(void* user_data, pj_pool_t* pool,
                                      pj_str_t* realm, pj_str_t* nonce);
static pj_status_t stun_auth_get_cred(const pj_stun_msg* msg, void* user_data,
                                      pj_pool_t* pool, pj_str_t* realm,
                                      pj_str_t* username, pj_str_t* nonce,
                                      pj_stun_passwd_type* data_type,
                                      pj_str_t* data);
static pj_status_t stun_auth_get_password(
  const pj_stun_msg* msg, void* user_data, const pj_str_t* realm,
  const pj_str_t* username, pj_pool_t* pool, pj_stun_passwd_type* data_type,
  pj_str_t* data);
                       ...
                       pj_ice_cand_type type, const pj_sockaddr* base_addr)
{
    ...
    pj_ansi_snprintf(buf, sizeof(buf), "%c%x", get_type_prefix(type), val);
...
/* Init component */
static pj_status_t init_comp(pj_ice_sess* ice, unsigned comp_id,
                             pj_ice_sess_comp* comp)
{
                   ...
                   pj_ice_sess** p_ice)
{
    ...
    pj_grp_lock_add_handler(ice->grp_lock, pool, ice, &ice_on_destroy);
    ...
    *p_ice = ice;
    ...
    LOG4((ice->obj_name, "ICE session created, comp_cnt=%d, role is %s agent",
...
pj_ice_sess_set_options(pj_ice_sess* ice, const pj_ice_sess_options* opt)
{
    ...
    if (ice->is_trickling) {
        LOG5(
          (ice->obj_name, "Trickle ICE is active (%s mode)",
        ...
        if (ice->opt.aggressive) {
            ...
            LOG4((ice->obj_name,
                  "Warning: aggressive nomination is disabled"
...
static void destroy_ice(pj_ice_sess* ice, pj_status_t reason)
{
    ...
    pj_timer_heap_cancel_if_active(ice->stun_cfg.timer_heap, &ice->clist.timer,
                                   PJ_FALSE);
...
static pj_uint32_t CALC_CAND_PRIO(pj_ice_sess* ice, pj_ice_cand_type type,
                                  pj_uint32_t local_pref, pj_uint32_t comp_id)
{
#if PJNATH_ICE_PRIO_STD
    return ((ice->prefs[type] & 0xFF) << 24) + ((local_pref & 0xFFFF) << 8) +
           (((256 - comp_id) & 0xFF) << 0);
#else
    enum
    {
                     ...
                     unsigned* p_cand_id)
{
    ...
    PJ_ASSERT_RETURN(
      ice && comp_id && foundation && addr && base_addr && addr_len, PJ_EINVAL);
    ...
    LOG4((ice->obj_name,
          ...
          lcand->id, lcand->comp_id, cand_type_names[lcand->type],
          (int)lcand->foundation.slen, lcand->foundation.ptr, ice->tmp.txt,
          pj_sockaddr_get_port(&lcand->addr),
          pj_sockaddr_print(&lcand->base_addr, address, sizeof(address), 2),
          pj_sockaddr_get_port(&lcand->base_addr), lcand->prio, lcand->prio));
...
PJ_DEF(pj_status_t)
pj_ice_sess_find_default_cand(pj_ice_sess* ice, unsigned comp_id, int* cand_id)
{
    unsigned i;
    ...
    *cand_id = -1;
    ...
    for (i = 0; i < ice->lcand_cnt; ++i) {
        pj_ice_sess_cand* lcand = &ice->lcand[i];
        if (lcand->comp_id == comp_id && lcand->type == PJ_ICE_CAND_TYPE_HOST) {
            *cand_id = GET_LCAND_ID(lcand);
            pj_grp_lock_release(ice->grp_lock);
            return PJ_SUCCESS;
        }
...
PJ_INLINE(int)
CMP_CHECK_STATE(const pj_ice_sess_check* c1, const pj_ice_sess_check* c2)
{
...
PJ_INLINE(int)
CMP_CHECK_PRIO(const pj_ice_sess_check* c1, const pj_ice_sess_check* c2)
{
                              ...
                              const pj_ice_sess_check* check)
{
    ...
    len =
      pj_ansi_snprintf(buffer, bufsize, "%d: [%d] %s:%d-->%s:%d",
                       (int)GET_CHECK_ID(clist, check), check->lcand->comp_id,
                       pj_sockaddr_print(&lcand->addr, laddr, sizeof(laddr), 2),
                       pj_sockaddr_get_port(&lcand->addr),
                            ...
                            pj_ice_sess_checklist_state st)
{
    if (clist->state != st) {
        LOG5((ice->obj_name, "Checklist: state changed from %s to %s",
              clist_state_name[clist->state], clist_state_name[st]));
        clist->state = st;
    }
}
...
static void sort_checklist(pj_ice_sess* ice, pj_ice_sess_checklist* clist)
{
    ...
    for (i = 0; i < clist->count - 1; ++i) {
        ...
        for (j = i + 1; j < clist->count; ++j) {
            int cmp_state =
              CMP_CHECK_STATE(&clist->checks[j], &clist->checks[highest]);
            if (cmp_state > 0 ||
        ...
        if (highest != i) {
            ...
            pj_memcpy(&clist->checks[highest], &tmp, sizeof(pj_ice_sess_check));
...
static void remove_check(pj_ice_sess* ice, pj_ice_sess_checklist* clist,
                         unsigned check_idx, const char* reason)
{
    LOG5((ice->obj_name, "Check %s pruned (%s)",
          dump_check(ice->tmp.txt, sizeof(ice->tmp.txt), clist,
                     &clist->checks[check_idx]),
          reason));

    pj_array_erase(clist->checks, sizeof(clist->checks[0]), clist->count,
                   check_idx);
    --clist->count;
}
            ...
            srflx->type == PJ_ICE_CAND_TYPE_PRFLX)
        {
            ...
            if (j == ice->lcand_cnt) {
                ...
                LOG4((
                  ...
                  pj_sockaddr_print(&srflx->base_addr, baddr, sizeof(baddr), 2),
...
static void ice_keep_alive(pj_ice_sess* ice, pj_bool_t send_now)
{
    if (send_now) {
        ...
        status = pj_stun_session_create_ind(comp->stun_sess,
                                            PJ_STUN_BINDING_INDICATION, &tdata);
        if (status != PJ_SUCCESS)
            goto done;
    ...
    if (ice->timer.id == TIMER_NONE) {
        ...
        pj_timer_heap_schedule_w_grp_lock(ice->stun_cfg.timer_heap, &ice->timer,
...
static void on_ice_complete(pj_ice_sess* ice, pj_status_t status)
{
    if (!ice->is_complete) {
        ...
        /* Log message */
        LOG4(
          (ice->obj_name, "ICE process complete, status=%s",
           pj_strerror(status, ice->tmp.errmsg, sizeof(ice->tmp.errmsg)).ptr));
        ...
        if (ice->cb.on_ice_complete) {
            ...
            pj_timer_heap_schedule_w_grp_lock(
              ...
              TIMER_COMPLETION_CALLBACK, ice->grp_lock);
        }
    }
}
...
static pj_bool_t check_ice_complete(pj_ice_sess* ice)
{
        ...
        if (ice->role == PJ_ICE_SESS_ROLE_CONTROLLED) {
            ...
            if (i < ice->comp_cnt) {
                    ...
                    ice->opt.controlled_agent_want_nom_timeout >= 0)
                {
                    ...
                    pj_timer_heap_schedule_w_grp_lock(
                      ice->stun_cfg.timer_heap, &ice->timer, &delay,
                      TIMER_CONTROLLED_WAIT_NOM, ice->grp_lock);
    ...
    if (/*check->err_code == PJ_SUCCESS && */
        ice->role == PJ_ICE_SESS_ROLE_CONTROLLING && !ice->is_nominating &&
        ice->timer.id == TIMER_NONE)
    {
        ...
        LOG4((ice->obj_name, "Scheduling nominated check in %d ms",
        ...
        pj_timer_heap_schedule_w_grp_lock(ice->stun_cfg.timer_heap, &ice->timer,
                                          &delay, TIMER_START_NOMINATED_CHECK,
                                          ice->grp_lock);
        return PJ_FALSE;
    }
...
static pj_bool_t on_check_complete(pj_ice_sess* ice, pj_ice_sess_check* check)
{
    pj_ice_sess_comp* comp;
    unsigned i;
            ...
            if (c->lcand->comp_id == check->lcand->comp_id) {
                if (c->state < PJ_ICE_SESS_CHECK_STATE_IN_PROGRESS) {
                    ...
                    check_set_state(ice, c, PJ_ICE_SESS_CHECK_STATE_FAILED,
                           ...
                           (PJ_ICE_CANCEL_ALL || CMP_CHECK_PRIO(c, check) < 0))
                {
...
static void end_of_cand_ind_timer(pj_timer_heap_t* th, pj_timer_entry* te)
{
    ...
    if (ice->is_trickling && !ice->is_complete) {
        LOG5((ice->obj_name,
              "End-of-candidate timer timeout, any future "
...
static pj_status_t add_rcand_and_update_checklist(
  pj_ice_sess* ice, unsigned rem_cand_cnt, const pj_ice_sess_cand rem_cand[],
  pj_bool_t trickle_done)
{
    ...
    for (i = 0; i < rem_cand_cnt; ++i) {
        ...
        if (ice->rcand_cnt >= PJ_ICE_MAX_CAND) {
            ...
            PJ_PERROR(
              3, (ice->obj_name, PJ_ETOOMANY, "Cannot add remote candidate %s",
                  pj_sockaddr_print(&rem_cand[i].addr, tmp, sizeof(tmp), 3)));
            continue;
        }
    ...
    for (i = 0; i < ice->lcand_cnt; ++i) {
		...
		if (k < clist->count)
            ...
            chk->foundation_idx =
              get_check_foundation_idx(ice, lcand, rcand, PJ_TRUE);
            ...
            if (ice->is_trickling) {
                ...
                for (k = 0; k < clist->count; ++k) {
                     ...
                     */
                    if (clist->checks[k].lcand->comp_id == lcand->comp_id &&
                        pj_cmp_timestamp(&clist->checks[k].prio, &chk->prio) >
                          0)
                    {
                        break;
                    }
                ...
                /* Unfreeze */
                if (k == clist->count)
                    check_set_state(ice, chk, PJ_ICE_SESS_CHECK_STATE_WAITING,
                                    0);
            }
    ...
    if (ice->opt.trickle != PJ_ICE_SESS_TRICKLE_DISABLED && clist->count > 0 &&
        !ice->is_complete && clist->state == PJ_ICE_SESS_CHECKLIST_ST_RUNNING)
    {
                              ...
                              pj_bool_t trickle_done)
{
    ...
    PJ_ASSERT_RETURN(
      ice && ((rem_cand_cnt == 0) || (rem_ufrag && rem_passwd && rem_cand)),
      PJ_EINVAL);
    ...
    {
        LOG5((ice->obj_name,
              "Ignored remote candidate update due to remote "
              "ufrag/pwd mismatch"));
        rem_cand_cnt = 0;
    }
...
static pj_status_t perform_check(pj_ice_sess* ice, pj_ice_sess_checklist* clist,
                                 unsigned check_id, pj_bool_t nominate)
{
    pj_ice_sess_comp* comp;
    pj_ice_msg_data* msg_data;
    pj_ice_sess_check* check;
    ...
    LOG5((ice->obj_name, "Sending connectivity check for check %s",
          dump_check(ice->tmp.txt, sizeof(ice->tmp.txt), clist, check)));
    pj_log_push_indent();

    /* Create request */
    status =
      pj_stun_session_create_req(comp->stun_sess, PJ_STUN_BINDING_REQUEST,
                                 PJ_STUN_MAGIC, NULL, &check->tdata);
    if (status != PJ_SUCCESS) {
    ...
    status = pj_stun_session_send_msg(
      comp->stun_sess, msg_data, PJ_FALSE, PJ_TRUE, &rcand->addr,
 ...
 */
static pj_status_t start_periodic_check(pj_timer_heap_t* th, pj_timer_entry* te)
{
    timer_data* td;
    pj_ice_sess* ice;
    pj_ice_sess_checklist* clist;
...
PJ_DEF(pj_status_t) pj_ice_sess_start_check(pj_ice_sess* ice)
{
    ...
    if (ice->is_trickling && !pj_timer_entry_running(&ice->timer_end_of_cand)) {
        ...
        status = pj_timer_heap_schedule_w_grp_lock(
          ice->stun_cfg.timer_heap, &ice->timer_end_of_cand, &delay, PJ_TRUE,
          ice->grp_lock);
        if (status != PJ_SUCCESS) {
            LOG4((ice->obj_name,
                                    ...
                                    unsigned addr_len)
{
    ...
    status = (*ice->cb.on_tx_pkt)(ice, sd->comp_id, msg_data->transport_id, pkt,
                                  pkt_size, dst_addr, addr_len);
                                     ...
                                     unsigned src_addr_len)
{
    ...
    if (status != PJ_SUCCESS) {
        ...
        LOG4(
          (ice->obj_name, "Check %s%s: connectivity check FAILED: %s",
           dump_check(ice->tmp.txt, sizeof(ice->tmp.txt), &ice->clist, check),
           (check->nominated ? " (nominated)" : " (not nominated)"), errmsg));
        pj_log_push_indent();
        ...
        ((pj_sockaddr*)src_addr)->addr.sa_family == pj_AF_INET6())
    {
        ...
        status = pj_sockaddr_synthesize(pj_AF_INET6(), &synth_addr,
                                        &check->rcand->addr);
        if (status == PJ_SUCCESS && pj_sockaddr_cmp(&synth_addr, src_addr) == 0)
        {
    ...
    LOG4((ice->obj_name, "Check %s%s: connectivity check SUCCESS",
          dump_check(ice->tmp.txt, sizeof(ice->tmp.txt), &ice->clist, check),
          (check->nominated ? " (nominated)" : " (not nominated)")));

    /* Get the STUN XOR-MAPPED-ADDRESS attribute. */
    xaddr = (pj_stun_xor_mapped_addr_attr*)pj_stun_msg_find_attr(
    ...
    if (lcand == NULL) {
        ...
        status = pj_ice_sess_add_cand(
          ice, check->lcand->comp_id, msg_data->transport_id,
          PJ_ICE_CAND_TYPE_PRFLX,
#if PJNATH_ICE_PRIO_STD
          65535 - (pj_uint16_t)ice->lcand_cnt,
#else
          ((1 << PJ_ICE_LOCAL_PREF_BITS) - 1) - ice->lcand_cnt,
#endif
          &foundation, &xaddr->sockaddr, &check->lcand->base_addr,
          &check->lcand->base_addr, pj_sockaddr_get_len(&xaddr->sockaddr),
          &cand_id);
        if (status != PJ_SUCCESS) {
            check_set_state(ice, check, PJ_ICE_SESS_CHECK_STATE_FAILED, status);
            on_check_complete(ice, check);
                                      ...
                                      unsigned src_addr_len)
{
    ...
    if (msg->hdr.type != PJ_STUN_BINDING_REQUEST) {
        pj_stun_session_respond(sess, rdata, PJ_STUN_SC_BAD_REQUEST, NULL,
                                token, PJ_TRUE, src_addr, src_addr_len);
        return PJ_SUCCESS;
    }
    ...
    prio_attr = (pj_stun_priority_attr*)pj_stun_msg_find_attr(
      msg, PJ_STUN_ATTR_PRIORITY, 0);
    if (prio_attr == NULL) {
    ...
    uc_attr = (pj_stun_use_candidate_attr*)pj_stun_msg_find_attr(
      msg, PJ_STUN_ATTR_USE_CANDIDATE, 0);

    /* Get ICE-CONTROLLING or ICE-CONTROLLED */
    role_attr = (pj_stun_uint64_attr*)pj_stun_msg_find_attr(
      msg, PJ_STUN_ATTR_ICE_CONTROLLING, 0);
    if (role_attr == NULL) {
        role_attr = (pj_stun_uint64_attr*)pj_stun_msg_find_attr(
          msg, PJ_STUN_ATTR_ICE_CONTROLLED, 0);
    }
    ...
    if (ice->tx_ufrag.slen == 0) {
        ...
        uname_attr = (pj_stun_string_attr*)pj_stun_msg_find_attr(
          msg, PJ_STUN_ATTR_USERNAME, 0);
     ...
     */
    if (ice->role == PJ_ICE_SESS_ROLE_CONTROLLING && role_attr &&
        role_attr->hdr.type == PJ_STUN_ATTR_ICE_CONTROLLING)
    {
        if (pj_cmp_timestamp(&ice->tie_breaker, &role_attr->value) < 0) {
            ...
            pj_stun_session_respond(sess, rdata, PJ_STUN_SC_ROLE_CONFLICT, NULL,
                                    token, PJ_TRUE, src_addr, src_addr_len);
            pj_grp_lock_release(ice->grp_lock);
            return PJ_SUCCESS;
        }

    } else if (ice->role == PJ_ICE_SESS_ROLE_CONTROLLED && role_attr &&
               role_attr->hdr.type == PJ_STUN_ATTR_ICE_CONTROLLED)
    {
        if (pj_cmp_timestamp(&ice->tie_breaker, &role_attr->value) < 0) {
            /* Generate 487 response */
            pj_stun_session_respond(sess, rdata, PJ_STUN_SC_ROLE_CONFLICT, NULL,
    ...
    status = pj_stun_msg_add_sockaddr_attr(
      tdata->pool, tdata->msg, PJ_STUN_ATTR_XOR_MAPPED_ADDR, PJ_TRUE,
                                  ...
                                  const pj_ice_rx_check* rcheck)
{
    ...
    if (i == ice->rcand_cnt) {
        ...
        rcand->foundation.slen =
          pj_ansi_snprintf(rcand->foundation.ptr, 36, "f%p", p);
    ...
    if (i != ice->clist.count) {
            ...
            c->state == PJ_ICE_SESS_CHECK_STATE_WAITING)
        {
            ...
            if (ice->is_nominating && !ice->opt.aggressive) {
                LOG5((ice->obj_name,
                      "Triggered check for check %d not "
                      "performed because nomination is in progress",
                      i));
            } else {
                ...
                LOG5((ice->obj_name,
                      "Performing triggered check for "
                      "check %d",
                      i));
                pj_log_push_indent();
             ...
             */
            LOG5((ice->obj_name,
                  "Triggered check for check %d not performed "
            ...
            if (rcheck->use_candidate) {
                for (j = 0; j < ice->valid_list.count; ++j) {
                    ...
                    if (vc->lcand->transport_id == c->lcand->transport_id &&
                        ...
                        /* Update valid check and nominated check for the
                         * component */
    ...
    else if (ice->clist.count < PJ_ICE_MAX_CHECKS)
    {
        ...
        LOG4((ice->obj_name,
              "Error: unable to perform triggered check: "
                                         ...
                                         unsigned src_addr_len)
{
    ...
    if (msg->hdr.type == PJ_STUN_BINDING_INDICATION) {
        LOG5((sd->ice->obj_name,
              "Received Binding Indication keep-alive "
              "for component %d",
              sd->comp_id));
    } else {
        LOG4((sd->ice->obj_name,
              "Received unexpected %s indication "
              "for component %d",
              pj_stun_get_method_name(msg->hdr.type), sd->comp_id));
    }
                      ...
                      const pj_sockaddr_t* src_addr, int src_addr_len)
{
     ...
     */
    status =
      pj_stun_msg_check((const pj_uint8_t*)pkt, pkt_size,
                        PJ_STUN_IS_DATAGRAM | PJ_STUN_NO_FINGERPRINT_CHECK);
    if (status == PJ_SUCCESS) {
        status = pj_stun_session_on_rx_pkt(comp->stun_sess, pkt, pkt_size,
                                           PJ_STUN_IS_DATAGRAM, msg_data, NULL,
