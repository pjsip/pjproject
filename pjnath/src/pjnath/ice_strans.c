...
static pj_status_t ice_tx_pkt(pj_ice_sess* ice, unsigned comp_id,
                              unsigned transport_id, const void* pkt,
                              pj_size_t size, const pj_sockaddr_t* dst_addr,
                              unsigned dst_addr_len);
static void ice_rx_data(pj_ice_sess* ice, unsigned comp_id,
...
static pj_bool_t stun_on_rx_data(pj_stun_sock* stun_sock, void* pkt,
                                 unsigned pkt_len,
...
static pj_bool_t stun_on_status(pj_stun_sock* stun_sock, pj_stun_sock_op op,
                                pj_status_t status);

/* TURN callbacks */
static void turn_on_rx_data(pj_turn_sock* turn_sock, void* pkt,
 ...
 */
PJ_DEF(void)
pj_ice_strans_cfg_copy(pj_pool_t* pool, pj_ice_strans_cfg* dst,
                       const pj_ice_strans_cfg* src)
{
    ...
    for (i = 0; i < src->stun_tp_cnt; ++i) {
        if (src->stun_tp[i].server.slen)
            pj_strdup(pool, &dst->stun_tp[i].server, &src->stun_tp[i].server);
    }
    ...
    for (i = 0; i < src->turn_tp_cnt; ++i) {
        if (src->turn_tp[i].server.slen)
            pj_strdup(pool, &dst->turn_tp[i].server, &src->turn_tp[i].server);
...
static pj_status_t add_update_turn(pj_ice_strans* ice_st,
                                   pj_ice_strans_comp* comp, unsigned idx,
                                   unsigned max_cand_cnt)
{
    ...
    if (ice_st->cfg.comp[comp_idx].qos_params.flags)
        pj_memcpy(&sock_cfg->qos_params, &ice_st->cfg.comp[comp_idx].qos_params,
    ...
    status = pj_turn_sock_create(&ice_st->cfg.stun_cfg, turn_cfg->af,
                                 turn_cfg->conn_type, &turn_sock_cb, sock_cfg,
    ...
    /* Start allocation */
    status = pj_turn_sock_alloc(comp->turn[idx].sock, &turn_cfg->server,
                                turn_cfg->port, ice_st->cfg.resolver,
                                 ...
                                 pj_ice_sess_cand* rcand)
{
    ...
    if (lcand->type != rcand->type || lcand->status != rcand->status ||
                                     ...
                                     unsigned max_cand_cnt)
{
    ...
    status =
      pj_stun_sock_create(&ice_st->cfg.stun_cfg, NULL, stun_cfg->af,
                          &stun_sock_cb, sock_cfg, data, &comp->stun[idx].sock);
    if (status != PJ_SUCCESS)
        return status;
    ...
    while (stun_cfg->server.slen) {
        ...
        pj_ice_calc_foundation(ice_st->pool, &cand->foundation, cand->type,
                               &cand->base_addr);
        comp->cand_cnt++;
        max_cand_cnt--;

        /* Set default candidate to srflx */
        if (comp->cand_list[comp->default_cand].type !=
            ...
            if (cand_duplicate) {
                PJ_LOG(
                  4, (ice_st->obj_name,
                      ...
                      comp->comp_id,
                      pj_sockaddr_print(&cand->addr, addrinfo, sizeof(addrinfo),
                                        3),
                      cand->transport_id));
...
static pj_status_t create_comp(pj_ice_strans* ice_st, unsigned comp_id)
{
    ...
    for (i = 0; i < ice_st->cfg.stun_tp_cnt; ++i) {
        unsigned max_cand_cnt =
          PJ_ICE_ST_MAX_CAND - comp->cand_cnt - ice_st->cfg.turn_tp_cnt;
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(3, (ice_st->obj_name, status,
                          "Failed creating STUN transport #%d for comp %d", i,
                          comp->comp_id));
            // return status;
        }
    }
    ...
    for (i = 0; i < ice_st->cfg.turn_tp_cnt; ++i) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(3, (ice_st->obj_name, status,
                          "Failed creating TURN transport #%d for comp %d", i,
                          comp->comp_id));
...
static pj_status_t alloc_send_buf(pj_ice_strans* ice_st, unsigned buf_size)
{
    if (buf_size > ice_st->buf_size) {
        ...
        ice_st->buf_pool = pj_pool_create(
          ice_st->pf, "ice_buf",
          (buf_size + sizeof(pending_send)) * ice_st->num_buf, 512, NULL);
        if (!ice_st->buf_pool)
            return PJ_ENOMEM;
        ...
        for (i = 0; i < ice_st->num_buf; i++) {
            ice_st->send_buf[i].buffer =
              pj_pool_alloc(ice_st->buf_pool, buf_size);
        }
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_create(const char* name, const pj_ice_strans_cfg* cfg,
                     unsigned comp_cnt, void* user_data,
                     const pj_ice_strans_cb* cb, pj_ice_strans** p_ice_st)
{
    pj_pool_t* pool;
    ...
    PJ_ASSERT_RETURN(comp_cnt && cb && p_ice_st && comp_cnt <= PJ_ICE_MAX_COMP,
                     PJ_EINVAL);
    ...
    PJ_LOG(4, (ice_st->obj_name,
               "Creating ICE stream transport with %d component(s)", comp_cnt));
    pj_log_push_indent();
    ...
    pj_grp_lock_add_handler(ice_st->grp_lock, pool, ice_st, &ice_st_on_destroy);

    pj_ice_strans_cfg_copy(pool, &ice_st->cfg, cfg);

    /* To maintain backward compatibility, check if old/deprecated setting is
     * set and the new setting is not, copy the value to the new setting.
     */
    ...
    ice_st->comp = (pj_ice_strans_comp**)pj_pool_calloc(
      pool, comp_cnt, sizeof(pj_ice_strans_comp*));
...
static void sess_init_update(pj_ice_strans* ice_st)
{
            ...
            if (cand->status == PJ_EPENDING) {
                PJ_LOG(
                  5, (ice_st->obj_name,
                      ...
                      comp->comp_id, j, pj_ice_get_cand_type_name(cand->type)));
                return;
            }
    ...
    if (ice_st->cb.on_ice_complete)
        (*ice_st->cb.on_ice_complete)(ice_st, PJ_ICE_STRANS_OP_INIT, status);
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_get_options(pj_ice_strans* ice_st, pj_ice_sess_options* opt)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_set_options(pj_ice_strans* ice_st, const pj_ice_sess_options* opt)
{
    PJ_ASSERT_RETURN(ice_st && opt, PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_update_comp_cnt(pj_ice_strans* ice_st, unsigned comp_cnt)
{
    unsigned i;
    ...
    PJ_LOG(4,
           (ice_st->obj_name,
            "Updated ICE stream transport components number to %d", comp_cnt));
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_init_ice(pj_ice_strans* ice_st, pj_ice_sess_role role,
                       ...
                       const pj_str_t* local_passwd)
{
    ...
    status = pj_ice_sess_create(&ice_st->cfg.stun_cfg, ice_st->obj_name, role,
                                ice_st->comp_cnt, &ice_cb, local_ufrag,
                                local_passwd, ice_st->grp_lock, &ice_st->ice);
    if (status != PJ_SUCCESS)
        return status;
    ...
    for (i = 0; i < ice_st->comp_cnt; ++i) {
        ...
        for (j = 0; j < comp->cand_cnt; ++j) {
            ...
            if (cand->status != PJ_SUCCESS) {
                PJ_LOG(
                  5, (ice_st->obj_name,
                      "Candidate %d of comp %d is not added (pending)", j, i));
                continue;
            }
             ...
             * is not IPv4.
             */
            if (comp->ipv4_mapped && cand->addr.addr.sa_family != pj_AF_INET())
            {
                continue;
            }
            ...
            status = pj_ice_sess_add_cand(
              ice_st->ice, comp->comp_id, cand->transport_id, cand->type,
              cand->local_pref, &cand->foundation, &cand->addr,
              &cand->base_addr, &cand->rel_addr,
              pj_sockaddr_get_len(&cand->addr), (unsigned*)&ice_cand_id);
            if (status != PJ_SUCCESS)
                goto on_error;
        }
    }
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_get_ufrag_pwd(pj_ice_strans* ice_st, pj_str_t* loc_ufrag,
                            pj_str_t* loc_pwd, pj_str_t* rem_ufrag,
                            pj_str_t* rem_pwd)
{
    PJ_ASSERT_RETURN(ice_st && ice_st->ice, PJ_EINVALIDOP);

    if (loc_ufrag)
        *loc_ufrag = ice_st->ice->rx_ufrag;
    if (loc_pwd)
        *loc_pwd = ice_st->ice->rx_pass;
    ...
    if (rem_ufrag || rem_pwd) {
        ...
        if (rem_ufrag)
            *rem_ufrag = ice_st->ice->tx_ufrag;
        if (rem_pwd)
            *rem_pwd = ice_st->ice->tx_pass;
    }
 ...
 */
PJ_DEF(unsigned)
pj_ice_strans_get_cands_count(pj_ice_strans* ice_st, unsigned comp_id)
{
    unsigned i, cnt;

    PJ_ASSERT_RETURN(
      ice_st && ice_st->ice && comp_id && comp_id <= ice_st->comp_cnt, 0);
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_enum_cands(pj_ice_strans* ice_st, unsigned comp_id,
                         unsigned* count, pj_ice_sess_cand cand[])
{
    unsigned i, cnt;
    ...
    for (i = 0; i < ice_st->ice->lcand_cnt && cnt < *count; ++i) {
        if (ice_st->ice->lcand[i].comp_id != comp_id)
            continue;
        pj_memcpy(&cand[cnt], &ice_st->ice->lcand[i], sizeof(pj_ice_sess_cand));
        ++cnt;
    }
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_get_def_cand(pj_ice_strans* ice_st, unsigned comp_id,
                           pj_ice_sess_cand* cand)
{
    const pj_ice_sess_check* valid_pair;

    PJ_ASSERT_RETURN(ice_st && comp_id && comp_id <= ice_st->comp_cnt && cand,
                     PJ_EINVAL);
    ...
    for (n = 0; n < ice_st->cfg.turn_tp_cnt; ++n) {
        ...
        for (i = 0; i < comp_cnt; ++i) {
            ...
            if (count && !comp->turn[n].err_cnt && comp->turn[n].sock) {
                status =
                  pj_turn_sock_set_perm(comp->turn[n].sock, count, addrs,
                                        PJ_ICE_ST_USE_TURN_PERMANENT_PERM);
                if (status != PJ_SUCCESS) {
                    pj_ice_strans_stop_ice(ice_st);
                    return status;
                }
...
pj_ice_strans_update_check_list(
  pj_ice_strans* ice_st, const pj_str_t* rem_ufrag, const pj_str_t* rem_passwd,
  unsigned rem_cand_cnt, const pj_ice_sess_cand rem_cand[], pj_bool_t rcand_end)
{
    pj_bool_t checklist_created;
    pj_status_t status;

    PJ_ASSERT_RETURN(
      ice_st && ((rem_cand_cnt == 0) || (rem_ufrag && rem_passwd && rem_cand)),
      PJ_EINVAL);
    PJ_ASSERT_RETURN(ice_st->ice, PJ_EINVALIDOP);
    ...
    if (rem_ufrag && !checklist_created) {
        status = pj_ice_sess_create_check_list(
          ice_st->ice, rem_ufrag, rem_passwd, rem_cand_cnt, rem_cand);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(
              4, (ice_st->obj_name, status, "Failed setting up remote ufrag"));
            pj_grp_lock_release(ice_st->grp_lock);
            return status;
        }
    }
    ...
    if (ice_st->ice->is_trickling) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4,
                      (ice_st->obj_name, status, "Failed updating checklist"));
            pj_grp_lock_release(ice_st->grp_lock);
            return status;
        }
    }
...
pj_ice_strans_get_valid_pair(const pj_ice_strans* ice_st, unsigned comp_id)
{
    PJ_ASSERT_RETURN(ice_st && comp_id && comp_id <= ice_st->comp_cnt, NULL);
...
static pj_status_t use_buffer(pj_ice_strans* ice_st, unsigned comp_id,
                              const void* data, pj_size_t data_len,
                              const pj_sockaddr_t* dst_addr, int dst_addr_len,
                              void** buffer)
{
    unsigned idx;
    pj_status_t status;
 ...
 */
static pj_status_t send_data(pj_ice_strans* ice_st, unsigned comp_id,
                             const void* data, pj_size_t data_len,
                             const pj_sockaddr_t* dst_addr, int dst_addr_len,
                             pj_bool_t use_buf, pj_bool_t call_cb)
{
    pj_ice_strans_comp* comp;
    ...
    if (def_cand->status == PJ_SUCCESS) {
        ...
        if (def_cand->type == PJ_ICE_CAND_TYPE_RELAYED) {
            ...
            status = pj_turn_sock_sendto(
              comp->turn[tp_idx].sock, (const pj_uint8_t*)buf,
            ...
            if (comp->ipv4_mapped) {
                ...
                {
                    status = pj_sockaddr_synthesize(
                      pj_AF_INET6(), &comp->synth_addr, dst_addr);
                    if (status != PJ_SUCCESS)
                        goto on_return;

                    pj_sockaddr_cp(&comp->dst_addr, dst_addr);
                    comp->synth_addr_len =
                      pj_sockaddr_get_len(&comp->synth_addr);
                }
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_sendto(pj_ice_strans* ice_st, unsigned comp_id, const void* data,
                     ...
                     int dst_addr_len)
{
    ...
    PJ_LOG(1, (ice_st->obj_name,
               "pj_ice_strans_sendto() is deprecated. "
               "Application is recommended to use "
               "pj_ice_strans_sendto2() instead."));
    status = send_data(ice_st, comp_id, data, data_len, dst_addr, dst_addr_len,
                       PJ_TRUE, PJ_FALSE);
    if (status == PJ_EPENDING)
        status = PJ_SUCCESS;
                      ...
                      int dst_addr_len)
{
    ...
    return send_data(ice_st, comp_id, data, data_len, dst_addr, dst_addr_len,
                     PJ_TRUE, PJ_FALSE);
}
            ...
            if (check) {
                ...
                PJ_LOG(
                  4, (ice_st->obj_name,
                      ...
                      "%s candidate %s",
                      i + 1, pj_ice_get_cand_type_name(check->lcand->type), lip,
                      pj_ice_get_cand_type_name(check->rcand->type), rip));
...
static void on_ice_complete(pj_ice_sess* ice, pj_status_t status)
{
    ...
    if (cb.on_ice_complete) {
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4, (ice_st->obj_name, status,
                          "ICE negotiation failed after %ds:%03d", msec / 1000,
                          msec % 1000));
        } else {
            unsigned i;
            enum
            ...
            PJ_LOG(4,
                   (ice_st->obj_name, "ICE negotiation success after %ds:%03d",
                    msec / 1000, msec % 1000));
                              ...
                              unsigned dst_addr_len)
{
    ...
    pj_grp_lock_acquire(ice_st->grp_lock);
    if (ice_st->num_buf > 0 &&
        (!ice_st->send_buf || ice_st->send_buf[ice_st->buf_idx].buffer != pkt))
    {
        use_buf = PJ_TRUE;
        status =
          use_buffer(ice_st, comp_id, pkt, size, dst_addr, dst_addr_len, &buf);
        if (status == PJ_EPENDING || status != PJ_SUCCESS) {
            pj_grp_lock_release(ice_st->grp_lock);
            return status;
        }
    ...
    TRACE_PKT((comp->ice_st->obj_name,
               "Component %d TX packet to %s:%d with transport %d", comp_id,
               pj_sockaddr_print(dst_addr, daddr, sizeof(addr), 2),
               pj_sockaddr_get_port(dst_addr), tp_typ));

    if (tp_typ == TP_TURN) {
        if (comp->turn[tp_idx].sock) {
            status = pj_turn_sock_sendto(comp->turn[tp_idx].sock,
                                         (const pj_uint8_t*)buf, (unsigned)size,
                                         dst_addr, dst_addr_len);
        } else {
            status = PJ_EINVALIDOP;
        }
        ...
        status =
          pj_stun_sock_sendto(comp->stun[tp_idx].sock, NULL, buf,
                              (unsigned)size, 0, dest_addr, dest_addr_len);
    } else {
                        ...
                        const pj_sockaddr_t* src_addr, unsigned src_addr_len)
{
    ...
    if (ice_st->cb.on_rx_data) {
        (*ice_st->cb.on_rx_data)(ice_st, comp_id, pkt, size, src_addr,
                                 src_addr_len);
    }
}
...
static void check_pending_send(pj_ice_strans* ice_st)
{
    ...
    if (ice_st->num_buf > 0 && ice_st->buf_idx != ice_st->empty_idx) {
        ...
        pj_grp_lock_release(ice_st->grp_lock);
        send_data(ice_st, ps->comp_id, ps->buffer, ps->data_len, &ps->dst_addr,
                  ps->dst_addr_len, PJ_FALSE, PJ_TRUE);
    } else {
        ice_st->is_pending = PJ_FALSE;
        pj_grp_lock_release(ice_st->grp_lock);
    }
                                 ...
                                 unsigned addr_len)
{
    ...
    if (ice_st->ice == NULL) {
        ...
        if (status != PJ_SUCCESS) {
            ice_st_perror(comp->ice_st, "Error processing packet", status);
        }
    }
    ...
    case PJ_STUN_SOCK_DNS_OP:
        if (status != PJ_SUCCESS) {
            ...
            if (!ice_st->cfg.stun_tp[tp_idx].ignore_stun_error) {
                ...
                PJ_LOG(4, (ice_st->obj_name,
                           "STUN error is ignored for comp %d", comp->comp_id));
            }
        }
        break;
            ...
            if (status == PJ_SUCCESS) {
                ...
                if (dup) {
                    ...
                    if (pj_ice_strans_has_sess(ice_st)) {
                        status = pj_ice_sess_add_cand(
                          ice_st->ice, comp->comp_id, cand->transport_id,
                          cand->type, cand->local_pref, &cand->foundation,
                          &cand->addr, &cand->base_addr, &cand->rel_addr,
                          pj_sockaddr_get_len(&cand->addr), NULL);
                    }
                }
                ...
                {
                    (*ice_st->cb.on_new_candidate)(ice_st, (dup ? NULL : cand),
                                                   init_done);
                }
                ...
                if (op == PJ_STUN_SOCK_MAPPED_ADDR_CHANGE &&
                    ...
                    (*ice_st->cb.on_ice_complete)(
                      ice_st, PJ_ICE_STRANS_OP_ADDR_CHANGE, status);
                }
            }
        }
    ...
    if (comp->ice_st->ice == NULL) {
        ...
        if (status != PJ_SUCCESS) {
            ice_st_perror(comp->ice_st,
                          "Error processing packet from TURN relay", status);
        }
    }
                          ...
                          pj_turn_state_t new_state)
{
    ...
    if (new_state == PJ_TURN_STATE_READY) {
        ...
        pj_ice_calc_foundation(comp->ice_st->pool, &cand->foundation,
                               PJ_ICE_CAND_TYPE_RELAYED, &rel_info.relay_addr);
        cand->status = PJ_SUCCESS;
        ...
        */
            ...
            pj_ice_strans_has_sess(comp->ice_st))
        {
            ...
            /* Add the candidate */
            status = pj_ice_sess_add_cand(
              comp->ice_st->ice, comp->comp_id, cand->transport_id, cand->type,
              cand->local_pref, &cand->foundation, &cand->addr,
              &cand->base_addr, &cand->rel_addr,
              pj_sockaddr_get_len(&cand->addr), NULL);
            if (status != PJ_SUCCESS) {
            ...
            /* Gather remote addresses for this component */
            for (j = 0; j < sess->rcand_cnt && count < PJ_ARRAY_SIZE(addrs);
                 ++j) {
