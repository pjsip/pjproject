...
static void turn_on_channel_bound(pj_turn_session* sess,
                                  ...
                                  unsigned addr_len, unsigned ch_num);
static void turn_on_rx_data(pj_turn_session* sess, void* pkt, unsigned pkt_len,
                            const pj_sockaddr_t* peer_addr, unsigned addr_len);
static void turn_on_state(pj_turn_session* sess, pj_turn_state_t old_state,
...
static void turn_on_connection_bind_status(pj_turn_session* sess,
                                           ...
                                           const pj_sockaddr_t* peer_addr,
                                           unsigned addr_len);
static void turn_on_connect_complete(pj_turn_session* sess, pj_status_t status,
                                     pj_uint32_t conn_id,
                                     const pj_sockaddr_t* peer_addr,
                                     unsigned addr_len);

static pj_bool_t on_data_read(pj_turn_sock* turn_sock, void* data,
...
static pj_bool_t on_connect_complete_asock(pj_activesock_t* asock,
                                           pj_status_t status);
static pj_bool_t on_data_read_asock(pj_activesock_t* asock, void* data,
                                    pj_size_t size, pj_status_t status,
                                    pj_size_t* remainder);
...
static pj_bool_t on_connect_complete_ssl_sock(pj_ssl_sock_t* ssl_sock,
                                              pj_status_t status);
static pj_bool_t on_data_read_ssl_sock(pj_ssl_sock_t* ssl_sock, void* data,
                                       pj_size_t size, pj_status_t status,
                                       pj_size_t* remainder);
#endif

static pj_bool_t dataconn_on_data_read(pj_activesock_t* asock, void* data,
                                       pj_size_t size, pj_status_t status,
                                       pj_size_t* remainder);
                    ...
                    void* user_data, pj_turn_sock** p_turn_sock)
{
    ...
    status = pj_turn_session_create(cfg, pool->obj_name, af, conn_type,
                                    turn_sock->grp_lock, &sess_cb, 0, turn_sock,
                                    &turn_sock->sess);
    if (status != PJ_SUCCESS) {
        destroy(turn_sock);
        return status;
 ...
 */
PJ_DEF(void) pj_turn_sock_set_log(pj_turn_sock* turn_sock, unsigned flags)
{
                   ...
                   const pj_turn_alloc_param* param)
{
     ...
     */
    if (param) {
        pj_turn_alloc_param_copy(turn_sock->pool, &turn_sock->alloc_param,
                                 param);
    } else {
    ...
    if (status != PJ_SUCCESS) {
        ...
        PJ_LOG(4, (turn_sock->obj_name,
                   "TURN session destroyed in setting "
                   "TURN server"));
        pj_grp_lock_release(turn_sock->grp_lock);
        return PJ_EGONE;
    }
                    ...
                    unsigned addr_len)
{
    ...
    return pj_turn_session_sendto(turn_sock->sess, pkt, pkt_len, addr,
                                  addr_len);
}
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_sock_connect(pj_turn_sock* turn_sock, const pj_sockaddr_t* peer_addr,
                     unsigned addr_len)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_sock_disconnect(pj_turn_sock* turn_sock, const pj_sockaddr_t* peer_addr,
                        unsigned addr_len)
                                     ...
                                     pj_status_t status)
{
    ...
    if (turn_sock->conn_type != PJ_TURN_TP_TLS)
        status =
          pj_activesock_start_read(turn_sock->active_sock, turn_sock->pool,
 ...
 */
static unsigned has_packet(pj_turn_sock* turn_sock, const void* buf,
                           pj_size_t bufsize)
{
    pj_bool_t is_stun;
...
static pj_status_t send_pkt(pj_turn_session* sess, pj_bool_t internal,
                            const pj_uint8_t* pkt, unsigned pkt_len,
                            const pj_sockaddr_t* dst_addr,
                            unsigned dst_addr_len)
{
    pj_turn_sock* turn_sock =
      (pj_turn_sock*)pj_turn_session_get_user_data(sess);
    ...
    if (turn_sock->conn_type == PJ_TURN_TP_UDP) {
        status = pj_activesock_sendto(turn_sock->active_sock, send_key, pkt,
                                      &len, 0, dst_addr, dst_addr_len);
        ...
        if (pj_sockaddr_cmp(&info.server, dst_addr) == 0) {
            ...
            status = pj_activesock_send(turn_sock->active_sock, send_key, pkt,
                                        &len, 0);
        } else {
            ...
            for (i = 0; i < PJ_TURN_MAX_TCP_CONN_CNT; ++i) {
                ...
                if (pj_sockaddr_cmp(&conn->peer_addr, dst_addr) == 0) {
                    status = pj_activesock_send(conn->asock, &conn->send_key,
                                                pkt, &len, 0);
                    break;
                }
            }
        ...
        status =
          pj_activesock_send(turn_sock->active_sock, send_key, pkt, &len, 0);
    }
#if PJ_HAS_SSL_SOCK
    else if (turn_sock->conn_type == PJ_TURN_TP_TLS)
    {
        status = pj_ssl_sock_send(turn_sock->ssl_sock, send_key, pkt, &len, 0);
    }
#endif
    else
    {
...
static pj_status_t turn_on_send_pkt(pj_turn_session* sess,
                                    ...
                                    unsigned dst_addr_len)
{
    return send_pkt(sess, PJ_FALSE, pkt, pkt_len, dst_addr, dst_addr_len);
}
...
static pj_status_t turn_on_stun_send_pkt(pj_turn_session* sess,
                                         ...
                                         unsigned dst_addr_len)
{
    return send_pkt(sess, PJ_TRUE, pkt, pkt_len, dst_addr, dst_addr_len);
}
                            ...
                            const pj_sockaddr_t* peer_addr, unsigned addr_len)
{
    ...
    if (turn_sock->cb.on_rx_data) {
        (*turn_sock->cb.on_rx_data)(turn_sock, pkt, pkt_len, peer_addr,
                                    addr_len);
    }
}
                          ...
                          pj_turn_state_t new_state)
{
    ...
    if (turn_sock->sess && new_state == PJ_TURN_STATE_RESOLVED) {
        ...
        if (turn_sock->conn_type != PJ_TURN_TP_TLS) {
            ...
            /* Bind socket */
            status = pj_sock_bind_random(
              sock, &bound_addr, turn_sock->setting.port_range, max_bind_retry);
            if (status != PJ_SUCCESS) {
                pj_sock_close(sock);
                pj_turn_sock_destroy(turn_sock);
                return;
            ...
            status = pj_sock_apply_qos2(
              sock, turn_sock->setting.qos_type, &turn_sock->setting.qos_params,
                    ...
                    if (sobuf_size < turn_sock->setting.so_rcvbuf_size) {
                        PJ_LOG(4,
                               (turn_sock->obj_name,
                                ...
                                " now=%d, configured=%d",
                                sobuf_size, turn_sock->setting.so_rcvbuf_size));
                    } else {
                    ...
                    if (sobuf_size < turn_sock->setting.so_sndbuf_size) {
                        PJ_LOG(4,
                               (turn_sock->obj_name,
                                ...
                                " now=%d, configured=%d",
                                sobuf_size, turn_sock->setting.so_sndbuf_size));
                    } else {
            ...
            status =
              pj_activesock_create(turn_sock->pool, sock, sock_type, &asock_cfg,
                                   turn_sock->cfg.ioqueue, &asock_cb, turn_sock,
                                   &turn_sock->active_sock);
            if (status != PJ_SUCCESS)
                pj_sock_close(sock);
        }
        ...
        else
        {
                       ...
                       turn_sock->setting.tls_cfg.privkey_buf.slen)
            {
                status = pj_ssl_cert_load_from_buffer(
                  turn_sock->pool, &turn_sock->setting.tls_cfg.ca_buf,
            ...
            status =
              pj_ssl_sock_create(turn_sock->pool, &param, &turn_sock->ssl_sock);
            ...
            if (turn_sock->cert) {
                status = pj_ssl_sock_set_certificate(
                  turn_sock->ssl_sock, turn_sock->pool, turn_sock->cert);
        ...
        PJ_LOG(5,
               (turn_sock->pool->obj_name, "Connecting to %s",
                pj_sockaddr_print(&info.server, addrtxt, sizeof(addrtxt), 3)));
        ...
        else if (turn_sock->conn_type == PJ_TURN_TP_TCP)
        {
            status = pj_activesock_start_connect(
              turn_sock->active_sock, turn_sock->pool, &info.server,
        ...
        else if (turn_sock->conn_type == PJ_TURN_TP_TLS)
        {
            ...
            status =
              pj_ssl_sock_start_connect2(turn_sock->ssl_sock, &connect_param);
        }
#endif
        if (status == PJ_SUCCESS) {
            ...
            PJ_PERROR(
              3,
              (turn_sock->pool->obj_name, status, "Failed to connect to %s",
               pj_sockaddr_print(&info.server, addrtxt, sizeof(addrtxt), 3)));
            pj_turn_sock_destroy(turn_sock);
            return;
        }
    ...
    if (new_state >= PJ_TURN_STATE_DESTROYING && turn_sock->sess) {
        ...
        pj_timer_heap_schedule_w_grp_lock(turn_sock->cfg.timer_heap,
                                          &turn_sock->timer, &delay,
                                          TIMER_DESTROY, turn_sock->grp_lock);
    }
}
...
static pj_bool_t dataconn_on_data_read(pj_activesock_t* asock, void* data,
                                       ...
                                       pj_size_t* remainder)
{
    tcp_data_conn_t* conn =
      (tcp_data_conn_t*)pj_activesock_get_user_data(asock);
    ...
    *remainder = size;
    while (*remainder > 0) {
        if (conn->state == DATACONN_STATE_READY) {
            /* Application data */
            if (turn_sock->cb.on_rx_data) {
                (*turn_sock->cb.on_rx_data)(
                  turn_sock, data, (unsigned)*remainder, &conn->peer_addr,
                  conn->peer_addr_len);
            }
            *remainder = 0;
                                              ...
                                              pj_status_t status)
{
    ...
    if (status == PJ_SUCCESS) {
        ...
        status = pj_turn_session_connection_bind(turn_sock->sess, conn->pool,
                                       ...
                                       unsigned addr_len)
{
    ...
    if (turn_sock->setting.so_rcvbuf_size > 0) {
        unsigned sobuf_size = turn_sock->setting.so_rcvbuf_size;
        status =
          pj_sock_setsockopt_sobuf(sock, pj_SO_RCVBUF(), PJ_TRUE, &sobuf_size);
        if (status != PJ_SUCCESS) {
            pj_perror(3, turn_sock->obj_name, status,
                      "Failed setting SO_RCVBUF");
        } else {
            if (sobuf_size < turn_sock->setting.so_rcvbuf_size) {
                ...
                PJ_LOG(
                  5, (turn_sock->obj_name, "SO_RCVBUF set to %d", sobuf_size));
            }
        }
    }
    if (turn_sock->setting.so_sndbuf_size > 0) {
        unsigned sobuf_size = turn_sock->setting.so_sndbuf_size;
        status =
          pj_sock_setsockopt_sobuf(sock, pj_SO_SNDBUF(), PJ_TRUE, &sobuf_size);
        if (status != PJ_SUCCESS) {
            pj_perror(3, turn_sock->obj_name, status,
                      "Failed setting SO_SNDBUF");
        } else {
            if (sobuf_size < turn_sock->setting.so_sndbuf_size) {
                ...
                PJ_LOG(
                  5, (turn_sock->obj_name, "SO_SNDBUF set to %d", sobuf_size));
            }
        }
    }
    ...
    status = pj_activesock_create(pool, sock, pj_SOCK_STREAM(), &asock_cfg,
                                  turn_sock->cfg.ioqueue, &asock_cb, new_conn,
                                  &new_conn->asock);
    if (status != PJ_SUCCESS)
        goto on_return;
    ...
    pj_turn_session_get_info(turn_sock->sess, &info);
    status = pj_activesock_start_connect(new_conn->asock, pool, &info.server,
                                         pj_sockaddr_get_len(&info.server));
    if (status == PJ_SUCCESS) {
...
on_return:
    if (status == PJ_EPENDING) {
        PJ_LOG(5, (pool->obj_name, "Accepting connection from peer %s",
                   pj_sockaddr_print(peer_addr, addrtxt, sizeof(addrtxt), 3)));
    } else {
        /* not PJ_SUCCESS */
                                           ...
                                           unsigned addr_len)
{
    ...
    for (i = 0; i < PJ_TURN_MAX_TCP_CONN_CNT; ++i) {
        ...
        if (c->id == conn_id && pj_sockaddr_cmp(peer_addr, &c->peer_addr) == 0)
        {
            conn = c;
            break;
        }
...
on_return:
    if (status == PJ_EPENDING) {
        PJ_LOG(5, (pool->obj_name, "Connecting to peer %s",
                   pj_sockaddr_print(peer_addr, addrtxt, sizeof(addrtxt), 3)));
    } else {
        /* not PJ_SUCCESS */
        pj_perror(4, pool->obj_name, status, "Failed in connect to peer %s",
                  pj_sockaddr_print(peer_addr, addrtxt, sizeof(addrtxt), 3));
