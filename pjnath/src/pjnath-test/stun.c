    ...
    for (i = 0; i < PJ_ARRAY_SIZE(tests); ++i) {
        ...
        if (t->pdu) {
            status = pj_stun_msg_decode(
              pool, (pj_uint8_t*)t->pdu, t->pdu_len,
              PJ_STUN_IS_DATAGRAM | PJ_STUN_CHECK_PACKET, &msg, NULL, NULL);
        ...
        pj_stun_create_key(pool, &key, NULL, &USERNAME, PJ_STUN_PASSWD_PLAIN,
                           &PASSWORD);
        ...
        status = pj_stun_msg_decode(pool, buf, len,
                                    ...
                                    &msg2, NULL, NULL);
        if (status != PJ_SUCCESS) {
            PJ_LOG(1, (THIS_FILE, "    subsequent decoding failed: %s",
                       err(status)));
            rc = -50;
            goto on_return;
        }
...
static int verify1(pj_stun_msg* msg)
{
    ...
    aunk = (pj_stun_unknown_attr*)pj_stun_msg_find_attr(
      msg, PJ_STUN_ATTR_UNKNOWN_ATTRIBUTES, 0);
    if (aunk == NULL) {
...
static struct test_vector
{
    ...
    { PJ_STUN_BINDING_REQUEST,
      ...
      88, USE_MESSAGE_INTEGRITY | USE_FINGERPRINT, "evtj:h6vY",
      "VOkJxbRl1RmTxUk/WvJxBt", "", "", &create_msgint1 }
...
static char* print_binary(const pj_uint8_t* data, unsigned data_len)
{
    ...
    for (i = 0; i < data_len;) {
        ...
        pj_ansi_snprintf(p, 1500 - (p - buf), "%04d-%04d   ", i,
                         (i + 20 < data_len) ? i + 20 : data_len);
        p += 12;
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(test_vectors); ++i) {
        ...
        if (v->options & USE_MESSAGE_INTEGRITY) {
            ...
            pj_stun_create_key(pool, &key, pj_cstr(&r, v->realm),
                               pj_cstr(&s1, v->username), PJ_STUN_PASSWD_PLAIN,
        ...
        if (v->options & USE_MESSAGE_INTEGRITY) {
            if (PJ_STUN_IS_REQUEST(msg->hdr.type)) {
                ...
                if (status2 != PJ_SUCCESS) {
                    ...
                    PJ_LOG(1,
                           (THIS_FILE, "    Request authentication failed: %s",
                            errmsg));
                    rc = -1070;
                    goto on_return;
                }
                ...
                pj_status_t status2;
                status2 =
                  pj_stun_authenticate_response(buf, (unsigned)len, msg, &key);
                if (status2 != PJ_SUCCESS) {
                    char errmsg[PJ_ERR_MSG_SIZE];
                    pj_strerror(status2, errmsg, sizeof(errmsg));
                    PJ_LOG(1,
                           (THIS_FILE, "    Response authentication failed: %s",
                            errmsg));
                    rc = -1080;
                    goto on_return;
                }
...
static pj_stun_msg* create_msgint1(pj_pool_t* pool, test_vector* v)
{
    ...
    status =
      pj_stun_msg_add_uint64_attr(pool, msg, PJ_STUN_ATTR_ICE_CONTROLLED, &u64);
    if (status != PJ_SUCCESS)
        goto on_error;
...
static pj_stun_msg* create_msgint2(pj_pool_t* pool, test_vector* v)
{
    ...
    status = pj_stun_msg_add_sockaddr_attr(
      pool, msg, PJ_STUN_ATTR_XOR_MAPPED_ADDR, PJ_TRUE, &mapped_addr,
...
static pj_stun_msg* create_msgint3(pj_pool_t* pool, test_vector* v)
{
    ...
    status = pj_sockaddr_init(
      pj_AF_INET6(), &mapped_addr,
      pj_cstr(&s1, "2001:db8:1234:5678:11:2233:4455:6677"), 32853);
    if (status != PJ_SUCCESS)
        goto on_error;

    status =
      pj_stun_msg_add_sockaddr_attr(pool, msg, PJ_STUN_ATTR_XOR_MAPPED_ADDR,
                                    PJ_TRUE, &mapped_addr, sizeof(pj_sockaddr));
    if (status != PJ_SUCCESS)
        goto on_error;
...
static int handle_unknown_non_mandatory(void)
{
    ...
    rc = pj_stun_msg_create(pool, PJ_STUN_BINDING_REQUEST, PJ_STUN_MAGIC, NULL,
                            &msg0);
    rc +=
      pj_stun_msg_add_string_attr(pool, msg0, PJ_STUN_ATTR_USERNAME, &USERNAME);
    ...
    rc += pj_stun_msg_decode(pool, packet, len,
                             PJ_STUN_IS_DATAGRAM | PJ_STUN_CHECK_PACKET, &msg1,
                             NULL, NULL);
