...
#define RETURN_ERROR(rc) \
    { \
        app_perror("", rc); \
        return rc; \
    }

static pj_bool_t stun_on_data_recvfrom(pj_activesock_t* asock, void* data,
                                       pj_size_t size,
                                       const pj_sockaddr_t* src_addr,
                                       int addr_len, pj_status_t status);
static pj_bool_t turn_tcp_on_data_read(pj_activesock_t* asock, void* data,
                                       pj_size_t size, pj_status_t status,
                                       pj_size_t* remainder);
#if USE_TLS
static pj_bool_t turn_tls_on_data_read(pj_ssl_sock_t* ssock, void* data,
                                       pj_size_t size, pj_status_t status,
                                       pj_size_t* remainder);
#endif
static pj_bool_t turn_udp_on_data_recvfrom(pj_activesock_t* asock, void* data,
                                           pj_size_t size,
                                           const pj_sockaddr_t* src_addr,
                                           int addr_len, pj_status_t status);
static pj_bool_t turn_on_data_read(test_server* asock, void* data,
                                   pj_size_t size,
                                   const pj_sockaddr_t* src_addr, int addr_len,
                                   pj_status_t status);
...
static pj_bool_t turn_tls_on_accept_complete2(pj_ssl_sock_t* ssock,
                                              ...
                                              pj_status_t status);
#endif
static pj_bool_t alloc_on_data_recvfrom(pj_activesock_t* asock, void* data,
                                        pj_size_t size,
                                        const pj_sockaddr_t* src_addr,
                                        int addr_len, pj_status_t status);
                               ...
                               const char* domain, test_server** p_test_srv)
{
    ...
    if (flags & CREATE_DNS_SERVER) {
        status = pj_dns_server_create(mem, test_srv->stun_cfg->ioqueue,
                                      GET_AF(use_ipv6), DNS_SERVER_PORT, 0,
                                      &test_srv->dns_server);
        if (status != PJ_SUCCESS) {
    ...
    if (flags & CREATE_STUN_SERVER) {
        ...
        pj_sockaddr_init(GET_AF(use_ipv6), &bound_addr, NULL, STUN_SERVER_PORT);

        status = pj_activesock_create_udp(
          pool, &bound_addr, NULL, test_srv->stun_cfg->ioqueue, &stun_sock_cb,
          test_srv, &test_srv->stun_sock, NULL);
        if (status != PJ_SUCCESS) {
            destroy_test_server(test_srv);
            RETURN_ERROR(status);
        }
        ...
        if (test_srv->dns_server && (flags & CREATE_STUN_SERVER_DNS_SRV)) {
             ...
             */
            pj_ansi_snprintf(strbuf, sizeof(strbuf), "_stun._udp.%s", domain);
            pj_strdup2(pool, &res_name, strbuf);
            pj_ansi_snprintf(strbuf, sizeof(strbuf), "stun.%s", domain);
    ...
    if (flags & CREATE_TURN_SERVER) {
        ...
        if (tp_type == PJ_TURN_TP_UDP) {
            ...
            status = pj_activesock_create_udp(
              pool, &bound_addr, NULL, test_srv->stun_cfg->ioqueue,
              &turn_sock_cb, test_srv, &test_srv->turn_sock, NULL);
            ...
            status =
              pj_sock_socket(GET_AF(use_ipv6), pj_SOCK_STREAM(), 0, &sock_fd);
            if (status != PJ_SUCCESS) {
                RETURN_ERROR(status);
            }
            ...
            status =
              pj_activesock_create(pool, sock_fd, pj_SOCK_STREAM(), NULL,
                                   test_srv->stun_cfg->ioqueue, &turn_sock_cb,
                                   test_srv, &test_srv->turn_sock);
            if (status != PJ_SUCCESS) {
                pj_sock_close(sock_fd);
                RETURN_ERROR(status);
            }

            status = pj_activesock_start_accept(test_srv->turn_sock, pool);
        }
#if USE_TLS
        else if (tp_type == PJ_TURN_TP_TLS)
        {
            ...
            status = pj_ssl_cert_load_from_files(
              pool, &ca_file, &cert_file, &privkey_file, &privkey_pass, &cert);
            if (status != PJ_SUCCESS) {
                if (ssock_serv)
                    pj_ssl_sock_close(ssock_serv);
            }
        ...
        if (test_srv->dns_server && (flags & CREATE_TURN_SERVER_DNS_SRV)) {
            ...
            switch (tp_type) {
            case PJ_TURN_TP_TCP:
                pj_ansi_snprintf(strbuf, sizeof(strbuf), "_turn._tcp.%s",
                                 domain);
                break;
            case PJ_TURN_TP_TLS:
                pj_ansi_snprintf(strbuf, sizeof(strbuf), "_turns._tcp.%s",
                                 domain);
                break;
            default:
                pj_ansi_snprintf(strbuf, sizeof(strbuf), "_turn._udp.%s",
                                 domain);
            }
            pj_strdup2(pool, &res_name, strbuf);
            pj_ansi_snprintf(strbuf, sizeof(strbuf), "turn.%s", domain);
            pj_strdup2(pool, &target, strbuf);
            pj_dns_init_srv_rr(&rr, &res_name, PJ_DNS_CLASS_IN, 60, 0, 0,
                                       ...
                                       int addr_len, pj_status_t status)
{
    ...
    if (req->hdr.type != PJ_STUN_BINDING_REQUEST) {
        pj_stun_msg_create_response(pool, req, PJ_STUN_SC_BAD_REQUEST, NULL,
                                    &resp);
        goto send_pkt;
    }
...
send_pkt:
    status =
      pj_stun_msg_encode(resp, (pj_uint8_t*)data, MAX_STUN_PKT, 0, NULL, &size);
    if (status != PJ_SUCCESS)
        goto on_return;

    len = size;
    status = pj_activesock_sendto(asock, &test_srv->send_key, data, &len, 0,
                                  src_addr, addr_len);
...
static pj_stun_msg* create_success_response(test_server* test_srv,
                                            ...
                                            pj_stun_msg* req, pj_pool_t* pool,
                                            unsigned lifetime,
                                            pj_str_t* auth_key)
{
    pj_stun_msg* resp;
    ...
    /* Add TURN_NONCE */
    pj_stun_msg_add_string_attr(pool, resp, PJ_STUN_ATTR_NONCE,
                                pj_cstr(&tmp, TURN_NONCE));
    /* Add LIFETIME */
    pj_stun_msg_add_uint_attr(pool, resp, PJ_STUN_ATTR_LIFETIME, lifetime);
    if (lifetime != 0) {
        /* Add XOR-RELAYED-ADDRESS */
        pj_stun_msg_add_sockaddr_attr(pool, resp, PJ_STUN_ATTR_XOR_RELAYED_ADDR,
                                      PJ_TRUE, &alloc->alloc_addr,
                                      pj_sockaddr_get_len(&alloc->alloc_addr));
        /* Add XOR-MAPPED-ADDRESS */
        pj_stun_msg_add_sockaddr_attr(pool, resp, PJ_STUN_ATTR_XOR_MAPPED_ADDR,
                                      PJ_TRUE, &alloc->client_addr,
...
#if USE_TLS
static pj_bool_t turn_tls_on_data_read(pj_ssl_sock_t* ssl_sock, void* data,
                                       pj_size_t size, pj_status_t status,
                                       pj_size_t* remainder)
{
...
static pj_bool_t turn_on_data_read(test_server* test_srv, void* data,
                                   pj_size_t size,
                                   const pj_sockaddr_t* src_addr, int addr_len,
                                   pj_status_t status)
{
    ...
    /* Find the client */
    for (i = 0; i < test_srv->turn_alloc_cnt; i++) {
        if (pj_sockaddr_cmp(&test_srv->turn_alloc[i].client_addr, src_addr) ==
            0)
            break;
    }
                          ...
                          PJ_STUN_NO_FINGERPRINT_CHECK) != PJ_SUCCESS)
    {
        ...
        if (i == test_srv->turn_alloc_cnt) {
            /* Invalid data */
            PJ_LOG(1, (THIS_FILE, "TURN Server received strayed data"));
            goto on_return;
        }
        ...
        PJ_LOG(5,
               (THIS_FILE, "Relaying %d bytes data from client %s to peer %s",
                cd.length, client_info, peer_info));
        sent = cd.length;
        pj_activesock_sendto(alloc->sock, &alloc->send_key, pcd + 1, &sent, 0,
                             &alloc->perm[j],
    ...
    status = pj_stun_msg_decode(
      pool, (pj_uint8_t*)data, size,
      PJ_STUN_IS_DATAGRAM | PJ_STUN_CHECK_PACKET | PJ_STUN_NO_FINGERPRINT_CHECK,
      &req, NULL, NULL);
    if (status != PJ_SUCCESS) {
        char errmsg[PJ_ERR_MSG_SIZE];
        pj_strerror(status, errmsg, sizeof(errmsg));
        PJ_LOG(1, ("", "STUN message decode error from client %s: %s",
                   client_info, errmsg));
        goto on_return;
    }
    ...
    if (i == test_srv->turn_alloc_cnt) {
        ...
        if (req->hdr.type != PJ_STUN_ALLOCATE_REQUEST) {
            PJ_LOG(1, (THIS_FILE, "Invalid %s %s from client %s",
                       ...
                       pj_stun_get_class_name(req->hdr.type), client_info));
        ...
        if (test_srv->turn_alloc_cnt == MAX_TURN_ALLOC) {
            pj_stun_msg_create_response(
              pool, req, PJ_STUN_SC_INSUFFICIENT_CAPACITY, NULL, &resp);
            goto send_pkt;
        }

        /* Get USERNAME attribute */
        uname = (pj_stun_username_attr*)pj_stun_msg_find_attr(
          req, PJ_STUN_ATTR_USERNAME, 0);

        /* Reject if it doesn't have MESSAGE-INTEGRITY or USERNAME attributes or
         * the user is incorrect
         */
        if (pj_stun_msg_find_attr(req, PJ_STUN_ATTR_MESSAGE_INTEGRITY, 0) ==
              NULL ||
            uname == NULL || pj_stricmp2(&uname->value, TURN_USERNAME) != 0)
        {
            pj_str_t tmp;

            pj_stun_msg_create_response(pool, req, PJ_STUN_SC_UNAUTHORIZED,
                                        NULL, &resp);
            pj_stun_msg_add_string_attr(pool, resp, PJ_STUN_ATTR_REALM,
                                        &test_srv->domain);
            pj_stun_msg_add_string_attr(pool, resp, PJ_STUN_ATTR_NONCE,
                                        pj_cstr(&tmp, TURN_NONCE));
            goto send_pkt;
        ...
        alloc->pool =
          pj_pool_create(test_srv->stun_cfg->pf, "alloc", 512, 512, NULL);
        ...
        status = pj_activesock_create_udp(
          alloc->pool, &alloc->alloc_addr, NULL, test_srv->stun_cfg->ioqueue,
          &alloc_sock_cb, alloc, &alloc->sock, &alloc->alloc_addr);
        if (status != PJ_SUCCESS) {
            pj_pool_release(alloc->pool);
            pj_stun_msg_create_response(pool, req, PJ_STUN_SC_SERVER_ERROR,
                                        NULL, &resp);
        ...
        status =
          pj_activesock_start_recvfrom(alloc->sock, alloc->pool, 1500, 0);
        if (status != PJ_SUCCESS) {
        ...
        pj_stun_msg_add_sockaddr_attr(
          alloc->pool, alloc->data_ind, PJ_STUN_ATTR_XOR_PEER_ADDR, PJ_TRUE,
          &alloc->alloc_addr, pj_sockaddr_get_len(&alloc->alloc_addr));
        pj_stun_msg_add_binary_attr(alloc->pool, alloc->data_ind,
                                    PJ_STUN_ATTR_DATA, (pj_uint8_t*)"", 1);

        /* Create response */
        resp =
          create_success_response(test_srv, alloc, req, pool, 600, &auth_key);
        if (resp == NULL) {
            pj_activesock_close(alloc->sock);
            pj_pool_release(alloc->pool);
            pj_stun_msg_create_response(pool, req, PJ_STUN_SC_SERVER_ERROR,
        ...
        if (req->hdr.type == PJ_STUN_ALLOCATE_REQUEST) {
            ...
            resp =
              create_success_response(test_srv, alloc, req, pool, 0, &auth_key);
            ...
            lf_attr = (pj_stun_lifetime_attr*)pj_stun_msg_find_attr(
              req, PJ_STUN_ATTR_LIFETIME, 0);
            if (lf_attr && lf_attr->value != 0) {
                resp = create_success_response(test_srv, alloc, req, pool, 600,
                                               &auth_key);
                pj_array_erase(test_srv->turn_alloc,
                               sizeof(test_srv->turn_alloc[0]),
                               test_srv->turn_alloc_cnt, i);
                --test_srv->turn_alloc_cnt;
            } else
                resp = create_success_response(test_srv, alloc, req, pool, 0,
                                               &auth_key);
            ...
            for (i = 0; i < req->attr_count; ++i) {
                if (req->attr[i]->type == PJ_STUN_ATTR_XOR_PEER_ADDR) {
                    pj_stun_xor_peer_addr_attr* pa =
                      (pj_stun_xor_peer_addr_attr*)req->attr[i];
                    unsigned j;

                    for (j = 0; j < alloc->perm_cnt; ++j) {
                        if (pj_sockaddr_cmp(&alloc->perm[j], &pa->sockaddr) ==
                            0)
                            break;
                    }
                    ...
                    {
                        char peer_info[PJ_INET6_ADDRSTRLEN];
                        pj_sockaddr_print(&pa->sockaddr, peer_info,
                                          sizeof(peer_info), 3);

                        pj_sockaddr_cp(&alloc->perm[alloc->perm_cnt],
                                       &pa->sockaddr);
                        ++alloc->perm_cnt;

                        PJ_LOG(
                          5,
                          ("", "Permission %s added to client %s, perm_cnt=%d",
            ...
            pa = (pj_stun_xor_peer_addr_attr*)pj_stun_msg_find_attr(
              req, PJ_STUN_ATTR_XOR_PEER_ADDR, 0);
            da = (pj_stun_data_attr*)pj_stun_msg_find_attr(
              req, PJ_STUN_ATTR_DATA, 0);
            if (pa && da) {
                unsigned j;
                char peer_info[PJ_INET6_ADDRSTRLEN];
                pj_ssize_t sent;

                pj_sockaddr_print(&pa->sockaddr, peer_info, sizeof(peer_info),
                                  3);
                ...
                if (j == alloc->perm_cnt) {
                    PJ_LOG(
                      5,
                      ("", "SendIndication to %s is rejected (no permission)",
                       peer_info, client_info, alloc->perm_cnt));
                } else {
                    PJ_LOG(
                      5, (THIS_FILE,
                          "Relaying %d bytes data from client %s to peer %s, "
                          "perm_cnt=%d",
                          da->length, client_info, peer_info, alloc->perm_cnt));

                    sent = da->length;
                    pj_activesock_sendto(alloc->sock, &alloc->send_key,
                                         da->data, &sent, 0, &pa->sockaddr,
                                         pj_sockaddr_get_len(&pa->sockaddr));
                }
            } else {
                PJ_LOG(1, (THIS_FILE, "Invalid Send Indication from %s",
                           client_info));
            }
            ...
            pa = (pj_stun_xor_peer_addr_attr*)pj_stun_msg_find_attr(
              req, PJ_STUN_ATTR_XOR_PEER_ADDR, 0);
            cna = (pj_stun_channel_number_attr*)pj_stun_msg_find_attr(
              req, PJ_STUN_ATTR_CHANNEL_NUMBER, 0);
...
send_pkt:
    if (resp) {
        ...
        status = pj_stun_msg_encode(resp, (pj_uint8_t*)data, MAX_STUN_PKT, 0,
                                    &auth_key, &size);
        if (status != PJ_SUCCESS)
            goto on_return;
        ...
        default:
            status =
              pj_activesock_sendto(test_srv->turn_sock, &test_srv->send_key,
                                   data, &len, 0, src_addr, addr_len);
        }
    }
                                             ...
                                             pj_status_t status)
{
    ...
    sstatus = pj_activesock_create(test_srv->pool, newsock, pj_SOCK_STREAM(),
                                   NULL, test_srv->stun_cfg->ioqueue, &asock_cb,
                                   test_srv, &test_srv->cl_turn_sock);
    if (sstatus != PJ_SUCCESS) {
        goto on_exit;
    }

    sstatus = pj_activesock_start_read(test_srv->cl_turn_sock, test_srv->pool,
                                       MAX_STUN_PKT, 0);
    if (sstatus != PJ_SUCCESS) {
        goto on_exit;
    }
                                        ...
                                        int addr_len, pj_status_t status)
{
    ...
    if (i == alloc->perm_cnt) {
        PJ_LOG(
          5, ("", "Client %s received %d bytes unauthorized data from peer %s",
    ...
    pa = (pj_stun_xor_peer_addr_attr*)pj_stun_msg_find_attr(
      alloc->data_ind, PJ_STUN_ATTR_XOR_PEER_ADDR, 0);
    da = (pj_stun_data_attr*)pj_stun_msg_find_attr(alloc->data_ind,
                                                   PJ_STUN_ATTR_DATA, 0);
    pj_assert(pa && da);
    ...
    /* Send */
    sent = size;
    PJ_LOG(5, ("", "Forwarding %d bytes data from peer %s to client %s", sent,
