                               ...
                               struct test_session** p_sess)
{
    ...
    status = pj_turn_sock_create(sess->stun_cfg, GET_AF(use_ipv6), tp_type,
    ...
    status =
      pj_dns_resolver_create(mem, "resolver", 0, sess->stun_cfg->timer_heap,
    ...
    if (status != PJ_SUCCESS) {
        ...
        status =
          pj_dns_resolver_set_ns(sess->resolver, 1, &dns_srv, &dns_srv_port);
...
static void set_server_flag(struct test_session_cfg* test_cfg,
                            pj_bool_t use_ipv6, pj_turn_tp_type tp_type)
{
    pj_uint32_t flag = TURN_UDP;
    test_cfg->srv.flags &=
      ~(SERVER_IPV4 + SERVER_IPV6 + TURN_UDP + TURN_TCP + TURN_TLS);
    switch (tp_type) {
    case PJ_TURN_TP_TCP:
        flag = TURN_TCP;
        break;
...
static int state_progression_test(pj_stun_config* stun_cfg, pj_bool_t use_ipv6,
                                  pj_turn_tp_type tp_type)
{
    ...
    for (i = 0; i <= 1; ++i) {
        ...
        PJ_LOG(3,
               ("", "   %s DNS SRV resolution", (i == 0 ? "without" : "with")));
        ...
        if ((result.state_called & (1 << PJ_TURN_STATE_ALLOCATING)) == 0) {
            PJ_LOG(3,
                   ("", "    error: PJ_TURN_STATE_ALLOCATING is not called"));
            return -155;
        }
        ...
        if ((result.state_called & (1 << PJ_TURN_STATE_DEALLOCATING)) == 0) {
            PJ_LOG(3,
                   ("", "    error: PJ_TURN_STATE_DEALLOCATING is not called"));
            return -170;
        }

        if ((result.state_called & (1 << PJ_TURN_STATE_DEALLOCATED)) == 0) {
            PJ_LOG(3,
                   ("", "    error: PJ_TURN_STATE_DEALLOCATED is not called"));
            return -180;
        }

        if ((result.state_called & (1 << PJ_TURN_STATE_DESTROYING)) == 0) {
            PJ_LOG(3,
                   ("", "    error: PJ_TURN_STATE_DESTROYING is not called"));
            return -190;
        }
...
static int destroy_test(pj_stun_config* stun_cfg, pj_bool_t with_dns_srv,
                        pj_bool_t in_callback, pj_bool_t use_ipv6,
                        pj_turn_tp_type tp_type)
{
    struct test_session_cfg test_cfg = { {
                                           /* Client cfg */
    ...
    PJ_LOG(3,
           ("", "  destroy test %s %s (%s)", (in_callback ? "in callback" : ""),
    ...
    for (target_state = PJ_TURN_STATE_RESOLVING;
         target_state <= PJ_TURN_STATE_READY; ++target_state)
    {
        enum
        {
            TIMEOUT = 60
        ...
        PJ_LOG(
          3, ("", "   %s", pj_turn_state_name((pj_turn_state_t)target_state)));
