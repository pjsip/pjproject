...
static void set_stun_turn_cfg(struct ice_ept* ept, pj_ice_strans_cfg* ice_cfg,
                              char* serverip, pj_bool_t use_ipv6)
{
...
static int create_ice_strans(struct test_sess* test_sess, struct ice_ept* ept,
                             pj_ice_strans** p_ice)
{
    pj_ice_strans* ice;
    ...
    /* Create ICE stream transport */
    status = pj_ice_strans_create(NULL, &ice_cfg, ept->cfg.comp_cnt, (void*)ept,
                                  &ice_cb, &ice);
    if (status != PJ_SUCCESS) {
...
/* Create test session */
static int create_sess(pj_stun_config* stun_cfg, unsigned server_flag,
                       struct test_cfg* caller_cfg, struct test_cfg* callee_cfg,
                       struct sess_param* test_param, struct test_sess** p_sess)
{
    pj_pool_t* pool;
    ...
    sess->caller.result.init_status = sess->caller.result.nego_status =
      PJ_EPENDING;

    pj_memcpy(&sess->callee.cfg, callee_cfg, sizeof(*callee_cfg));
    sess->callee.result.init_status = sess->callee.result.nego_status =
      PJ_EPENDING;
                                 ...
                                 const pj_ice_sess_cand* cand, pj_bool_t last)
{
    ...
    if (cand) {
        PJ_LOG(
          4,
          (THIS_FILE,
           INDENT "%p: discovered a new candidate "
                  "comp=%d, type=%s, addr=%s, baseaddr=%s, end=%d",
           ept->ice, cand->comp_id, pj_ice_get_cand_type_name(cand->type),
...
static pj_status_t start_ice(struct ice_ept* ept, const struct ice_ept* remote)
{
    ...
    if (ept->cfg.trickle == PJ_ICE_SESS_TRICKLE_DISABLED) {
        ...
        for (i = 0; i < remote->cfg.comp_cnt; ++i) {
            ...
            status = pj_ice_strans_enum_cands(remote->ice, i + 1, &cnt,
                                              rcand + rcand_cnt);
            if (status != PJ_SUCCESS) {
                      ...
                      int start_err)
{
    ...
    for (i = 0; i < min_cnt; ++i) {
        ...
        if (c1 == NULL) {
            PJ_LOG(3, (THIS_FILE,
                       INDENT "err: unable to get valid pair for ice1 "
                              "component %d",
                       i + 1));
            return start_err - 2;
        }
        ...
        if (c2 == NULL) {
            PJ_LOG(3, (THIS_FILE,
                       INDENT "err: unable to get valid pair for ice2 "
                              "component %d",
                       i + 1));
            return start_err - 4;
        }
        ...
        if (pj_sockaddr_cmp(&c1->rcand->addr, &c2->lcand->addr) != 0) {
            PJ_LOG(3, (THIS_FILE,
                       INDENT "err: candidate pair does not match "
                              "for component %d",
                       i + 1));
            return start_err - 6;
        }
    ...
    for (; i < max_cnt; ++i) {
        ...
        {
            PJ_LOG(3, (THIS_FILE,
                       INDENT "err: ice1 shouldn't have valid pair "
                              "for component %d",
                       i + 1));
            return start_err - 8;
        }
        ...
        {
            PJ_LOG(3, (THIS_FILE,
                       INDENT "err: ice2 shouldn't have valid pair "
                              "for component %d",
                       i + 1));
            return start_err - 9;
        }
...
static int perform_test2(const char* title, pj_stun_config* stun_cfg,
                         unsigned server_flag, struct test_cfg* caller_cfg,
                         ...
                         struct sess_param* test_param)
{
    ...
    char add_title1[16];
    char add_title2[16];
    pj_bool_t client_mix_test =
      ((callee_cfg->client_flag & (CLIENT_IPV4 + CLIENT_IPV6)) !=
       (caller_cfg->client_flag & (CLIENT_IPV4 + CLIENT_IPV6)));

    sprintf(
      add_title1, "%s%s%s", (server_flag & SERVER_IPV4) ? "IPv4" : "",
...
#define ALL_READY \
    (sess->caller.result.init_status != PJ_EPENDING && \
    ...
    if (sess->caller.result.init_status !=
        sess->caller.cfg.expected.init_status) {
        app_perror(INDENT "err: caller init", sess->caller.result.init_status);
        destroy_sess(sess, 500);
        return -102;
    }
    if (sess->callee.result.init_status !=
        sess->callee.cfg.expected.init_status) {
    ...
    for (i = 0; i < sess->param->worker_cnt; ++i) {
        ...
        status =
          pj_thread_create(sess->pool, "worker_thread", worker_thread_proc,
                           sess, 0, 0, &sess->worker_threads[i]);
        if (status != PJ_SUCCESS) {
    ...
    if (sess->caller.result.nego_status !=
        sess->caller.cfg.expected.nego_status) {
        app_perror(INDENT "err: caller negotiation failed",
                   sess->caller.result.nego_status);
        destroy_sess(sess, 500);
        return -150;
    }

    if (sess->callee.result.nego_status !=
        sess->callee.cfg.expected.nego_status) {
        app_perror(INDENT "err: callee negotiation failed",
                   sess->callee.result.nego_status);
        destroy_sess(sess, 500);
        return -160;
    }
...
static void set_client_server_flag(unsigned server_flag, unsigned caller_flag,
...
static int perform_test(const char* title, pj_stun_config* stun_cfg,
                        unsigned server_flag, struct test_cfg* caller_cfg,
                        struct test_cfg* callee_cfg)
{
    struct sess_param test_param;
    ...
    set_client_server_flag(SERVER_IPV4, CLIENT_IPV4, CLIENT_IPV4, &server_flag,
                           &caller_cfg->client_flag, &callee_cfg->client_flag);

    pj_bzero(&test_param, sizeof(test_param));

    rc = perform_test2(title, stun_cfg, server_flag, caller_cfg, callee_cfg,
                       &test_param);
    ...
    if (enable_ipv6_test()) {
        ...
        if (rc == PJ_SUCCESS) {
            pj_bzero(&test_param, sizeof(test_param));
            set_client_server_flag(
              SERVER_IPV4 + SERVER_IPV6, CLIENT_IPV4 + CLIENT_IPV6,
              CLIENT_IPV4 + CLIENT_IPV6, &server_flag, &caller_cfg->client_flag,
              &callee_cfg->client_flag);
        ...
        if (rc == PJ_SUCCESS) {
            pj_bzero(&test_param, sizeof(test_param));
            set_client_server_flag(
              SERVER_IPV4 + SERVER_IPV6, CLIENT_IPV4, CLIENT_IPV6, &server_flag,
              &caller_cfg->client_flag, &callee_cfg->client_flag);
...
int ice_test(void)
{
    ...
    struct sess_cfg_t
    {
        ...
        /*  Role    comp#   host?   stun?   turn?   flag?  ans_del snd_del
           des_del */
        { "hosts candidates only",
          ...
          { ROLE1,
            ...
            0,
            0,
            { PJ_SUCCESS, PJ_SUCCESS, PJ_SUCCESS } },
          { ROLE2,
            1,
            YES,
            NO,
    ...
    if (1) {
        struct sess_cfg_t cfg = { "Basic with host candidates",
                                  0x0,
                                  /*  Role    comp#   host?   stun?   turn?
                                     flag?  ans_del snd_del des_del */
                                  { ROLE1,
                                    1,
                                    YES,
                                    NO,
        ...
        rc = perform_test(cfg.title, &stun_cfg, cfg.server_flag, &cfg.ua1,
                          &cfg.ua2);
        if (rc != 0)
            goto on_return;
        ...
        cfg.ua1.comp_cnt = 2;
        cfg.ua2.comp_cnt = 2;
        rc = perform_test("Basic with host candidates, 2 components", &stun_cfg,
                          cfg.server_flag, &cfg.ua1, &cfg.ua2);
        if (rc != 0)
            goto on_return;
    }
    ...
    if (1) {
        struct sess_cfg_t cfg = { "Basic with srflx candidates",
                                  0xFFFF,
                                  /*  Role    comp#   host?   stun?   turn?
                                     flag?  ans_del snd_del des_del */
                                  { ROLE1,
        ...
        rc = perform_test("Basic with srflx candidates, 2 components",
                          &stun_cfg, cfg.server_flag, &cfg.ua1, &cfg.ua2);
        if (rc != 0)
            goto on_return;
    }
    ...
    if (1) {
        struct sess_cfg_t cfg = { "Basic with relay candidates",
                                  0xFFFF,
                                  /*  Role    comp#   host?   stun?   turn?
                                     flag?  ans_del snd_del des_del */
                                  { ROLE1,
    ...
    if (1) {
        struct sess_cfg_t cfg = { "STUN resolution failure",
                                  0x0,
                                  /*  Role    comp#   host?   stun?   turn?
                                     flag?  ans_del snd_del des_del */
                                  { ROLE1,
                                    ...
                                    0,
                                    0,
                                    { PJ_SUCCESS, PJNATH_ESTUNTIMEDOUT, -1 } },
                                  { ROLE2,
                                    2,
                                    NO,
                                    YES,
                                    ...
                                    0,
                                    0,
                                    { PJ_SUCCESS, PJNATH_ESTUNTIMEDOUT,
                                      -1 } } };
    ...
    if (1) {
        struct sess_cfg_t cfg = {
            ...
            { ROLE1,
              ...
              0,
              0,
              { PJ_SUCCESS, PJ_STATUS_FROM_STUN_CODE(401), -1 } },
            { ROLE2,
              2,
              NO,
              NO,
    ...
    if (1) {
        struct sess_cfg_t cfg = { "STUN failure, testing TURN deallocation",
                ...
                for (k1 = 1; k1 <= 2; ++k1) {
                    ...
                    for (k2 = 1; k2 <= 2; ++k2) {
                        ...
                        sprintf(title,
                                ...
                                pj_ice_sess_role_name(role[j].ua2), delay[d],
                                k1, k2);
...
int ice_one_conc_test(pj_stun_config* stun_cfg, int err_quit)
{
    ...
    rc = perform_test2(cfg.title, stun_cfg, cfg.server_flag, &cfg.ua1, &cfg.ua2,
                       &test_param);
    if (rc != 0 && err_quit)
        return rc;
...
static void timer_new_cand(pj_timer_heap_t* th, pj_timer_entry* te)
{
    ...
    for (i = 0; i < ncomp; ++i) {
        ...
        if (cnt > data->caller_last_cand_cnt[i]) {
            ...
            rc = pj_ice_strans_update_check_list(
              callee->ice, &caller->ufrag, &caller->pass, new_cnt,
              &cand[cnt - new_cnt], caller_last_cand && (i == ncomp - 1));
            if (rc != PJ_SUCCESS) {
                app_perror(
                  INDENT "err: callee pj_ice_strans_update_check_list()", rc);
                continue;
            }

            data->caller_last_cand_cnt[i] = cnt;
            PJ_LOG(
              4, (THIS_FILE, INDENT "Updated callee with %d new candidates %s",
            ...
            for (j = 0; j < new_cnt; ++j) {
                ...
                PJ_LOG(
                  4, (THIS_FILE,
                      INDENT "%d: comp=%d, type=%s, addr=%s, baseaddr=%s",
                      j + 1, c->comp_id, pj_ice_get_cand_type_name(c->type),
        ...
        if (cnt > data->callee_last_cand_cnt[i]) {
            ...
            /* Update remote with new candidates */
            rc = pj_ice_strans_update_check_list(
              caller->ice, &callee->ufrag, &callee->pass, new_cnt,
              &cand[cnt - new_cnt], callee_last_cand && (i == ncomp - 1));
            if (rc != PJ_SUCCESS) {
                app_perror(
                  INDENT "err: caller pj_ice_strans_update_check_list()", rc);
                continue;
            }

            data->callee_last_cand_cnt[i] = cnt;
            PJ_LOG(
              4, (THIS_FILE, INDENT "Updated caller with %d new candidates %s",
...
static int perform_trickle_test(const char* title, pj_stun_config* stun_cfg,
                                unsigned server_flag,
                                ...
                                struct sess_param* test_param)
{
    ...
    PJ_LOG(3, (THIS_FILE, "%s, %d vs %d components", title,
               caller_cfg->comp_cnt, callee_cfg->comp_cnt));
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(cfg) && !rc; ++i) {
        ...
        for (c1 = 1; c1 <= 2 && !rc; ++c1) {
            for (c2 = 1; c2 <= 2 && !rc; ++c2) {
                ...
                rc = perform_trickle_test(cfg[i].title, &stun_cfg,
                                          cfg[i].server_flag, &cfg[i].ua1,
                                          &cfg[i].ua2, &test_param);
            }
        }
    }
