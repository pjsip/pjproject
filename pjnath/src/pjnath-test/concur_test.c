...
static int server_thread_proc(void* p)
{
    ...
    while (!test_sess->thread_quit_flag) {
        ...
        n = pj_sock_select((int)test_sess->server_sock + 1, &rdset, NULL, NULL,
                           &timeout);
        if (n == 1 && PJ_FD_ISSET(test_sess->server_sock, &rdset)) {
            ...
            status = pj_sock_recvfrom(test_sess->server_sock, pkt, &pkt_len, 0,
                                      &client_addr, &addr_len);
            if (status != PJ_SUCCESS) {
                continue;
            }

            status = pj_stun_msg_decode(pool, pkt, pkt_len, PJ_STUN_IS_DATAGRAM,
                                        &stun_req, NULL, NULL);
            if (status != PJ_SUCCESS) {
            ...
            status = pj_stun_msg_create_response(
              pool, stun_req, PJ_STUN_SC_BAD_REQUEST, NULL, &stun_res);
            if (status != PJ_SUCCESS) {
            ...
            status =
              pj_stun_msg_encode(stun_res, pkt, sizeof(pkt), 0, NULL, &res_len);
            if (status != PJ_SUCCESS) {
                                     ...
                                     pj_stun_sock_op op, pj_status_t status)
{
    struct stun_test_session* test_sess =
      (struct stun_test_session*)pj_stun_sock_get_user_data(stun_sock);
...
static int stun_destroy_test_session(struct stun_test_session* test_sess)
{
    ...
    for (i = 0; i < MAX_SOCK_CLIENTS; ++i) {
        ...
        status = pj_stun_sock_create(&test_sess->stun_cfg, name, pj_AF_INET(),
                                     &stun_cb, NULL, test_sess, &stun_sock[i]);
        if (status != PJ_SUCCESS) {
...
static int stun_destroy_test(void)
{
    ...
    status = pj_ioqueue_create(pool, PJ_IOQUEUE_MAX_HANDLES,
    ...
    status = pj_sock_bind(test_sess.server_sock, &bind_addr,
                          pj_sockaddr_get_len(&bind_addr));
    pj_assert(status == PJ_SUCCESS);

    /* Set socket to nonblocking to avoid stuck in recv/recvfrom() on concurrent
     * events */
    ...
    for (i = 0; i < SERVER_THREAD_CNT; ++i) {
        status = pj_thread_create(pool, NULL, &server_thread_proc, &test_sess,
                                  0, 0, &test_sess.server_threads[i]);
        pj_assert(status == PJ_SUCCESS);
    }

    for (i = 0; i < WORKER_THREAD_CNT; ++i) {
        status = pj_thread_create(pool, NULL, &worker_thread_proc, &test_sess,
     ...
     */
    PJ_LOG(3,
           (THIS_FILE, "    Destroy in main thread while callback is running"));
    for (i = 0; i < LOOP; ++i) {
        int sleep = pj_rand() % 5;
        ...
        test_sess.param.server_wait_for_event = PJ_TRUE;
        stun_destroy_test_session(&test_sess);
        PJ_LOG(3, (THIS_FILE, "        stun test c: clients with response: %d",
                   test_sess.param.client_got_response));
