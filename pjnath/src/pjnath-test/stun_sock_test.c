...
static pj_bool_t srv_on_data_recvfrom(pj_activesock_t* asock, void* data,
                                      pj_size_t size,
                                      ...
                                      int addr_len, pj_status_t status)
{
    ...
    if (srv->flag & RESPOND_STUN) {
        ...
        /* Create response */
        status =
          pj_stun_msg_create(pool, PJ_STUN_BINDING_RESPONSE, PJ_STUN_MAGIC,
        ...
        /* Encode */
        status =
          pj_stun_msg_encode(res_msg, (pj_uint8_t*)data, 100, 0, NULL, &size);
        if (status != PJ_SUCCESS) {
        ...
        /* Send back */
        sent = size;
        pj_activesock_sendto(asock, &srv->send_key, data, &sent, 0, src_addr,
                             addr_len);
...
static pj_status_t create_server(pj_pool_t* pool, pj_ioqueue_t* ioqueue,
                                 unsigned flag, pj_bool_t use_ipv6,
                                 struct stun_srv** p_srv)
{
    ...
    status =
      pj_activesock_create_udp(pool, &srv->addr, NULL, ioqueue, &activesock_cb,
                               srv, &srv->asock, &srv->addr);
    if (status != PJ_SUCCESS)
        return status;
...
static pj_bool_t stun_sock_on_rx_data(pj_stun_sock* stun_sock, void* pkt,
                                      unsigned pkt_len,
                                      const pj_sockaddr_t* src_addr,
                                      unsigned addr_len)
{
...
static pj_status_t create_client(pj_stun_config* cfg,
                                 ...
                                 pj_bool_t destroy_on_err, pj_bool_t use_ipv6)
{
    pj_pool_t* pool;
    struct stun_client* client;
                        ...
                        pj_bool_t use_ipv6)
{
    ...
    if (client->on_rx_data_cnt != 0) {
        PJ_LOG(
          3, (THIS_FILE, "    error: client shouldn't have received anything"));
        ret = -70;
        goto on_return;
    }
                             ...
                             pj_bool_t use_ipv6)
{
    ...
    status =
      create_server(client->pool, cfg->ioqueue, RESPOND_STUN, use_ipv6, &srv);
    if (status != PJ_SUCCESS) {
        destroy_client(client);
        return -120;
    }
...
static int keep_alive_test(pj_stun_config* cfg, pj_bool_t use_ipv6)
{
    ...
    status = create_server(client->pool, cfg->ioqueue,
                           RESPOND_STUN | WITH_XOR_MAPPED, use_ipv6, &srv);
    if (status != PJ_SUCCESS) {
        destroy_client(client);
        return -320;
    }
    ...
    pj_sockaddr_init(GET_AF(use_ipv6), &mapped_addr, &srv->ip_to_send,
                     srv->port_to_send);
    ...
    {
        char txt[100];
        PJ_LOG(3, (THIS_FILE, "     sending to %s",
                   pj_sockaddr_print(&info.srv_addr, txt, sizeof(txt), 3)));
    }
    status =
      pj_stun_sock_sendto(client->sock, NULL, &ret, sizeof(ret), 0,
                          &info.srv_addr, pj_sockaddr_get_len(&info.srv_addr));
    ...
    if (client->on_status_cnt != 0) {
        PJ_LOG(3, (THIS_FILE, "    error: on_status() must not be called on
    successful" "keep-alive when mapped-address does not change")); ret = -430;
        goto on_return;
    }
    */
    /* Check that client doesn't receive anything */
    ...
    if (client->last_op != PJ_STUN_SOCK_MAPPED_ADDR_CHANGE) {
        PJ_LOG(3,
               (THIS_FILE, "    error: expecting keep-alive operation status"));
        ret = -470;
        goto on_return;
    }
