...
static pj_status_t on_tx_stun_msg(pj_stun_session* sess, void* token,
...
PJ_DEF(pj_status_t) pj_turn_srv_create(pj_pool_factory* pf, pj_turn_srv** p_srv)
{
    ...
    /* Server mutex */
    status =
      pj_lock_create_recursive_mutex(pool, srv->obj_name, &srv->core.lock);
    if (status != PJ_SUCCESS)
        goto on_error;
    ...
    /* Array of listeners */
    srv->core.listener = (pj_turn_listener**)pj_pool_calloc(
      pool, MAX_LISTENERS, sizeof(srv->core.listener[0]));
    ...
    status =
      pj_stun_session_create(&srv->core.stun_cfg, srv->obj_name, &sess_cb,
                             PJ_FALSE, NULL, &srv->core.stun_sess);
    if (status != PJ_SUCCESS) {
        goto on_error;
    }
    ...
    srv->core.thread = (pj_thread_t**)pj_pool_calloc(pool, srv->core.thread_cnt,
                                                     sizeof(pj_thread_t*));

    /* Start the worker threads */
    for (i = 0; i < srv->core.thread_cnt; ++i) {
        status = pj_thread_create(pool, srv->obj_name, &server_thread_proc, srv,
                                  0, 0, &srv->core.thread[i]);
        if (status != PJ_SUCCESS)
            goto on_error;
    }
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_srv_add_listener(pj_turn_srv* srv, pj_turn_listener* lis)
{
    unsigned index;
    ...
    PJ_LOG(4, (srv->obj_name, "Listener %s/%s added at index %d", lis->obj_name,
               lis->info, lis->id));
 ...
 */
PJ_DEF(void)
pj_turn_transport_add_ref(pj_turn_transport* transport,
                          pj_turn_allocation* alloc)
{
 ...
 */
PJ_DEF(void)
pj_turn_transport_dec_ref(pj_turn_transport* transport,
                          pj_turn_allocation* alloc)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_srv_register_allocation(pj_turn_srv* srv, pj_turn_allocation* alloc)
{
    /* Add to hash tables */
    pj_lock_acquire(srv->core.lock);
    pj_hash_set(alloc->pool, srv->tables.alloc, &alloc->hkey,
                sizeof(alloc->hkey), 0, alloc);
    pj_hash_set(alloc->pool, srv->tables.res, &alloc->relay.hkey,
                sizeof(alloc->relay.hkey), 0, &alloc->relay);
    pj_lock_release(srv->core.lock);
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_srv_unregister_allocation(pj_turn_srv* srv, pj_turn_allocation* alloc)
{
    /* Unregister from hash tables */
    pj_lock_acquire(srv->core.lock);
    pj_hash_set(alloc->pool, srv->tables.alloc, &alloc->hkey,
                sizeof(alloc->hkey), 0, NULL);
    pj_hash_set(alloc->pool, srv->tables.res, &alloc->relay.hkey,
                sizeof(alloc->relay.hkey), 0, NULL);
    pj_lock_release(srv->core.lock);
 ...
 */
static pj_status_t on_tx_stun_msg(pj_stun_session* sess, void* token,
                                  const void* pdu, pj_size_t pdu_size,
                                  const pj_sockaddr_t* dst_addr,
                                  unsigned addr_len)
{
...
static pj_status_t stun_respond(pj_stun_session* sess,
                                ...
                                const pj_stun_rx_data* rdata, unsigned code,
                                const char* errmsg, pj_bool_t cache,
                                const pj_sockaddr_t* dst_addr,
                                unsigned addr_len)
{
    pj_status_t status;
    ...
    /* Send the response */
    return pj_stun_session_send_msg(sess, transport, cache, PJ_FALSE, dst_addr,
                                    addr_len, tdata);
}
 ...
 */
static pj_status_t on_rx_stun_request(pj_stun_session* sess,
                                      const pj_uint8_t* pdu, unsigned pdu_len,
                                      ...
                                      unsigned src_addr_len)
{
     ...
     * in this function.
     */
    status = pj_turn_allocation_create(transport, src_addr, src_addr_len, rdata,
                                       sess, &alloc);
    if (status != PJ_SUCCESS) {
...
static void handle_binding_request(pj_turn_pkt* pkt, unsigned options)
{
    ...
    /* Create response */
    status =
      pj_stun_msg_create_response(pkt->pool, request, 0, NULL, &response);
    if (status != PJ_SUCCESS)
        return;
    ...
    /* Send response */
    pkt->transport->sendto(pkt->transport, pdu, len, 0, &pkt->src.clt_addr,
                           pkt->src_addr_len);
}
 ...
 */
PJ_DEF(void) pj_turn_srv_on_rx_pkt(pj_turn_srv* srv, pj_turn_pkt* pkt)
{
    pj_turn_allocation* alloc;

    /* Get TURN allocation from the source address */
    pj_lock_acquire(srv->core.lock);
    alloc = (pj_turn_allocation*)pj_hash_get(srv->tables.alloc, &pkt->src,
                                             sizeof(pkt->src), NULL);
    pj_lock_release(srv->core.lock);
    ...
    if (alloc) {
        ...
        if (status != PJ_SUCCESS) {
             ...
             */
            if ((*pkt->pkt != 0x00 && *pkt->pkt != 0x01) || pkt->len > 1600 ||
                (options & PJ_STUN_IS_DATAGRAM))
            {
                ...
                PJ_LOG(
                  5, (srv->obj_name, "Non-STUN packet from %s is dropped: %s",
        ...
        status = pj_stun_session_on_rx_pkt(
          srv->core.stun_sess, pkt->pkt, pkt->len, options, pkt->transport,
          &parsed_len, &pkt->src.clt_addr, pkt->src_addr_len);
        if (status != PJ_SUCCESS) {
            char errmsg[PJ_ERR_MSG_SIZE];
            char ip[PJ_INET6_ADDRSTRLEN + 10];
            ...
            pj_strerror(status, errmsg, sizeof(errmsg));
            PJ_LOG(5,
                   (srv->obj_name, "Error processing STUN packet from %s: %s",
                    pj_sockaddr_print(&pkt->src.clt_addr, ip, sizeof(ip), 3),
                    errmsg));
        }
