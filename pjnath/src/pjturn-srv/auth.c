...
} g_cred[] = { { "100", "100" }, { "700", "700" }, { "701", "701" } };
                     ...
                     pj_str_t* data)
{
    ...
    if (pj_stricmp2(realm, g_realm)) {
        LOG((THIS_FILE, "auth error: invalid realm '%.*s'", (int)realm->slen,
             realm->ptr));
        return PJ_EINVAL;
    }
    ...
    LOG((THIS_FILE, "auth error: user '%.*s' not found", (int)username->slen,
         username->ptr));
    return PJ_ENOTFOUND;
}
                     ...
                     const pj_str_t* nonce)
{
    ...
    if (pj_stricmp2(nonce, THE_NONCE)) {
        LOG((THIS_FILE, "auth error: invalid nonce '%.*s'", (int)nonce->slen,
             nonce->ptr));
        return PJ_FALSE;
    }
