...
static pj_status_t create_relay(pj_turn_srv* srv, pj_turn_allocation* alloc,
                                const pj_stun_msg* msg,
                                const alloc_request* req,
                                pj_turn_relay_res* relay);
static void destroy_relay(pj_turn_relay_res* relay);
static void on_rx_from_peer(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
                      ...
                      pj_status_t status)
{
    ...
    PJ_LOG(
      4, (alloc->obj_name, "%s for client %s: %s", title, alloc->info, errmsg));
}

/* Parse ALLOCATE request */
static pj_status_t parse_allocate_req(alloc_request* cfg, pj_stun_session* sess,
                                      ...
                                      unsigned src_addr_len)
{
    ...
    attr_bw =
      (pj_stun_uint_attr*)pj_stun_msg_find_attr(req, PJ_STUN_ATTR_BANDWIDTH, 0);
    if (attr_bw) {
    ...
    if (cfg->bandwidth > MAX_CLIENT_BANDWIDTH) {
        pj_stun_session_respond(
          sess, rdata, PJ_STUN_SC_ALLOCATION_QUOTA_REACHED, "Invalid bandwidth",
    ...
    attr_req_tp = (pj_stun_uint_attr*)pj_stun_msg_find_attr(
      req, PJ_STUN_ATTR_REQ_TRANSPORT, 0);
    if (attr_req_tp == NULL) {
        pj_stun_session_respond(sess, rdata, PJ_STUN_SC_BAD_REQUEST,
                                "Missing REQUESTED-TRANSPORT attribute", NULL,
    ...
    attr_res_token = (pj_stun_res_token_attr*)pj_stun_msg_find_attr(
      req, PJ_STUN_ATTR_RESERVATION_TOKEN, 0);
    if (attr_res_token) {
    ...
    attr_lifetime =
      (pj_stun_uint_attr*)pj_stun_msg_find_attr(req, PJ_STUN_ATTR_LIFETIME, 0);
    if (attr_lifetime) {
        cfg->lifetime = attr_lifetime->value;
        if (cfg->lifetime < MIN_LIFETIME) {
            pj_stun_session_respond(sess, rdata, PJ_STUN_SC_BAD_REQUEST,
                                    "LIFETIME too short", NULL, PJ_TRUE,
                                          ...
                                          const pj_stun_rx_data* rdata)
{
    ...
    pj_stun_msg_add_sockaddr_attr(
      tdata->pool, tdata->msg, PJ_STUN_ATTR_XOR_RELAYED_ADDR, PJ_TRUE,
      &alloc->relay.hkey.addr, pj_sockaddr_get_len(&alloc->relay.hkey.addr));

    /* Add LIFETIME. */
    pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_LIFETIME,
                              (unsigned)alloc->relay.lifetime);

    /* Add BANDWIDTH */
    pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_BANDWIDTH,
                              alloc->bandwidth);
    ...
    pj_stun_msg_add_sockaddr_attr(
      ...
      &alloc->hkey.clt_addr, pj_sockaddr_get_len(&alloc->hkey.clt_addr));

    /* Send the response */
    return pj_stun_session_send_msg(
      srv_sess, transport, PJ_TRUE, PJ_FALSE, &alloc->hkey.clt_addr,
      pj_sockaddr_get_len(&alloc->hkey.clt_addr), tdata);
}
...
static pj_status_t init_cred(pj_turn_allocation* alloc, const pj_stun_msg* req)
{
    ...
    realm = (const pj_stun_realm_attr*)pj_stun_msg_find_attr(
      req, PJ_STUN_ATTR_REALM, 0);
    PJ_ASSERT_RETURN(realm != NULL, PJ_EBUG);

    user = (const pj_stun_username_attr*)pj_stun_msg_find_attr(
      req, PJ_STUN_ATTR_USERNAME, 0);
    PJ_ASSERT_RETURN(user != NULL, PJ_EBUG);

    nonce = (const pj_stun_nonce_attr*)pj_stun_msg_find_attr(
      req, PJ_STUN_ATTR_NONCE, 0);
    PJ_ASSERT_RETURN(nonce != NULL, PJ_EBUG);

    /* Lookup the password */
    status =
      pj_turn_get_password(NULL, NULL, &realm->value, &user->value, alloc->pool,
    ...
    pj_strdup(alloc->pool, &alloc->cred.data.static_cred.username,
              &user->value);
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_allocation_create(pj_turn_transport* transport,
                          ...
                          pj_turn_allocation** p_alloc)
{
    ...
    status =
      pj_lock_create_recursive_mutex(pool, alloc->obj_name, &alloc->lock);
    if (status != PJ_SUCCESS) {
        goto on_error;
    }
    ...
    /* Done */
    pj_sockaddr_print(&alloc->relay.hkey.addr, str_tmp, sizeof(str_tmp), 3);
 ...
 */
PJ_DEF(void)
pj_turn_allocation_on_transport_closed(pj_turn_allocation* alloc,
                                       pj_turn_transport* tp)
{
    PJ_LOG(5, (alloc->obj_name,
               "Transport %s unexpectedly closed, destroying "
               "allocation %s",
               tp->info, alloc->info));
...
static void alloc_shutdown(pj_turn_allocation* alloc)
{
    ...
    pj_timer_heap_schedule(alloc->server->core.timer_heap, &alloc->relay.timer,
                           &destroy_delay);
}
...
static void relay_timeout_cb(pj_timer_heap_t* heap, pj_timer_entry* e)
{
    ...
    if (e->id == TIMER_ID_TIMEOUT) {
        ...
        PJ_LOG(4,
               (alloc->obj_name, "Client %s refresh timed-out, shutting down..",
                alloc->info));
        ...
        PJ_LOG(4, (alloc->obj_name, "Client %s destroying..", alloc->info));
                                ...
                                pj_turn_relay_res* relay)
{
    ...
    /* Save realm */
    sa =
      (pj_stun_string_attr*)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_REALM, 0);
    PJ_ASSERT_RETURN(sa, PJ_EINVALIDOP);
    pj_strdup(pool, &relay->realm, &sa->value);

    /* Save username */
    sa = (pj_stun_string_attr*)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_USERNAME,
                                                     0);
    PJ_ASSERT_RETURN(sa, PJ_EINVALIDOP);
    ...
    pj_timer_entry_init(&relay->timer, TIMER_ID_NONE, relay, &relay_timeout_cb);
    ...
    if (status != PJ_SUCCESS) {
        PJ_LOG(4, (THIS_FILE, "pj_sock_getsockname() failed: err %d", status));
        pj_sock_close(relay->tp.sock);
        relay->tp.sock = PJ_INVALID_SOCKET;
        return status;
    }
    ...
    if (status != PJ_SUCCESS) {
        PJ_LOG(
          4, (THIS_FILE, "pj_ioqueue_register_sock() failed: err %d", status));
        pj_sock_close(relay->tp.sock);
        relay->tp.sock = PJ_INVALID_SOCKET;
        return status;
    }
...
static void send_reply_err(pj_turn_allocation* alloc,
                           const pj_stun_rx_data* rdata, pj_bool_t cache,
                           int code, const char* errmsg)
{
    pj_status_t status;

    status = pj_stun_session_respond(
      alloc->sess, rdata, code, errmsg, NULL, cache, &alloc->hkey.clt_addr,
                          ...
                          const pj_stun_rx_data* rdata)
{
    ...
    if (PJ_STUN_GET_METHOD(tdata->msg->hdr.type) != PJ_STUN_CHANNEL_BIND_METHOD)
    {
        ...
        if (interval != 0) {
            pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg,
                                      PJ_STUN_ATTR_BANDWIDTH, alloc->bandwidth);
        }
    }

    status = pj_stun_session_send_msg(
      alloc->sess, NULL, PJ_TRUE, PJ_FALSE, &alloc->hkey.clt_addr,
      pj_sockaddr_get_len(&alloc->hkey.clt_addr), tdata);
    if (status != PJ_SUCCESS) {
...
static pj_turn_permission* lookup_permission_by_addr(
  pj_turn_allocation* alloc, const pj_sockaddr_t* peer_addr, unsigned addr_len)
{
    pj_turn_permission* perm;
    ...
    perm = (pj_turn_permission*)pj_hash_get(
      alloc->peer_table, pj_sockaddr_get_addr(peer_addr),
      pj_sockaddr_get_addr_len(peer_addr), NULL);
    return perm ? check_permission_expiry(perm) : NULL;
}

/* Lookup permission in hash table by the channel number */
static pj_turn_permission* lookup_permission_by_chnum(pj_turn_allocation* alloc,
                                                      unsigned chnum)
{
 ...
 */
PJ_DEF(void)
pj_turn_allocation_on_rx_client_pkt(pj_turn_allocation* alloc, pj_turn_pkt* pkt)
{
    pj_bool_t is_stun;
    pj_status_t status;
    ...
    if (is_stun) {
        ...
        status = pj_stun_session_on_rx_pkt(
          alloc->sess, pkt->pkt, pkt->len, options, NULL, &parsed_len,
 ...
 * on_rx_from_peer().
 */
static void handle_peer_pkt(pj_turn_allocation* alloc, pj_turn_relay_res* rel,
                            ...
                            const pj_sockaddr* src_addr)
{
    ...
    /* Lookup permission */
    perm =
      lookup_permission_by_addr(alloc, src_addr, pj_sockaddr_get_len(src_addr));
    if (perm == NULL) {
    ...
    if (perm->channel != PJ_TURN_INVALID_CHANNEL) {
        ...
        if (len > PJ_TURN_MAX_PKT_LEN) {
            ...
            PJ_LOG(4, (alloc->obj_name,
                       "Client %s: discarded data from %s "
        ...
        /* Send to client */
        alloc->transport->sendto(
          alloc->transport, rel->tp.tx_pkt, len + sizeof(pj_turn_channel_data),
          0, &alloc->hkey.clt_addr, pj_sockaddr_get_len(&alloc->hkey.clt_addr));
    } else {
        ...
        pj_stun_msg_add_binary_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_DATA,
                                    (const pj_uint8_t*)pkt, len);

        pj_stun_session_send_msg(
          alloc->sess, NULL, PJ_FALSE, PJ_FALSE, &alloc->hkey.clt_addr,
          pj_sockaddr_get_len(&alloc->hkey.clt_addr), tdata);
    }
}
                            ...
                            pj_ssize_t bytes_read)
{
    ...
    do {
        if (bytes_read > 0) {
            handle_peer_pkt(rel->allocation, rel, rel->tp.rx_pkt, bytes_read,
                            &rel->tp.src_addr);
        }
        ...
        status =
          pj_ioqueue_recvfrom(key, op_key, rel->tp.rx_pkt, &bytes_read, 0,
                                      ...
                                      unsigned src_addr_len)
{
    ...
    if (msg->hdr.type == PJ_STUN_REFRESH_REQUEST) {
        ...
        /* Get LIFETIME attribute */
        lifetime = (pj_stun_lifetime_attr*)pj_stun_msg_find_attr(
          msg, PJ_STUN_ATTR_LIFETIME, 0);

        /* Get BANDWIDTH attribute */
        bandwidth = (pj_stun_bandwidth_attr*)pj_stun_msg_find_attr(
          msg, PJ_STUN_ATTR_BANDWIDTH, 0);
        ...
        if (lifetime && lifetime->value == 0) {
            ...
            PJ_LOG(
              4, (alloc->obj_name,
                  "Client %s request to dealloc, shutting down", alloc->info));
        ...
        ch_attr = (pj_stun_channel_number_attr*)pj_stun_msg_find_attr(
          msg, PJ_STUN_ATTR_CHANNEL_NUMBER, 0);
        ...
        p1 =
          lookup_permission_by_chnum(alloc, PJ_STUN_GET_CH_NB(ch_attr->value));
         ...
         */
        p2 =
          lookup_permission_by_addr(alloc, &peer_attr->sockaddr,
                                         ...
                                         unsigned src_addr_len)
{
    ...
    pj_sock_sendto(alloc->relay.tp.sock, data_attr->data, &len, 0,
                   &peer_attr->sockaddr,
                   pj_sockaddr_get_len(&peer_attr->sockaddr));
