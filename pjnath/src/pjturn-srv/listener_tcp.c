...
static void show_err(const char* sender, const char* title, pj_status_t status)
{
                            ...
                            pj_turn_listener** p_listener)
{
    ...
    /* Init bind address */
    status =
      pj_sockaddr_init(af, &tcp_lis->base.addr, bound_addr, (pj_uint16_t)port);
    if (status != PJ_SUCCESS)
        goto on_error;
    ...
    status =
      pj_ioqueue_register_sock(pool, srv->core.ioqueue, tcp_lis->base.sock,
                               tcp_lis, &ioqueue_cb, &tcp_lis->key);

    /* Create op keys */
    tcp_lis->accept_op = (struct accept_op*)pj_pool_calloc(
    ...
    /* Done */
    PJ_LOG(4,
           (tcp_lis->base.obj_name, "Listener %s created", tcp_lis->base.info));
...
static pj_status_t tcp_sendto(pj_turn_transport* tp, const void* packet,
                              pj_size_t size, unsigned flag,
                              const pj_sockaddr_t* addr, int addr_len);
static void tcp_destroy(struct tcp_transport* tcp);
static void tcp_add_ref(pj_turn_transport* tp, pj_turn_allocation* alloc);
static void tcp_dec_ref(pj_turn_transport* tp, pj_turn_allocation* alloc);
                             ...
                             pj_sockaddr_t* src_addr, int src_addr_len)
{
    ...
    /* Init pkt */
    tcp->recv_op.pkt.pool =
      pj_pool_create(lis->server->core.pf, "tcpkt%p", 1000, 1000, NULL);
    ...
    do {
        ...
        if (bytes_read > 0) {
            ...
            if (tcp->alloc) {
                ...
                pj_turn_srv_on_rx_pkt(tcp->base.listener->server,
                                      &recv_op->pkt);
            }
        ...
        status =
          pj_ioqueue_recv(tcp->key, op_key, recv_op->pkt.pkt + recv_op->pkt.len,
                          &bytes_read, 0);
