...
static void turn_on_rx_data(pj_turn_sock* relay, void* pkt, unsigned pkt_len,
...
static int init()
{
    ...
    for (i = 0; i < (int)PJ_ARRAY_SIZE(g.peer); ++i) {
        ...
        status =
          pj_stun_sock_create(&g.stun_config, name, pj_AF_INET(), &stun_sock_cb,
                              &ss_cfg, &g.peer[i], &g.peer[i].stun_sock);
        if (status != PJ_SUCCESS) {
        ...
        status = pj_stun_sock_start(g.peer[i].stun_sock, &server, port, NULL);
        if (status != PJ_SUCCESS) {
    ...
    CHECK(
      pj_thread_create(g.pool, "stun", &worker_thread, NULL, 0, 0, &g.thread));
...
static pj_status_t create_relay(void)
{
    ...
    if (o.nameserver) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_LOG(
              1, (THIS_FILE, "Error configuring nameserver (err=%d)", status));
            return status;
        }
    }
...
static void menu(void)
{
    ...
    if (g.relay) {
        ...
        if (info.state >= PJ_TURN_STATE_READY)
            pj_sockaddr_print(&info.relay_addr, relay_addr, sizeof(relay_addr),
                              3);
        else
    ...
    pj_sockaddr_print(&g.peer[0].mapped_addr, peer0_addr, sizeof(peer0_addr),
                      3);
    pj_sockaddr_print(&g.peer[1].mapped_addr, peer1_addr, sizeof(peer1_addr),
                      3);
    ...
    while (!g.quit) {
        ...
        case 's':
            ...
            status = pj_turn_sock_sendto(
              g.relay, (const pj_uint8_t*)input, strlen(input) + 1,
              &peer->mapped_addr, pj_sockaddr_get_len(&peer->mapped_addr));
        ...
        case '1':
            ...
            pj_stun_sock_sendto(peer->stun_sock, NULL, input, strlen(input) + 1,
                                0, &g.relay_addr,
...
static void usage(void)
{
    ...
    puts(
      " --stun-srv, -S  NAME  Use this STUN srv instead of TURN for Binding "
      "discovery");
...
int main(int argc, char* argv[])
{
    struct pj_getopt_option long_options[] = {
        { "realm", 1, 0, 'r' },    { "username", 1, 0, 'u' },
        { "password", 1, 0, 'p' }, { "fingerprint", 0, 0, 'F' },
        { "tcp", 0, 0, 'T' },      { "help", 0, 0, 'h' },
        { "stun-srv", 1, 0, 'S' }, { "nameserver", 1, 0, 'N' }
    };
    int c, opt_id;
    char* pos;
    pj_status_t status;

    while ((c = pj_getopt_long(argc, argv, "r:u:p:S:N:hFT", long_options,
                               &opt_id)) != -1)
    {
        switch (c) {
        case 'r':
