...
/* Frequency default value (admitted 8000 Hz, 16000 Hz, 32000 Hz, 44100 Hz and
 * 48000 Hz) */
...
static pj_status_t factory_refresh(pjmedia_aud_dev_factory* f)
{
    ...
    wf->dev_info = (struct bddev_info*)pj_pool_calloc(
      wf->pool, wf->dev_count, sizeof(struct bddev_info));

    // Capture device properties
    for (i = 0; i < captureDeviceCount; i++) {
        wf->dev_info[i].deviceId = i;
        bdIMADpj_getDeviceCapabilities(BD_IMAD_CAPTURE_DEVICES,
                                       &wf->dev_info[i].info.caps);
    ...
    for (i = 0; i < playbackDeviceCount; i++) {
        ...
        bdIMADpj_getDeviceCapabilities(
    ...
    for (i = 0; i < wf->dev_count; i++) {
        PJ_LOG(4, (THIS_FILE, " dev_id %d: %s  (in=%d, out=%d)", i,
                   wf->dev_info[i].info.name, wf->dev_info[i].info.input_count,
                                         ...
                                         pjmedia_aud_param* param)
{
    ...
    param->samples_per_frame = di->info.default_samples_per_sec *
                               param->channel_count *
                               BD_IMAD_MSECOND_PER_BUFFER / 1000;
...
void bdimad_CaptureCallback(void* buffer, int samples, void* user_data)
{
    ...
    if (strm->rec_thread_initialized == 0 || !pj_thread_is_registered()) {
        ...
        status = pj_thread_register("bd_CaptureCallback", strm->rec_thread_desc,
                                    &strm->rec_thread);
    ...
    if (strm->fmt_id == PJMEDIA_FORMAT_L16) {
        if (nsamples >= strm->samples_per_frame) {
            ...
            if (strm->rec_buf_count) {
                ...
                strm->timestampCapture.u64 +=
                  strm->samples_per_frame / strm->channel_count;
            }
...
int bdimad_PlaybackCallback(void* buffer, int samples, void* user_data)
{
    ...
    if (strm->play_thread_initialized == 0 || !pj_thread_is_registered()) {
        ...
        status = pj_thread_register("bd_PlaybackCallback",
                                    strm->play_thread_desc, &strm->play_thread);
        if (status != PJ_SUCCESS)
            goto on_break;
    ...
    if (strm->fmt_id == PJMEDIA_FORMAT_L16) {
        ...
        while (nsamples_req && status == 0) {
            if (nsamples_req >= strm->samples_per_frame) {
                ...
                pjmedia_copy_samples((pj_int16_t*)buffer, strm->play_buf,
                                     nsamples_req);
                strm->play_buf_count = strm->samples_per_frame - nsamples_req;
                pjmedia_move_samples(strm->play_buf,
                                     strm->play_buf + nsamples_req,
                                     strm->play_buf_count);
                nsamples_req = 0;
...
static pj_status_t init_streams(struct bd_factory* wf, struct bd_stream* strm,
                                const pjmedia_aud_param* prm)
{
    ...
    strm->bytes_per_frame =
      (prm->clock_rate * ((prm->channel_count * prm->bits_per_sample) / 8)) *
      ptime / 1000;
    ...
    mbstowcs(bdImadPjDiagnosticFolderPath, BD_IMAD_DIAGNOSTIC_PATH,
             strlen(BD_IMAD_DIAGNOSTIC_PATH));
    ...
    if (prm->clock_rate != 8000 && prm->clock_rate != 16000 &&
        prm->clock_rate != 32000 && prm->clock_rate != 44100 &&
        prm->clock_rate != 48000)
    {
        PJ_LOG(
          4,
          (THIS_FILE,
    ...
    errorInitAEC =
      bdIMADpj_InitAEC(&strm->bdIMADpjInstance, &strm->bdIMADpjSettingsPtr,
                       &strm->bdIMADpjWarningPtr);

    {
        int auxInt = (prm->ec_enabled == PJ_TRUE ? 1 : 0);
        bdIMADpj_setParameter(strm->bdIMADpjInstance,
                              BD_PARAM_IMAD_PJ_AEC_ENABLE, &auxInt);
        auxInt = 1;
        // Mic control On by default
        bdIMADpj_setParameter(strm->bdIMADpjInstance,
                              BD_PARAM_IMAD_PJ_MIC_CONTROL_ENABLE, &auxInt);
                                        ...
                                        const void* pval)
{
    ...
    if (cap == PJMEDIA_AUD_DEV_CAP_EC) {
        ...
        /* AEC setting */
        res = bdIMADpj_setParameter(strm->bdIMADpjInstance,
                                    BD_PARAM_IMAD_PJ_AEC_ENABLE, &aecOnOff);
        if (res == BD_PJ_OK) {
    ...
    if (cap == PJMEDIA_AUD_DEV_CAP_OUTPUT_ROUTE) {
        ...
        res = bdIMADpj_setRouteOutputDevice(strm->bdIMADpjInstance,
                                            (bdIMADpj_out_dev_route)outputRoute,
                                            &strm->bdIMADpjWarningPtr);
        if (res == BD_PJ_OK) {
                                               ...
                                               pjmedia_aud_stream** p_aud_strm)
{
    ...
    strm->rec_buf = (pj_int16_t*)pj_pool_alloc(pool, strm->bytes_per_frame);
    if (!strm->rec_buf) {
        pj_pool_release(pool);
        return PJ_ENOMEM;
    }
    ...
    strm->play_buf = (pj_int16_t*)pj_pool_alloc(pool, strm->bytes_per_frame);
    if (!strm->play_buf) {
        pj_pool_release(pool);
        return PJ_ENOMEM;
    }
    ...
    if (param->flags & PJMEDIA_AUD_DEV_CAP_EC) {
        stream_set_capBDIMAD(&strm->base, PJMEDIA_AUD_DEV_CAP_EC,
                             &param->ec_enabled);
    }
...
static pj_status_t factory_create_stream(pjmedia_aud_dev_factory* f,
                                         ...
                                         pjmedia_aud_stream** p_aud_strm)
{
    return factory_create_streamBDIMAD(f, param, rec_cb, play_cb, user_data,
                                       p_aud_strm);
}
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
                                    ...
                                    pjmedia_aud_param* pi)
{
    ...
    // Get the AEC setting
    if (stream_get_cap(s, PJMEDIA_AUD_DEV_CAP_EC, &pi->ec_enabled) ==
        PJ_SUCCESS) {
    ...
    if (stream_get_cap(s, PJMEDIA_AUD_DEV_CAP_OUTPUT_ROUTE,
                                        ...
                                        pjmedia_aud_dev_cap cap, void* pval)
{
    ...
    if (cap == PJMEDIA_AUD_DEV_CAP_INPUT_VOLUME_SETTING) {
        ...
        res = bdIMADpj_getRouteOutputDevice(
          strm->bdIMADpjInstance, (bdIMADpj_out_dev_route*)&routeOutDev);
        if (res == BD_PJ_OK) {
