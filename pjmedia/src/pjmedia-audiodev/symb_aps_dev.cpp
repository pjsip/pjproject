...
class CQueueHandler : public CActive {
   public:
    ...
    {
        CQueueHandler* self =
          new (ELeave) CQueueHandler(aObserver, aQ, aWriteQ, aType);
   ...
   private:
    ...
    CQueueHandler(MQueueHandlerObserver* aObserver,
                  RMsgQueue<TAPSCommBuffer>* aQ,
                  RMsgQueue<TAPSCommBuffer>* aWriteQ, TQueueHandlerType aType)
...
class CPjAudioEngine : public CBase, MQueueHandlerObserver {
   ...
   private:
    CPjAudioEngine(struct aps_stream* parent_strm, PjAudioCallback rec_cb,
...
TInt CPjAudioEngine::InitRecL()
{
    ...
    iRecHandler =
      CQueueHandler::NewL(this, &iReadQ, NULL, CQueueHandler::ERecordQueue);
...
static void RecCb(TAPSCommBuffer& buf, void* user_data)
{
    ...
    case PJMEDIA_FORMAT_G729: {
        ...
        if (buf.iBuffer[0] != 0 || buf.iBuffer[1] != 0) {
            ...
            const TDesC8& p = bitstream->CompressG729Frame(
              buf.iBuffer.Right(src_len), src_len == SID_LEN);
    ...
    case PJMEDIA_FORMAT_PCMA: {
        ...
        while (samples_processed < aps_g711_frame_len) {
            unsigned tmp;
            const pj_uint8_t* pb =
              (const pj_uint8_t*)buf.iBuffer.Ptr() + 2 + samples_processed;
...
static pj_status_t factory_init(pjmedia_aud_dev_factory* f)
{
    ...
    TFourCC fourcc[] = { TFourCC(KMCPFourCCIdAMRNB), TFourCC(KMCPFourCCIdG711),
                         TFourCC(KMCPFourCCIdG729), TFourCC(KMCPFourCCIdILBC) };
...
#    endif
        ...
        if (supported) {
            ...
            switch (i) {
            case 0: /* AMRNB */
                pjmedia_format_init_audio(&ext_fmt, PJMEDIA_FORMAT_AMR, 8000, 1,
                                          16, 20, 7400, 12200);
                ...
                break;
            case 1: /* G.711 */
                pjmedia_format_init_audio(&ext_fmt, PJMEDIA_FORMAT_PCMU, 8000,
                                          1, 16, 20, 64000, 64000);
                af->dev_info.ext_fmt[fmt_cnt] = ext_fmt;
                // af->dev_info.ext_fmt[fmt_cnt].vad = PJ_FALSE;
                ++fmt_cnt;
                pjmedia_format_init_audio(&ext_fmt, PJMEDIA_FORMAT_PCMA, 8000,
                                          1, 16, 20, 64000, 64000);
                af->dev_info.ext_fmt[fmt_cnt] = ext_fmt;
                // af->dev_info.ext_fmt[fmt_cnt].vad = PJ_FALSE;
                ++fmt_cnt;
                ...
                break;
            case 2: /* G.729 */
                pjmedia_format_init_audio(&ext_fmt, PJMEDIA_FORMAT_G729, 8000,
                                          1, 16, 20, 8000, 8000);
                af->dev_info.ext_fmt[fmt_cnt] = ext_fmt;
                // af->dev_info.ext_fmt[fmt_cnt].vad = PJ_FALSE;
                ++fmt_cnt;
                break;
            case 3: /* iLBC */
                pjmedia_format_init_audio(&ext_fmt, PJMEDIA_FORMAT_ILBC, 8000,
                                          1, 16, 30, 13333, 15200);
                af->dev_info.ext_fmt[fmt_cnt] = ext_fmt;
                // af->dev_info.ext_fmt[fmt_cnt].vad = PJ_TRUE;
                ++fmt_cnt;
                                         ...
                                         pjmedia_aud_stream** p_aud_strm)
{
    ...
    TRAPD(err, strm->engine = CPjAudioEngine::NewL(
                 strm, aps_rec_cb, aps_play_cb, strm, aps_setting));
    if (err != KErrNone) {
        pj_pool_release(pool);
