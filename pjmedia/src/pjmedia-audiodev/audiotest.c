                 ...
                 pjmedia_aud_test_results* result)
{
    ...
    PJ_LOG(3, (THIS_FILE, " Please wait while test is in progress (~%d secs)..",
    ...
    result->rec.min_interval =
      DIV_ROUND(test_data.capture_data.delay.min, 1000);
    result->rec.max_interval =
      DIV_ROUND(test_data.capture_data.delay.max, 1000);
    result->rec.avg_interval =
      DIV_ROUND(test_data.capture_data.delay.mean, 1000);
    ...
    result->play.min_interval =
      DIV_ROUND(test_data.playback_data.delay.min, 1000);
    result->play.max_interval =
      DIV_ROUND(test_data.playback_data.delay.max, 1000);
    result->play.avg_interval =
      DIV_ROUND(test_data.playback_data.delay.mean, 1000);
    ...
    if (param->dir == PJMEDIA_DIR_CAPTURE_PLAYBACK) {
        ...
        drift =
          play_diff > cap_diff ? play_diff - cap_diff : cap_diff - play_diff;
        ...
        if (drift < (int)param->samples_per_frame) {
            ...
            msec_dur = (test_data.capture_data.last_timestamp -
                        test_data.capture_data.first_timestamp) *
                       1000 / test_data.param->clock_rate;
