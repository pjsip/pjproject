...
static pjmedia_aud_dev_factory_op android_op = {
    &android_init,         &android_destroy,       &android_get_dev_count,
    &android_get_dev_info, &android_default_param, &android_create_stream,
    &android_refresh
};

static pjmedia_aud_stream_op android_strm_op = {
    &strm_get_param, &strm_get_cap, &strm_set_cap,
...
static int AndroidRecorderCallback(void* userData)
{
    ...
    /* Get methods ids */
    read_method =
      (*jni_env)->GetMethodID(jni_env, stream->record_class, "read", "([BII)I");
    record_method = (*jni_env)->GetMethodID(jni_env, stream->record_class,
                                            "startRecording", "()V");
    stop_method =
      (*jni_env)->GetMethodID(jni_env, stream->record_class, "stop", "()V");
    ...
    while (!stream->quit_flag) {
        ...
        bytesRead = (*jni_env)->CallIntMethod(
          jni_env, stream->record, read_method, inputBuffer, 0, size);
...
{
    struct android_aud_stream* stream = (struct android_aud_stream*)userData;
    jmethodID write_method = 0, play_method = 0, stop_method = 0,
              flush_method = 0;
    ...
    /* Get methods ids */
    write_method =
      (*jni_env)->GetMethodID(jni_env, stream->track_class, "write", "([BII)I");
    play_method =
      (*jni_env)->GetMethodID(jni_env, stream->track_class, "play", "()V");
    stop_method =
      (*jni_env)->GetMethodID(jni_env, stream->track_class, "stop", "()V");
    flush_method =
      (*jni_env)->GetMethodID(jni_env, stream->track_class, "flush", "()V");
    ...
    while (!stream->quit_flag) {
        ...
        bytesWritten = (*jni_env)->CallIntMethod(
          jni_env, stream->track, write_method, outputBuffer, 0, size);
                                         ...
                                         pjmedia_aud_stream** p_aud_strm)
{
    ...
    buffSize =
      stream->param.samples_per_frame * stream->param.bits_per_sample / 8;
    ...
    if (stream->dir & PJMEDIA_DIR_CAPTURE) {
        ...
        bufsize_method = (*jni_env)->GetStaticMethodID(
          jni_env, stream->record_class, "getMinBufferSize", "(III)I");
        if (bufsize_method == 0) {
            PJ_LOG(3, (THIS_FILE,
        ...
        inputBuffSizeRec = (*jni_env)->CallStaticIntMethod(
          jni_env, stream->record_class, bufsize_method, param->clock_rate,
          channelInCfg, sampleFormat);
    ...
    if (stream->dir & PJMEDIA_DIR_PLAYBACK) {
        ...
        /* Get the min buffer size function */
        bufsize_method = (*jni_env)->GetStaticMethodID(
          jni_env, stream->track_class, "getMinBufferSize", "(III)I");
        if (bufsize_method == 0) {
            PJ_LOG(3, (THIS_FILE,
                       "Unable to find audio track "
                       "getMinBufferSize() method"));
            status = PJMEDIA_EAUD_SYSERR;
            goto on_error;
        }

        inputBuffSizePlay = (*jni_env)->CallStaticIntMethod(
          jni_env, stream->track_class, bufsize_method, param->clock_rate,
          channelOutCfg, sampleFormat);
    ...
    if (stream->dir & PJMEDIA_DIR_CAPTURE) {
        ...
        constructor_method = (*jni_env)->GetMethodID(
          jni_env, stream->record_class, "<init>", "(IIIII)V");
        ...
        do {
            record_obj = (*jni_env)->NewObject(
              jni_env, stream->record_class, constructor_method, mic_source,
              param->clock_rate, channelInCfg, sampleFormat, inputBuffSizeRec);
            if (record_obj == 0) {
            ...
            if (method_id == 0) {
                PJ_LOG(3, (THIS_FILE,
                           "Unable to find audio record getState() "
                           "method"));
                status = PJMEDIA_EAUD_SYSERR;
                goto on_error;
            }
        ...
        if (stream->record == 0) {
            ...
            release_method = (*jni_env)->GetMethodID(
              jni_env, stream->record_class, "release", "()V");
    ...
    if (stream->dir & PJMEDIA_DIR_PLAYBACK) {
        ...
        /* Get pointer to the constructor */
        constructor_method = (*jni_env)->GetMethodID(
          jni_env, stream->track_class, "<init>", "(IIIIII)V");
        if (constructor_method == 0) {
        ...
        track_obj =
          (*jni_env)->NewObject(jni_env, stream->track_class,
                                constructor_method, 0, /* STREAM_VOICE_CALL */
                                param->clock_rate, channelOutCfg, sampleFormat,
                                inputBuffSizePlay, 1 /* MODE_STREAM */);
        if (track_obj == 0) {
        ...
        if (stream->track == 0) {
            ...
            release_method = (*jni_env)->GetMethodID(
              jni_env, stream->track_class, "release", "()V");
        ...
        if (method_id == 0) {
            PJ_LOG(3, (THIS_FILE,
                       "Unable to find audio track getState() "
                       "method"));
            status = PJMEDIA_EAUD_SYSERR;
            goto on_error;
        }
        state = (*jni_env)->CallIntMethod(jni_env, stream->track, method_id);
        if (state == 0) { /* STATE_UNINITIALIZED */
        ...
        status =
          pj_thread_create(stream->pool, "android_track", AndroidTrackCallback,
                           stream, 0, 0, &stream->play_thread);
        if (status != PJ_SUCCESS)
            goto on_error;
                                ...
                                const void* value)
{
        ...
        (stream->param.dir & PJMEDIA_DIR_PLAYBACK))
    {
        if (stream->track) {
            ...
            if (vol_method) {
                retval = (*jni_env)->CallIntMethod(
                  jni_env, stream->track, vol_method, vol / 100, vol / 100);
            }
