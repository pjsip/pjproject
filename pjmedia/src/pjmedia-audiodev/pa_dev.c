...
static pj_status_t pa_get_dev_info(pjmedia_aud_dev_factory* f, unsigned index,
                                   pjmedia_aud_dev_info* info);
static pj_status_t pa_default_param(pjmedia_aud_dev_factory* f, unsigned index,
...
static pjmedia_aud_dev_factory_op pa_op = {
    &pa_init,          &pa_destroy,       &pa_get_dev_count, &pa_get_dev_info,
    &pa_default_param, &pa_create_stream, &pa_refresh
};

static pjmedia_aud_stream_op pa_strm_op = { &strm_get_param, &strm_get_cap,
                                            &strm_set_cap,   &strm_start,
                                            &strm_stop,      &strm_destroy };

static int PaRecorderCallback(const void* input, void* output,
                              ...
                              PaStreamCallbackFlags statusFlags, void* userData)
{
    ...
    if (nsamples >= stream->samples_per_frame) {
        ...
        if (stream->rec_buf_count) {
            ...
            stream->rec_timestamp.u64 +=
              stream->samples_per_frame / stream->channel_count;
        }
        ...
        if (nsamples && status == 0) {
            ...
            pjmedia_copy_samples(stream->rec_buf, (pj_int16_t*)input, nsamples);
        }
...
static int PaPlayerCallback(const void* input, void* output,
                            unsigned long frameCount,
                            const PaStreamCallbackTimeInfo* timeInfo,
                            PaStreamCallbackFlags statusFlags, void* userData)
{
...
static int PaRecorderPlayerCallback(const void* input, void* output,
                                    unsigned long frameCount,
                                    const PaStreamCallbackTimeInfo* timeInfo,
                                    ...
                                    void* userData)
{
    ...
    rc = PaRecorderCallback(input, output, frameCount, timeInfo, statusFlags,
                            userData);
    if (rc != paContinue)
        return rc;

    rc = PaPlayerCallback(input, output, frameCount, timeInfo, statusFlags,
                          userData);
    return rc;
}
...
#    endif
    ...
    PJ_LOG(4,
           (THIS_FILE, "PortAudio sound library initialized, status=%d", err));
    PJ_LOG(4, (THIS_FILE, "PortAudio host api count=%d", Pa_GetHostApiCount()));
    PJ_LOG(4, (THIS_FILE, "Sound device count=%d", pa_get_dev_count(f)));
...
static pj_status_t create_rec_stream(struct pa_aud_factory* pa,
                                     const pjmedia_aud_param* param,
                                     pjmedia_aud_rec_cb rec_cb, void* user_data,
                                     pjmedia_aud_stream** p_snd_strm)
{
    ...
    stream->rec_buf = (pj_int16_t*)pj_pool_alloc(
      pool, stream->samples_per_frame * stream->bytes_per_sample);
    stream->rec_buf_count = 0;
    ...
    err = Pa_OpenStream(&stream->rec_strm, &inputParam, NULL, param->clock_rate,
                        paFrames, paClipOff, &PaRecorderCallback, stream);
    if (err != paNoError) {
        pj_pool_release(pool);
    ...
    PJ_LOG(5,
           (THIS_FILE,
            "Opened device %s (%s) for recording, sample "
            "rate=%d, ch=%d, "
            "bits=%d, %d samples per frame, latency=%d ms",
            paDevInfo->name, paHostApiInfo->name, paRate, param->channel_count,
            param->bits_per_sample, param->samples_per_frame, paLatency));
                                      ...
                                      pjmedia_aud_stream** p_snd_strm)
{
    ...
    stream->play_buf = (pj_int16_t*)pj_pool_alloc(
      pool, stream->samples_per_frame * stream->bytes_per_sample);
    stream->play_buf_count = 0;
    ...
    if (param->flags & PJMEDIA_AUD_DEV_CAP_OUTPUT_LATENCY)
        ...
        outputParam.suggestedLatency =
          PJMEDIA_SND_DEFAULT_PLAY_LATENCY / 1000.0;
    ...
    err =
      Pa_OpenStream(&stream->play_strm, NULL, &outputParam, param->clock_rate,
                    paFrames, paClipOff, &PaPlayerCallback, stream);
    if (err != paNoError) {
        pj_pool_release(pool);
        return PJMEDIA_AUDIODEV_ERRNO_FROM_PORTAUDIO(err);
    }
    ...
    PJ_LOG(5, (THIS_FILE,
               "Opened device %d: %s(%s) for playing, sample rate=%d"
               ", ch=%d, "
               "bits=%d, %d samples per frame, latency=%d ms",
               play_id, paDevInfo->name, paHostApiInfo->name, paRate,
               param->channel_count, param->bits_per_sample,
               param->samples_per_frame, paLatency));
                                       ...
                                       pjmedia_aud_stream** p_snd_strm)
{
    ...
    if (rec_id == play_id) {
        err =
          Pa_OpenStream(&paStream, &inputParam, &outputParam, param->clock_rate,
                        paFrames, paClipOff, &PaRecorderPlayerCallback, stream);
        if (err == paNoError) {
    ...
    if (paStream == NULL) {
        /* Open input stream */
        err =
          Pa_OpenStream(&stream->rec_strm, &inputParam, NULL, param->clock_rate,
                        paFrames, paClipOff, &PaRecorderCallback, stream);
        if (err == paNoError) {
            /* Open output stream */
            err = Pa_OpenStream(&stream->play_strm, NULL, &outputParam,
                                param->clock_rate, paFrames, paClipOff,
    ...
    PJ_LOG(5, (THIS_FILE,
               "Opened device %s(%s)/%s(%s) for recording and "
               "playback, sample rate=%d, ch=%d, "
               "bits=%d, %d samples per frame, input latency=%d ms, "
               "output latency=%d ms",
               paRecDevInfo->name, paRecHostApiInfo->name, paPlayDevInfo->name,
               paPlayHostApiInfo->name, paRate, param->channel_count,
               param->bits_per_sample, param->samples_per_frame, paInputLatency,
               paOutputLatency));
...
static pj_status_t strm_get_param(pjmedia_aud_stream* s, pjmedia_aud_param* pi)
{
    ...
    pi->clock_rate =
      (unsigned)(paPlaySI ? paPlaySI->sampleRate : paRecSI->sampleRate);
    ...
    if (paRecSI) {
        ...
        pi->input_latency_ms =
          (unsigned)(paRecSI ? paRecSI->inputLatency * 1000 : 0);
    }
    if (paPlaySI) {
        pi->flags |= PJMEDIA_AUD_DEV_CAP_OUTPUT_LATENCY;
        pi->output_latency_ms =
          (unsigned)(paPlaySI ? paPlaySI->outputLatency * 1000 : 0);
    }
...
/* API: get capability */
static pj_status_t strm_get_cap(pjmedia_aud_stream* s, pjmedia_aud_dev_cap cap,
                                void* pval)
{
    struct pa_aud_stream* strm = (struct pa_aud_stream*)s;
...
static pj_status_t strm_destroy(pjmedia_aud_stream* s)
{
    ...
    PJ_LOG(5, (THIS_FILE, "Closing %.*s: %lu underflow, %lu overflow",
               (int)stream->name.slen, stream->name.ptr, stream->underflow,
               stream->overflow));
