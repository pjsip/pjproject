...
class TBitStream {
   public:
    ...
    /*!
      ...
      */
    const TDesC8& CompressG729Frame(const TDesC8& aSrc,
                                    TBool aIsSIDFrame = EFalse);
    ...
    /*!
      ...
      */
    const TDesC8& ExpandG729Frame(const TDesC8& aSrc,
                                  TBool aIsSIDFrame = EFalse);
...
const TDesC8& TBitStream::CompressG729Frame(const TDesC8& aSrc,
                                            TBool aIsSIDFrame)
{
    // reset data
    ...
    for (TInt i = 0, pIdx = 0; i < numParams; i++, pIdx += 2) {
        TUint8 paramBits =
          (aIsSIDFrame) ? KG729SIDPayloadBits[i] : KG729FullPayloadBits[i];
        if (paramBits > 8) {
...
void TBitStream::Compress(TUint8 aValue, TUint8 aNumOfBits)
{
    ...
    } else if (shl > 0)  // bits fit into current byte
    {
        ...
        iData[++iIdx] |=
          (aValue << (8 - iBitOffset));  // push remaining bits to next byte
    }
}
