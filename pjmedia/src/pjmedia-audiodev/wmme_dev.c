...
static void get_dev_names(pjmedia_aud_dev_factory* f)
{
    ...
    for (nDevice = 0; nDevice < cDevices; ++nDevice) {
        ...
        if (FAILED(hr))
            goto cleanup;
        hr =
          IPropertyStore_GetValue(pProps, &PKEY_Device_FriendlyName, &varName);
        if (FAILED(hr))
            goto cleanup;

        for (i = 0; i < wf->dev_count; ++i) {
            if (0 == wcscmp(wf->dev_info[i].endpointId, pwszID)) {
                pj_unicode_to_ansi(varName.pwszVal, wcslen(varName.pwszVal),
...
#    define WIC_WOC(wic, woc, field) (wic ? wic->field : woc->field)
    ...
    pjmedia_format_init_audio(&wdi->info.ext_fmt[0], PJMEDIA_FORMAT_PCMU, 8000,
                              1, 8, 20000, 64000, 64000);
    pjmedia_format_init_audio(&wdi->info.ext_fmt[0], PJMEDIA_FORMAT_PCMA, 8000,
                              1, 8, 20000, 64000, 64000);
}
...
static pj_status_t factory_refresh(pjmedia_aud_dev_factory* f)
{
    ...
    wf->dev_info = (struct wmme_dev_info*)pj_pool_calloc(
      wf->pool, devCount, sizeof(struct wmme_dev_info));
    ...
    if (inputDeviceCount && outputDeviceCount) {
        ...
        if (mr == MMSYSERR_NOERROR) {
            ...
            if (mr == MMSYSERR_NOERROR) {
                build_dev_info(WAVE_MAPPER, &wf->dev_info[wf->dev_count], &wic,
                               &woc);
    ...
    if (inputDeviceCount > 0) {
        ...
        for (i = (waveMapperAdded ? 0 : -1); i < inputDeviceCount; ++i) {
            ...
            build_dev_info(uDeviceID, &wf->dev_info[wf->dev_count], &wic, NULL);
...
#    if PJMEDIA_WMME_DEV_USE_MMDEVICE_API != 0
            ...
            if (mr == MMSYSERR_NOERROR) {
                ...
                *epid =
                  (const wchar_t*)pj_pool_calloc(wf->pool, cbEndpointId, 1);
                mr = waveInMessage((HWAVEIN)IntToPtr(uDeviceID),
...
#    endif
    ...
    if (outputDeviceCount > 0) {
        ...
        for (i = (waveMapperAdded ? 0 : -1); i < outputDeviceCount; ++i) {
            ...
            build_dev_info(uDeviceID, &wf->dev_info[wf->dev_count], NULL, &woc);
...
#    endif
    ...
    PJ_LOG(4, (THIS_FILE, "WMME found %d devices:", wf->dev_count));
    for (c = 0; c < wf->dev_count; ++c) {
        PJ_LOG(4, (THIS_FILE, " dev_id %d: %s  (in=%d, out=%d)", c,
                   wf->dev_info[c].info.name, wf->dev_info[c].info.input_count,
                                     ...
                                     const pjmedia_aud_param* prm)
{
    ...
    if (prm->ext_fmt.id == PJMEDIA_FORMAT_L16) {
        ...
        wfx->nBlockAlign = (pj_uint16_t)(prm->channel_count * BYTES_PER_SAMPLE);
        wfx->nAvgBytesPerSec =
          prm->clock_rate * prm->channel_count * BYTES_PER_SAMPLE;
                ...
                prm->ext_fmt.id == PJMEDIA_FORMAT_PCMU))
    {
        ...
        wfx->wFormatTag =
          (pj_uint16_t)((prm->ext_fmt.id == PJMEDIA_FORMAT_PCMA) ?
                          WAVE_FORMAT_ALAW :
                          WAVE_FORMAT_MULAW);
        wfx->nChannels = (pj_uint16_t)prm->channel_count;
        wfx->nSamplesPerSec = prm->clock_rate;
        wfx->nAvgBytesPerSec = prm->clock_rate * prm->channel_count;
        wfx->nBlockAlign = (pj_uint16_t)(wfx->nAvgBytesPerSec * ptime / 1000);
...
static pj_status_t init_player_stream(struct wmme_factory* wf, pj_pool_t* pool,
                                      ...
                                      unsigned buffer_count)
{
     ...
     * Open wave device.
     */
    mr = waveOutOpen(&wmme_strm->hWave.Out, wf->dev_info[prm->play_id].deviceId,
     ...
     */
    wmme_strm->WaveHdr =
      (WAVEHDR*)pj_pool_zalloc(pool, sizeof(WAVEHDR) * buffer_count);
    for (i = 0; i < buffer_count; ++i) {
        wmme_strm->WaveHdr[i].lpData =
          pj_pool_zalloc(pool, parent->bytes_per_frame);
        wmme_strm->WaveHdr[i].dwBufferLength = parent->bytes_per_frame;
        mr = waveOutPrepareHeader(wmme_strm->hWave.Out,
                                  &(wmme_strm->WaveHdr[i]), sizeof(WAVEHDR));
        if (mr != MMSYSERR_NOERROR) {
            return PJMEDIA_AUDIODEV_ERRNO_FROM_WMME_OUT(mr);
        }
    ...
    PJ_LOG(4,
           (THIS_FILE,
            ...
            wf->dev_info[prm->play_id].info.name, get_fmt_name(prm->ext_fmt.id),
...
static pj_status_t init_capture_stream(struct wmme_factory* wf, pj_pool_t* pool,
                                       struct wmme_stream* parent,
                                       struct wmme_channel* wmme_strm,
                                       const pjmedia_aud_param* prm,
                                       unsigned buffer_count)
{
     ...
     * Open wave device.
     */
    mr = waveInOpen(&wmme_strm->hWave.In, wf->dev_info[prm->rec_id].deviceId,
                    &wfx, (DWORD_PTR)wmme_strm->hEvent, 0, flag);
    if (mr != MMSYSERR_NOERROR) {
    ...
    PJ_LOG(4,
           (THIS_FILE,
            ...
            "(format=%s, clock_rate=%d, "
            "channel_count=%d, samples_per_frame=%d (%dms))",
            wf->dev_info[prm->rec_id].info.name, get_fmt_name(prm->ext_fmt.id),
            prm->clock_rate, prm->channel_count, prm->samples_per_frame,
            prm->samples_per_frame * 1000 / prm->clock_rate));
                 ...
                 ++burst)
            {
                ...
                if (strm->fmt_id == PJMEDIA_FORMAT_L16) {
                    ...
                    if (frame->type == PJMEDIA_FRAME_TYPE_NONE) {
                        ...
                        unsigned sz;
                        sz = pjmedia_frame_ext_copy_payload(
                          strm->xfrm, buffer, strm->bytes_per_frame);
                        if (sz < strm->bytes_per_frame) {
                            pj_memset((char*)buffer + sz, strm->silence_char,
...
#    endif
            ...
            {
                char* buffer =
                  (char*)wmme_strm->WaveHdr[wmme_strm->dwBufIdx].lpData;
                unsigned cap_len =
                ...
                */
                ...
                if (strm->fmt_id == PJMEDIA_FORMAT_L16) {
                    ...
                    pjmedia_frame_ext_append_subframe(
                      strm->xfrm, buffer, strm->bytes_per_frame * 8,
                ...
                if (mr != MMSYSERR_NOERROR) {
                    ...
                    PJ_PERROR(
                      3, (THIS_FILE, status, "Failed to add recording buffer"));
                    break;
                }
    ...
    if (status != PJ_SUCCESS) {
        ...
        ts = (signalled_dir == PJMEDIA_DIR_PLAYBACK) ?
               &strm->play_strm.timestamp :
               &strm->rec_strm.timestamp;
                                         ...
                                         pjmedia_aud_stream** p_aud_strm)
{
    ...
    if (param->dir & PJMEDIA_DIR_PLAYBACK) {
        ...
        status = init_player_stream(wf, strm->pool, strm, &strm->play_strm,
                                    param, buf_count);
    ...
    if (param->dir & PJMEDIA_DIR_CAPTURE) {
        ...
        status = init_capture_stream(wf, strm->pool, strm, &strm->rec_strm,
                                     param, buf_count);
    ...
    if (strm->fmt_id != PJMEDIA_FORMAT_L16) {
        ...
        strm->xfrm = (pjmedia_frame_ext*)pj_pool_alloc(pool, strm->xfrm_size);
    }
