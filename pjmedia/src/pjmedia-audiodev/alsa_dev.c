...
static pjmedia_aud_dev_factory_op alsa_factory_op = {
    &alsa_factory_init,          &alsa_factory_destroy,
    &alsa_factory_get_dev_count, &alsa_factory_get_dev_info,
    &alsa_factory_default_param, &alsa_factory_create_stream,
    &alsa_factory_refresh
};

static pjmedia_aud_stream_op alsa_stream_op = {
    &alsa_stream_get_param, &alsa_stream_get_cap, &alsa_stream_set_cap,
    &alsa_stream_start,     &alsa_stream_stop,    &alsa_stream_destroy
};

#    if ENABLE_TRACING == 0
static void null_alsa_error_handler(const char* file, int line,
...
static pj_status_t add_dev(struct alsa_factory* af, const char* dev_name)
{
    ...
    if (pb_result < 0 && ca_result < 0) {
        TRACE_((THIS_FILE,
                "Unable to open sound device %s, setting "
         ...
         elem = snd_mixer_elem_next(elem))
    {
        ...
        if (snd_mixer_selem_is_active(elem)) {
            if (snd_mixer_selem_has_playback_volume(elem)) {
                ...
                TRACE_(
                  (THIS_FILE, "Playback mixer name: %s", af->pb_mixer_name));
            }
            if (snd_mixer_selem_has_capture_volume(elem)) {
                ...
                TRACE_(
                  (THIS_FILE, "Capture mixer name: %s", af->cap_mixer_name));
            }
        }
    }
    snd_mixer_close(handle);
...
static int ca_thread_func(void* arg)
{
    ...
    PJ_LOG(5, (THIS_FILE,
               "ca_thread_func(%u): Set thread priority "
    ...
    if (result) {
        if (result == EPERM)
            PJ_LOG(5, (THIS_FILE,
                       "Unable to increase thread priority, "
                       "root access needed."));
        else
            PJ_LOG(5, (THIS_FILE,
                       "Unable to increase thread priority, "
                                 ...
                                 const pjmedia_aud_param* param)
{
    ...
    result =
      snd_pcm_open(&stream->pb_pcm, stream->af->devs[param->play_id].name,
    ...
    TRACE_(
      (THIS_FILE, "open_playback: set channels: %d", param->channel_count));
    result = snd_pcm_hw_params_set_channels(stream->pb_pcm, params,
                                            param->channel_count);
    if (result < 0) {
        PJ_LOG(3,
               (THIS_FILE,
                "Unable to set a channel count of %d for "
                "playback device '%s'",
                param->channel_count, stream->af->devs[param->play_id].name));
    ...
    stream->pb_frames =
      (snd_pcm_uframes_t)param->samples_per_frame / param->channel_count;
    TRACE_(
      (THIS_FILE, "open_playback: set period size: %d", stream->pb_frames));
    ...
    //			tmp_period_size : stream->pb_frames;
    TRACE_(
      (THIS_FILE, "open_playback: period size set to: %d", tmp_period_size));
    ...
    /* Set our buffer */
    stream->pb_buf_size =
      stream->pb_frames * param->channel_count * (param->bits_per_sample / 8);
    stream->pb_buf = (char*)pj_pool_alloc(stream->pool, stream->pb_buf_size);

    TRACE_(
      (THIS_FILE, "open_playback: buffer size set to: %d", (int)tmp_buf_size));
    ...
    PJ_LOG(5, (THIS_FILE,
               "Opened device alsa(%s) for playing, sample rate=%d"
               ", ch=%d, bits=%d, period size=%d frames, latency=%d ms",
               stream->af->devs[param->play_id].name, rate,
               param->channel_count, param->bits_per_sample, stream->pb_frames,
               (int)stream->param.output_latency_ms));
                                ...
                                const pjmedia_aud_param* param)
{
    ...
    result = snd_pcm_open(&stream->ca_pcm, stream->af->devs[param->rec_id].name,
    ...
    /* Set number of channels */
    TRACE_((THIS_FILE, "open_capture: set channels: %d", param->channel_count));
    ...
    if (result < 0) {
        PJ_LOG(3, (THIS_FILE,
                   "Unable to set a channel count of %d for "
                   "capture device '%s'",
                   param->channel_count, stream->af->devs[param->rec_id].name));
    ...
    stream->ca_frames =
      (snd_pcm_uframes_t)param->samples_per_frame / param->channel_count;
    TRACE_((THIS_FILE, "open_capture: set period size: %d", stream->ca_frames));
    ...
    //			tmp_period_size : stream->ca_frames;
    TRACE_(
      (THIS_FILE, "open_capture: period size set to: %d", tmp_period_size));
    ...
    /* Set our buffer */
    stream->ca_buf_size =
      stream->ca_frames * param->channel_count * (param->bits_per_sample / 8);
    stream->ca_buf = (char*)pj_pool_alloc(stream->pool, stream->ca_buf_size);

    TRACE_(
      (THIS_FILE, "open_capture: buffer size set to: %d", (int)tmp_buf_size));
    ...
    PJ_LOG(5, (THIS_FILE,
               "Opened device alsa(%s) for capture, sample rate=%d"
               ", ch=%d, bits=%d, period size=%d frames, latency=%d ms",
               stream->af->devs[param->rec_id].name, rate, param->channel_count,
                                       ...
                                       const void* value)
{
         ...
         pj_ansi_strlen(af->cap_mixer_name)))
    {
        ...
        if (cap == PJMEDIA_AUD_DEV_CAP_OUTPUT_VOLUME_SETTING) {
            ...
            if (snd_mixer_selem_set_playback_volume_all(elem, vol * max / 100) <
                0) {
                return PJMEDIA_EAUD_SYSERR;
            }
        } else if (cap == PJMEDIA_AUD_DEV_CAP_INPUT_VOLUME_SETTING) {
            snd_mixer_selem_get_capture_volume_range(elem, &min, &max);
            if (snd_mixer_selem_set_capture_volume_all(elem, vol * max / 100) <
                0) {
                return PJMEDIA_EAUD_SYSERR;
            }
        }
...
static pj_status_t alsa_stream_start(pjmedia_aud_stream* s)
{
    ...
    if (stream->param.dir & PJMEDIA_DIR_PLAYBACK) {
        status = pj_thread_create(stream->pool, "alsasound_playback",
                                  pb_thread_func, stream,
                                  0,  // ZERO,
                                  0, &stream->pb_thread);
        if (status != PJ_SUCCESS)
            return status;
...
static pj_status_t alsa_stream_stop(pjmedia_aud_stream* s)
{
    ...
    if (stream->pb_thread) {
        TRACE_((THIS_FILE,
                ...
                (unsigned)syscall(SYS_gettid)));
        pj_thread_join(stream->pb_thread);
        TRACE_((THIS_FILE, "alsa_stream_stop(%u): playback stopped.",
                (unsigned)syscall(SYS_gettid)));
        pj_thread_destroy(stream->pb_thread);
        stream->pb_thread = NULL;
    }
    ...
    if (stream->ca_thread) {
        TRACE_((THIS_FILE, "alsa_stream_stop(%u): Waiting for capture to stop.",
                (unsigned)syscall(SYS_gettid)));
        pj_thread_join(stream->ca_thread);
        TRACE_((THIS_FILE, "alsa_stream_stop(%u): capture stopped.",
                (unsigned)syscall(SYS_gettid)));
