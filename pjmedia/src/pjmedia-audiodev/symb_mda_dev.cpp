...
CPjAudioInputEngine::CPjAudioInputEngine(struct mda_stream* parent_strm,
    ...
    : state_(STATE_INACTIVE),
      ...
      lastError_(KErrNone),
      timeStamp_(0),
      frameLen_(parent_strm->param.samples_per_frame * BYTES_PER_SAMPLE),
      frameRecBuf_(NULL),
      frameRecBufLen_(0)
{}
...
CPjAudioInputEngine* CPjAudioInputEngine::NewLC(struct mda_stream* parent,
                                                ...
                                                void* user_data)
{
    CPjAudioInputEngine* self =
      new (ELeave) CPjAudioInputEngine(parent, rec_cb, user_data);
...
pj_status_t CPjAudioInputEngine::StartRecord()
{
    ...
    PJ_LOG(4,
           (THIS_FILE,
            "Opening sound device for capture, "
            "clock rate=%d, channel count=%d..",
            parentStrm_->param.clock_rate, parentStrm_->param.channel_count));
...
void CPjAudioInputEngine::MaiscOpenComplete(TInt aError)
{
    ...
    if (parentStrm_->param.flags & PJMEDIA_AUD_DEV_CAP_INPUT_VOLUME_SETTING) {
    ...
    iInputStream_->SetPriority(EPriorityNormal, EMdaPriorityPreferenceTime);
...
void CPjAudioInputEngine::MaiscBufferCopied(TInt aError, const TDesC8& aBuffer)
{
    lastError_ = aError;
    if (aError != KErrNone) {
    ...
    if (frameRecBufLen_ || aBuffer.Length() < frameLen_) {
        pj_memcpy(frameRecBuf_ + frameRecBufLen_, (void*)aBuffer.Ptr(),
                  aBuffer.Length());
...
{
    frameBufSize_ = parentStrm_->param.samples_per_frame * BYTES_PER_SAMPLE;
...
CPjAudioOutputEngine* CPjAudioOutputEngine::NewLC(
  struct mda_stream* parent_strm, pjmedia_aud_play_cb play_cb, void* user_data)
{
    CPjAudioOutputEngine* self =
      new (ELeave) CPjAudioOutputEngine(parent_strm, play_cb, user_data);
    CleanupStack::PushL(self);
    self->ConstructL();
    return self;
}

CPjAudioOutputEngine* CPjAudioOutputEngine::NewL(struct mda_stream* parent_strm,
                                                 pjmedia_aud_play_cb play_cb,
                                                 void* user_data)
{
...
pj_status_t CPjAudioOutputEngine::StartPlay()
{
    ...
    PJ_LOG(4,
           (THIS_FILE,
            "Opening sound device for playback, "
            "clock rate=%d, channel count=%d..",
            parentStrm_->param.clock_rate, parentStrm_->param.channel_count));
...
void CPjAudioOutputEngine::MaoscOpenComplete(TInt aError)
{
    ...
    if (aError == KErrNone) {
        ...
        iSettings.iChannels = get_channel_cap(parentStrm_->param.channel_count);
...
void CPjAudioOutputEngine::MaoscBufferCopied(TInt aError, const TDesC8& aBuffer)
{
                                         ...
                                         pjmedia_aud_stream** p_aud_strm)
{
    ...
    if (strm->param.dir & PJMEDIA_DIR_PLAYBACK) {
        TRAPD(err, strm->out_engine =
                     CPjAudioOutputEngine::NewL(strm, play_cb, user_data));
        if (err != KErrNone) {
            pj_pool_release(pool);
            return PJ_RETURN_OS_ERROR(err);
        }
    ...
    if (strm->param.dir & PJMEDIA_DIR_CAPTURE) {
        TRAPD(err, strm->in_engine =
                     CPjAudioInputEngine::NewL(strm, rec_cb, user_data));
        if (err != KErrNone) {
            strm->in_engine = NULL;
