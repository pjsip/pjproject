...
static pjmedia_aud_dev_factory_op opensl_op = {
    &opensl_init,         &opensl_destroy,       &opensl_get_dev_count,
    &opensl_get_dev_info, &opensl_default_param, &opensl_create_stream,
    &opensl_refresh
};
...
static pj_status_t opensl_init(pjmedia_aud_dev_factory* f)
{
    ...
    result =
      (*pa->engineObject)
        ->GetInterface(pa->engineObject, SL_IID_ENGINE, &pa->engineEngine);
    if (result != SL_RESULT_SUCCESS) {
    ...
    /* Create output mix */
    result = (*pa->engineEngine)
               ->CreateOutputMix(pa->engineEngine, &pa->outputMixObject, 0,
                                 NULL, NULL);
    if (result != SL_RESULT_SUCCESS) {
    ...
    result =
      (*pa->outputMixObject)->Realize(pa->outputMixObject, SL_BOOLEAN_FALSE);
    if (result != SL_RESULT_SUCCESS) {
...
#    endif
        ...
        result =
          (*pa->engineEngine)
            ->CreateAudioPlayer(pa->engineEngine, &stream->playerObj, &audioSrc,
                                &audioSnk, numIface, ids, req);
        if (result != SL_RESULT_SUCCESS) {
...
#    ifdef __ANDROID__
        ...
        result = (*stream->playerObj)
                   ->GetInterface(stream->playerObj,
...
#    endif
        ...
        /* Realize the player */
        result =
          (*stream->playerObj)->Realize(stream->playerObj, SL_BOOLEAN_FALSE);
        if (result != SL_RESULT_SUCCESS) {
        ...
        result =
          (*stream->playerObj)
            ->GetInterface(stream->playerObj, SL_IID_PLAY, &stream->playerPlay);
        if (result != SL_RESULT_SUCCESS) {
        ...
        result = (*stream->playerObj)
                   ->GetInterface(stream->playerObj, SL_IID_BUFFERQUEUE,
                                  &stream->playerBufQ);
        if (result != SL_RESULT_SUCCESS) {
        ...
        result = (*stream->playerObj)
                   ->GetInterface(stream->playerObj, SL_IID_VOLUME,
                                  &stream->playerVol);

        /* Register callback on the buffer queue */
        result = (*stream->playerBufQ)
                   ->RegisterCallback(stream->playerBufQ, bqPlayerCallback,
                                      (void*)stream);
        if (result != SL_RESULT_SUCCESS) {
        ...
        for (i = 0; i < NUM_BUFFERS; i++) {
            stream->playerBuffer[i] =
              (char*)pj_pool_alloc(stream->pool, stream->playerBufferSize);
        }
    }
...
#    endif
        ...
        result =
          (*pa->engineEngine)
            ->CreateAudioRecorder(pa->engineEngine, &stream->recordObj,
                                  &audioSrc, &audioSnk, numIface, ids, req);
        if (result != SL_RESULT_SUCCESS) {
...
#    ifdef __ANDROID__
        /* Set Android configuration */
        result = (*stream->recordObj)
                   ->GetInterface(stream->recordObj,
...
#        endif
            result = (*recorderConfig)
                       ->SetConfiguration(recorderConfig,
...
#    endif
        ...
        result =
          (*stream->recordObj)->Realize(stream->recordObj, SL_BOOLEAN_FALSE);
        if (result != SL_RESULT_SUCCESS) {
        ...
        result = (*stream->recordObj)
                   ->GetInterface(stream->recordObj, SL_IID_RECORD,
                                  &stream->recordRecord);
        if (result != SL_RESULT_SUCCESS) {
        ...
        /* Get the buffer queue interface */
        result = (*stream->recordObj)
                   ->GetInterface(stream->recordObj, W_SL_IID_BUFFERQUEUE,
                                  &stream->recordBufQ);
        if (result != SL_RESULT_SUCCESS) {
        ...
        /* Register callback on the buffer queue */
        result = (*stream->recordBufQ)
                   ->RegisterCallback(stream->recordBufQ, bqRecorderCallback,
                                      (void*)stream);
        if (result != SL_RESULT_SUCCESS) {
        ...
        for (i = 0; i < NUM_BUFFERS; i++) {
            stream->recordBuffer[i] =
              (char*)pj_pool_alloc(stream->pool, stream->recordBufferSize);
        }
    }
        ...
        if (strm->playerVol) {
            ...
            if (res == SL_RESULT_SUCCESS) {
                vol = (SLmillibel)(*(int*)value *
                                     ((int)mvol - SL_MILLIBEL_MIN) / 100 +
                                   SL_MILLIBEL_MIN);
                res = (*strm->playerVol)->SetVolumeLevel(strm->playerVol, vol);
                if (res == SL_RESULT_SUCCESS)
                    return PJ_SUCCESS;
            }
        }
...
static pj_status_t strm_start(pjmedia_aud_stream* s)
{
    ...
    if (stream->recordBufQ && stream->recordRecord) {
        ...
        for (i = 0; i < NUM_BUFFERS; i++) {
            result = (*stream->recordBufQ)
                       ->Enqueue(stream->recordBufQ, stream->recordBuffer[i],
        ...
        result =
          (*stream->recordRecord)
            ->SetRecordState(stream->recordRecord, SL_RECORDSTATE_RECORDING);
        if (result != SL_RESULT_SUCCESS) {
    ...
    if (stream->playerPlay && stream->playerBufQ) {
        ...
        result = (*stream->playerPlay)
                   ->SetPlayState(stream->playerPlay, SL_PLAYSTATE_PLAYING);
        if (result != SL_RESULT_SUCCESS) {
        ...
        for (i = 0; i < NUM_BUFFERS; i++) {
            ...
            result = (*stream->playerBufQ)
                       ->Enqueue(stream->playerBufQ, stream->playerBuffer[i],
...
static pj_status_t strm_stop(pjmedia_aud_stream* s)
{
    ...
    if (stream->recordBufQ && stream->recordRecord) {
        ...
        (*stream->recordRecord)
          ->SetRecordState(stream->recordRecord, SL_RECORDSTATE_STOPPED);
    ...
    if (stream->playerBufQ && stream->playerPlay) {
        ...
        /*
                ...
                result = (*stream->playerBufQ)->GetState(stream->playerBufQ,
           &state); while (state.count) {
                    (*stream->playerBufQ)->GetState(stream->playerBufQ, &state);
                } */
        /* Stop player */
        (*stream->playerPlay)
          ->SetPlayState(stream->playerPlay, SL_PLAYSTATE_STOPPED);
    }
