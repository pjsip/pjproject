...
static pjmedia_aud_dev_factory_op bb10_factory_op = {
    &bb10_factory_init,          &bb10_factory_destroy,
    &bb10_factory_get_dev_count, &bb10_factory_get_dev_info,
    &bb10_factory_default_param, &bb10_factory_create_stream,
    &bb10_factory_refresh
};

static pjmedia_aud_stream_op bb10_stream_op = {
    &bb10_stream_get_param, &bb10_stream_get_cap, &bb10_stream_set_cap,
    &bb10_stream_start,     &bb10_stream_stop,    &bb10_stream_destroy
};
...
static pj_status_t bb10_add_dev(struct bb10_factory* af)
{
    ...
    if ((pb_result = audio_manager_snd_pcm_open_name(
           AUDIO_TYPE_VIDEO_CHAT, &pcm_handle, &handle, (char*)"voice",
    ...
    if ((ca_result = audio_manager_snd_pcm_open_name(
           AUDIO_TYPE_VIDEO_CHAT, &pcm_handle, &handle, (char*)"voice",
                                             ...
                                             pjmedia_aud_dev_info* info)
{
    ...
    info->caps = PJMEDIA_AUD_DEV_CAP_INPUT_LATENCY |
                 PJMEDIA_AUD_DEV_CAP_OUTPUT_LATENCY | PJMEDIA_AUD_DEV_CAP_EC;
                                              ...
                                              pjmedia_aud_param* param)
{
    ...
    TRACE_((THIS_FILE,
            "bb10_factory_default_param clock = %d flags = %d"
            " spf = %d",
    ...
    while (!stream->quit) {
        ...
        if (result != size || result < 0) {
            ...
            if (snd_pcm_plugin_status(stream->pb_pcm, &status) < 0) {
                 ...
                 * continue */
                PJ_LOG(4,
                       (THIS_FILE, "underrun: playback channel status error"));
            } else {
...
static int ca_thread_func(void* arg)
{
    ...
    if ((result =
           snd_pcm_plugin_prepare(stream->ca_pcm, SND_PCM_CHANNEL_CAPTURE)) < 0)
    {
                                                 ...
                                                 bool speaker)
{
    ...
    if (speaker) {
        ret = audio_manager_set_handle_type(
          stream->pb_ctrl_audio_manager_handle, AUDIO_TYPE_VIDEO_CHAT,
          AUDIO_DEVICE_SPEAKER, AUDIO_DEVICE_DEFAULT);
    } else {
        ret = audio_manager_set_handle_type(
          stream->pb_ctrl_audio_manager_handle, AUDIO_TYPE_VOICE,
          AUDIO_DEVICE_DEFAULT, AUDIO_DEVICE_DEFAULT);
    }
    ...
    if (ret == 0) {
        ret = audio_manager_set_handle_routing_conditions(
          stream->pb_ctrl_audio_manager_handle, SETTINGS_NEVER_RESET);
    }

    if (ret != 0) {
        TRACE_(
          (THIS_FILE, "audio_manager_set_handle_type error ret = %d", ret));
        return PJMEDIA_EAUD_SYSERR;
    } else {
        return PJ_SUCCESS;
    }
                                      ...
                                      const pjmedia_aud_param* param)
{
    ...
    if ((ret = audio_manager_snd_pcm_open_name(
           AUDIO_TYPE_VIDEO_CHAT, &stream->pb_pcm,
           &stream->pb_audio_manager_handle, (char*)"voice",
           SND_PCM_OPEN_PLAYBACK)) < 0)
    {
    ...
    pp.buf.block.frag_size =
      param->samples_per_frame * param->bits_per_sample / 8;
                                     ...
                                     const pjmedia_aud_param* param)
{
    ...
    if ((ret = audio_manager_snd_pcm_open_name(
           AUDIO_TYPE_VIDEO_CHAT, &stream->ca_pcm,
           &stream->ca_audio_manager_handle, (char*)"voice",
           SND_PCM_OPEN_CAPTURE)) < 0)
    {
        TRACE_((THIS_FILE, "audio_manager_snd_pcm_open_name ret = %d", ret));
        return PJMEDIA_EAUD_SYSERR;
    ...
    rate = param->clock_rate;
    stream->ca_frames =
      (unsigned long)param->samples_per_frame / param->channel_count;
                                              ...
                                              pjmedia_aud_stream** p_strm)
{
    ...
    {
        status =
          bb10_stream_set_cap(&stream->base, PJMEDIA_AUD_DEV_CAP_OUTPUT_ROUTE,
                              &param->output_route);
        if (status != PJ_SUCCESS) {
...
static pj_status_t bb10_stream_start(pjmedia_aud_stream* s)
{
    ...
    if (stream->param.dir & PJMEDIA_DIR_PLAYBACK) {
        status =
          pj_thread_create(stream->pool, "bb10sound_playback", pb_thread_func,
                           stream, 0, 0, &stream->pb_thread);
        if (status != PJ_SUCCESS)
            return status;
    }
    ...
    if (stream->param.dir & PJMEDIA_DIR_CAPTURE) {
        status =
          pj_thread_create(stream->pool, "bb10sound_playback", ca_thread_func,
                           stream, 0, 0, &stream->ca_thread);
        if (status != PJ_SUCCESS) {
            stream->quit = PJ_TRUE;
