...
static pjmedia_aud_dev_factory_op factory_op = {
    &wasapi_factory_init,          &wasapi_factory_destroy,
    &wasapi_factory_get_dev_count, &wasapi_factory_get_dev_info,
    &wasapi_factory_default_param, &wasapi_factory_create_stream,
    &wasapi_factory_refresh
};

static pjmedia_aud_stream_op stream_op = {
    &wasapi_stream_get_param, &wasapi_stream_get_cap, &wasapi_stream_set_cap,
    &wasapi_stream_start,     &wasapi_stream_stop,    &wasapi_stream_destroy
};
                                     ...
                                     struct wasapi_stream* strm)
{
    if (prm->ext_fmt.id == PJMEDIA_FORMAT_L16) {
        ...
        wfx->nBlockAlign =
          (pj_uint16_t)(prm->channel_count * strm->bytes_per_sample);
        wfx->nAvgBytesPerSec =
          prm->clock_rate * prm->channel_count * strm->bytes_per_sample;
...
static int PJ_THREAD_FUNC wasapi_dev_thread(void* arg)
{
    ...
    while (status == PJ_SUCCESS) {
        ...
        rc =
          WaitForMultipleObjectsEx(eventCount, events, FALSE, INFINITE, FALSE);
        ...
        if (signalled_dir == PJMEDIA_DIR_PLAYBACK) {
            ...
            incoming_frame =
              ...
              (strm->bytes_per_sample * strm->param.channel_count);
            ...
            while (packet_size) {
                ...
                hr = strm->cap_client->GetBuffer(
                  (BYTE**)&cap_buf, &next_frame_size, &cap_flag, NULL, NULL);

                if (FAILED(hr) || (next_frame_size == 0)) {
                    PJ_LOG(4, (THIS_FILE,
                               "Error getting next buffer, \
			       next frame size : %d",
                               next_frame_size));
                    packet_size = 0;
                    continue;
                ...
                if (nsamples >= strm->param.samples_per_frame) {
                    ...
                    if (strm->cap_buf_count) {
                        ...
                        chunk_count =
                          strm->param.samples_per_frame - strm->cap_buf_count;

                        pjmedia_copy_samples(
                          strm->cap_buf + strm->cap_buf_count,
                     ...
                     */
                    pjmedia_copy_samples(
                      strm->cap_buf + strm->cap_buf_count, (pj_int16_t*)cap_buf,
                      next_frame_size * strm->param.channel_count);
...
#    if defined(USE_ASYNC_ACTIVATE)
    ...
    hr =
      ActivateAudioInterfaceAsync(ws->cap_id->Data(), __uuidof(IAudioClient2),
                                  NULL, ws->cap_aud_act.Get(), &async_op);
                                    ...
                                    const pjmedia_aud_param* prm)
{
    ...
    hr = ws->default_cap_dev->Initialize(
      AUDCLNT_SHAREMODE_SHARED, stream_flags,
      // 2000 * 10000,
      ptime * ws->param.input_latency_ms * 10000, 0, &wfx, NULL);
    ...
    PJ_LOG(4, (THIS_FILE,
               ...
               prm->clock_rate, ws->param.input_latency_ms, prm->channel_count,
               prm->samples_per_frame, ptime));
...
#    if defined(USE_ASYNC_ACTIVATE)
    ...
    hr = ActivateAudioInterfaceAsync(ws->pb_id->Data(), __uuidof(IAudioClient2),
                                     NULL, ws->pb_aud_act.Get(), &async_op);
                                     ...
                                     const pjmedia_aud_param* prm)
{
    ...
    hr = ws->default_pb_dev->Initialize(AUDCLNT_SHAREMODE_SHARED, stream_flags,
    ...
    PJ_LOG(4, (THIS_FILE,
               ...
               "clock_rate=%d, latency=%d, "
               "channel_count=%d, samples_per_frame=%d (%dms))",
               prm->clock_rate, ws->param.output_latency_ms, prm->channel_count,
               prm->samples_per_frame, ptime));
...
#    endif
    ...
    adi->routes =
      PJMEDIA_AUD_DEV_ROUTE_DEFAULT | PJMEDIA_AUD_DEV_ROUTE_EARPIECE |
      PJMEDIA_AUD_DEV_ROUTE_LOUDSPEAKER | PJMEDIA_AUD_DEV_ROUTE_BLUETOOTH;
                                                ...
                                                pjmedia_aud_param* param)
{
    ...
    TRACE_((THIS_FILE,
            "wasapi_factory_default_param clock = %d flags = %d"
            " spf = %d",
            param->clock_rate, param->flags, param->samples_per_frame));
                                                ...
                                                pjmedia_aud_stream** p_strm)
{
    ...
    strm->quit_event =
      CreateEventEx(NULL, NULL, CREATE_EVENT_MANUAL_RESET, EVENT_ALL_ACCESS);
    if (!strm->quit_event) {
...
static pj_status_t wasapi_stream_stop(pjmedia_aud_stream* strm)
{
    ...
    if (ws->default_pb_dev) {
        ...
        if (FAILED(hr)) {
            PJ_LOG(
              4, (THIS_FILE, "Error stopping wasapi playback stream:0x%x", hr));
        } else {
    ...
    if (ws->default_cap_dev) {
        ...
        if (FAILED(hr_tmp)) {
            hr = hr_tmp;
            PJ_LOG(
              4, (THIS_FILE, "Error stopping wasapi capture stream:0x%x", hr));
        } else {
