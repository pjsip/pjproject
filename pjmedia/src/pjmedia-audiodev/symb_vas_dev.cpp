...
static pj_status_t stream_set_cap(pjmedia_aud_stream* strm,
                                  pjmedia_aud_dev_cap cap, const void* value);
...
static pjmedia_aud_dev_factory_op factory_op = {
    ...
    &factory_get_dev_info, &factory_default_param, &factory_create_stream,
    &factory_refresh
};

static pjmedia_aud_stream_op stream_op = { &stream_get_param, &stream_get_cap,
                                           &stream_set_cap,   &stream_start,
                                           &stream_stop,      &stream_destroy };
...
class CPjAudioEngine : public CBase,
   ...
   public:
    ...
    static CPjAudioEngine* NewL(struct vas_stream* parent_strm,
                                PjAudioCallback rec_cb, PjAudioCallback play_cb,
                                void* user_data,
   ...
   private:
    CPjAudioEngine(struct vas_stream* parent_strm, PjAudioCallback rec_cb,
                   PjAudioCallback play_cb, void* user_data,
                   const CPjAudioSetting& setting);
    void ConstructL();
    ...
    void Event(const CVoIPAudioDownlinkStream& aSrc, TInt aEventType,
               TInt aError);

    // From MVoIPUplinkObserver
    void EmptyBuffer(const CVoIPAudioUplinkStream& aSrc,
                     CVoIPDataBuffer* aBuffer);
    void Event(const CVoIPAudioUplinkStream& aSrc, TInt aEventType,
               TInt aError);
...
CPjAudioEngine* CPjAudioEngine::NewL(struct vas_stream* parent_strm,
                                     ...
                                     const CPjAudioSetting& setting)
{
    CPjAudioEngine* self = new (ELeave)
      CPjAudioEngine(parent_strm, rec_cb, play_cb, user_data, setting);
...
void CPjAudioEngine::ConstructL()
{
    ...
    if (parentStrm_->param.dir != PJMEDIA_DIR_CAPTURE) {
        err = iFactory->CreateDownlinkStream(
          ver, CVoIPUtilityFactory::EVoIPCall, iVoIPDnlink);
        User::LeaveIfError(err);
    }

    if (parentStrm_->param.dir != PJMEDIA_DIR_PLAYBACK) {
        err = iFactory->CreateUplinkStream(ver, CVoIPUtilityFactory::EVoIPCall,
                                           iVoIPUplink);
        User::LeaveIfError(err);
    }
}
...
CPjAudioEngine::CPjAudioEngine(struct vas_stream* parent_strm,
                               PjAudioCallback rec_cb, PjAudioCallback play_cb,
                               void* user_data, const CPjAudioSetting& setting)
...
TInt CPjAudioEngine::StartPlay()
{
    ...
    case EG711: {
        CVoIPG711DecoderIntfc* g711dec_if = (CVoIPG711DecoderIntfc*)dec_fmt_if;
        err =
          g711dec_if->SetMode((CVoIPFormatIntfc::TG711CodecMode)setting_.mode);
    } break;

    case EILBC: {
        CVoIPILBCDecoderIntfc* ilbcdec_if = (CVoIPILBCDecoderIntfc*)dec_fmt_if;
        err =
          ilbcdec_if->SetMode((CVoIPFormatIntfc::TILBCCodecMode)setting_.mode);
    } break;
...
TInt CPjAudioEngine::StartRec()
{
    ...
    switch (setting_.format) {
    case EG711: {
        CVoIPG711EncoderIntfc* g711enc_if = (CVoIPG711EncoderIntfc*)enc_fmt_if;
        err =
          g711enc_if->SetMode((CVoIPFormatIntfc::TG711CodecMode)setting_.mode);
    } break;

    case EILBC: {
        CVoIPILBCEncoderIntfc* ilbcenc_if = (CVoIPILBCEncoderIntfc*)enc_fmt_if;
        err =
          ilbcenc_if->SetMode((CVoIPFormatIntfc::TILBCCodecMode)setting_.mode);
    } break;
...
TInt CPjAudioEngine::ActivateSpeaker(TBool active)
{
    ...
    if (iVoIPDnlink) {
        err = iVoIPDnlink->SetAudioDevice(
          active ? CVoIPAudioDownlinkStream::ELoudSpeaker :
...
static void PlayCbPcm2(CVoIPDataBuffer* buf, void* user_data)
{
    ...
    if (f.type != PJMEDIA_FRAME_TYPE_AUDIO) {
        pjmedia_zero_samples((pj_int16_t*)f.buf, strm->param.samples_per_frame);
    }
    f.size = strm->param.samples_per_frame << 1;
...
static void RecCbPcm(CVoIPDataBuffer* buf, void* user_data)
{
    ...
    while (samples_processed < vas_g711_frame_len) {
        ...
        samples_req = (strm->param.samples_per_frame /
                       strm->param.channel_count / strm->resample_factor) -
                      strm->rec_buf_len;
                                   ...
                                   strm->resample_factor)
        {
            ...
            if (strm->rec_resample) {
                ...
                while (resampled < strm->rec_buf_len) {
                    pjmedia_resample_run(
                      strm->rec_resample, &strm->rec_buf[resampled],
                      strm->pcm_buf + resampled * strm->resample_factor);
                    resampled += 80;
                }
                f.buf = strm->pcm_buf;
            } else {
            ...
            if (strm->param.channel_count != 1) {
                pjmedia_convert_channel_1ton(
                  (pj_int16_t*)f.buf, (pj_int16_t*)f.buf,
                  strm->param.channel_count,
                  strm->param.samples_per_frame / strm->param.channel_count, 0);
            }
...
static void PlayCbPcm(CVoIPDataBuffer* buf, void* user_data)
{
    ...
    while (samples_processed < g711_frame_len) {
        ...
        if (strm->play_buf_len == 0) {
            ...
            if (strm->param.channel_count != 1) {
                pjmedia_convert_channel_nto1(
                  (pj_int16_t*)f.buf, (pj_int16_t*)f.buf,
                  strm->param.channel_count, strm->param.samples_per_frame,
                  PJ_FALSE, 0);
            }
        ...
        pjmedia_ulaw_encode((pj_uint8_t*)&strm->play_buf[strm->play_buf_start],
                            &strm->play_buf[strm->play_buf_start], tmp);
...
static void RecCb(CVoIPDataBuffer* buf, void* user_data)
{
    ...
    case PJMEDIA_FORMAT_G729: {
        ...
        if (buffer[0] != 0) {
            ...
            const TDesC8& p = bitstream->CompressG729Frame(
              buffer.Right(src_len), src_len == SID_LEN);

            pjmedia_frame_ext_append_subframe(frame, p.Ptr(), p.Length() << 3,
                                              80);
    ...
    case PJMEDIA_FORMAT_ILBC: {
        ...
        samples_got = strm->param.ext_fmt.det.aud.avg_bps == 15200 ? 160 : 240;
        ...
        if (buffer[0] != 0) {
            ...
            pjmedia_frame_ext_append_subframe(frame, p, len << 3, samples_got);
        } else { /* We got null frame. */
    ...
    case PJMEDIA_FORMAT_PCMA: {
        ...
        while (samples_processed < vas_g711_frame_len) {
            unsigned tmp;
            const pj_uint8_t* pb =
              (const pj_uint8_t*)buffer.Ptr() + 2 + samples_processed;
...
static void PlayCb(CVoIPDataBuffer* buf, void* user_data)
{
    ...
    case PJMEDIA_FORMAT_G729: {
        ...
        if (frame->base.type == PJMEDIA_FRAME_TYPE_EXTENDED) {
            ...
            if (sf->data && sf->bitlen) {
                ...
                const TDesC8& dst = bitstream->ExpandG729Frame(src, sid_frame);
                if (sid_frame) {
                    buffer.Append(2);
                    buffer.Append(0);
                } else {
    ...
    case PJMEDIA_FORMAT_ILBC: {
        ...
        if (frame->base.type == PJMEDIA_FRAME_TYPE_EXTENDED) {
            ...
            /* VAS iLBC frame is 20ms or 30ms */
            frame_len = strm->param.ext_fmt.det.aud.avg_bps == 15200 ? 38 : 50;
            buffer.AppendFill(0, frame_len);
        }
    } break;
...
static pj_status_t factory_init(pjmedia_aud_dev_factory* f)
{
    ...
    af->dev_info.caps = PJMEDIA_AUD_DEV_CAP_EXT_FORMAT |
                        ...
                        PJMEDIA_AUD_DEV_CAP_VAD | PJMEDIA_AUD_DEV_CAP_CNG;
    af->dev_info.routes =
      PJMEDIA_AUD_DEV_ROUTE_EARPIECE | PJMEDIA_AUD_DEV_ROUTE_LOUDSPEAKER;
     ...
     * querying formats.
     */
    err = vas_factory_->CreateUplinkStream(
      vas_version, CVoIPUtilityFactory::EVoIPCall, vas_uplink);
    if (err != KErrNone)
        goto on_error;

    err = vas_factory_->CreateDownlinkStream(
      vas_version, CVoIPUtilityFactory::EVoIPCall, vas_dnlink);
    if (err != KErrNone)
        goto on_error;
    ...
    for (TInt i = 0; i < dnlink_formats.Count(); i++) {
        ...
        case EAMR_NB:
            pjmedia_format_init_audio(&ext_fmt, PJMEDIA_FORMAT_AMR, 8000, 1, 16,
                                      20, 7400, 12200);
        ...
        case EG729:
            pjmedia_format_init_audio(&ext_fmt, PJMEDIA_FORMAT_G729, 8000, 1,
                                      16, 20, 8000, 8000);
            af->dev_info.ext_fmt[ext_fmt_cnt] = ext_fmt;
        ...
        case EILBC:
            pjmedia_format_init_audio(&ext_fmt, PJMEDIA_FORMAT_ILBC, 8000, 1,
                                      16, 30, 13333, 15200);
            af->dev_info.ext_fmt[ext_fmt_cnt] = ext_fmt;
            // af->dev_info.ext_fmt[ext_fmt_cnt].vad = PJ_TRUE;
            break;
...
#    if PJMEDIA_AUDIO_DEV_SYMB_VAS_VERSION == 2
        case EG711_10MS:
#    endif
            pjmedia_format_init_audio(&ext_fmt, PJMEDIA_FORMAT_PCMU, 8000, 1,
                                      16, 20, 64000, 64000);
            af->dev_info.ext_fmt[ext_fmt_cnt] = ext_fmt;
            // af->dev_info.ext_fmt[ext_fmt_cnt].vad = PJ_FALSE;
            ++ext_fmt_cnt;
            pjmedia_format_init_audio(&ext_fmt, PJMEDIA_FORMAT_PCMA, 8000, 1,
                                      16, 20, 64000, 64000);
            af->dev_info.ext_fmt[ext_fmt_cnt] = ext_fmt;
            // af->dev_info.ext_fmt[ext_fmt_cnt].vad = PJ_FALSE;
            break;
                                         ...
                                         pjmedia_aud_stream** p_aud_strm)
{
    ...
    PJ_ASSERT_RETURN(
      param->clock_rate == 8000 ||
        (param->clock_rate == 16000 && param->ext_fmt.id == PJMEDIA_FORMAT_L16),
      PJ_EINVAL);
    ...
    PJ_ASSERT_RETURN(
      param->channel_count == 1 ||
        (param->channel_count == 2 && param->ext_fmt.id == PJMEDIA_FORMAT_L16),
      PJ_EINVAL);
...
#    endif
    ...
    vas_setting.plc =
      (strm->param.flags & PJMEDIA_AUD_DEV_CAP_PLC) && strm->param.plc_enabled;
...
#    endif
    ...
    strm->play_buf = (pj_int16_t*)pj_pool_zalloc(
      pool, (strm->param.samples_per_frame / strm->resample_factor /
             strm->param.channel_count)
              << 1);
    strm->play_buf_len = 0;
    strm->play_buf_start = 0;
        ...
        strm->param.ext_fmt.id == PJMEDIA_FORMAT_L16)
    {
        ...
        if (strm->param.dir & PJMEDIA_DIR_CAPTURE) {
            ...
            status = pjmedia_resample_create(pool, PJ_TRUE, PJ_FALSE, 1, 8000,
                                             strm->param.clock_rate, 80,
                                             &strm->rec_resample);
        ...
        if (strm->param.dir & PJMEDIA_DIR_PLAYBACK) {
            ...
            status = pjmedia_resample_create(
              pool, PJ_TRUE, PJ_FALSE, 1, strm->param.clock_rate, 8000,
              80 * strm->resample_factor, &strm->play_resample);
            if (status != PJ_SUCCESS)
                return status;
        }
    }
    ...
    {
        strm->pcm_buf =
          (pj_int16_t*)pj_pool_zalloc(pool, strm->param.samples_per_frame << 1);
    }

    /* Create the audio engine. */
    TRAPD(err, strm->engine = CPjAudioEngine::NewL(
                 strm, vas_rec_cb, vas_play_cb, strm, vas_setting));
    if (err != KErrNone) {
        pj_pool_release(pool);
        return PJ_RETURN_OS_ERROR(err);
    }
    ...
    case PJMEDIA_AUD_DEV_CAP_OUTPUT_ROUTE:
        if (strm->param.dir & PJMEDIA_DIR_PLAYBACK) {
            ...
            case PJMEDIA_AUD_DEV_ROUTE_EARPIECE:
                ...
                status =
                  (err == KErrNone) ? PJ_SUCCESS : PJ_RETURN_OS_ERROR(err);
                break;
        ...
        case PJMEDIA_AUD_DEV_CAP_INPUT_VOLUME_SETTING:
            if (strm->param.dir & PJMEDIA_DIR_CAPTURE) {
                ...
                if (max_gain > 0) {
                    ...
                    status = (err==KErrNone)?
        PJ_SUCCESS:PJ_RETURN_OS_ERROR(err); } else { status =
...
static pj_status_t stream_start(pjmedia_aud_stream* strm)
{
    ...
    if (stream->engine) {
        ...
        if (stream->engine->IsStarted()) {
            ...
            if (stream->param.flags & PJMEDIA_AUD_DEV_CAP_OUTPUT_VOLUME_SETTING)
            {
