...
static pj_status_t oboe_get_dev_info(pjmedia_aud_dev_factory* f, unsigned index,
                                     pjmedia_aud_dev_info* info);
static pj_status_t oboe_default_param(pjmedia_aud_dev_factory* f,
                                      unsigned index, pjmedia_aud_param* param);
...
static pjmedia_aud_dev_factory_op oboe_op = {
    &oboe_init,         &oboe_destroy,       &oboe_get_dev_count,
    &oboe_get_dev_info, &oboe_default_param, &oboe_create_stream,
    &oboe_refresh
};

static pjmedia_aud_stream_op oboe_strm_op = { &strm_get_param, &strm_get_cap,
...
static pj_status_t jni_init_ids()
{
    ...
    GET_SMETHOD_ID(jobjs.dev_info.cls, "PjAudioDevInfo", "RefreshDevices",
                   "(Landroid/content/Context;)V", jobjs.dev_info.m_refresh);
    GET_FIELD_ID(jobjs.dev_info.cls, "PjAudioDevInfo", "id", "I",
                 jobjs.dev_info.f_id);
    GET_FIELD_ID(jobjs.dev_info.cls, "PjAudioDevInfo", "name",
                 "Ljava/lang/String;", jobjs.dev_info.f_name);
    GET_FIELD_ID(jobjs.dev_info.cls, "PjAudioDevInfo", "direction", "I",
                 jobjs.dev_info.f_direction);
    GET_FIELD_ID(jobjs.dev_info.cls, "PjAudioDevInfo", "supportedClockRates",
                 "[I", jobjs.dev_info.f_sup_clockrates);
    GET_FIELD_ID(jobjs.dev_info.cls, "PjAudioDevInfo", "supportedChannelCounts",
                 "[I", jobjs.dev_info.f_sup_channels);
    ...
    GET_METHOD_ID(jobjs.activity_thread.cls, "ActivityThread", "getApplication",
...
static jobject get_global_context(JNIEnv* jni_env)
{
    ...
    jobject cur_at = jni_env->CallStaticObjectMethod(
      jobjs.activity_thread.cls, jobjs.activity_thread.m_current);
    if (cur_at == NULL)
        return NULL;

    context =
      jni_env->CallObjectMethod(cur_at, jobjs.activity_thread.m_get_app);
    return context;
}
...
static pj_status_t oboe_refresh(pjmedia_aud_dev_factory* ff)
{
    ...
    jni_env->CallStaticVoidMethod(jobjs.dev_info.cls, jobjs.dev_info.m_refresh,
                                  context);
    ...
    f->dev_info = (aud_dev_info*)pj_pool_calloc(f->dev_pool, dev_count,
                                                sizeof(aud_dev_info));
    ...
    for (i = 0; i < dev_count; i++) {
        ...
        jdev_info = jni_env->CallStaticObjectMethod(
          jobjs.dev_info.cls, jobjs.dev_info.m_get_info, i);
        if (jdev_info == NULL)
            continue;
        ...
        /* Get name info */
        jstring jstrtmp =
          (jstring)jni_env->GetObjectField(jdev_info, jobjs.dev_info.f_name);
    ...
    PJ_LOG(4, (THIS_FILE, "Oboe audio device initialized with %d device(s):",
               f->dev_count));

    for (i = 0; i < f->dev_count; i++) {
        aud_dev_info* adi = &f->dev_info[i];
        PJ_LOG(4, (THIS_FILE, "%2d (native id=%d): %s (%s%s%s)", i, adi->id,
                   adi->info.name, adi->info.input_count ? "in" : "",
...
static pj_status_t oboe_get_dev_info(pjmedia_aud_dev_factory* ff,
                                     unsigned index, pjmedia_aud_dev_info* info)
{
    struct oboe_aud_factory* f = (struct oboe_aud_factory*)ff;
...
class AtomicQueue {
   public:
    ...
    void put(void* frame)
    {
        ...
        /* Increment read pointer if next write is overlapping (next_ptr == read
         * ptr) */
        unsigned next_read_ptr =
          (next_ptr == maxFrameCnt - 1) ? 0 : (next_ptr + 1);
...
class MyOboeEngine : oboe::AudioStreamDataCallback,
   ...
   public:
    ...
    pj_status_t Start()
    {
        ...
        /* Create queue */
        unsigned latency =
          (dir == PJMEDIA_DIR_CAPTURE ? stream->param.input_latency_ms :
        ...
        PJ_LOG(
          3, (THIS_FILE, "Oboe stream %s queue size=%d frames (latency=%d ms)",
        ...
        status = pj_thread_create(stream->pool, "android_oboe", &AudioThread,
                                  this, 0, 0, &thread);
        if (status != PJ_SUCCESS) {
            pj_mutex_unlock(mutex);
            return status;
        ...
        if (result != oboe::Result::OK) {
            PJ_LOG(3, (THIS_FILE, "Oboe stream %s open failed (err=%d/%s)",
        ...
        if (result != oboe::Result::OK) {
            PJ_LOG(3, (THIS_FILE, "Oboe stream %s start failed (err=%d/%s)",
                       dir_st, result, oboe::convertToText(result)));
            pj_mutex_unlock(mutex);
            return PJMEDIA_EAUD_SYSERR;
        }
        ...
        PJ_LOG(
          4, (THIS_FILE,
              ...
              dir_st, stream->param.play_id, stream->param.clock_rate,
              stream->param.channel_count, stream->param.samples_per_frame,
              stream->param.samples_per_frame * 1000 / stream->param.clock_rate,
              oboe_stream->getAudioApi(),
              (oboe_stream->usesAAudio() ? "AAudio" : "other"),
              (oboe_stream->getSharingMode() == oboe::SharingMode::Exclusive ?
                 "yes" :
                 "no"),
    ...
    void Stop()
    {
        ...
        if (!mutex || thread_quit) {
            PJ_LOG(5, (THIS_FILE,
                       "Oboe stream %s stop request when "
                       "already stopped.",
                       dir_st));
            return;
        }
    ...
    void onErrorAfterClose(oboe::AudioStream* oboeStream, oboe::Result result)
    {
        __android_log_print(ANDROID_LOG_INFO, THIS_FILE,
                            "Oboe %s got onErrorAfterClose(%d)", dir_st,
                            result);
        ...
        if (!err_thread_registered || !pj_thread_is_registered()) {
            ...
            pj_thread_register("oboe_err_thread", err_thread_desc, &tmp_thread);
                                      ...
                                      pjmedia_aud_stream** p_aud_strm)
{
    ...
    PJ_ASSERT_RETURN(param->channel_count >= 1 && param->channel_count <= 2,
                     PJ_EINVAL);
    PJ_ASSERT_RETURN(
      param->bits_per_sample == 8 || param->bits_per_sample == 16, PJ_EINVAL);
...
/* API: set capability */
static pj_status_t strm_set_cap(pjmedia_aud_stream* s, pjmedia_aud_dev_cap cap,
                                const void* value)
{
    struct oboe_aud_stream* strm = (struct oboe_aud_stream*)s;
