...
static const struct
{
    ...
    PJ_BUILD_ERR(PJMEDIA_EAUD_INVCAP,
                 "Invalid or unsupported audio capability"),
    PJ_BUILD_ERR(PJMEDIA_EAUD_INVOP,
                 "Invalid or unsupported audio device operation"),
 ...
 */
PJ_DEF(pj_str_t)
pjmedia_audiodev_strerror(pj_status_t statcode, char* buf, pj_size_t bufsize)
{
    pj_str_t errstr;
...
#    if PJMEDIA_AUDIO_DEV_HAS_WMME
           ...
           statcode < PJMEDIA_AUDIODEV_WMME_OUT_ERROR_END))
    {
        ...
        MMRESULT(WINAPI * waveGetErrText)
        (UINT mmrError, LPTSTR pszText, UINT cchText);
...
#    endif
    ...
    /* Error not found. */
    errstr.ptr = buf;
    errstr.slen = pj_ansi_snprintf(
      buf, bufsize, "Unknown pjmedia-audiodev error %d", statcode);
