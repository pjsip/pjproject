...
void pjmedia_rtcp_xr_init(pjmedia_rtcp_xr_session* session,
                          ...
                          pj_uint8_t gmin, unsigned frames_per_packet)
{
...
PJ_DEF(void)
pjmedia_rtcp_build_rtcp_xr(pjmedia_rtcp_xr_session* sess, unsigned rpt_types,
                           void** rtcp_pkt, int* len)
{
    ...
    if ((rpt_types == 0 || (rpt_types & PJMEDIA_RTCP_XR_DLRR)) && sess->rx_lrr)
    {
        ...
        sess->stat.rx.stat_sum.count > 0)
    {
        ...
        r->begin_seq =
          pj_htons((pj_uint16_t)(sess->stat.rx.stat_sum.begin_seq & 0xFFFF));
        r->end_seq =
          pj_htons((pj_uint16_t)(sess->stat.rx.stat_sum.end_seq & 0xFFFF));
        ...
        sess->rtcp_session->stat.rx.pkt)
    {
        ...
        if (ctotal) {
            ...
            if (c11 == 0)
                ...
                sess->stat.rx.voip_mtc.gap_den =
                  (pj_uint8_t)((c14 << 8) / (c11 + c14));
            if (p23 == 0)
                sess->stat.rx.voip_mtc.burst_den = 0;
            else
                sess->stat.rx.voip_mtc.burst_den =
                  (pj_uint8_t)((p23 << 8) / (p23 + p32));
            ...
            sess->stat.rx.voip_mtc.gap_dur =
              (pj_uint16_t)((c11 + c14 + c13) * m / c13);
            sess->stat.rx.voip_mtc.burst_dur =
              (pj_uint16_t)((ctotal - (c11 + c14 + c13)) * m / c13);

            /* Callculate loss/discard rates, scaled 0-256 */
            sess->stat.rx.voip_mtc.loss_rate =
              (pj_uint8_t)((sess->voip_mtc_stat.loss_count << 8) / ctotal);
            sess->stat.rx.voip_mtc.discard_rate =
              (pj_uint8_t)((sess->voip_mtc_stat.discard_count << 8) / ctotal);
        } else {
        ...
        if (sess->stat.rtt.last)
            sess->stat.rx.voip_mtc.rnd_trip_delay =
              (pj_uint16_t)(sess->stat.rtt.last / 1000);
        else if (sess->rtcp_session->stat.rtt.last)
            sess->stat.rx.voip_mtc.rnd_trip_delay =
              (pj_uint16_t)(sess->rtcp_session->stat.rtt.last / 1000);
...
#    endif
        ...
        sess->stat.rx.voip_mtc.end_sys_delay =
          (pj_uint16_t)(sess->stat.rx.voip_mtc.rnd_trip_delay / 2 +
                        sess->stat.rx.voip_mtc.jb_nom + est_extra_delay);
        ...
        r->signal_lvl =
          (pj_uint8_t)((sess->stat.rx.voip_mtc.signal_lvl >= 0) ?
                         sess->stat.rx.voip_mtc.signal_lvl :
                         (sess->stat.rx.voip_mtc.signal_lvl + 256));
        r->noise_lvl = (pj_uint8_t)((sess->stat.rx.voip_mtc.noise_lvl >= 0) ?
...
void pjmedia_rtcp_xr_rx_rtcp_xr(pjmedia_rtcp_xr_session* sess, const void* pkt,
                                pj_size_t size)
{
    ...
    const pjmedia_rtcp_xr_rb_header* rb_hdr =
      (pjmedia_rtcp_xr_rb_header*)rtcp_xr->buf;
    ...
    if (rb_dlrr) {
        ...
        TRACE_((sess->name,
                "Rx RTCP XR DLRR: lrr=%p, dlrr=%p (%d:%03dms), "
                "now=%p, rtt=%p",
                lrr, dlrr, dlrr / 65536, (dlrr % 65536) * 1000 / 65536, now,
                (pj_uint32_t)eedelay));
        ...
        if (now - dlrr >= lrr) {
            ...
            PJ_LOG(5, (sess->name,
                       ...
                       (dlrr % 65536) * 1000 / 65536, dlrr - (now - lrr)));
        }
    }
    ...
    if (rb_voip_mtc) {
        ...
        /* signal & noise level encoded in two's complement form */
        sess->stat.tx.voip_mtc.signal_lvl =
          (pj_int8_t)((rb_voip_mtc->signal_lvl > 127) ?
                        ((int)rb_voip_mtc->signal_lvl - 256) :
                        rb_voip_mtc->signal_lvl);
        sess->stat.tx.voip_mtc.noise_lvl =
          (pj_int8_t)((rb_voip_mtc->noise_lvl > 127) ?
...
void pjmedia_rtcp_xr_rx_rtp(pjmedia_rtcp_xr_session* sess, unsigned seq,
                            int lost, int dup, int discarded, int jitter,
...
PJ_DEF(pj_status_t)
pjmedia_rtcp_xr_update_info(pjmedia_rtcp_xr_session* sess, unsigned info,
                            pj_int32_t val)
{
    int v = val;
