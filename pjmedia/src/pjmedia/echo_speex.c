...
#    else
    if (channel_count != 1) {
        PJ_LOG(2, ("echo_speex.c",
                   "Multichannel EC is not supported by this "
...
#    endif
    ...
    echo->decorr =
      speex_decorrelate_new(clock_rate, channel_count, echo->spf_per_channel);
    if (echo->decorr == NULL)
        return PJ_ENOMEM;

    /* Set sampling rate */
    sampling_rate = clock_rate;
    speex_echo_ctl(echo->state, SPEEX_ECHO_SET_SAMPLING_RATE, &sampling_rate);
    ...
    for (i = 0; i < channel_count; i++) {
        ...
        echo->preprocess[i] =
          speex_preprocess_state_init(echo->spf_per_channel, clock_rate);
        ...
        speex_preprocess_ctl(echo->preprocess[i], SPEEX_PREPROCESS_SET_DENOISE,
                             &enabled);
    ...
    echo->tmp_frame = (pj_int16_t*)pj_pool_zalloc(
      pool, sizeof(pj_int16_t) * channel_count * samples_per_frame);
    if (!echo->tmp_frame) {
        speex_aec_destroy(echo);
        return PJ_ENOMEM;
    }
                      ...
                      void* reserved)
{
    ...
    /* Sanity checks */
    PJ_ASSERT_RETURN(
      echo && rec_frm && play_frm && options == 0 && reserved == NULL,
      PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t) speex_aec_playback(void* state, pj_int16_t* play_frm)
{
    speex_ec* echo = (speex_ec*)state;
     ...
     */
    if (echo->channel_count > 1) {
        pjmedia_copy_samples(echo->tmp_frame, play_frm,
...
speex_aec_capture(void* state, pj_int16_t* rec_frm, unsigned options)
{
    ...
    /* Cancel echo */
    speex_echo_capture(echo->state, (spx_int16_t*)rec_frm,
                       (spx_int16_t*)echo->tmp_frame);
