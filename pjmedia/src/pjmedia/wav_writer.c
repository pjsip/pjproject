 ...
 */
PJ_DEF(pj_status_t)
pjmedia_wav_writer_port_create(pj_pool_t* pool, const char* filename,
                               unsigned sampling_rate, unsigned channel_count,
                               unsigned samples_per_frame,
                               unsigned bits_per_sample, unsigned flags,
                               pj_ssize_t buff_size, pjmedia_port** p_port)
{
    ...
    pjmedia_port_info_init(&fport->base.info, &name, SIGNATURE, sampling_rate,
    ...
    wave_hdr.fmt_hdr.bytes_per_sec =
      sampling_rate * channel_count * fport->bytes_per_sample;
    wave_hdr.fmt_hdr.block_align =
      (pj_uint16_t)(fport->bytes_per_sample * channel_count);
    wave_hdr.fmt_hdr.bits_per_sample =
      (pj_uint16_t)(fport->bytes_per_sample * 8);
    ...
    *p_port = &fport->base;
    ...
    PJ_LOG(4,
           (THIS_FILE, "File writer '%.*s' created: samp.rate=%d, bufsize=%uKB",
            (int)fport->base.info.name.slen, fport->base.info.name.ptr,
            PJMEDIA_PIA_SRATE(&fport->base.info), fport->bufsize / 1000));
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_wav_writer_port_set_cb(pjmedia_port* port, pj_size_t pos,
                               void* user_data,
                                                 ...
                                                 void* usr_data))
{
    ...
    PJ_LOG(1, (THIS_FILE,
               "pjmedia_wav_writer_port_set_cb() is deprecated. "
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_wav_writer_port_set_cb2(pjmedia_port* port, pj_size_t pos,
                                void* user_data,
...
static pj_status_t file_put_frame(pjmedia_port* this_port, pjmedia_frame* frame)
{
    ...
    PJ_ASSERT_RETURN(
      fport->writepos + frame_size <= fport->buf + fport->bufsize,
