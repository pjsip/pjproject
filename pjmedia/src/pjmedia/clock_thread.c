...
pjmedia_clock_src_get_time_msec(const pjmedia_clock_src* clocksrc)
{
    ...
    return (pj_uint32_t)((1.0 * ts.u32.hi * 0xFFFFFFFFUL + ts.u32.lo) * 1000.0 /
                         clocksrc->clock_rate);
#else
    if (ts.u32.lo > 0x3FFFFFUL)
        return (pj_uint32_t)(0xFFFFFFFFUL / clocksrc->clock_rate * ts.u32.hi *
                               1000UL +
                             ts.u32.lo / clocksrc->clock_rate * 1000UL);
    else
        return (pj_uint32_t)(0xFFFFFFFFUL / clocksrc->clock_rate * ts.u32.hi *
                               1000UL +
                             ts.u32.lo * 1000UL / clocksrc->clock_rate);
#endif
}
                     ...
                     void* user_data, pjmedia_clock** p_clock)
{
    ...
    param.usec_interval =
      (unsigned)(samples_per_frame * USEC_IN_SEC / channel_count / clock_rate);
    param.clock_rate = clock_rate;
    return pjmedia_clock_create2(pool, &param, options, cb, user_data, p_clock);
}
                      ...
                      void* user_data, pjmedia_clock** p_clock)
{
    ...
    PJ_ASSERT_RETURN(
      pool && param->usec_interval && param->clock_rate && p_clock, PJ_EINVAL);
    ...
    clock->interval.u64 = param->usec_interval * clock->freq.u64 / USEC_IN_SEC;
    clock->next_tick.u64 = 0;
    clock->timestamp.u64 = 0;
    clock->max_jump = MAX_JUMP_MSEC * clock->freq.u64 / 1000;
    clock->timestamp_inc = (unsigned)(param->usec_interval * param->clock_rate /
...
PJ_DEF(pj_status_t) pjmedia_clock_start(pjmedia_clock* clock)
{
    ...
    if ((clock->options & PJMEDIA_CLOCK_NO_ASYNC) == 0 && !clock->thread) {
        status = pj_thread_create(clock->pool, "clock", &clock_thread, clock, 0,
                                  0, &clock->thread);
        if (status != PJ_SUCCESS) {
            clock->running = PJ_FALSE;
            return status;
...
/* Calculate next tick */
PJ_INLINE(void) clock_calc_next_tick(pjmedia_clock* clock, pj_timestamp* now)
{
...
pjmedia_clock_wait(pjmedia_clock* clock, pj_bool_t wait, pj_timestamp* ts)
{
    ...
    PJ_ASSERT_RETURN((clock->options & PJMEDIA_CLOCK_NO_ASYNC) != 0, PJ_FALSE);
