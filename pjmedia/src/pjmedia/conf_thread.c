                                           ...
                                           unsigned *p_port )
{
    ...
    /* Put the port to the reserved slot and queue the operation
     * under the same mutex hold, so that other threads calling
     * connect/disconnect will see both the port and the queued
...
static SLOT_TYPE conf_reserve_port(pjmedia_conf *conf)
{
    ...
    slot = (SLOT_TYPE)(pslot - conf->free_port_slots);
    ...
    if (conf_port->is_new) {
        ...
        while (ope != conf->op_queue) {
            ...
            if (cancel_op) {
                ...
                /* Release the reference held by a cancelled replace op on its
                 * pending new port (see pjmedia_conf_replace_port()).
                 */
                if (cancel_op->type == PJMEDIA_CONF_OP_REPLACE_PORT &&
 ...
 * connections/settings. Slot-pool ownership is moved off the old port's group
 * lock so the caller can safely destroy the old port. See conference.c for the
 * serial-bridge counterpart; the parallel-only per-port buffers/slists are
 * format-independent and are left intact across the swap.
 */
static void detach_slot_port(struct conf_port *cport)
{
    pjmedia_port *old_port = cport->port;

    /* Keep tx/rx settings so the app's enable/disable/mute survives; the
     * read/write paths skip a slot whose port is NULL. */
    cport->port = NULL;
                                   ...
                                   const pjmedia_conf_op_param *prm)
{
    ...
    struct conf_port *cport = conf->ports[slot];
    pjmedia_audio_format_detail *afd;
    unsigned new_rate, new_spf, new_cc;
    ...
    slot_pool = cport->pool ? cport->pool : prm->replace_port.pool;

    if (cport->port)
        detach_slot_port(cport);

    afd = pjmedia_format_get_audio_format_detail(&strm_port->info.fmt, 1);
    new_rate = afd->clock_rate;
    new_spf = PJMEDIA_AFD_SPF(afd);
    new_cc = afd->channel_count;

    fmt_changed = (new_rate != cport->sampling_rate ||
                   new_spf != cport->samples_per_frame ||
                   new_cc != cport->channel_count);

    /* Rebuild resamplers/buffers only on an actual format change; the
     * parallel per-port buffers/slists are format-independent and untouched. */
    if (fmt_changed) {
        ...
        unsigned build_idx;
        pj_bool_t need_resample = (new_rate != conf->sampling_rate);
        pj_bool_t need_buffers = (new_rate != conf->sampling_rate ||
                                  new_cc != conf->channel_count ||
         ...
         * across repeated format changes. The pool pair is created lazily.
         * Building into the INACTIVE pool keeps the slot's current resources
         * valid until we commit (build-then-commit).
         */
        if (!cport->buf_pool[0]) {
            /* Match the slot pool's alignment so the conversion buffers keep
             * the same SIMD alignment they had when allocated from it. */
#ifdef SIMD_ALIGNMENT
            cport->buf_pool[0] = pj_pool_aligned_create(cport->pool->factory,
                                     "confbuf", 512, 512, SIMD_ALIGNMENT, NULL);
            cport->buf_pool[1] = pj_pool_aligned_create(cport->pool->factory,
                                     "confbuf", 512, 512, SIMD_ALIGNMENT, NULL);
#else
        ...
        /* Build all replacement resources into temporaries first; commit to
         * the slot only after every step succeeds, so a failure leaves the
         * slot's previous valid format/resamplers/buffers intact. */
        if (need_resample) {
        ...
        if (need_buffers) {
            ...
            port_ptime = new_spf / new_cc * 1000 / new_rate;
            conf_ptime = conf->samples_per_frame / conf->channel_count *
                         1000 / conf->sampling_rate;

            if (0 != (port_ptime * new_rate * new_cc % 1000)) {
            ...
            new_buf_cap = new_rate * buff_ptime / 1000;
            if (new_cc > conf->channel_count)
                new_buf_cap *= new_cc;
            else
        ...
        cport->rx_buf = new_rx_buf;
        cport->tx_buf = new_tx_buf;
        cport->rx_buf_cap = new_buf_cap;
        cport->tx_buf_cap = new_buf_cap;
        cport->rx_buf_count = 0;
        cport->tx_buf_count = 0;
        cport->sampling_rate = new_rate;
        cport->samples_per_frame = new_spf;
        cport->channel_count = new_cc;

        /* The freshly-built pool is now the active one. */
        cport->buf_pool_idx = build_idx;
    }

    /* The group lock and queue reference were already set up by
     * pjmedia_conf_replace_port(); add the ownership reference + on-destroy
     * handler that keep the port alive while it is attached.
     */
    if (!strm_port->grp_lock)
        pjmedia_port_init_grp_lock(strm_port, slot_pool, NULL);
    if (strm_port->grp_lock) {
    ...
    /* Release the queue reference taken in pjmedia_conf_replace_port(); the
     * ownership reference added above now keeps the port alive.
     */
    if (strm_port->grp_lock)
        pj_grp_lock_dec_ref(strm_port->grp_lock);
    return PJ_SUCCESS;
...
on_error:
    /* The op did not complete: release the queue reference so the caller's
     * port can be destroyed. The port was not attached to the slot.
     */
    if (strm_port && strm_port->grp_lock)
        pj_grp_lock_dec_ref(strm_port->grp_lock);
    return status;
}
...
static pj_status_t thread_pool_start(pjmedia_conf *conf)
{
    pj_atomic_value_t worker_threads;
    int i;
...
static void perform_get_frame(pjmedia_conf *conf)
{
    ...
    while ((i = pj_atomic_dec_and_get(conf->active_ports_idx)) >= 0) {
        ...
        SLOT_TYPE port_idx;
        struct conf_port *conf_port;
        unsigned rx_adj_level;

        port_idx = conf->active_ports[i];
        pj_assert(port_idx < conf->max_ports);
        conf_port = conf->ports[port_idx];
        PJ_ASSERT_ON_FAIL(conf_port, continue);
        rx_adj_level = conf_port->rx_adj_level; /*for auto-vectorizer*/
        ...
        /* Skip if the slot currently has no attached port (detached and
