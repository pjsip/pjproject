...
static pj_status_t transport_attach(pjmedia_transport* tp, void* user_data,
 ...
 */
static void ice_on_rx_data(pj_ice_strans* ice_st, unsigned comp_id, void* pkt,
                           pj_size_t size, const pj_sockaddr_t* src_addr,
                           unsigned src_addr_len);
static void ice_on_ice_complete(pj_ice_strans* ice_st, pj_ice_strans_op op,
                                pj_status_t status);
...
static pj_status_t parse_cand(const char* obj_name, pj_pool_t* pool,
...
{
    return pjmedia_ice_create3(endpt, name, comp_cnt, cfg, cb, options, NULL,
                               p_tp);
}
...
pjmedia_ice_create3(pjmedia_endpt* endpt, const char* name, unsigned comp_cnt,
                    const pj_ice_strans_cfg* cfg, const pjmedia_ice_cb* cb,
                    unsigned options, void* user_data, pjmedia_transport** p_tp)
{
    pj_pool_t* pool;
                          ...
                          void* user_data)
{
    ...
    for (il = tp_ice->listener.next; il != &tp_ice->listener; il = il->next) {
        if (pj_memcmp(&il->cb, cb, sizeof(cb)) == 0 &&
            il->user_data == user_data)
            break;
    }
...
pjmedia_ice_sdp_has_trickle(const pjmedia_sdp_session* sdp, unsigned med_idx)
{
    ...
    a =
      pjmedia_sdp_attr_find(sdp->attr_count, sdp->attr, &STR_ICE_OPTIONS, NULL);
    if (a && pj_strstr(&a->value, &STR_TRICKLE))
        return PJ_TRUE;
                           ...
                           const pj_ice_sess_cand rcand[], pj_bool_t rcand_end)
{
    ...
    status = pj_ice_strans_update_check_list(
      tp_ice->ice_st, rem_ufrag, rem_passwd, rcand_cnt, rcand, rcand_end);
    if (status != PJ_SUCCESS)
        return status;
    ...
    if (!pj_ice_strans_sess_is_running(tp_ice->ice_st)) {
        ...
        if (i != comp_cnt) {
            pj_str_t rufrag;
            pj_ice_strans_get_ufrag_pwd(tp_ice->ice_st, NULL, NULL, &rufrag,
                                        NULL);
            if (rufrag.slen > 0) {
                PJ_LOG(3, (THIS_FILE, "Trickle ICE starts connectivity check"));
                status =
                  pj_ice_strans_start_ice(tp_ice->ice_st, NULL, NULL, 0, NULL);
            }
        }
    }
                               ...
                               pj_bool_t* end_of_cand)
{
    ...
    if (end_of_cand) {
        a =
          pjmedia_sdp_attr_find(m->attr_count, m->attr, &STR_END_OF_CAND, NULL);
        if (!a) {
                               ...
                               pj_bool_t end_of_cand)
{
    ...
    if (i == sdp->media_count) {
        if (sdp->media_count >= PJMEDIA_MAX_SDP_MEDIA) {
            PJ_LOG(3, (THIS_FILE,
                       "Trickle ICE failed to encode candidates, "
    ...
    if (!a || !pj_strstr(&a->value, &STR_TRICKLE)) {
        a =
          pjmedia_sdp_attr_create(sdp_pool, STR_ICE_OPTIONS.ptr, &STR_TRICKLE);
                                    ...
                                    pj_bool_t* p_end_of_cand)
{
    ...
    for (i = 0; i < comp_cnt; ++i) {
        ...
        status = pj_ice_strans_enum_cands(tp_ice->ice_st, i + 1, &cnt,
                                          &cand[cand_cnt]);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(
              3, (tp_ice->base.name, status,
                  "Failed enumerating local candidates for comp-id=%d", i + 1));
            continue;
        }
        cand_cnt += cnt;
     ...
     */
    status =
      pjmedia_ice_trickle_encode_sdp(sdp_pool, sdp, &tp_ice->sdp_mid, &ufrag,
                                     &pwd, cand_cnt, cand, end_of_cand);
    if (status != PJ_SUCCESS) {
                       ...
                       pj_status_t err)
{
    if (err != PJ_SUCCESS) {
        PJ_PERROR(4,
                  (tp_ice->base.name, err, "Stopping ICE, reason=%s", reason));
    } else {
                               ...
                               const pj_ice_sess_cand* cand)
{
    ...
    len = pj_ansi_snprintf(
      buffer, max_len, "%.*s %u UDP %u %s %u typ ", (int)cand->foundation.slen,
      cand->foundation.ptr, (unsigned)cand->comp_id, cand->prio,
    ...
    case PJ_ICE_CAND_TYPE_PRFLX:
        len2 = pj_ansi_snprintf(
          buffer + len, max_len - len, "%s raddr %s rport %d",
          pj_ice_get_cand_type_name(cand->type),
          pj_sockaddr_print(&cand->rel_addr, ipaddr, sizeof(ipaddr), 0),
                         ...
                         const pjmedia_sdp_attr** p_ice_pwd)
{
    ...
    attr = pjmedia_sdp_attr_find(rem_m->attr_count, rem_m->attr, &STR_ICE_UFRAG,
                                 NULL);
    if (attr == NULL) {
    ...
    *p_ice_ufrag = attr;
    ...
    attr =
      pjmedia_sdp_attr_find(rem_m->attr_count, rem_m->attr, &STR_ICE_PWD, NULL);
    if (attr == NULL) {
...
static pj_status_t encode_session_in_sdp(
  struct transport_ice* tp_ice, pj_pool_t* sdp_pool,
  pjmedia_sdp_session* sdp_local, unsigned media_index, unsigned comp_cnt,
  pj_bool_t restart_session, pj_bool_t rtcp_mux, pj_bool_t trickle)
{
    enum
    {
        ATTR_BUF_LEN = 160, /* Max len of a=candidate attr */
    ...
    /* Get ufrag and pwd from current session */
    pj_ice_strans_get_ufrag_pwd(tp_ice->ice_st, &local_ufrag, &local_pwd, NULL,
                                NULL);
        ...
        pj_ice_strans_get_state(tp_ice->ice_st) != PJ_ICE_STRANS_STATE_FAILED)
    {
        ...
        attr =
          pjmedia_sdp_attr_create(sdp_pool, STR_ICE_UFRAG.ptr, &local_ufrag);
        pjmedia_sdp_attr_add(&m->attr_count, m->attr, attr);
        ...
        if (comp_cnt == 2 &&
            (check = pj_ice_strans_get_valid_pair(tp_ice->ice_st, COMP_RTCP)) !=
              NULL &&
            (a_rtcp = pjmedia_sdp_attr_find(m->attr_count, m->attr, &STR_RTCP,
                                            0)) != NULL)
        {
            pjmedia_sdp_attr_remove(&m->attr_count, m->attr, a_rtcp);

            a_rtcp =
              pjmedia_sdp_attr_create_rtcp(sdp_pool, &check->lcand->addr);
            if (a_rtcp)
        ...
        for (comp = 0; comp < comp_cnt; ++comp) {
            ...
            value.ptr = attr_buf;
            value.slen =
              print_sdp_cand_attr(attr_buf, ATTR_BUF_LEN, check->lcand);
            if (value.slen < 0) {
                pj_assert(!"Not enough attr_buf to print candidate");
                return PJ_EBUG;
            }
            ...
            if (pj_ice_strans_get_role(tp_ice->ice_st) ==
                ...
                pj_sockaddr_print(&check->rcand->addr, rem_addr,
                                  sizeof(rem_addr), 0);
                len = pj_ansi_snprintf(
                  rem_cand.ptr + rem_cand.slen, RATTR_BUF_LEN - rem_cand.slen,
                  "%s%u %s %u", (rem_cand.slen == 0 ? "" : " "), comp + 1,
                  rem_addr, pj_sockaddr_get_port(&check->rcand->addr));
        ...
        if (pj_ice_strans_get_role(tp_ice->ice_st) ==
            PJ_ICE_SESS_ROLE_CONTROLLING) {
            attr =
              pjmedia_sdp_attr_create(sdp_pool, STR_REM_CAND.ptr, &rem_cand);
            pjmedia_sdp_attr_add(&m->attr_count, m->attr, attr);
        }
                                     ...
                                     PJ_ICE_STRANS_STATE_FAILED))
    {
        ...
        if (!restart_session) {
            ...
            attr =
              pjmedia_sdp_attr_create(sdp_pool, STR_ICE_PWD.ptr, &local_pwd);
            pjmedia_sdp_attr_add(&m->attr_count, m->attr, attr);
        ...
        for (comp = 0; comp < comp_cnt; ++comp) {
            ...
            for (i = 0; i < cand_cnt; ++i) {
                ...
                value.slen =
                  print_sdp_cand_attr(attr_buf, ATTR_BUF_LEN, &cand[i]);
                if (value.slen < 0) {
                    pj_assert(!"Not enough attr_buf to print candidate");
                    return PJ_EBUG;
                }
                ...
                value.ptr = attr_buf;
                attr =
                  pjmedia_sdp_attr_create(sdp_pool, STR_CANDIDATE.ptr, &value);
                pjmedia_sdp_attr_add(&m->attr_count, m->attr, attr);
            }
        }
    } else {
    ...
    if (comp_cnt == 1) {
        ...
        if (m->bandw_count + 2 <= PJ_ARRAY_SIZE(m->bandw)) {
            m->bandw[m->bandw_count] =
              PJ_POOL_ZALLOC_T(sdp_pool, pjmedia_sdp_bandw);
    ...
    if (trickle) {
        ...
        end_of_cand = tp_ice->end_of_cand;
        status =
          pjmedia_ice_trickle_encode_sdp(sdp_pool, sdp_local, &tp_ice->sdp_mid,
                                         NULL, NULL, 0, NULL, end_of_cand);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(3, (tp_ice->base.name, status,
                                        ...
                                        unsigned media_index)
{
    ...
    /* Encode ICE in SDP */
    status = encode_session_in_sdp(
      tp_ice, sdp_pool, loc_sdp, media_index, tp_ice->comp_cnt, PJ_FALSE,
                                  ...
                                  struct sdp_state* sdp_state)
{
    ...
    */
    ...
    if (tp_ice->comp_cnt > 1) {
         ...
         */
        attr = pjmedia_sdp_attr_find(rem_m->attr_count, rem_m->attr, &STR_RTCP,
                                     NULL);
        has_rtcp = (attr != NULL);
        ...
        if (attr) {
            ...
            if (rtcp_attr.addr.slen) {
                ...
                */
                ...
                /* Assign RTCP address */
                status = pj_sockaddr_init(rem_af, &rtcp_addr, &rtcp_attr.addr,
                ...
                /* Assign RTCP address */
                status = pj_sockaddr_init(rem_af, &rtcp_addr, NULL,
                                          (pj_uint16_t)rtcp_attr.port);
                if (status != PJ_SUCCESS) {
                    return PJMEDIA_SDP_EINRTCP;
                }
    ...
    for (i = 0; i < rem_m->attr_count; ++i) {
        pj_ice_sess_cand cand;
        unsigned disable_ice_mismatch =
          tp_ice->options & PJMEDIA_ICE_DISABLE_ICE_MISMATCH;

        if (pj_strcmp(&rem_m->attr[i]->name, &STR_CANDIDATE) != 0)
            continue;

        status = parse_cand(tp_ice->base.name, tmp_pool, &rem_m->attr[i]->value,
                            &cand);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(
              4, (tp_ice->base.name, status,
                  "Error in parsing SDP candidate attribute '%.*s', "
                  "candidate is ignored",
                  (int)rem_m->attr[i]->value.slen, rem_m->attr[i]->value.ptr));
            continue;
        }
         ...
         pj_ice_strans_sess_is_complete(tp_ice->ice_st)))
    {
        ...
        pj_ice_strans_get_ufrag_pwd(tp_ice->ice_st, NULL, NULL, &rem_run_ufrag,
                                    &rem_run_pwd);
    ...
    /* Detect our role */
    if (pjmedia_sdp_attr_find(rem_sdp->attr_count, rem_sdp->attr, &STR_ICE_LITE,
                              NULL) != NULL)
    {
    ...
    PJ_LOG(4, (tp_ice->base.name,
               ...
               (sdp_state->match_comp_cnt != 0), sdp_state->match_comp_cnt,
               sdp_state->ice_mismatch, sdp_state->ice_restart,
                                         ...
                                         unsigned media_index)
{
    ...
    /* Verify the offer */
    status =
      verify_ice_sdp(tp_ice, sdp_pool, rem_sdp, media_index,
                     PJ_ICE_SESS_ROLE_CONTROLLED, &tp_ice->rem_offer_state);
    if (status != PJ_SUCCESS) {
    ...
    with_trickle = tp_ice->rem_offer_state.has_trickle &&
                   tp_ice->trickle_ice != PJ_ICE_SESS_TRICKLE_DISABLED;
    status =
      encode_session_in_sdp(tp_ice, sdp_pool, loc_sdp, media_index,
                            tp_ice->rem_offer_state.match_comp_cnt, PJ_FALSE,
                            tp_ice->use_rtcp_mux, with_trickle);
    if (status != PJ_SUCCESS) {
        set_no_ice(tp_ice, "Error encoding SDP answer", status);
        return status;
    }
                                            ...
                                            unsigned media_index)
{
    ...
    if (pj_ice_strans_has_sess(tp_ice->ice_st)) {
         ...
         * ICE session now.
         */
        status = pj_ice_strans_init_ice(
          tp_ice->ice_st, PJ_ICE_SESS_ROLE_CONTROLLED, NULL, NULL);
        if (status != PJ_SUCCESS) {
                                          ...
                                          unsigned media_index)
{
    ...
    if (rem_sdp && status == PJ_SUCCESS) {
            ...
            pjmedia_ice_sdp_has_trickle(rem_sdp, media_index))
        {
            ...
            status = pjmedia_ice_trickle_decode_sdp(rem_sdp, media_index, NULL,
                                                    &ufrag, &pwd, &cand_cnt,
                                                    cand, &end_of_cand);
            if (status == PJ_SUCCESS)
                status = pj_ice_strans_update_check_list(
                  tp_ice->ice_st, &ufrag, &pwd, cand_cnt, cand, end_of_cand);
            if (status != PJ_SUCCESS) {
                                        ...
                                        unsigned media_index)
{
    ...
    if ((tp_ice->media_option & PJMEDIA_TPMED_NO_TRANSPORT_CHECKING) == 0) {
        ...
        tp_proto_rem =
          m_rem ? pjmedia_sdp_transport_get_proto(&m_rem->desc.transport) : 0;
    ...
    if (tp_ice->initial_sdp) {
        if (rem_sdp) {
            status = create_initial_answer(tp_ice, sdp_pool, sdp_local, rem_sdp,
                                           media_index);
        } else {
            status =
              create_initial_offer(tp_ice, sdp_pool, sdp_local, media_index);
        }
    } else {
        if (rem_sdp) {
            ...
            status =
              create_subsequent_offer(tp_ice, sdp_pool, sdp_local, media_index);
        }
    }
...
static pj_status_t start_ice(struct transport_ice* tp_ice, pj_pool_t* tmp_pool,
                             const pjmedia_sdp_session* rem_sdp,
                             unsigned media_index)
{
    pjmedia_sdp_media* rem_m = rem_sdp->media[media_index];
    ...
    cand = (pj_ice_sess_cand*)pj_pool_calloc(tmp_pool, PJ_ICE_MAX_CAND,
                                         ...
                                         unsigned media_index)
{
    ...
    if (current_oa_role == ROLE_OFFERER) {
        ...
        if (!initial_oa) {
             ...
             */
            get_ice_attr(sdp_local, sdp_local->media[media_index], &ufrag_attr,
                         &pwd_attr);
            status = pj_ice_strans_init_ice(
              tp_ice->ice_st, tp_ice->rem_offer_state.local_role,
              &ufrag_attr->value, &pwd_attr->value);
            if (status != PJ_SUCCESS) {
                PJ_PERROR(1, (tp_ice->base.name, status,
         ...
         */
        if (tp_ice->rem_offer_state.local_role ==
              PJ_ICE_SESS_ROLE_CONTROLLING &&
                              ...
                              NULL) != NULL)
    {
        ...
        if (opt.aggressive) {
            ...
            PJ_LOG(4, (tp_ice->base.name,
                       "Forcefully set ICE to use regular "
...
static pj_status_t transport_attach(pjmedia_transport* tp, void* stream,
                                    const pj_sockaddr_t* rem_addr,
                                    const pj_sockaddr_t* rem_rtcp,
                                    unsigned addr_len,
                                    void (*rtp_cb)(void*, void*, pj_ssize_t),
                                     ...
                                     pjmedia_transport_attach_param* att_param)
{
    ...
    tp_ice->use_rtcp_mux =
      (pj_sockaddr_has_addr(&att_param->rem_addr) &&
       pj_sockaddr_cmp(&att_param->rem_addr, &att_param->rem_rtcp) == 0);
                                      ...
                                      pj_size_t size)
{
    ...
    status = pj_ice_strans_sendto2(tp_ice->ice_st, 1, pkt, size,
                                   &tp_ice->remote_rtp, tp_ice->addr_len);
                                        ...
                                        pj_size_t size)
{
    ...
    if (tp_ice->comp_cnt > 1 || tp_ice->use_rtcp_mux) {
        ...
        status = pj_ice_strans_sendto2(tp_ice->ice_st, comp_id, pkt, size, addr,
                                       addr_len);
        if (status == PJ_EPENDING)
            status = PJ_SUCCESS;
                           ...
                           unsigned src_addr_len)
{
    ...
    if (comp_id == 1 && (tp_ice->rtp_cb || tp_ice->rtp_cb2)) {
        ...
        if (!discard) {
            if (tp_ice->rtp_cb2) {
                ...
                param.size = size;
                param.src_addr =
                  (tp_ice->use_ice ? NULL : (pj_sockaddr_t*)src_addr);
  ...
  (PJMEDIA_TRANSPORT_SWITCH_REMOTE_ADDR == 1)
        if (rem_switch &&
            ...
            PJ_LOG(4, (tp_ice->base.name, "Remote RTP address switched to %s",
            ...
            if (tp_ice->use_rtcp_mux) {
                ...
                port =
                  (pj_uint16_t)(pj_sockaddr_get_port(&tp_ice->remote_rtp) + 1);
            ...
            (tp_ice->options & PJMEDIA_ICE_NO_SRC_ADDR_CHECKING) == 0)
        {
            if (pj_sockaddr_cmp(&tp_ice->remote_rtcp, src_addr) == 0) {
                ...
                if (tp_ice->rtcp_src_cnt < PJMEDIA_RTCP_NAT_PROBATION_CNT) {
                    ...
                    pj_assert(tp_ice->addr_len ==
                              pj_sockaddr_get_len(src_addr));

                    PJ_LOG(
                      4,
                      (tp_ice->base.name, "Remote RTCP address switched to %s",
                       pj_sockaddr_print(&tp_ice->remote_rtcp, addr_text,
                                         sizeof(addr_text), 3)));
                }
            }
                                ...
                                pj_status_t result)
{
    ...
    pj_perror(5, tp_ice->base.name, result,
              "ICE operation complete"
              " (op=%d%s)",
              op,
