...
static pj_status_t fill_buffer(struct file_reader_port* fport)
{
    ...
    while (size_left > 0) {
        ...
        status = pj_file_read(fport->fd,
                              &fport->buf[fport->bufsize - size_left], &size);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_wav_player_port_create(pj_pool_t* pool, const char* filename,
                               ...
                               pj_ssize_t buff_size, pjmedia_port** p_port)
{
        ...
        wave_hdr.fmt_hdr.fmt != PJMEDIA_FMT_TAG)
    {
        pj_file_close(fport->fd);
        TRACE_(
          (THIS_FILE, "actual value|expected riff=%x|%x, wave=%x|%x fmt=%x|%x",
           wave_hdr.riff_hdr.riff, PJMEDIA_RIFF_TAG, wave_hdr.riff_hdr.wave,
           PJMEDIA_WAVE_TAG, wave_hdr.fmt_hdr.fmt, PJMEDIA_FMT_TAG));
        return PJMEDIA_ENOTVALIDWAVE;
    }
    ...
    fport->bytes_per_sample =
      (pj_uint16_t)(wave_hdr.fmt_hdr.bits_per_sample / 8);
    ...
    for (;;) {
        ...
        PJ_CHECK_OVERFLOW_UINT32_TO_LONG(subchunk.len, pj_file_close(fport->fd);
                                         return PJMEDIA_ENOTVALIDWAVE;);
    ...
    if (wave_hdr.data_hdr.len <
        ptime * wave_hdr.fmt_hdr.sample_rate * wave_hdr.fmt_hdr.nchan / 1000)
    {
        pj_file_close(fport->fd);
        return PJMEDIA_EWAVETOOSHORT;
    }
    ...
    pjmedia_port_info_init(&fport->base.info, &name, SIGNATURE,
                           wave_hdr.fmt_hdr.sample_rate, wave_hdr.fmt_hdr.nchan,
    ...
    *p_port = &fport->base;
    ...
    PJ_LOG(4, (THIS_FILE,
               ...
               ad->clock_rate, ad->channel_count, fport->bufsize / 1000,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_wav_player_get_info(pjmedia_port* port, pjmedia_wav_player_info* info)
{
    struct file_reader_port* fport;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_wav_player_port_set_pos(pjmedia_port* port, pj_uint32_t bytes)
{
    struct file_reader_port* fport;
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_wav_player_set_eof_cb(pjmedia_port* port, void* user_data,
                                                ...
                                                void* usr_data))
{
    ...
    PJ_LOG(1, (THIS_FILE,
               "pjmedia_wav_player_set_eof_cb() is deprecated. "
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_wav_player_set_eof_cb2(pjmedia_port* port, void* user_data,
...
static pj_status_t file_get_frame(pjmedia_port* this_port, pjmedia_frame* frame)
{
    ...
    if ((fport->readpos + frame_size) <= (fport->buf + fport->bufsize)) {
        ...
        pj_memcpy(((char*)frame->buf) + endread, fport->buf,
                  frame_size - endread);
