...
pjmedia_sdp_attr_create(pj_pool_t* pool, const char* name,
...
PJ_DEF(pjmedia_sdp_attr*)
pjmedia_sdp_attr_clone(pj_pool_t* pool, const pjmedia_sdp_attr* rhs)
{
    pjmedia_sdp_attr* attr;
...
PJ_DEF(pjmedia_sdp_attr*)
pjmedia_sdp_attr_find(unsigned count, pjmedia_sdp_attr* const attr_array[],
                      const pj_str_t* name, const pj_str_t* c_fmt)
{
    unsigned i;
...
PJ_DEF(pjmedia_sdp_attr*)
pjmedia_sdp_attr_find2(unsigned count, pjmedia_sdp_attr* const attr_array[],
                       const char* c_name, const pj_str_t* c_fmt)
{
    pj_str_t name;
...
PJ_DEF(pj_status_t)
pjmedia_sdp_attr_add(unsigned* count, pjmedia_sdp_attr* attr_array[],
                     pjmedia_sdp_attr* attr)
{
...
PJ_DEF(unsigned)
pjmedia_sdp_attr_remove_all(unsigned* count, pjmedia_sdp_attr* attr_array[],
                            const char* name)
{
...
PJ_DEF(pj_status_t)
pjmedia_sdp_attr_remove(unsigned* count, pjmedia_sdp_attr* attr_array[],
                        pjmedia_sdp_attr* attr)
{
    unsigned i, removed = 0;
...
PJ_DEF(pj_status_t)
pjmedia_sdp_attr_get_rtpmap(const pjmedia_sdp_attr* attr,
...
PJ_DEF(pj_status_t)
pjmedia_sdp_attr_get_fmtp(const pjmedia_sdp_attr* attr, pjmedia_sdp_fmtp* fmtp)
{
...
PJ_DEF(pj_status_t)
pjmedia_sdp_attr_get_rtcp(const pjmedia_sdp_attr* attr,
...
PJ_DEF(pjmedia_sdp_attr*)
pjmedia_sdp_attr_create_rtcp(pj_pool_t* pool, const pj_sockaddr* a)
{
    enum
    {
    ...
    if (a->addr.sa_family == pj_AF_INET()) {
        attr->value.slen = pj_ansi_snprintf(
          attr->value.ptr, ATTR_LEN, "%u IN IP4 %s", pj_sockaddr_get_port(a),
          pj_sockaddr_print(a, tmp_addr, sizeof(tmp_addr), 0));
    } else if (a->addr.sa_family == pj_AF_INET6()) {
        attr->value.slen = pj_ansi_snprintf(
          attr->value.ptr, ATTR_LEN, "%u IN IP6 %s", pj_sockaddr_get_port(a),
          pj_sockaddr_print(a, tmp_addr, sizeof(tmp_addr), 0));
...
PJ_DEF(pj_status_t)
pjmedia_sdp_attr_get_ssrc(const pjmedia_sdp_attr* attr,
                          pjmedia_sdp_ssrc_attr* ssrc)
{
    ...
    PJ_TRY
    {
        ...
        if (!pj_scan_is_eof(&scanner) && pj_scan_get_char(&scanner) == ':' &&
...
PJ_DEF(pjmedia_sdp_attr*)
pjmedia_sdp_attr_create_ssrc(pj_pool_t* pool, pj_uint32_t ssrc,
                             const pj_str_t* cname)
{
    ...
    attr->value.ptr = (char*)pj_pool_alloc(pool, cname->slen + 7 /* " cname:"*/
                                                   ...
                                                   + 1 /* NULL */);
    attr->value.slen =
      pj_ansi_snprintf(attr->value.ptr, cname->slen + 18, "%u cname:%.*s", ssrc,
...
PJ_DEF(pj_status_t)
pjmedia_sdp_attr_to_rtpmap(pj_pool_t* pool, const pjmedia_sdp_attr* attr,
...
PJ_DEF(pj_status_t)
pjmedia_sdp_rtpmap_to_attr(pj_pool_t* pool, const pjmedia_sdp_rtpmap* rtpmap,
                           pjmedia_sdp_attr** p_attr)
{
    ...
    len = pj_ansi_snprintf(tempbuf, sizeof(tempbuf), "%.*s %.*s/%u%s%.*s",
                           (int)rtpmap->pt.slen, rtpmap->pt.ptr,
                           (int)rtpmap->enc_name.slen, rtpmap->enc_name.ptr,
                           rtpmap->clock_rate, (rtpmap->param.slen ? "/" : ""),
...
static int print_connection_info(pjmedia_sdp_conn* c, char* buf, int len)
{
    ...
    printed = pj_ansi_snprintf(
      buf, len, "c=%.*s %.*s %.*s\r\n", (int)c->net_type.slen, c->net_type.ptr,
      (int)c->addr_type.slen, c->addr_type.ptr, (int)c->addr.slen, c->addr.ptr);
...
PJ_DEF(pjmedia_sdp_conn*)
pjmedia_sdp_conn_clone(pj_pool_t* pool, const pjmedia_sdp_conn* rhs)
{
    pjmedia_sdp_conn* c = PJ_POOL_ALLOC_T(pool, pjmedia_sdp_conn);
    if (!c)
        return NULL;

    if (!pj_strdup(pool, &c->net_type, &rhs->net_type))
        return NULL;
    if (!pj_strdup(pool, &c->addr_type, &rhs->addr_type))
        return NULL;
    if (!pj_strdup(pool, &c->addr, &rhs->addr))
        return NULL;
...
pjmedia_sdp_bandw_clone(pj_pool_t* pool, const pjmedia_sdp_bandw* rhs)
{
    ...
    if (!pj_strdup(pool, &b->modifier, &rhs->modifier))
        return NULL;
    b->value = rhs->value;
...
static pj_ssize_t print_bandw(const pjmedia_sdp_bandw* bandw, char* buf,
                              pj_size_t len)
{
    char* p = buf;
...
static pj_ssize_t print_attr(const pjmedia_sdp_attr* attr, char* buf,
                             pj_size_t len)
{
    char* p = buf;
...
PJ_DEF(int)
pjmedia_sdp_media_print(const pjmedia_sdp_media* media, char* buf,
                        pj_size_t size)
{
    return print_media_desc(media, buf, size);
}

PJ_DEF(pjmedia_sdp_media*)
pjmedia_sdp_media_clone(pj_pool_t* pool, const pjmedia_sdp_media* rhs)
{
    unsigned int i;
...
PJ_DEF(pjmedia_sdp_attr*)
pjmedia_sdp_media_find_attr(const pjmedia_sdp_media* m, const pj_str_t* name,
                            const pj_str_t* fmt)
{
...
PJ_DEF(pjmedia_sdp_attr*)
pjmedia_sdp_media_find_attr2(const pjmedia_sdp_media* m, const char* name,
                             const pj_str_t* fmt)
{
    PJ_ASSERT_RETURN(m && name, NULL);
...
PJ_DEF(pj_status_t)
pjmedia_sdp_media_add_attr(pjmedia_sdp_media* m, pjmedia_sdp_attr* attr)
{
    return pjmedia_sdp_attr_add(&m->attr_count, m->attr, attr);
}

PJ_DEF(pj_status_t)
pjmedia_sdp_session_add_attr(pjmedia_sdp_session* s, pjmedia_sdp_attr* attr)
{
    return pjmedia_sdp_attr_add(&s->attr_count, s->attr, attr);
}

PJ_DEF(unsigned)
pjmedia_sdp_media_remove_all_attr(pjmedia_sdp_media* m, const char* name)
{
    return pjmedia_sdp_attr_remove_all(&m->attr_count, m->attr, name);
}

PJ_DEF(pj_status_t)
pjmedia_sdp_media_remove_attr(pjmedia_sdp_media* m, pjmedia_sdp_attr* attr)
{
    return pjmedia_sdp_attr_remove(&m->attr_count, m->attr, attr);
}

static int print_session(const pjmedia_sdp_session* ses, char* buf,
                         pj_ssize_t len)
{
    char* p = buf;
    char* end = buf + len;
    ...
    if (len < 5 + 2 + ses->origin.user.slen + 18 + ses->origin.net_type.slen +
                ses->origin.addr.slen + 2)
    {
        return -1;
    }
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_sdp_parse(pj_pool_t* pool, char* buf, pj_size_t len,
                  pjmedia_sdp_session** p_sdp)
{
    ...
    {
        PJ_PERROR(
          4, (THIS_FILE, ctx.last_error, "Error parsing SDP in line %d col %d",
 ...
 */
PJ_DEF(int)
pjmedia_sdp_print(const pjmedia_sdp_session* desc, char* buf, pj_size_t size)
{
...
pjmedia_sdp_session_clone(pj_pool_t* pool, const pjmedia_sdp_session* rhs)
{
...
/* Validate SDP session descriptor. */
PJ_DEF(pj_status_t)
pjmedia_sdp_validate2(const pjmedia_sdp_session* sdp, pj_bool_t strict)
{
    unsigned i;
...
PJ_DEF(pj_status_t)
pjmedia_sdp_transport_cmp(const pj_str_t* t1, const pj_str_t* t2)
{
...
PJ_DEF(pjmedia_sdp_media*)
pjmedia_sdp_media_clone_deactivate(pj_pool_t* pool,
