...
static const struct
{
    ...
    PJ_BUILD_ERR(PJMEDIA_SDP_EMISSINGRTPMAP,
    ...
    PJ_BUILD_ERR(PJMEDIA_SDPNEG_EINSTATE,
                 "Invalid SDP negotiator state for operation"),
    PJ_BUILD_ERR(PJMEDIA_SDPNEG_ENOINITIAL,
                 "No initial local SDP in SDP negotiator"),
    ...
    PJ_BUILD_ERR(PJMEDIA_SDPNEG_EMISMEDIA,
                 "SDP media count mismatch in offer/answer"),
    PJ_BUILD_ERR(PJMEDIA_SDPNEG_EINVANSMEDIA,
                 "SDP media type mismatch in offer/answer"),
    PJ_BUILD_ERR(PJMEDIA_SDPNEG_EINVANSTP,
                 "SDP media transport type mismatch in offer/answer"),
    PJ_BUILD_ERR(PJMEDIA_SDPNEG_EANSNOMEDIA,
                 "No common SDP media payload in answer"),
    PJ_BUILD_ERR(PJMEDIA_SDPNEG_ENOMEDIA,
                 "No active media stream after negotiation"),
    PJ_BUILD_ERR(PJMEDIA_SDPNEG_NOANSCODEC,
                 "No suitable codec for remote offer"),
    PJ_BUILD_ERR(PJMEDIA_SDPNEG_NOANSTELEVENT,
                 "No suitable telephone-event for remote offer"),
    PJ_BUILD_ERR(PJMEDIA_SDPNEG_NOANSUNKNOWN,
    ...
    PJ_BUILD_ERR(PJMEDIA_SDP_EPORTNOTEQUAL,
                 "Port in SDP media descriptor not equal"),
    PJ_BUILD_ERR(PJMEDIA_SDP_ETPORTNOTEQUAL,
                 "Transport in SDP media descriptor not equal"),
    PJ_BUILD_ERR(PJMEDIA_SDP_EFORMATNOTEQUAL,
                 "Format in SDP media descriptor not equal"),
    ...
    PJ_BUILD_ERR(PJMEDIA_RTP_EBADDEST,
    ...
    PJ_BUILD_ERR(PJMEDIA_ENCCLOCKRATE,
                 "Media ports have incompatible clock rate"),
    PJ_BUILD_ERR(PJMEDIA_ENCSAMPLESPFRAME,
                 "Media ports have incompatible samples per frame"),
    PJ_BUILD_ERR(PJMEDIA_ENCTYPE, "Media ports have incompatible media type"),
    PJ_BUILD_ERR(PJMEDIA_ENCBITS,
                 "Media ports have incompatible bits per sample"),
    PJ_BUILD_ERR(PJMEDIA_ENCBYTES,
                 "Media ports have incompatible bytes per frame"),
    PJ_BUILD_ERR(PJMEDIA_ENCCHANNEL,
                 "Media ports have incompatible number of channels"),
    ...
    PJ_BUILD_ERR(PJMEDIA_EFRMFILETOOBIG,
                 "Sound frame too large for file buffer"),
...
#    if defined(PJMEDIA_HAS_SRTP) && (PJMEDIA_HAS_SRTP != 0)
    ...
    PJ_BUILD_ERR(PJMEDIA_SRTP_DTLS_ENOPROFILE,
                 "No valid SRTP protection profile found")
#    endif
 ...
 */
PJ_DEF(pj_str_t)
pjmedia_strerror(pj_status_t statcode, char* buf, pj_size_t bufsize)
{
    pj_str_t errstr;
...
#    endif
            ...
            /* PJMEDIA error */
            if (statcode >= PJMEDIA_ERRNO_START && statcode < PJMEDIA_ERRNO_END)
            {
    ...
    errstr.ptr = buf;
    errstr.slen =
      pj_ansi_snprintf(buf, bufsize, "Unknown pjmedia error %d", statcode);
