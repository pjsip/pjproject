...
PJ_DEF(pj_status_t) webrtc_aec3_get_stat(void* state, pjmedia_echo_stat* p_stat)
{
    webrtc_ec* echo = (webrtc_ec*)state;
    unsigned i = 0;
    ...
    while (echo->get_metrics && i < 100000)
        i++;
    ...
    p_stat->stat_info.slen = pj_ansi_snprintf(
      ...
      p_stat->delay, p_stat->return_loss, p_stat->return_loss_enh);
