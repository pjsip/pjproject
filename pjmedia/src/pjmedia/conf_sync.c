...
#if PJMEDIA_CONF_BACKEND == PJMEDIA_CONF_SYNC_SERIAL_BRIDGE_BACKEND
...
struct conf_port
{
    ...
    unsigned            *listener_adj_level;
                                        /**< Array of listeners' level
                                             adjustment.                    */
    unsigned            *listener_nest; /**< Array of listeners' nesting
                                             counters.                      */
...
struct pjmedia_conf
{
    ...
    SLOT_TYPE             next_free_slot; /**< Next free slot.              */
    SLOT_TYPE            *free_slots_list; /**< Linked list of free slots.  */
...
static pj_status_t init_slot_free_list(pj_pool_t *pool,
                                       pjmedia_conf *conf);
static SLOT_TYPE reserve_next_free_slot(pjmedia_conf *conf);
static pj_status_t release_reserved_slot(pjmedia_conf *conf,
                                         SLOT_TYPE slot);
                                     ...
                                     struct conf_port **p_conf_port)
{
    ...
    /* Create nesting counter array */
    conf_port->listener_nest = (unsigned *) pj_pool_zalloc(pool,
                                       conf->max_ports * sizeof(unsigned));
    PJ_ASSERT_ON_FAIL(conf_port->listener_nest,
                      {status = PJ_ENOMEM; goto on_return;});
                                         ...
                                         pjmedia_conf **p_conf )
{
    ...
    status = init_slot_free_list(pool, conf);
    if (status != PJ_SUCCESS)
        return status;
                                           ...
                                           unsigned *p_port )
{
    ...
    /* Get next free port slot in the conference bridge */
    index = reserve_next_free_slot(conf);
    ...
    if (status != PJ_SUCCESS) {
        /* Release the prev. reserved slot */
        release_reserved_slot(conf, index);
        pj_mutex_unlock(conf->mutex);
        return status;
    }
                                               ...
                                               int adj_level )
{
    ...
    /* Check if connection has been made already */
    for (i=0; i<src_port->listener_cnt; ++i) {
        if (src_port->listener_slots[i] == sink_slot) {
            /* only increment nesting */
            ++src_port->listener_nest[i];
            pj_mutex_unlock(conf->mutex);
            return PJ_EEXISTS;
        }
    }
                                                  ...
                                                  unsigned sink_slot )
{
    ...
    if ((i != src_port->listener_cnt) && (src_port->listener_nest[i] == 0U)) {
        /* Sanity checks */
        ...
        pj_array_erase(src_port->listener_adj_level, sizeof(unsigned),
                       src_port->listener_cnt, i);
        pj_array_erase(src_port->listener_nest, sizeof(unsigned),
                       src_port->listener_cnt, i);
        --conf->connect_cnt;
        --src_port->listener_cnt;
        --dst_port->transmitter_cnt;
    ...
    else if (i != src_port->listener_cnt) {
        /* only decrement nesting */
        --src_port->listener_nest[i];
        status = PJ_EBUSY;
    }
    else
                                              ...
                                              unsigned port )
{
    ...
    /* Release the slot if it's reserved */
    release_reserved_slot(conf, port);
...
/*
 * Initialize the slot free list.
 */
static pj_status_t init_slot_free_list(pj_pool_t *pool, pjmedia_conf *conf)
{
    unsigned i;

    conf->free_slots_list = (SLOT_TYPE*)
                            pj_pool_zalloc(pool, conf->max_ports*sizeof(SLOT_TYPE));
    PJ_ASSERT_RETURN(conf->free_slots_list, PJ_ENOMEM);

    /* First free slot */
    conf->next_free_slot = 1;

    /* Slot 0 is used for master port, mark as reserved */
    conf->free_slots_list[0] = conf->max_ports;

    /* Init the linked list */
    for (i=1; i<conf->max_ports-1; ++i) {
            conf->free_slots_list[i] = i+1;
    }

    conf->free_slots_list[i] = conf->max_ports;
...
static SLOT_TYPE reserve_next_free_slot(pjmedia_conf *conf)
{
    SLOT_TYPE next = conf->next_free_slot;

    if (next != conf->max_ports) {
        conf->next_free_slot = conf->free_slots_list[next];
        conf->free_slots_list[next] = conf->max_ports;
    }
 ...
 */
static pj_status_t release_reserved_slot(pjmedia_conf *conf, SLOT_TYPE slot)
{
    PJ_ASSERT_RETURN((slot < conf->max_ports) &&
                     (conf->free_slots_list[slot] == conf->max_ports), PJ_EINVAL);

    conf->free_slots_list[slot] = conf->next_free_slot;
    conf->next_free_slot = slot;

    return PJ_SUCCESS;
}


#endif   /* PJMEDIA_CONF_BACKEND == PJMEDIA_CONF_SYNC_SERIAL_BRIDGE_BACKEND */
