 ...
 */
PJ_DEF(pj_status_t)
pjmedia_splitcomb_create(pj_pool_t* pool, unsigned clock_rate,
                         ...
                         pjmedia_port** p_splitcomb)
{
    ...
    *p_splitcomb = NULL;
    ...
    sc->get_buf = (TMP_SAMP_TYPE*)pj_pool_alloc(
      pool, samples_per_frame * sizeof(TMP_SAMP_TYPE) / channel_count);
    PJ_ASSERT_RETURN(sc->get_buf, PJ_ENOMEM);

    sc->put_buf = (TMP_SAMP_TYPE*)pj_pool_alloc(
      pool, samples_per_frame * sizeof(TMP_SAMP_TYPE) / channel_count);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_splitcomb_set_channel(pjmedia_port* splitcomb, unsigned ch_num,
                              unsigned options, pjmedia_port* port)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_splitcomb_create_rev_channel(pj_pool_t* pool, pjmedia_port* splitcomb,
                                     unsigned ch_num, unsigned options,
                                     pjmedia_port** p_chport)
{
    ...
    pjmedia_port_info_init(
      &port->info, &name, SIGNATURE_PORT, sc_afd->clock_rate, 1,
      sc_afd->bits_per_sample,
      PJMEDIA_PIA_SPF(&splitcomb->info) / sc_afd->channel_count);
    ...
    status = pjmedia_delay_buf_create(
      pool, "scombdb-dn", p_afd->clock_rate, PJMEDIA_PIA_SPF(&port->info),
      p_afd->channel_count, buf_cnt * p_afd->frame_time_usec / 1000,
      buf_options, &rport->buf[DIR_DOWNSTREAM].dbuf);
    ...
    status = pjmedia_delay_buf_create(
      pool, "scombdb-up", p_afd->clock_rate, PJMEDIA_PIA_SPF(&port->info),
      p_afd->channel_count, buf_cnt * p_afd->frame_time_usec / 1000,
    ...
    rport->tmp_up_buf =
      (pj_int16_t*)pj_pool_alloc(pool, PJMEDIA_PIA_AVG_FSZ(&port->info));
 ...
 */
static void extract_mono_frame(const pj_int16_t* in, pj_int16_t* out,
                               unsigned ch, unsigned ch_cnt,
 ...
 */
static void store_mono_frame(const pj_int16_t* in, pj_int16_t* out, unsigned ch,
                             unsigned ch_cnt, unsigned samples_count)
{
    unsigned i;
    ...
    if (frame->type == PJMEDIA_FRAME_TYPE_NONE) {
        for (ch = 0; ch < PJMEDIA_PIA_CCNT(&this_port->info); ++ch) {
            ...
            if (!sc->port_desc[ch].reversed) {
                ...
                /* Generate zero frame. */
                pjmedia_zero_samples(sc->put_buf, PJMEDIA_PIA_SPF(&port->info));
    ...
    for (ch = 0; ch < PJMEDIA_PIA_CCNT(&this_port->info); ++ch) {
        ...
        if (!port) {
            ...
            if (status != PJ_SUCCESS ||
                ...
                pjmedia_zero_samples(sc->get_buf, PJMEDIA_PIA_SPF(&port->info));
            }
        ...
        store_mono_frame(sc->get_buf, (pj_int16_t*)frame->buf, ch,
                         PJMEDIA_PIA_CCNT(&this_port->info),
                         PJMEDIA_PIA_SPF(&this_port->info) /
                           PJMEDIA_PIA_CCNT(&this_port->info));
