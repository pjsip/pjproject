...
#if OPENSSL_VERSION_NUMBER >= 0x10100000L && defined(OPENSSL_API_COMPAT) && \
...
static pj_status_t dtls_media_create(pjmedia_transport* tp, pj_pool_t* sdp_pool,
                                     unsigned options,
                                     const pjmedia_sdp_session* sdp_remote,
                                     unsigned media_index);
static pj_status_t dtls_encode_sdp(pjmedia_transport* tp, pj_pool_t* sdp_pool,
                                   pjmedia_sdp_session* sdp_local,
                                   const pjmedia_sdp_session* sdp_remote,
                                   unsigned media_index);
static pj_status_t dtls_media_start(pjmedia_transport* tp, pj_pool_t* tmp_pool,
                                    const pjmedia_sdp_session* sdp_local,
                                    const pjmedia_sdp_session* sdp_remote,
                                    unsigned media_index);
static pj_status_t dtls_media_stop(pjmedia_transport* tp);
static pj_status_t dtls_destroy(pjmedia_transport* tp);
static pj_status_t dtls_on_recv_rtp(pjmedia_transport* tp, const void* pkt,
                                    pj_size_t size);

static void on_ice_complete2(pjmedia_transport* tp, pj_ice_strans_op op,
...
static char* ossl_profiles[OPENSSL_PROFILE_NUM] = { "SRTP_AES128_CM_SHA1_80",
                                                    "SRTP_AES128_CM_SHA1_32",
                                                    "SRTP_AEAD_AES_256_GCM",
                                                    "SRTP_AEAD_AES_128_GCM" };
static char* pj_profiles[OPENSSL_PROFILE_NUM] = { "AES_CM_128_HMAC_SHA1_80",
...
static pj_status_t dtls_init()
{
    ...
    if (valid_profiles_cnt == 0) {
        ...
        for (j = 0; j < PJ_ARRAY_SIZE(ossl_profiles); ++j) {
            ...
            if (rc == 0) {
                valid_pj_profiles_list[valid_profiles_cnt] = pj_profiles[j];
                               ...
                               pjmedia_transport** p_keying)
{
    ...
    pool = pj_pool_create(srtp->pool->factory, "dtls%p", 2000, 256, NULL);
...
static pj_status_t STATUS_FROM_SSL_ERR(dtls_srtp* ds, unsigned long err)
{
    pj_status_t status;
...
static pj_status_t ssl_generate_cert(X509** p_cert, EVP_PKEY** p_priv_key)
{
    ...
    if (!X509_set_issuer_name(cert, cert_name))
        goto on_error;
    ...
    if (!X509_sign(cert, priv_key, EVP_sha1()))
        goto on_error;
...
static pj_status_t ssl_create(dtls_srtp* ds)
{
    ...
    if (!SSL_ctrl(ds->ossl_ssl, DTLS_CTRL_SET_LINK_MTU, PJMEDIA_MAX_MTU, NULL))
    {
...
static pj_status_t ssl_get_srtp_material(dtls_srtp* ds)
{
    ...
    /* Parse SRTP master key & salt from keying material */
    {
        ...
        pj_memcpy(k1, p, key_len);
        p += key_len;
        pj_memcpy(k2, p, key_len);
        p += key_len;
        pj_memcpy(k1 + key_len, p, salt_len);
        p += salt_len;
...
static pj_status_t ssl_match_fingerprint(dtls_srtp* ds)
{
    ...
    if (!pj_strnicmp2(&ds->rem_fingerprint, "SHA-256 ", 8))
        ...
        PJ_LOG(4, (ds->base.name,
                   "Hash algo specified in remote SDP for "
...
static pj_status_t ssl_flush_wbio(dtls_srtp* ds)
{
    ...
    if (status != PJ_SUCCESS) {
        pj_perror(4, ds->base.name, status, "Failed to get SRTP material");
        goto on_return;
    }
...
on_return:
    if (ds->srtp->setting.cb.on_srtp_nego_complete) {
        (*ds->srtp->setting.cb.on_srtp_nego_complete)(&ds->srtp->base, status);
    }
...
static pj_status_t ssl_handshake(dtls_srtp* ds)
{
    ...
    if (!ds->clock) {
        status =
          pjmedia_clock_create(ds->pool, 4, 1, 1, PJMEDIA_CLOCK_NO_HIGHEST_PRIO,
                               clock_cb, ds, &ds->clock);
        if (status != PJ_SUCCESS)
            goto on_return;
    }
                                           ...
                                           unsigned media_index)
{
    ...
    if (!a)
        a = pjmedia_sdp_attr_find(sdp->attr_count, sdp->attr, &ID_SETUP, NULL);
    if (!a)
    ...
    if (!a)
        a = pjmedia_sdp_attr_find(sdp->attr_count, sdp->attr, &ID_FINGERPRINT,
                                  NULL);
    if (!a) {
...
static pj_status_t get_rem_addrs(dtls_srtp* ds,
                                 const pjmedia_sdp_session* sdp_remote,
                                 unsigned media_index, pj_sockaddr* rem_rtp,
                                 pj_sockaddr* rem_rtcp, pj_bool_t* rtcp_mux)
{
    ...
    if (!use_ice_info) {
        ...
        if (af != pj_AF_UNSPEC()) {
            pj_sockaddr_init(af, rem_rtp, &conn->addr, m_rem->desc.port);
        } else {
            return PJ_EAFNOTSUP;
        }
         ...
         */
        a =
          pjmedia_sdp_attr_find2(m_rem->attr_count, m_rem->attr, "rtcp", NULL);
        if (a) {
    ...
    if (rtcp_mux) {
        a = pjmedia_sdp_attr_find2(m_rem->attr_count, m_rem->attr, "rtcp-mux",
                                   NULL);
...
static pj_status_t ssl_on_recv_packet(dtls_srtp* ds, const void* data,
                                      pj_size_t len)
{
    char tmp[128];
    pj_size_t nwritten;

    if (ds->ossl_rbio == NULL)
        return PJ_EGONE;
    ...
    if (!ds->use_ice && !ds->nego_completed) {
        ...
        if (pj_sockaddr_cmp(&ds->rem_addr, &info.src_rtp_name)) {
...
#if DTLS_DEBUG
            {
                ...
                PJ_LOG(2,
                       (ds->base.name,
                        "Re-attached transport to update "
                        "remote addr=%s:%d",
                                     ...
                                     unsigned media_index)
{
    ...
    if (ds->srtp->offerer_side) {
        ...
        if (!attr_fp)
            attr_fp = pjmedia_sdp_attr_find(
              sdp_remote->attr_count, sdp_remote->attr, &ID_FINGERPRINT, NULL);
                                   ...
                                   unsigned media_index)
{
    ...
    if (ds->srtp->offerer_side) {
        ...
        if (!a)
            a = pjmedia_sdp_attr_find(sdp_local->attr_count, sdp_local->attr,
                                      &ID_SETUP, NULL);
        if (!a) {
            pj_str_t val;
        ...
        status = parse_setup_finger_attr(ds, PJ_TRUE, sdp_remote, media_index);
        if (status != PJ_SUCCESS)
            goto on_return;

        /* Add attribute a=setup:active/passive if we are client/server. */
        a = pjmedia_sdp_attr_create(
          ds->pool, ID_SETUP.ptr,
            ...
            if (status == PJ_SUCCESS) {
                ...
                if (pj_sockaddr_has_addr(&ds->rem_addr) &&
                    ...
                    (pj_sockaddr_cmp(&ds->rem_addr, &rem_rtp) ||
                     (!use_rtcp_mux && pj_sockaddr_has_addr(&ds->rem_rtcp) &&
    ...
    if (ds->srtp->offerer_side) {
        ...
        m_loc->desc.transport = sdp_remote->media[media_index]->desc.transport;
    }
                                    ...
                                    unsigned media_index)
{
    ...
    if (ds->srtp->offerer_side) {
        ...
        /* Parse a=setup and a=fingerprint */
        status = parse_setup_finger_attr(ds, PJ_FALSE, sdp_remote, media_index);
        if (status != PJ_SUCCESS)
            goto on_return;
    ...
    /* Check and update ICE and rtcp-mux status */
    {
        ...
        get_rem_addrs(ds, sdp_remote, media_index, &ds->rem_addr, &ds->rem_rtcp,
                      NULL);
    }
...
PJ_DEF(pj_status_t)
pjmedia_transport_srtp_dtls_get_fingerprint(pjmedia_transport* tp,
                                            ...
                                            pj_size_t* len)
{
    ...
    PJ_ASSERT_RETURN(pj_ansi_strcmp(hash, "SHA-256") == 0 ||
                       pj_ansi_strcmp(hash, "SHA-1") == 0,
                     PJ_EINVAL);
    PJ_UNUSED_ARG(tp);

    return ssl_get_fingerprint(dtls_cert, pj_ansi_strcmp(hash, "SHA-256") == 0,
                               buf, len);
}
...
pjmedia_transport_srtp_dtls_start_nego(
  pjmedia_transport* tp, const pjmedia_srtp_dtls_nego_param* param)
{
