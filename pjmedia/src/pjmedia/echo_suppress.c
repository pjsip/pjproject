                 ...
                 void** p_state)
{
    ...
    ec->rec_hist = (pj_uint16_t*)pj_pool_alloc(
      pool, ec->templ_cnt * sizeof(ec->rec_hist[0]));

    /* Note: play history has twice number of elements */
    ec->play_hist = (pj_uint16_t*)pj_pool_alloc(
      pool, ec->play_hist_cnt * sizeof(ec->play_hist[0]));

    ec->corr_sum =
      (float*)pj_pool_alloc(pool, ec->tail_cnt * sizeof(ec->corr_sum[0]));
    ec->tmp_corr =
      (float*)pj_pool_alloc(pool, ec->tail_cnt * sizeof(ec->tmp_corr[0]));
    ec->min_factor =
      (float*)pj_pool_alloc(pool, ec->tail_cnt * sizeof(ec->min_factor[0]));
    ec->avg_factor =
      (float*)pj_pool_alloc(pool, ec->tail_cnt * sizeof(ec->avg_factor[0]));
    ec->tmp_factor =
      (float*)pj_pool_alloc(pool, ec->tail_cnt * sizeof(ec->tmp_factor[0]));
    echo_supp_reset(ec);
                                ...
                                unsigned level)
{
    ...
    if (state != ec->talk_state) {
        TRACE_((THIS_FILE, "[%03d.%03d] %s --> %s, level=%u",
                ...
                ((ec->update_cnt * SEGMENT_PTIME) % 1000),
                state_names[ec->talk_state], state_names[state], level));
        ec->talk_state = state;
    }
}
                             ...
                             const pj_int16_t* play_frm)
{
    ...
    if (ec->sum_rec_level == 0) {
        ...
        ec->rec_corr = ec->rec_corr -
                       ((float)ec->rec_hist[0] / old_rec_frm_level) +
    ...
    if (ec->sum_play_level0 == 0) {
        ...
        ec->play_corr0 = ec->play_corr0 -
                         ((float)ec->play_hist[0] / old_play_frm_level) +
    ...
    for (i = 1; i < ec->tail_cnt; ++i) {
        ...
        sum_play_level =
          sum_play_level - ec->play_hist[i - 1] + ec->play_hist[end - 1];
        play_corr = play_corr -
                    ((float)ec->play_hist[i] / ec->play_hist[i - 1]) +
    ...
    for (i = 1; i < ec->tail_cnt - 1; ++i) {
        ...
        ec->avg_factor[i] =
          ((ec->avg_factor[i] * ec->tail_cnt) + ec->tmp_factor[i]) /
          (ec->tail_cnt + 1);
    ...
    if (ec->tail_index != prev_index) {
        ...
        PJ_LOG(4, (THIS_FILE,
                   ...
                   (ec->tail_cnt - ec->tail_index) * SEGMENT_PTIME, imin / 1000,
                   imin % 1000, iavg / 1000, iavg % 1000));
    }
...
/* Amplify frame */
static void amplify_frame(pj_int16_t* frm, unsigned length, pj_ufloat_t factor)
{
    unsigned i;
                      ...
                      void* reserved)
{
    ...
    if (ec->tail_index < 0) {
        ...
        if (ec->talk_state == ST_REM_TALK) {
            ...
            ec->residue =
              ((ec->residue * ec->running_cnt) + level) / (ec->running_cnt + 1);
...
PJ_DEF(pj_status_t) echo_supp_get_stat(void* state, pjmedia_echo_stat* p_stat)
{
    echo_supp* ec = (echo_supp*)state;
    ...
    p_stat->stat_info.slen = pj_ansi_snprintf(
      p_stat->buf_, sizeof(p_stat->buf_),
      "Echo suppressor learning %s at t=%03d.%03ds, tail=%d ms,\n"
      "          factor min/avg=%d.%03d/%d.%03d",
      (ec->learning ? "in progress" : "done"), (p_stat->duration / 1000),
      (p_stat->duration % 1000), p_stat->tail, p_stat->min_factor / 1000,
      p_stat->min_factor % 1000, p_stat->avg_factor / 1000,
      p_stat->avg_factor % 1000);
