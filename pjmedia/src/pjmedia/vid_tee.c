                       ...
                       unsigned max_dst_cnt, pjmedia_port** p_vid_tee)
{
    ...
    tee->dst_ports = (vid_tee_dst_port*)pj_pool_calloc(
      pool, max_dst_cnt, sizeof(vid_tee_dst_port));
    tee->tee_conv = (struct vid_tee_conv_t*)pj_pool_calloc(
      pool, max_dst_cnt, sizeof(struct vid_tee_conv_t));
    tee->put_frm_flag = (pj_uint8_t*)pj_pool_calloc(
      pool, max_dst_cnt, sizeof(tee->put_frm_flag[0]));
    ...
    status = pjmedia_port_info_init2(&tee->base.info,
                                     ...
                                     TEE_PORT_SIGN, PJMEDIA_DIR_ENCODING, fmt);
    if (status != PJ_SUCCESS)
        return status;
...
static void realloc_buf(vid_tee_port* vid_tee, unsigned buf_cnt,
                        pj_size_t buf_size)
{
    unsigned i;
    ...
    if (!vid_tee->buf_pool) {
        vid_tee->buf_pool =
          pj_pool_create(vid_tee->pf, "video tee buffer", 1000, 1000, NULL);
    }

    for (i = 0; i < vid_tee->buf_cnt; i++) {
        if (!vid_tee->buf[i])
            vid_tee->buf[i] =
              pj_pool_alloc(vid_tee->buf_pool, vid_tee->buf_size);
    }
}
                             ...
                             pjmedia_port* port)
{
    ...
    realloc_buf(tee, (option & PJMEDIA_VID_TEE_DST_DO_IN_PLACE_PROC) ? 1 : 0,
                tee->buf_size);
                              ...
                              pjmedia_port* port)
{
        ...
        vfd->size.h != vid_tee->info.fmt.det.vid.size.h)
    {
        ...
        realloc_buf(tee,
                    (option & PJMEDIA_VID_TEE_DST_DO_IN_PLACE_PROC) ? 2 : 1,
                    vafp.framebytes);
        ...
        status = pjmedia_converter_create(
          NULL, tee->pool, &conv_param, &tee->tee_conv[tee->dst_port_cnt].conv);
        if (status != PJ_SUCCESS)
            return status;
        ...
        realloc_buf(tee,
                    (option & PJMEDIA_VID_TEE_DST_DO_IN_PLACE_PROC) ? 1 : 0,
                    tee->buf_size);
    }
...
static pj_status_t tee_put_frame(pjmedia_port* port, pjmedia_frame* frame)
{
    ...
    pj_bzero(tee->put_frm_flag,
             tee->dst_port_cnt * sizeof(tee->put_frm_flag[0]));
    ...
    for (i = 0; i < tee->dst_port_cnt; ++i) {
        ...
        if (tee->tee_conv[i].conv) {
            ...
            status =
              pjmedia_converter_convert(tee->tee_conv[i].conv, frame, &frame_);
            if (status != PJ_SUCCESS) {
                PJ_PERROR(3, (THIS_FILE, status,
                              "Failed to convert frame for destination"
                              " port %d (%.*s)",
                              i, tee->dst_ports[i].dst->info.name.slen,
