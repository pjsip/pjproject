                                             ...
                                             unsigned* clock_rate)
{
    ...
    if (!local_clock_rate)
        local_clock_rate = *clock_rate;
    if (!local_channel_cnt)
        local_channel_cnt = *channel_cnt;
     ...
     */
    if (!enc_clock_rate)
        enc_clock_rate = 48000;
    if (!enc_channel_cnt)
        enc_channel_cnt = 1;

    *clock_rate =
      (enc_clock_rate < local_clock_rate) ? enc_clock_rate : local_clock_rate;
                                              ...
                                              const pjmedia_sdp_media* rem_m)
{
    ...
    if (pt < 96) {
        ...
        status = pjmedia_codec_mgr_find_codecs_by_id(mgr, &codec_id_st, &i,
                                                     &p_info, NULL);
        if (status != PJ_SUCCESS)
            return status;
        ...
        for (i = 0; i < rem_m->desc.fmt_count; ++i) {
            if (pjmedia_sdp_neg_fmt_match(pool, (pjmedia_sdp_media*)local_m,
                                          fmti, (pjmedia_sdp_media*)rem_m, i,
                                          0) == PJ_SUCCESS)
            {
                /* Found matched codec. */
    ...
    if (!pj_stricmp2(&si->fmt.encoding_name, "opus")) {
        get_opus_channels_and_clock_rate(
          &si->param->setting.enc_fmtp, &si->param->setting.dec_fmtp,
          &si->fmt.channel_cnt, &si->fmt.clock_rate);
    }

    /* Get the remote ptime for our encoder. */
    attr =
      pjmedia_sdp_attr_find2(rem_m->attr_count, rem_m->attr, "ptime", NULL);
    if (attr) {
    ...
    attr =
      pjmedia_sdp_attr_find2(rem_m->attr_count, rem_m->attr, "maxptime", NULL);
    if (attr) {
        pj_str_t tmp_val = attr->value;
                             ...
                             unsigned stream_idx)
{
    ...
    if (status == PJ_ERESOLVE && rem_af == pj_AF_INET()) {
         ...
         */
        status = pj_sockaddr_init(pj_AF_INET6(), &si->rem_addr, &rem_conn->addr,
                                  rem_m->desc.port);
    }
    if (status != PJ_SUCCESS) {
        /* Invalid IP address. */
    ...
    } else if (pjmedia_sdp_media_find_attr(local_m, &STR_SENDONLY, NULL) !=
               NULL) {
        /* Send only stream. */

        si->dir = PJMEDIA_DIR_ENCODING;

    } else if (pjmedia_sdp_media_find_attr(local_m, &STR_RECVONLY, NULL) !=
               NULL) {
        /* Recv only stream. */
    ...
    attr =
      pjmedia_sdp_attr_find2(rem_m->attr_count, rem_m->attr, "rtcp-mux", NULL);
    if (attr)
        si->rtcp_mux = PJ_TRUE;
