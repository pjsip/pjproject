...
static struct plc_alg plc_wsola = { &plc_wsola_create, &plc_wsola_save,
                                    &plc_wsola_generate };
 ...
 */
PJ_DEF(pj_status_t) pjmedia_plc_save(pjmedia_plc* plc, pj_int16_t* frame)
{
 ...
 */
PJ_DEF(pj_status_t) pjmedia_plc_generate(pjmedia_plc* plc, pj_int16_t* frame)
{
    PJ_ASSERT_RETURN(plc && frame, PJ_EINVAL);
                              ...
                              unsigned samples_per_frame)
{
    ...
    status = pjmedia_wsola_create(pool, clock_rate, samples_per_frame, 1, flag,
                                  &o->wsola);
