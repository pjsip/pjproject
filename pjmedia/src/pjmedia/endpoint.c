...
pj_bool_t pjmedia_add_rtpmap_for_static_pt = PJMEDIA_ADD_RTPMAP_FOR_STATIC_PT;

/* Config to control use of RFC3890 TIAS */
pj_bool_t pjmedia_add_bandwidth_tias_in_sdp = PJMEDIA_ADD_BANDWIDTH_TIAS_IN_SDP;
                      ...
                      unsigned worker_cnt, pjmedia_endpt** p_endpt)
{
    ...
    if (endpt->ioqueue == NULL) {
        ...
        if (worker_cnt == 0) {
            PJ_LOG(4, (THIS_FILE,
                       "Warning: no worker thread is created in"
    ...
    for (i = 0; i < worker_cnt; ++i) {
        status = pj_thread_create(endpt->pool, "media", &worker_proc, endpt, 0,
                                  0, &endpt->thread[i]);
        if (status != PJ_SUCCESS)
            goto on_error;
    }
...
static pj_status_t init_sdp_media(pjmedia_sdp_media* m, pj_pool_t* pool,
                                  ...
                                  pjmedia_dir dir)
{
    ...
    m->conn->addr_type =
      (addr->addr.sa_family == pj_AF_INET()) ? STR_IP4 : STR_IP6;
...
#    endif
    ...
    for (i = 0; i < endpt->codec_mgr.codec_cnt; ++i) {
        ...
        if (codec_param.setting.dec_fmtp.cnt > 0) {
            ...
            /* Print codec PT */
            n = pj_ansi_snprintf(buf, MAX_FMTP_STR_LEN - buf_len, "%d", pt);
            ...
            for (ii = 0; ii < dec_fmtp->cnt; ++ii) {
                ...
                /* Print delimiter */
                n = pj_ansi_snprintf(&buf[buf_len], MAX_FMTP_STR_LEN - buf_len,
                                     (ii == 0 ? " " : ";"));
                buf_len = PJ_MIN(buf_len + n, MAX_FMTP_STR_LEN);
                ...
                if (dec_fmtp->param[ii].name.slen)
                    n = pj_ansi_snprintf(
                      &buf[buf_len], MAX_FMTP_STR_LEN - buf_len, "%.*s=%.*s",
  ...
  PJMEDIA_RTP_PT_TELEPHONE_EVENTS != 0
    ...
    if (endpt->has_telephone_event) {
        for (i = 0; i < televent_num; i++) {
            ...
            pj_ansi_snprintf(buf, sizeof(buf), "%d telephone-event/%d", pt,
                             televent_clockrates[i]);
                               ...
                               pjmedia_sdp_media** p_m)
{
    ...
    status =
      pjmedia_vid_codec_mgr_enum_codecs(NULL, &cnt, codec_info, codec_prio);
    ...
    for (i = 0; i < cnt; ++i) {
        ...
        if (i > PJMEDIA_MAX_SDP_FMT) {
             ...
             */
            PJ_PERROR(3,
                      (THIS_FILE, PJ_ETOOMANY, "Skipping some video codecs"));
            break;
        }
        ...
        if (codec_param.dec_fmtp.cnt > 0) {
            ...
            /* Print codec PT */
            n = pj_ansi_snprintf(buf, MAX_FMTP_STR_LEN - buf_len, "%d",
                                 codec_info[i].pt);
            buf_len = PJ_MIN(buf_len + n, MAX_FMTP_STR_LEN);
        ...
        /* Find maximum bitrate in this media */
        vfd =
          pjmedia_format_get_video_format_detail(&codec_param.enc_fmt, PJ_TRUE);
                         ...
                         pjmedia_sdp_session** p_sdp)
{
    ...
    status = pjmedia_endpt_create_audio_sdp(endpt, pool, &sock_info[0], 0, &m);
    if (status != PJ_SUCCESS)
        return status;
...
#if defined(PJMEDIA_HAS_VIDEO) && (PJMEDIA_HAS_VIDEO != 0)
    {
        ...
        for (i = 1; i < stream_cnt; ++i) {
            status =
              pjmedia_endpt_create_video_sdp(endpt, pool, &sock_info[i], 0, &m);
            if (status != PJ_SUCCESS)
                return status;
            sdp->media[sdp->media_count++] = m;
        }
...
PJ_DEF(pj_status_t) pjmedia_endpt_dump(pjmedia_endpt* endpt)
{
    ...
    if (pjmedia_codec_mgr_enum_codecs(&endpt->codec_mgr, &count, codec_info,
                                      prio) != PJ_SUCCESS)
    {
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (pjmedia_codec_mgr_get_default_param(
              &endpt->codec_mgr, &codec_info[i], &param) != PJ_SUCCESS)
        {
        ...
        PJ_LOG(
          ...
          (THIS_FILE,
           ...
           type, i, codec_info[i].pt, (int)codec_info[i].encoding_name.slen,
           codec_info[i].encoding_name.ptr, codec_info[i].clock_rate / 1000,
           codec_info[i].channel_cnt, good_number(bps, param.info.avg_bps),
           param.info.frm_ptime * param.setting.frm_per_pkt,
           (param.setting.vad ? " vad" : ""), (param.setting.cng ? " cng" : ""),
