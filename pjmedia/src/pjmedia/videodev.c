...
static struct cap_info
{
    const char* name;
    const char* info;
} cap_infos[] = { DEFINE_CAP("format", "Video format"),
 ...
 * factory only.
 */
#    define MAKE_DEV_ID(f_id, index) \
        (((f_id & 0xFFFF) << 16) | (index & 0xFFFF))
...
PJ_DEF(const char*)
pjmedia_vid_dev_cap_name(pjmedia_vid_dev_cap cap, const char** p_desc)
{
    const char* desc;
    unsigned i;
...
static pj_status_t get_cap_pointer(const pjmedia_vid_dev_param* param,
                                   pjmedia_vid_dev_cap cap, void** ptr,
                                   unsigned* size)
{
#    define FIELD_INFO(name) \
        *ptr = (void*)&param->name; \
...
/* API: init driver */
PJ_DEF(pj_status_t) pjmedia_vid_driver_init(unsigned drv_idx, pj_bool_t refresh)
{
    ...
    if (dev_cnt + vid_subsys.dev_cnt > MAX_DEVS) {
        PJ_LOG(4, (THIS_FILE,
                   "%d device(s) cannot be registered because"
                              ...
                              unsigned* p_local_index)
{
    ...
    if (id < 0) {
        ...
        for (i = 0; i < vid_subsys.drv_cnt; ++i) {
            pjmedia_vid_driver* drv = &vid_subsys.drv[i];
            if (id == PJMEDIA_VID_DEFAULT_CAPTURE_DEV && drv->cap_dev_idx >= 0)
            {
                id = drv->cap_dev_idx;
...
pjmedia_vid_dev_get_global_index(const pjmedia_vid_dev_factory* f,
                                 unsigned local_idx, pjmedia_vid_dev_index* pid)
{
...
PJ_DEF(pj_status_t)
pjmedia_vid_dev_get_info(pjmedia_vid_dev_index id, pjmedia_vid_dev_info* info)
{
    pjmedia_vid_dev_factory* f;
    unsigned index;
    pj_status_t status;
...
/* API: find device */
PJ_DEF(pj_status_t)
pjmedia_vid_dev_lookup(const char* drv_name, const char* dev_name,
                       pjmedia_vid_dev_index* id)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_dev_default_param(pj_pool_t* pool, pjmedia_vid_dev_index id,
...
PJ_DEF(pj_status_t)
pjmedia_vid_dev_stream_create(pjmedia_vid_dev_param* prm,
                              ...
                              pjmedia_vid_dev_stream** p_vid_strm)
{
    ...
    PJ_ASSERT_RETURN(
      (prm->dir != PJMEDIA_DIR_CAPTURE_RENDER) || (cap_f == rend_f),
      PJMEDIA_EVID_INVDEV);

    /* Create the stream */
    status = f->op->create_stream(f, prm, cb, user_data, p_vid_strm);
    if (status != PJ_SUCCESS)
        return status;
...
PJ_DEF(pj_status_t)
pjmedia_vid_dev_stream_get_param(pjmedia_vid_dev_stream* strm,
                                 pjmedia_vid_dev_param* param)
{
    pj_status_t status;
...
PJ_DEF(pj_status_t)
pjmedia_vid_dev_stream_get_cap(pjmedia_vid_dev_stream* strm,
...
PJ_DEF(pj_status_t)
pjmedia_vid_dev_stream_set_cap(pjmedia_vid_dev_stream* strm,
...
PJ_DEF(pj_status_t)
pjmedia_vid_dev_stream_get_frame(pjmedia_vid_dev_stream* strm,
                                 pjmedia_frame* frame)
{
...
PJ_DEF(pj_status_t)
pjmedia_vid_dev_stream_put_frame(pjmedia_vid_dev_stream* strm,
...
PJ_DEF(pj_status_t) pjmedia_vid_dev_stream_destroy(pjmedia_vid_dev_stream* strm)
{
    strm->sys.is_running = PJ_FALSE;
