                        ...
                        pjmedia_resample** p_resample)
{
    ...
    resample->state =
      speex_resampler_init(channel_count, rate_in, rate_out, quality, &err);
    ...
    *p_resample = resample;
    ...
    PJ_LOG(5,
           (THIS_FILE, "resample created: quality=%d, ch=%d, in/out rate=%d/%d",
                     ...
                     pj_int16_t* output)
{
    ...
    speex_resampler_process_interleaved_int(
      resample->state, (const spx_int16_t*)input, &in_length,
      (spx_int16_t*)output, &out_length);
