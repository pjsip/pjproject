...
static pj_status_t jb_framelist_init(pj_pool_t* pool, jb_framelist_t* framelist,
                                     unsigned frame_size, unsigned max_count)
{
    ...
    framelist->content = (char*)pj_pool_alloc(
      pool, (pj_size_t)framelist->frame_size * framelist->max_count);
    framelist->frame_type = (int*)pj_pool_alloc(
      pool, sizeof(framelist->frame_type[0]) * framelist->max_count);
    framelist->content_len = (pj_size_t*)pj_pool_alloc(
      pool, sizeof(framelist->content_len[0]) * framelist->max_count);
    framelist->bit_info = (pj_uint32_t*)pj_pool_alloc(
      pool, sizeof(framelist->bit_info[0]) * framelist->max_count);
    framelist->ts = (pj_uint32_t*)pj_pool_alloc(
      pool, sizeof(framelist->ts[0]) * framelist->max_count);
...
static pj_status_t jb_framelist_reset(jb_framelist_t* framelist)
{
    ...
    pj_memset(framelist->frame_type, PJMEDIA_JB_MISSING_FRAME,
              sizeof(framelist->frame_type[0]) * framelist->max_count);

    pj_bzero(framelist->content_len,
             sizeof(framelist->content_len[0]) * framelist->max_count);
...
static pj_bool_t jb_framelist_get(jb_framelist_t* framelist, void* frame,
                                  pj_size_t* size,
                                  pjmedia_jb_frame_type* p_type,
                                  pj_uint32_t* bit_info, pj_uint32_t* ts,
                                  int* seq)
{
    if (framelist->size) {
            ...
            if (prev_discarded) {
                ...
                *p_type = PJMEDIA_JB_MISSING_FRAME;
                ...
                if (max_size < frm_size) {
                    ...
                    PJ_LOG(4, (THIS_FILE,
                               "Warning: buffer too small for the "
                               "retrieved frame!"));
                }
                ...
                pj_memcpy(
                  ...
                  copy_size);
                *p_type =
                  (pjmedia_jb_frame_type)framelist->frame_type[framelist->head];
                if (size)
                    *size = copy_size;
                if (bit_info)
...
static pj_bool_t jb_framelist_peek(jb_framelist_t* framelist, unsigned offset,
                                   const void** frame, pj_size_t* size,
                                   ...
                                   int* seq)
{
    ...
    if (type)
        *type = (pjmedia_jb_frame_type)framelist->frame_type[pos];
    if (size)
...
static pj_status_t jb_framelist_put_at(jb_framelist_t* framelist, int index,
                                       const void* frame, unsigned frame_size,
                                       pj_uint32_t bit_info, pj_uint32_t ts,
                                       unsigned frame_type)
{
    int distance;
    ...
    if (distance < 0) {
        if (framelist->origin - index < MAX_MISORDER) {
            /* too late */
            TRACE__((THIS_FILE, "Put frame #%d: too late (distance=%d)", index,
                     distance));
            return PJ_ETOOSMALL;
        } else if (framelist->origin + framelist->size >= MAX_FRAME_INDEX) {
            /* sequence restart */
            TRACE__((THIS_FILE,
                     "Put frame #%d: sequence restart (distance=%d, "
                     "orig=%d, size=%d)",
                     index, distance, framelist->origin, framelist->size));
            framelist->origin = index - framelist->size;
            distance = framelist->size;
        } else {
            /* jump too far, reset the buffer */
            TRACE__((THIS_FILE, "Put frame #%d: far jump (distance=%d)", index,
                     distance));
    ...
    if (distance >= (int)framelist->max_count) {
        if (distance > MAX_DROPOUT) {
            ...
            TRACE__(
              (THIS_FILE, "Put frame #%d: rejected due to JB full", index));
            return PJ_ETOOMANY;
        }
    }
    ...
    if (PJMEDIA_JB_NORMAL_FRAME == frame_type) {
        /* copy frame content */
        pj_memcpy(framelist->content + pos * framelist->frame_size, frame,
                  frame_size);
    }
...
static pj_status_t jb_framelist_discard(jb_framelist_t* framelist, int index)
{
    unsigned pos;
    ...
    /* Get the slot position */
    pos =
      (framelist->head + (index - framelist->origin)) % framelist->max_count;
...
PJ_DEF(pj_status_t)
pjmedia_jbuf_create(pj_pool_t* pool, const pj_str_t* name, unsigned frame_size,
                    unsigned ptime, unsigned max_count, pjmedia_jbuf** p_jb)
{
    pjmedia_jbuf* jb;
    pj_status_t status;
...
PJ_DEF(pj_status_t) pjmedia_jbuf_set_ptime(pjmedia_jbuf* jb, unsigned ptime)
{
    ...
    jb->jb_min_shrink_gap = PJMEDIA_JBUF_DISC_MIN_GAP / ptime;
    jb->jb_max_burst =
      (int)PJ_MAX(MAX_BURST_MSEC / ptime, jb->jb_max_count * 3 / 4);
...
PJ_DEF(pj_status_t) pjmedia_jbuf_set_fixed(pjmedia_jbuf* jb, unsigned prefetch)
{
    ...
    jb->jb_min_prefetch = jb->jb_max_prefetch = jb->jb_prefetch =
      jb->jb_init_prefetch = prefetch;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_jbuf_set_adaptive(pjmedia_jbuf* jb, unsigned prefetch,
                          unsigned min_prefetch, unsigned max_prefetch)
{
    PJ_ASSERT_RETURN(jb, PJ_EINVAL);
    PJ_ASSERT_RETURN(min_prefetch <= max_prefetch && prefetch <= max_prefetch &&
...
PJ_DEF(pj_status_t)
pjmedia_jbuf_set_discard(pjmedia_jbuf* jb, pjmedia_jb_discard_algo algo)
{
    PJ_ASSERT_RETURN(jb, PJ_EINVAL);
    PJ_ASSERT_RETURN(
      algo >= PJMEDIA_JB_DISCARD_NONE && algo <= PJMEDIA_JB_DISCARD_PROGRESSIVE,
      PJ_EINVAL);
...
PJ_DEF(pj_status_t) pjmedia_jbuf_destroy(pjmedia_jbuf* jb)
{
    PJ_LOG(5, (jb->jb_name.ptr,
               ...
               jb->jb_eff_level, jb->jb_delay.min, jb->jb_delay.max,
               jb->jb_delay.mean, pj_math_stat_get_stddev(&jb->jb_delay),
               jb->jb_burst.min, jb->jb_burst.max, jb->jb_burst.mean,
               pj_math_stat_get_stddev(&jb->jb_burst), jb->jb_lost,
               jb->jb_discard, jb->jb_empty));
...
static void jbuf_calculate_jitter(pjmedia_jbuf* jb)
{
    ...
    else if (jb->jb_level > jb->jb_eff_level)
    {
        ...
        jb->jb_eff_level =
          PJ_MIN(jb->jb_max_hist_level, (int)(jb->jb_max_count * 4 / 5));
...
static void jbuf_discard_static(pjmedia_jbuf* jb)
{
    ...
    if (diff >= STA_DISC_SAFE_SHRINKING_DIFF) {
        ...
        if (seq_origin - jb->jb_discard_ref >= jb->jb_min_shrink_gap) {
            ...
            TRACE__((jb->jb_name.ptr, "JB shrinking %d frame(s), cur size=%d",
                     diff, jb_framelist_eff_size(&jb->jb_framelist)));
        }
    }
}
...
static void jbuf_discard_progressive(pjmedia_jbuf* jb)
{
    ...
    if (last_seq >= (jb->jb_discard_ref + (int)jb->jb_discard_dist)) {
        ...
        TRACE__((jb->jb_name.ptr,
                 ...
                 "burst=%d/%d",
                 discard_seq, jb->jb_discard_ref, jb->jb_discard_dist,
                 jb_framelist_origin(&jb->jb_framelist), cur_size,
                 jb_framelist_size(&jb->jb_framelist), jb->jb_eff_level,
                 burst_level));
...
PJ_DEF(void)
pjmedia_jbuf_put_frame(pjmedia_jbuf* jb, const void* frame,
                       pj_size_t frame_size, int frame_seq)
{
    pjmedia_jbuf_put_frame3(jb, frame, frame_size, 0, frame_seq, 0, NULL);
}

PJ_DEF(void)
pjmedia_jbuf_put_frame2(pjmedia_jbuf* jb, const void* frame,
                        pj_size_t frame_size, pj_uint32_t bit_info,
                        int frame_seq, pj_bool_t* discarded)
{
...
PJ_DEF(void)
pjmedia_jbuf_put_frame3(pjmedia_jbuf* jb, const void* frame,
                        pj_size_t frame_size, pj_uint32_t bit_info,
                        int frame_seq, pj_uint32_t ts, pj_bool_t* discarded)
{
    ...
    if (frame_size > jb->jb_frame_size) {
        PJ_LOG(4, (THIS_FILE,
                   "Warning: frame too large for jitter buffer, "
    ...
    if (status == PJ_SUCCESS) {
        if (jb->jb_prefetching) {
            TRACE__((jb->jb_name.ptr, "PUT prefetch_cnt=%d/%d", new_size,
                     jb->jb_prefetch));
 ...
 */
PJ_DEF(void)
pjmedia_jbuf_get_frame(pjmedia_jbuf* jb, void* frame, char* p_frame_type)
{
 ...
 */
PJ_DEF(void)
pjmedia_jbuf_get_frame2(pjmedia_jbuf* jb, void* frame, pj_size_t* size,
                        char* p_frame_type, pj_uint32_t* bit_info)
{
 ...
 */
PJ_DEF(void)
pjmedia_jbuf_get_frame3(pjmedia_jbuf* jb, void* frame, pj_size_t* size,
                        char* p_frame_type, pj_uint32_t* bit_info,
                        pj_uint32_t* ts, int* seq)
{
    if (jb->jb_prefetching) {
        ...
        *p_frame_type = PJMEDIA_JB_ZERO_PREFETCH_FRAME;
        ...
        res = jb_framelist_get(&jb->jb_framelist, frame, size, &ftype, bit_info,
                               ts, seq);
        if (res) {
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_jbuf_get_state(const pjmedia_jbuf* jb, pjmedia_jb_state* state)
{
...
PJ_DEF(void)
pjmedia_jbuf_peek_frame(pjmedia_jbuf* jb, unsigned offset, const void** frame,
                        pj_size_t* size, char* p_frm_type,
                        pj_uint32_t* bit_info, pj_uint32_t* ts, int* seq)
{
