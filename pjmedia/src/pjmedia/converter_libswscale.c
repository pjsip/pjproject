...
static pjmedia_converter_op liswscale_converter_op = {
    &libswscale_conv_convert, &libswscale_conv_destroy,
                                           ...
                                           pjmedia_frame* dst_frame)
{
    ...
    struct fmt_info *src = &fcv->src, *dst = &fcv->dst;
    int h;
    ...
    h = sws_scale(fcv->sws_ctx, (const uint8_t* const*)src->apply_param.planes,
                  src->apply_param.strides, 0, src->apply_param.size.h,
  ...
  const pjmedia_coord* dst_pos, pjmedia_converter_convert_setting* param)
{
    ...
    for (j = 0; j < src->fmt_info->plane_cnt; ++j) {
        ...
        int y = src_pos->y * ap->plane_bytes[j] / ap->strides[j] / ap->size.h;
        ap->planes[j] +=
    ...
    for (j = 0; j < dst->fmt_info->plane_cnt; ++j) {
        ...
        int y = dst_pos->y * ap->plane_bytes[j] / ap->strides[j] / ap->size.h;
        ap->planes[j] +=
