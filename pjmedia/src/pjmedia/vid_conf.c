 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_conf_create(pj_pool_t* parent_pool,
                        ...
                        pjmedia_vid_conf** p_vid_conf)
{
    ...
    /* Allocate ports */
    vid_conf->ports = (vconf_port**)pj_pool_zalloc(
      pool, vid_conf->opt.max_slot_cnt * sizeof(vconf_port*));
    if (!vid_conf->ports) {
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_conf_add_port(pjmedia_vid_conf* vid_conf, pj_pool_t* parent_pool,
                          pjmedia_port* port, const pj_str_t* name, void* opt,
                          unsigned* p_slot)
{
    pj_pool_t* pool = NULL;
    ...
    PJ_ASSERT_RETURN(
      port->info.fmt.type == PJMEDIA_TYPE_VIDEO &&
    ...
    /* Init put/get_frame() intervals */
    {
        ...
        pj_uint32_t vconf_interval =
          (pj_uint32_t)(TS_CLOCK_RATE * 1.0 / vid_conf->opt.frame_rate);
        cport->ts_interval =
          (pj_uint32_t)(TS_CLOCK_RATE * 1.0 / fps->num * fps->denum);

        /* Normalize the interval */
        if (cport->ts_interval < vconf_interval) {
            cport->ts_interval = vconf_interval;
            PJ_LOG(3, (THIS_FILE,
                       "Warning: frame rate of port %s is higher "
    ...
    /* Allocate buffer for put/get_frame() */
    {
        ...
        if (port->put_frame) {
            ...
            status = pjmedia_video_format_fill_black(
              &port->info.fmt, cport->put_buf, cport->put_buf_size);
            if (status != PJ_SUCCESS) {
        ...
        if (port->get_frame) {
            ...
            status = pjmedia_video_format_fill_black(
              &port->info.fmt, cport->get_buf, cport->get_buf_size);
            if (status != PJ_SUCCESS) {
                PJ_PERROR(4, (THIS_FILE, status,
    ...
    cport->listener_slots = (unsigned*)pj_pool_zalloc(
      pool, vid_conf->opt.max_slot_cnt * sizeof(unsigned));
    ...
    cport->transmitter_slots = (unsigned*)pj_pool_zalloc(
      pool, vid_conf->opt.max_slot_cnt * sizeof(unsigned));
    ...
    cport->render_states = (render_state**)pj_pool_zalloc(
      pool, vid_conf->opt.max_slot_cnt * sizeof(render_state*));
    ...
    cport->render_pool = (pj_pool_t**)pj_pool_zalloc(
      pool, vid_conf->opt.max_slot_cnt * sizeof(pj_pool_t*));
    ...
    PJ_LOG(4, (THIS_FILE, "Added port %d (%.*s)", index, (int)cport->name.slen,
               cport->name.ptr));
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_conf_remove_port(pjmedia_vid_conf* vid_conf, unsigned slot)
{
    vconf_port* cport;
    op_entry* ope;
...
static void op_remove_port(pjmedia_vid_conf* vid_conf, const op_param* prm)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Removed port %d (%.*s)", slot, (int)cport->name.slen,
               cport->name.ptr));
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_conf_enum_ports(pjmedia_vid_conf* vid_conf, unsigned slots[],
                            unsigned* count)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_conf_get_port_info(pjmedia_vid_conf* vid_conf, unsigned slot,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_conf_connect_port(pjmedia_vid_conf* vid_conf, unsigned src_slot,
                              unsigned sink_slot, void* opt)
{
    vconf_port *src_port, *dst_port;
    unsigned i;

    /* Check arguments */
    PJ_ASSERT_RETURN(vid_conf && src_slot < vid_conf->opt.max_slot_cnt &&
    ...
    if (!src_port || !src_port->port->get_frame || !dst_port ||
        !dst_port->port->put_frame)
    {
        PJ_LOG(4, (THIS_FILE,
                   "Failed connecting video ports, make sure that "
    ...
    if (i == src_port->listener_cnt) {
        ...
        PJ_LOG(5, (THIS_FILE, "Video connect ports %d->%d requested", src_slot,
                   sink_slot));
...
static void op_connect_ports(pjmedia_vid_conf* vid_conf, const op_param* prm)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_conf_disconnect_port(pjmedia_vid_conf* vid_conf, unsigned src_slot,
                                 unsigned sink_slot)
{
    vconf_port *src_port, *dst_port;
    unsigned i, j;
...
static void op_disconnect_ports(pjmedia_vid_conf* vid_conf, const op_param* prm)
{
    unsigned src_slot, sink_slot;
    vconf_port *src_port, *dst_port;
    unsigned i, j, k;
    ...
    PJ_LOG(4, (THIS_FILE, "Port %d (%.*s) stop transmitting to port %d (%.*s)",
               src_slot, (int)src_port->name.slen, src_port->name.ptr,
               sink_slot, (int)dst_port->name.slen, dst_port->name.ptr));
}
...
static void on_clock_tick(const pj_timestamp* now, void* user_data)
{
    ...
    for (i = 0, ci = 0;
         i < vid_conf->opt.max_slot_cnt && ci < vid_conf->port_cnt; ++i)
    {
        unsigned j;
            ...
            if (src->got_frame) {
                ...
                if (status == PJ_SUCCESS) {
                    ...
                    PJ_PERROR(5, (THIS_FILE, status,
                                  ...
                                  "to port %d [%s]",
                                  src->idx, src->port->info.name.ptr, sink->idx,
...
static void update_render_state(pjmedia_vid_conf* vid_conf, vconf_port* cp)
{
    ...
    {
        TRACE_((THIS_FILE,
                "This port only has single source with "
    ...
    for (i = 0; i < cp->transmitter_cnt && i < 4; ++i) {
        ...
        TRACE_((THIS_FILE, "src#%d=%s/%dx%d->%dx%d@%d,%d dst=%dx%d@%d,%d", i,
                pjmedia_fourcc_name(tr_fmt_id[i], tmp_buf), tr_size[i].w,
                tr_size[i].h, rs->src_rect.size.w, rs->src_rect.size.h,
                rs->src_rect.coord.x, rs->src_rect.coord.y, rs->dst_rect.size.w,
                rs->dst_rect.size.h, rs->dst_rect.coord.x,
                rs->dst_rect.coord.y));

        /* Create converter */
        pjmedia_format_init_video(&cparam.src, rs->src_fmt_id,
                                  rs->src_rect.size.w, rs->src_rect.size.h, 0,
                                  1);
        pjmedia_format_init_video(&cparam.dst, rs->dst_fmt_id,
                                  rs->dst_rect.size.w, rs->dst_rect.size.h, 0,
                                  1);
                                    ...
                                    unsigned transmitter_idx)
{
     ...
     * changed, but we haven't received the event yet.
     */
    if (pj_memcmp(&sink->format, &sink->port->info.fmt, sizeof(pjmedia_format)))
    {
    ...
    if (sink->transmitter_cnt == 1 && (!rs || !rs->converter)) {
        ...
        status = pjmedia_converter_convert2(
          rs->converter, &src_frame, &rs->src_frame_size, &rs->src_rect.coord,
          &dst_frame, &rs->dst_frame_size, &rs->dst_rect.coord, NULL);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4, (THIS_FILE, status,
...
PJ_DEF(pj_status_t)
pjmedia_vid_conf_update_port(pjmedia_vid_conf* vid_conf, unsigned slot)
{
    vconf_port* cport;
    op_entry* ope;
...
static void op_update_port(pjmedia_vid_conf* vid_conf, const op_param* prm)
{
    ...
    if (cmp_fps(new_fmt, old_fmt)) {
        ...
        PJ_LOG(4, (THIS_FILE, "Port %d (%s): updated frame rate %d -> %d", slot,
                   cport->name.ptr,
    ...
    if (cmp_size(new_fmt, old_fmt)) {
        ...
        if (cport->port->put_frame) {
            ...
            /* Initialize sink buffer with black color. */
            status = pjmedia_video_format_fill_black(
              &cport->port->info.fmt, cport->put_buf, cport->put_buf_size);
            if (status != PJ_SUCCESS) {
                PJ_PERROR(4, (THIS_FILE, status,
                              "Warning: failed to init sink buffer "
                              " with black"));
        ...
        PJ_LOG(4, (THIS_FILE, "Port %d (%s): updated frame size %dx%d -> %dx%d",
                   slot, cport->name.ptr, old_fmt->det.vid.size.w,
                   old_fmt->det.vid.size.h, new_fmt->det.vid.size.w,
