...
static const char* state_str[] = {
    "STATE_NULL",      "STATE_LOCAL_OFFER", "STATE_REMOTE_OFFER",
    "STATE_WAIT_NEGO", "STATE_DONE",
};
...
static pj_status_t custom_fmt_match(pj_pool_t* pool, const pj_str_t* fmt_name,
...
{
    if ((int)state >= 0 &&
        state < (pjmedia_sdp_neg_state)PJ_ARRAY_SIZE(state_str))
                                     ...
                                     pjmedia_sdp_neg** p_neg)
{
    ...
    *p_neg = NULL;
    ...
    PJ_ASSERT_RETURN((status = pjmedia_sdp_validate(local)) == PJ_SUCCESS,
                     status);
...
static pjmedia_sdp_media* sdp_media_clone_deactivate(
  pj_pool_t* pool, const pjmedia_sdp_media* rem_med,
  const pjmedia_sdp_media* local_med, const pjmedia_sdp_session* local_sess)
{
...
#if PJMEDIA_SDP_NEG_COMPARE_BEFORE_INC_VERSION
        if (pjmedia_sdp_session_cmp(neg->neg_local_sdp, neg->initial_sdp, 0) !=
            PJ_SUCCESS)
        {
...
{
    pjmedia_dir old_dir = PJMEDIA_DIR_ENCODING_DECODING, new_dir;
 ...
 * from remote.
 */
static pj_status_t process_m_answer(pj_pool_t* pool, pjmedia_sdp_media* offer,
                                    pjmedia_sdp_media* answer,
                                    pj_bool_t allow_asym)
{
    ...
    if (pjmedia_sdp_transport_cmp(&answer->desc.transport,
                                  &offer->desc.transport) != PJ_SUCCESS)
    {
                ...
                for (j = 0; j < answer->desc.fmt_count; ++j) {
                    ...
                    if (a) {
                             ...
                             (ar.param.slen == 1 && *ar.param.ptr == '1')))
                        {
                            ...
                            if (custom_fmt_match(pool, &or_.enc_name, offer, i,
                                                 answer, j, 0) == PJ_SUCCESS) {
                                /* Match! */
                                break;
                            }
                        }
 ...
 */
PJ_INLINE(void)
rewrite_pt(pj_pool_t* pool, pj_str_t* attr_val, const pj_str_t* old_pt,
           const pj_str_t* new_pt)
{
    ...
    if (len_diff > 0) {
        ...
        *attr_val = new_val;
        ...
        pj_memmove(attr_val->ptr, attr_val->ptr - len_diff, attr_val->slen + 1);
    }
...
{
    unsigned i;
    pj_bool_t master_has_codec = 0, master_has_other = 0,
              found_matching_codec = 0, found_matching_telephone_event = 0,
    ...
    if (offer->desc.port == 0) {
        answer =
          sdp_media_clone_deactivate(pool, offer, preanswer, preanswer_sdp);
        *p_answer = answer;
        return PJ_SUCCESS;
    }
                    ...
                    if (a) {
                              ...
                              *lr.param.ptr == '1')))
                        {
                            ...
                            if (is_codec) {
                                ...
                                /* Call custom format matching callbacks */
                                if (custom_fmt_match(
                                      pool, &or_.enc_name, o_med, o_fmt_idx,
                                      a_med, a_fmt_idx,
                                      ALLOW_MODIFY_ANSWER) != PJ_SUCCESS)
                                {
                                    continue;
                                }
                                found_matching_codec = 1;
                                ...
                                tel_clockrate[ntel_clockrate++] =
                                  or_.clock_rate;
                                found_matching_telephone_event = 1;
                            }

                            pt_offer[pt_answer_count] =
                              prefer_remote_codec_order ? offer->desc.fmt[i] :
                                                          offer->desc.fmt[j];
    ...
    */
    ...
    if (found_matching_telephone_event) {
        ...
        for (i = 0; i < pt_answer_count;) {
            ...
            a =
              pjmedia_sdp_media_find_attr2(preanswer, "rtpmap", &pt_answer[i]);
            pj_assert(a);
            ...
            if (j == nclockrate) {
                pj_array_erase(pt_answer, sizeof(pt_answer[0]), pt_answer_count,
                               i);
                pj_array_erase(pt_offer, sizeof(pt_offer[0]), pt_answer_count,
                               i);
                pt_answer_count--;
            } else {
                ++matched_cnt;
    ...
    for (i = pt_answer_count; i < answer->desc.fmt_count; ++i) {
        ...
        a =
          pjmedia_sdp_media_find_attr2(answer, "rtpmap", &answer->desc.fmt[i]);
        if (a) {
            pjmedia_sdp_media_remove_attr(answer, a);
        }

        /* Remove fmtp for this format */
        a = pjmedia_sdp_media_find_attr2(answer, "fmtp", &answer->desc.fmt[i]);
        if (a) {
            pjmedia_sdp_media_remove_attr(answer, a);
        }
    }
    ...
    apply_answer_symmetric_pt(pool, answer, pt_answer_count, pt_offer,
                              pt_answer);
#endif
                                 ...
                                 pjmedia_sdp_session** p_answer)
{
     ...
     */
    PJ_ASSERT_RETURN((status = pjmedia_sdp_validate(offer)) == PJ_SUCCESS,
                     status);
        ...
        for (im = NULL, j = 0; j < initial->media_count; ++j) {
                ...
                media_used[j] == 0)
            {
                ...
                status2 = match_offer(pool, prefer_remote_codec_order,
                                      answer_with_multiple_codecs, om, im,
                                      initial, &am);
                          ...
                          pj_bool_t allow_asym)
{
    ...
    if (neg->has_remote_answer) {
        ...
        status =
          create_answer(pool, neg->prefer_remote_codec_order,
                        neg->answer_with_multiple_codecs, neg->neg_local_sdp,
                        neg->neg_remote_sdp, &answer);
        if (status == PJ_SUCCESS) {
                                    ...
                                    unsigned a_fmt_idx, unsigned option)
{
    ...
    for (i = 0; i < fmt_match_cb_cnt; ++i) {
        if (pj_stricmp(fmt_name, &fmt_match_cb[i].fmt_name) == 0) {
            ...
            return (*fmt_match_cb[i].cb)(pool, offer, o_fmt_idx, answer,
                                         a_fmt_idx, option);
        }
    }
                                      ...
                                      pjmedia_sdp_neg_fmt_match_cb cb)
{
    ...
    if (cb == NULL) {
        ...
        pj_array_erase(fmt_match_cb, sizeof(fmt_match_cb[0]), fmt_match_cb_cnt,
                       i);
        fmt_match_cb_cnt--;
                          ...
                          unsigned a_fmt_idx, unsigned option)
{
    ...
    return custom_fmt_match(pool, &o_rtpmap.enc_name, offer, o_fmt_idx, answer,
                            a_fmt_idx, option);
}
