...
static pj_status_t g711_parse(pjmedia_codec* codec, void* pkt,
                              pj_size_t pkt_size, const pj_timestamp* timestamp,
...
static pj_status_t g711_decode(pjmedia_codec* codec,
                               ...
                               struct pjmedia_frame* output);
#    if !PLC_DISABLED
static pj_status_t g711_recover(pjmedia_codec* codec, unsigned output_buf_len,
                                struct pjmedia_frame* output);
#    endif

/* Definition for G711 codec operations. */
static pjmedia_codec_op g711_op = { &g711_init,   &g711_open,  &g711_close,
                                    &g711_modify, &g711_parse, &g711_encode,
                                    &g711_decode,
#    if !PLC_DISABLED
                                    &g711_recover
#    else
...
static pjmedia_codec_factory_op g711_factory_op = {
    &g711_test_alloc,  &g711_default_attr,  &g711_enum_codecs,
    &g711_alloc_codec, &g711_dealloc_codec, &pjmedia_codec_g711_deinit
};
...
PJ_DEF(pj_status_t) pjmedia_codec_g711_init(pjmedia_endpt* endpt)
{
    ...
    /* Create mutex. */
    status =
      pj_mutex_create_simple(g711_factory.pool, "g611", &g711_factory.mutex);
    ...
    status = pjmedia_codec_mgr_register_factory(codec_mgr, &g711_factory.base);
    if (status != PJ_SUCCESS)
        return status;
...
PJ_DEF(pj_status_t) pjmedia_codec_g711_deinit(void)
{
    ...
    status =
      pjmedia_codec_mgr_unregister_factory(codec_mgr, &g711_factory.base);
    g711_factory.endpt = NULL;
                                   ...
                                   const pjmedia_codec_info* id)
{
    ...
    return (id->pt == PJMEDIA_RTP_PT_PCMU || id->pt == PJMEDIA_RTP_PT_PCMA) ?
             0 :
             -1;
}
...
#    endif
        ...
        status = pjmedia_silence_det_create(
          g711_factory.pool, 8000, SAMPLES_PER_FRAME, &codec_priv->vad);
        if (status != PJ_SUCCESS) {
            pj_mutex_unlock(g711_factory.mutex);
            return status;
        }
                               ...
                               struct pjmedia_frame* output)
{
    ...
    if (priv->vad_enabled) {
        ...
        silence_period = pj_timestamp_diff32(&priv->last_tx, &input->timestamp);

        is_silence = pjmedia_silence_det_detect(
          priv->vad, (const pj_int16_t*)input->buf, (input->size >> 1), NULL);
        if (is_silence &&
