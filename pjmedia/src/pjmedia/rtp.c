...
PJ_DEF(pj_status_t)
pjmedia_rtp_session_init(pjmedia_rtp_session* ses, int default_pt,
...
PJ_DEF(pj_status_t)
pjmedia_rtp_session_init2(pjmedia_rtp_session* ses,
...
PJ_DEF(pj_status_t)
pjmedia_rtp_encode_rtp(pjmedia_rtp_session* ses, int pt, int m, int payload_len,
                       int ts_len, const void** rtphdr, int* hdrlen)
{
    /* Update timestamp */
...
PJ_DEF(pj_status_t)
pjmedia_rtp_decode_rtp(pjmedia_rtp_session* ses, const void* pkt, int pkt_len,
                       const pjmedia_rtp_hdr** hdr, const void** payload,
                       unsigned* payloadlen)
{
    pjmedia_rtp_dec_hdr dec_hdr;

    return pjmedia_rtp_decode_rtp2(ses, pkt, pkt_len, hdr, &dec_hdr, payload,
                                   payloadlen);
}

PJ_DEF(pj_status_t)
pjmedia_rtp_decode_rtp2(pjmedia_rtp_session* ses, const void* pkt, int pkt_len,
                        const pjmedia_rtp_hdr** hdr,
                        pjmedia_rtp_dec_hdr* dec_hdr, const void** payload,
                        unsigned* payloadlen)
{
    int offset;
...
PJ_DEF(void)
pjmedia_rtp_session_update(pjmedia_rtp_session* ses, const pjmedia_rtp_hdr* hdr,
...
PJ_DEF(void)
pjmedia_rtp_session_update2(pjmedia_rtp_session* ses,
                            const pjmedia_rtp_hdr* hdr,
                            pjmedia_rtp_status* p_seq_st, pj_bool_t check_pt)
{
...
void pjmedia_rtp_seq_update(pjmedia_rtp_seq_session* sess, pj_uint16_t seq,
