...
static struct cap_info
{
    const char* name;
    const char* info;
} cap_infos[] = { DEFINE_CAP("ext-fmt", "Extended/non-PCM format"),
                  DEFINE_CAP("latency-in", "Input latency/buffer size setting"),
                  DEFINE_CAP("latency-out",
                             "Output latency/buffer size setting"),
...
/* API: init driver */
PJ_DEF(pj_status_t) pjmedia_aud_driver_init(unsigned drv_idx, pj_bool_t refresh)
{
    ...
    if (dev_cnt + aud_subsys.dev_cnt > PJMEDIA_AUD_MAX_DEVS) {
        PJ_LOG(4, (THIS_FILE,
                   ...
                   aud_subsys.dev_cnt + dev_cnt - PJMEDIA_AUD_MAX_DEVS));
    ...
    */
    ...
    drv->play_dev_idx = drv->rec_dev_idx = drv->dev_idx =
    ...
    for (i = 0; i < dev_cnt; ++i) {
        ...
        if (drv->dev_idx < 0 && info.input_count && info.output_count) {
...
PJ_DEF(const char*)
pjmedia_aud_dev_cap_name(pjmedia_aud_dev_cap cap, const char** p_desc)
{
    const char* desc;
    unsigned i;
...
static pj_status_t get_cap_pointer(const pjmedia_aud_param* param,
                                   pjmedia_aud_dev_cap cap, void** ptr,
                                   unsigned* size)
{
...
PJ_DEF(pj_status_t)
pjmedia_aud_param_set_cap(pjmedia_aud_param* param, pjmedia_aud_dev_cap cap,
                          const void* pval)
{
    void* cap_ptr;
    unsigned cap_size;
...
PJ_DEF(pj_status_t)
pjmedia_aud_param_get_cap(const pjmedia_aud_param* param,
...
/* API: find device */
PJ_DEF(pj_status_t)
pjmedia_aud_dev_lookup(const char* drv_name, const char* dev_name,
                       pjmedia_aud_dev_index* id)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_aud_dev_default_param(pjmedia_aud_dev_index id,
...
PJ_DEF(pj_status_t)
pjmedia_aud_stream_create(const pjmedia_aud_param* prm,
                          ...
                          pjmedia_aud_stream** p_aud_strm)
{
    ...
    PJ_ASSERT_RETURN(
      (param.dir != PJMEDIA_DIR_CAPTURE_PLAYBACK) || (rec_f == play_f),
      PJMEDIA_EAUD_INVDEV);

    /* Create the stream */
    status =
      f->op->create_stream(f, &param, rec_cb, play_cb, user_data, p_aud_strm);
    if (status != PJ_SUCCESS)
        return status;
...
PJ_DEF(pj_status_t)
pjmedia_aud_stream_get_cap(pjmedia_aud_stream* strm, pjmedia_aud_dev_cap cap,
                           void* value)
{
...
PJ_DEF(pj_status_t)
pjmedia_aud_stream_set_cap(pjmedia_aud_stream* strm, pjmedia_aud_dev_cap cap,
                           const void* value)
{
