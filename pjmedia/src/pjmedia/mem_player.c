...
static pj_status_t mem_put_frame(pjmedia_port* this_port, pjmedia_frame* frame);
static pj_status_t mem_get_frame(pjmedia_port* this_port, pjmedia_frame* frame);
static pj_status_t mem_on_destroy(pjmedia_port* this_port);

PJ_DEF(pj_status_t)
pjmedia_mem_player_create(pj_pool_t* pool, const void* buffer, pj_size_t size,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_mem_player_set_eof_cb(pjmedia_port* port, void* user_data,
                                                ...
                                                void* usr_data))
{
    ...
    PJ_LOG(1, (THIS_FILE,
               "pjmedia_mem_player_set_eof_cb() is deprecated. "
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_mem_player_set_eof_cb2(pjmedia_port* port, void* user_data,
...
static pj_status_t player_on_event(pjmedia_event* event, void* user_data)
{
...
static pj_status_t mem_get_frame(pjmedia_port* this_port, pjmedia_frame* frame)
{
        ...
        if (player->cb2) {
            if (!player->subscribed) {
                ...
                status2 = pjmedia_event_subscribe(NULL, &player_on_event,
                                                  player, player);
                player->subscribed =
                  (status2 == PJ_SUCCESS) ? PJ_TRUE : PJ_FALSE;
            }
    ...
    if (delayed_cb) {
        pjmedia_event event;
        pjmedia_event_init(&event, PJMEDIA_EVENT_CALLBACK, NULL, player);
        pjmedia_event_publish(NULL, player, &event,
                              PJMEDIA_EVENT_PUBLISH_POST_EVENT);
        /* Should not access player port after this since
         * it might have been destroyed by the callback.
