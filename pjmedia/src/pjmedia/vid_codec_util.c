...
struct mpi_resolution_t
{
    ...
    { { "CIF", 3 }, { 352, 288 } },     { { "QCIF", 4 }, { 176, 144 } },
    { { "SQCIF", 5 }, { 88, 72 } },     { { "CIF4", 4 }, { 704, 576 } },
    { { "CIF16", 5 }, { 1408, 1142 } },
};

#    define CALC_H264_MB_NUM(size) (((size.w + 15) / 16) * ((size.h + 15) / 16))
#    define CALC_H264_MBPS(size, fps) \
        CALC_H264_MB_NUM(size) * fps.num / fps.denum
...
static pj_status_t parse_custom_res_fmtp(const pj_str_t* fmtp_val,
                                         pjmedia_rect_size* size, unsigned* mpi)
{
    const char *p, *p_end;
    pj_str_t token;
...
/* H263 fmtp parser */
PJ_DEF(pj_status_t)
pjmedia_vid_codec_parse_h263_fmtp(const pjmedia_codec_fmtp* fmtp,
...
pjmedia_vid_codec_h263_apply_fmtp(pjmedia_vid_codec_param* param)
{
    if (param->dir & PJMEDIA_DIR_ENCODING) {
        ...
        vfd = pjmedia_format_get_video_format_detail(&param->enc_fmt, PJ_TRUE);
        ...
        /* Get remote param */
        status = pjmedia_vid_codec_parse_h263_fmtp(&param->enc_fmtp, &fmtp_rem);
        if (status != PJ_SUCCESS)
            return status;
            ...
            for (i = 0; i < fmtp_rem.mpi_cnt && !matched; ++i) {
                ...
                for (j = 0; j < fmtp_loc.mpi_cnt; ++j) {
                        ...
                        fmtp_rem.mpi[i].size.h == fmtp_loc.mpi[j].size.h)
                    {
                        ...
                        mpi = PJ_MAX(fmtp_rem.mpi[i].val, fmtp_loc.mpi[j].val);
                        matched = PJ_TRUE;
                        break;
                    }
                    ...
                    diff =
                      loc_sq > rem_sq ? (loc_sq - rem_sq) : (rem_sq - loc_sq);
                    if (diff < min_diff) {
    ...
    if (param->dir & PJMEDIA_DIR_DECODING) {
        ...
        status = pjmedia_vid_codec_parse_h263_fmtp(&param->dec_fmtp, &fmtp);
        if (status != PJ_SUCCESS)
            return status;

        vfd = pjmedia_format_get_video_format_detail(&param->dec_fmt, PJ_TRUE);
                                     ...
                                     pjmedia_vid_codec_h264_fmtp* fmtp)
{
    const h264_level_info_t level_info[] = {
        { 10, 1485, 99, 64 },          { 9, 1485, 99, 128 }, /*< level 1b */
        { 11, 3000, 396, 192 },        { 12, 6000, 396, 384 },
        { 13, 11880, 396, 768 },       { 20, 11880, 396, 2000 },
        { 21, 19800, 792, 4000 },      { 22, 20250, 1620, 4000 },
        { 30, 40500, 1620, 10000 },    { 31, 108000, 3600, 14000 },
        { 32, 216000, 5120, 20000 },   { 40, 245760, 8192, 20000 },
        { 41, 245760, 8192, 50000 },   { 42, 522240, 8704, 50000 },
        { 50, 589824, 22080, 135000 }, { 51, 983040, 36864, 240000 },
    };
    unsigned i, tmp;
    pj_str_t endst;
    ...
    for (i = 0; i < fmtp->cnt; ++i) {
        ...
        } else if (pj_stricmp(&fmtp->param[i].name, &SPROP_PARAMETER_SETS) == 0)
        {
            ...
            while (sps_st.slen) {
                ...
                nal =
                  &h264_fmtp->sprop_param_sets[h264_fmtp->sprop_param_sets_len];
                tmp_len = PJ_ARRAY_SIZE(h264_fmtp->sprop_param_sets) -
                          (int)h264_fmtp->sprop_param_sets_len -
                          PJ_ARRAY_SIZE(start_code);
                status = pj_base64_decode(
                  &tmp_st, nal + PJ_ARRAY_SIZE(start_code), &tmp_len);
                if (status != PJ_SUCCESS)
                    return PJMEDIA_SDP_EINFMTP;
                                 ...
                                 unsigned a_fmt_idx, unsigned option)
{
    ...
    status = pjmedia_stream_info_parse_fmtp(
      NULL, offer, pj_strtoul(&offer->desc.fmt[o_fmt_idx]), &o_fmtp_raw);
    if (status != PJ_SUCCESS)
        return status;
    ...
    /* Parse answer */
    status = pjmedia_stream_info_parse_fmtp(
      NULL, answer, pj_strtoul(&answer->desc.fmt[a_fmt_idx]), &a_fmtp_raw);
    if (status != PJ_SUCCESS)
        return status;
...
#    endif
        ...
        for (i = 0; i < a_fmtp_raw.cnt; ++i) {
            ...
            } else if (pj_stricmp(&a_fmtp_raw.param[i].name,
                                    ...
                                    pj_bool_t is_decoding)
{
    ...
    pjmedia_ratio def_fps = { DEFAULT_H264_FPS_NUM, DEFAULT_H264_FPS_DENUM };
...
pjmedia_vid_codec_h264_apply_fmtp(pjmedia_vid_codec_param* param)
{
    if (param->dir & PJMEDIA_DIR_ENCODING) {
        ...
        /* Get remote param */
        status = pjmedia_vid_codec_h264_parse_fmtp(&param->enc_fmtp, &fmtp);
        if (status != PJ_SUCCESS)
            return status;
    ...
    if (param->dir & PJMEDIA_DIR_DECODING) {
        ...
        status = pjmedia_vid_codec_h264_parse_fmtp(&param->dec_fmtp, &fmtp);
        if (status != PJ_SUCCESS)
            return status;
                                 ...
                                 pjmedia_vid_codec_vpx_fmtp* vpx_fmtp)
{
    ...
    for (i = 0; i < fmtp->cnt; ++i) {
        ...
        if (pj_stricmp(&fmtp->param[i].name, &MAX_FS) == 0) {
            ...
            tmp = pj_strtoul(&fmtp->param[i].val);
            vpx_fmtp->profile_id =
              (pj_uint8_t)PJ_MAX(tmp, vpx_fmtp->profile_id);
        }
    }
...
pjmedia_vid_codec_vpx_apply_fmtp(pjmedia_vid_codec_param* param)
{
    if (param->dir & PJMEDIA_DIR_ENCODING) {
        ...
        /* Get remote param */
        status = pjmedia_vid_codec_vpx_parse_fmtp(&param->enc_fmtp, &fmtp);
        if (status != PJ_SUCCESS)
            return status;
        ...
        if (fmtp.max_fs > 0) {
            ...
            if (vfd->size.w > max_res || vfd->size.h > max_res) {
                 ...
                 */
                unsigned larger =
                  (vfd->size.w > vfd->size.h) ? vfd->size.w : vfd->size.h;
    ...
    if (param->dir & PJMEDIA_DIR_DECODING) {
        ...
        /* Get remote param */
        status = pjmedia_vid_codec_vpx_parse_fmtp(&param->dec_fmtp, &fmtp);
        if (status != PJ_SUCCESS)
            return status;
