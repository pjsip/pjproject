 ...
 * Default settings
 */
#define DEF_RECALC_ON_VOICED \
    4000 /* Time to recalculate threshold \
            in voiced condition, in ms	  */
#define DEF_RECALC_ON_SILENCE \
    2000 /* Time to recalculate threshold \
            in silence condition, in ms.  */
#define DEF_BEFORE_SILENCE \
    400                    /* Silence time before really changing \
...
pjmedia_silence_det_apply(pjmedia_silence_det* sd, pj_uint32_t level)
{
    ...
    if (level > sd->threshold || level >= PJMEDIA_SILENCE_DET_MAX_THRESHOLD) {
        sd->silence_timer = 0;
        ...
        case STATE_VOICED:
            if (sd->voiced_timer > sd->recalc_on_voiced) {
                ...
                TRACE_((THIS_FILE,
                        "Re-adjust threshold (in talk burst)"
                        "to %d",
                        sd->threshold));
        ...
        case STATE_SILENCE:
            if (sd->silence_timer >= sd->recalc_on_silence) {
                ...
                TRACE_((THIS_FILE,
                        "Re-adjust threshold (in silence)"
                        "to %d",
                        sd->threshold));
