 ...
 */
PJ_DEF(pj_status_t)
pjmedia_port_info_init(pjmedia_port_info* info, const pj_str_t* name,
                       unsigned signature, unsigned clock_rate,
...
PJ_DEF(pj_status_t)
pjmedia_port_info_init2(pjmedia_port_info* info, const pj_str_t* name,
                        unsigned signature, pjmedia_dir dir,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_port_get_frame(pjmedia_port* port, pjmedia_frame* frame)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_port_put_frame(pjmedia_port* port, pjmedia_frame* frame)
{
    PJ_ASSERT_RETURN(port && frame, PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_port_init_grp_lock(pjmedia_port* port, pj_pool_t* pool,
                           pj_grp_lock_t* glock)
{
    ...
    if (port->on_destroy == NULL) {
        PJ_LOG(3, (THIS_FILE,
                   "Media port %s is using group lock but does not "
    ...
    if (!grp_lock) {
        ...
        status = pj_grp_lock_create_w_handler(pool, NULL, port,
                                              &port_on_destroy, &grp_lock);
    } else {
        /* Just add handler, and use internal group lock pool */
        status =
          pj_grp_lock_add_handler(grp_lock, NULL, port, &port_on_destroy);
    }
