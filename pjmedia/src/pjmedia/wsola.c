                              ...
                              unsigned template_cnt, int first)
{
    ...
    for (sr = beg; sr != end; ++sr) {
        ...
        for (i = 0; i < template_cnt - 8; i += 8) {
            corr -= (int)sr[i + 0] + (int)sr[i + 1] + (int)sr[i + 2] +
                    (int)sr[i + 3] + (int)sr[i + 4] + (int)sr[i + 5] +
                    (int)sr[i + 6] + (int)sr[i + 7];
        }
...
static void overlapp_add(pj_int16_t dst[], unsigned count, pj_int16_t l[],
                         pj_int16_t r[], float w[])
{
    unsigned i;
...
enum
{
    WINDOW_MAX_VAL = (1 << WINDOW_BITS) - 1
};
...
static void overlapp_add_simple(pj_int16_t dst[], unsigned count,
                                pj_int16_t l[], pj_int16_t r[])
{
    int step = ((WINDOW_MAX_VAL + 1) / count), stepdown = WINDOW_MAX_VAL;
    unsigned i;
...
static void create_win(pj_pool_t* pool, pj_uint16_t** pw, unsigned count)
{
    unsigned i;
    pj_uint16_t* w =
      (pj_uint16_t*)pj_pool_calloc(pool, count, sizeof(pj_uint16_t));
 ...
 */
static void fade_in(pj_int16_t buf[], int count, int fade_in_pos, int fade_cnt)
{
#    if defined(PJ_HAS_FLOATING_POINT) && PJ_HAS_FLOATING_POINT != 0
...
static void wsola_fade_out(pjmedia_wsola* wsola, pj_int16_t buf[], int count)
{
    pj_int16_t* end = buf + count;
...
PJ_DEF(pj_status_t)
pjmedia_wsola_create(pj_pool_t* pool, unsigned clock_rate,
                     unsigned samples_per_frame, unsigned channel_count,
                     unsigned options, pjmedia_wsola** p_wsola)
{
    pjmedia_wsola* wsola;
    pj_status_t status;
    ...
    wsola->templ_size =
      (pj_uint16_t)(TEMPLATE_PTIME * clock_rate * channel_count / 1000);
    if (wsola->templ_size > samples_per_frame)
        wsola->templ_size = wsola->samples_per_frame;

    /* Calculate hanning window size */
    wsola->hanning_size =
      (pj_uint16_t)(HANNING_PTIME * clock_rate * channel_count / 1000);
    ...
    /* Create merge buffer */
    wsola->merge_buf = (pj_int16_t*)pj_pool_calloc(pool, wsola->hanning_size,
                                                   sizeof(pj_int16_t));

    /* Setup with PLC */
    if ((options & PJMEDIA_WSOLA_NO_PLC) == 0) {
        wsola->min_extra = wsola->hanning_size;
        wsola->expand_sr_min_dist =
          (pj_uint16_t)(EXP_MIN_DIST * wsola->samples_per_frame);
        wsola->expand_sr_max_dist =
          (pj_uint16_t)(EXP_MAX_DIST * wsola->samples_per_frame);
    }
    ...
    if ((options & PJMEDIA_WSOLA_NO_DISCARD) == 0) {
        wsola->erase_buf = (pj_int16_t*)pj_pool_calloc(
          pool, samples_per_frame * ERASE_CNT, sizeof(pj_int16_t));
    }
...
PJ_DEF(pj_status_t)
pjmedia_wsola_set_max_expand(pjmedia_wsola* wsola, unsigned msec)
{
...
PJ_DEF(pj_status_t) pjmedia_wsola_reset(pjmedia_wsola* wsola, unsigned options)
{
...
static void expand(pjmedia_wsola* wsola, unsigned needed)
{
    ...
    pjmedia_circ_buf_get_read_regions(wsola->buf, &reg1, &reg1_len, &reg2,
                                      &reg2_len);
    CHECK_(reg2_len == 0);
    ...
    for (rep = 1;; ++rep) {
        ...
        start =
          find_pitch(templ, templ - wsola->expand_sr_max_dist,
                     templ - wsola->expand_sr_min_dist, wsola->templ_size, 1);
        ...
        if (wsola->options & PJMEDIA_WSOLA_NO_HANNING) {
            overlapp_add_simple(wsola->merge_buf, wsola->hanning_size, templ,
                                start);
        } else {
            ...
            overlapp_add(wsola->merge_buf, wsola->hanning_size, templ, start,
                         wsola->hanning);
        }
        ...
        if (generated >= needed) {
            TRACE_(
              (THIS_FILE, "WSOLA frame expanded after %d iterations", rep));
            break;
        }
    }
}
...
PJ_DEF(pj_status_t)
pjmedia_wsola_save(pjmedia_wsola* wsola, pj_int16_t frm[], pj_bool_t prev_lost)
{
    unsigned buf_len;
    pj_status_t status;
    ...
    if (prev_lost) {
        ...
        if ((wsola->options & PJMEDIA_WSOLA_NO_FADING) == 0) {
            ...
            fade_in_pos = wsola->fade_out_pos * count / wsola->max_expand_cnt;

            /* Fade-in it */
            fade_in(frm, wsola->samples_per_frame, fade_in_pos, count);
        }
...
PJ_DEF(pj_status_t)
pjmedia_wsola_generate(pjmedia_wsola* wsola, pj_int16_t frm[])
{
    unsigned samples_len, samples_req;
    pj_status_t status = PJ_SUCCESS;

    CHECK_(pjmedia_circ_buf_get_len(wsola->buf) >=
           wsola->hist_size + wsola->min_extra);
...
PJ_DEF(pj_status_t)
pjmedia_wsola_discard(pjmedia_wsola* wsola, pj_int16_t buf1[],
                      unsigned buf1_cnt, pj_int16_t buf2[], unsigned buf2_cnt,
                      unsigned* del_cnt)
{
        ...
        *del_cnt = compress(wsola, buf1, buf1_cnt, *del_cnt);
        ...
        if (buf2_cnt >= ERASE_CNT * wsola->samples_per_frame) {
            ...
            *del_cnt = compress(wsola, buf1, buf1_cnt, *del_cnt);
            ...
            if (max < buf2_cnt) {
                pjmedia_move_samples(buf2, buf2 + (*del_cnt), buf2_cnt - max);
            }
        } else {
