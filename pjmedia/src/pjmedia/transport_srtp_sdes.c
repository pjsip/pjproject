...
static pj_status_t sdes_media_create(pjmedia_transport* tp, pj_pool_t* sdp_pool,
                                     unsigned options,
                                     const pjmedia_sdp_session* sdp_remote,
                                     unsigned media_index);
static pj_status_t sdes_encode_sdp(pjmedia_transport* tp, pj_pool_t* sdp_pool,
                                   pjmedia_sdp_session* sdp_local,
                                   const pjmedia_sdp_session* sdp_remote,
                                   unsigned media_index);
static pj_status_t sdes_media_start(pjmedia_transport* tp, pj_pool_t* pool,
                                    const pjmedia_sdp_session* sdp_local,
                                    const pjmedia_sdp_session* sdp_remote,
                                    unsigned media_index);
static pj_status_t sdes_media_stop(pjmedia_transport* tp);

static pjmedia_transport_op sdes_op = { NULL,
                                        NULL,
                                        NULL,
                                        NULL,
                                        NULL,
 ...
 */
static pj_status_t generate_crypto_attr_value(pj_pool_t* pool, char* buffer,
                                              int* buffer_len,
                                              ...
                                              int tag)
{
  ...
  (PJ_SSL_SOCK_IMP == PJ_SSL_SOCK_IMP_OPENSSL)
...
#else
            PJ_LOG(3, (THIS_FILE,
                       "Warning: simple random generator is used "
        ...
        crypto->key.ptr =
          (char*)pj_pool_zalloc(pool, crypto_suites[cs_idx].cipher_key_len);
    ...
    status = pj_base64_encode((pj_uint8_t*)crypto->key.ptr,
                              (int)crypto->key.slen, b64_key, &b64_key_len);
    if (status != PJ_SUCCESS) {
        PJ_PERROR(4,
                  (THIS_FILE, status, "Failed encoding plain key to base64"));
        return status;
    }

    b64_key[b64_key_len] = '\0';

    PJ_ASSERT_RETURN(*buffer_len >= (crypto->name.slen + b64_key_len + 16),
                     PJ_ETOOSMALL);

    /* Print the crypto attribute value. */
    print_len = pj_ansi_snprintf(buffer, *buffer_len, "%d %s inline:%s", tag,
                                 crypto_suites[cs_idx].name, b64_key);
...
static pj_status_t parse_attr_crypto(pj_pool_t* pool,
                                     ...
                                     pjmedia_srtp_crypto* crypto, int* tag)
{
    ...
    *tag = pj_strtoul(&token);
    ...
    itmp = MAX_KEY_LEN;
    status = pj_base64_decode(&token, (pj_uint8_t*)crypto->key.ptr, &itmp);
    if (status != PJ_SUCCESS) {
        PJ_PERROR(
          4, (THIS_FILE, status, "Failed decoding crypto key from base64"));
        return status;
    }
    ...
    */
    ...
    if (srtp->offerer_side) {
        ...
        case PJMEDIA_SRTP_DISABLED:
            ...
            return PJ_SUCCESS;
        case PJMEDIA_SRTP_OPTIONAL:
            m_loc->desc.transport = (srtp->peer_use == PJMEDIA_SRTP_MANDATORY) ?
                                      ID_RTP_SAVP :
                                      ID_RTP_AVP;
            break;
        case PJMEDIA_SRTP_MANDATORY:
        ...
        if (pjmedia_sdp_media_find_attr(m_loc, &ID_CRYPTO, NULL) == NULL) {
            ...
            for (i = 0; i < srtp->setting.crypto_count; ++i) {
                ...
                buffer_len = MAXLEN;
                status =
                  generate_crypto_attr_value(srtp->pool, buffer, &buffer_len,
                                             &srtp->setting.crypto[i], tag);
                if (status != PJ_SUCCESS)
                    return status;
        ...
        if (pjmedia_sdp_media_find_attr(m_loc, &ID_CRYPTO, NULL) == NULL) {
            ...
            for (i = 0; i < m_rem->attr_count; ++i) {
                ...
                status =
                  parse_attr_crypto(srtp->pool, m_rem->attr[i], &tmp_rx_crypto,
                                    &tags[cr_attr_count]);
                if (status != PJ_SUCCESS)
                    return status;
            ...
            status = generate_crypto_attr_value(
              ...
              &srtp->setting.crypto[matched_idx], chosen_tag);
            if (status != PJ_SUCCESS)
                return status;
            ...
            if (buffer_len) {
                pj_strset(&attr_value, buffer, buffer_len);
                attr =
                  pjmedia_sdp_attr_create(sdp_pool, ID_CRYPTO.ptr, &attr_value);
                m_loc->attr[m_loc->attr_count++] = attr;
            }
                                     ...
                                     int* count)
{
    ...
    for (i = 0; i < (int)m_loc->attr_count; ++i) {
        ...
        status = parse_attr_crypto(pool, m_loc->attr[i], &tmp_crypto, &loc_tag);
        if (status != PJ_SUCCESS)
            return status;
                                    ...
                                    unsigned media_index)
{
    ...
    if (!srtp->offerer_side) {
        ...
        if ((pj_stricmp(&srtp->tx_policy_neg.name, &loc_crypto[0].name) == 0) &&
            (pj_stricmp(&srtp->tx_policy_neg.key, &loc_crypto[0].key) != 0))
        {
    ...
    for (i = 0; i < m_rem->attr_count; ++i) {
        ...
        status = parse_attr_crypto(srtp->pool, m_rem->attr[i], &tmp_tx_crypto,
                                   &rem_tag);
        if (status != PJ_SUCCESS)
            return status;
        ...
        for (j = 0; j < (int)loc_cryto_cnt; ++j) {
            if (pj_stricmp(&tmp_tx_crypto.name, &loc_crypto[j].name) == 0) {
