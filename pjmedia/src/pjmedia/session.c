                              ...
                              const pjmedia_sdp_session* remote)
{
    ...
    for (i = 0; i < si->stream_cnt; ++i) {
        ...
        status = pjmedia_stream_info_from_sdp(&si->stream_info[i], pool, endpt,
                                              local, remote, i);
                       ...
                       pjmedia_session** p_session)
{
    ...
    pool = pjmedia_endpt_create_pool(endpt, "session", PJMEDIA_SESSION_SIZE,
                                     PJMEDIA_SESSION_INC);
    ...
    for (i = 0; i < (int)si->stream_cnt; ++i) {
        ...
        status = pjmedia_stream_create(
          endpt, session->pool, &session->stream_info[i],
          (transports ? transports[i] : NULL), session, &session->stream[i]);
        if (status == PJ_SUCCESS)
...
{
    PJ_ASSERT_RETURN(session && stat && index < session->stream_cnt, PJ_EINVAL);
