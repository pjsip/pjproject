...
pjmedia_resample_port_create(pj_pool_t* pool, pjmedia_port* dn_port,
                             unsigned clock_rate, unsigned opt,
                             pjmedia_port** p_port)
{
    ...
    status = pjmedia_resample_create(
      pool, (opt & PJMEDIA_RESAMPLE_USE_LINEAR) == 0,
      (opt & PJMEDIA_RESAMPLE_USE_SMALL_FILTER) == 0, d_afd->channel_count,
      d_afd->clock_rate, r_afd->clock_rate, PJMEDIA_PIA_SPF(&dn_port->info),
      &rport->resample_get);
    ...
    status = pjmedia_resample_create(
      pool, (opt & PJMEDIA_RESAMPLE_USE_LINEAR) == 0,
      (opt & PJMEDIA_RESAMPLE_USE_SMALL_FILTER) == 0, d_afd->channel_count,
      r_afd->clock_rate, d_afd->clock_rate, PJMEDIA_PIA_SPF(&rport->base.info),
      &rport->resample_put);
                                      ...
                                      pjmedia_frame* frame)
{
    ...
    if (frame->type == PJMEDIA_FRAME_TYPE_AUDIO) {
        pjmedia_resample_run(rport->resample_put, (const pj_int16_t*)frame->buf,
                             rport->put_buf);
                                      ...
                                      pjmedia_frame* frame)
{
    ...
    if (tmp_frame.type != PJMEDIA_FRAME_TYPE_AUDIO) {
        ...
        frame->size = tmp_frame.size < PJMEDIA_PIA_AVG_FSZ(&rport->base.info) ?
                        tmp_frame.size :
                        PJMEDIA_PIA_AVG_FSZ(&rport->base.info);
        if (tmp_frame.size) {
    ...
    pjmedia_resample_run(rport->resample_get, (const pj_int16_t*)tmp_frame.buf,
