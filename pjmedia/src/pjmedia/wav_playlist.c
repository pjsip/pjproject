                ...
                if (fport->cb2) {
                    ...
                    if (!fport->subscribed) {
                        status = pjmedia_event_subscribe(NULL, &file_on_event,
                                                         fport, fport);
                        fport->subscribed =
                          (status == PJ_SUCCESS) ? PJ_TRUE : PJ_FALSE;
                    }
                    ...
                    if (fport->subscribed && fport->eof != 2) {
                        ...
                        pjmedia_event_init(&event, PJMEDIA_EVENT_CALLBACK, NULL,
                                           fport);
                    ...
                    PJ_LOG(5,
                           (THIS_FILE, "File port %.*s EOF, calling callback",
                    ...
                    status =
                      (*fport->cb)(&fport->base, fport->base.port_data.pdata);
                            ...
                            pj_ssize_t buff_size, pjmedia_port** p_port)
{
    ...
    for (index = 0; index < file_count; index++) {
        ...
        if (!pj_file_exists(filename)) {
            PJ_LOG(4, (THIS_FILE, "WAV playlist error: file '%s' not found",
                       filename));
            return PJ_ENOTFOUND;
        }
    }
    ...
    fport->fd_list =
      (pj_oshandle_t*)pj_pool_zalloc(pool, sizeof(pj_oshandle_t) * file_count);
    if (!fport->fd_list) {
        return PJ_ENOMEM;
    }
    ...
    fport->fsize_list =
      (pj_off_t*)pj_pool_alloc(pool, sizeof(pj_off_t) * file_count);
    if (!fport->fsize_list) {
        return PJ_ENOMEM;
    }

    /* Create start of WAVE data list */
    fport->start_data_list =
      (unsigned*)pj_pool_alloc(pool, sizeof(unsigned) * file_count);
    ...
    for (index = file_count - 1; index >= 0; index--) {
        ...
        /* Open file. */
        status =
          pj_file_open(pool, filename, PJ_O_RDONLY, &fport->fd_list[index]);
        if (status != PJ_SUCCESS)
            goto on_error;
        ...
        if (wavehdr.fmt_hdr.len > 16) {
            ...
            status =
              pj_file_setpos(fport->fd_list[index], size_to_read, PJ_SEEK_CUR);
            if (status != PJ_SUCCESS) {
                goto on_error;
            }
        }
        ...
        for (;;) {
            ...
            PJ_CHECK_OVERFLOW_UINT32_TO_LONG(
              subchunk.len, status = PJMEDIA_ENOTVALIDWAVE; goto on_error;);
        ...
        /* Validate length. */
        if (wavehdr.data_hdr.len >
            fport->fsize_list[index] - fport->start_data_list[index])
        {
            status = PJMEDIA_EWAVEUNSUPP;
            goto on_error;
        }
        if (wavehdr.data_hdr.len <
            ptime * wavehdr.fmt_hdr.sample_rate * wavehdr.fmt_hdr.nchan / 1000)
        {
            status = PJMEDIA_EWAVETOOSHORT;
            goto on_error;
        }
        ...
        if (!has_wave_info) {
            ...
            afd->avg_bps = afd->max_bps =
              afd->clock_rate * afd->channel_count * afd->bits_per_sample;
    ...
    *p_port = &fport->base;
    ...
    PJ_LOG(4, (THIS_FILE,
               ...
               (int)port_label->slen, port_label->ptr, afd->clock_rate,
               afd->channel_count, fport->bufsize / 1000));
                                                  ...
                                                  void* usr_data))
{
    ...
    PJ_LOG(1, (THIS_FILE,
               "pjmedia_wav_playlist_set_eof_cb() is deprecated. "
