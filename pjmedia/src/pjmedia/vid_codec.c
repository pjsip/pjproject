 ...
 */
PJ_DEF(pjmedia_vid_codec_param*)
                              ...
                              const pjmedia_vid_codec_param* src)
{
    ...
    for (i = 0; i < src->dec_fmtp.cnt; ++i) {
        ...
        pj_strdup(pool, &p->dec_fmtp.param[i].val, &src->dec_fmtp.param[i].val);
    }
    for (i = 0; i < src->enc_fmtp.cnt; ++i) {
        pj_strdup(pool, &p->enc_fmtp.param[i].name,
                  &src->enc_fmtp.param[i].name);
        pj_strdup(pool, &p->enc_fmtp.param[i].val, &src->enc_fmtp.param[i].val);
    }
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_codec_mgr_create(pj_pool_t* pool, pjmedia_vid_codec_mgr** p_mgr)
{
                                       ...
                                       pjmedia_vid_codec_factory* factory)
{
    ...
    /* Save the codecs */
    for (i = 0; i < count; ++i) {
        pj_memcpy(&mgr->codec_desc[mgr->codec_cnt + i], &info[i],
                             ...
                             unsigned max_len)
{
    ...
    len =
      pj_ansi_snprintf(id, max_len, "%.*s/%u", (int)info->encoding_name.slen,
                       info->encoding_name.ptr, info->pt);
                                        ...
                                        unsigned prio[])
{
    ...
    for (i = 0; i < mgr->codec_cnt; ++i) {
        if (codec_id->slen == 0 ||
            pj_strnicmp2(codec_id, mgr->codec_desc[i].id, codec_id->slen) == 0)
        {
            if (p_info)
                                        ...
                                        const pjmedia_vid_codec_param* param)
{
    ...
    codec_desc->def_param =
      PJ_POOL_ZALLOC_T(pool, pjmedia_vid_codec_default_param);
