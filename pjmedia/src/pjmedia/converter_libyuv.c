...
static pj_status_t libyuv_conv_convert2(
  pjmedia_converter* converter, pjmedia_frame* src_frame,
  const pjmedia_rect_size* src_frame_size, const pjmedia_coord* src_pos,
  pjmedia_frame* dst_frame, const pjmedia_rect_size* dst_frame_size,
...
static pjmedia_converter_factory_op libyuv_factory_op = {
    &factory_create_converter, &factory_destroy_factory
};

static pjmedia_converter_op libyuv_converter_op = { &libyuv_conv_convert,
                                                    &libyuv_conv_destroy,
...
typedef int (*conv_planar_to_planar_method)(
  const uint8* src1, int src_stride1, const uint8* src2, int src_stride2,
  const uint8* src3, int src_stride3, uint8* dst1, int dst_stride1, uint8* dst2,
  int dst_stride2, uint8* dst3, int dst_stride3, int width, int height);

typedef int (*scale_pack_method)(const uint8* src_argb, int src_stride_argb,
                                 int src_width, int src_height, uint8* dst_argb,
                                 int dst_stride_argb, int dst_width,
                                 int dst_height, enum FilterMode filtering);

typedef int (*scale_planar_method)(
  const uint8* src_y, int src_stride_y, const uint8* src_u, int src_stride_u,
  const uint8* src_v, int src_stride_v, int src_width, int src_height,
  uint8* dst_y, int dst_stride_y, uint8* dst_u, int dst_stride_u, uint8* dst_v,
  int dst_stride_v, int dst_width, int dst_height, enum FilterMode filtering);
...
#    define MAP_CONV_PACK_TO_PACK(src, dst, method) \
        GET_PJ_FORMAT(src), GET_PJ_FORMAT(dst), CONV_PACK_TO_PACK, \
          (gen_conv_func)&method
#    define MAP_CONV_PACK_TO_PLANAR(src, dst, method) \
        GET_PJ_FORMAT(src), GET_PJ_FORMAT(dst), CONV_PACK_TO_PLANAR, \
          (gen_conv_func)&method
#    define MAP_CONV_PLANAR_TO_PACK(src, dst, method) \
        GET_PJ_FORMAT(src), GET_PJ_FORMAT(dst), CONV_PLANAR_TO_PACK, \
          (gen_conv_func)&method
#    define MAP_CONV_PLANAR_TO_PLANAR(src, dst, method) \
        GET_PJ_FORMAT(src), GET_PJ_FORMAT(dst), CONV_PLANAR_TO_PLANAR, \
          (gen_conv_func)&method
#    define MAP_SCALE_PACK(fmt, method) \
        GET_PJ_FORMAT(fmt), GET_PJ_FORMAT(fmt), SCALE_PACK, \
          (gen_conv_func)&method
#    define MAP_SCALE_PLANAR(fmt, method) \
        GET_PJ_FORMAT(fmt), GET_PJ_FORMAT(fmt), SCALE_PLANAR, \
          (gen_conv_func)&method
...
static pj_status_t get_converter_map(pj_uint32_t src_id, pj_uint32_t dst_id,
                                     const pjmedia_rect_size* src_size,
                                     const pjmedia_rect_size* dst_size,
                                     int act_num, converter_act* act)
{
...
#    define GET_MAP(src) \
    ...
    act[act_idx].src_fmt_info.vid_fmt_info = pjmedia_get_video_format_info(
      pjmedia_video_format_mgr_instance(), src_id);

    act[act_idx].dst_fmt_info.vid_fmt_info = pjmedia_get_video_format_info(
      pjmedia_video_format_mgr_instance(), dst_id);
    ...
    act[act_idx].src_fmt_info.apply_param.size =
      (!act_idx) ? *src_size : act[act_idx - 1].dst_fmt_info.apply_param.size;
 ...
 */
static int set_converter_act(pj_uint32_t src_id, pj_uint32_t dst_id,
                             const pjmedia_rect_size* src_size,
                             const pjmedia_rect_size* dst_size,
                             converter_act* act)
{
...
static pj_bool_t check_converter_act(const converter_act* act, int act_num,
                                     pj_uint32_t src_id,
                                     const pjmedia_rect_size* src_size,
                                     pj_uint32_t dst_id,
                                     const pjmedia_rect_size* dst_size)
                                            ...
                                            pjmedia_converter** p_cv)
{
    ...
    /* Only supports video */
    if (prm->src.type != PJMEDIA_TYPE_VIDEO || prm->dst.type != prm->src.type ||
    ...
    src_fmt_info = pjmedia_get_video_format_info(
      pjmedia_video_format_mgr_instance(), prm->src.id);
    ...
    dst_fmt_info = pjmedia_get_video_format_info(
      pjmedia_video_format_mgr_instance(), prm->dst.id);
    ...
    lconv->act_num =
      set_converter_act(src_fmt_info->id, dst_fmt_info->id, &src_detail->size,
                        &dst_detail->size, lconv->act);

    if (!lconv->act_num) {
        return status;
    }

    if (!check_converter_act(lconv->act, lconv->act_num, src_fmt_info->id,
                             &src_detail->size, dst_fmt_info->id,
                             &dst_detail->size))
    {
        return status;
    }
                                       ...
                                       pjmedia_frame* dst_frame)
{
    ...
    for (; i < lconv->act_num; ++i) {
        ...
        struct fmt_info* src_fmt_info = (i == 0) ?
                                          &lconv->act[i].src_fmt_info :
        ...
        case SCALE_PACK:
            (*lconv->act[i].method.scale_pack)(
              ...
              dst_fmt_info->apply_param.strides[0],
              dst_fmt_info->apply_param.size.w,
              dst_fmt_info->apply_param.size.h, LIBYUV_FILTER_MODE);
            break;
        case SCALE_PLANAR:
...
static pj_status_t libyuv_conv_convert2(
  ...
  const pjmedia_rect_size* src_frame_size, const pjmedia_coord* src_pos,
  pjmedia_frame* dst_frame, const pjmedia_rect_size* dst_frame_size,
  const pjmedia_coord* dst_pos, pjmedia_converter_convert_setting* param)
{
    struct libyuv_converter* lconv = (struct libyuv_converter*)converter;
    int i = 0;
    ...
    for (i = 0; i < lconv->act_num; ++i) {
        /* Use destination info as the source info for the next act. */
        struct fmt_info* src_fmt_info =
          (i == 0) ? src_info : &lconv->act[i - 1].dst_fmt_info;
        ...
        if (i == 0) {
            ...
            for (j = 0; j < src_fmt_info->vid_fmt_info->plane_cnt; ++j) {
                int y = src_pos->y * (int)ap->plane_bytes[j] / ap->strides[j] /
                        ap->size.h;
                ap->planes[j] +=
                  y * ap->strides[j] + src_pos->x * ap->strides[j] / ap->size.w;
            }
        ...
        if (i == lconv->act_num - 1) {
            ...
            for (j = 0; j < dst_fmt_info->vid_fmt_info->plane_cnt; ++j) {
                int y = dst_pos->y * (int)ap->plane_bytes[j] / ap->strides[j] /
                        ap->size.h;
                ap->planes[j] +=
                  y * ap->strides[j] + dst_pos->x * ap->strides[j] / ap->size.w;
            }
...
PJ_DEF(pj_status_t)
pjmedia_libyuv_converter_shutdown(pjmedia_converter_mgr* mgr, pj_pool_t* pool)
{
    PJ_UNUSED_ARG(pool);
