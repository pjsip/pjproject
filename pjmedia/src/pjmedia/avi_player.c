...
#    define file_read2(fd, data, size, bits) \
        file_read3(fd, data, size, bits, NULL)
                                  ...
                                  pjmedia_avi_streams** p_streams)
{
    ...
    if (fport[0]->fsize <=
        sizeof(riff_hdr_t) + sizeof(avih_hdr_t) + sizeof(strl_hdr_t))
    {
    ...
    {
        PJ_LOG(3, (THIS_FILE,
                   "Warning!!! Possibly unsupported AVI format: "
                   "flags:%d, pad:%d",
                   avi_hdr.avih_hdr.flags, avi_hdr.avih_hdr.pad));
    }
    ...
    for (i = 0; i < avi_hdr.avih_hdr.num_streams; i++) {
        ...
        /* Read strl header */
        status =
          file_read(fport[0]->fd, &avi_hdr.strl_hdr[i], sizeof(strl_hdr_t));
        if (status != PJ_SUCCESS)
            goto on_error;

        elem =
          COMPARE_TAG(avi_hdr.strl_hdr[i].data_type, PJMEDIA_AVI_VIDS_TAG) ?
            sizeof(strf_video_hdr_t) :
          COMPARE_TAG(avi_hdr.strl_hdr[i].data_type, PJMEDIA_AVI_AUDS_TAG) ?
            sizeof(strf_audio_hdr_t) :
            0;
        ...
        if (elem == sizeof(strf_video_hdr_t))
            data_to_host2(
              &avi_hdr.strf_hdr[i],
              sizeof(strf_video_hdr_sizes) / sizeof(strf_video_hdr_sizes[0]),
              strf_video_hdr_sizes);
        else if (elem == sizeof(strf_audio_hdr_t))
            data_to_host2(
              &avi_hdr.strf_hdr[i],
              sizeof(strf_audio_hdr_sizes) / sizeof(strf_audio_hdr_sizes[0]),
              strf_audio_hdr_sizes);

        /* Skip the remainder of the header */
        size_to_read =
          avi_hdr.strl_hdr[i].list_sz - (sizeof(strl_hdr_t) - 8) - elem;
    ...
    status = pj_file_setpos(fport[0]->fd,
                            avi_hdr.avih_hdr.list_sz + sizeof(riff_hdr_t) + 8,
                            PJ_SEEK_SET);
    if (status != PJ_SUCCESS) {
        goto on_error;
    }
    ...
    do {
        ...
        PJ_CHECK_OVERFLOW_UINT32_TO_LONG(ch.len - read, status = PJ_EINVAL;
                                         goto on_error;);
    ...
    for (i = 0, nstr = 0; i < avi_hdr.avih_hdr.num_streams; i++) {
        ...
        if (COMPARE_TAG(avi_hdr.strl_hdr[i].data_type, PJMEDIA_AVI_VIDS_TAG)) {
            ...
            strf_audio_hdr_t* hdr =
              (strf_audio_hdr_t*)&avi_hdr.strf_hdr[i].strf_audio_hdr;
        ...
        if (nstr > 0) {
            ...
            /* Open file. */
            status =
              pj_file_open(pool, filename, PJ_O_RDONLY, &fport[nstr]->fd);
            if (status != PJ_SUCCESS)
                goto on_error;
    ...
    for (i = 0; i < nstr; i++) {
        ...
        if (COMPARE_TAG(strl_hdr->data_type, PJMEDIA_AVI_VIDS_TAG)) {
            ...
            vfi = pjmedia_get_video_format_info(
              pjmedia_video_format_mgr_instance(), strl_hdr->codec);

            fport[i]->bits_per_sample = (vfi ? vfi->bpp : 0);
            fport[i]->usec_per_frame = avi_hdr.avih_hdr.usec_per_frame;
            pjmedia_format_init_video(
              &fport[i]->base.info.fmt, fport[i]->fmt_id, strf_hdr->biWidth,
              strf_hdr->biHeight, strl_hdr->rate, strl_hdr->scale);
#    if 0
...
#    endif
            ...
            pjmedia_format_init_audio(
              &fport[i]->base.info.fmt, fport[i]->fmt_id, strf_hdr->sample_rate,
              strf_hdr->nchannels, strf_hdr->bits_per_sample,
              20000 /* fport[i]->usec_per_frame */, strf_hdr->bytes_per_sec * 8,
    ...
    *p_streams = pj_pool_alloc(pool, sizeof(pjmedia_avi_streams));
    ...
    (*p_streams)->streams =
      pj_pool_calloc(pool, (*p_streams)->num_streams, sizeof(pjmedia_port*));
    for (i = 0; i < nstr; i++)
    ...
    PJ_LOG(4, (THIS_FILE,
               ...
               "%d media ports",
               (int)fport[0]->base.info.name.slen, fport[0]->base.info.name.ptr,
                                                ...
                                                void* usr_data))
{
    ...
    PJ_LOG(1, (THIS_FILE,
               "pjmedia_avi_stream_set_eof_cb() is deprecated. "
...
static pj_status_t avi_get_frame(pjmedia_port* this_port, pjmedia_frame* frame)
{
    ...
    if (fport->eof) {
        PJ_LOG(5, (THIS_FILE, "File port %.*s EOF",
                   (int)fport->base.info.name.slen, fport->base.info.name.ptr));
        ...
        if (fport->cb2) {
            ...
            if (!fport->subscribed) {
                status =
                  pjmedia_event_subscribe(NULL, &file_on_event, fport, fport);
    ...
    do {
        ...
        if (fport->size_left == 0) {
            ...
            PJ_CHECK_OVERFLOW_UINT32_TO_LONG(ch.len, status = PJ_EINVAL;
                                             goto on_error2;);
            ch_len = ch.len;

            cid = (char*)&ch.id;
            if (cid[0] >= '0' && cid[0] <= '9' && cid[1] >= '0' &&
                cid[1] <= '9') {
            ...
            TRACE_((THIS_FILE,
                    "Reading movi data at pos %u (%x), id: %.*s, "
                    "length: %u",
                    (unsigned long)pos, (unsigned long)pos, 4, cid, ch.len));

            /* We are only interested in data with our stream id */
            if (stream_id != fport->stream_id) {
                if (COMPARE_TAG(ch.id, PJMEDIA_AVI_LIST_TAG))
                    PJ_LOG(5, (THIS_FILE,
                               "Unsupported LIST tag found in "
                               "the movi data."));
                else if (COMPARE_TAG(ch.id, PJMEDIA_AVI_RIFF_TAG)) {
                    PJ_LOG(3, (THIS_FILE,
                               "Unsupported format: multiple "
        ...
        if (frame->type == PJMEDIA_FRAME_TYPE_AUDIO) {
            if (size_to_read > fport->size_left)
                size_to_read = fport->size_left;
            status = file_read3(
              fport->fd, (char*)frame->buf + frame->size - size_to_read,
              size_to_read, fport->bits_per_sample, &size_read);
            if (status != PJ_SUCCESS)
                goto on_error2;
    ...
    if (frame->type == PJMEDIA_FRAME_TYPE_AUDIO) {
        ...
        if (fport->usec_per_frame) {
            fport->next_ts.u64 +=
              (fport->usec_per_frame * fport->base.info.fmt.det.aud.clock_rate /
               1000000);
        } else {
            fport->next_ts.u64 +=
              (frame->size * fport->base.info.fmt.det.aud.clock_rate /
               (fport->base.info.fmt.det.aud.avg_bps / 8));
        }
    } else {
        if (fport->usec_per_frame) {
            fport->next_ts.u64 +=
              (fport->usec_per_frame * VIDEO_CLOCK_RATE / 1000000);
        } else {
