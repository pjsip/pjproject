...
static pj_status_t send_rtcp(pjmedia_vid_stream* stream, pj_bool_t with_sdes,
                             pj_bool_t with_bye, pj_bool_t with_fb_nack,
                             pj_bool_t with_fb_pli);

static void on_rx_rtcp(void* data, void* pkt, pj_ssize_t bytes_read);
...
PJ_INLINE(int)
trace_jb_print_state(pjmedia_vid_stream* stream, char** buf, pj_ssize_t len)
{
    char* p = *buf;
    char* endp = *buf + len;
    ...
    len = pj_ansi_snprintf(p, endp - p, "%d, %d, %d", state.size, state.burst,
                           state.prefetch);
    if ((len < 0) || (len >= endp - p))
        return -1;
...
static void trace_jb_put(pjmedia_vid_stream* stream, const pjmedia_rtp_hdr* hdr,
                         unsigned payloadlen, unsigned frame_cnt)
{
    ...
    len = pj_ansi_snprintf(p, endp - p, "PUT,%d,%d,,%d,%d,%d,", payloadlen,
                           frame_cnt, pj_ntohs(hdr->seq), pj_ntohl(hdr->ts),
                           hdr->m);
    if ((len < 0) || (len >= endp - p))
        goto on_insuff_buffer;
    p += len;
                           ...
                           const char* event_name)
{
    ...
    PJ_LOG(4, (pi->name.ptr, " %s format %s: %dx%d %s%s %d/%d(~%d)fps",
               (chan->dir == PJMEDIA_DIR_DECODING ? "Decoding" : "Encoding"),
               event_name, pi->fmt.det.vid.size.w, pi->fmt.det.vid.size.h,
 ...
 */
static pj_status_t stream_event_cb(pjmedia_event* event, void* user_data)
{
               ...
               event->type == PJMEDIA_EVENT_RX_RTCP_FB)
    {
        ...
        if (data->cap.type == PJMEDIA_RTCP_FB_NACK) {
            if (data->cap.param.slen == 0 && stream->rtcp_fb_nack_cap_idx >= 0)
            {
                /* Generic NACK */

                /* Update event data capability before republishing */
                data->cap =
                  stream->info.loc_rtcp_fb.caps[stream->rtcp_fb_nack_cap_idx];
                       ...
                       stream->rtcp_fb_pli_cap_idx >= 0)
            {
                ...
                /* Update event data capability before republishing */
                data->cap =
                  stream->info.loc_rtcp_fb.caps[stream->rtcp_fb_pli_cap_idx];
            }
        }
    }
 ...
 */
static void publish_tp_event(pjmedia_event_type event_type, pj_status_t status,
                             pj_bool_t is_rtp, pjmedia_dir dir,
                             pjmedia_vid_stream* stream)
{
    ...
    /* Publish event. */
    pjmedia_event_init(&ev, event_type, &ts_now, stream);
...
#        if PJMEDIA_STREAM_ENABLE_KA == PJMEDIA_STREAM_KA_EMPTY_RTP
    ...
    TRC_(
      (channel->port.info.name.ptr, "Sending keep-alive (RTCP and empty RTP)"));

    /* Send RTP */
    status = pjmedia_rtp_encode_rtp(&stream->enc->rtp, stream->enc->pt, 0, 1, 0,
                                    (const void**)&pkt, &pkt_len);
    pj_assert(status == PJ_SUCCESS);

    pj_memcpy(stream->enc->buf, pkt, pkt_len);
    pjmedia_transport_send_rtp(stream->transport, stream->enc->buf, pkt_len);
...
#        elif PJMEDIA_STREAM_ENABLE_KA == PJMEDIA_STREAM_KA_USER
    ...
    /* Send to RTCP port */
    pjmedia_transport_send_rtcp(stream->transport, stream->enc->buf, pkt_len);
                             ...
                             pj_bool_t with_fb_pli)
{
    ...
    if (with_sdes) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4,
                      (stream->name.ptr, status, "Error generating RTCP SDES"));
        } else {
            len += (int)sdes_len;
        }
    }
    ...
    if (with_bye) {
        ...
        status =
          pjmedia_rtcp_build_rtcp_bye(&stream->rtcp, pkt + len, &bye_len, NULL);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4,
                      (stream->name.ptr, status, "Error generating RTCP BYE"));
        } else {
            len += (int)bye_len;
        }
    }
    ...
    if (with_fb_nack && stream->rtcp_fb_nack.pid >= 0) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(
              4, (stream->name.ptr, status, "Error generating RTCP-FB NACK"));
        } else {
            len += (int)fb_len;
        }
    }
    ...
    if (with_fb_pli) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(
              4, (stream->name.ptr, status, "Error generating RTCP-FB PLI"));
        } else {
            len += (int)fb_len;
...
static void check_tx_rtcp(pjmedia_vid_stream* stream)
{
        ...
        pj_elapsed_msec(&stream->rtcp_last_tx, &now) >= stream->rtcp_interval)
    {
        ...
        status =
          send_rtcp(stream, !stream->rtcp_sdes_bye_disabled, PJ_FALSE,
                    stream->pending_rtcp_fb_nack, stream->pending_rtcp_fb_pli);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4, (stream->name.ptr, status, "Error sending RTCP"));
        }
...
static void on_rx_rtp(pjmedia_tp_cb_param* param)
{
    ...
    status = pjmedia_rtp_decode_rtp(&channel->rtp, pkt, (int)bytes_read, &hdr,
                                    &payload, &payloadlen);
    if (status != PJ_SUCCESS) {
...
#    endif
    if (seq_st.status.value) {
        TRC_((channel->port.info.name.ptr,
              ...
              seq_st.status.flag.badpt, seq_st.status.flag.badssrc,
              seq_st.status.flag.dup, seq_st.status.flag.outorder,
              seq_st.status.flag.probation, seq_st.status.flag.restart));

        if (seq_st.status.flag.badpt) {
            PJ_LOG(4,
                   (channel->port.info.name.ptr, "Bad RTP pt %d (expecting %d)",
    ...
    if (param->src_addr) {
        pj_bool_t badssrc =
          (stream->info.has_rem_ssrc && seq_st.status.flag.badssrc);
...
#    endif
    ...
    if (status != 0) {
        LOGERR_(
          (channel->port.info.name.ptr, status, "Jitter buffer put() error"));
        pkt_discarded = PJ_TRUE;
        goto on_return;
    }
...
on_return:
    ...
    pjmedia_rtcp_rx_rtp2(&stream->rtcp, pj_ntohs(hdr->seq), pj_ntohl(hdr->ts),
                         payloadlen, pkt_discarded);
    ...
    if (stream->rtcp.received >= 10 && !stream->initial_rr) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(
              4, (stream->name.ptr, status, "Error sending initial RTCP RR"));
        } else {
...
#    endif
    ...
    if (frame->type == PJMEDIA_FRAME_TYPE_VIDEO && frame->size == 0) {
        pjmedia_rtp_encode_rtp(&channel->rtp, channel->pt, 1, 0, rtp_ts_len,
    ...
    status = pjmedia_vid_codec_encode_begin(
      stream->codec, &enc_opt, frame,
      channel->buf_size - sizeof(pjmedia_rtp_hdr), &frame_out, &has_more_data);
    if (status != PJ_SUCCESS) {
        LOGERR_(
          (channel->port.info.name.ptr, status, "Codec encode_begin() error"));
    ...
    for (;;) {
        status = pjmedia_rtp_encode_rtp(
          &channel->rtp, channel->pt, (has_more_data == PJ_FALSE ? 1 : 0),
          (int)frame_out.size, rtp_ts_len, (const void**)&rtphdr, &rtphdrlen);
        if (status != PJ_SUCCESS) {
            LOGERR_(
              (channel->port.info.name.ptr, status, "RTP encode_rtp() error"));
            return status;
        }
        ...
        if (frame_out.size != 0 && stream->transport) {
            ...
            status = pjmedia_transport_send_rtp(
              stream->transport, (char*)channel->buf,
        ...
        /* Encode more! */
        status = pjmedia_vid_codec_encode_more(
          stream->codec, channel->buf_size - sizeof(pjmedia_rtp_hdr),
          &frame_out, &has_more_data);
        if (status != PJ_SUCCESS) {
            LOGERR_((channel->port.info.name.ptr, status,
...
#    if TRACE_RC
    /* Trace log for rate control */
    {
        ...
        PJ_LOG(5, (stream->name.ptr, "total pkt=%d size=%d sleep=%d", pkt_cnt,
                   total_sent, total_sleep));
                                ...
                                pjmedia_frame* frame)
{
    ...
    for (cnt = 0;;) {
        ...
        pjmedia_jbuf_peek_frame(stream->jb, cnt, NULL, NULL, &ptype, NULL, &ts,
                                &seq);
    ...
    if (got_frame) {
        ...
        for (i = 0; i < frm_pkt_cnt; ++i) {
            ...
            pjmedia_jbuf_peek_frame(
              stream->jb, i, (const void**)&stream->rx_frames[i].buf,
              &stream->rx_frames[i].size, &ptype, NULL, NULL, &frm_last_seq);
        ...
        if (status != PJ_SUCCESS) {
            LOGERR_(
              (channel->port.info.name.ptr, status, "codec decode() error"));
...
static pj_status_t get_frame(pjmedia_port* port, pjmedia_frame* frame)
{
    ...
    if (stream->fmt_event.type != PJMEDIA_EVENT_NONE) {
        ...
        dump_port_info(
          fmt_chg_data->dir == PJMEDIA_DIR_DECODING ? stream->dec : stream->enc,
          "changed");
 ...
 */
static pj_status_t create_channel(pj_pool_t* pool, pjmedia_vid_stream* stream,
                                  pjmedia_dir dir, unsigned pt,
                                  ...
                                  pjmedia_vid_channel** p_channel)
{
    ...
    if (dir == PJMEDIA_DIR_ENCODING) {
        ...
        min_out_pkt_size = sizeof(pjmedia_rtcp_sr_pkt) +
                           ...
                           (4 + (unsigned)stream->cname.slen) + 32;
    ...
    PJ_LOG(5, (name.ptr, "%s channel created %dx%d %s%s%.*s %d/%d(~%d)fps",
               ...
               info->codec_info.encoding_name.ptr, pi->fmt.det.vid.fps.num,
               pi->fmt.det.vid.fps.denum,
               pi->fmt.det.vid.fps.num / pi->fmt.det.vid.fps.denum));
                          ...
                          void* user_data, pjmedia_vid_stream** p_stream)
{
    ...
    if (!pool) {
        own_pool = pjmedia_endpt_create_pool(
          endpt, "vstrm%p", PJMEDIA_VSTREAM_SIZE, PJMEDIA_VSTREAM_INC);
    ...
    stream->name.slen =
      pj_ansi_snprintf(stream->name.ptr, M, "vstrm%p", stream);

    /* Create and initialize codec: */
    status = pjmedia_vid_codec_mgr_alloc_codec(
      stream->codec_mgr, &info->codec_info, &stream->codec);
    if (status != PJ_SUCCESS)
        goto err_cleanup;
    ...
    if (!info->codec_param) {
        ...
        status = pjmedia_vid_codec_mgr_get_default_param(
          stream->codec_mgr, &info->codec_info, &def_param);
        if (status != PJ_SUCCESS)
            goto err_cleanup;
    ...
    info->codec_param->enc_mtu -=
      (sizeof(pjmedia_rtp_hdr) + PJMEDIA_STREAM_RESV_PAYLOAD_LEN);
...
#    endif
    ...
    pjmedia_event_subscribe(NULL, &stream_event_cb, stream, stream->codec);
...
#    endif
    ...
    /* Get frame length in timestamp unit */
    stream->frame_ts_len =
      info->codec_info.clock_rate * vfd_enc->fps.denum / vfd_enc->fps.num;
    ...
    status = create_channel(pool, stream, PJMEDIA_DIR_DECODING, info->rx_pt,
                            info, &stream->dec);
    if (status != PJ_SUCCESS)
        goto err_cleanup;

    /* Create encoder channel */
    status = create_channel(pool, stream, PJMEDIA_DIR_ENCODING, info->tx_pt,
                            info, &stream->enc);
    if (status != PJ_SUCCESS)
        goto err_cleanup;
    ...
    if (info->jb_max >= frm_ptime) {
        ...
        stream->dec_max_delay =
          info->codec_info.clock_rate * info->jb_max / 1000;
    } else {
    ...
    stream->dec_delay_cnt =
      ((PJMEDIA_VID_STREAM_DECODE_MIN_DELAY_MSEC * vfd_dec->fps.num) +
    ...
    stream->rx_frames =
      pj_pool_calloc(pool, stream->rx_frame_cnt, sizeof(stream->rx_frames[0]));

    /* Create jitter buffer */
    status = pjmedia_jbuf_create(
      pool, &stream->dec->port.info.name, PJMEDIA_MAX_MRU,
      1000 * vfd_enc->fps.denum / vfd_enc->fps.num, jb_max, &stream->jb);
    if (status != PJ_SUCCESS)
        goto err_cleanup;
    ...
    /* Init RTCP session: */
    {
        ...
        pjmedia_event_subscribe(NULL, &stream_event_cb, stream, &stream->rtcp);
    }
    ...
    stream->out_rtcp_pkt_size = sizeof(pjmedia_rtcp_sr_pkt) +
                                sizeof(pjmedia_rtcp_common) +
                                (4 + (unsigned)stream->cname.slen) + 32 + 32;
...
#    endif
    ...
    stream->info.codec_param =
      pjmedia_vid_codec_param_clone(pool, info->codec_param);
        ...
        for (i = 0; i < lfi->cap_count; ++i) {
            if (lfi->caps[i].type == PJMEDIA_RTCP_FB_NACK) {
                if (lfi->caps[i].param.slen == 0) {
                    ...
                    PJ_LOG(5,
                           (stream->name.ptr, "Receive RTCP-FB generic NACK"));
 ...
 */
PJ_DEF(pj_status_t) pjmedia_vid_stream_send_keyframe(pjmedia_vid_stream* stream)
{
    pj_timestamp now;
 ...
 * Send RTCP BYE.
 */
PJ_DEF(pj_status_t) pjmedia_vid_stream_send_rtcp_bye(pjmedia_vid_stream* stream)
{
    PJ_ASSERT_RETURN(stream, PJ_EINVAL);
 ...
 * Send RTCP PLI.
 */
PJ_DEF(pj_status_t) pjmedia_vid_stream_send_rtcp_pli(pjmedia_vid_stream* stream)
{
    PJ_ASSERT_RETURN(stream, PJ_EINVAL);
