...
#        define WebRtcAec_Init(handle, clock, sclock) \
            WebRtcAecm_Init(handle, clock)
...
#    if PJMEDIA_WEBRTC_AEC_USE_MOBILE
    PJ_LOG(3, (THIS_FILE,
               "WebRTC AEC mobile successfully created with "
               "options %d",
               options));
#    else
    PJ_LOG(3, (THIS_FILE,
               "WebRTC AEC successfully created with "
               "options %d",
               options));
#    endif
...
#    if PJMEDIA_WEBRTC_AEC_USE_MOBILE
        status = WebRtcAecm_Process(
          echo->AEC_inst, &rec_frm[frm_idx], (echo->NS_inst ? buf_ptr : NULL),
          out_buf_ptr, echo->subframe_len, echo->tail);
#    else
...
#    if !PJMEDIA_WEBRTC_AEC_USE_MOBILE
        if (echo->NS_inst) {
            ...
            buf_ptr = echo->tmp_buf2;
            out_buf_ptr = echo->tmp_buf;
            WebRtcNs_Process(echo->NS_inst, &buf_ptr, echo->channel_count,
                             &out_buf_ptr);
        }
#    endif
...
PJ_DEF(pj_status_t) webrtc_aec_get_stat(void* state, pjmedia_echo_stat* p_stat)
{
    webrtc_ec* echo = (webrtc_ec*)state;

    if (WebRtcAec_GetDelayMetrics(echo->AEC_inst, &p_stat->delay, &p_stat->std,
                                  &p_stat->frac_delay) != 0)
    {
        return PJ_EUNKNOWN;
    }
