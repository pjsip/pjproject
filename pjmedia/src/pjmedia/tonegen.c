...
#    define GEN_INIT(var, R, F, A) \
        var.add = ((DATA)F) / R, var.c = 0, var.vol = A
#    define GEN_SAMP(val, var) \
        val = (short)(sin(var.c * 2 * M_PI) * var.vol); \
        var.c += var.add
...
#    define GEN_INIT(var, R, F, A) \
        var.a = (DATA)(2.0 * sin(M_PI * F / R)); \
        var.s0 = 0; \
        var.s1 = (DATA)(0 - (int)A)
#    define GEN_SAMP(val, var) \
        var.s0 = var.s0 - var.a * var.s1; \
...
static int cordic_ctab[] = { 0x03243F6A, 0x01DAC670, 0x00FADBAF, 0x007F56EA,
                             0x003FEAB7, 0x001FFD55, 0x000FFFAA, 0x0007FFF5,
                             0x0003FFFE, 0x0001FFFF, 0x0000FFFF, 0x00007FFF,
...
#    define VOL(var, v) (((v)*var.vol) >> 15)
#    define GEN_INIT(var, R, F, A) \
        var.add = MAXI / R * F, var.c = 0, var.vol = A
#    define GEN_SAMP(val, var) \
        val = (short)VOL(var, SIN(var.c) >> 16); \
        var.c += var.add
...
static void init_generate_single_tone(struct gen_state* state,
                                      unsigned clock_rate, unsigned freq,
                                      unsigned vol)
{
...
static void generate_single_tone(struct gen_state* state,
                                 unsigned channel_count, unsigned samples,
                                 short buf[])
{
...
static void init_generate_dual_tone(struct gen_state* state,
                                    unsigned clock_rate, unsigned freq1,
                                    unsigned freq2, unsigned vol)
{
...
static void generate_dual_tone(struct gen_state* state, unsigned channel_count,
                               unsigned samples, short buf[])
{
    short* end = buf + samples;
...
static void init_generate_tone(struct gen_state* state, unsigned clock_rate,
                               unsigned freq1, unsigned freq2, unsigned vol)
{
    if (freq2)
...
static void generate_tone(struct gen_state* state, unsigned channel_count,
                          unsigned samples, short buf[])
{
    if (!state->has_tone2)
...
static pjmedia_tone_digit_map digit_map = { 16,
                                            {
                                              { '0', 941, 1336 },
                                              { '1', 697, 1209 },
                                              { '2', 697, 1336 },
                        ...
                        unsigned options, pjmedia_port** p_port)
{
    ...
    PJ_ASSERT_RETURN(pool && clock_rate && channel_count && samples_per_frame &&
                       bits_per_sample == 16 && p_port != NULL,
                     PJ_EINVAL);
    ...
    if (name == NULL || name->slen == 0)
        name = &STR_TONE_GEN;
    status =
      pjmedia_port_info_init(&tonegen->base.info, name, SIGNATURE, clock_rate,
                       ...
                       pjmedia_port** p_port)
{
    return pjmedia_tonegen_create2(pool, NULL, clock_rate, channel_count,
                                   samples_per_frame, bits_per_sample, options,
                                   p_port);
}
 ...
 */
static pj_status_t tonegen_get_frame(pjmedia_port* port, pjmedia_frame* frame)
{
    struct tonegen* tonegen = (struct tonegen*)port;
    short *dst, *end;
    ...
    if (tonegen->cur_digit > tonegen->count) {
        ...
        if ((tonegen->options | tonegen->playback_options) &
    ...
    while (dst < end) {
        ...
        on_samp =
          dig->on_msec * clock_rate / 1000 * PJMEDIA_PIA_CCNT(&port->info);
        off_samp =
          dig->off_msec * clock_rate / 1000 * PJMEDIA_PIA_CCNT(&port->info);
        ...
        if (tonegen->dig_samples < on_samp) {
            ...
            if (cnt > required)
                cnt = required;
            generate_tone(&tonegen->state, PJMEDIA_PIA_CCNT(&port->info), cnt,
                          dst);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_tonegen_play(pjmedia_port* port, unsigned count,
                     const pjmedia_tone_desc tones[], unsigned options)
{
    ...
    PJ_ASSERT_RETURN(
      port && port->info.signature == SIGNATURE && count && tones, PJ_EINVAL);
    ...
    /* Copy digits */
    pj_memcpy(tonegen->digits + tonegen->count, tones,
              count * sizeof(pjmedia_tone_desc));
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_tonegen_play_digits(pjmedia_port* port, unsigned count,
                            const pjmedia_tone_digit digits[], unsigned options)
{
    struct tonegen* tonegen = (struct tonegen*)port;
    ...
    PJ_ASSERT_RETURN(
      port && port->info.signature == SIGNATURE && count && digits, PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_tonegen_get_digit_map(pjmedia_port* port,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_tonegen_set_digit_map(pjmedia_port* port, pjmedia_tone_digit_map* m)
{
    struct tonegen* tonegen = (struct tonegen*)port;
