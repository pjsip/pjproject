...
static void on_rx_rtp(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
                      pj_ssize_t bytes_read);
static void on_rtp_data_sent(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
                             pj_ssize_t bytes_sent);
static void on_rx_rtcp(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
                       pj_ssize_t bytes_read);
...
static pj_status_t transport_encode_sdp(pjmedia_transport* tp, pj_pool_t* pool,
...
{
    return pjmedia_transport_udp_create3(endpt, pj_AF_INET(), name, addr, port,
                                         options, p_tp);
}
                              ...
                              pjmedia_transport** p_tp)
{
    ...
    /* Bind RTCP socket */
    status =
      pj_sockaddr_init(af, &si.rtcp_addr_name, addr, (pj_uint16_t)(port + 1));
    if (status != PJ_SUCCESS)
        goto on_error;
                             ...
                             pjmedia_transport** p_tp)
{
    ...
    status = pj_ioqueue_register_sock(pool, ioqueue, tp->rtp_sock, tp, &rtp_cb,
                                      &tp->rtp_key);
    if (status != PJ_SUCCESS)
        goto on_error;
                      ...
                      pj_ssize_t bytes_read)
{
  ...
  (PJMEDIA_TRANSPORT_SWITCH_REMOTE_ADDR == 1)
        if (rem_switch &&
            ...
            PJ_LOG(4, (udp->base.name, "Remote RTP address switched to %s",
            ...
            if (udp->use_rtcp_mux) {
                ...
                pj_sockaddr_cp(&udp->rem_rtcp_addr, &udp->rem_rtp_addr);
                port =
                  (pj_uint16_t)(pj_sockaddr_get_port(&udp->rem_rtp_addr) + 1);
    ...
    } while (status != PJ_EPENDING && status != PJ_ECANCELLED && udp->started);
}
        ...
        if (bytes_read > 0 &&
            ...
            if (pj_sockaddr_cmp(&udp->rem_rtcp_addr, &udp->rtcp_src_addr) == 0)
            {
                ...
                if (udp->rtcp_src_cnt >= PJMEDIA_RTCP_NAT_PROBATION_CNT) {
                    ...
                    PJ_LOG(
                      4, (udp->base.name, "Remote RTCP address switched to %s",
                          pj_sockaddr_print(&udp->rtcp_src_addr, addr_text,
                                            sizeof(addr_text), 3)));
                }
            }
                                      ...
                                      pjmedia_transport_info* info)
{
    ...
    info->sock_info.rtcp_addr_name =
      (udp->use_rtcp_mux ? udp->rtp_addr_name : udp->rtcp_addr_name);
...
static pj_status_t tp_attach(pjmedia_transport* tp, void* user_data,
                             const pj_sockaddr_t* rem_addr,
                             const pj_sockaddr_t* rem_rtcp, unsigned addr_len,
                             void (*rtp_cb)(void*, void*, pj_ssize_t),
                             ...
                             void (*rtcp_cb)(void*, void*, pj_ssize_t))
{
    ...
    status =
      pj_sockaddr_synthesize(sock_addr.addr.sa_family, &remote_addr, rem_addr);
    if (status != PJ_SUCCESS) {
        pj_perror(3, tp->name, status,
                  "Failed to synthesize the correct"
                  "IP address for RTP");
    }
    ...
    if (rtcp_addr && pj_sockaddr_has_addr(rtcp_addr)) {
        status = pj_sockaddr_synthesize(sock_addr.addr.sa_family, &remote_rtcp,
                                        rem_rtcp);
        if (status != PJ_SUCCESS) {
            pj_perror(3, tp->name, status,
                      "Failed to synthesize the correct"
...
static pj_status_t transport_attach(pjmedia_transport* tp, void* user_data,
                                    ...
                                    void (*rtcp_cb)(void*, void*, pj_ssize_t))
{
    return tp_attach(tp, user_data, rem_addr, rem_rtcp, addr_len, rtp_cb, NULL,
                     rtcp_cb);
}
                                     ...
                                     pjmedia_transport_attach_param* att_param)
{
    return tp_attach(tp, att_param->user_data,
                     ...
                     (pj_sockaddr_t*)&att_param->rem_rtcp, att_param->addr_len,
                     att_param->rtp_cb, att_param->rtp_cb2, att_param->rtcp_cb);
}

/* Called by application when it no longer needs the transport */
static void transport_detach(pjmedia_transport* tp, void* user_data)
{
    struct transport_udp* udp = (struct transport_udp*)tp;
                                      ...
                                      pj_size_t size)
{
    ...
    sent = size;
    status = pj_ioqueue_sendto(udp->rtp_key, &udp->rtp_pending_write[id].op_key,
                               pw->buffer, &sent, 0, &udp->rem_rtp_addr,
                               udp->addr_len);
    ...
    udp->rtp_write_op_id =
      (udp->rtp_write_op_id + 1) % PJ_ARRAY_SIZE(udp->rtp_pending_write);
                                        ...
                                        pj_size_t size)
{
    ...
    sent = size;
    status =
      pj_ioqueue_sendto((udp->use_rtcp_mux ? udp->rtp_key : udp->rtcp_key),
                        &udp->rtcp_write_op, pkt, &sent, 0, addr, addr_len);
                                        ...
                                        unsigned media_index)
{
    ...
    if (udp->enable_rtcp_mux) {
        ...
        if (!udp->use_rtcp_mux) {
             ...
             * rejects it.
             */
            attr = pjmedia_sdp_attr_create_rtcp(pool, &udp->rtcp_addr_name);
            if (attr)
                                         ...
                                         unsigned media_index)
{
    ...
    status = pj_ioqueue_recvfrom(udp->rtp_key, &udp->rtp_read_op, udp->rtp_pkt,
                                 &size, PJ_IOQUEUE_ALWAYS_ASYNC,
                                 &udp->rtp_src_addr, &udp->rtp_addrlen);
    if (status != PJ_EPENDING) {
        PJ_PERROR(
          3, (udp->base.name, status, "media_start(): recvfrom RTP failed"));
        return status;
    }
    ...
    status = pj_ioqueue_recvfrom(udp->rtcp_key, &udp->rtcp_read_op,
                                 udp->rtcp_pkt, &size, PJ_IOQUEUE_ALWAYS_ASYNC,
                                 &udp->rtcp_src_addr, &udp->rtcp_addr_len);
    if (status != PJ_EPENDING) {
        PJ_PERROR(
          3, (udp->base.name, status, "media_start(): recvfrom RTCP failed"));
        pj_ioqueue_clear_key(udp->rtp_key);
        return status;
    }
