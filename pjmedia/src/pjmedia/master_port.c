                           ...
                           pjmedia_master_port** p_m)
{
    ...
    PJ_ASSERT_RETURN(
      PJMEDIA_PIA_SPF(&u_port->info) == PJMEDIA_PIA_SPF(&d_port->info),
    ...
    /* Create media clock */
    status =
      pjmedia_clock_create(pool, clock_rate, channel_count, samples_per_frame,
...
pjmedia_master_port_set_uport(pjmedia_master_port* m, pjmedia_port* port)
{
    ...
    if (m->d_port) {
        PJ_ASSERT_RETURN(
          PJMEDIA_PIA_PTIME(&port->info) == PJMEDIA_PIA_PTIME(&m->d_port->info),
          PJMEDIA_ENCSAMPLESPFRAME);
    }
...
pjmedia_master_port_set_dport(pjmedia_master_port* m, pjmedia_port* port)
{
    ...
    if (m->u_port) {
        PJ_ASSERT_RETURN(
          PJMEDIA_PIA_PTIME(&port->info) == PJMEDIA_PIA_PTIME(&m->u_port->info),
          PJMEDIA_ENCSAMPLESPFRAME);
    }
