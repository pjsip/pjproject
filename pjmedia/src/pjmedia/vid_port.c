...
static pj_status_t vidstream_render_cb(pjmedia_vid_dev_stream* stream,
                                       void* user_data, pjmedia_frame* frame);
static pj_status_t vidstream_event_cb(pjmedia_event* event, void* user_data);
static pj_status_t client_port_event_cb(pjmedia_event* event, void* user_data);
...
static pj_uint32_t match_format_id(pj_uint32_t req_id, pj_uint32_t sup_id)
{
    ...
    req_fmt_info = pjmedia_get_video_format_info(
      pjmedia_video_format_mgr_instance(), req_id);

    sup_fmt_info = pjmedia_get_video_format_info(
      pjmedia_video_format_mgr_instance(), sup_id);
...
static void test_find_closest_fmt(pjmedia_vid_dev_info* di)
{
    ...
    pjmedia_rect_size find_size[] = {
        { 720, 480 },  { 352, 288 }, { 400, 300 },
        { 1600, 900 }, { 255, 352 }, { 500, 500 },
    };
    ...
    pj_uint32_t find_id[] = { PJMEDIA_FORMAT_RGB24, PJMEDIA_FORMAT_RGBA,
                              PJMEDIA_FORMAT_AYUV, PJMEDIA_FORMAT_YUY2,
                              PJMEDIA_FORMAT_I420 };
    ...
    for (i = 0; i < di->fmt_cnt; i++) {
        ...
        TRACE_FIND_FMT((THIS_FILE, "id:%s size:%d*%d fps:%d/%d", fmt_name,
                        vid_fd->size.w, vid_fd->size.h, vid_fd->fps.num,
                        vid_fd->fps.denum));
    }
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(find_id); i++) {
        for (j = 0; j < PJ_ARRAY_SIZE(find_fps); j++) {
            for (k = 0; k < PJ_ARRAY_SIZE(find_size); k++) {
                ...
                TRACE_FIND_FMT((THIS_FILE,
                                "Trying to find closest match "
                                "id:%s size:%dx%d fps:%d/%d",
                                fmt_name, find_size[k].w, find_size[k].h,
                                find_fps[j].num, find_fps[j].denum));

                match_prop =
                  find_closest_fmt(find_id[i], &find_size[k], &find_fps[j], di);
                     ...
                     find_fps[j].num * find_fps[j].denum))
                {
                    ...
                    TRACE_FIND_FMT((THIS_FILE,
                                    "Closest format = "
                                    "id:%s size:%dx%d fps:%d/%d",
                                    fmt_name, match_prop.size.w,
                                    match_prop.size.h, match_prop.fps.num,
                        ...
                        pjmedia_vid_port** p_vid_port)
{
    ...
    PJ_LOG(4, (THIS_FILE,
               ...
               dev_name, vid_dir_name(prm->vidparam.dir), fmt_name, vfd->size.w,
...
#    endif
        ...
        match_prop =
          find_closest_fmt(prm->vidparam.fmt.id, &vfd->size, &vfd->fps, &di);
    ...
    vp->sync_clocksrc.max_sync_ticks =
      PJMEDIA_CLOCK_SYNC_MAX_RESYNC_DURATION * 1000 / vp->clocksrc.ptime_usec;
    ...
    pjmedia_fourcc_name(vparam.fmt.id, fmt_name);
    PJ_LOG(4, (THIS_FILE, "Device %s opened: format=%s, size=%dx%d @%d:%d fps",
               dev_name, fmt_name, vparam.fmt.det.vid.size.w,
               vparam.fmt.det.vid.size.h, vparam.fmt.det.vid.fps.num,
               vparam.fmt.det.vid.fps.denum));
        ...
        ((vp->dir & PJMEDIA_DIR_ENCODING) || vp->stream_role == ROLE_PASSIVE))
    {
        ...
        status = pjmedia_clock_create2(
          pool, &param, PJMEDIA_CLOCK_NO_HIGHEST_PRIO,
          (vp->dir & PJMEDIA_DIR_ENCODING) ? &enc_clock_cb : &dec_clock_cb, vp,
          &vp->clock);
        if (status != PJ_SUCCESS)
            goto on_error;
        ...
        pjmedia_port_info_init2(&pp->base.info, &vp->dev_name,
                                PJMEDIA_SIG_VID_PORT, prm->vidparam.dir,
                                &prm->vidparam.fmt);
    ...
    if (need_frame_buf) {
        ...
        status = pjmedia_video_format_fill_black(
          &vp->conv.conv_param.src, vp->frm_buf->buf, vp->frm_buf_size);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4, (THIS_FILE, status,
                          "Warning: failed to init buffer with black"));
        }

        status = pj_mutex_create_simple(pool, vp->dev_name.ptr, &vp->frm_mutex);
        if (status != PJ_SUCCESS)
            goto on_error;
    }
...
PJ_DEF(pj_status_t)
pjmedia_vid_port_subscribe_event(pjmedia_vid_port* vp, pjmedia_port* port)
{
...
PJ_DEF(pj_status_t)
pjmedia_vid_port_unsubscribe_event(pjmedia_vid_port* vp, pjmedia_port* port)
{
    PJ_ASSERT_RETURN(vp && port, PJ_EINVAL);
...
PJ_DEF(pj_status_t)
pjmedia_vid_port_connect(pjmedia_vid_port* vp, pjmedia_port* port,
                         pj_bool_t destroy)
{
    ...
    pjmedia_event_subscribe(NULL, &client_port_event_cb, vp, vp->client_port);
...
PJ_DEF(pj_status_t) pjmedia_vid_port_disconnect(pjmedia_vid_port* vp)
{
    ...
    pjmedia_event_unsubscribe(NULL, &client_port_event_cb, vp, vp->client_port);
    vp->client_port = NULL;
...
PJ_DEF(pj_status_t) pjmedia_vid_port_start(pjmedia_vid_port* vp)
{
    ...
    if (status != PJ_SUCCESS) {
        PJ_PERROR(
          4, (THIS_FILE, status, "Warning: failed to init buffer with black"));
    }
...
static pj_status_t client_port_event_cb(pjmedia_event* event, void* user_data)
{
    ...
    if (event->type == PJMEDIA_EVENT_FMT_CHANGED) {
        ...
        vfd_cur =
          pjmedia_format_get_video_format_detail(&vid_param.fmt, PJ_TRUE);
        if (!vfd_cur)
         ...
         */
        if (vp->dir == PJMEDIA_DIR_RENDER && vp->stream_role == ROLE_PASSIVE &&
            vp->role == ROLE_ACTIVE)
        {
            pj_bool_t fps_only;
             ...
             vp->conv.conv_param.dst.det.vid.size.w))
        {
            status = pjmedia_vid_dev_stream_set_cap(
              vp->strm, PJMEDIA_VID_DEV_CAP_FORMAT, &vp->conv.conv_param.dst);
            if (status != PJ_SUCCESS) {
                pjmedia_event e;
...
static pj_status_t convert_frame(pjmedia_vid_port* vp, pjmedia_frame* src_frame,
                                 pjmedia_frame* dst_frame)
{
    ...
    if (vp->conv.conv) {
        ...
        status = pjmedia_converter_convert(vp->conv.conv, src_frame, dst_frame);
    }
...
static void copy_frame_to_buffer(pjmedia_vid_port* vp, pjmedia_frame* frame)
{
                                       ...
                                       void* user_data, pjmedia_frame* frame)
{
    ...
    if (vp->role == ROLE_ACTIVE) {
        unsigned frame_ts =
          vp->clocksrc.clock_rate / 1000 * vp->clocksrc.ptime_usec / 1000;
            ...
            if (nsync_frame == 0) {
                ...
                if (diff >= 0) {
                    ...
                    if (vp->sync_clocksrc.max_sync_ticks > 0) {
                        skip_mod =
                          init_sync_frame / vp->sync_clocksrc.max_sync_ticks +
                          2;
                    } else
                        skip_mod = init_sync_frame + 2;

                    PJ_LOG(
                      5, (THIS_FILE, "synchronization: early by %d ms", diff));
                    ...
                    for (i = 0; i < ndrop; i++) {
                        vp->frm_buf->size = vp->frm_buf_size;
                        status =
                          pjmedia_port_get_frame(vp->client_port, vp->frm_buf);
                        if (status != PJ_SUCCESS) {
                            pjmedia_clock_src_update(&vp->clocksrc, NULL);
                            return status;
                        }
                                           ...
                                           pjmedia_frame* frame)
{
    ...
    if (vp->stream_role == ROLE_PASSIVE) {
        ...
        return pjmedia_vid_dev_stream_put_frame(
          vp->strm, (vp->conv.conv ? &frame_ : frame));
    } else {
