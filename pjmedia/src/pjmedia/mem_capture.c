...
struct mem_rec
{
    ...
    pj_bool_t eof;
    void* user_data;
    pj_status_t (*cb)(pjmedia_port* port, void* user_data);
    pj_bool_t subscribed;
    void (*cb2)(pjmedia_port*, void*);
};

static pj_status_t rec_put_frame(pjmedia_port* this_port, pjmedia_frame* frame);
static pj_status_t rec_get_frame(pjmedia_port* this_port, pjmedia_frame* frame);
static pj_status_t rec_on_destroy(pjmedia_port* this_port);

PJ_DEF(pj_status_t)
pjmedia_mem_capture_create(pj_pool_t* pool, void* buffer, pj_size_t size,
                           ...
                           unsigned options, pjmedia_port** p_port)
{
    ...
    /* Create the rec */
    pjmedia_port_info_init(&rec->base.info, &name, SIGNATURE, clock_rate,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_mem_capture_set_eof_cb(pjmedia_port* port, void* user_data,
                                                 ...
                                                 void* usr_data))
{
    ...
    PJ_LOG(1, (THIS_FILE,
               "pjmedia_mem_capture_set_eof_cb() is deprecated. "
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_mem_capture_set_eof_cb2(pjmedia_port* port, void* user_data,
...
PJ_DEF(pj_size_t) pjmedia_mem_capture_get_size(pjmedia_port* port)
{
    ...
    PJ_ASSERT_RETURN(port->info.signature == SIGNATURE, 0);
...
static pj_status_t rec_on_event(pjmedia_event* event, void* user_data)
{
        ...
        if (rec->write_pos == endpos) {
            ...
            if (rec->cb2) {
                if (!rec->subscribed) {
                    ...
                    status =
                      pjmedia_event_subscribe(NULL, rec_on_event, rec, rec);
                    rec->subscribed =
