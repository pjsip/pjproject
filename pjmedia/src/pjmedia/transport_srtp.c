...
#    if defined(PJMEDIA_SRTP_HAS_AES_GCM_256) && \
    ...
    { "AEAD_AES_256_GCM", SRTP_AES_GCM_256, 44, 12, SRTP_NULL_AUTH, 0, 16, 16,
      sec_serv_conf_and_auth, &srtp_aes_gcm_256_openssl },

    /* cipher AES_GCM, NULL auth, auth tag len = 8 octets */
    { "AEAD_AES_256_GCM_8", SRTP_AES_GCM_256, 44, 12, SRTP_NULL_AUTH, 0, 8, 8,
      sec_serv_conf_and_auth, &srtp_aes_gcm_256_openssl },
#    endif
#    if defined(PJMEDIA_SRTP_HAS_AES_CM_256) && \
    ...
    { "AES_256_CM_HMAC_SHA1_80", SRTP_AES_ICM_256, 46, 14, SRTP_HMAC_SHA1, 20,
      10, 10, sec_serv_conf_and_auth, NULL,
      &srtp_crypto_policy_set_aes_cm_256_hmac_sha1_80 },

    /* cipher AES_CM_256, auth SRTP_HMAC_SHA1, auth tag len = 10 octets */
    { "AES_256_CM_HMAC_SHA1_32", SRTP_AES_ICM_256, 46, 14, SRTP_HMAC_SHA1, 20,
      4, 10, sec_serv_conf_and_auth, NULL,
...
#    if defined(PJMEDIA_SRTP_HAS_AES_CM_192) && \
    ...
    { "AES_192_CM_HMAC_SHA1_80", SRTP_AES_ICM_192, 38, 14, SRTP_HMAC_SHA1, 20,
      10, 10, sec_serv_conf_and_auth, &srtp_aes_icm_192 },

    /* cipher AES_CM_192, auth SRTP_HMAC_SHA1, auth tag len = 4 octets */
    { "AES_192_CM_HMAC_SHA1_32", SRTP_AES_ICM_192, 38, 14, SRTP_HMAC_SHA1, 20,
      4, 10, sec_serv_conf_and_auth, &srtp_aes_icm_192 },
#    endif
#    if defined(PJMEDIA_SRTP_HAS_AES_GCM_128) && \
      (PJMEDIA_SRTP_HAS_AES_GCM_128 != 0)

    /* cipher AES_GCM, NULL auth, auth tag len = 16 octets */
    { "AEAD_AES_128_GCM", SRTP_AES_GCM_128, 28, 12, SRTP_NULL_AUTH, 0, 16, 16,
      sec_serv_conf_and_auth, &srtp_aes_gcm_128_openssl },

    /* cipher AES_GCM, NULL auth, auth tag len = 8 octets */
    { "AEAD_AES_128_GCM_8", SRTP_AES_GCM_128, 28, 12, SRTP_NULL_AUTH, 0, 8, 8,
      sec_serv_conf_and_auth, &srtp_aes_gcm_128_openssl },
#    endif
#    if defined(PJMEDIA_SRTP_HAS_AES_CM_128) && \
    ...
    { "AES_CM_128_HMAC_SHA1_80", SRTP_AES_ICM_128, 30, 14, SRTP_HMAC_SHA1, 20,
      10, 10, sec_serv_conf_and_auth },

    /* cipher AES_CM_128, auth SRTP_HMAC_SHA1, auth tag len = 4 octets */
    { "AES_CM_128_HMAC_SHA1_32", SRTP_AES_ICM_128, 30, 14, SRTP_HMAC_SHA1, 20,
      4, 10, sec_serv_conf_and_auth },
#    endif
...
static pjmedia_transport_op transport_srtp_op = { &transport_get_info,
...
pjmedia_transport_srtp_dtls_start_nego(
  pjmedia_transport* srtp, const pjmedia_srtp_dtls_nego_param* param)
{
    PJ_UNUSED_ARG(srtp);
    PJ_UNUSED_ARG(param);
    return PJ_ENOTSUP;
...
PJ_DEF(pj_status_t)
pjmedia_transport_srtp_dtls_get_fingerprint(pjmedia_transport* srtp,
                                            const char* hash, char* buf,
                                            pj_size_t* len)
{
    PJ_UNUSED_ARG(srtp);
    PJ_UNUSED_ARG(hash);
...
#        if defined(PJMEDIA_SRTP_HAS_DEINIT) && PJMEDIA_SRTP_HAS_DEINIT != 0
    err = srtp_deinit();
#        elif defined(PJMEDIA_SRTP_HAS_SHUTDOWN) && \
          PJMEDIA_SRTP_HAS_SHUTDOWN != 0
    err = srtp_shutdown();
#        else
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_srtp_enum_crypto(unsigned* count, pjmedia_srtp_crypto crypto[])
{
    unsigned i, max;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_srtp_enum_keying(unsigned* count, pjmedia_srtp_keying_method keying[])
{
    unsigned max;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_transport_srtp_create(pjmedia_endpt* endpt, pjmedia_transport* tp,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_transport_srtp_get_setting(pjmedia_transport* tp,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_transport_srtp_modify_setting(pjmedia_transport* tp,
                                      const pjmedia_srtp_setting* opt)
{
    transport_srtp* srtp = (transport_srtp*)tp;
    srtp->setting = *opt;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_transport_srtp_start(pjmedia_transport* tp,
                             ...
                             const pjmedia_srtp_crypto* rx)
{
    ...
    if (srtp->setting.tx_roc.roc != 0 && srtp->setting.tx_roc.ssrc != 0) {
        ...
        status =
          srtp_set_stream_roc(srtp->srtp_tx_ctx, srtp->setting.tx_roc.ssrc,
                              srtp->setting.tx_roc.roc);
        PJ_LOG(4, (THIS_FILE,
                   "Initializing SRTP TX ROC to SSRC %d with "
                   "ROC %d %s\n",
                   srtp->setting.tx_roc.ssrc, srtp->setting.tx_roc.roc,
    ...
    if (srtp->setting.rx_roc.roc != 0 && srtp->setting.rx_roc.ssrc != 0) {
        srtp_err_status_t status;
        status =
          srtp_set_stream_roc(srtp->srtp_rx_ctx, srtp->setting.rx_roc.ssrc,
                              srtp->setting.rx_roc.roc);
        PJ_LOG(4, (THIS_FILE,
                   "Initializing SRTP RX ROC from SSRC %d with "
                   "ROC %d %s\n",
...
PJ_DEF(pj_status_t) pjmedia_transport_srtp_stop(pjmedia_transport* srtp)
{
    ...
    if (err != srtp_err_status_ok) {
        PJ_LOG(4,
               (p_srtp->pool->obj_name, "Failed to dealloc RX SRTP context: %s",
                get_libsrtp_errstr(err)));
    }
    err = srtp_dealloc(p_srtp->srtp_tx_ctx);
    if (err != srtp_err_status_ok) {
        PJ_LOG(4,
               (p_srtp->pool->obj_name, "Failed to dealloc TX SRTP context: %s",
                get_libsrtp_errstr(err)));
    }
...
static pj_status_t start_srtp(transport_srtp* srtp)
{
    ...
    {
        pj_status_t status;
        status = pjmedia_transport_srtp_start(&srtp->base, &srtp->tx_policy_neg,
                                              &srtp->rx_policy_neg);
        if (status != PJ_SUCCESS)
            return status;
        ...
        PJ_LOG(4, (srtp->pool->obj_name, "SRTP started, keying=%s, crypto=%s",
...
pjmedia_transport_srtp_get_member(pjmedia_transport* tp)
{
    transport_srtp* srtp = (transport_srtp*)tp;
                                      ...
                                      pjmedia_transport_info* info)
{
    ...
    PJ_ASSERT_RETURN(
      info->specific_info_cnt < PJMEDIA_TRANSPORT_SPECIFIC_INFO_MAXCNT,
      PJ_ETOOMANY);
    PJ_ASSERT_RETURN(
      sizeof(pjmedia_srtp_info) <= PJMEDIA_TRANSPORT_SPECIFIC_INFO_MAXSIZE,
      PJ_ENOMEM);
                                     ...
                                     pjmedia_transport_attach_param* param)
{
    ...
    srtp->use_rtcp_mux =
      (pj_sockaddr_has_addr(&param->rem_addr) &&
       pj_sockaddr_cmp(&param->rem_addr, &param->rem_rtcp) == 0);
...
#    endif
    ...
    if (err == srtp_err_status_ok) {
        status =
          pjmedia_transport_send_rtp(srtp->member_tp, srtp->rtp_tx_buffer, len);
    } else {
        status = PJMEDIA_ERRNO_FROM_LIBSRTP(err);
    }
...
#    if TEST_ROC
    if (srtp->setting.rx_roc.ssrc == 0) {
        ...
        if (status == srtp_err_status_ok) {
            ...
            PJ_LOG(
              4, (THIS_FILE, "Setting RX ROC %s", get_libsrtp_errstr(status)));
        }
    }
#    endif
...
#    if PJMEDIA_SRTP_CHECK_RTP_SEQ_ON_RESTART
    if (srtp->probation_cnt > 0 && (err == srtp_err_status_replay_old ||
                                    err == srtp_err_status_replay_fail))
    {
        ...
        rx = srtp->rx_policy;
        status =
          pjmedia_transport_srtp_start((pjmedia_transport*)srtp, &tx, &rx);
        if (status != PJ_SUCCESS) {
...
#    if PJMEDIA_SRTP_CHECK_ROC_ON_RESTART
          ...
          srtp->setting.prev_rx_roc.roc != srtp->setting.rx_roc.roc)
    {
        ...
        new_roc =
          (roc == srtp->setting.rx_roc.roc ? srtp->setting.prev_rx_roc.roc :
                                             srtp->setting.rx_roc.roc);
...
#    endif
    ...
    if (err != srtp_err_status_ok) {
        PJ_LOG(5, (srtp->pool->obj_name,
                   "Failed to unprotect SRTP, pkt size=%d, err=%s", size,
                   get_libsrtp_errstr(err)));
    } else {
        cb = srtp->rtp_cb;
        cb2 = srtp->rtp_cb2;
...
static void srtp_rtcp_cb(void* user_data, void* pkt, pj_ssize_t size)
{
    ...
    if (err != srtp_err_status_ok) {
        PJ_LOG(5, (srtp->pool->obj_name,
                   "Failed to unprotect SRTCP, pkt size=%d, err=%s", size,
                   get_libsrtp_errstr(err)));
    } else {
        cb = srtp->rtcp_cb;
        cb_data = srtp->user_data;
                                          ...
                                          unsigned media_index)
{
    ...
    for (i = 0; i < srtp->keying_cnt;) {
        pj_status_t st;
        st = pjmedia_transport_media_create(srtp->keying[i], sdp_pool, options,
                                        ...
                                        unsigned media_index)
{
    ...
    status = pjmedia_transport_encode_sdp(srtp->member_tp, sdp_pool, sdp_local,
                                          sdp_remote, media_index);
    if (status != PJ_SUCCESS)
        return status;
    ...
    for (i = 0; i < srtp->keying_cnt;) {
        pj_status_t st;
        st = pjmedia_transport_encode_sdp(srtp->keying[i], sdp_pool, sdp_local,
                                          sdp_remote, media_index);
        if (st != PJ_SUCCESS) {
            /* This keying method returns error, remove it */
            pj_array_erase(srtp->keying, sizeof(srtp->keying[0]),
                                         ...
                                         unsigned media_index)
{
    ...
    status = pjmedia_transport_media_start(srtp->member_tp, pool, sdp_local,
                                           sdp_remote, media_index);
    if (status != PJ_SUCCESS)
        return status;
     ...
     */
    for (i = 0; i < srtp->keying_cnt;) {
        status = pjmedia_transport_media_start(srtp->keying[i], pool, sdp_local,
                                               sdp_remote, media_index);
        if (status != PJ_SUCCESS) {
            /* This keying method returns error, remove it */
            pj_array_erase(srtp->keying, sizeof(srtp->keying[0]),
...
/* Utility */
PJ_DEF(pj_status_t)
pjmedia_transport_srtp_decrypt_pkt(pjmedia_transport* tp, pj_bool_t is_rtp,
                                   void* pkt, int* pkt_len)
{
    transport_srtp* srtp = (transport_srtp*)tp;
    srtp_err_status_t err;
    ...
    if (err != srtp_err_status_ok) {
        PJ_LOG(5, (srtp->pool->obj_name,
                   "Failed to unprotect SRTP, pkt size=%d, err=%s", *pkt_len,
                   get_libsrtp_errstr(err)));
    }
