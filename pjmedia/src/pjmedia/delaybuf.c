                         ...
                         pjmedia_delay_buf** p_b)
{
    ...
    PJ_ASSERT_RETURN(
      pool && samples_per_frame && clock_rate && channel_count && p_b,
      PJ_EINVAL);
...
static void shrink_buffer(pjmedia_delay_buf* b, unsigned erase_cnt)
{
    ...
    pjmedia_circ_buf_get_read_regions(b->circ_buf, &buf1, &buf1len, &buf2,
                                      &buf2len);
    status =
      pjmedia_wsola_discard(b->wsola, buf1, buf1len, buf2, buf2len, &erase_cnt);
    ...
    if ((status == PJ_SUCCESS) && (erase_cnt > 0)) {
        ...
        pjmedia_circ_buf_set_len(
          b->circ_buf, pjmedia_circ_buf_get_len(b->circ_buf) - erase_cnt);

        PJ_LOG(5, (b->obj_name, "%d samples reduced, buf_cnt=%d", erase_cnt,
...
static void update(pjmedia_delay_buf* b, enum OP op)
{
    ...
    if (b->recalc_timer <= 0) {
        unsigned new_eff_cnt =
          (b->max_level + SAFE_MARGIN) * b->samples_per_frame;
                          ...
                          b->samples_per_frame + b->eff_cnt)
    {
        ...
        PJ_LOG(
          4, (b->obj_name, "Buffer size adjusted from %d to %d (eff_cnt=%d)",
              old_buf_cnt, pjmedia_circ_buf_get_len(b->circ_buf), b->eff_cnt));
    }
}
