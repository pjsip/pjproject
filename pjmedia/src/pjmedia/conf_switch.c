    ...
    if (frm_src->type == PJMEDIA_FRAME_TYPE_EXTENDED) {
        ...
        for (i = 0; i < f_src->subframe_cnt; ++i) {
            ...
            pjmedia_frame_ext_append_subframe(
              f_dst, sf->data, sf->bitlen,
              f_src->samples_cnt / f_src->subframe_cnt);
        ...
        while (f_start < f_end) {
            ...
            pjmedia_copy_samples(
              (pj_int16_t*)frm_dst->buf + (frm_dst->size >> 1), f_start,
              nsamples_to_copy);
            ...
            } else if ((frm_dst->size >> 1) == cport_dst->samples_per_frame) {
        ...
        if (cport_dst->port &&
            ...
            if (frm_dst->size != 0) {
                pjmedia_zero_samples(
                  (pj_int16_t*)frm_dst->buf,
                  cport_dst->samples_per_frame - (frm_dst->size >> 1));
            ...
            if (f_dst->samples_cnt != 0) {
                ...
                pjmedia_frame_ext_append_subframe(
                  f_dst, NULL, 0,
                  (pj_uint16_t)(cport_dst->samples_per_frame -
                                f_dst->samples_cnt));
                if (cport_dst->slot) {
                    pjmedia_port_put_frame(cport_dst->port, frm_dst);
        ...
        /* Synchronize clock. */
        while (pj_cmp_timestamp(&cport_dst->ts_clock, &cport_dst->ts_tx) > 0) {
...
static pj_status_t get_frame(pjmedia_port* this_port, pjmedia_frame* frame)
{
    ...
    for (i = 1, ci = 1; i < conf->max_ports && ci < conf->port_cnt; ++i) {
        ...
        while (pj_cmp_timestamp(&cport->ts_clock, &cport->ts_rx) > 0) {
            ...
            for (j = 0; j < cport->listener_cnt; ++j) {
                 ...
                 */
                listener->tx_level =
                  (cport->rx_level * listener->tx_adj_level) >> 8;
            }
        }
    }
    ...
    for (i = 1, ci = 1; i < conf->max_ports && ci < conf->port_cnt; ++i) {
        ...
        if (cport->tx_setting == PJMEDIA_PORT_MUTE ||
        ...
        if (f_src->type == PJMEDIA_FRAME_TYPE_EXTENDED) {
            ...
            if (f_src->size)
                pjmedia_move_samples(
                  (pj_int16_t*)f_src->buf,
                  (pj_int16_t*)f_src->buf + this_cport->samples_per_frame,
                  f_src->size >> 1);
 ...
 * Recorder callback.
 */
static pj_status_t put_frame(pjmedia_port* this_port, pjmedia_frame* f)
{
    pjmedia_conf* conf = (pjmedia_conf*)this_port->port_data.pdata;
    ...
    if (f->type == PJMEDIA_FRAME_TYPE_AUDIO) {
        if (cport->rx_adj_level != NORMAL_LEVEL) {
            ...
            level =
              pjmedia_calc_avg_signal((const pj_int16_t*)f->buf, f->size >> 1);
        }
    } else if (f->type == PJMEDIA_FRAME_TYPE_EXTENDED) {
        /* For extended frame, level is unknown, so we just set
         * it to NORMAL_LEVEL.
    ...
    for (j = 0; j < cport->listener_cnt; ++j) {
        ...
        /* Skip if this listener doesn't want to receive audio */
        if (listener->tx_setting == PJMEDIA_PORT_DISABLE) {
            pj_add_timestamp32(&listener->ts_tx, listener->samples_per_frame);
            listener->tx_level = 0;
            continue;
        }
