 ...
 * Codec operations
 */
static pj_status_t vpx_codec_init(pjmedia_vid_codec* codec, pj_pool_t* pool);
...
static pj_status_t vpx_codec_encode_more(pjmedia_vid_codec* codec,
                                         ...
                                         pjmedia_frame* output,
                                         pj_bool_t* has_more);
static pj_status_t vpx_codec_decode_(pjmedia_vid_codec* codec, pj_size_t count,
                                     pjmedia_frame packets[], unsigned out_size,
                                     pjmedia_frame* output);

/* Definition for VPX codecs operations. */
static pjmedia_vid_codec_op vpx_codec_op = {
    &vpx_codec_init,        &vpx_codec_open,      &vpx_codec_close,
    &vpx_codec_modify,      &vpx_codec_get_param, &vpx_codec_encode_begin,
    &vpx_codec_encode_more, &vpx_codec_decode_,   NULL
};

/* Definition for VPX codecs factory operations. */
static pjmedia_vid_codec_factory_op vpx_factory_op = {
    &vpx_test_alloc, &vpx_default_attr, &vpx_enum_info, &vpx_alloc_codec,
    &vpx_dealloc_codec
};
...
PJ_DEF(pj_status_t)
pjmedia_codec_vpx_vid_init(pjmedia_vid_codec_mgr* mgr, pj_pool_factory* pf)
{
    pj_status_t status;
                                  ...
                                  pjmedia_vid_codec_param* codec_param)
{
    ...
    vpx_data = (vpx_codec_data*)codec->codec_data;
    vpx_data->prm = pjmedia_vid_codec_param_clone(vpx_data->pool, codec_param);
    param = vpx_data->prm;
    ...
    vpx_data->dec_buf =
      (pj_uint8_t*)pj_pool_alloc(vpx_data->pool, vpx_data->dec_buf_size);
    ...
    status =
      pjmedia_vpx_packetizer_create(vpx_data->pool, &pktz_cfg, &vpx_data->pktz);
    if (status != PJ_SUCCESS)
        return status;
                                          ...
                                          pj_bool_t* has_more)
{
    ...
    vpx_img_wrap(&img, VPX_IMG_FMT_I420, vpx_data->prm->enc_fmt.det.vid.size.w,
                 vpx_data->prm->enc_fmt.det.vid.size.h, 1,
    ...
    res = vpx_codec_encode(&vpx_data->enc, &img, vpx_data->ets.u64, 1, flags,
                           VPX_DL_REALTIME);
    if (res) {
                                         ...
                                         pj_bool_t* has_more)
{
    ...
    if (vpx_data->enc_processed < vpx_data->enc_frame_size) {
        ...
        status = pjmedia_vpx_packetize(
          vpx_data->pktz, vpx_data->enc_frame_size, &vpx_data->enc_processed,
          vpx_data->enc_frame_is_keyframe, &p, &payload_len);
                                     ...
                                     pjmedia_frame* output)
{
    ...
    /* Decode */
    res = vpx_codec_decode(&vpx_data->dec, vpx_data->dec_buf, whole_len, 0,
                           VPX_DL_REALTIME);
    if (res) {
        ...
        img->d_h != vpx_data->prm->dec_fmt.det.vid.size.h)
    {
        ...
        PJ_LOG(4, (THIS_FILE, "Frame size changed: %dx%d --> %dx%d",
                   ...
                   vpx_data->prm->dec_fmt.det.vid.size.h, img->d_w, img->d_h));
