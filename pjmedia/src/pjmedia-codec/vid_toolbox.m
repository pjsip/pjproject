...
#include <pj/math.h>
  ...
  PJMEDIA_HAS_VID_TOOLBOX_CODEC != 0 && defined(PJMEDIA_HAS_VIDEO) && \
  (PJMEDIA_HAS_VIDEO != 0)
 ...
 * Codec operations
 */
static pj_status_t vtool_codec_init(pjmedia_vid_codec* codec, pj_pool_t* pool);
...
static pj_status_t vtool_codec_encode_more(pjmedia_vid_codec* codec,
                                           ...
                                           pjmedia_frame* output,
                                           pj_bool_t* has_more);
static pj_status_t vtool_codec_decode(pjmedia_vid_codec* codec, pj_size_t count,
...
static pjmedia_vid_codec_op vtool_codec_op = {
    &vtool_codec_init,        &vtool_codec_open,      &vtool_codec_close,
    &vtool_codec_modify,      &vtool_codec_get_param, &vtool_codec_encode_begin,
    &vtool_codec_encode_more, &vtool_codec_decode,    NULL
};

/* Definition for Video Toolbox codecs factory operations. */
static pjmedia_vid_codec_factory_op vtool_factory_op = {
    &vtool_test_alloc, &vtool_default_attr, &vtool_enum_info,
    &vtool_alloc_codec, &vtool_dealloc_codec
};
...
PJ_DEF(pj_status_t)
pjmedia_codec_vid_toolbox_init(pjmedia_vid_codec_mgr* mgr, pj_pool_factory* pf)
{
    ...
    status = pjmedia_vid_codec_mgr_register_factory(mgr, &vtool_factory.base);
    if (status != PJ_SUCCESS)
        goto on_error;
                                    ...
                                    const pjmedia_vid_codec_info* info)
{
    ...
    if (info->fmt_id == PJMEDIA_FORMAT_H264 && info->pt == VT_H264_PT) {
        return PJ_SUCCESS;
    }
...
static void encode_cb(void* outputCallbackRefCon, void* sourceFrameRefCon,
                      ...
                      CMSampleBufferRef sampleBuffer)
{
    ...
    if (array)
        dict = (CFDictionaryRef)CFArrayGetValueAtIndex(array, 0);
...
static OSStatus create_encoder(vtool_codec_data* vtool_data)
{
    ...
    ret = VTCompressionSessionCreate(NULL, (int)param->enc_fmt.det.vid.size.w,
                                     ...
                                     encode_cb, vtool_data, &vtool_data->enc);
    if (ret != noErr) {
...
#    define SET_PROPERTY(sess, prop, val) \
        { \
            ...
            if (ret != noErr) \
                PJ_LOG( \
                  5, (THIS_FILE, "Failed to set session property %s", #prop)); \
        }

    SET_PROPERTY(vtool_data->enc, kVTCompressionPropertyKey_ProfileLevel,
    ...
    SET_PROPERTY(vtool_data->enc,
                 ...
                 kCFBooleanFalse);
    SET_PROPERTY(vtool_data->enc, kVTCompressionPropertyKey_AverageBitRate,
                 (__bridge CFTypeRef) @(param->enc_fmt.det.vid.avg_bps));
    vtool_data->enc_fps =
      param->enc_fmt.det.vid.fps.num / param->enc_fmt.det.vid.fps.denum;
    SET_PROPERTY(vtool_data->enc, kVTCompressionPropertyKey_ExpectedFrameRate,
                 (__bridge CFTypeRef) @(vtool_data->enc_fps));
    SET_PROPERTY(vtool_data->enc, kVTCompressionPropertyKey_DataRateLimits,
                 ((__bridge CFArrayRef) // [Bytes, second]
                  @[ @(param->enc_fmt.det.vid.max_bps >> 3), @(1) ]));
    SET_PROPERTY(vtool_data->enc, kVTCompressionPropertyKey_MaxKeyFrameInterval,
                 (__bridge CFTypeRef)
                   @(KEYFRAME_INTERVAL * param->enc_fmt.det.vid.fps.num /
                                    ...
                                    pjmedia_vid_codec_param* codec_param)
{
    ...
    vtool_data = (vtool_codec_data*)codec->codec_data;
    vtool_data->prm =
      pjmedia_vid_codec_param_clone(vtool_data->pool, codec_param);
    ...
    if (1) {
        ...
        if (!vtool_data->whole) {
            ...
            vtool_data->enc_buf =
              pj_pool_alloc(vtool_data->pool, vtool_data->enc_buf_size);
        }
    }
    ...
    if (h264_fmtp.sprop_param_sets_len) {
        ...
        if (i >= code_size) {
            ...
            vtool_data->dec_pps_size =
              h264_fmtp.sprop_param_sets_len - code_size - i;
    ...
    vtool_data->dec_buf =
      (pj_uint8_t*)pj_pool_alloc(vtool_data->pool, vtool_data->dec_buf_size);
...
#    if TARGET_OS_IPHONE
    ret = CVPixelBufferCreate(NULL, vtool_data->prm->enc_fmt.det.vid.size.w,
...
#    else
    ret = CVPixelBufferCreateWithPlanarBytes(
      NULL, vtool_data->prm->enc_fmt.det.vid.size.w,
      vtool_data->prm->enc_fmt.det.vid.size.h,
      kCVPixelFormatType_420YpCbCr8Planar, /* I420 */
      NULL, vtool_data->enc_input_size, 3, /* number of planes of I420 */
      base_addr, (size_t*)plane_w, (size_t*)plane_h, (size_t*)plane_bpr, NULL,
      NULL, NULL, &image_buf);
#    endif
    ...
    ret = VTCompressionSessionEncodeFrame(vtool_data->enc, image_buf, ts, dur,
...
#    endif
        ...
        if (ret == noErr) {
            ...
            ret = VTCompressionSessionEncodeFrame(
              vtool_data->enc, image_buf, ts, dur,
              (__bridge CFDictionaryRef)frm_prop, NULL, NULL);
        }
    }
                                           ...
                                           pj_bool_t* has_more)
{
    ...
    status = pjmedia_h264_packetize(
      vtool_data->pktz, (pj_uint8_t*)vtool_data->enc_buf,
      vtool_data->enc_frame_size, &vtool_data->enc_processed, &payload,
      &payload_len);
    if (status != PJ_SUCCESS) {
        /* Reset */
        vtool_data->enc_frame_size = vtool_data->enc_processed = 0;
...
static void decode_cb(void* decompressionOutputRefCon, void* sourceFrameRefCon,
...
static OSStatus create_decoder(struct vtool_codec_data* vtool_data)
{
    uint8_t* param_ptrs[2] = { vtool_data->dec_sps, vtool_data->dec_pps };
    ...
    ret = CMVideoFormatDescriptionCreateFromH264ParameterSets(
      kCFAllocatorDefault, 2, (const uint8_t* const*)param_ptrs, param_sizes,
      code_size, &dec_format);
    if (ret != noErr) {
        PJ_LOG(4, (THIS_FILE,
                                      ...
                                      unsigned out_size, pjmedia_frame* output)
{
    ...
    if (vtool_data->whole) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            status = pjmedia_h264_unpacketize(
              vtool_data->pktz, (pj_uint8_t*)packets[i].buf, packets[i].size,
              vtool_data->dec_buf, vtool_data->dec_buf_size, &whole_len);
            if (status != PJ_SUCCESS) {
        ...
        whole_len <= code_size + 1)
    {
        PJ_LOG(4, (THIS_FILE,
                   ...
                   "size: %d, buffer: %d",
                   whole_len, vtool_data->dec_buf_size));
        status = PJMEDIA_CODEC_EFRMTOOSHORT;
    }
...
#    endif
        ...
        if (nalu_type == 7) {
            ...
            vtool_data->dec_sps_size =
              PJ_MIN(frm_size - code_size, sizeof(vtool_data->dec_sps));
            pj_memcpy(vtool_data->dec_sps, &start[code_size],
                      vtool_data->dec_sps_size);
        } else if (nalu_type == 8) {
            /* NALU type 8 is the PPS parameter NALU */
            vtool_data->dec_pps_size =
              PJ_MIN(frm_size - code_size, sizeof(vtool_data->dec_pps));
                   ...
                   (!decode_whole || (buf_pos + frm_size >= whole_len)))
        {
            ...
            ret = CMBlockBufferCreateWithMemoryBlock(NULL, start, frm_size,
                                                     kCFAllocatorNull, NULL, 0,
                                                     frm_size, 0, &block_buf);
            if (ret == noErr) {
                const size_t sample_size = frm_size;
                ret =
                  CMSampleBufferCreate(kCFAllocatorDefault, block_buf, true,
                                       NULL, NULL, vtool_data->dec_format, 1, 0,
                                       NULL, 1, &sample_size, &sample_buf);
                if (ret != noErr) {
            ...
            if (ret == noErr) {
                ...
                ret = VTDecompressionSessionDecodeFrame(
                  vtool_data->dec, sample_buf, 0, NULL, NULL);
                if (ret == kVTInvalidSessionErr) {
#    if TARGET_OS_IPHONE
                    /* Just return if app is not active, i.e. in the bg. */
                    __block UIApplicationState state;
...
#    endif
                ...
                if ((ret != noErr) || (vtool_data->dec_status != noErr)) {
                    ...
                    OSStatus err_code =
                      (ret != noErr) ? ret : vtool_data->dec_status;

                    PJ_LOG(5, (THIS_FILE,
                               "Failed to decode frame %d of size "
                               "%d %s:%d",
                               nalu_type, frm_size, ret_err, err_code));
                } else {
                    has_frame = PJ_TRUE;
                    output->type = PJMEDIA_FRAME_TYPE_VIDEO;
                    output->timestamp = packets[0].timestamp;
...
on_return:
    if (!has_frame) {
        ...
        PJ_LOG(5, (THIS_FILE,
                   "Decode couldn't produce picture, "
