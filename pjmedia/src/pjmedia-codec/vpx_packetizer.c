 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vpx_packetize(const pjmedia_vpx_packetizer *pktz,
                      pj_size_t bits_len,
                      unsigned *bits_pos,
                      ...
                      pj_size_t *payload_len)
{
    ...
    *payload_len = PJ_MIN(remaining_size, max_size);
    ...
    if (pktz->cfg.fmt_id == PJMEDIA_FORMAT_VP8)
    {
        ...
        bits[1] = 0x80;
        bits[2] = ((pktz->pictureId & 0x7f00) >> 8) | 0x80;
        bits[3] = pktz->pictureId & 0xff;
        /* Set S: Start of VP8 partition. */
        if (*bits_pos == 0)
        {
            bits[0] |= 0x10;
            ((pjmedia_vpx_packetizer *)pktz)->pictureId++;
        }
    }
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vpx_unpacketize(pjmedia_vpx_packetizer *pktz,
