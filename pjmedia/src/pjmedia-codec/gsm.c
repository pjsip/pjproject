...
static pj_status_t gsm_codec_init(pjmedia_codec* codec, pj_pool_t* pool);
...
static pj_status_t gsm_codec_parse(pjmedia_codec* codec, void* pkt,
...
static pjmedia_codec_op gsm_op = {
    &gsm_codec_init,   &gsm_codec_open,   &gsm_codec_close,  &gsm_codec_modify,
    &gsm_codec_parse,  &gsm_codec_encode, &gsm_codec_decode,
#    if !PLC_DISABLED
    &gsm_codec_recover
#    else
    NULL
...
static pjmedia_codec_factory_op gsm_factory_op = {
    &gsm_test_alloc,  &gsm_default_attr,  &gsm_enum_codecs,
    &gsm_alloc_codec, &gsm_dealloc_codec, &pjmedia_codec_gsm_deinit
};
...
PJ_DEF(pj_status_t) pjmedia_codec_gsm_init(pjmedia_endpt* endpt)
{
    ...
    gsm_codec_factory.pool =
      pjmedia_endpt_create_pool(endpt, "gsm", 4000, 4000);
    ...
    status =
      pjmedia_codec_mgr_register_factory(codec_mgr, &gsm_codec_factory.base);
    if (status != PJ_SUCCESS)
        goto on_error;
...
PJ_DEF(pj_status_t) pjmedia_codec_gsm_deinit(void)
{
    ...
    */
    ...
    status =
      pjmedia_codec_mgr_unregister_factory(codec_mgr, &gsm_codec_factory.base);
...
#    if !PLC_DISABLED
        /* Create PLC */
        status = pjmedia_plc_create(gsm_codec_factory.pool, 8000, 160, 0,
                                    &gsm_data->plc);
        if (status != PJ_SUCCESS) {
            pj_mutex_unlock(gsm_codec_factory.mutex);
            return status;
...
#    endif
        ...
        status = pjmedia_silence_det_create(gsm_codec_factory.pool, 8000, 160,
                                            &gsm_data->vad);
        if (status != PJ_SUCCESS) {
            pj_mutex_unlock(gsm_codec_factory.mutex);
            return status;
                                    ...
                                    struct pjmedia_frame* output)
{
    ...
    gsm_decode(gsm_data->decoder, (unsigned char*)input->buf,
               (short*)output->buf);
