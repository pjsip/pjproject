 ...
 */
  ...
  PJMEDIA_HAS_FFMPEG_VID_CODEC != 0 && defined(PJMEDIA_HAS_VIDEO) && \
  (PJMEDIA_HAS_VIDEO != 0)
...
#        endif
#        define AV_OPT_SET(obj, name, val, opt) \
            (av_opt_set(obj, name, val, opt) == 0)
#        define AV_OPT_SET_INT(obj, name, val) \
            (av_opt_set_int(obj, name, val, 0) == 0)
#    else
#        define AVCODEC_HAS_ENCODE(c) (c->encode)
#        define AV_OPT_SET(obj, name, val, opt) \
            (av_set_string3(obj, name, val, opt, NULL) == 0)
#        define AV_OPT_SET_INT(obj, name, val) \
            (av_set_int(obj, name, val) != NULL)
#    endif
...
static pj_status_t ffmpeg_codec_init(pjmedia_vid_codec* codec, pj_pool_t* pool);
...
static pjmedia_vid_codec_op ffmpeg_op = { &ffmpeg_codec_init,
                                          &ffmpeg_codec_open,
                                          &ffmpeg_codec_close,
                                          &ffmpeg_codec_modify,
...
static pjmedia_vid_codec_factory_op ffmpeg_factory_op = {
    &ffmpeg_test_alloc, &ffmpeg_default_attr, &ffmpeg_enum_codecs,
    &ffmpeg_alloc_codec, &ffmpeg_dealloc_codec
};
...
#    define FUNC_FMT_MATCH(name) \
        pj_status_t(name)(pj_pool_t * pool, pjmedia_sdp_media * offer, \
...
#        error \
          "Must use libavcodec version 53.20 or later to enable FFMPEG H264"
#    endif
...
static pj_status_t vpx_preopen(ffmpeg_private* ff)
{
    ...
    if (!ff->param.ignore_fmtp) {
        ...
        status =
          pjmedia_vid_codec_vpx_parse_fmtp(&ff->param.dec_fmtp, &vpx_fmtp);
...
static FUNC_UNPACKETIZE(vpx_unpacketize)
{
    ...
    status =
      pjmedia_vpx_unpacketize(data->pktz, payload, payload_len, &desc_len);
    if (status != PJ_SUCCESS)
        return status;
...
static pj_status_t h264_preopen(ffmpeg_private* ff)
{
    ...
    /* Parse remote fmtp */
    status =
      pjmedia_vid_codec_h264_parse_fmtp(&ff->param.enc_fmtp, &data->fmtp);
    if (status != PJ_SUCCESS)
        return status;
...
#        endif
    ...
    if (ff->param.dir & PJMEDIA_DIR_ENCODING) {
        ...
        vfd =
          pjmedia_format_get_video_format_detail(&ff->param.enc_fmt, PJ_TRUE);
        ...
        if (profile && !AV_OPT_SET(ctx->priv_data, "profile", profile, 0)) {
            PJ_LOG(3,
                   (THIS_FILE, "Failed to set H264 profile to '%s'", profile));
        }
...
#        endif
        ...
        if (!AV_OPT_SET_INT(ctx->priv_data, "slice-max-size",
                            ff->param.enc_mtu)) {
...
{
    h264_data* data = (h264_data*)ff->data;
    return pjmedia_h264_unpacketize(data->pktz, payload, payload_len, bits,
                                    bits_len, bits_pos);
}
...
{
    h263_data* data = (h263_data*)ff->data;
    return pjmedia_h263_unpacketize(data->pktz, payload, payload_len, bits,
                                    bits_len, bits_pos);
}
  ...
  const pjmedia_vid_codec_info* info)
{
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(codec_desc); ++i) {
        ...
        if (desc->enabled && (desc->info.fmt_id == info->fmt_id) &&
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_codec_ffmpeg_vid_init(pjmedia_vid_codec_mgr* mgr, pj_pool_factory* pf)
{
    pj_pool_t* pool;
    AVCodec* c;
    pj_status_t status;
...
#    endif
        ...
        if (c->pix_fmts && AVCODEC_HAS_ENCODE(c)) {
            ...
            if (raw_fmt_cnt < raw_fmt_cnt_should_be) {
                PJ_LOG(6, (THIS_FILE,
                           "Codec [%s/%s] have %d raw formats, "
                           "recognized only %d raw formats",
                           c->name, c->long_name, raw_fmt_cnt_should_be,
                           raw_fmt_cnt));
            }
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(codec_desc); ++i) {
        ...
        if (desc->base_fmt_id && (!desc->dec || !desc->enc)) {
            ...
            if (!desc->info.dec_fmt_id_cnt) {
                ...
                pj_memcpy(
                  desc->info.dec_fmt_id, base_desc->info.dec_fmt_id,
            ...
            if (copied_dir != PJMEDIA_DIR_NONE) {
                const char* dir_name[] = { NULL, "encoder", "decoder",
                                           "codec" };
                PJ_LOG(5, (THIS_FILE, "The %.*s %s is using base codec (%.*s)",
                           desc->info.encoding_name.slen,
                           desc->info.encoding_name.ptr, dir_name[copied_dir],
        ...
        if (desc->sdp_fmt_match) {
            status = pjmedia_sdp_neg_register_fmt_match_cb(
              &desc->info.encoding_name, desc->sdp_fmt_match);
        ...
        if (!desc->enc) {
            PJ_LOG(
              4, (THIS_FILE, "Cannot find %.*s encoder in ffmpeg library",
                  desc->info.encoding_name.slen, desc->info.encoding_name.ptr));
        }
        if (!desc->dec) {
            PJ_LOG(
              4, (THIS_FILE, "Cannot find %.*s decoder in ffmpeg library",
                  desc->info.encoding_name.slen, desc->info.encoding_name.ptr));
        }
    }

    /* Register codec factory to codec manager. */
    status = pjmedia_vid_codec_mgr_register_factory(mgr, &ffmpeg_factory.base);
    if (status != PJ_SUCCESS)
        goto on_error;
                                       ...
                                       pjmedia_vid_codec_param* attr)
{
    ...
    /* Encoded format */
    pjmedia_format_init_video(&attr->enc_fmt, desc->info.fmt_id, desc->size.w,
                              desc->size.h, desc->fps.num, desc->fps.denum);

    /* Decoded format */
    pjmedia_format_init_video(&attr->dec_fmt, desc->info.dec_fmt_id[0],
                              desc->size.w, desc->size.h, desc->fps.num,
                              desc->fps.denum);
...
static pj_status_t open_ffmpeg_codec(ffmpeg_private* ff, pj_mutex_t* ff_mutex)
{
    ...
    status = pjmedia_format_id_to_PixelFormat(ff->param.dec_fmt.id, &pix_fmt);
    if (status != PJ_SUCCESS)
        return status;
                                     ...
                                     pjmedia_vid_codec_param* attr)
{
    ...
    if (!ff->whole) {
        ...
        ff->dec_buf_size =
          PJ_MAX((unsigned)ff->dec_vafp.framebytes, ff->dec_buf_size);
...
static pj_status_t ffmpeg_packetize(pjmedia_vid_codec* codec, pj_uint8_t* bits,
                                    pj_size_t bits_len, unsigned* bits_pos,
                                    pj_uint8_t* payload, pj_size_t* payload_len,
                                    pj_bool_t is_keyframe)
{
    ...
    if (ff->desc->packetize) {
        return (*ff->desc->packetize)(ff, bits, bits_len, bits_pos, payload,
                                      payload_len, is_keyframe);
    }
                                      ...
                                      pj_size_t bits_len, unsigned* bits_pos)
{
    ...
    if (ff->desc->unpacketize) {
        return (*ff->desc->unpacketize)(ff, payload, payload_len, bits,
                                        bits_len, bits_pos);
    }
...
#    else
    ...
    err =
      avcodec_encode_video(ff->enc_ctx, avpacket.data, avpacket.size, &avframe);
#    endif
                                             ...
                                             pj_bool_t* has_more)
{
    ...
    *has_more = PJ_FALSE;
    ...
    if (ff->whole) {
        ...
        status = ffmpeg_codec_encode_whole(
          codec, opt, input, (unsigned)whole_frm.size, &whole_frm);
        if (status != PJ_SUCCESS)
            return status;

        ff->enc_buf_is_keyframe =
          (whole_frm.bit_info & PJMEDIA_VID_FRM_KEYFRAME);
                                            ...
                                            pj_bool_t* has_more)
{
    ...
    *has_more = PJ_FALSE;
    ...
    status = ffmpeg_packetize(codec, (pj_uint8_t*)ff->enc_buf,
                              ff->enc_frame_len, &ff->enc_processed, payload,
                              &payload_len, ff->enc_buf_is_keyframe);
    if (status != PJ_SUCCESS)
        return status;
                                       ...
                                       pj_bool_t got_keyframe)
{
        ...
        ff->dec_ctx->height != (int)vafp->size.h)
    {
        ...
        status =
          PixelFormat_to_pjmedia_format_id(ff->dec_ctx->pix_fmt, &new_fmt_id);
        if (status != PJ_SUCCESS)
            return status;
...
#    else
    err = avcodec_decode_video(ff->dec_ctx, &avframe, &got_picture,
...
#    endif
    if (err < 0) {
         ...
         */
        status =
          check_decode_result(codec, &input->timestamp, avframe.key_frame);
        if (status != PJ_SUCCESS)
            return status;
                                       ...
                                       unsigned out_size, pjmedia_frame* output)
{
    ...
    if (ff->whole) {
        ...
        for (i = 0; i < pkt_count; ++i) {
            ...
            status =
              ffmpeg_unpacketize(codec, packets[i].buf, packets[i].size,
                                 ff->dec_buf, ff->dec_buf_size, &whole_len);
            if (status != PJ_SUCCESS) {
