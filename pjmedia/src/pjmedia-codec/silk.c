...
#    define CALC_BITRATE_QUALITY(quality, max_br) (quality * max_br / 10)
...
static pj_status_t silk_codec_init(pjmedia_codec* codec, pj_pool_t* pool);
...
static pj_status_t silk_codec_parse(pjmedia_codec* codec, void* pkt,
                                    pj_size_t pkt_size,
...
static pjmedia_codec_op silk_op = { &silk_codec_init,   &silk_codec_open,
                                    &silk_codec_close,  &silk_codec_modify,
                                    &silk_codec_parse,  &silk_codec_encode,
                                    &silk_codec_decode, &silk_codec_recover };

/* Definition for SILK codec factory operations. */
static pjmedia_codec_factory_op silk_factory_op = {
    &silk_test_alloc,  &silk_default_attr,  &silk_enum_codecs,
    &silk_alloc_codec, &silk_dealloc_codec, &pjmedia_codec_silk_deinit
};
...
PJ_DEF(pj_status_t) pjmedia_codec_silk_init(pjmedia_endpt* endpt)
{
    ...
    /* Create mutex. */
    status =
      pj_mutex_create_simple(silk_factory.pool, "silk", &silk_factory.mutex);
    ...
    status = pjmedia_codec_mgr_register_factory(codec_mgr, &silk_factory.base);
    if (status != PJ_SUCCESS)
        return status;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_codec_silk_set_config(unsigned clock_rate,
                              const pjmedia_codec_silk_setting* opt)
{
    ...
    for (i = 0; i < sizeof(silk_factory.silk_param) /
                      sizeof(silk_factory.silk_param[0]);
         ++i)
    {
        if (silk_factory.silk_param[i].clock_rate == clock_rate) {
            ...
            if (opt->quality >= 0)
                quality = opt->quality;
            silk_factory.silk_param[i].bitrate = CALC_BITRATE_QUALITY(
              quality, silk_factory.silk_param[i].max_bitrate);
...
PJ_DEF(pj_status_t) pjmedia_codec_silk_deinit(void)
{
    ...
    status =
      pjmedia_codec_mgr_unregister_factory(codec_mgr, &silk_factory.base);
    silk_factory.endpt = NULL;
                                   ...
                                   pjmedia_codec_param* attr)
{
    ...
    if (err) {
        PJ_LOG(3,
               (THIS_FILE, "Failed to get encoder state size (err=%d)", err));
    ...
    silk->samples_per_frame = FRAME_LENGTH_MS * attr->info.clock_rate / 1000;
    ...
    silk->enc_ctl.packetSize =
      attr->setting.frm_per_pkt * silk->samples_per_frame;
    ...
    if (err) {
        PJ_LOG(3,
               (THIS_FILE, "Failed to get decoder state size (err=%d)", err));
        return PJMEDIA_CODEC_EFAILED;
    }
    ...
    silk->dec_buf_sz = attr->info.clock_rate * attr->info.channel_cnt *
                       attr->info.frm_ptime / 1000 * silk->pcm_bytes_per_sample;
    ...
    attr->info.max_rx_frame_size =
      attr->info.max_bps * attr->info.frm_ptime / 8 / 1000;
                                     ...
                                     struct pjmedia_frame* output)
{
    ...
    err =
      SKP_Silk_SDK_Encode(silk->enc_st, &silk->enc_ctl, (SKP_int16*)input->buf,
                          nsamples, (SKP_uint8*)output->buf, &out_size);
    if (err) {
    ...
    if (toc.framesInPacket == 0) {
        ...
        if (toc.framesInPacket - 1 > (SKP_int)silk->dec_buf_cnt) {
            ...
            for (i = silk->dec_buf_cnt + 1; i < (unsigned)toc.framesInPacket;
                 i++) {
                silk->dec_buf[i - 1] =
                  pj_pool_alloc(silk->pool, silk->dec_buf_sz);
            }
                                      ...
                                      struct pjmedia_frame* output)
{
    ...
    err =
      SKP_Silk_SDK_Decode(silk->dec_st, &silk->dec_ctl, 1, /* Lost frame flag */
                          NULL, 0, output->buf, &out_size);
    if (err) {
