...
static pj_status_t and_media_codec_parse(pjmedia_codec* codec, void* pkt,
                                         pj_size_t pkt_size,
...
static pjmedia_codec_op and_media_op = {
    &and_media_codec_init,   &and_media_codec_open,   &and_media_codec_close,
    &and_media_codec_modify, &and_media_codec_parse,  &and_media_codec_encode,
    &and_media_codec_decode, &and_media_codec_recover
};

/* Definition for Android MediaCodec codecs factory operations. */
static pjmedia_codec_factory_op and_media_factory_op = {
    &and_media_test_alloc,    &and_media_default_attr,
    &and_media_enum_codecs,   &and_media_alloc_codec,
    &and_media_dealloc_codec, &pjmedia_codec_and_media_aud_deinit
};
                            ...
                            pj_size_t max_pkt_size)
{
    enum
    {
        MAX_FRAMES_PER_PACKET = PJMEDIA_MAX_FRAME_DURATION_MS / 20
    };
    ...
    for (i = 0; i < nframes; ++i) {
        pjmedia_codec_amr_bit_info* info =
          (pjmedia_codec_amr_bit_info*)&frames[i].bit_info;
        ...
        if (setting->amr_nb) {
            ...
            frames[i].size = (info->frame_type <= 9) ?
                               pjmedia_codec_amrwb_framelen[info->frame_type] :
                               0;
        }
        p += frames[i].size + 1;
    }
                                   ...
                                   pj_bool_t is_encoder)
{
    ...
    AMediaCodec* codec =
      (is_encoder ? and_media_data->enc : and_media_data->dec);
    ...
    PJ_LOG(4, (THIS_FILE, "%s [0x%x] started",
               is_encoder ? "Encoder" : "Decoder", codec));
    return PJ_SUCCESS;
}
...
PJ_DEF(pj_status_t) pjmedia_codec_and_media_aud_init(pjmedia_endpt* endpt)
{
    ...
    and_media_factory.pool =
      pjmedia_endpt_create_pool(endpt, "Android MediaCodec codecs", 4000, 4000);
...
#    if PJMEDIA_HAS_AND_MEDIA_AMRNB
    ...
    status = pjmedia_sdp_neg_register_fmt_match_cb(
      &codec_name, &pjmedia_codec_amr_match_sdp);
    if (status != PJ_SUCCESS)
        goto on_error;
#    endif
...
#    endif
    ...
    status =
      pjmedia_codec_mgr_register_factory(codec_mgr, &and_media_factory.base);
    if (status != PJ_SUCCESS)
        goto on_error;
...
PJ_DEF(pj_status_t) pjmedia_codec_and_media_aud_deinit(void)
{
    ...
    status =
      pjmedia_codec_mgr_unregister_factory(codec_mgr, &and_media_factory.base);
                                          ...
                                          pjmedia_codec_param* attr)
{
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(and_media_codec); ++i) {
            ...
            (id->pt == (unsigned)and_media_codec[i].pt))
        {
            ...
            attr->info.frm_ptime =
              (pj_uint16_t)(and_media_codec[i].samples_per_frame * 1000 /
                                         ...
                                         pjmedia_codec** p_codec)
{
    ...
    if (!and_media_codec[idx].has_native_vad) {
        ...
        status = pjmedia_silence_det_create(
          pool, and_media_codec[idx].clock_rate,
          and_media_codec[idx].samples_per_frame, &codec_data->vad);
        if (status != PJ_SUCCESS) {
            goto on_error;
        }
    }
...
#    if PJMEDIA_HAS_AND_MEDIA_AMRNB
        ...
        and_media_data->codec_id == AND_AUD_CODEC_AMRWB)
    {
        ...
        for (i = 0; i < attr->setting.dec_fmtp.cnt; ++i) {
            ...
            {
                octet_align =
                  (pj_uint8_t)pj_strtoul(&attr->setting.dec_fmtp.param[i].val);
                break;
            }
        }
        ...
        s->enc_setting.amr_nb =
          (pj_uint8_t)(and_media_data->codec_id == AND_AUD_CODEC_AMRNB);
        s->enc_setting.octet_aligned = octet_align;
        s->enc_setting.reorder = 0;
        s->enc_setting.cmr = 15;
        s->dec_setting.amr_nb =
          (pj_uint8_t)(and_media_data->codec_id == AND_AUD_CODEC_AMRNB);
        ...
        PJ_LOG(4, (THIS_FILE,
                   "Encoder setting octet_aligned=%d reorder=%d"
                   " cmr=%d enc_mode=%d",
                                         ...
                                         pjmedia_frame frames[])
{
    ...
    while (pkt_size >= codec_data->frame_size && count < *frame_cnt) {
        ...
        frames[count].timestamp.u64 =
          ts->u64 + count * and_media_data->samples_per_frame;
                                          ...
                                          struct pjmedia_frame* output)
{
    ...
    while (nsamples >= samples_per_frame) {
        ...
        if (buf_idx >= 0) {
            ...
            PJ_LOG(
              4, (THIS_FILE, "Encoder dequeueInputBuffer failed[%d]", buf_idx));
            goto on_return;
        }

        for (i = 0; i < CODEC_WAIT_RETRY; ++i) {
            buf_idx = AMediaCodec_dequeueOutputBuffer(
              codec_data->enc, &buf_info, CODEC_DEQUEUE_TIMEOUT);
            if (buf_idx == -1) {
        ...
        if (buf_idx < 0) {
            PJ_LOG(
              4, (THIS_FILE, "Encoder dequeueOutputBuffer failed %d", buf_idx));
            goto on_return;
        }

        output_buf =
          AMediaCodec_getOutputBuffer(codec_data->enc, buf_idx, &output_size);
        if (!output_buf) {
            PJ_LOG(4, (THIS_FILE,
                                          ...
                                          struct pjmedia_frame* output)
{
    ...
    buf_idx =
      AMediaCodec_dequeueInputBuffer(codec_data->dec, CODEC_DEQUEUE_TIMEOUT);
    ...
    input_buf =
      AMediaCodec_getInputBuffer(codec_data->dec, buf_idx, &input_size);
    if (input_buf == 0) {
        PJ_LOG(4, (THIS_FILE,
                   "Decoder getInputBuffer failed "
                   "return input_buf=%d, size=%d",
    ...
    am_status = AMediaCodec_queueInputBuffer(
      codec_data->dec, buf_idx, 0, input_.size, input->timestamp.u32.lo, 0);
    if (am_status != AMEDIA_OK) {
    ...
    for (i = 0; i < CODEC_WAIT_RETRY; ++i) {
        buf_idx = AMediaCodec_dequeueOutputBuffer(codec_data->dec, &buf_info,
                                                  CODEC_DEQUEUE_TIMEOUT);
        if (buf_idx == -1) {
            /* Timeout, wait until output buffer is availble. */
            PJ_LOG(
              5, (THIS_FILE, "Decoder dequeueOutputBuffer timeout[%d]", i + 1));
            pj_thread_sleep(CODEC_THREAD_WAIT);
        } else {
            break;
        }
    ...
    output_buf =
      AMediaCodec_getOutputBuffer(codec_data->dec, buf_idx, &output_size);
    if (output_buf == NULL) {
        am_status =
          AMediaCodec_releaseOutputBuffer(codec_data->dec, buf_idx, 0);
        if (am_status != AMEDIA_OK) {
