 ...
 * Codec operations
 */
static pj_status_t oh264_codec_init(pjmedia_vid_codec* codec, pj_pool_t* pool);
...
static pj_status_t oh264_codec_encode_more(pjmedia_vid_codec* codec,
                                           ...
                                           pjmedia_frame* output,
                                           pj_bool_t* has_more);
static pj_status_t oh264_codec_decode(pjmedia_vid_codec* codec, pj_size_t count,
...
static pjmedia_vid_codec_op oh264_codec_op = {
    &oh264_codec_init,        &oh264_codec_open,      &oh264_codec_close,
    &oh264_codec_modify,      &oh264_codec_get_param, &oh264_codec_encode_begin,
    &oh264_codec_encode_more, &oh264_codec_decode,    NULL
};

/* Definition for OpenH264 codecs factory operations. */
static pjmedia_vid_codec_factory_op oh264_factory_op = {
    &oh264_test_alloc, &oh264_default_attr, &oh264_enum_info,
    &oh264_alloc_codec, &oh264_dealloc_codec
};
...
PJ_DEF(pj_status_t)
pjmedia_codec_openh264_vid_init(pjmedia_vid_codec_mgr* mgr, pj_pool_factory* pf)
{
    ...
    status = pjmedia_vid_codec_mgr_register_factory(mgr, &oh264_factory.base);
    if (status != PJ_SUCCESS)
        goto on_error;
                                    ...
                                    const pjmedia_vid_codec_info* info)
{
    ...
    if (info->fmt_id == PJMEDIA_FORMAT_H264 && info->pt == OH264_PT) {
        return PJ_SUCCESS;
    }
                                    ...
                                    pjmedia_vid_codec_param* codec_param)
{
    ...
    oh264_data = (oh264_codec_data*)codec->codec_data;
    oh264_data->prm =
      pjmedia_vid_codec_param_clone(oh264_data->pool, codec_param);
...
#    endif
    ...
    eprm.fMaxFrameRate = (param->enc_fmt.det.vid.fps.num * 1.0f /
                          param->enc_fmt.det.vid.fps.denum);
    eprm.iTemporalLayerNum = 1;
    eprm.uiIntraPeriod = 0; /* I-Frame interval in frames */
    eprm.eSpsPpsIdStrategy = (oh264_data->whole ? CONSTANT_ID : INCREASING_ID);
    ...
    memcpy(&elayer->sSliceArgument, &elayer_ctx.sSliceArgument,
    ...
    oh264_data->esrc_pic->iStride[1] = oh264_data->esrc_pic->iStride[2] =
      oh264_data->esrc_pic->iStride[0] >> 1;

    oh264_data->enc_input_size =
      oh264_data->esrc_pic->iPicWidth * oh264_data->esrc_pic->iPicHeight * 3 >>
      1;
    ...
    if (rc != cmResultSuccess) {
        PJ_LOG(4, (THIS_FILE,
                   "SVC encoder SetOption videoFormatI420 failed, "
                   "rc=%d",
                   rc));
        return PJMEDIA_CODEC_EFAILED;
    }
    ...
    oh264_data->dec_buf =
      (pj_uint8_t*)pj_pool_alloc(oh264_data->pool, oh264_data->dec_buf_size);
                                            ...
                                            pj_bool_t* has_more)
{
    ...
    oh264_data->esrc_pic->pData[1] =
      oh264_data->esrc_pic->pData[0] +
      (oh264_data->esrc_pic->iPicWidth * oh264_data->esrc_pic->iPicHeight);
    oh264_data->esrc_pic->pData[2] =
      oh264_data->esrc_pic->pData[1] +
      (oh264_data->esrc_pic->iPicWidth * oh264_data->esrc_pic->iPicHeight >> 2);
                                           ...
                                           pj_bool_t* has_more)
{
    ...
    if (oh264_data->enc_processed < oh264_data->enc_frame_size) {
        ...
        status = pjmedia_h264_packetize(
          oh264_data->pktz, oh264_data->enc_frame_whole,
          oh264_data->enc_frame_size, &oh264_data->enc_processed, &payload,
          &payload_len);
        if (status != PJ_SUCCESS) {
            /* Reset */
            oh264_data->enc_frame_size = oh264_data->enc_processed = 0;
            *has_more =
              (oh264_data->enc_processed < oh264_data->enc_frame_size) ||
    ...
    status = pjmedia_h264_packetize(
      oh264_data->pktz, oh264_data->enc_frame_whole, oh264_data->enc_frame_size,
      &oh264_data->enc_processed, &payload, &payload_len);
    if (status != PJ_SUCCESS) {
        /* Reset */
        oh264_data->enc_frame_size = oh264_data->enc_processed = 0;
...
static int write_yuv(pj_uint8_t* buf, unsigned dst_len, unsigned char* pData[3],
                     int iStride[2], int iWidth, int iHeight)
{
    unsigned req_size;
    pj_uint8_t* dst = buf;
...
static pj_status_t oh264_got_decoded_frame(
  pjmedia_vid_codec* codec, struct oh264_codec_data* oh264_data,
  unsigned char* pData[3], SBufferInfo* sDstBufInfo, pj_timestamp* timestamp,
  unsigned out_size, pjmedia_frame* output)
{
    ...
    */
    ...
    int len = write_yuv((pj_uint8_t*)output->buf, out_size, pDst, iStride,
                        iWidth, iHeight);
    if (len > 0) {
                                      ...
                                      unsigned out_size, pjmedia_frame* output)
{
    ...
    if (oh264_data->whole) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            status = pjmedia_h264_unpacketize(
              oh264_data->pktz, (pj_uint8_t*)packets[i].buf, packets[i].size,
              oh264_data->dec_buf, oh264_data->dec_buf_size, &whole_len);
            if (status != PJ_SUCCESS) {
    ...
    for (frm_cnt = 0;; ++frm_cnt) {
        ...
        for (i = 0; buf_pos + i < whole_len; i++) {
            if (oh264_data->dec_buf[buf_pos + i] == 0 &&
                ...
                oh264_data->dec_buf[buf_pos + i + 2] == 1 && i > 1)
            {
                break;
            }
        }
        ...
        if (0 && sDstBufInfo.iBufferStatus == 1) {
            ...
            status =
              oh264_got_decoded_frame(codec, oh264_data, pData, &sDstBufInfo,
                                      &packets[0].timestamp, out_size, output);
    ...
    if (sDstBufInfo.iBufferStatus == 1 && !(ret & dsRefLost) &&
        !(ret & dsNoParamSets) && !(ret & dsDepLayerLost))
    {
