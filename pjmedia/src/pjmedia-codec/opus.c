...
static pj_status_t codec_parse(pjmedia_codec* codec, void* pkt,
...
static pj_status_t codec_decode(pjmedia_codec* codec,
                                ...
                                unsigned output_buf_len,
                                struct pjmedia_frame* output);
static pj_status_t codec_recover(pjmedia_codec* codec, unsigned output_buf_len,
                                 struct pjmedia_frame* output);

/* Definition for Opus operations. */
static pjmedia_codec_op opus_op = { &codec_init,   &codec_open,   &codec_close,
                                    &codec_modify, &codec_parse,  &codec_encode,
                                    &codec_decode, &codec_recover };

/* Definition for Opus factory operations. */
static pjmedia_codec_factory_op opus_factory_op = {
    &factory_test_alloc,  &factory_default_attr,  &factory_enum_codecs,
    &factory_alloc_codec, &factory_dealloc_codec, &pjmedia_codec_opus_deinit
};
...
PJ_DEF(pj_status_t) pjmedia_codec_opus_init(pjmedia_endpt* endpt)
{
    ...
    opus_codec_factory.pool =
      pjmedia_endpt_create_pool(endpt, "opus-factory", 1024, 1024);
    ...
    status =
      pjmedia_codec_mgr_register_factory(codec_mgr, &opus_codec_factory.base);
...
PJ_DEF(pj_status_t) pjmedia_codec_opus_deinit(void)
{
    ...
    status =
      pjmedia_codec_mgr_unregister_factory(codec_mgr, &opus_codec_factory.base);
    if (status != PJ_SUCCESS)
                                     ...
                                     pjmedia_codec_param* param)
{
    ...
    status = pjmedia_codec_mgr_find_codecs_by_id(codec_mgr, &opus_str, &count,
                                                 info, NULL);
    if (status != PJ_SUCCESS)
        return status;
...
static pj_status_t codec_open(pjmedia_codec* codec, pjmedia_codec_param* attr)
{
    ...
    if (!opus_data->enc) {
        ...
        opus_data->enc =
          pj_pool_zalloc(opus_data->pool, opus_encoder_get_size(2));
    }
    if (!opus_data->dec) {
        /* Allocate memory for max 2 channels */
        opus_data->dec =
          pj_pool_zalloc(opus_data->pool, opus_decoder_get_size(2));
    }
    if (!opus_data->enc_packer) {
        opus_data->enc_packer =
          pj_pool_zalloc(opus_data->pool, opus_repacketizer_get_size());
    }
    if (!opus_data->dec_packer) {
        opus_data->dec_packer =
          pj_pool_zalloc(opus_data->pool, opus_repacketizer_get_size());
    }
    if (!opus_data->enc || !opus_data->dec || !opus_data->enc_packer ||
        !opus_data->dec_packer)
    {
    ...
    TRACE_((THIS_FILE, "%s:%d: sample_rate: %u", __FUNCTION__, __LINE__,
            opus_data->cfg.sample_rate));

    /* Initialize encoder */
    err = opus_encoder_init(opus_data->enc, opus_data->cfg.sample_rate,
    ...
    opus_encoder_ctl(
      opus_data->enc,
      OPUS_SET_BITRATE(auto_bit_rate ? OPUS_AUTO : attr->info.avg_bps));
    /* Set VAD */
    ...
    opus_encoder_ctl(
      opus_data->enc,
      OPUS_SET_MAX_BANDWIDTH(get_opus_bw_constant(opus_data->cfg.sample_rate)));
    /* Set expected packet loss */
    opus_encoder_ctl(opus_data->enc,
    ...
    PJ_LOG(5, (THIS_FILE,
               "Initialize Opus encoder, sample rate: %d, "
               "avg bitrate: %d, vad: %d, plc: %d, pkt loss: %d, "
               "complexity: %d, constant bit rate: %d",
               opus_data->cfg.sample_rate, attr->info.avg_bps,
               attr->setting.vad ? 1 : 0, attr->setting.plc ? 1 : 0,
               opus_data->cfg.packet_loss, opus_data->cfg.complexity,
               opus_data->cfg.cbr ? 1 : 0));

    /* Initialize decoder */
    err = opus_decoder_init(opus_data->dec, opus_data->cfg.sample_rate,
    ...
    opus_data->dec_frame[0].buf = pj_pool_zalloc(
      opus_data->pool, (opus_data->cfg.sample_rate / 1000) * 60 *
                         attr->info.channel_cnt * 2 /* bytes per sample */);
    opus_data->dec_frame[1].type = PJMEDIA_FRAME_TYPE_NONE;
    opus_data->dec_frame[1].buf = pj_pool_zalloc(
      opus_data->pool, (opus_data->cfg.sample_rate / 1000) * 60 *
                         attr->info.channel_cnt * 2 /* bytes per sample */);
                                ...
                                const pjmedia_codec_param* attr)
{
    ...
    opus_encoder_ctl(
      opus_data->enc,
      OPUS_SET_BITRATE(attr->info.avg_bps ? attr->info.avg_bps : OPUS_AUTO));
    /* Set VAD */
    opus_encoder_ctl(opus_data->enc, OPUS_SET_DTX(attr->setting.vad ? 1 : 0));
    /* Set PLC */
    opus_encoder_ctl(opus_data->enc,
...
#    endif
    ...
    for (i = 0; i < num_frames; ++i) {
        ...
        if (i == 0) {
            ...
            nsamples = opus_packet_get_nb_samples(frames[i].buf, frames[i].size,
                                                  opus_data->cfg.sample_rate);
            if (nsamples <= 0) {
                PJ_LOG(5, (THIS_FILE,
                           "Parse failed to get samples number! "
                           "(err=%d)",
                           nsamples));
                pj_mutex_unlock(opus_data->mutex);
                return PJMEDIA_CODEC_EFAILED;
            ...
            if (ptime != opus_data->dec_ptime) {
                PJ_LOG(4, (THIS_FILE,
                           "Opus ptime change detected: %d ms "
                           "--> %d ms",
                                ...
                                struct pjmedia_frame* output)
{
    ...
    samples_per_frame =
      (opus_data->cfg.sample_rate * opus_data->enc_ptime) / 1000;
    frame_size =
      samples_per_frame * opus_data->cfg.channel_cnt * sizeof(opus_int16);

    opus_repacketizer_init(opus_data->enc_packer);
    while (input->size - in_pos >= frame_size) {
        size = opus_encode(
          opus_data->enc, (const opus_int16*)(((char*)input->buf) + in_pos),
          samples_per_frame, tmp_buf + out_pos,
          (tmp_bytes_left < frame_size ? tmp_bytes_left : frame_size));
        if (size < 0) {
            ...
            opus_repacketizer_cat(opus_data->enc_packer, tmp_buf + out_pos,
                                  size);
            out_pos += size;
            tmp_bytes_left -= size;
        }
    ...
    if (size) {
        size = opus_repacketizer_out(opus_data->enc_packer, output->buf,
                                     output_buf_len);
        if (size < 0) {
            PJ_LOG(4, (THIS_FILE, "Encode failed! (%d), out_size: %u", size,
                       output_buf_len));
            pj_mutex_unlock(opus_data->mutex);
            return PJMEDIA_CODEC_EFAILED;
        }
                                ...
                                struct pjmedia_frame* output)
{
    ...
    decoded_samples = opus_decode(
      opus_data->dec,
      inframe->type == PJMEDIA_FRAME_TYPE_AUDIO ? inframe->buf : NULL,
      inframe->type == PJMEDIA_FRAME_TYPE_AUDIO ? inframe->size : 0,
      (opus_int16*)output->buf, frm_size, fec);
    ...
    output->size =
      decoded_samples * sizeof(opus_int16) * opus_data->cfg.channel_cnt;
    output->type = PJMEDIA_FRAME_TYPE_AUDIO;
                                 ...
                                 struct pjmedia_frame* output)
{
    ...
    if (opus_data->dec_frame_index == -1) {
        ...
        unsigned samples_per_frame;
        samples_per_frame =
          opus_data->cfg.sample_rate * opus_data->dec_ptime / 1000;
        output->type = PJMEDIA_FRAME_TYPE_AUDIO;
    ...
    decoded_samples = opus_decode(
      ...
      inframe->type == PJMEDIA_FRAME_TYPE_AUDIO ? inframe->buf : NULL,
      inframe->type == PJMEDIA_FRAME_TYPE_AUDIO ? inframe->size : 0,
      (opus_int16*)output->buf, frm_size, 0);
