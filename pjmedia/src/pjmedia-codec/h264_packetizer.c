 ...
 */
static pj_uint8_t* find_next_nal_unit(pj_uint8_t* start, pj_uint8_t* end)
{
    pj_uint8_t* p = start;
                               ...
                               pjmedia_h264_packetizer** p)
{
    ...
    if (cfg && cfg->mode != PJMEDIA_H264_PACKETIZER_MODE_NON_INTERLEAVED &&
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_h264_packetize(pjmedia_h264_packetizer* pktz, pj_uint8_t* buf,
                       pj_size_t buf_len, unsigned* pos,
...
#    endif
        ...
        nal_end - nal_start > pktz->cfg.mtu)
    {
        ...
        PJ_LOG(2, (THIS_FILE, "MTU too small for H.264 (required=%u, MTU=%u)",
                   nal_end - nal_start, pktz->cfg.mtu));
        return PJ_ETOOSMALL;
    }
...
#    if DBG_PACKETIZE
        PJ_LOG(3, ("h264pack",
                   "Packetized fragmented H264 NAL unit "
                   "(pos=%d, type=%d, NRI=%d, S=%d, E=%d, len=%d/%d)",
                   *payload - buf, TYPE, NRI, *p >> 7, (*p >> 6) & 1,
                   *payload_len, buf_len));
#    endif
...
#    if DBG_PACKETIZE
    PJ_LOG(3, ("h264pack",
               "Packetized single H264 NAL unit "
                         ...
                         unsigned* bits_pos)
{
    ...
    nal_start_code =
      nal_start + PJ_ARRAY_SIZE(nal_start) - pktz->cfg.unpack_nal_start;
...
#    if DBG_UNPACKETIZE
        PJ_LOG(3, ("h264unpack", "Unpacked %d H264 NAL units (len=%d)", cnt,
                   payload_len));
#    endif
...
#    if DBG_UNPACKETIZE
        PJ_LOG(3, ("h264unpack",
                   "Unpacked fragmented H264 NAL unit "
                   "(type=%d, NRI=%d, len=%d)",
