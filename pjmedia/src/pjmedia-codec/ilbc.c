...
#if defined(PJMEDIA_ILBC_CODEC_USE_COREAUDIO) && \
  PJMEDIA_ILBC_CODEC_USE_COREAUDIO
...
static pj_status_t ilbc_codec_init(pjmedia_codec* codec, pj_pool_t* pool);
...
static pj_status_t ilbc_codec_parse(pjmedia_codec* codec, void* pkt,
...
static pjmedia_codec_op ilbc_op = { &ilbc_codec_init,   &ilbc_codec_open,
                                    &ilbc_codec_close,  &ilbc_codec_modify,
                                    &ilbc_codec_parse,  &ilbc_codec_encode,
                                    &ilbc_codec_decode, &ilbc_codec_recover };

/* Definition for iLBC codec factory operations. */
static pjmedia_codec_factory_op ilbc_factory_op = {
    &ilbc_test_alloc,  &ilbc_default_attr,  &ilbc_enum_codecs,
    &ilbc_alloc_codec, &ilbc_dealloc_codec, &pjmedia_codec_ilbc_deinit
};
 ...
 */
PJ_DEF(pj_status_t) pjmedia_codec_ilbc_init(pjmedia_endpt* endpt, int mode)
{
    ...
    status = pjmedia_codec_mgr_register_factory(codec_mgr, &ilbc_factory.base);
...
PJ_DEF(pj_status_t) pjmedia_codec_ilbc_deinit(void)
{
    ...
    status =
      pjmedia_codec_mgr_unregister_factory(codec_mgr, &ilbc_factory.base);
    ilbc_factory.endpt = NULL;
                                    ...
                                    pjmedia_codec** p_codec)
{
    ...
    pool = pjmedia_endpt_create_pool(ilbc_factory.endpt, "iLBC%p", 2000, 2000);
    ...
    pj_ansi_snprintf(codec->obj_name, sizeof(codec->obj_name), "ilbc%p", codec);
...
#    if defined(PJMEDIA_ILBC_CODEC_USE_COREAUDIO) && \
    ...
    srcFormat.mBytesPerFrame =
      srcFormat.mChannelsPerFrame * srcFormat.mBitsPerChannel >> 3;
    srcFormat.mFramesPerPacket = 1;
    srcFormat.mBytesPerPacket =
      srcFormat.mBytesPerFrame * srcFormat.mFramesPerPacket;
...
#    endif
    ...
    for (i = 0; i < attr->setting.dec_fmtp.cnt; ++i) {
        if (pj_stricmp(&attr->setting.dec_fmtp.param[i].name, &STR_MODE) == 0) {
            dec_fmtp_mode =
              (pj_uint16_t)pj_strtoul(&attr->setting.dec_fmtp.param[i].val);
            break;
        }
    }
    ...
    for (i = 0; i < attr->setting.enc_fmtp.cnt; ++i) {
        if (pj_stricmp(&attr->setting.enc_fmtp.param[i].name, &STR_MODE) == 0) {
            enc_fmtp_mode =
              (pj_uint16_t)pj_strtoul(&attr->setting.enc_fmtp.param[i].val);
            break;
        }
    }
    ...
    if (enc_fmtp_mode != dec_fmtp_mode) {
        ...
        PJ_LOG(
          4, (ilbc_codec->obj_name,
              "Normalized iLBC encoder and decoder modes to %d", DEFAULT_MODE));
    }
...
#    else
    ilbc_codec->dec_samples_per_frame =
      initDecode(&ilbc_codec->dec, dec_fmtp_mode, attr->setting.penh);
#    endif
    ...
    PJ_LOG(4,
           (ilbc_codec->obj_name, "iLBC codec opened, mode=%d", dec_fmtp_mode));
                                    ...
                                    unsigned* frame_cnt, pjmedia_frame frames[])
{
    ...
    while (pkt_size >= ilbc_codec->dec_frame_size && count < *frame_cnt) {
        ...
        frames[count].timestamp.u64 =
          ts->u64 + (pj_uint64_t)count * ilbc_codec->dec_samples_per_frame;
  ...
  AudioStreamPacketDescription** outDataPacketDescription, void* inUserData)
{
    ...
    if (*ioNumberDataPackets) {
        ioData->mBuffers[0].mData =
          ilbc_codec->enc_buffer + ilbc_codec->enc_buffer_offset;
        ioData->mBuffers[0].mDataByteSize =
          *ioNumberDataPackets * ilbc_codec->enc_samples_per_frame << 1;
  ...
  AudioStreamPacketDescription** outDataPacketDescription, void* inUserData)
{
    ...
    if (*ioNumberDataPackets) {
        ioData->mBuffers[0].mData =
          ilbc_codec->dec_buffer + ilbc_codec->dec_buffer_offset;
        ioData->mBuffers[0].mDataByteSize =
          *ioNumberDataPackets * ilbc_codec->dec_frame_size;
                                     ...
                                     struct pjmedia_frame* output)
{
    ...
    if (ilbc_codec->vad_enabled) {
        ...
        silence_period =
          pj_timestamp_diff32(&ilbc_codec->last_tx, &input->timestamp);
...
#    if defined(PJMEDIA_ILBC_CODEC_USE_COREAUDIO) && \
      PJMEDIA_ILBC_CODEC_USE_COREAUDIO
        ...
        err =
          AudioConverterFillComplexBuffer(ilbc_codec->enc, encodeDataProc,
                                          ilbc_codec, &npackets, &theABL, NULL);
...
#    else
        ...
        iLBC_encode((unsigned char*)output->buf + output->size,
                    ilbc_codec->enc_block, &ilbc_codec->enc);
...
#    if defined(PJMEDIA_ILBC_CODEC_USE_COREAUDIO) && \
    ...
    if (ilbc_codec->dec_buffer) {
        err =
          AudioConverterFillComplexBuffer(ilbc_codec->dec, decodeDataProc,
                                          ilbc_codec, &npackets, &theABL, NULL);
        if (err == noErr) {
            output->size = npackets * (ilbc_codec->dec_samples_per_frame << 1);
        }
