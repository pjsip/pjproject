 ...
 */
#if defined(PJMEDIA_HAS_PASSTHROUGH_CODECS) && \
...
static pj_status_t enum_codecs(pjmedia_codec_factory* factory, unsigned* count,
...
static pjmedia_codec_op codec_op = { &codec_init,   &codec_open,
                                     &codec_close,  &codec_modify,
                                     &codec_parse,  &codec_encode,
...
static pjmedia_codec_factory_op codec_factory_op = {
    &test_alloc,  &default_attr,  &enum_codecs,
    &alloc_codec, &dealloc_codec, &pjmedia_codec_passthrough_deinit
};
...
#    if PJMEDIA_HAS_PASSTHROUGH_CODEC_AMR
    {
      1, "AMR", PJMEDIA_RTP_PT_AMR, PJMEDIA_FORMAT_AMR, 8000, 1, 160, 7400,
      12200, 2, 1, 1, &parse_amr, &pack_amr
      /*, {1, {{{"octet-align", 11}, {"1", 1}}} } */
    },
#    endif

#    if PJMEDIA_HAS_PASSTHROUGH_CODEC_G729
    { 1, "G729", PJMEDIA_RTP_PT_G729, PJMEDIA_FORMAT_G729, 8000, 1, 80, 8000,
      8000, 2, 1, 1 },
#    endif
...
#    if PJMEDIA_HAS_PASSTHROUGH_CODEC_PCMU
    { 1, "PCMU", PJMEDIA_RTP_PT_PCMU, PJMEDIA_FORMAT_PCMU, 8000, 1, 80, 64000,
      64000, 2, 1, 1 },
#    endif

#    if PJMEDIA_HAS_PASSTHROUGH_CODEC_PCMA
    { 1, "PCMA", PJMEDIA_RTP_PT_PCMA, PJMEDIA_FORMAT_PCMA, 8000, 1, 80, 64000,
      64000, 2, 1, 1 },
#    endif
};
                            ...
                            struct pjmedia_frame* output)
{
    ...
    for (i = 0; i < input->subframe_cnt; ++i) {
        ...
        if (len == 0) {
            ...
            info->frame_type =
              pjmedia_codec_amr_get_mode2(enc_setting->amr_nb, len);
        }
        info->good_quality = 1;
...
PJ_DEF(pj_status_t) pjmedia_codec_passthrough_init(pjmedia_endpt* endpt)
{
    ...
    codec_factory.pool =
      pjmedia_endpt_create_pool(endpt, "Passthrough codecs", 4000, 4000);
...
PJ_DEF(pj_status_t) pjmedia_codec_passthrough_deinit(void)
{
    ...
    status =
      pjmedia_codec_mgr_unregister_factory(codec_mgr, &codec_factory.base);
                                ...
                                pjmedia_codec_param* attr)
{
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(codec_desc); ++i) {
            ...
            (id->pt == (unsigned)codec_desc[i].pt))
        {
            ...
            attr->info.frm_ptime =
              (pj_uint16_t)(codec_desc[i].samples_per_frame * 1000 /
...
#    if PJMEDIA_HAS_PASSTHROUGH_CODEC_AMR
    ...
    if (desc->pt == PJMEDIA_RTP_PT_AMR || desc->pt == PJMEDIA_RTP_PT_AMRWB) {
        ...
        for (i = 0; i < attr->setting.dec_fmtp.cnt; ++i) {
            ...
            {
                octet_align = (pj_uint8_t)(pj_strtoul(
        ...
        for (i = 0; i < attr->setting.enc_fmtp.cnt; ++i) {
            ...
            if (pj_stricmp(&attr->setting.enc_fmtp.param[i].name,
                ...
                while (l--) {
                    if ((desc->pt == PJMEDIA_RTP_PT_AMR && *p >= '0' &&
                         *p <= '7') ||
                        (desc->pt == PJMEDIA_RTP_PT_AMRWB && *p >= '0' &&
                         *p <= '8'))
                    {
...
#    if PJMEDIA_HAS_PASSTHROUGH_CODEC_ILBC
    ...
    if (desc->pt == PJMEDIA_RTP_PT_ILBC) {
        ...
        /* Both sides of a bi-directional session MUST use the same "mode"
         * value. In this point, possible values are only 20 or 30, so when
         * encoder and decoder modes are not same, just use the default mode, it
         * is 30.
         */
                               ...
                               unsigned* frame_cnt, pjmedia_frame frames[])
{
    ...
    while (pkt_size >= codec_data->avg_frame_size && count < *frame_cnt) {
        ...
        frames[count].timestamp.u64 =
          ts->u64 + count * codec_data->samples_per_frame;
...
#    if PJMEDIA_HAS_PASSTHROUGH_CODEC_AMR
    ...
    if (desc->pt == PJMEDIA_RTP_PT_AMR || desc->pt == PJMEDIA_RTP_PT_AMRWB) {
        ...
        pjmedia_frame_ext_append_subframe(
          output_, input_.buf, (pj_uint16_t)(input_.size << 3),
...
#    endif
    ...
    pjmedia_frame_ext_append_subframe(
      output_, input->buf, (pj_uint16_t)(input->size << 3),
      (pj_uint16_t)codec_data->samples_per_frame);
    output->timestamp = input->timestamp;
                                 ...
                                 struct pjmedia_frame* output)
{
    ...
    pjmedia_frame_ext_append_subframe(
      output_, NULL, 0, (pj_uint16_t)codec_data->samples_per_frame);
