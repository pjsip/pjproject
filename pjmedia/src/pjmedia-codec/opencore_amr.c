...
static pj_status_t amr_codec_init(pjmedia_codec* codec, pj_pool_t* pool);
...
static pj_status_t amr_codec_parse(pjmedia_codec* codec, void* pkt,
...
static pjmedia_codec_op amr_op = { &amr_codec_init,   &amr_codec_open,
                                   &amr_codec_close,  &amr_codec_modify,
                                   &amr_codec_parse,  &amr_codec_encode,
                                   &amr_codec_decode, &amr_codec_recover };

/* Definition for AMR codec factory operations. */
static pjmedia_codec_factory_op amr_factory_op = {
    &amr_test_alloc,  &amr_default_attr,  &amr_enum_codecs,
    &amr_alloc_codec, &amr_dealloc_codec, &pjmedia_codec_opencore_amr_deinit
};
...
static const pj_uint16_t* amr_bitrates[2] = { pjmedia_codec_amrnb_bitrates,
                                              pjmedia_codec_amrwb_bitrates };
...
#    endif
    ...
    amr_codec_factory.pool =
      pjmedia_endpt_create_pool(endpt, "amr", 1000, 1000);
...
#    endif
    ...
    status =
      pjmedia_codec_mgr_register_factory(codec_mgr, &amr_codec_factory.base);
    if (status != PJ_SUCCESS)
        goto on_error;
...
PJ_DEF(pj_status_t) pjmedia_codec_opencore_amr_deinit(void)
{
    ...
    status =
      pjmedia_codec_mgr_unregister_factory(codec_mgr, &amr_codec_factory.base);
...
static pj_status_t amr_set_config(unsigned idx,
                                  const pjmedia_codec_amr_config* config)
{
    unsigned nbitrates;
                                   ...
                                   pjmedia_codec** p_codec)
{
    ...
    pool =
      pjmedia_endpt_create_pool(amr_codec_factory.endpt, "amr-inst", 512, 512);
                                  ...
                                  pjmedia_codec_param* attr)
{
    ...
    for (i = 0; i < attr->setting.enc_fmtp.cnt; ++i) {
        ...
        if (pj_stricmp(&attr->setting.enc_fmtp.param[i].name,
            ...
            while (l--) {
                if (*p >= '0' &&
                    (unsigned)*p <= ('0' + amr_bitrates_size[idx] - 1)) {
...
#    endif
    ...
    TRACE_((THIS_FILE,
            "AMR codec allocated: clockrate=%d vad=%d, plc=%d,"
            " bitrate=%d",
            amr_data->clock_rate, amr_data->vad_enabled, amr_data->plc_enabled,
                                    ...
                                    const pjmedia_codec_param* attr)
{
    ...
    if (amr_data->enc_setting.amr_nb && prev_vad_state != amr_data->vad_enabled)
    {
                                   ...
                                   unsigned* frame_cnt, pjmedia_frame frames[])
{
    ...
    if (cmr < amr_bitrates_size[idx] && amr_data->enc_mode != cmr) {
        ...
        TRACE_((THIS_FILE, "AMR encoder switched mode to %d (%dbps)",
                amr_data->enc_mode, amr_bitrates[idx][amr_data->enc_mode]));
    }
...
#    ifdef USE_AMRNB
            size = Encoder_Interface_Encode(
              amr_data->encoder, amr_data->enc_mode, speech, bitstream, 0);
#    else
            size = 0;
#    endif
        } else {
#    ifdef USE_AMRWB
            size = E_IF_encode(amr_data->encoder, amr_data->enc_mode, speech,
                               bitstream, 0);
#    else
            size = 0;
#    endif
        ...
        bitstream += size;
        out_size += size;
        TRACE_((THIS_FILE, "AMR encode(): mode=%d, size=%d", amr_data->enc_mode,
                out_size));
    }
    ...
    for (i = 0; i < nframes; ++i) {
        ...
        if (info->frame_type == 15) /* DTX*/
            ++dtx_cnt;
        else if (info->frame_type == 8 &&
                 amr_data->enc_setting.amr_nb) /* SID */
            ++sid_cnt;
        else if (info->frame_type == 9 &&
                 !amr_data->enc_setting.amr_nb) /* SID */
            ++sid_cnt;
    }
    ...
    if (sid_cnt + dtx_cnt == nframes) {
        ...
        dtx_duration =
          pj_timestamp_diff32(&amr_data->last_tx, &input->timestamp);
                                    ...
                                    struct pjmedia_frame* output)
{
    ...
    TRACE_((THIS_FILE, "AMR decode(): mode=%d, ft=%d, size=%d", info->mode,
            info->frame_type, input_.size));
...
#    ifdef USE_AMRWB
        D_IF_decode(amr_data->decoder, bitstream, (pj_int16_t*)output->buf, 0);
#    endif
    }
