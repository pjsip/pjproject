...
static pj_status_t ipp_codec_parse(pjmedia_codec* codec, void* pkt,
...
static pjmedia_codec_op ipp_op = { &ipp_codec_init,   &ipp_codec_open,
                                   &ipp_codec_close,  &ipp_codec_modify,
                                   &ipp_codec_parse,  &ipp_codec_encode,
                                   &ipp_codec_decode, &ipp_codec_recover };

/* Definition for IPP codecs factory operations. */
static pjmedia_codec_factory_op ipp_factory_op = {
    &ipp_test_alloc,  &ipp_default_attr,  &ipp_enum_codecs,
    &ipp_alloc_codec, &ipp_dealloc_codec, &pjmedia_codec_ipp_deinit
};
...
#        if defined(PJ_HAS_FLOATING_POINT) && (PJ_HAS_FLOATING_POINT != 0)
    { 1, "G729", PJMEDIA_RTP_PT_G729, &USC_G729AFP_Fxns, 8000, 1, 80, 8000,
      11800, 2, 1, 1, &predecode_g729, NULL, NULL },
#        else
    { 1, "G729", PJMEDIA_RTP_PT_G729, &USC_G729I_Fxns, 8000, 1, 80, 8000, 11800,
      2, 1, 1, &predecode_g729, NULL, NULL },
#        endif
#    endif

#    if PJMEDIA_HAS_INTEL_IPP_CODEC_G723_1
    /* This is actually G.723.1 */
    { 1, "G723", PJMEDIA_RTP_PT_G723, &USC_G723_Fxns, 8000, 1, 240, 6300, 6300,
      1, 1, 1, &predecode_g723, &parse_g723, NULL },
#    endif

#    if PJMEDIA_HAS_INTEL_IPP_CODEC_G726
    { 0, "G726-16", PJMEDIA_RTP_PT_G726_16, &USC_G726_Fxns, 8000, 1, 80, 16000,
      16000, 2, 0, 0, NULL, NULL, NULL },
    { 0, "G726-24", PJMEDIA_RTP_PT_G726_24, &USC_G726_Fxns, 8000, 1, 80, 24000,
      24000, 2, 0, 0, NULL, NULL, NULL },
    { 1, "G726-32", PJMEDIA_RTP_PT_G726_32, &USC_G726_Fxns, 8000, 1, 80, 32000,
      32000, 2, 0, 0, NULL, NULL, NULL },
    { 0, "G726-40", PJMEDIA_RTP_PT_G726_40, &USC_G726_Fxns, 8000, 1, 80, 40000,
      40000, 2, 0, 0, NULL, NULL, NULL },
    /* Old definition of G726-32 */
    { 1, "G721", PJMEDIA_RTP_PT_G721, &USC_G726_Fxns, 8000, 1, 80, 32000, 32000,
      2, 0, 0, NULL, NULL, NULL },
#    endif

#    if PJMEDIA_HAS_INTEL_IPP_CODEC_G728
    { 1, "G728", PJMEDIA_RTP_PT_G728, &USC_G728_Fxns, 8000, 1, 80, 16000, 16000,
                              ...
                              unsigned* frame_cnt, pjmedia_frame frames[])
{
    ...
    while (pkt_size && count < *frame_cnt) {
        ...
        frames[count].timestamp.u64 =
          ts->u64 + count * ipp_codec[codec_data->codec_idx].samples_per_frame;
                            ...
                            pj_size_t* pkt_size, pj_size_t max_pkt_size)
{
    ...
    for (;;) {
        ...
        frames[nframes].size =
          info->frame_type <= SID_FT ? framelen_tbl[info->frame_type] : 0;
                             ...
                             unsigned* frame_cnt, pjmedia_frame frames[])
{
        ...
        s->enc_mode != cmr)
    {
        ...
        codec_data->info->params.modes.bitrate =
          s->enc_setting.amr_nb ? pjmedia_codec_amrnb_bitrates[s->enc_mode] :
        ...
        PJ_LOG(4, (THIS_FILE, "AMR%s switched encoding mode to: %d (%dbps)",
                   (s->enc_setting.amr_nb ? "" : "-WB"), s->enc_mode,
                   codec_data->info->params.modes.bitrate));
    }
...
PJ_DEF(pj_status_t) pjmedia_codec_ipp_init(pjmedia_endpt* endpt)
{
    ...
    ipp_factory.pool =
      pjmedia_endpt_create_pool(endpt, "IPP codecs", 4000, 4000);
    if (!ipp_factory.pool)
        return PJ_ENOMEM;
                                    ...
                                    pjmedia_codec_param* attr)
{
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(ipp_codec); ++i) {
            ...
            (id->pt == (unsigned)ipp_codec[i].pt))
        {
            ...
            attr->info.frm_ptime =
              (pj_uint16_t)(ipp_codec[i].samples_per_frame * 1000 /
                                   ...
                                   pjmedia_codec** p_codec)
{
    ...
    if (!ipp_codec[idx].has_native_vad) {
        pj_status_t status;
        status = pjmedia_silence_det_create(pool, ipp_codec[idx].clock_rate,
                                  ...
                                  pjmedia_codec_param* attr)
{
    ...
    if (USC_NoError !=
        ippc->fxns->std.GetInfo((USC_Handle)NULL, codec_data->info)) {
    ...
    codec_data->info->params.modes.vad =
      attr->setting.vad && ippc->has_native_vad;
...
#    if PJMEDIA_HAS_INTEL_IPP_CODEC_G729
    if (ippc->pt == PJMEDIA_RTP_PT_G729) {
        ...
        for (i = 0; i < attr->setting.enc_fmtp.cnt; ++i) {
            if (pj_stricmp2(&attr->setting.enc_fmtp.param[i].name, "annexb") ==
                0) {
                if (pj_stricmp2(&attr->setting.enc_fmtp.param[i].val, "no") ==
                    0) {
                    attr->setting.vad = 0;
...
#    endif
    ...
    if (USC_NoError !=
        ippc->fxns->std.NumAlloc(&codec_data->info->params, &nb_membanks))
    {
    ...
    membanks =
      (USC_MemBank*)pj_pool_zalloc(pool, sizeof(USC_MemBank) * nb_membanks);
    /* Get size of each memory block */
    if (USC_NoError !=
        ippc->fxns->std.MemAlloc(&codec_data->info->params, membanks))
    {
    ...
    if (USC_NoError != ippc->fxns->std.Init(&codec_data->info->params, membanks,
                                            &codec_data->enc))
    {
...
#    if PJMEDIA_HAS_INTEL_IPP_CODEC_AMR
    ...
    if (ippc->pt == PJMEDIA_RTP_PT_AMR || ippc->pt == PJMEDIA_RTP_PT_AMRWB) {
        ...
        enc_mode =
          pjmedia_codec_amr_get_mode(codec_data->info->params.modes.bitrate);
        ...
        for (i = 0; i < attr->setting.enc_fmtp.cnt; ++i) {
            ...
            if (pj_stricmp(&attr->setting.enc_fmtp.param[i].name,
                ...
                while (l--) {
                    if ((ippc->pt == PJMEDIA_RTP_PT_AMR && *p >= '0' &&
                         *p <= '7') ||
                        (ippc->pt == PJMEDIA_RTP_PT_AMRWB && *p >= '0' &&
                         *p <= '8'))
                    {
...
#    if PJMEDIA_HAS_INTEL_IPP_CODEC_G722_1
        ...
        if (pt >= PJMEDIA_RTP_PT_G722_1_16 && pt <= PJMEDIA_RTP_PT_G7221_RSV2 &&
