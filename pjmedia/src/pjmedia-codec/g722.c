...
static pj_status_t g722_codec_init(pjmedia_codec* codec, pj_pool_t* pool);
...
static pj_status_t g722_codec_parse(pjmedia_codec* codec, void* pkt,
...
static pjmedia_codec_op g722_op = { &g722_codec_init,   &g722_codec_open,
                                    &g722_codec_close,  &g722_codec_modify,
                                    &g722_codec_parse,  &g722_codec_encode,
                                    &g722_codec_decode,
#    if !PLC_DISABLED
                                    &g722_codec_recover
#    else
...
static pjmedia_codec_factory_op g722_factory_op = {
    &g722_test_alloc,  &g722_default_attr,  &g722_enum_codecs,
    &g722_alloc_codec, &g722_dealloc_codec, &pjmedia_codec_g722_deinit
};
...
PJ_DEF(pj_status_t) pjmedia_codec_g722_init(pjmedia_endpt* endpt)
{
    ...
    g722_codec_factory.pool = pjmedia_endpt_create_pool(
      endpt, "g722", PJMEDIA_POOL_LEN_G722_CODEC, PJMEDIA_POOL_INC_G722_CODEC);
    ...
    status =
      pjmedia_codec_mgr_register_factory(codec_mgr, &g722_codec_factory.base);
    if (status != PJ_SUCCESS)
        goto on_error;
...
PJ_DEF(pj_status_t) pjmedia_codec_g722_deinit(void)
{
    ...
    status =
      pjmedia_codec_mgr_unregister_factory(codec_mgr, &g722_codec_factory.base);
...
#    endif
        ...
        status = pjmedia_silence_det_create(g722_codec_factory.pool, 16000,
                                            SAMPLES_PER_FRAME, &g722_data->vad);
        if (status != PJ_SUCCESS) {
            pj_mutex_unlock(g722_codec_factory.mutex);
                                     ...
                                     struct pjmedia_frame* output)
{
    ...
    if (g722_data->vad_enabled) {
        ...
        if (is_silence && (PJMEDIA_CODEC_MAX_SILENCE_PERIOD == -1 ||
                           silence_duration <
                             PJMEDIA_CODEC_MAX_SILENCE_PERIOD * 16000 / 1000))
        {
                                     ...
                                     struct pjmedia_frame* output)
{
    ...
    TRACE_((THIS_FILE,
            "G722 decode(): inbuf=%p, insize=%d, outbuf=%p,"
            "outsize=%d",
