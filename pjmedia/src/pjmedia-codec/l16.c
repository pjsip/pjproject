...
static pj_status_t l16_open(pjmedia_codec* codec, pjmedia_codec_param* attr);
static pj_status_t l16_close(pjmedia_codec* codec);
static pj_status_t l16_modify(pjmedia_codec* codec,
                              const pjmedia_codec_param* attr);
static pj_status_t l16_parse(pjmedia_codec* codec, void* pkt,
...
static pj_status_t l16_decode(pjmedia_codec* codec,
                              ...
                              struct pjmedia_frame* output);
#    if !PLC_DISABLED
static pj_status_t l16_recover(pjmedia_codec* codec, unsigned output_buf_len,
                               struct pjmedia_frame* output);
#    endif

/* Definition for L16 codec operations. */
static pjmedia_codec_op l16_op = { &l16_init,   &l16_open,  &l16_close,
                                   &l16_modify, &l16_parse, &l16_encode,
                                   &l16_decode,
#    if !PLC_DISABLED
                                   &l16_recover
#    else
...
static pjmedia_codec_factory_op l16_factory_op = {
    &l16_test_alloc,  &l16_default_attr,  &l16_enum_codecs,
    &l16_alloc_codec, &l16_dealloc_codec, &pjmedia_codec_l16_deinit
};
...
pjmedia_codec_l16_init(pjmedia_endpt* endpt, unsigned options)
{
    ...
    /* Create mutex. */
    status =
      pj_mutex_create_simple(l16_factory.pool, "l16", &l16_factory.mutex);
    ...
    status = pjmedia_codec_mgr_register_factory(codec_mgr, &l16_factory.base);
    if (status != PJ_SUCCESS)
        return status;
...
PJ_DEF(pj_status_t) pjmedia_codec_l16_deinit(void)
{
    ...
    status = pjmedia_codec_mgr_unregister_factory(codec_mgr, &l16_factory.base);
    l16_factory.endpt = NULL;
...
#    if !PLC_DISABLED
    /* Create PLC */
    status = pjmedia_plc_create(pool, id->clock_rate, data->frame_size >> 1, 0,
                                &data->plc);
    if (status != PJ_SUCCESS) {
        pj_mutex_unlock(l16_factory.mutex);
        return status;
                              ...
                              struct pjmedia_frame* output)
{
    ...
    if (data->vad_enabled) {
        ...
        is_silence = pjmedia_silence_det_detect(
          data->vad, (const pj_int16_t*)input->buf, (input->size >> 1), NULL);
