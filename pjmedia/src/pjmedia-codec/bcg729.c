...
static pj_status_t bcg729_codec_init(pjmedia_codec* codec, pj_pool_t* pool);
...
static pj_status_t bcg729_codec_parse(pjmedia_codec* codec, void* pkt,
                                      pj_size_t pkt_size,
...
static pjmedia_codec_op bcg729_op = {
    &bcg729_codec_init,   &bcg729_codec_open,   &bcg729_codec_close,
    &bcg729_codec_modify, &bcg729_codec_parse,  &bcg729_codec_encode,
    &bcg729_codec_decode, &bcg729_codec_recover
};

/* Definition for BCG729 codec factory operations. */
static pjmedia_codec_factory_op bcg729_factory_op = {
    &bcg729_test_alloc,  &bcg729_default_attr,  &bcg729_enum_codecs,
    &bcg729_alloc_codec, &bcg729_dealloc_codec, &pjmedia_codec_bcg729_deinit
};
...
PJ_DEF(pj_status_t) pjmedia_codec_bcg729_init(pjmedia_endpt* endpt)
{
    ...
    /* Create pool */
    bcg729_factory.pool =
      pjmedia_endpt_create_pool(endpt, "bcg729", 4000, 4000);
    ...
    status =
      pjmedia_codec_mgr_register_factory(codec_mgr, &bcg729_factory.base);
    if (status != PJ_SUCCESS)
        goto on_error;
...
PJ_DEF(pj_status_t) pjmedia_codec_bcg729_deinit(void)
{
    ...
    status =
      pjmedia_codec_mgr_unregister_factory(codec_mgr, &bcg729_factory.base);
    bcg729_factory.endpt = NULL;
                                       ...
                                       pjmedia_codec_param* attr)
{
    ...
    attr->info.frm_ptime = (pj_uint16_t)(G729_SAMPLES_PER_FRAME * 1000 /
                                         G729_CHANNEL_COUNT / G729_CLOCK_RATE);
                                     ...
                                     pjmedia_codec_param* attr)
{
    ...
    bcg729_data->encoder =
      initBcg729EncoderChannel(bcg729_data->vad_enabled ? 1 : 0);
                                       ...
                                       struct pjmedia_frame* output)
{
    ...
    bcg729Decoder(bcg729_data->decoder, (unsigned char*)input->buf,
                  G729_FRAME_SIZE, 0, 0, 0, (short*)output->buf);
                                        ...
                                        struct pjmedia_frame* output)
{
    ...
    if (bcg729_data->plc_enabled) {
        bcg729Decoder(bcg729_data->decoder, NULL, G729_FRAME_SIZE, 1, 0, 0,
                      (short*)output->buf);
    } else {
