...
#define GET_FMTP_IVAL_BASE(ival, base, fmtp, param, default_val) \
    do { \
        ...
        pj_strset( \
          &s, p + param.slen, \
          fmtp.fmt_param.slen - (p - fmtp.fmt_param.ptr) - param.slen); \
                                          ...
                                          unsigned fmt_idx)
{
    ...
    attr =
      pjmedia_sdp_media_find_attr2(media, "fmtp", &media->desc.fmt[fmt_idx]);
    ...
    if (attr) {
        ...
        if (p) {
            ...
            pj_strset(&s, p + STR_OCTET_ALIGN.slen,
                      fmtp.fmt_param.slen - (p - fmtp.fmt_param.ptr) -
                        STR_OCTET_ALIGN.slen);
...
PJ_DEF(pj_status_t)
pjmedia_codec_amr_match_sdp(pj_pool_t* pool, pjmedia_sdp_media* offer,
                            ...
                            unsigned a_fmt_idx, unsigned option)
{
    ...
    /* Parse offerer FMTP */
    attr_ofr =
      pjmedia_sdp_media_find_attr2(offer, "fmtp", &offer->desc.fmt[o_fmt_idx]);
    if (attr_ofr) {
    ...
    if (a_crc != o_crc || a_robust_sorting != o_robust_sorting ||
