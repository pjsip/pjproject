...
static pj_status_t spx_codec_parse(pjmedia_codec* codec, void* pkt,
...
static pjmedia_codec_op spx_op = { &spx_codec_init,   &spx_codec_open,
                                   &spx_codec_close,  &spx_codec_modify,
                                   &spx_codec_parse,  &spx_codec_encode,
                                   &spx_codec_decode, &spx_codec_recover };

/* Definition for Speex codec factory operations. */
static pjmedia_codec_factory_op spx_factory_op = {
    &spx_test_alloc,  &spx_default_attr,  &spx_enum_codecs,
    &spx_alloc_codec, &spx_dealloc_codec, &pjmedia_codec_speex_deinit
};
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_codec_speex_init(pjmedia_endpt* endpt, unsigned options, int quality,
                         int complexity)
{
    ...
    /* Get defaults */
    if (quality < 0)
        quality = PJMEDIA_CODEC_SPEEX_DEFAULT_QUALITY;
    if (complexity < 0)
        complexity = PJMEDIA_CODEC_SPEEX_DEFAULT_COMPLEXITY;
    ...
    /* Create mutex. */
    status =
      pj_mutex_create_simple(spx_factory.pool, "speex", &spx_factory.mutex);
    ...
    spx_factory.speex_param[PARAM_NB].mode =
    ...
    spx_factory.speex_param[PARAM_WB].mode =
    ...
    spx_factory.speex_param[PARAM_UWB].mode =
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_codec_speex_set_param(unsigned clock_rate, int quality, int complexity)
{
    unsigned i;
                                   ...
                                   unsigned* count, pjmedia_codec_info codecs[])
{
    ...
    *count = 0;
     ...
     */
    for (i = PJ_ARRAY_SIZE(spx_factory.speex_param) - 1; i >= 0 && *count < max;
         --i)
    {
                                   ...
                                   pjmedia_codec** p_codec)
{
    ...
    if (!pj_list_empty(&spx_factory.codec_list)) {
        ...
        codec->codec_data =
          pj_pool_alloc(spx_factory.pool, sizeof(struct spx_private));
    }
...
{
    static const int inband_skip_table[NB_SUBMODES] = { 1,  1,  4,  4, 4,  4,
                                                        4,  4,  8,  8, 16, 16,
                                                        32, 32, 64, 64 };
    ...
    while (speex_bits_remaining(bits) >= 5) {
        ...
        TRACE__((THIS_FUNC, "Control bits: %d at %d", submode,
                 bits->charPtr * 8 + bits->bitPtr));
        ...
        if (submode == 15) {
            ...
            /* NB frame */
            int advance = submode;
            speex_mode_query(&speex_nb_mode, SPEEX_SUBMODE_BITS_PER_FRAME,
                             &advance);
            if (advance < 0) {
                TRACE__((THIS_FUNC,
                         "Invalid mode encountered. "
                                   ...
                                   unsigned* frame_cnt, pjmedia_frame frames[])
{
    ...
    samples_per_frame =
      spx_factory.speex_param[spx->param_id].samples_per_frame;
           ...
           spx->dec_bits.charPtr != char_ptr)
    {
        ...
        frames[count].timestamp.u64 =
          ts->u64 + (pj_uint64_t)count * samples_per_frame;
                                    ...
                                    struct pjmedia_frame* output)
{
    ...
    output->size =
      speex_bits_write(&spx->enc_bits, (char*)output->buf, output_buf_len);
