...
static pjmedia_codec_factory_op codec_factory_op = {
    ...
    &alloc_codec, &dealloc_codec, &pjmedia_codec_g7221_deinit
};
...
PJ_DEF(pj_status_t) pjmedia_codec_g7221_init(pjmedia_endpt* endpt)
{
    ...
    codec_factory.pool =
      pjmedia_endpt_create_pool(endpt, "G722.1 codec", 4000, 4000);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_codec_g7221_set_mode(unsigned sample_rate, unsigned bitrate,
                             pj_bool_t enabled)
{
    pjmedia_codec_mgr* codec_mgr;
    unsigned i;
    ...
    for (i = 0; i < codec_factory.mode_count; ++i) {
            ...
            codec_factory.modes[i].bitrate == bitrate)
        {
            ...
            pjmedia_codec_mgr_unregister_factory(codec_mgr,
                                                 &codec_factory.base);
            pjmedia_codec_mgr_register_factory(codec_mgr, &codec_factory.base);
            return PJ_SUCCESS;
        }
    }
                               ...
                               pjmedia_codec** p_codec)
{
    ...
    status = pjmedia_silence_det_create(
      pool, id->clock_rate, id->clock_rate * 20 / 1000, &codec_data->vad);
    if (status != PJ_SUCCESS) {
        pj_mutex_unlock(codec_factory.mutex);
        return status;
    }
...
static pj_status_t codec_open(pjmedia_codec* codec, pjmedia_codec_param* attr)
{
    ...
    codec_data->samples_per_frame =
      (pj_uint16_t)(attr->info.clock_rate * 20 / 1000);
    codec_data->number_of_regions =
      (pj_uint16_t)(attr->info.clock_rate <= WB_SAMPLE_RATE ?
                      NUMBER_OF_REGIONS :
                                ...
                                struct pjmedia_frame* output)
{
    ...
    while (processed < nsamples) {
        ...
        mag_shift =
          samples_to_rmlt_coefs(pcm_input, codec_data->enc_old_frame, mlt_coefs,
                                codec_data->samples_per_frame);

        /* Encode the mlt coefs. Note that encoder output stream is
         * 16 bit array, so we need to take care about endianness.
         */
        encoder(codec_data->frame_size_bits, codec_data->number_of_regions,
                mlt_coefs, mag_shift, (Word16*)out_bits);
                                ...
                                struct pjmedia_frame* output)
{
    ...
    PJ_ASSERT_RETURN(
      output_buf_len >= (unsigned)(codec_data->samples_per_frame << 1),
    ...
    decoder(&bitobj, &codec_data->dec_randobj, codec_data->number_of_regions,
            mlt_coefs, &mag_shift, &codec_data->dec_old_mag_shift,
            codec_data->dec_old_mlt_coefs, frame_error_flag);

    /* Convert the mlt_coefs to PCM samples */
    rmlt_coefs_to_samples(mlt_coefs, codec_data->dec_old_frame,
                          (Word16*)output->buf, codec_data->samples_per_frame,
                          mag_shift);
