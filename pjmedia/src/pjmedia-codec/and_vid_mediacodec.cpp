...
#include <pj/log.h>
  ...
  PJMEDIA_HAS_ANDROID_MEDIACODEC != 0 && defined(PJMEDIA_HAS_VIDEO) && \
  (PJMEDIA_HAS_VIDEO != 0)
...
static pj_status_t and_media_codec_get_param(pjmedia_vid_codec* codec,
                                             pjmedia_vid_codec_param* param);
static pj_status_t and_media_codec_encode_begin(
  pjmedia_vid_codec* codec, const pjmedia_vid_encode_opt* opt,
...
static pjmedia_vid_codec_factory_op and_media_factory_op = {
    &and_media_test_alloc, &and_media_default_attr, &and_media_enum_info,
    &and_media_alloc_codec, &and_media_dealloc_codec
};
...
typedef pj_status_t (*decode_cb)(pjmedia_vid_codec* codec, pj_size_t count,
...
static pj_status_t encode_more_h264(and_media_codec_data* and_media_data,
                                    unsigned out_size, pjmedia_frame* output,
                                    pj_bool_t* has_more);
static pj_status_t decode_h264(pjmedia_vid_codec* codec, pj_size_t count,
                               pjmedia_frame packets[], unsigned out_size,
                               pjmedia_frame* output);
#    endif
...
static pj_status_t encode_more_vpx(and_media_codec_data* and_media_data,
                                   unsigned out_size, pjmedia_frame* output,
                                   pj_bool_t* has_more);
static pj_status_t decode_vpx(pjmedia_vid_codec* codec, pj_size_t count,
                              pjmedia_frame packets[], unsigned out_size,
                              pjmedia_frame* output);
#    endif
...
static pj_str_t H264_sw_decoder[] = { { (char*)"OMX.google.h264.decoder\0",
                                        23 } };
static pj_str_t H264_hw_decoder[] = { { (char*)"OMX.qcom.video.decoder.avc\0",
                                        26 },
                                      { (char*)"OMX.Exynos.avc.dec\0", 18 } };
#    endif

#    if PJMEDIA_HAS_AND_MEDIA_VP8
static pj_str_t VP8_sw_encoder[] = { { (char*)"OMX.google.vp8.encoder\0",
                                       23 } };
static pj_str_t VP8_hw_encoder[] = {
    { (char*)"OMX.qcom.video.encoder.vp8\0", 26 },
    { (char*)"OMX.Exynos.vp8.Encoder\0", 22 }
};
static pj_str_t VP8_sw_decoder[] = { { (char*)"OMX.google.vp8.decoder\0",
                                       23 } };
static pj_str_t VP8_hw_decoder[] = { { (char*)"OMX.qcom.video.decoder.vp8\0",
                                       26 },
                                     { (char*)"OMX.Exynos.vp8.dec\0", 18 } };
#    endif

#    if PJMEDIA_HAS_AND_MEDIA_VP9
static pj_str_t VP9_sw_encoder[] = { { (char*)"OMX.google.vp9.encoder\0",
                                       23 } };
static pj_str_t VP9_hw_encoder[] = {
    { (char*)"OMX.qcom.video.encoder.vp9\0", 26 },
    { (char*)"OMX.Exynos.vp9.Encoder\0", 22 }
};
static pj_str_t VP9_sw_decoder[] = { { (char*)"OMX.google.vp9.decoder\0",
                                       23 } };
static pj_str_t VP9_hw_decoder[] = { { (char*)"OMX.qcom.video.decoder.vp9\0",
                                       26 },
...
#    if PJMEDIA_HAS_AND_MEDIA_VP8
    { 0,
      ...
      &decode_vpx,
      { 2,
        { { { (char*)"max-fr", 6 }, { (char*)"30", 2 } },
          { { (char*)" max-fs", 7 }, { (char*)"580", 3 } } } } },
#    endif
#    if PJMEDIA_HAS_AND_MEDIA_VP9
    { 0,
      "VP9",
      "Android MediaCodec VP9 codec",
      "video/x-vnd.on2.vp9",
      NULL,
      NULL,
      AND_MED_VP9_PT,
...
static pj_status_t configure_encoder(and_media_codec_data* and_media_data)
{
    ...
    AMediaFormat_setInt32(
      vid_fmt, AND_MEDIA_KEY_FRAME_RATE,
    ...
    if (am_status != AMEDIA_OK) {
        PJ_LOG(4,
               (THIS_FILE, "Encoder configure failed, status=%d", am_status));
...
static pj_status_t configure_decoder(and_media_codec_data* and_media_data)
{
    ...
    am_status =
      AMediaCodec_configure(and_media_data->dec, vid_fmt, NULL, NULL, 0);
                                 ...
                                 pj_pool_factory* pf)
{
    ...
    and_media_factory.pool =
      pj_pool_create(pf, "and_media_vid_factory", 256, 256, NULL);
...
#    if PJMEDIA_HAS_AND_MEDIA_H264
    ...
    status = pjmedia_sdp_neg_register_fmt_match_cb(
      &h264_name, &pjmedia_vid_codec_h264_match_sdp);
...
#    endif
    ...
    status =
      pjmedia_vid_codec_mgr_register_factory(mgr, &and_media_factory.base);
    if (status != PJ_SUCCESS)
        goto on_error;
                                          ...
                                          pjmedia_vid_codec_param* attr)
{
    ...
    /* Encoded format */
    pjmedia_format_init_video(&attr->enc_fmt, info->fmt_id, DEFAULT_WIDTH,
...
void add_codec(struct and_media_codec* codec, unsigned* count,
...
static void get_codec_name(pj_bool_t is_enc, pj_bool_t prio,
                           pjmedia_format_id fmt_id, pj_str_t** codec_name,
                           unsigned* codec_num)
{
                                       ...
                                       pjmedia_vid_codec_info info[])
{
    ...
    for (i = 0, *count = 0; i < PJ_ARRAY_SIZE(and_media_codec) && *count < max;
        ...
        get_codec_name(PJ_TRUE, PJ_TRUE, and_media_codec[i].fmt_id, &enc_name,
                       &num_enc);
        ...
        get_codec_name(PJ_FALSE, PJ_TRUE, and_media_codec[i].fmt_id, &dec_name,
                       &num_dec);
        ...
        PJ_LOG(
          4, (THIS_FILE, "Found encoder [%d]: %.*s and decoder: %.*s ", *count,
...
static void create_codec(struct and_media_codec_data* and_media_data)
{
    ...
    PJ_LOG(4,
           (THIS_FILE, "Created encoder: %s, decoder: %s", enc_name, dec_name));
}
                                        ...
                                        pjmedia_vid_codec_param* codec_param)
{
    ...
    and_media_data = (and_media_codec_data*)codec->codec_data;
    and_media_data->prm =
      pjmedia_vid_codec_param_clone(and_media_data->pool, codec_param);
    param = and_media_data->prm;
    if (and_media_codec[and_media_data->codec_idx].open_codec) {
        status =
          and_media_codec[and_media_data->codec_idx].open_codec(and_media_data);
        if (status != PJ_SUCCESS)
            return status;
    }
    ...
    if (and_media_data->dec_buf_size == 0) {
        and_media_data->dec_buf_size =
          (MAX_RX_WIDTH * MAX_RX_HEIGHT * 3 >> 1) + (MAX_RX_WIDTH);
    }
    and_media_data->dec_buf = (pj_uint8_t*)pj_pool_alloc(
      and_media_data->pool, and_media_data->dec_buf_size);
...
#    if __ANDROID_API__ >= 26
        ...
        if (am_status != AMEDIA_OK)
            PJ_LOG(4,
                   (THIS_FILE, "Encoder setParameters failed %d", am_status));
...
#    endif
    ...
    if (buf_idx >= 0) {
        ...
        pj_uint8_t* input_buf = AMediaCodec_getInputBuffer(
          and_media_data->enc, buf_idx, &output_size);
        if (input_buf && output_size >= input->size) {
            pj_memcpy(input_buf, input->buf, input->size);
            am_status = AMediaCodec_queueInputBuffer(
              and_media_data->enc, buf_idx, 0, input->size, 0, 0);
            if (am_status != AMEDIA_OK) {
        ...
        PJ_LOG(4,
               (THIS_FILE, "Encoder dequeueInputBuffer failed[%d]", buf_idx));
        goto on_return;
    }
    ...
    for (i = 0; i < CODEC_WAIT_RETRY; ++i) {
        ...
        if (buf_idx == -1) {
            ...
            PJ_LOG(
              5, (THIS_FILE, "Encoder dequeueOutputBuffer timeout[%d]", i + 1));
    ...
    if (buf_idx >= 0) {
        ...
        if (!output_buf) {
            PJ_LOG(4, (THIS_FILE,
                       "Encoder failed getting output buffer, "
        ...
        if (and_media_data->whole) {
            ...
            *has_more = PJ_FALSE;
            ...
            AMediaCodec_releaseOutputBuffer(and_media_data->enc, buf_idx, 0);
        ...
        if (buf_idx == -2) {
            ...
            /* Format change. */
            AMediaFormat* vid_fmt =
              AMediaCodec_getOutputFormat(and_media_data->enc);
            ...
            PJ_LOG(5, (THIS_FILE,
                       "Encoder detect new width %d, height %d, "
                                               ...
                                               pj_bool_t* has_more)
{
    ...
    status = and_media_codec[and_media_data->codec_idx].encode_more(
      and_media_data, out_size, output, has_more);
    if (!(*has_more)) {
...
static int write_yuv(pj_uint8_t* buf, unsigned dst_len, unsigned char* input,
                     int stride_len, int iWidth, int iHeight)
{
    unsigned req_size;
    pj_uint8_t* dst = buf;
  ...
  struct and_media_codec_data* and_media_data)
{
    ...
    and_media_data->dec_input_buf = AMediaCodec_getInputBuffer(
      and_media_data->dec, buf_idx, &and_media_data->dec_input_buf_max_size);
}
                                    ...
                                    pjmedia_frame* output)
{
    ...
    {
        am_status = AMediaCodec_queueInputBuffer(
          and_media_data->dec, and_media_data->dec_input_buf_idx, 0,
          and_media_data->dec_input_buf_len, input_ts->u32.lo, buf_flag);
        if (am_status != AMEDIA_OK) {
    ...
    if (buf_idx >= 0) {
        ...
        pj_uint8_t* output_buf = AMediaCodec_getOutputBuffer(
          and_media_data->dec, buf_idx, &output_size);
        if (output_buf == NULL) {
            am_status =
              AMediaCodec_releaseOutputBuffer(and_media_data->dec, buf_idx, 0);
            PJ_LOG(4, (THIS_FILE, "Decoder getOutputBuffer failed"));
            return status;
        }
        len =
          write_yuv((pj_uint8_t*)output->buf, and_media_data->dec_buf_info.size,
                    output_buf, and_media_data->dec_stride_len,
                    and_media_data->prm->dec_fmt.det.vid.size.w,
                    and_media_data->prm->dec_fmt.det.vid.size.h);
        ...
        PJ_LOG(4,
               (THIS_FILE, "Decoder dequeueOutputBuffer failed [%d]", buf_idx));
    }
    return status;
}
                                          ...
                                          pjmedia_frame* output)
{
    ...
    if (and_media_codec[and_media_data->codec_idx].decode) {
        status = and_media_codec[and_media_data->codec_idx].decode(
          codec, count, packets, out_size, output);
    }
    if (status != PJ_SUCCESS) {
        return status;
    }
...
static pj_status_t open_h264(and_media_codec_data* and_media_data)
{
    ...
    if (h264_fmtp.sprop_param_sets_len) {
        ...
        if (i >= code_size) {
            ...
            h264_data->dec_pps_len =
              h264_fmtp.sprop_param_sets_len + med_code_size - code_size - i;
                                    ...
                                    pj_bool_t* has_more)
{
    ...
    status = pjmedia_h264_packetize(
      h264_data->pktz, data_buf, and_media_data->enc_frame_size,
      &and_media_data->enc_processed, &payload, &payload_len);
    if (status != PJ_SUCCESS) {
        /* Reset */
        and_media_data->enc_frame_size = and_media_data->enc_processed = 0;
        *has_more =
          (and_media_data->enc_processed < and_media_data->enc_frame_size);
                               ...
                               pjmedia_frame* output)
{
    ...
    if (and_media_data->whole) {
        for (i = 0; i < count; ++i) {
            ...
            pj_memcpy(and_media_data->dec_buf + whole_len,
                      (pj_uint8_t*)packets[i].buf, packets[i].size);
        ...
        for (i = 0; i < count; ++i) {
            ...
            status = pjmedia_h264_unpacketize(
              h264_data->pktz, (pj_uint8_t*)packets[i].buf, packets[i].size,
              and_media_data->dec_buf, and_media_data->dec_buf_size,
              &whole_len);
            if (status != PJ_SUCCESS) {
    ...
    {
        PJ_LOG(4, (THIS_FILE,
                   "Decoding buffer overflow or unpacketize error "
                   "size: %d, buffer: %d",
                   whole_len, and_media_data->dec_buf_size));
        return PJMEDIA_CODEC_EFRMTOOSHORT;
    }
                                   ...
                                   pj_bool_t* has_more)
{
    ...
    if (and_media_data->enc_processed < and_media_data->enc_frame_size) {
        ...
        pj_bool_t is_keyframe =
          and_media_data->enc_buf_info.flags & AND_MEDIA_FRM_TYPE_KEYFRAME;

        status = pjmedia_vpx_packetize(
          vpx_data->pktz, and_media_data->enc_frame_size,
          &and_media_data->enc_processed, is_keyframe, &p, &payload_len);
        if (status != PJ_SUCCESS) {
            return status;
        }
        pj_memcpy(
                              ...
                              pjmedia_frame* output)
{
    ...
    if (and_media_data->whole) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            status = and_media_decode(
              codec, and_media_data, (pj_uint8_t*)packets[i].buf + desc_len,
              packet_size, 0, &packets[0].timestamp, write_output, output);
            if (status != PJ_SUCCESS)
                return status;
