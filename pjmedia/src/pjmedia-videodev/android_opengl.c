...
/* Supported formats */
static andgl_fmt_info andgl_fmts[] = { { PJMEDIA_FORMAT_BGRA } };
...
/* Operations */
static pjmedia_vid_dev_stream_op stream_op = { &andgl_stream_get_param,
...
static pj_status_t init_opengl(void* data)
{
    ...
    strm->surface =
      eglCreateWindowSurface(strm->display, config, strm->window, 0);
    if (strm->surface == EGL_NO_SURFACE)
        EGL_ERR("create window surface");

    strm->context =
      eglCreateContext(strm->display, config, EGL_NO_CONTEXT, context_attr);
    ...
    if (!eglSwapBuffers(stream->display, stream->surface)) {
        if (eglGetError() == EGL_BAD_SURFACE && stream->err_rend > 0) {
            ...
            if (stream->err_rend == 0) {
                PJ_LOG(3, (THIS_FILE,
                           "Stopping OpenGL rendering due to "
  ...
  void* user_data, pjmedia_vid_dev_stream** p_vid_strm)
{
    ...
    if (param->flags & PJMEDIA_VID_DEV_CAP_OUTPUT_WINDOW) {
        status = andgl_stream_set_cap(
          &strm->base, PJMEDIA_VID_DEV_CAP_OUTPUT_WINDOW, &param->window);
    }

    if (status != PJ_SUCCESS) {
        PJ_LOG(3, (THIS_FILE,
                   "Failed to initialize OpenGL with the specified"
                   " output window"));
        goto on_error;
    }
                                        ...
                                        const void* pval)
{
    ...
    if (cap == PJMEDIA_VID_DEV_CAP_FORMAT) {
        ...
        PJ_PERROR(
          4, (THIS_FILE, status, "Re-initializing OpenGL with native window %p",
              strm->window));
        return status;
    }
