...
static pj_status_t
qt_factory_create_stream(pjmedia_vid_dev_factory* f,
                         pjmedia_vid_dev_param* param,
...
static pjmedia_vid_dev_factory_op factory_op = {
    &qt_factory_init,          &qt_factory_destroy,
    &qt_factory_get_dev_count, &qt_factory_get_dev_info,
    &qt_factory_default_param, &qt_factory_create_stream,
    &qt_factory_refresh
};

static pjmedia_vid_dev_stream_op stream_op = { &qt_stream_get_param,
                                               &qt_stream_get_cap,
                                               &qt_stream_set_cap,
                                               &qt_stream_start,
                                               NULL,
...
static pj_status_t qt_factory_refresh(pjmedia_vid_dev_factory* f)
{
    ...
    qf->dev_info = (struct qt_dev_info*)pj_pool_calloc(
      qf->dev_pool, dev_count, sizeof(struct qt_dev_info));
    for (i = 0; i < [dev_array count]; i++) {
        QTCaptureDevice* dev = [dev_array objectAtIndex:i];
        if ([dev hasMediaType:QTMediaTypeVideo] ||
            [dev hasMediaType:QTMediaTypeMuxed]) {
            ...
            [[dev localizedDisplayName]
              getCString:qdi->info.name
            ...
            for (k = 0; k < [[dev formatDescriptions] count]; k++) {
                ...
                QTFormatDescription* desc =
                  [[dev formatDescriptions] objectAtIndex:k];
                for (l = 0; l < PJ_ARRAY_SIZE(qt_fmts); l++) {
                    if ([desc formatType] == qt_fmts[l].qt_format) {
                        ...
                        pjmedia_format_init_video(
                          fmt, qt_fmts[l].pjmedia_format, DEFAULT_WIDTH,
    ...
    PJ_LOG(4, (THIS_FILE, "qt video has %d devices", qf->dev_count));
       ...
       fromConnection:(QTCaptureConnection*)connection
{
    ...
    if (strm->cap_thread_initialized == 0 || !pj_thread_is_registered()) {
        pj_thread_register("qt_cap", strm->cap_thread_desc, &strm->cap_thread);
...
static void init_qt(struct qt_stream* strm)
{
    ...
    QTCaptureDevice* videoDevice = [QTCaptureDevice
      deviceWithUniqueID:
        [NSString
          stringWithCString:strm->qf->dev_info[strm->param.cap_id].dev_id
                   encoding:[NSString defaultCStringEncoding]]];
    ...
    strm->dev_input = [[QTCaptureDeviceInput alloc] initWithDevice:videoDevice];
    ...
    success = [strm->cap_session addOutput:strm->video_output error:&error];
    if (!success) {
        strm->status = PJMEDIA_EVID_SYSERR;
        return;
    }
    ...
    [strm->video_output
      setPixelBufferAttributes:
        [NSDictionary
          dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:qfi->qt_format],
    ...
    {
        [strm->video_output
          setMinimumVideoFrameInterval:(1.0f * vfd->fps.denum /
                                        (double)vfd->fps.num)];
    }
