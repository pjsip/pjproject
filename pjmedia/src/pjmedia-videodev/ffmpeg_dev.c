...
static pjmedia_vid_dev_factory_op factory_op = {
    &ffmpeg_factory_init,          &ffmpeg_factory_destroy,
    &ffmpeg_factory_get_dev_count, &ffmpeg_factory_get_dev_info,
    &ffmpeg_factory_default_param, &ffmpeg_factory_create_stream,
    &ffmpeg_factory_refresh
};

static pjmedia_vid_dev_stream_op stream_op = {
    &ffmpeg_stream_get_param, &ffmpeg_stream_get_cap,   &ffmpeg_stream_set_cap,
    &ffmpeg_stream_start,     &ffmpeg_stream_get_frame, NULL,
    &ffmpeg_stream_stop,      &ffmpeg_stream_destroy
};
    ...
    *dev_cnt = 0;
           ...
           IEnumMoniker_Next(enum_cat, 1, &moniker, &fetched) == S_OK)
    {
        ...
        if (SUCCEEDED(hr)) {
            ...
            hr = IPropertyBag_Read(prop_bag, L"FriendlyName", &var_name, NULL);
            if (SUCCEEDED(hr) && var_name.bstrVal) {
                char tmp[MAX_DEV_NAME_LEN] = { 0 };
                WideCharToMultiByte(CP_ACP, 0, var_name.bstrVal,
                                    (int)wcslen(var_name.bstrVal), tmp,
                                    MAX_DEV_NAME_LEN, NULL, NULL);
                pj_ansi_snprintf(dev_names[i++], MAX_DEV_NAME_LEN, "video=%s",
                                 tmp);
            }
...
#    endif
        ...
        for (dev_idx = 0; dev_idx < dev_cnt && ff->dev_count < MAX_DEV_CNT;
            ...
            ctx = avformat_alloc_context();
            if (!ctx ||
                avformat_open_input(&ctx, dev_names[dev_idx], p, NULL) != 0)
                continue;
            ...
            for (i = 0; i < info->base.fmt_cnt; ++i) {
                ...
                pjmedia_format_init_video(fmt, fmt_id, codec->width,
                                          codec->height, 15, 1);
            }
