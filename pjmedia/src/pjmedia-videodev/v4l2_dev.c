...
static pjmedia_vid_dev_factory_op factory_op = {
    &vid4lin_factory_init,          &vid4lin_factory_destroy,
    &vid4lin_factory_get_dev_count, &vid4lin_factory_get_dev_info,
    &vid4lin_factory_default_param, &vid4lin_factory_create_stream,
    &vid4lin_factory_refresh
};

static pjmedia_vid_dev_stream_op stream_op = {
    &vid4lin_stream_get_param, &vid4lin_stream_get_cap,
    &vid4lin_stream_set_cap,   &vid4lin_stream_start,
    &vid4lin_stream_get_frame, NULL,
    &vid4lin_stream_stop,      &vid4lin_stream_destroy
};
...
static void v4l2_get_supported_size(int fd, pj_uint32_t fmt_id,
                                    pjmedia_vid_dev_info* info)
{
    int i = 0;
         ...
         i++)
    {
        ...
        pjmedia_format_init_video(&info->fmt[info->fmt_cnt++], fmt_id,
                                  v4l_sizes[i].w, v4l_sizes[i].h, DEFAULT_FPS,
                                  1);
    }
}
...
static pj_status_t v4l2_scan_devs(vid4lin_factory* f)
{
        ...
        for (j = 0; fmt_cnt < PJ_ARRAY_SIZE(fmt_cap); ++j) {
            ...
            for (k = 0; k < PJ_ARRAY_SIZE(v4l2_fmt_maps); ++k) {
                if (v4l2_fmt_maps[k].v4l2_fmt_id == fdesc.pixelformat) {
                    ...
                    PJ_LOG(5,
                           (THIS_FILE, "   Supported: %s", fdesc.description));
                    break;
                }
            }
    ...
    if (f->dev_count > old_count || f->dev_info == NULL) {
        f->dev_info = (vid4lin_dev_info*)pj_pool_calloc(
          f->dev_pool, f->dev_count, sizeof(vid4lin_dev_info));
    }
...
static pj_status_t vid4lin_factory_refresh(pjmedia_vid_dev_factory* f)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Video4Linux2 has %d devices", cf->dev_count));
...
static pj_status_t vid4lin_stream_init_streaming(vid4lin_stream* stream)
{
    ...
    stream->buffers =
      pj_pool_calloc(stream->pool, req.count, sizeof(*stream->buffers));
    stream->buf_cnt = 0;
...
/* API: create stream */
static pj_status_t vid4lin_factory_create_stream(
  ...
  pjmedia_vid_dev_stream** p_vid_strm)
{
    ...
    if (status != PJ_SUCCESS) {
        PJ_LOG(
          1, (THIS_FILE, "Error: unable to initiate I/O on %s", stream->name));
        goto on_error;
    }
