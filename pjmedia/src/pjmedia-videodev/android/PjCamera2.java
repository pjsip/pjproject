...
public class PjCamera2 {
    ...
    native void PushFrame2(long userData_, ByteBuffer plane0, int rowStride0,
                           int pixStride0, ByteBuffer plane1, int rowStride1,
                           int pixStride1, ByteBuffer plane2, int rowStride2,
                           int pixStride2);

    ImageReader.OnImageAvailableListener imageAvailListener =
      new ImageReader.OnImageAvailableListener() {
          @Override public void onImageAvailable(ImageReader reader)
          {
              ...
              /* Get planes buffers. According to the docs, the buffers are
               * always direct buffers */
              Image.Plane[] planes = image.getPlanes();
              ByteBuffer plane0 = planes[0].getBuffer();
              ByteBuffer plane1 =
                planes.length > 1 ? planes[1].getBuffer() : null;
              ByteBuffer plane2 =
                planes.length > 2 ? planes[2].getBuffer() : null;
              assert plane0.isDirect();

              // for (Image.Plane p: planes) {
              //	Log.d(TAG, String.format("size=%d bytes,
              //getRowStride()=%d getPixelStride()=%d",
              //p.getBuffer().remaining(), p.getRowStride(),
              //p.getPixelStride()));
              // }

              PushFrame2(userData, plane0, planes[0].getRowStride(),
                         planes[0].getPixelStride(), plane1,
                         plane1 != null ? planes[1].getRowStride() : 0,
                         plane1 != null ? planes[1].getPixelStride() : 0,
                         plane2, plane2 != null ? planes[2].getRowStride() : 0,
                         plane2 != null ? planes[2].getPixelStride() : 0);

              image.close();
          }
      };

    private final CameraDevice.StateCallback camStateCallback =
      new CameraDevice.StateCallback() {
          ...
          @Override public void onClosed(CameraDevice c)
          {
