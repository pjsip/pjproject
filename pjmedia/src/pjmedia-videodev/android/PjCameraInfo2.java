    ...
    private static int Refresh()
    {
        ...
        for (int i = 0; i < camIds.length; ++i) {
            ...
            try {
                ...
                Log.w(TAG,
                      String.format(
                        "%3d: id=%s skipped due to failure in querying info", i,
                        camIds[i]));
                continue;
            }
            ...
            Log.i(TAG, String.format("%3d: id=%s formats=%s", i, pjci.id,
                                     Arrays.toString(outFmts)));
            if (fmtCnt == 0) {
                Log.w(TAG,
                      String.format(
                        "%3d: id=%s skipped due to no PJSIP compatible format",
                        i, pjci.id));
                continue;
            }
            ...
            try {
                ...
                pjci.supportedSize =
                  new int[] { 176, 144, 320,  240, 352,  288,
            ...
            Log.i(TAG, String.format(
                         "%3d: id=%s facing=%d orient=%d formats=%s sizes=%s",
