...
static const struct
{
    ...
    PJ_BUILD_ERR(PJMEDIA_EVID_INVCAP,
                 "Invalid or unsupported video capability"),
    PJ_BUILD_ERR(PJMEDIA_EVID_INVOP,
                 "Invalid or unsupported video device operation"),
 ...
 */
PJ_DEF(pj_str_t)
pjmedia_videodev_strerror(pj_status_t statcode, char* buf, pj_size_t bufsize)
{
    pj_str_t errstr;
...
#    endif /* PJ_HAS_ERROR_STRING */
    ...
    errstr.slen = pj_ansi_snprintf(
      buf, bufsize, "Unknown pjmedia-videodev error %d", statcode);
