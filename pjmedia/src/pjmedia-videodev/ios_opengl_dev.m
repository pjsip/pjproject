...
/* Operations */
static pjmedia_vid_dev_stream_op stream_op = { &iosgl_stream_get_param,
...
- (void)init_gl
{
    ...
    eagl_layer.drawableProperties = [NSDictionary
...
#    if ALLOW_DELAYED_INITIALIZATION
    if (stream->status != PJ_SUCCESS) {
        if (stream->status == PJMEDIA_EVID_INIT) {
            [self init_gl];
            NSLog(@"Initializing OpenGL now %s",
                  stream->status == PJ_SUCCESS ? "success" : "failed");
        }
...
#    endif
    ...
    pjmedia_vid_dev_opengl_draw(stream->gl_buf, stream->vid_size.w,
                                stream->vid_size.h, stream->render_buf);
                                        ...
                                        const void* pval)
{
    ...
    if (cap == PJMEDIA_VID_DEV_CAP_FORMAT) {
        ...
        dispatch_sync_on_main_queue(^{
          strm->gl_view.bounds =
            CGRectMake(0, 0, strm->param.disp_size.w, strm->param.disp_size.h);
        });
        return PJ_SUCCESS;
        ...
        dispatch_sync_on_main_queue(^{
          strm->gl_view.center = CGPointMake(
        ...
        if (strm->param.orient == PJMEDIA_ORIENT_UNKNOWN)
            return PJ_SUCCESS;
        dispatch_sync_on_main_queue(^{
          strm->gl_view.transform = CGAffineTransformMakeRotation(
