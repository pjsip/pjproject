...
static pj_status_t
darwin_factory_create_stream(pjmedia_vid_dev_factory* f,
                             pjmedia_vid_dev_param* param,
...
static pjmedia_vid_dev_factory_op factory_op = {
    &darwin_factory_init,          &darwin_factory_destroy,
    &darwin_factory_get_dev_count, &darwin_factory_get_dev_info,
    &darwin_factory_default_param, &darwin_factory_create_stream,
    &darwin_factory_refresh
};

static pjmedia_vid_dev_stream_op stream_op = {
    &darwin_stream_get_param, &darwin_stream_get_cap,
    &darwin_stream_set_cap,   &darwin_stream_start,
    &darwin_stream_get_frame, &darwin_stream_put_frame,
    &darwin_stream_stop,      &darwin_stream_destroy
};
...
static pj_status_t darwin_factory_refresh(pjmedia_vid_dev_factory* f)
{
    ...
    qf->dev_info = (struct darwin_dev_info*)pj_pool_calloc(
      qf->pool, MAX_DEV_COUNT, sizeof(struct darwin_dev_info));
    qf->dev_count = 0;
...
#    if TARGET_OS_IPHONE
            qdi->info.caps =
              PJMEDIA_VID_DEV_CAP_INPUT_PREVIEW | PJMEDIA_VID_DEV_CAP_SWITCH;
#    endif
            qdi->dev = device;
        }
    }
    ...
    for (i = 0; i < qf->dev_count; i++) {
        qdi = &qf->dev_info[i];
        qdi->info.caps |=
          PJMEDIA_VID_DEV_CAP_FORMAT | PJMEDIA_VID_DEV_CAP_OUTPUT_WINDOW |
          PJMEDIA_VID_DEV_CAP_OUTPUT_RESIZE |
          PJMEDIA_VID_DEV_CAP_OUTPUT_POSITION |
          PJMEDIA_VID_DEV_CAP_OUTPUT_HIDE | PJMEDIA_VID_DEV_CAP_ORIENTATION;
        ...
        for (l = 0; l < PJ_ARRAY_SIZE(darwin_fmts); l++) {
            ...
            if (qdi->info.dir == PJMEDIA_DIR_RENDER) {
                ...
                pjmedia_format_init_video(fmt, darwin_fmts[l].pjmedia_format,
                ...
                for (m = 0; m < PJ_ARRAY_SIZE(darwin_sizes) &&
                            qdi->info.fmt_cnt < PJMEDIA_VID_DEV_INFO_FMT_CNT;
                     m++)
                {
                    if ([dev supportsAVCaptureSessionPreset:darwin_sizes[m]
                                                              .preset_str]) {
                        /* Landscape video */
                        fmt = &qdi->info.fmt[qdi->info.fmt_cnt++];
                        pjmedia_format_init_video(
                          fmt, darwin_fmts[l].pjmedia_format,
                          darwin_sizes[m].supported_size_w,
                          darwin_sizes[m].supported_size_h, DEFAULT_FPS, 1);
                        /* Portrait video */
                        fmt = &qdi->info.fmt[qdi->info.fmt_cnt++];
                        pjmedia_format_init_video(
                          fmt, darwin_fmts[l].pjmedia_format,
                          darwin_sizes[m].supported_size_h,
                          darwin_sizes[m].supported_size_w, DEFAULT_FPS, 1);
                    }
                }
            }
        }
    ...
    PJ_LOG(4, (THIS_FILE,
               "Darwin video initialized with %d devices:", qf->dev_count));
    for (i = 0; i < qf->dev_count; i++) {
        qdi = &qf->dev_info[i];
        PJ_LOG(
          4, (THIS_FILE, "%2d: [%s] %s - %s", i,
...
- (void)update_image
{
    ...
    CGImageRef cgIm = CGImageCreate(
      stream->size.w, stream->size.h, 8, 32, stream->bytes_per_row, colorSpace,
      kCGImageAlphaFirst | kCGBitmapByteOrder32Little,
      stream->render_data_provider, 0, false, kCGRenderingIntentDefault);
    ...
    if (stream->is_planar && stream->capture_buf) {
        if (stream->param.fmt.id == PJMEDIA_FORMAT_I420) {
            ...
            if (stream->vid_size.h > height) {
                pj_memset(Y, 16,
                          (stream->vid_size.h - height) * stream->vid_size.w);
            }
            ...
            if (stream->vid_size.h > height) {
                pj_size_t UV_size =
                  (stream->vid_size.h - height) * stream->vid_size.w / 4;
...
static pj_status_t darwin_init_view(struct darwin_stream* strm)
{
    ...
    CGRect view_rect =
      CGRectMake(0, 0, param->fmt.det.vid.size.w, param->fmt.det.vid.size.h);
...
#    if TARGET_OS_IPHONE
            ...
            [dev lockForConfiguration:NULL])
        {
            dev.activeVideoMinFrameDuration =
              CMTimeMake(vfd->fps.denum, vfd->fps.num);
...
#    endif
        ...
        NSError* error;
        strm->dev_input = [AVCaptureDeviceInput deviceInputWithDevice:dev
                                                                error:&error];
        ...
        strm->video_output.videoSettings = [NSDictionary
          dictionaryWithObjectsAndKeys:
#    if !TARGET_OS_IPHONE
            /* On Mac, we need to set the buffer's dimension
             * to avoid extra padding.
             */
            [NSNumber numberWithDouble:darwin_sizes[i].supported_size_w],
            (id)kCVPixelBufferWidthKey,
            [NSNumber numberWithDouble:darwin_sizes[i].supported_size_h],
...
#    endif
        ...
        strm->queue =
          dispatch_queue_create("vout_queue", DISPATCH_QUEUE_SERIAL);
        ...
        [[NSNotificationCenter defaultCenter]
          addObserver:strm->vout_delegate
        ...
        if (param->flags & PJMEDIA_VID_DEV_CAP_INPUT_PREVIEW) {
            darwin_stream_set_cap(&strm->base,
...
#    if TARGET_OS_IPHONE
        ...
        strm->render_data_provider = CGDataProviderCreateWithData(
          NULL, strm->render_buf, strm->frame_size, NULL);
#    endif
    }
...
#    endif
    ...
    case PJMEDIA_VID_DEV_CAP_SWITCH: {
        ...
        AVCaptureDeviceInput* new_dev_input =
          [AVCaptureDeviceInput deviceInputWithDevice:di[p->target_id].dev
                                                error:&error];
...
#    if TARGET_OS_IPHONE
    ...
    case PJMEDIA_VID_DEV_CAP_OUTPUT_RESIZE: {
        ...
        dispatch_sync_on_main_queue(^{
          ...
          r.size = CGSizeMake(strm->param.disp_size.w, strm->param.disp_size.h);
    ...
    case PJMEDIA_VID_DEV_CAP_OUTPUT_POSITION: {
        ...
        dispatch_sync_on_main_queue(^{
          strm->render_view.center = CGPointMake(
            strm->param.window_pos.x + strm->param.disp_size.w / 2.0,
            strm->param.window_pos.y + strm->param.disp_size.h / 2.0);
        });
        return PJ_SUCCESS;
    }
...
#    if TARGET_OS_IPHONE
            dispatch_sync_on_main_queue(^{
              strm->render_view.transform = CGAffineTransformMakeRotation(
...
#    endif
        ...
        vidcon = [strm->video_output connectionWithMediaType:AVMediaTypeVideo];
        ...
        if (!strm->conv.conv) {
            ...
            orig_size.w =
              (strm->size.w > strm->size.h ? strm->size.w : strm->size.h);
            orig_size.h =
              (strm->size.w > strm->size.h ? strm->size.h : strm->size.w);
            ...
            status = pjmedia_vid_dev_conv_create_converter(
              &strm->conv, strm->pool, &strm->param.fmt, orig_size, strm->size,
              (support_ori ? PJ_FALSE : PJ_TRUE), MAINTAIN_ASPECT_RATIO);
...
static pj_status_t darwin_stream_start(pjmedia_vid_dev_stream* strm)
{
    ...
    if (stream->cap_session) {
        [stream->cap_session performSelectorOnMainThread:@selector(startRunning)
                                              withObject:nil
                                           waitUntilDone:YES];
