...
#include <pj/os.h>
  ...
  defined(PJMEDIA_VIDEO_DEV_HAS_ANDROID) && PJMEDIA_VIDEO_DEV_HAS_ANDROID != 0
...
static pjmedia_vid_dev_factory_op factory_op = {
    &and_factory_init,          &and_factory_destroy,
    &and_factory_get_dev_count, &and_factory_get_dev_info,
    &and_factory_default_param, &and_factory_create_stream,
    &and_factory_refresh
};

static pjmedia_vid_dev_stream_op stream_op = { &and_stream_get_param,
                                               &and_stream_get_cap,
                                               &and_stream_set_cap,
                                               &and_stream_start,
                                               NULL,
...
#    if USE_CAMERA2
static void JNICALL OnGetFrame2(JNIEnv* env, jobject obj, jlong user_data,
                                jobject plane0, jint rowStride0,
                                jint pixStride0, jobject plane1,
                                jint rowStride1, jint pixStride1,
                                jobject plane2, jint rowStride2,
                                jint pixStride2);
#    else
static void JNICALL OnGetFrame(JNIEnv* env, jobject obj, jbyteArray data,
                               jint length, jlong user_data);
#    endif
...
#    define GET_CLASS(class_path, class_name, cls) \
        ...
        if (cls == NULL || (*jni_env)->ExceptionCheck(jni_env)) { \
            ...
            PJ_LOG( \
              3, (THIS_FILE, "[JNI] Unable to find class '" class_name "'")); \
            ...
            if (cls == NULL) { \
                PJ_LOG(3, (THIS_FILE, \
                           "[JNI] Unable to get global ref for " \
                           "class '" class_name "'")); \
                status = PJMEDIA_EVID_SYSERR; \
                goto on_return; \
            } \
...
#    define GET_SMETHOD_ID(cls, class_name, method_name, signature, id) \
        id = \
          (*jni_env)->GetStaticMethodID(jni_env, cls, method_name, signature); \
        if (id == 0) { \
            PJ_LOG(3, (THIS_FILE, \
                       "[JNI] Unable to find static method '" method_name \
                       "' in class '" class_name "'")); \
            status = PJMEDIA_EVID_SYSERR; \
            goto on_return; \
        }
#    define GET_FIELD_ID(cls, class_name, field_name, signature, id) \
    ...
    GET_METHOD_ID(jobjs.cam.cls, PJ_CAMERA, "<init>",
                  "(IIIIIJLandroid/view/SurfaceView;)V", jobjs.cam.m_init);
    GET_METHOD_ID(jobjs.cam.cls, PJ_CAMERA, "Start", "()I", jobjs.cam.m_start);
    GET_METHOD_ID(jobjs.cam.cls, PJ_CAMERA, "Stop", "()V", jobjs.cam.m_stop);
...
#    undef GET_FIELD_ID
    ...
    {
#    if USE_CAMERA2
        JNINativeMethod m = { "PushFrame2",
                              "(JLjava/nio/ByteBuffer;IILjava/nio/"
                              "ByteBuffer;IILjava/nio/ByteBuffer;II)V",
                              (void*)&OnGetFrame2 };
#    else
...
#    endif
        if ((*jni_env)->RegisterNatives(jni_env, jobjs.cam.cls, &m, 1)) {
            PJ_LOG(3, (THIS_FILE,
                       "[JNI] Failed in registering native "
                       "function 'OnGetFrame()'"));
            status = PJMEDIA_EVID_SYSERR;
        }
    }
...
static pj_status_t and_factory_refresh(pjmedia_vid_dev_factory* ff)
{
    ...
    f->dev_info = (and_dev_info*)pj_pool_calloc(f->dev_pool, dev_count,
                                                sizeof(and_dev_info));
    ...
    for (i = 0; i < dev_count; i++) {
        ...
        jdev_info = (*jni_env)->CallStaticObjectMethod(
          jni_env, jobjs.cam_info.cls, jobjs.cam_info.m_get_info, i);
        if (jdev_info == NULL)
            continue;

        /* Get camera facing: 0=back 1=front */
        facing =
          (*jni_env)->GetIntField(jni_env, jdev_info, jobjs.cam_info.f_facing);
        if (facing < 0)
            goto on_skip_dev;
        ...
        vdi->caps =
          PJMEDIA_VID_DEV_CAP_SWITCH | PJMEDIA_VID_DEV_CAP_ORIENTATION;
        ...
        if (jtmp) {
            ...
            for (j = 0; j < cnt; j++) {
                ...
                if (fmt == PJMEDIA_FORMAT_I420)
                    has_i420 = PJ_TRUE;
                else if (fmt == PJMEDIA_FORMAT_YV12)
                    adi->has_yv12 = PJ_TRUE;
                else if (fmt == PJMEDIA_FORMAT_NV21)
                    adi->has_nv21 = PJ_TRUE;
            }
            ...
            for (k = 0; k < adi->sup_size_cnt && vdi->fmt_cnt < max_fmt_cnt - 1;
                 k++) {
                /* Landscape video */
                pjmedia_format_init_video(
                  &vdi->fmt[vdi->fmt_cnt++], PJMEDIA_FORMAT_I420,
                  adi->sup_size[k].w, adi->sup_size[k].h, DEFAULT_FPS, 1);
                /* Portrait video */
                pjmedia_format_init_video(
                  &vdi->fmt[vdi->fmt_cnt++], PJMEDIA_FORMAT_I420,
                  adi->sup_size[k].h, adi->sup_size[k].w, DEFAULT_FPS, 1);
            }
...
#    if !USE_CAMERA2
            ...
            if (adi->has_yv12) {
                for (k = 0;
                     k < adi->sup_size_cnt && vdi->fmt_cnt < max_fmt_cnt - 1;
                     k++) {
                    /* Landscape video */
                    pjmedia_format_init_video(
                      &vdi->fmt[vdi->fmt_cnt++], PJMEDIA_FORMAT_YV12,
                      adi->sup_size[k].w, adi->sup_size[k].h, DEFAULT_FPS, 1);
                    /* Portrait video */
                    pjmedia_format_init_video(
                      &vdi->fmt[vdi->fmt_cnt++], PJMEDIA_FORMAT_YV12,
            ...
            if (adi->has_nv21) {
                for (k = 0;
                    ...
                    /* Landscape video */
                    pjmedia_format_init_video(
                      &vdi->fmt[vdi->fmt_cnt++], PJMEDIA_FORMAT_NV21,
                      adi->sup_size[k].w, adi->sup_size[k].h, DEFAULT_FPS, 1);
                    /* Portrait video */
                    pjmedia_format_init_video(
                      &vdi->fmt[vdi->fmt_cnt++], PJMEDIA_FORMAT_NV21,
...
#    endif
    ...
    PJ_LOG(4,
           (THIS_FILE, "Android video capture initialized with %d device(s):",
            f->dev_count));
    for (i = 0; i < f->dev_count; i++) {
        ...
        for (j = 0; j < adi->info.fmt_cnt; j++) {
            ...
            slen = pj_ansi_snprintf(p, plen, "%s/%dx%d ", tmp_str2, vfd->size.w,
                                    vfd->size.h);
            if (slen < 0 || slen >= plen)
                break;
            plen -= slen;
...
/* API: create stream */
static pj_status_t and_factory_create_stream(
  pjmedia_vid_dev_factory* ff, pjmedia_vid_dev_param* param,
  const pjmedia_vid_dev_cb* cb, void* user_data,
  pjmedia_vid_dev_stream** p_vid_strm)
{
    and_factory* f = (and_factory*)ff;
                                        ...
                                        pjmedia_vid_dev_param* pi)
{
    ...
    if (and_stream_get_cap(s, PJMEDIA_VID_DEV_CAP_OUTPUT_WINDOW, &pi->window) ==
        PJ_SUCCESS)
    {
                                      ...
                                      pjmedia_vid_dev_cap cap, const void* pval)
{
    ...
    case PJMEDIA_VID_DEV_CAP_SWITCH: {
        ...
        with_attach = jni_get_env(&jni_env);
        res = (*jni_env)->CallIntMethod(jni_env, strm->jcam, jobjs.cam.m_switch,
                                        adi->dev_idx);
        if (res < 0) {
    ...
    case PJMEDIA_VID_DEV_CAP_ORIENTATION: {
        ...
        if (!strm->conv.conv) {
            status = pjmedia_vid_dev_conv_create_converter(
              &strm->conv, strm->pool, &strm->param.fmt, strm->cam_size,
              strm->param.fmt.det.vid.size, PJ_TRUE, MAINTAIN_ASPECT_RATIO);
                                ...
                                jint pixStride2)
{
    ...
    if (strm->thread_initialized == 0 || !pj_thread_is_registered()) {
        ...
        status =
          pj_thread_register("and_cam", strm->thread_desc, &strm->thread);
        if (status != PJ_SUCCESS)
            return;
    ...
    else if (p1 - p2 == 1 && pixStride0 == 1 && pixStride1 == 2 &&
             pixStride2 == 2)
    {
        /* Strip out Y padding */
        if (rowStride0 > strm->cam_size.w) {
    ...
    else
    {
        ...
        PJ_LOG(1, (THIS_FILE,
                   "Unrecognized image format from Android camera2, "
                   "please report the following plane format:"));
        PJ_LOG(1, (THIS_FILE,
                   " Planes (buf/len/row_stride/pix_stride):"
                   " p0=%p/%d/%d/%d p1=%p/%d/%d/%d p2=%p/%d/%d/%d",
                   p0, p0_len, rowStride0, pixStride0, p1, p1_len, rowStride1,
                   pixStride1, p2, p2_len, rowStride2, pixStride2));
...
#        endif
    ...
    status =
      pjmedia_vid_dev_conv_resize_and_rotate(&strm->conv, f.buf, &frame_buf);
    if (status == PJ_SUCCESS) {
        f.buf = frame_buf;
    }
