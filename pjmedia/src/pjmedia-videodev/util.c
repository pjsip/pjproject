...
#    endif
    ...
    if (handle_rotation) {
        ...
        if (maintain_aspect_ratio) {
             ...
             */
            pj_size_t long_s =
              (conv->rot_size.h > conv->rot_size.w ? conv->rot_size.h :
                                                     conv->rot_size.w);
            pj_size_t short_s =
              (conv->rot_size.h > conv->rot_size.w ? conv->rot_size.w :
                                                     conv->rot_size.h);
    ...
    status = pjmedia_converter_create(NULL, pool, &conv_param, &conv->conv);
    ...
    conv->conv_frame_size = conv->rot_size.w * conv->rot_size.h * vfi->bpp / 8;
    conv->conv_buf = pj_pool_alloc(pool, conv->src_frame_size);

    pjmedia_vid_dev_conv_set_rotation(conv, PJMEDIA_ORIENT_NATURAL);

    PJ_LOG(4, (THIS_FILE,
               "Orientation converter created: %dx%d to %dx%d, "
               "maintain aspect ratio=%s",
               conv_param.src.det.vid.size.w, conv_param.src.det.vid.size.h,
               conv_param.dst.det.vid.size.w, conv_param.dst.det.vid.size.h,
...
#    if defined(PJMEDIA_HAS_LIBYUV) && PJMEDIA_HAS_LIBYUV != 0
            ...
            I420Rotate(src, src_size.w, src + p_len, src_size.w / 2,
                       src + p_len + p_len / 4, src_size.w / 2, dst, dst_size.w,
                       dst + p_len, dst_size.w / 2, dst + p_len + p_len / 4,
                       dst_size.w / 2, src_size.w, src_size.h, mode);
