...
static pj_status_t vid_event_cb(pjmedia_event* event, void* user_data)
{
...
static int capture_render_loopback(pj_bool_t active, int cap_dev_id,
                                   int rend_dev_id, const pjmedia_format* fmt)
{
    pj_pool_t* pool;
    ...
    PJ_LOG(3, (THIS_FILE, "  %s (%s) ===> %s (%s)\t%s\t%dx%d\t@%d:%d fps",
               cdi.name, cdi.driver, rdi.name, rdi.driver,
               pjmedia_get_video_format_info(NULL, fmt->id)->name,
               fmt->det.vid.size.w, fmt->det.vid.size.h, fmt->det.vid.fps.num,
               fmt->det.vid.fps.denum));
    ...
    status = pjmedia_vid_port_connect(
      ...
      pjmedia_vid_port_get_passive_port(active ? renderer : capture), PJ_FALSE);
    if (status != PJ_SUCCESS) {
        rc = 140;
        goto on_return;
    }
...
static int find_device(pjmedia_dir dir, pj_bool_t has_callback)
{
...
static int vidport_test(void)
{
    int i, j, k, l;
    int cap_id, rend_id;
    pjmedia_format_id test_fmts[] = { PJMEDIA_FORMAT_RGBA,
                                      PJMEDIA_FORMAT_I420 };
                ...
                for (l = 0; l < PJ_ARRAY_SIZE(test_fmts); ++l) {
                    ...
                    PJ_LOG(
                      ...
                      (THIS_FILE,
                       "capturer %s (stream: %s) ===> "
                       "renderer %s (stream: %s)",
                       (i ? "active" : "passive"), (j ? "active" : "passive"),
                       (i ? "passive" : "active"), (k ? "active" : "passive")));

                    pjmedia_format_init_video(&fmt, test_fmts[l], 640, 480, 25,
                                              1);
