...
#define RESET() \
    memset(buf1, 0, sizeof(buf1)), memset(buf2, 0, sizeof(buf2)), \
      memset(frm1, 0, sizeof(frm1)), memset(frm2, 0, sizeof(frm2))
...
static void save_file(const char* file, short frame[], unsigned count)
{
...
int compress(pj_pool_t* pool, const char* filein, const char* fileout,
             int rate10)
{
    enum
    {
        BUF_CNT = SAMPLES_PER_FRAME * 10
    };
    FILE *in, *out;
    pjmedia_wsola* wsola;
    short buf[BUF_CNT];
