...
static pj_status_t init()
{
    ...
    for (i = 0; i < pj_snd_get_dev_count(); ++i) {
        ...
        PJ_LOG(3, (THIS_FILE, "  %d: %s\t(%d in, %d out", i, info->name,
...
static int create_ses_by_remote_sdp(int local_port, char* sdp)
{
    ...
    skinfo.rtp_sock = skinfo.rtcp_sock =
      pj_sock_socket(pj_AF_INET(), pj_SOCK_DGRAM(), 0, 0);
    ...
    if (pj_sock_bind(skinfo.rtp_sock,
                     (struct pj_sockaddr*)&skinfo.rtp_addr_name,
    ...
    for (i = 0; i < count; ++i) {
        ...
        printf("  Stream %d: %.*s %s local=%s:%d remote=%.*s:%d\n", i,
               info[i]->type.slen, info[i]->type.ptr, dir, local_ip,
               pj_sockaddr_get_port(&info[i]->sock_info.rtp_addr_name),
               info[i]->rem_addr.slen, info[i]->rem_addr.ptr,
               info[i]->rem_port);
    }
