 ...
 */
static int codec_test_encode(pjmedia_codec_mgr* mgr, char* codec_name,
                             unsigned bitrate, const char* wav_file,
                             const char* ref_encoded_file)
{
    ...
    count = 1;
    status =
      pjmedia_codec_mgr_find_codecs_by_id(mgr, &codec_id, &count, ci, NULL);
    ...
    /* Open WAV file */
    status =
      pjmedia_wav_player_port_create(pool, wav_file, codec_param.info.frm_ptime,
                                     PJMEDIA_FILE_NO_LOOP, 0, &wav_port);
    if (status != PJ_SUCCESS) {
        rc = -80;
        goto on_return;
    }
    ...
    samples_per_frame =
      codec_param.info.clock_rate * codec_param.info.frm_ptime / 1000;
    ...
    for (;;) {
        ...
        status =
          pjmedia_codec_encode(codec, &in_frame, samples_per_frame, &out_frame);
        if (status != PJ_SUCCESS) {
            rc = -95;
            goto on_return;
        }
 ...
 */
static int read_ITU_format(FILE* fp_bitstream, short* out_words,
 ...
 */
static int codec_test_decode(pjmedia_codec_mgr* mgr, char* codec_name,
                             unsigned bitrate, unsigned encoded_len,
                             ...
                             void (*manip)(short* pcm, unsigned count))
{
    ...
    is_itu_format =
      pj_ansi_stricmp(in_encoded_file + strlen(in_encoded_file) - 4, ".itu") ==
      0;
    ...
    for (;;) {
        ...
        if (has_frame) {
            count2 = 2;
            if (pjmedia_codec_parse(codec, pkt, encoded_len, &ts, &count2,
                                    in_frame) != PJ_SUCCESS)
            {
                rc = -100;
                goto on_return;
    ...
    for (;;) {
        ...
        if (memcmp(pkt, out_frame.buf, samples_per_frame * 2)) {
            ...
            PJ_LOG(1,
                   (THIS_FILE, "     failed: mismatch at samples %d", pos + i));
            rc = -200;
            break;
        }
...
int codec_test_vectors(void)
{
...
#if PJMEDIA_HAS_G7221_CODEC
        { "G7221/16000/1", 24000, "../src/test/vectors/g722_1_enc_in.wav",
          "../src/test/vectors/g722_1_enc_out_24000_be.pak" },
        { "G7221/16000/1", 32000, "../src/test/vectors/g722_1_enc_in.wav",
...
#if PJMEDIA_HAS_G7221_CODEC
        { "G7221/16000/1", 24000, 60, &g7221_pcm_manip,
          "../src/test/vectors/g722_1_enc_out_24000_be.pak",
          "../src/test/vectors/g722_1_dec_out_24000.pcm" },
        { "G7221/16000/1", 32000, 80, &g7221_pcm_manip,
          "../src/test/vectors/g722_1_enc_out_32000_be.pak",
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(enc_vectors); ++i) {
        ...
        PJ_LOG(3, (THIS_FILE, "    %s @%d bps %s ==> %s",
                   enc_vectors[i].codec_name, enc_vectors[i].bit_rate,
                   enc_vectors[i].wav_file, enc_vectors[i].ref_encoded_file));
        rc = codec_test_encode(mgr, enc_vectors[i].codec_name,
                               enc_vectors[i].bit_rate, enc_vectors[i].wav_file,
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(dec_vectors); ++i) {
        if (!dec_vectors[i].codec_name)
            continue;
        PJ_LOG(3, (THIS_FILE, "    %s @%d bps %s ==> %s",
                   dec_vectors[i].codec_name, dec_vectors[i].bit_rate,
                   dec_vectors[i].enc_file, dec_vectors[i].ref_pcm_file));
        rc = codec_test_decode(
          mgr, dec_vectors[i].codec_name, dec_vectors[i].bit_rate,
          dec_vectors[i].encoded_frame_len, dec_vectors[i].enc_file,
          dec_vectors[i].ref_pcm_file, dec_vectors[i].manip);
        if (rc != 0)
            rc_final = rc;
    }
