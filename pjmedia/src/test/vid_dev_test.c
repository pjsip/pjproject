    ...
    for (i = 0; i < dev_cnt; ++i) {
        ...
        if (status == PJ_SUCCESS) {
            ...
            for (j = 0; j < di.fmt_cnt; ++j) {
                ...
                PJ_LOG(3,
                       (THIS_FILE, "       %s", (vfi ? vfi->name : "unknown")));
            }
        }
    }
                                   ...
                                   const pjmedia_format* fmt)
{
    ...
    status = pjmedia_vid_port_connect(
      capture, pjmedia_vid_port_get_passive_port(renderer), PJ_FALSE);
    if (status != PJ_SUCCESS) {
        rc = 140;
        goto on_return;
    }
                ...
                for (l = 0; l < PJ_ARRAY_SIZE(test_sizes); ++l) {
                    ...
                    for (m = 0; m < PJ_ARRAY_SIZE(test_fpses); ++m) {
                        ...
                        pjmedia_format_init_video(
                          &fmt, test_fmts[k], test_sizes[l].w, test_sizes[l].h,
                          test_fpses[m].num, test_fpses[m].denum);
