...
static struct test
{
    ...
    { /*********************************************************************
      ...
      { {
          ...
          "a=rtpmap:0 PCMU/8000\r\n"
          "a=rtpmap:8 PCMA/8000\r\n"
          // By #1088, the formats won't be negotiated when the media has port
          // 0.
          //"m=video 0 RTP/AVP 31\r\n"
...
static const char* find_diff(const char* s1, const char* s2, int* line)
{
    *line = 1;
    while (*s2 && *s1) {
        if (*s2 != *s1)
...
static int compare_sdp_string(const char* cmp_title, const char* title1,
                              ...
                              pj_status_t logical_cmp)
{
    ...
    if (logical_cmp != PJ_SUCCESS) {
        ...
        PJ_LOG(3, (THIS_FILE,
                   ...
                   "%s:\n"
                   "%s\n",
                   cmp_title, errbuf, title1, sdpbuf1, title2, sdpbuf2));
        ...
        PJ_LOG(3, (THIS_FILE,
                   ...
                   "%s:\n"
                   "%s\n",
                   cmp_title, title1, sdpbuf1, title2, sdpbuf2));
                             ...
                             struct offer_answer* oa)
{
    ...
    status = pjmedia_sdp_parse(pool, oa->sdp1, pj_ansi_strlen(oa->sdp1), &sdp1);
    ...
    if (oa->type == LOCAL_OFFER) {
        ...
        if (neg == NULL) {
            ...
            if (status != PJ_SUCCESS) {
                app_perror(status,
                           "   error: pjmedia_sdp_neg_create_w_local_offer");
                return -20;
            }
            *p_neg = neg;
            ...
            if (status != PJ_SUCCESS) {
                app_perror(status,
                           "   error: pjmedia_sdp_neg_modify_local_offer");
                return -30;
            }
        }
        ...
        status =
          pjmedia_sdp_parse(pool, oa->sdp2, pj_ansi_strlen(oa->sdp2), &sdp2);
        if (status != PJ_SUCCESS) {
        ...
        status =
          pjmedia_sdp_parse(pool, oa->sdp3, pj_ansi_strlen(oa->sdp3), &sdp3);
        if (status != PJ_SUCCESS) {
        ...
        if (status != PJ_SUCCESS) {
            ...
            compare_sdp_string("Logical cmp after negotiatin remote answer",
                               ...
                               "The correct active local sdp", sdp3, status);
            return -110;
        }

        /* Compare the string representation oa both sdps */
        status =
          compare_sdp_string("String cmp after negotiatin remote answer",
                             "Active local sdp from negotiator", active,
                             "The correct active local sdp", sdp3, PJ_SUCCESS);
        if (status != 0)
            return -120;
        ...
        if (neg == NULL) {
            ...
            status =
              pjmedia_sdp_neg_create_w_remote_offer(pool, sdp2, sdp1, &neg);
            if (status != PJ_SUCCESS) {
                app_perror(status,
                           "   error: pjmedia_sdp_neg_create_w_remote_offer");
                return -220;
            }
            *p_neg = neg;
            ...
            if (status != PJ_SUCCESS) {
                app_perror(status,
                           "   error: pjmedia_sdp_neg_set_local_answer");
                return -235;
            }
        }
        ...
        if (status != PJ_SUCCESS) {
            compare_sdp_string("Logical cmp after negotiating remote offer",
                               ...
                               "The correct local answer", sdp3, status);
        ...
        status =
          compare_sdp_string("String cmp after negotiating remote offer",
                             "Local answer from negotiator", answer,
                             "The correct local answer", sdp3, PJ_SUCCESS);
        if (status != 0)
            return -290;
    }
...
static int perform_test(pj_pool_t* pool, int test_index)
{
    ...
    for (i = 0; i < test[test_index].offer_answer_count; ++i) {
        ...
        if (rc != 0) {
            PJ_LOG(
              3, (THIS_FILE, "  test %d offer answer %d failed with status %d",
                  test_index, i, rc));
            return rc;
        }
    }
