...
static pj_status_t codec_on_event(pjmedia_event* event, void* user_data)
{
    ...
    if (event->type == PJMEDIA_EVENT_FMT_CHANGED) {
        ...
        status = pjmedia_vid_dev_stream_set_cap(
          ...
          PJMEDIA_VID_DEV_CAP_FORMAT, &codec_param.dec_fmt);
        if (status != PJ_SUCCESS)
            return status;
    }
...
static pj_status_t codec_put_frame(pjmedia_port* port, pjmedia_frame* frame)
{
    enum
    {
        MAX_PACKETS = 50
    ...
    status = pjmedia_port_put_frame(
      pjmedia_vid_port_get_passive_port(port_data->rdr_port), frame);
    if (status != PJ_SUCCESS)
        goto on_error;
...
static int enum_codecs()
{
    ...
    for (i = 0; i < cnt; ++i) {
        PJ_LOG(3, (THIS_FILE, "  %-16.*s %c%c %s", info[i].encoding_name.slen,
                   info[i].encoding_name.ptr,
...
#    endif
    ...
    /* Prepare codec */
    {
        ...
        pjmedia_event_subscribe(NULL, &codec_on_event, &codec_port_data, codec);
    }
    ...
    status =
      pjmedia_port_info_init2(&codec_port.info, &port_name, 0x1234,
                              PJMEDIA_DIR_ENCODING, &codec_param.dec_fmt);
    if (status != PJ_SUCCESS) {
        rc = 260;
        goto on_return;
    }
    ...
    codec_port_data.enc_buf = pj_pool_alloc(pool, codec_port_data.enc_buf_size);
    codec_port_data.pack_buf_size = codec_port_data.enc_buf_size;
    codec_port_data.pack_buf =
      pj_pool_alloc(pool, codec_port_data.pack_buf_size);

    codec_port.put_frame = &codec_put_frame;
    codec_port.port_data.pdata = &codec_port_data;

    /* Connect capture to codec port */
    status = pjmedia_vid_port_connect(capture, &codec_port, PJ_FALSE);
    if (status != PJ_SUCCESS) {
        rc = 270;
        goto on_return;
    }
