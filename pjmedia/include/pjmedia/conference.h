...
enum pjmedia_conf_option
{
    ...
    PJMEDIA_CONF_SMALL_FILTER =
      4,                        /**< Use small filter table when resampling */
 ...
 */
PJ_DECL(pj_status_t)
pjmedia_conf_create(pj_pool_t* pool, unsigned max_slots, unsigned sampling_rate,
...
pjmedia_conf_add_port(pjmedia_conf* conf, pj_pool_t* pool,
                      pjmedia_port* strm_port, const pj_str_t* name,
                      unsigned* p_slot);
 ...
 */
PJ_DECL(pj_status_t)
pjmedia_conf_remove_port(pjmedia_conf* conf, unsigned slot);
 ...
 */
PJ_DECL(pj_status_t)
pjmedia_conf_enum_ports(pjmedia_conf* conf, unsigned ports[], unsigned* count);
