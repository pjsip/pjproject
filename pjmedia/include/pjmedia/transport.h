 ...
 *    for UDP media transport, it is created with
 ...
 *
 *
 * \subsection PJMEDIA_TRANSPORT_H_ATTACH Attaching and Using the Media
 Transport.
 *
 ...
 * \section offer_answer Interaction with SDP Offer/Answer
   ...
   for subsequent media session by repeating the
 #pjmedia_transport_media_create(), #pjmedia_transport_encode_sdp(),
 #pjmedia_transport_media_start(), and #pjmedia_transport_media_stop() above.
...
struct pjmedia_transport_op
{
     ...
     */
    pj_status_t (*attach)(
      pjmedia_transport* tp, void* user_data, const pj_sockaddr_t* rem_addr,
     ...
     * calling this function directly.
     */
    pj_status_t (*send_rtp)(pjmedia_transport* tp, const void* pkt,
                            pj_size_t size);
     ...
     * calling this function directly.
     */
    pj_status_t (*send_rtcp)(pjmedia_transport* tp, const void* pkt,
                             pj_size_t size);
     ...
     * calling this function directly.
     */
    pj_status_t (*send_rtcp2)(pjmedia_transport* tp, const pj_sockaddr_t* addr,
     ...
     * calling this function directly.
     */
    pj_status_t (*media_create)(pjmedia_transport* tp, pj_pool_t* sdp_pool,
                                unsigned options,
     ...
     * calling this function directly.
     */
    pj_status_t (*encode_sdp)(pjmedia_transport* tp, pj_pool_t* sdp_pool,
     ...
     * calling this function directly.
     */
    pj_status_t (*media_start)(pjmedia_transport* tp, pj_pool_t* tmp_pool,
     ...
     * calling this function directly.
     */
    pj_status_t (*simulate_lost)(pjmedia_transport* tp, pjmedia_dir dir,
                                 unsigned pct_lost);
...
struct pjmedia_transport_info
{
     ...
     */
    pjmedia_transport_specific_info
      spc_info[PJMEDIA_TRANSPORT_SPECIFIC_INFO_MAXCNT];
};
...
pjmedia_transport_get_info(pjmedia_transport* tp, pjmedia_transport_info* info)
{
 ...
 */
PJ_INLINE(void*)
pjmedia_transport_info_get_spc_info(pjmedia_transport_info* info,
                                    pjmedia_transport_type type)
{
    unsigned i;
 ...
 */
PJ_INLINE(pjmedia_transport*)
pjmedia_transport_info_get_transport(pjmedia_transport_info* info,
                                     pjmedia_transport_type type)
{
    unsigned i;
    for (i = 0; i < info->specific_info_cnt; ++i) {
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_transport_attach2(pjmedia_transport* tp,
                          pjmedia_transport_attach_param* att_param)
{
    if (tp->op->attach2) {
        ...
        return (*tp->op->attach)(
          tp, att_param->user_data, (pj_sockaddr_t*)&att_param->rem_addr,
          (pj_sockaddr_t*)&att_param->rem_rtcp, att_param->addr_len,
          att_param->rtp_cb, att_param->rtcp_cb);
    }
}
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_transport_attach(pjmedia_transport* tp, void* user_data,
                         const pj_sockaddr_t* rem_addr,
                         const pj_sockaddr_t* rem_rtcp, unsigned addr_len,
                         void (*rtp_cb)(void* user_data, void* pkt, pj_ssize_t),
                         void (*rtcp_cb)(void* usr_data, void* pkt, pj_ssize_t))
{
    if (tp->op->attach2) {
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_transport_send_rtp(pjmedia_transport* tp, const void* pkt,
                           pj_size_t size)
{
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_transport_send_rtcp(pjmedia_transport* tp, const void* pkt,
                            pj_size_t size)
{
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_transport_send_rtcp2(pjmedia_transport* tp, const pj_sockaddr_t* addr,
                             unsigned addr_len, const void* pkt, pj_size_t size)
{
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_transport_media_create(pjmedia_transport* tp, pj_pool_t* sdp_pool,
                               unsigned options,
                               const pjmedia_sdp_session* rem_sdp,
                               unsigned media_index)
{
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_transport_encode_sdp(pjmedia_transport* tp, pj_pool_t* sdp_pool,
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_transport_media_start(pjmedia_transport* tp, pj_pool_t* tmp_pool,
                              const pjmedia_sdp_session* sdp_local,
                              const pjmedia_sdp_session* sdp_remote,
                              unsigned media_index)
{
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_transport_simulate_lost(pjmedia_transport* tp, pjmedia_dir dir,
                                unsigned pct_lost)
{
