...
pjmedia_circ_buf_create(pj_pool_t* pool, unsigned capacity,
 ...
 */
PJ_INLINE(void)
pjmedia_circ_buf_set_len(pjmedia_circ_buf* circbuf, unsigned len)
{
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_circ_buf_adv_read_ptr(pjmedia_circ_buf* circbuf, unsigned count)
{
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_circ_buf_adv_write_ptr(pjmedia_circ_buf* circbuf, unsigned count)
{
 ...
 */
PJ_INLINE(void)
pjmedia_circ_buf_get_read_regions(pjmedia_circ_buf* circbuf, pj_int16_t** reg1,
                                  unsigned* reg1_len, pj_int16_t** reg2,
                                  unsigned* reg2_len)
{
    *reg1 = circbuf->start;
    *reg1_len = circbuf->len;
    if (*reg1 + *reg1_len > circbuf->buf + circbuf->capacity) {
        *reg1_len =
          (unsigned)(circbuf->buf + circbuf->capacity - circbuf->start);
        *reg2 = circbuf->buf;
    ...
    PJMEDIA_CIRC_BUF_CHECK(*reg1_len != 0 ||
                           (*reg1_len == 0 && circbuf->len == 0));
 ...
 */
PJ_INLINE(void)
pjmedia_circ_buf_get_write_regions(pjmedia_circ_buf* circbuf, pj_int16_t** reg1,
                                   unsigned* reg1_len, pj_int16_t** reg2,
                                   unsigned* reg2_len)
{
    ...
    *reg1_len = circbuf->capacity - circbuf->len;
    ...
    PJMEDIA_CIRC_BUF_CHECK(*reg1_len != 0 ||
                           (*reg1_len == 0 && circbuf->len == 0));
    PJMEDIA_CIRC_BUF_CHECK(*reg1_len + *reg2_len ==
                           circbuf->capacity - circbuf->len);
}
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_circ_buf_read(pjmedia_circ_buf* circbuf, pj_int16_t* data,
                      unsigned count)
{
    ...
    pjmedia_circ_buf_get_read_regions(circbuf, &reg1, &reg1cnt, &reg2,
                                      &reg2cnt);
    if (reg1cnt >= count) {
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_circ_buf_write(pjmedia_circ_buf* circbuf, pj_int16_t* data,
                       unsigned count)
{
    pj_int16_t *reg1, *reg2;
    unsigned reg1cnt, reg2cnt;
    ...
    pjmedia_circ_buf_get_write_regions(circbuf, &reg1, &reg1cnt, &reg2,
                                       &reg2cnt);
    if (reg1cnt >= count) {
 ...
 */
PJ_INLINE(pj_status_t)
pjmedia_circ_buf_copy(pjmedia_circ_buf* circbuf, unsigned start_idx,
                      pj_int16_t* data, unsigned count)
{
    pj_int16_t *reg1, *reg2;
    unsigned reg1cnt, reg2cnt;
