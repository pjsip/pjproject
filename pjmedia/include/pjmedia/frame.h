 ...
 */
PJ_INLINE(void) pjmedia_frame_copy(pjmedia_frame* dst, const pjmedia_frame* src)
{
    dst->type = src->type;
 ...
 */
PJ_INLINE(void)
pjmedia_frame_ext_append_subframe(pjmedia_frame_ext* frm, const void* src,
                                  unsigned bitlen, unsigned samples_cnt)
{
 ...
 */
PJ_INLINE(unsigned)
pjmedia_frame_ext_copy_payload(const pjmedia_frame_ext* frm, void* dst,
                               unsigned maxlen)
{
    unsigned i, copied = 0;
    for (i = 0; i < frm->subframe_cnt; ++i) {
...
pjmedia_frame_ext_pop_subframes(pjmedia_frame_ext* frm, unsigned n)
{
    ...
    move_len = ((pj_uint8_t*)sf - move_src + sizeof(sf->bitlen) +
                ((sf->bitlen + 7) >> 3));
    pj_memmove((pj_uint8_t*)frm + sizeof(pjmedia_frame_ext), move_src,
               move_len);

    frm->samples_cnt = (pj_uint16_t)(frm->samples_cnt -
                                     n * frm->samples_cnt / frm->subframe_cnt);
 ...
 */
PJ_INLINE(void)
pjmedia_copy_samples(pj_int16_t* dst, const pj_int16_t* src, unsigned count)
{
#if 1
 ...
 */
PJ_INLINE(void)
pjmedia_move_samples(pj_int16_t* dst, const pj_int16_t* src, unsigned count)
{
#if 1
