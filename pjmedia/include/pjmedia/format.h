 ...
 */
PJ_INLINE(unsigned)
PJMEDIA_SPF(unsigned clock_rate, unsigned usec_ptime, unsigned channel_count)
{
#if PJ_HAS_INT64
    return ((unsigned)((pj_uint64_t)usec_ptime * clock_rate * channel_count /
                       1000000));
#elif PJ_HAS_FLOATING_POINT
    return (
      (unsigned)(1.0 * usec_ptime * clock_rate * channel_count / 1000000));
#else
    return ((unsigned)(usec_ptime / 1000L * clock_rate * channel_count / 1000));
#endif
}
 ...
 */
PJ_INLINE(unsigned)
PJMEDIA_SPF2(unsigned clock_rate, const pjmedia_ratio* fr,
             unsigned channel_count)
{
#if PJ_HAS_INT64
    return ((unsigned)((pj_uint64_t)clock_rate * fr->denum / fr->num /
                       channel_count));
#elif PJ_HAS_FLOATING_POINT
...
#if PJ_HAS_INT64
    return (
      (unsigned)((pj_uint64_t)1000000 * frame_rate->denum / frame_rate->num));
#elif PJ_HAS_FLOATING_POINT
    return ((unsigned)(1000000.0 * frame_rate->denum / frame_rate->num));
#else
    return ((unsigned)((1000L * frame_rate->denum / frame_rate->num) * 1000));
#endif
}
 ...
 */
PJ_INLINE(void)
pjmedia_format_init_audio(pjmedia_format* fmt, pj_uint32_t fmt_id,
                          unsigned clock_rate, unsigned channel_count,
                          unsigned bits_per_sample, unsigned frame_time_usec,
