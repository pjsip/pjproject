...
public:
     ...
     */
    explicit DeferredResponse(const OnInstantMessageParam& param)  PJSUA2_THROW(Error);
    ...
    DeferredResponse();
      ...
      */
    DeferredResponse(const DeferredResponse& deferredResponse) PJSUA2_THROW(Error);

    /**
     * Copies the deferred response object by cloning the rx_data.
     */
    DeferredResponse& operator=(const DeferredResponse& deferredResponse) PJSUA2_THROW(Error);
