...
#define copy_advance_pair_quote_check(buf, str1, len1, str2, quotebegin, \
                                      quoteend) \
    do { \
        if (str2.slen) { \
...
#define copy_advance_pair_escape(buf, str1, len1, str2, unres) \
    do { \
        if (str2.slen) { \
            if (len1 + str2.slen >= (endbuf - buf)) \
                return -1; \
            pj_memcpy(buf, str1, len1); \
            printed = (int)pj_strncpy2_escape(buf + len1, &str2, \
                                              (endbuf - buf - len1), &unres); \
            if (printed < 0) \
                return -1; \
            buf += (printed + len1); \
        } \
 ...
 */
#define copy_advance_pair_quote_cond(buf, str1, len1, str2, quotebegin, \
                                     quoteend) \
    do { \
        if (str2.slen && *str2.ptr != quotebegin) \
            copy_advance_pair_quote(buf, str1, len1, str2, quotebegin, \
                                    quoteend); \
        else \
...
#define copy_advance_pair_quote_cond_always(buf, str1, len1, str2, quotebegin, \
    ...
    do { \
        if (!str2.slen) \
            ...
            copy_advance_pair_quote_cond(buf, str1, len1, str2, quotebegin, \
                                         quoteend); \
    } while (0)
