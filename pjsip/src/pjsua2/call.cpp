...
MathStat::MathStat() : n(0), max(0), min(0), last(0), mean(0)
{}
...
CallSendRequestParam::CallSendRequestParam() : method("")
{}
...
void CallSetting::fromPj(const pjsua_call_setting& prm)
{
    ...
    for (mi = PJMEDIA_MAX_SDP_MEDIA - 1; mi >= 0; mi--) {
        if (prm.media_dir[mi] != PJMEDIA_DIR_ENCODING_DECODING)
            break;
    }
    for (i = 0; i <= mi; i++) {
                       ...
                       const string& sdp_str)
{
    ...
    if (sdp_str != "") {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(
              4, (THIS_FILE, status, "Failed to parse SDP for call param"));
        }
    }
}
...
AudioMedia Call::getAudioMedia(int med_idx) const PJSUA2_THROW(Error)
{
    ...
    if (pj_ci.media[med_idx].type != PJMEDIA_TYPE_AUDIO) {
        PJSUA2_RAISE_ERROR3(PJ_EINVAL, "getAudioMedia()", "media is not audio");
    }
...
pjsip_dialog_cap_status Call::remoteHasCap(int htype, const string& hname,
                                           const string& token) const
{
    pj_str_t pj_hname = str2Pj(hname);
    pj_str_t pj_token = str2Pj(token);

    return pjsua_call_remote_has_cap(
      id, htype, (htype == PJSIP_H_OTHER) ? &pj_hname : NULL, &pj_token);
}
  ...
  PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(pjsua_call_make_call(
      acc.getId(), &pj_dst_uri, param.p_opt, this, param.p_msg_data, &id));
}

void Call::answer(const CallOpParam& prm) PJSUA2_THROW(Error)
{
    call_param param(prm.txOption, prm.opt, prm.reason, sdp_pool,
                     prm.sdp.wholeSdp);

    if (param.sdp) {
        PJSUA2_CHECK_EXPR(
          pjsua_call_answer_with_sdp(id, param.sdp, param.p_opt, prm.statusCode,
...
void Call::hangup(const CallOpParam& prm) PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(
      pjsua_call_hangup(id, prm.statusCode, param.p_reason, param.p_msg_data));
}
...
void Call::xfer(const string& dest, const CallOpParam& prm) PJSUA2_THROW(Error)
{
...
void Call::xferReplaces(const Call& dest_call, const CallOpParam& prm)
  PJSUA2_THROW(Error)
{
    call_param param(prm.txOption);
  ...
  PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(pjsua_call_send_typing_ind(
      id, (prm.isTyping ? PJ_TRUE : PJ_FALSE), param.p_msg_data));
}
...
string Call::dump(bool with_media, const string indent) PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(pjsua_call_dump(id, (with_media ? PJ_TRUE : PJ_FALSE),
                                      buffer, sizeof(buffer), indent.c_str()));
  ...
  PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(pjsua_call_get_med_transport_info(id, med_idx, &pj_mti));
    mti.fromPj(pj_mti);
    return mti;
}
