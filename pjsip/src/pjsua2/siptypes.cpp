...
AuthCredInfo::AuthCredInfo() : scheme("digest"), realm("*"), dataType(0)
{}

AuthCredInfo::AuthCredInfo(const string& param_scheme,
                           const string& param_realm,
                           const string& param_user_name,
                           const int param_data_type, const string param_data)
    : scheme(param_scheme),
      realm(param_realm),
...
TlsConfig::TlsConfig()
    : method(PJSIP_SSL_UNSPECIFIED_METHOD), qosType(PJ_QOS_TYPE_BEST_EFFORT)
{
...
pjsip_tls_setting TlsConfig::toPj() const
{
    ...
    ts.ciphers = ts.ciphers_num ? (pj_ssl_cipher*)&this->ciphers[0] : NULL;
...
void TransportConfig::writeObject(ContainerNode& node) const PJSUA2_THROW(Error)
{
...
void TransportInfo::fromPj(const pjsua_transport_info& tinfo)
{
    ...
    pj_ansi_snprintf(straddr, sizeof(straddr), "%.*s:%d",
                     (int)tinfo.local_name.host.slen, tinfo.local_name.host.ptr,
...
SipRxData::SipRxData() : pjRxData(NULL)
{}
...
void SipHeader::fromPj(const pjsip_hdr* hdr) PJSUA2_THROW(Error)
{
    ...
    char* end_name = pos;
    while (end_name > buf && pj_isspace(*(end_name - 1)))
        --end_name;

    // Trim whitespaces after colon
    char* start_val = pos + 1;
    while (*start_val && pj_isspace(*start_val))
        ++start_val;
...
SipEvent::SipEvent() : type(PJSIP_EVENT_UNKNOWN), pjEvent(NULL)
{}
...
void SipEvent::fromPj(const pjsip_event& ev)
{
    ...
    if (type == PJSIP_EVENT_TIMER) {
        ...
        body.tsxState.prevState =
          (pjsip_tsx_state_e)ev.body.tsx_state.prev_state;
...
SipTxData::SipTxData() : pjTxData(NULL)
{}
...
SipTransaction::SipTransaction()
    : role(PJSIP_ROLE_UAC),
      statusCode(PJSIP_SC_NULL),
      state(PJSIP_TSX_STATE_NULL),
      pjTransaction(NULL)
{}
...
SendTypingIndicationParam::SendTypingIndicationParam() : isTyping(false)
{}
