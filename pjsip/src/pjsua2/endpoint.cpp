...
TlsInfo::TlsInfo() : cipher(PJ_TLS_UNKNOWN_CIPHER), empty(true)
{}
...
void TlsInfo::fromPj(const pjsip_tls_state_info& info)
{
    ...
    pj_ssl_cert_get_verify_status_strings(ssock_info->verify_status, verif_msgs,
                                          &verif_msg_cnt);
...
SslCertInfo::SslCertInfo() : empty(true)
{}
...
///////////////////////////////////////////////////////////////////////////////
UaConfig::UaConfig() : mainThreadOnly(false)
{
    pjsua_config ua_cfg;
...
pjsua_config UaConfig::toPj() const
{
    ...
    for (i = 0;
         i < this->nameserver.size() && i < PJ_ARRAY_SIZE(pua_cfg.nameserver);
         ++i)
    {
    ...
    for (i = 0;
         i < this->stunServer.size() && i < PJ_ARRAY_SIZE(pua_cfg.stun_srv);
         ++i)
    {
    ...
    for (i = 0; i < this->outboundProxies.size() &&
                i < PJ_ARRAY_SIZE(pua_cfg.outbound_proxy);
         ++i)
    {
...
void Endpoint::on_timer(pj_timer_heap_t* timer_heap, pj_timer_entry* entry)
{
...
void Endpoint::on_pager2(pjsua_call_id call_id, const pj_str_t* from,
...
void Endpoint::on_pager_status2(pjsua_call_id call_id, const pj_str_t* to,
                                const pj_str_t* body, void* user_data,
                                pjsip_status_code status,
                                const pj_str_t* reason, pjsip_tx_data* tdata,
                                pjsip_rx_data* rdata, pjsua_acc_id acc_id)
{
    PJ_UNUSED_ARG(tdata);
...
void Endpoint::on_typing2(pjsua_call_id call_id, const pj_str_t* from,
                          const pj_str_t* to, const pj_str_t* contact,
...
void Endpoint::on_mwi_info(pjsua_acc_id acc_id, pjsua_mwi_info* mwi_info)
{
    OnMwiInfoParam prm;
...
void Endpoint::on_buddy_evsub_state(pjsua_buddy_id buddy_id, pjsip_evsub* sub,
                                    pjsip_event* event)
{
    PJ_UNUSED_ARG(sub);
...
void Endpoint::on_call_tsx_state(pjsua_call_id call_id, pjsip_transaction* tsx,
                                 pjsip_event* e)
{
    PJ_UNUSED_ARG(tsx);
...
void Endpoint::on_call_sdp_created(pjsua_call_id call_id,
                                   pjmedia_sdp_session* sdp, pj_pool_t* pool,
                                   const pjmedia_sdp_session* rem_sdp)
{
    ...
    if (orig_sdp != prm.sdp.wholeSdp) {
        ...
        status =
          pjmedia_sdp_parse(pool, dup_new_sdp.ptr, dup_new_sdp.slen, &new_sdp);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4,
                      (THIS_FILE, status, "Failed to parse the modified SDP"));
        } else {
                                   ...
                                   pjsua_on_stream_precreate_param* param)
{
    ...
    if (param->stream_info.type == PJMEDIA_TYPE_AUDIO) {
        ...
        param->stream_info.info.aud.jb_discard_algo =
          prm.streamInfo.jbDiscardAlgo;
#if defined(PJMEDIA_STREAM_ENABLE_KA) && (PJMEDIA_STREAM_ENABLE_KA != 0)
        param->stream_info.info.aud.use_ka = prm.streamInfo.useKa;
#endif
        param->stream_info.info.aud.rtcp_sdes_bye_disabled =
          prm.streamInfo.rtcpSdesByeDisabled;
...
#endif
        param->stream_info.info.vid.rtcp_sdes_bye_disabled =
          prm.streamInfo.rtcpSdesByeDisabled;
        param->stream_info.info.vid.codec_param->enc_fmt =
          prm.streamInfo.vidCodecParam.encFmt.toPj();
    }
}
...
void Endpoint::on_stream_destroyed(pjsua_call_id call_id, pjmedia_stream* strm,
                                   unsigned stream_idx)
{
    Call* call = Call::lookup(call_id);
    if (!call) {
    ...
    if (*code / 100 <= 2) {
        if (prm.newCall) {
            ...
            PJ_LOG(3, (THIS_FILE,
                       ...
                       "call transfer (call ID:%d)",
                       call_id));
        }
    }
}

void Endpoint::on_call_transfer_status(pjsua_call_id call_id, int st_code,
    ...
    *opt = prm.opt.toPj();
    if (prm.newCall && prm.newCall != call) {
        ...
        PJ_LOG(3, (THIS_FILE,
                   ...
                   "for call replace request (call ID:%d)",
                   call_id));
    }
}
...
void Endpoint::on_call_rx_offer(pjsua_call_id call_id,
                                ...
                                void* reserved, pjsip_status_code* code,
                                pjsua_call_setting* opt)
{
...
void Endpoint::on_call_tx_offer(pjsua_call_id call_id, void* reserved,
                                pjsua_call_setting* opt)
{
    PJ_UNUSED_ARG(reserved);
                                               ...
                                               const pjsip_event* e)
{
    ...
    int len =
      pjsip_uri_print(PJSIP_URI_IN_FROMTO_HDR, target, uristr, sizeof(uristr));
    if (len < 1) {
...
pj_status_t Endpoint::on_call_media_transport_state(
...
void Endpoint::on_call_media_event(pjsua_call_id call_id, unsigned med_idx,
                                   pjmedia_event* event)
{
    PendingOnMediaEventCallback* job = new PendingOnMediaEventCallback;
...
pjmedia_transport* Endpoint::on_create_media_transport(
  pjsua_call_id call_id, unsigned media_idx, pjmedia_transport* base_tp,
  unsigned flags)
{
    Call* call = Call::lookup(call_id);
    if (!call) {
...
void Endpoint::on_ip_change_progress(pjsua_ip_change_op op, pj_status_t status,
...
void Endpoint::libInit(const EpConfig& prmEpConfig) PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(
      pj_mutex_create_simple(pjsua_var.pool, "threadDesc", &threadDescMutex));

#if !DEPRECATED_FOR_TICKET_2232
    PJSUA2_CHECK_EXPR(
      pj_mutex_create_recursive(pjsua_var.pool, "mediaList", &mediaListMutex));
#endif
}
...
static void ept_log_write(int level, const char* sender, const char* format,
                          ...)
{
#if PJ_LOG_MAX_LEVEL >= 1
    va_list arg;
...
void Endpoint::utilLogWrite(int prmLevel, const string& prmSender,
                            const string& prmMsg)
{
...
Token Endpoint::utilTimerSchedule(unsigned prmMsecDelay, Token prmUserData)
  PJSUA2_THROW(Error)
{
    UserTimer* ut;
    pj_time_val delay;
...
void Endpoint::utilTimerCancel(Token prmTimerToken)
{
    ...
    if (ut->signature != TIMER_SIGNATURE) {
        PJ_LOG(
          1, (THIS_FILE, "Invalid timer token in Endpoint::utilTimerCancel()"));
        return;
    }
...
void Endpoint::natUpdateStunServers(const StringVector& servers, bool wait)
  PJSUA2_THROW(Error)
{
...
void Endpoint::natCheckStunServers(const StringVector& servers, bool wait,
...
void Endpoint::natCancelCheckStunServers(Token token, bool notify_cb)
  PJSUA2_THROW(Error)
{
...
void Endpoint::mediaAdd(AudioMedia& media)
{
    ...
    AudioMediaVector::iterator it =
      std::find(mediaList.begin(), mediaList.end(), &media);
...
void Endpoint::codecSetParam(const string& codec_id, const CodecParam param)
  PJSUA2_THROW(Error)
{
    pj_str_t codec_str = str2Pj(codec_id);
  ...
  PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(
      pjmedia_codec_opus_set_default_param(&new_opus_cfg, &param));
}
...
void Endpoint::resetVideoCodecParam(const string& codec_id) PJSUA2_THROW(Error)
{
#if PJSUA_HAS_VIDEO
    pj_str_t codec_str = str2Pj(codec_id);
...
pj_status_t Endpoint::on_auth_create_aka_response_callback(
  pj_pool_t* pool, const pjsip_digest_challenge* chal,
  const pjsip_cred_info* cred, const pj_str_t* method,
