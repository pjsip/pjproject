...
DeferredResponse& DeferredResponse::operator=(DeferredResponse&& deferredResponse) noexcept
{
    if (this != &deferredResponse) {
        transaction = deferredResponse.transaction;
                                  ...
                                  PJSUA2_THROW(Error)
{
    ...
    NODE_READ_BOOL      (this_node, autoProcessSipMessage);
                                   ...
                                   PJSUA2_THROW(Error)
{
    ...
    NODE_WRITE_BOOL     (this_node, autoProcessSipMessage);
...
void AccountConfig::toPj(pjsua_acc_config &ret) const
{
    ...
    ret.auto_process_sip_message = sipConfig.autoProcessSipMessage;
                           ...
                           const pjsua_media_config *mcfg)
{
    ...
    sipConfig.autoProcessSipMessage = PJ2BOOL(prm.auto_process_sip_message);
...
void Account::sendResponse(const pj::SendResponseParam& prm) PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(pjsua_acc_send_response(id, prm.deferredResponse.data, prm.deferredResponse.transaction,
