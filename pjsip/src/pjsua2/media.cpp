...
///////////////////////////////////////////////////////////////////////////////
Media::Media(pjmedia_type med_type) : type(med_type)
{}
...
///////////////////////////////////////////////////////////////////////////////
AudioMediaTransmitParam::AudioMediaTransmitParam() : level(1.0)
{}
...
void AudioMedia::registerMediaPort(MediaPort port) PJSUA2_THROW(Error)
{
    ...
    if (!Endpoint::instance().mediaExists(*this) && port != NULL) {
        ...
        mediaPool =
          pj_pool_create(&mediaCachingPool.factory, "media", 512, 512, NULL);
        ...
        PJSUA2_CHECK_EXPR(
          pjsua_conf_add_port(mediaPool, (pjmedia_port*)port, &id));
    }
  ...
  PJSUA2_THROW(Error)
{
    ...
    if (port != NULL) {
        ...
        PJSUA2_CHECK_EXPR(pjsua_conf_add_port(pool, (pjmedia_port*)port, &id));
    }
...
AudioMediaPlayer::AudioMediaPlayer() : playerId(PJSUA_INVALID_ID)
{}
...
void AudioMediaPlayer::createPlayer(const string& file_name, unsigned options)
  PJSUA2_THROW(Error)
{
    if (playerId != PJSUA_INVALID_ID) {
    ...
    PJSUA2_CHECK_EXPR(pjsua_player_create(&pj_name, options, &playerId));
...
void AudioMediaPlayer::createPlaylist(const StringVector& file_names,
                                      const string& label, unsigned options)
  PJSUA2_THROW(Error)
{
    if (playerId != PJSUA_INVALID_ID) {
        PJSUA2_RAISE_ERROR(PJ_EEXISTS);
    ...
    PJSUA2_CHECK_EXPR(
      pjsua_playlist_create(pj_files, i, &pj_lbl, options, &playerId));
...
AudioMediaPlayer* AudioMediaPlayer::typecastFromAudioMedia(AudioMedia* media)
{
    return static_cast<AudioMediaPlayer*>(media);
}

void AudioMediaPlayer::eof_cb(pjmedia_port* port, void* usr_data)
{
    PJ_UNUSED_ARG(port);
...
///////////////////////////////////////////////////////////////////////////////
AudioMediaRecorder::AudioMediaRecorder() : recorderId(PJSUA_INVALID_ID)
{}
...
void AudioMediaRecorder::createRecorder(const string& file_name,
                                        unsigned enc_type, long max_size,
                                        unsigned options) PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(pjsua_recorder_create(&pj_name, enc_type, NULL, -1,
                                            options, &recorderId));
...
ToneGenerator::ToneGenerator() : pool(NULL), tonegen(NULL)
{}
...
void ToneGenerator::play(const ToneDescVector& tones, bool loop)
  PJSUA2_THROW(Error)
{
    pj_status_t status;
...
void ToneGenerator::playDigits(const ToneDigitVector& digits, bool loop)
  PJSUA2_THROW(Error)
{
    pj_status_t status;
    ...
    status =
      pjmedia_tonegen_play_digits(tonegen, (unsigned)digits.size(), &digits[0],
                                  loop ? PJMEDIA_TONEGEN_LOOP : 0);
...
AudDevManager::AudDevManager() : devMedia(NULL)
{}
...
void AudDevManager::setEcOptions(unsigned tail_msec, unsigned options)
  PJSUA2_THROW(Error)
{
...
AudioDevInfo AudDevManager::getDevInfo(int id) const PJSUA2_THROW(Error)
{
                             ...
                             const string& dev_name) const PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(
      pjmedia_aud_dev_lookup(drv_name.c_str(), dev_name.c_str(), &pj_idx));
...
void AudDevManager::setExtFormat(const MediaFormatAudio& format, bool keep)
  PJSUA2_THROW(Error)
{
    pjmedia_format pj_format = format.toPj();

    PJSUA2_CHECK_EXPR(
      pjsua_snd_set_setting(PJMEDIA_AUD_DEV_CAP_EXT_FORMAT, &pj_format, keep));
}
...
MediaFormatAudio AudDevManager::getExtFormat() const PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(
      pjsua_snd_get_setting(PJMEDIA_AUD_DEV_CAP_EXT_FORMAT, &pj_format));
...
void AudDevManager::setInputLatency(unsigned latency_msec, bool keep)
  PJSUA2_THROW(Error)
{
...
unsigned AudDevManager::getInputLatency() const PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(
      pjsua_snd_get_setting(PJMEDIA_AUD_DEV_CAP_INPUT_LATENCY, &latency_msec));

    return latency_msec;
}

void AudDevManager::setOutputLatency(unsigned latency_msec, bool keep)
  PJSUA2_THROW(Error)
{
...
unsigned AudDevManager::getOutputLatency() const PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(
      pjsua_snd_get_setting(PJMEDIA_AUD_DEV_CAP_OUTPUT_LATENCY, &latency_msec));
...
void AudDevManager::setInputVolume(unsigned volume, bool keep)
  PJSUA2_THROW(Error)
{
    PJSUA2_CHECK_EXPR(pjsua_snd_set_setting(
      PJMEDIA_AUD_DEV_CAP_INPUT_VOLUME_SETTING, &volume, keep));
}
...
{
    PJSUA2_CHECK_EXPR(pjsua_snd_set_setting(
      PJMEDIA_AUD_DEV_CAP_OUTPUT_VOLUME_SETTING, &volume, keep));
}
...
unsigned AudDevManager::getOutputVolume() const PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(pjsua_snd_get_setting(
      PJMEDIA_AUD_DEV_CAP_OUTPUT_VOLUME_SETTING, &volume));
...
unsigned AudDevManager::getInputSignal() const PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(
      pjsua_snd_get_setting(PJMEDIA_AUD_DEV_CAP_INPUT_SIGNAL_METER, &signal));
...
unsigned AudDevManager::getOutputSignal() const PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(
      pjsua_snd_get_setting(PJMEDIA_AUD_DEV_CAP_OUTPUT_SIGNAL_METER, &signal));

    return signal;
}

void AudDevManager::setInputRoute(pjmedia_aud_dev_route route, bool keep)
  PJSUA2_THROW(Error)
{
    PJSUA2_CHECK_EXPR(
      pjsua_snd_set_setting(PJMEDIA_AUD_DEV_CAP_INPUT_ROUTE, &route, keep));
}
...
pjmedia_aud_dev_route AudDevManager::getInputRoute() const PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(
      pjsua_snd_get_setting(PJMEDIA_AUD_DEV_CAP_INPUT_ROUTE, &route));

    return route;
}

void AudDevManager::setOutputRoute(pjmedia_aud_dev_route route, bool keep)
  PJSUA2_THROW(Error)
{
    PJSUA2_CHECK_EXPR(
...
pjmedia_aud_dev_route AudDevManager::getOutputRoute() const PJSUA2_THROW(Error)
{
    pjmedia_aud_dev_route route = PJMEDIA_AUD_DEV_ROUTE_DEFAULT;
...
{
    PJSUA2_CHECK_EXPR(
      pjsua_snd_set_setting(PJMEDIA_AUD_DEV_CAP_VAD, &enable, keep));
}
...
bool AudDevManager::getVad() const PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(pjsua_snd_get_setting(PJMEDIA_AUD_DEV_CAP_VAD, &enable));
...
{
    PJSUA2_CHECK_EXPR(
      pjsua_snd_set_setting(PJMEDIA_AUD_DEV_CAP_CNG, &enable, keep));
}
...
bool AudDevManager::getCng() const PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(pjsua_snd_get_setting(PJMEDIA_AUD_DEV_CAP_CNG, &enable));
...
{
    PJSUA2_CHECK_EXPR(
      pjsua_snd_set_setting(PJMEDIA_AUD_DEV_CAP_PLC, &enable, keep));
}
...
bool AudDevManager::getPlc() const PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(pjsua_snd_get_setting(PJMEDIA_AUD_DEV_CAP_PLC, &enable));
...
void ExtraAudioDevice::open()
{
    ...
    param.base.samples_per_frame =
      master_info.samples_per_frame / master_info.channel_count;
...
///////////////////////////////////////////////////////////////////////////////
VideoWindow::VideoWindow(pjsua_vid_win_id win_id) : winId(win_id)
{
#if !PJSUA_HAS_VIDEO
...
VideoPreview::VideoPreview(int dev_id) : devId(dev_id), winId(PJSUA_INVALID_ID)
{
#if !PJSUA_HAS_VIDEO
    /* Suppress warning of unused field when video is disabled */
    PJ_UNUSED_ARG(winId);
...
#if PJSUA_HAS_VIDEO
    PJSUA2_CHECK_EXPR(
      pjmedia_vid_dev_lookup(drv_name.c_str(), dev_name.c_str(), &pj_idx));
#else
...
void VidDevManager::setFormat(int dev_id, const MediaFormatVideo& format,
                              bool keep) PJSUA2_THROW(Error)
{
#if PJSUA_HAS_VIDEO
    pjmedia_format pj_format = format.toPj();

    PJSUA2_CHECK_EXPR(pjsua_vid_dev_set_setting(
      dev_id, PJMEDIA_VID_DEV_CAP_FORMAT, &pj_format, keep));
#else
    PJ_UNUSED_ARG(dev_id);
    PJ_UNUSED_ARG(format);
    PJ_UNUSED_ARG(keep);
...
MediaFormatVideo VidDevManager::getFormat(int dev_id) const PJSUA2_THROW(Error)
{
    MediaFormatVideo vid_format;
    pj_bzero(&vid_format, sizeof(vid_format));
#if PJSUA_HAS_VIDEO
    pjmedia_format pj_format;
    PJSUA2_CHECK_EXPR(pjsua_vid_dev_get_setting(
      dev_id, PJMEDIA_VID_DEV_CAP_FORMAT, &pj_format));
...
void VidDevManager::setInputScale(int dev_id, const MediaSize& scale, bool keep)
  PJSUA2_THROW(Error)
{
#if PJSUA_HAS_VIDEO
    pjmedia_rect_size pj_size;
    ...
    pj_size.h = scale.h;
    PJSUA2_CHECK_EXPR(pjsua_vid_dev_set_setting(
      dev_id, PJMEDIA_VID_DEV_CAP_INPUT_SCALE, &pj_size, keep));
#else
    PJ_UNUSED_ARG(dev_id);
    PJ_UNUSED_ARG(scale);
    PJ_UNUSED_ARG(keep);
...
MediaSize VidDevManager::getInputScale(int dev_id) const PJSUA2_THROW(Error)
{
    ...
    pjmedia_rect_size pj_size;
    PJSUA2_CHECK_EXPR(pjsua_vid_dev_get_setting(
      dev_id, PJMEDIA_VID_DEV_CAP_INPUT_SCALE, &pj_size));
...
void VidDevManager::setOutputWindowFlags(int dev_id, int flags, bool keep)
  PJSUA2_THROW(Error)
{
#if PJSUA_HAS_VIDEO
    PJSUA2_CHECK_EXPR(pjsua_vid_dev_set_setting(
      dev_id, PJMEDIA_VID_DEV_CAP_OUTPUT_WINDOW_FLAGS, &flags, keep));
#else
    PJ_UNUSED_ARG(dev_id);
    PJ_UNUSED_ARG(flags);
    PJ_UNUSED_ARG(keep);
...
#if PJSUA_HAS_VIDEO
    PJSUA2_CHECK_EXPR(pjsua_vid_dev_get_setting(
      dev_id, PJMEDIA_VID_DEV_CAP_OUTPUT_WINDOW_FLAGS, &flags));
#else
    PJ_UNUSED_ARG(dev_id);
#endif
    return flags;
...
void VidDevManager::switchDev(int dev_id, const VideoSwitchParam& param)
  PJSUA2_THROW(Error)
{
#if PJSUA_HAS_VIDEO
    ...
    PJSUA2_CHECK_EXPR(pjsua_vid_dev_set_setting(
      dev_id, PJMEDIA_VID_DEV_CAP_SWITCH, &pj_param, PJ_FALSE));
#else
    PJ_UNUSED_ARG(dev_id);
    PJ_UNUSED_ARG(param);
#endif
...
void VidDevManager::setCaptureOrient(pjmedia_vid_dev_index dev_id,
                                     pjmedia_orient orient, bool keep)
  PJSUA2_THROW(Error)
{
#if PJSUA_HAS_VIDEO
    PJSUA2_CHECK_EXPR(pjsua_vid_dev_set_setting(
      dev_id, PJMEDIA_VID_DEV_CAP_ORIENTATION, &orient, keep));
#else
    PJ_UNUSED_ARG(dev_id);
...
VideoMedia::VideoMedia() : Media(PJMEDIA_TYPE_VIDEO), id(PJSUA_INVALID_ID)
{}
  ...
  PJSUA2_THROW(Error)
{
    ...
    PJSUA2_CHECK_EXPR(
      pjsua_vid_conf_add_port(pool, (pjmedia_port*)port, NULL, &id));
#else
    PJ_UNUSED_ARG(port);
    PJ_UNUSED_ARG(pool);
    PJSUA2_RAISE_ERROR(PJ_EINVALIDOP);
...
void VideoMedia::stopTransmit(const VideoMedia& sink) const PJSUA2_THROW(Error)
{
#if PJSUA_HAS_VIDEO
