...
static string jsonNode_unreadName(const ContainerNode* n) PJSUA2_THROW(Error);
static float jsonNode_readNumber(const ContainerNode*, const string&)
  PJSUA2_THROW(Error);
static bool jsonNode_readBool(const ContainerNode*, const string&)
  PJSUA2_THROW(Error);
static string jsonNode_readString(const ContainerNode*, const string&)
  PJSUA2_THROW(Error);
static StringVector jsonNode_readStringVector(const ContainerNode*,
                                              const string&)
  PJSUA2_THROW(Error);
static ContainerNode jsonNode_readContainer(const ContainerNode*, const string&)
  PJSUA2_THROW(Error);
static ContainerNode jsonNode_readArray(const ContainerNode*, const string&)
  PJSUA2_THROW(Error);
static void jsonNode_writeNumber(ContainerNode*, const string& name, float num)
  PJSUA2_THROW(Error);
static void jsonNode_writeBool(ContainerNode*, const string& name, bool value)
  PJSUA2_THROW(Error);
static void jsonNode_writeString(ContainerNode*, const string& name,
                                 const string& value) PJSUA2_THROW(Error);
static void jsonNode_writeStringVector(ContainerNode*, const string& name,
                                       const StringVector& value)
  PJSUA2_THROW(Error);
static ContainerNode jsonNode_writeNewContainer(ContainerNode*,
                                                const string& name)
  PJSUA2_THROW(Error);
static ContainerNode jsonNode_writeNewArray(ContainerNode*, const string& name)
  PJSUA2_THROW(Error);

static container_node_op json_op = {
    &jsonNode_hasUnread,         &jsonNode_unreadName,
    &jsonNode_readNumber,        &jsonNode_readBool,
    &jsonNode_readString,        &jsonNode_readStringVector,
    &jsonNode_readContainer,     &jsonNode_readArray,
    &jsonNode_writeNumber,       &jsonNode_writeBool,
    &jsonNode_writeString,       &jsonNode_writeStringVector,
    &jsonNode_writeNewContainer, &jsonNode_writeNewArray
};

///////////////////////////////////////////////////////////////////////////////
JsonDocument::JsonDocument() : root(NULL)
{
...
static pj_status_t json_file_writer(const char* s, unsigned size,
                                    void* user_data)
{
...
static pj_status_t json_string_writer(const char* s, unsigned size,
                                      void* user_data)
{
...
static void json_verify(struct json_node_data* jdat, const char* op,
                        const string& name, pj_json_val_type type)
{
     ...
     */
    if (jdat->jnode->type != PJ_JSON_VAL_ARRAY && name.size() &&
    ...
    if (type != PJ_JSON_VAL_NULL && jdat->childPtr->type != type) {
        char err_msg[80];
        pj_ansi_snprintf(err_msg, sizeof(err_msg),
                         "Type mismatch: expecting %d got %d", type,
                         jdat->childPtr->type);
        PJSUA2_RAISE_ERROR3(PJLIB_UTIL_EINJSON, op, err_msg);
    }
}

static string jsonNode_unreadName(const ContainerNode* node) PJSUA2_THROW(Error)
{
    json_node_data* jdat = (json_node_data*)&node->data;
...
static float jsonNode_readNumber(const ContainerNode* node, const string& name)
  PJSUA2_THROW(Error)
{
    json_node_data* jdat = (json_node_data*)&node->data;
...
static bool jsonNode_readBool(const ContainerNode* node, const string& name)
  PJSUA2_THROW(Error)
{
    json_node_data* jdat = (json_node_data*)&node->data;
...
static string jsonNode_readString(const ContainerNode* node, const string& name)
  PJSUA2_THROW(Error)
{
    json_node_data* jdat = (json_node_data*)&node->data;
  ...
  PJSUA2_THROW(Error)
{
    ...
    while (child != (pj_json_elem*)&jdat->childPtr->value.children) {
        if (child->type != PJ_JSON_VAL_STRING) {
            char err_msg[80];
            pj_ansi_snprintf(err_msg, sizeof(err_msg),
                             "Elements not string but type %d", child->type);
...
static ContainerNode jsonNode_readArray(const ContainerNode* node,
                                        const string& name) PJSUA2_THROW(Error)
{
    json_node_data* jdat = (json_node_data*)&node->data;
...
static void jsonNode_writeNumber(ContainerNode* node, const string& name,
...
static void jsonNode_writeBool(ContainerNode* node, const string& name,
...
static void jsonNode_writeString(ContainerNode* node, const string& name,
                                 const string& value) PJSUA2_THROW(Error)
{
    json_node_data* jdat = (json_node_data*)&node->data;
    pj_json_elem* el = jdat->doc->allocElement();
...
static void jsonNode_writeStringVector(ContainerNode* node, const string& name,
                                       const StringVector& value)
  PJSUA2_THROW(Error)
{
    json_node_data* jdat = (json_node_data*)&node->data;
