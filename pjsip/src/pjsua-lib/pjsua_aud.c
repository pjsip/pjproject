                           ...
                           pjsua_stream_info* psi)
{
    ...
    case PJMEDIA_TYPE_AUDIO:
        status =
          pjmedia_stream_get_info(call_med->strm.a.stream, &psi->info.aud);
        break;
#    if defined(PJMEDIA_HAS_VIDEO) && (PJMEDIA_HAS_VIDEO != 0)
    case PJMEDIA_TYPE_VIDEO:
        status =
          pjmedia_vid_stream_get_info(call_med->strm.v.stream, &psi->info.vid);
        break;
#    endif
    default:
                           ...
                           pjsua_stream_stat* stat)
{
    ...
    switch (call_med->type) {
    case PJMEDIA_TYPE_AUDIO:
        status = pjmedia_stream_get_stat(call_med->strm.a.stream, &stat->rtcp);
...
#    if defined(PJMEDIA_HAS_VIDEO) && (PJMEDIA_HAS_VIDEO != 0)
    case PJMEDIA_TYPE_VIDEO:
        status =
          pjmedia_vid_stream_get_stat(call_med->strm.v.stream, &stat->rtcp);
        if (status == PJ_SUCCESS)
...
pjsua_call_dial_dtmf(pjsua_call_id call_id, const pj_str_t* digits)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Call %d dialing DTMF %.*s", call_id,
               (int)digits->slen, digits->ptr));
    pj_log_push_indent();
...
#    if PJMEDIA_HAS_PASSTHROUGH_CODECS
    /* Register passthrough codecs */
    {
        ...
        codec_cfg.passthrough.setting.ilbc_mode = pjsua_var.media_cfg.ilbc_mode;
    }
#    endif /* PJMEDIA_HAS_PASSTHROUGH_CODECS */
    ...
    pjmedia_codec_mgr_set_codec_priority(
      pjmedia_endpt_get_codec_mgr(pjsua_var.med_endpt), &codec_id,
      PJMEDIA_CODEC_PRIO_NORMAL + 2);

    /* Set speex/8000 to next higher priority*/
    codec_id = pj_str("speex/8000");
    pjmedia_codec_mgr_set_codec_priority(
      pjmedia_endpt_get_codec_mgr(pjsua_var.med_endpt), &codec_id,
      PJMEDIA_CODEC_PRIO_NORMAL + 1);
    ...
    pjsua_var.mconf_cfg.samples_per_frame =
      pjsua_var.media_cfg.clock_rate * pjsua_var.mconf_cfg.channel_count *
    ...
    status = pjmedia_conf_create(
      pjsua_var.pool, pjsua_var.media_cfg.max_media_ports,
      pjsua_var.media_cfg.clock_rate, pjsua_var.mconf_cfg.channel_count,
      pjsua_var.mconf_cfg.samples_per_frame,
      pjsua_var.mconf_cfg.bits_per_sample, opt, &pjsua_var.mconf);
    if (status != PJ_SUCCESS) {
    ...
    pjsua_var.is_mswitch =
      pjmedia_conf_get_master_port(pjsua_var.mconf)->info.signature ==
      PJMEDIA_CONF_SWITCH_SIGNATURE;

    /* Create null port just in case user wants to use null sound. */
    status = pjmedia_null_port_create(
      pjsua_var.pool, pjsua_var.media_cfg.clock_rate,
      pjsua_var.mconf_cfg.channel_count, pjsua_var.mconf_cfg.samples_per_frame,
      pjsua_var.mconf_cfg.bits_per_sample, &pjsua_var.null_port);
...
void pjsua_check_snd_dev_idle()
{
     ...
     */
    if (pjsua_var.snd_idle_timer.id == PJ_FALSE && call_cnt == 0 &&
...
static void close_snd_timer_cb(pj_timer_heap_t* th, pj_timer_entry* entry)
{
    ...
    if (entry->id) {
        PJ_LOG(4,
               (THIS_FILE, "Closing sound device after idle for %d second(s)",
...
void pjsua_aud_stop_stream(pjsua_call_media* call_med)
{
    ...
    if (strm) {
        ...
        if (!call_med->call->hanging_up &&
            ...
            pjsua_var.ua_cfg.cb.on_stream_destroyed(call_med->call->index, strm,
                                                    call_med->idx);
        }
 ...
 */
static void dtmf_callback(pjmedia_stream* strm, void* user_data, int digit)
{
    pjsua_call_id call_id;
...
#    endif
        ...
        status =
          pjmedia_stream_create(pjsua_var.med_endpt, NULL, si, call_med->tp,
                                NULL, &call_med->strm.a.stream);
        if (status != PJ_SUCCESS) {
            goto on_return;
        }
        ...
        if (!call->hanging_up && pjsua_var.ua_cfg.cb.on_dtmf_event) {
            pjmedia_stream_set_dtmf_event_callback(
              call_med->strm.a.stream, &dtmf_event_callback,
              (void*)(pj_ssize_t)(call->index));
        } else if (!call->hanging_up && (pjsua_var.ua_cfg.cb.on_dtmf_digit ||
        ...
        if (!call->hanging_up && pjsua_var.ua_cfg.cb.on_stream_created2) {
            ...
            (*pjsua_var.ua_cfg.cb.on_stream_created)(
              call->index, call_med->strm.a.stream, strm_idx,
              &call_med->strm.a.media_port);
        }
         ...
         */
        {
            ...
            status = pjmedia_conf_add_port(
              pjsua_var.mconf, call->inv->pool, call_med->strm.a.media_port,
              &port_name, (unsigned*)&call_med->strm.a.conf_slot);
            if (status != PJ_SUCCESS) {
                goto on_return;
            }
        }
 ...
 */
PJ_DEF(pj_status_t)
pjsua_conf_get_port_info(pjsua_conf_port_id id, pjsua_conf_port_info* info)
{
                    ...
                    pjsua_conf_port_id* p_id)
{
    ...
    status =
      pjmedia_conf_add_port(pjsua_var.mconf, pool, port, NULL, (unsigned*)p_id);
    if (status != PJ_SUCCESS) {
        if (p_id)
                    ...
                    const pjsua_conf_connect_param* prm)
{
    ...
    PJ_LOG(4, (THIS_FILE, "%s connect: %d --> %d",
               (pjsua_var.is_mswitch ? "Switch" : "Conf"), source, sink));
    ...
    if (pjsua_var.is_mswitch) {
        ...
        status =
          pjmedia_conf_get_port_info(pjsua_var.mconf, peer_id, &peer_info);
        ...
        need_reopen = (pjsua_var.snd_port == NULL &&
         ...
         * though.
         */
        if (!need_reopen && port0_info.listener_cnt == 0 &&
            port0_info.transmitter_cnt == 0)
        {
        ...
        if (need_reopen) {
            if (pjsua_var.cap_dev != PJSUA_SND_NULL_DEV) {
                ...
                status = create_aud_param(
                  &param.base, pjsua_var.cap_dev, pjsua_var.play_dev,
                  peer_info.clock_rate, peer_info.channel_count,
                  peer_info.samples_per_frame, peer_info.bits_per_sample);
                if (status != PJ_SUCCESS) {
                            ...
                            unsigned* rx_level)
{
    ...
    return pjmedia_conf_get_signal_level(pjsua_var.mconf, slot, tx_level,
                                         rx_level);
}
                    ...
                    pjsua_player_id* p_id)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Creating file player: %.*s..", (int)filename->slen,
               filename->ptr));
    pj_log_push_indent();
    ...
    status = pjmedia_wav_player_port_create(
      ...
      pjsua_var.mconf_cfg.samples_per_frame * 1000 /
        pjsua_var.media_cfg.channel_count / pjsua_var.media_cfg.clock_rate,
      options, 0, &port);
    if (status != PJ_SUCCESS) {
    ...
    status =
      pjmedia_conf_add_port(pjsua_var.mconf, pool, port, filename, &slot);
    if (status != PJ_SUCCESS) {
                      ...
                      pjsua_player_id* p_id)
{
    ...
    status = pjmedia_wav_playlist_create(pool, label, file_names, file_count,
    ...
    status = pjmedia_conf_add_port(pjsua_var.mconf, pool, port,
                                   &port->info.name, &slot);
    if (status != PJ_SUCCESS) {
        pjmedia_port_destroy(port);
                      ...
                      pjsua_recorder_id* p_id)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Creating recorder %.*s..", (int)filename->slen,
               filename->ptr));
    pj_log_push_indent();
    ...
    if (pj_stricmp2(&ext, ".wav") == 0)
        ...
        PJ_LOG(1, (THIS_FILE,
                   "pjsua_recorder_create() error: unable to "
    ...
    if (file_format == FMT_WAV) {
        status = pjmedia_wav_writer_port_create(
          pool, path, pjsua_var.media_cfg.clock_rate,
          pjsua_var.mconf_cfg.channel_count,
          pjsua_var.mconf_cfg.samples_per_frame,
          pjsua_var.mconf_cfg.bits_per_sample, options, 0, &port);
    } else {
                                    ...
                                    unsigned bits_per_sample)
{
    ...
    status = pjmedia_aud_dev_default_param(
      (speaker_only ? playback_dev : capture_dev), param);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE,
                     "Error retrieving default audio "
                     "device parameters",
                     status);
        return status;
    }
...
static pj_status_t open_snd_dev(pjmedia_snd_port_param* param)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Opening sound device (%s) %s@%d/%d/%dms",
               ...
               get_fmt_name(param->base.ext_fmt.id), param->base.clock_rate,
     ...
     */
    if (!pjsua_var.is_mswitch && param->base.ext_fmt.id == PJMEDIA_FORMAT_PCM &&
        PJMEDIA_PIA_SRATE(&conf_port->info) != param->base.clock_rate)
    {
        ...
        status = pjmedia_resample_port_create(pjsua_var.snd_pool, conf_port,
                                              param->base.clock_rate,
                                              resample_opt, &resample_port);
        if (status != PJ_SUCCESS) {
            char errmsg[PJ_ERR_MSG_SIZE];
            pj_strerror(status, errmsg, sizeof(errmsg));
            PJ_LOG(4, (THIS_FILE, "Error creating resample port: %s", errmsg));
            close_snd_dev();
            goto on_error;
        }
    ...
    if (pjsua_var.is_mswitch) {
        if (param->base.flags & PJMEDIA_AUD_DEV_CAP_EXT_FORMAT) {
            ...
            pjmedia_format_init_audio(
              &conf_port->info.fmt, PJMEDIA_FORMAT_PCM, param->base.clock_rate,
              param->base.channel_count, param->base.bits_per_sample,
              ptime_usec, bps, bps);
        }
    }
    ...
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE,
                     "Unable to connect conference port to "
                     "sound device",
                     status);
...
static void close_snd_dev(void)
{
    ...
    if (pjsua_var.snd_port) {
        ...
        PJ_LOG(4, (THIS_FILE,
                   "Closing %s sound playback device and "
...
PJ_DEF(pj_status_t) pjsua_set_snd_dev2(const pjsua_snd_dev_param* snd_param)
{
    ...
    PJ_LOG(4,
           (THIS_FILE, "Set sound device: capture=%d, playback=%d, mode=%d",
            snd_param->capture_dev, snd_param->playback_dev, snd_param->mode));
        ...
        pjsua_var.snd_mode == snd_param->mode)
    {
        ...
        if (pjsua_var.snd_is_on ||
            ...
            PJ_LOG(4,
                   (THIS_FILE, "No changes in capture and playback devices"));
            PJSUA_UNLOCK();
            pj_log_pop_indent();
            return PJ_SUCCESS;
        }
    ...
    for (i = 0; i < alt_cr_cnt; ++i) {
        ...
        samples_per_frame = alt_cr[i] * pjsua_var.media_cfg.audio_frame_ptime *
                            pjsua_var.media_cfg.channel_count / 1000;
        pjmedia_snd_port_param_default(&param);
        param.ec_options = pjsua_var.media_cfg.ec_options;
        status = create_aud_param(
          &param.base, snd_param->capture_dev, snd_param->playback_dev,
          alt_cr[i], pjsua_var.media_cfg.channel_count, samples_per_frame, 16);
        if (status != PJ_SUCCESS)
            goto on_error;
...
PJ_DEF(pj_status_t) pjsua_set_null_snd_dev(void)
{
    ...
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Unable to create null sound device", status);
        PJSUA_UNLOCK();
        pj_log_pop_indent();
        return status;
    }
 ...
 */
PJ_DEF(pj_status_t)
pjsua_snd_set_setting(pjmedia_aud_dev_cap cap, const void* pval, pj_bool_t keep)
{
    pj_status_t status;
    ...
    if (keep) {
        status = pjmedia_aud_param_set_cap(&pjsua_var.aud_param, cap, pval);
    }
...
PJ_DEF(pj_status_t) pjsua_snd_get_setting(pjmedia_aud_dev_cap cap, void* pval)
{
    ...
    if (pjsua_snd_is_active()) {
        ...
        status = pjmedia_aud_param_get_cap(&pjsua_var.aud_param, cap, pval);
    }
                         ...
                         pjsua_ext_snd_dev** p_snd)
{
    ...
    status = pjmedia_splitcomb_create(
      pool, param->base.clock_rate, param->base.channel_count,
      param->base.samples_per_frame, param->base.bits_per_sample,
      0, /* options */
      &snd->splitcomb);
    if (status != PJ_SUCCESS)
        goto on_return;
    ...
    status = pjmedia_splitcomb_create_rev_channel(
      pool, snd->splitcomb, 0 /* channel #1 */, 0 /* options */,
      &snd->rev_port);
    if (status != PJ_SUCCESS)
        goto on_return;
 ...
 */
PJ_DEF(pjmedia_snd_port*) pjsua_ext_snd_dev_get_snd_port(pjsua_ext_snd_dev* snd)
{
