...
/* UPDATE method */
static const pjsip_method pjsip_update_method = { PJSIP_OTHER_METHOD,
                                                  { "UPDATE", 6 } };

/* This callback receives notification from invite session when the
 * session state has changed.
 */
static void pjsua_call_on_state_changed(pjsip_inv_session* inv, pjsip_event* e);

/* This callback is called by invite session framework when UAC session
 * has forked.
 */
static void pjsua_call_on_forked(pjsip_inv_session* inv, pjsip_event* e);
 ...
 */
static void pjsua_call_on_rx_offer(
  pjsip_inv_session* inv, struct pjsip_inv_on_rx_offer_cb_param* param);
...
/* End call session */
static pj_status_t call_inv_end_session(pjsua_call* call, unsigned code,
...
static void reset_call(pjsua_call_id id)
{
    ...
    pj_timer_entry_init(&call->reinv_timer, PJ_FALSE, (void*)(pj_size_t)id,
                        &reinv_timer_cb);
...
pj_status_t pjsua_call_subsys_init(const pjsua_config* cfg)
{
    ...
    pjsip_endpt_add_capability(pjsua_var.endpt, NULL, PJSIP_H_SUPPORTED, NULL,
                               1, &str_norefersub);

    /* Add "INFO" in Allow header, for DTMF and video key frame request. */
    pjsip_endpt_add_capability(pjsua_var.endpt, NULL, PJSIP_H_ALLOW, NULL, 1,
                               &pjsip_info_method.name);
 ...
 * Enum calls.
 */
PJ_DEF(pj_status_t) pjsua_enum_calls(pjsua_call_id ids[], unsigned* count)
{
    unsigned i, c;
...
static pjsua_call_id alloc_call_id(void)
{
    ...
    for (cid = pjsua_var.next_call_id; cid < (int)pjsua_var.ua_cfg.max_calls;
         ++cid) {
...
static pj_status_t on_make_call_med_tp_complete(
  pjsua_call_id call_id, const pjsua_med_tp_state_info* info)
{
    ...
    if (call->async_call.med_ch_deinit ||
        ...
        PJ_LOG(4, (THIS_FILE,
                   "Call has been hung up or media channel has "
                   "been deinitialized"));
        goto on_error;
    }
    ...
    pjsua_process_msg_data(tdata, call->async_call.call_var.out_call.msg_data);
...
on_error:
    if (inv == NULL && call_id != -1 && !cb_called && !call->hanging_up &&
                                      ...
                                      const pjmedia_sdp_session* rem_sdp)
{
        ...
        (call->opt.flag & PJSUA_CALL_REINIT_MEDIA))
    {
        ...
        status = pjsua_media_channel_init(call->index, role, call->secure_level,
                                          call->inv->pool_prov, rem_sdp, NULL,
                                          PJ_FALSE, NULL);
        if (status != PJ_SUCCESS) {
...
static void dlg_set_via(pjsip_dialog* dlg, pjsua_acc* acc)
{
               ...
               !pjsua_sip_acc_is_using_upnp(acc->index))
    {
        ...
        if (pjsua_acc_get_uac_addr(acc->index, dlg->pool, &acc->cfg.id,
                                   ...
                                   &via_tp) == PJ_SUCCESS)
        {
            pjsip_dlg_set_via_sent_by(dlg, &via_addr, (pjsip_transport*)via_tp);
        }
    }
}
...
static pj_status_t dlg_set_target(pjsip_dialog* dlg, const pj_str_t* target)
{
    ...
    /* Add the new target */
    status =
      pjsip_target_set_add_uri(&dlg->target_set, dlg->pool, target_uri, 0);
    if (status != PJ_SUCCESS)
        return status;

    /* Set it as current target */
    status = pjsip_target_set_set_current(
      &dlg->target_set, pjsip_target_set_get_next(&dlg->target_set));
    if (status != PJ_SUCCESS)
        return status;
...
void call_update_contact(pjsua_call* call, pj_str_t** new_contact)
{
    ...
    if (acc->cfg.force_contact.slen)
        ...
        *new_contact = &acc->contact;
        ...
        status = pjsua_acc_create_uac_contact(
          dlg->pool, &tmp_contact, acc->index, &dlg->remote.info_str);
        if (status == PJ_SUCCESS) {
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_make_call(pjsua_acc_id acc_id, const pj_str_t* dest_uri,
                     ...
                     const pjsua_msg_data* msg_data, pjsua_call_id* p_call_id)
{
    ...
    if (!acc->valid) {
        pjsua_perror(THIS_FILE,
                     "Unable to make call because account "
                     "is not valid",
                     PJ_EINVALIDOP);
    ...
    if (acc->contact.slen) {
        ...
        status =
          pjsua_acc_create_uac_contact(tmp_pool, &contact, acc_id, dest_uri);
        if (status != PJ_SUCCESS) {
    ...
    status = pjsip_dlg_create_uac(
      pjsip_ua_instance(), &acc->cfg.id, &contact, dest_uri,
      (msg_data && msg_data->target_uri.slen ? &msg_data->target_uri :
                                               dest_uri),
      &dlg);
    if (status != PJ_SUCCESS) {
    ...
    if (msg_data) {
        call->async_call.call_var.out_call.msg_data =
          pjsua_msg_data_clone(dlg->pool, msg_data);
    }
    ...
    if ((call->opt.flag & PJSUA_CALL_NO_SDP_OFFER) == 0) {
        /* Init media channel */
        status = pjsua_media_channel_init(
          call->index, PJSIP_ROLE_UAC, call->secure_level, dlg->pool, NULL,
          NULL, PJ_TRUE, &on_make_call_med_tp_complete);
    }
    if (status == PJ_SUCCESS) {
                                                 ...
                                                 pjsip_dialog* replaced_dlg)
{
    ...
    if (!replaced_call->hanging_up && pjsua_var.ua_cfg.cb.on_call_replaced)
        pjsua_var.ua_cfg.cb.on_call_replaced(replaced_call->index, call->index);
    ...
    PJ_LOG(4,
           (THIS_FILE, "Disconnecting replaced call %d", replaced_call->index));
...
pj_status_t create_temp_sdp(pj_pool_t* pool, const pjmedia_sdp_session* rem_sdp,
                            pjmedia_sdp_session** p_sdp)
{
...
static pj_status_t verify_request(const pjsua_call* call, pjsip_rx_data* rdata,
                                  pj_bool_t use_tmp_sdp, int* sip_err_code,
                                  pjsip_tx_data** response)
{
    ...
    if (status == PJ_SUCCESS) {
        ...
        status = pjsip_inv_verify_request3(rdata, call->inv->pool_prov,
                                           &options, offer, answer, NULL,
...
static pj_status_t on_incoming_call_med_tp_complete2(
  pjsua_call_id call_id, const pjsua_med_tp_state_info* info,
  pjsip_rx_data* rdata, int* sip_err_code, pjsip_tx_data** tdata)
{
    pjsua_call* call = &pjsua_var.calls[call_id];
    pjsip_dialog* dlg = call->async_call.dlg;
    pj_status_t status = (info ? info->status : PJ_SUCCESS);
...
on_return:
    if (status != PJ_SUCCESS) {
        ...
        if (call->inv->state > PJSIP_INV_STATE_NULL) {
            ...
            if (response == NULL) {
                status_ =
                  pjsip_inv_end_session(call->inv, err_code, NULL, &response);
            }
...
static pj_status_t on_incoming_call_med_tp_complete(
  pjsua_call_id call_id, const pjsua_med_tp_state_info* info)
{
...
pj_bool_t pjsua_call_on_incoming(pjsip_rx_data* rdata)
{
    ...
    if (call_id == PJSUA_INVALID_ID) {
        pjsip_endpt_respond_stateless(pjsua_var.endpt, rdata,
                                      PJSIP_SC_BUSY_HERE, NULL, NULL, NULL);
        PJ_LOG(2,
               (THIS_FILE, "Unable to accept incoming call (too many calls)"));
        goto on_return;
    }
     ...
     */
    status =
      pjsip_replaces_verify_request(rdata, &replaced_dlg, PJ_FALSE, &response);
    if (status != PJ_SUCCESS) {
        /*
                                 ...
                                 pjsua_var.ua_cfg.cb.on_call_replace_request2))
    {
        ...
        if (!replaced_call->hanging_up &&
            ...
            pjsua_var.ua_cfg.cb.on_call_replace_request(
              replaced_call->index, rdata, &st_code, &st_text);
        }

        if (!replaced_call->hanging_up &&
            pjsua_var.ua_cfg.cb.on_call_replace_request2) {
            pjsua_var.ua_cfg.cb.on_call_replace_request2(
              replaced_call->index, rdata, &st_code, &st_text, &call->opt);
        }
        ...
        if (st_code >= 300) {
            ...
            pjsip_endpt_respond(pjsua_var.endpt, NULL, rdata, st_code, &st_text,
                                NULL, NULL, NULL);
            goto on_return;
        }
    ...
    if (rdata->msg_info.msg->body) {
            ...
            !PJSIP_INV_ACCEPT_UNKNOWN_BODY)
        {
            if (sdp_info->body.ptr == NULL) {
                status =
                  PJSIP_ERRNO_FROM_SIP_STATUS(PJSIP_SC_UNSUPPORTED_MEDIA_TYPE);
            } else {
        ...
        if (status != PJ_SUCCESS) {
            ...
            if (sdp_info->body.ptr == NULL) {
                ...
                pjsua_perror(THIS_FILE,
                             "Unknown Content-Type in incoming "
                             "INVITE",
                             status);
                ...
                pjsip_endpt_respond(pjsua_var.endpt, NULL, rdata,
                                    PJSIP_SC_UNSUPPORTED_MEDIA_TYPE, NULL,
                                    &hdr_list, NULL, NULL);
            } else {
                const pj_str_t reason = pj_str("Bad SDP");
                pjsip_warning_hdr* w;

                pjsua_perror(THIS_FILE, "Bad SDP in incoming INVITE", status);

                w = pjsip_warning_hdr_create_from_status(
                  rdata->tp_info.pool, pjsip_endpt_name(pjsua_var.endpt),
                  status);
                pj_list_init(&hdr_list);
    ...
    if (status != PJ_SUCCESS) {
        ...
        if (response) {
            ...
            /* Respond with 500 (Internal Server Error) */
            pjsip_endpt_respond(pjsua_var.endpt, NULL, rdata, 500, NULL, NULL,
                                NULL, NULL);
        }
    ...
    status = pjsip_dlg_create_uas_and_inc_lock(pjsip_ua_instance(), rdata,
                                               &contact, &dlg);
    if (status != PJ_SUCCESS) {
        pjsip_endpt_respond_stateless(pjsua_var.endpt, rdata, 500, NULL, NULL,
                                      NULL);
        goto on_return;
    }
               ...
               !pjsua_sip_acc_is_using_upnp(acc_id))
    {
        ...
        target.slen = pjsip_uri_print(PJSIP_URI_IN_REQ_URI, dlg->target,
                                      target_buf, sizeof(target_buf));
        if (target.slen < 0)
            target.slen = 0;

        if (pjsua_acc_get_uac_addr(acc_id, dlg->pool, &target, &via_addr, NULL,
                                   NULL, &via_tp) == PJ_SUCCESS)
        {
            pjsip_dlg_set_via_sent_by(dlg, &via_addr, (pjsip_transport*)via_tp);
        }
    }
        ...
        pjsua_var.acc[acc_id].cfg.require_100rel == PJSUA_100REL_OPTIONAL)
    {
        ...
        cap_status =
          pjsip_dlg_remote_has_cap(dlg, PJSIP_H_SUPPORTED, NULL, &token);
    ...
    if (status != PJ_SUCCESS) {
        ...
        w = pjsip_warning_hdr_create_from_status(
          dlg->pool, pjsip_endpt_name(pjsua_var.endpt), status);
        pj_list_init(&hdr_list);
        pj_list_push_back(&hdr_list, w);
    ...
    if (offer || replaced_dlg) {
         ...
         * SIP extensions.
         */
        status = verify_request(call, rdata, PJ_TRUE, &sip_err_code, &response);
        ...
        status = pjsua_media_channel_init(
          call->index, PJSIP_ROLE_UAS, call->secure_level, rdata->tp_info.pool,
          offer, &sip_err_code, PJ_TRUE, &on_incoming_call_med_tp_complete);
        if (status == PJ_EPENDING) {
            ...
            status = on_incoming_call_med_tp_complete2(
              call_id, NULL, rdata, &sip_err_code, &response);
            if (status != PJ_SUCCESS) {
    ...
    /*
        status = pjsua_media_channel_create_sdp(call->index,
       rdata->tp_info.pool, offer, &answer, &sip_err_code); if (status !=
       PJ_SUCCESS) { pjsua_perror(THIS_FILE, "Error creating SDP answer",
       status); pjsip_endpt_respond(pjsua_var.endpt, NULL, rdata, sip_err_code,
       NULL, NULL, NULL, NULL); goto on_return;
        }
    */

    /* Init Session Timers */
    status =
      pjsip_timer_init_session(inv, &pjsua_var.acc[acc_id].cfg.timer_setting);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Session Timer init failed", status);
        pjsip_dlg_respond(dlg, rdata, PJSIP_SC_INTERNAL_SERVER_ERROR, NULL,
                          NULL, NULL);
    ...
    if (replaced_dlg) {
        ...
        if (pjsua_var.ua_cfg.cb.on_incoming_call) {
            ...
            pjsua_call_hangup(call_id, PJSIP_SC_TEMPORARILY_UNAVAILABLE, NULL,
                              NULL);
        }
    }
...
pj_status_t acquire_call(const char* title, pjsua_call_id call_id,
                         pjsua_call** p_call, pjsip_dialog** p_dlg)
{
    unsigned retry;
    pjsua_call* call = NULL;
    ...
    if (status != PJ_SUCCESS) {
        if (has_pjsua_lock == PJ_FALSE)
            ...
            PJ_LOG(1, (THIS_FILE,
                       "Timed-out trying to acquire dialog mutex "
                       "(possibly system has deadlocked) in %s",
                       title));
        return PJ_ETIMEDOUT;
    }
...
pjsua_call_get_info(pjsua_call_id call_id, pjsua_call_info* info)
{
    ...
    info->local_contact.slen = pjsip_uri_print(
      PJSIP_URI_IN_CONTACT_HDR, dlg->local.contact->uri,
      info->local_contact.ptr, sizeof(info->buf_.local_contact));
    ...
    if (dlg->remote.contact) {
        ...
        len = pjsip_uri_print(
          PJSIP_URI_IN_CONTACT_HDR, dlg->remote.contact->uri,
          info->remote_contact.ptr, sizeof(info->buf_.remote_contact));
        if (len < 0)
            len = 0;
    ...
    info->media_cnt = 0;
    for (mi = 0;
         mi < call->med_cnt && info->media_cnt < PJ_ARRAY_SIZE(info->media);
         ++mi)
    {
    ...
    for (mi = 0; mi < call->med_prov_cnt &&
                 info->prov_media_cnt < PJ_ARRAY_SIZE(info->prov_media);
         ++mi)
    {
 ...
 */
PJ_DEF(pjsip_dialog_cap_status)
...
static pj_status_t on_answer_call_med_tp_complete(
  pjsua_call_id call_id, const pjsua_med_tp_state_info* info)
{
    pjsua_call* call = &pjsua_var.calls[call_id];
    pjmedia_sdp_session* sdp;
    ...
    status = pjsua_media_channel_create_sdp(call_id, call->async_call.dlg->pool,
...
on_return:
    if (status != PJ_SUCCESS) {
        ...
        if (call->inv->state > PJSIP_INV_STATE_NULL) {
            ...
            status_ =
              pjsip_inv_end_session(call->inv, sip_err_code, NULL, &tdata);
                   ...
                   const pjsua_msg_data* msg_data)
{
    ...
    {
        PJ_LOG(3, (THIS_FILE,
                   "Unable to answer call (no incoming INVITE or "
                   "already answered)"));
        status = PJ_EINVALIDOP;
        goto on_return;
    }
    ...
    if (!call->med_ch_cb &&
        ...
        (!call->inv->neg || pjmedia_sdp_neg_get_state(call->inv->neg) ==
                              PJMEDIA_SDP_NEG_STATE_NULL))
    {
        ...
        status = pjsua_media_channel_init(
          call->index, PJSIP_ROLE_UAC, call->secure_level, dlg->pool, NULL,
          NULL, PJ_TRUE, &on_answer_call_med_tp_complete);
        if (status == PJ_SUCCESS) {
    ...
    if (call->med_ch_cb || !call->inv->last_answer) {
        ...
        PJ_LOG(4, (THIS_FILE,
                   "Pending answering call %d upon completion "
                   "of media transport",
                   call_id));
        ...
        if (opt) {
            answer->opt =
              PJ_POOL_ZALLOC_T(call->inv->pool_prov, pjsua_call_setting);
            *answer->opt = *opt;
        }
        if (reason) {
        ...
        if (msg_data) {
            answer->msg_data =
              pjsua_msg_data_clone(call->inv->pool_prov, msg_data);
        }
                           ...
                           const pjsua_msg_data* msg_data)
{
    ...
    status = acquire_call("pjsua_call_answer_with_sdp()", call_id, &call, &dlg);
    if (status != PJ_SUCCESS)
        return status;
                                        ...
                                        const pjsua_msg_data* msg_data)
{
    ...
    status = pjsip_inv_send_msg(call->inv, tdata);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Failed to send end session message", status);
        goto on_return;
    }
...
on_return:
    ...
    if (status != PJ_SUCCESS && call->inv) {
        ...
        if (call->hangup_retry >= CALL_HANGUP_MAX_RETRY) {
            ...
            PJ_LOG(1, (THIS_FILE,
                       "Call %d: failed to hangup after %d retries, "
        ...
        if (call->hangup_retry == 0) {
            pj_timer_entry_init(&call->hangup_timer, PJ_FALSE, (void*)call,
                                &hangup_timer_cb);

            call->hangup_code = code;
            if (reason) {
                pj_strdup(call->inv->pool_prov, &call->hangup_reason, reason);
            }
            if (msg_data) {
                  ...
                  const pjsua_msg_data* msg_data)
{
    ...
    if (call_id < 0 || call_id >= (int)pjsua_var.ua_cfg.max_calls) {
        PJ_LOG(1,
               (THIS_FILE, "pjsua_call_hangup(): invalid call id %d", call_id));
    }
    ...
    if (!call->hanging_up) {
            ...
            ((call->inv != NULL) && (call->inv->state == PJSIP_INV_STATE_NULL)))
        {
            ...
            PJ_LOG(4, (THIS_FILE,
                       "Will continue call %d hangup upon "
        ...
        PJ_LOG(4, (THIS_FILE,
                   "Call %d hangup request ignored as "
                   "it is on progress",
                   call_id));
    }
...
pjsua_call_process_redirect(pjsua_call_id call_id, pjsip_redirect_op cmd)
{
    ...
    status =
      acquire_call("pjsua_call_process_redirect()", call_id, &call, &dlg);
    if (status != PJ_SUCCESS)
        return status;
                     ...
                     const pjsua_msg_data* msg_data)
{
    ...
    if ((options & PJSUA_CALL_UPDATE_VIA) && pjsua_acc_is_valid(call->acc_id)) {
        dlg_set_via(call->inv->dlg, &pjsua_var.acc[call->acc_id]);
    }

    if ((call->opt.flag & PJSUA_CALL_UPDATE_TARGET) && msg_data &&
                     ...
                     const pjsua_msg_data* msg_data)
{
    ...
    if (call->local_hold && (call->opt.flag & PJSUA_CALL_UNHOLD) == 0) {
        ...
        status = pjsua_media_channel_create_sdp(
          call->index, call->inv->pool_prov, NULL, &sdp, NULL);
    }
    if (status != PJ_SUCCESS) {
                ...
                const pjsua_msg_data* msg_data)
{
    ...
    PJ_ASSERT_RETURN(
      call_id >= 0 && call_id < (int)pjsua_var.ua_cfg.max_calls && dest,
      PJ_EINVAL);
                         ...
                         unsigned options, const pjsua_msg_data* msg_data)
{
    ...
    PJ_ASSERT_RETURN(call_id >= 0 && call_id < (int)pjsua_var.ua_cfg.max_calls,
                     PJ_EINVAL);
    PJ_ASSERT_RETURN(
      dest_call_id >= 0 && dest_call_id < (int)pjsua_var.ua_cfg.max_calls,
      PJ_EINVAL);
    ...
    len = pjsip_uri_print(PJSIP_URI_IN_REQ_URI, uri, str_dest_buf + 1,
    ...
    call_id_len = (int)pj_strncpy2_escape(
      call_id_dest_buf, &dest_dlg->call_id->id, PJ_ARRAY_SIZE(call_id_dest_buf),
    ...
    len = pj_ansi_snprintf(
      str_dest_buf + str_dest.slen, sizeof(str_dest_buf) - str_dest.slen,
      "?%s"
      "Replaces=%.*s"
      "%%3Bto-tag%%3D%.*s"
      "%%3Bfrom-tag%%3D%.*s>",
      ((options & PJSUA_XFER_NO_REQUIRE_REPLACES) ? "" : "Require=replaces&"),
      call_id_len, call_id_dest_buf, (int)dest_dlg->remote.info->tag.slen,
      dest_dlg->remote.info->tag.ptr, (int)dest_dlg->local.info->tag.slen,
      dest_dlg->local.info->tag.ptr);

    PJ_ASSERT_ON_FAIL(
      len > 0 && len <= (int)sizeof(str_dest_buf) - str_dest.slen, {
          status = PJSIP_EURITOOLONG;
          goto on_error;
      });
                     ...
                     const pjsua_call_send_dtmf_param* param)
{
    ...
    PJ_ASSERT_RETURN(
      call_id >= 0 && call_id < (int)pjsua_var.ua_cfg.max_calls && param,
      PJ_EINVAL);

    PJ_LOG(4, (THIS_FILE, "Call %d sending DTMF %.*s using %s method", call_id,
               (int)param->digits.slen, param->digits.ptr,
                   ...
                   void* user_data)
{
    ...
    if (content) {
        PJ_LOG(4, (THIS_FILE, "Call %d sending %d bytes MESSAGE..", call_id,
                   (int)content->slen));
    } else {
        PJ_LOG(4, (THIS_FILE, "Call %d sending MESSAGE..", call_id));
    }
    ...
    status = pjsip_dlg_create_request(call->inv->dlg, &pjsip_message_method, -1,
                                      &tdata);
    if (status != PJ_SUCCESS) {
    ...
    /* Send the request. */
    status =
      pjsip_dlg_send_request(call->inv->dlg, tdata, pjsua_var.mod.id, im_data);
    if (status != PJ_SUCCESS) {
                           ...
                           const pjsua_msg_data* msg_data)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Call %d sending typing indication..", call_id));
    pj_log_push_indent();
    ...
    tdata->msg->body =
      pjsip_iscomposing_create_body(tdata->pool, is_typing, NULL, NULL, -1);
 ...
 */
PJ_DEF(pj_status_t)
pjsua_call_send_request(pjsua_call_id call_id, const pj_str_t* method_str,
                        const pjsua_msg_data* msg_data)
{
    pjsua_call* call;
    pjsip_dialog* dlg = NULL;
    ...
    PJ_LOG(4, (THIS_FILE, "Call %d sending %.*s request..", call_id,
               (int)method_str->slen, method_str->ptr));
    pj_log_push_indent();
...
static pj_bool_t is_non_av_fmt(const pjmedia_sdp_media* m, const pj_str_t* fmt)
{
...
static pj_bool_t check_ice_complete(pjsua_call* call, pj_bool_t* need_reinv)
{
    ...
    for (i = 0; i < call->med_cnt; ++i) {
        ...
        ice_info =
          (pjmedia_ice_transport_info*)pjmedia_transport_info_get_spc_info(
...
static pj_status_t process_pending_reinvite(pjsua_call* call)
{
    ...
    status =
      pjsua_media_channel_create_sdp(call->index, pool, NULL, &new_offer, NULL);
    if (status != PJ_SUCCESS) {
        ...
        ++call->lock_codec.retry_cnt < LOCK_CODEC_MAX_RETRY)
    {
        ...
        pjsua_perror(THIS_FILE, "Error creating UPDATE/re-INVITE", status);
        return status;
    }
    ...
    status = pjsip_inv_send_msg(inv, tdata);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Error sending UPDATE/re-INVITE", status);
        return status;
    }
                                    ...
                                    struct pj_timer_entry* te)
{
    ...
    PJ_LOG(4,
           (THIS_FILE, "Call %d: ICE trickle retransmitting 18x (retrans #%d)",
...
static void trickle_ice_recv_sip_info(pjsua_call* call, pjsip_rx_data* rdata)
{
    ...
    if (!sdp_info->sdp) {
        ...
        pjsua_perror(THIS_FILE,
                     "Failed to parse trickle ICE SDP in "
                     "incoming INFO",
                     err);
        return;
    }
    ...
    for (i = 0; i < sdp_info->sdp->media_count; ++i) {
        ...
        status = pjmedia_ice_trickle_decode_sdp(
          sdp_info->sdp, i, &mid, &ufrag, &pwd, &cand_cnt, cand, &end_of_cand);
        if (status != PJ_SUCCESS) {
            pjsua_perror(THIS_FILE,
                         "Failed to retrive ICE candidates from "
                         "SDP in incoming INFO",
                         status);
            continue;
        }
        ...
        for (j = 0; j < med_cnt; ++j) {
            pjsua_call_media* cm =
              use_med_prov ? &call->media_prov[j] : &call->media[j];
            tp = cm->tp_orig;
        ...
        if (j == med_cnt) {
            pjsua_perror(THIS_FILE,
                         "Cannot add remote candidates from SDP in "
        ...
        if (status != PJ_SUCCESS) {
            pjsua_perror(THIS_FILE,
                         "Failed to update ICE checklist from "
                         "incoming INFO",
                         status);
        }
    }
                                      ...
                                      struct pj_timer_entry* te)
{
    ...
    if (!forced) {
        for (i = 0; i < med_cnt; ++i) {
            pjsua_call_media* cm =
              use_med_prov ? &call->media_prov[i] : &call->media[i];
    ...
    status = pjmedia_endpt_create_base_sdp(pjsua_var.med_endpt, tmp_pool, NULL,
                                           &orig_addr, &sdp);
    if (status != PJ_SUCCESS)
        goto on_return;
    ...
    for (i = 0; i < med_cnt; ++i) {
        ...
        status =
          pjmedia_ice_trickle_send_local_cand(tp, tmp_pool, sdp, &end_of_cand);
    ...
    msg_data.msg_body.slen =
      pjmedia_sdp_print(sdp, msg_data.msg_body.ptr, PJSIP_MAX_PKT_LEN);
    if (msg_data.msg_body.slen == -1) {
        PJ_LOG(3, (THIS_FILE,
                   "Warning! Call %d: ICE trickle failed to print SDP for "
                   "SIP INFO due to insufficient buffer",
                   call->index));
        goto on_return;
    }
    ...
    if (all_end_of_cand) {
        PJ_LOG(4, (THIS_FILE,
                   "Call %d: ICE trickle stopped trickling "
 ...
 *   to start trickling.
 */
void pjsua_ice_check_start_trickling(pjsua_call* call, pj_bool_t forceful,
                                     pjsip_event* e)
{
    pjsip_inv_session* inv = call->inv;
    ...
    if (inv->state == PJSIP_INV_STATE_CONFIRMED) {
        ...
        else
        {
            ...
            if (has_100rel) {
                ...
                if (msg->type != PJSIP_REQUEST_MSG ||
                    pjsip_method_cmp(&msg->line.req.method, &pjsip_info_method))
                {
                    return;
                }
    ...
    if (forceful || call->trickle_ice.trickling < PJSUA_OP_STATE_RUNNING) {
        ...
        PJ_LOG(
          4, (THIS_FILE, "Call %d: ICE trickle start trickling", call->index));
    }
}
...
static void pjsua_call_on_state_changed(pjsip_inv_session* inv, pjsip_event* e)
{
    ...
    case PJSIP_INV_STATE_DISCONNECTED:
        ...
        {
            call->last_code =
              (pjsip_status_code)e->body.tsx_state.tsx->status_code;
            pj_strncpy(&call->last_text, &e->body.tsx_state.tsx->status_text,
                       sizeof(call->last_text_buf_));
        } else {
 ...
 */
        ...
        if (st_code != -1) {
            ...
            status = pjsip_xfer_notify(call->xfer_sub, ev_state, st_code, NULL,
                                       &tdata);
            if (status != PJ_SUCCESS) {
    ...
    if (inv->state == PJSIP_INV_STATE_DISCONNECTED &&
        e->type == PJSIP_EVENT_TSX_STATE && !call->hanging_up && call->inv &&
...
static void call_disconnect(pjsip_inv_session* inv, int code)
{
        ...
        code == PJSIP_SC_NOT_ACCEPTABLE_HERE)
    {
        ...
        status = pjmedia_endpt_create_sdp(pjsua_var.med_endpt, tdata->pool, 1,
                                          &ti.sock_info, &local_sdp);
        if (status == PJ_SUCCESS) {
            pjsip_create_sdp_body(tdata->pool, local_sdp, &tdata->msg->body);
        }
    }
#endif
                                       ...
                                       pj_status_t status)
{
    ...
    status = pjmedia_sdp_neg_get_active_local(call->inv->neg, &local_sdp);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Unable to retrieve currently active local SDP",
                     status);
        // call_disconnect(inv, PJSIP_SC_UNSUPPORTED_MEDIA_TYPE);
        goto on_return;
    }
    ...
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE,
                     "Unable to retrieve currently active remote SDP", status);
        // call_disconnect(inv, PJSIP_SC_UNSUPPORTED_MEDIA_TYPE);
        goto on_return;
    }
...
static pj_status_t modify_sdp_of_call_hold(pjsua_call* call, pj_pool_t* pool,
                                           pjmedia_sdp_session* sdp,
                                           ...
                                           pjmedia_sdp_session** p_sdp)
{
    ...
    /* Create new offer */
    status =
      pjsua_media_channel_create_sdp(call->index, pool, NULL, &sdp, NULL);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Unable to create local SDP", status);
        return status;
    }
                                   ...
                                   struct pjsip_inv_on_rx_offer_cb_param* param)
{
    ...
    PJ_LOG(4,
           (THIS_FILE, "Call %d: received updated media offer", call->index));
        ...
        param->rdata->msg_info.msg->line.req.method.id == PJSIP_INVITE_METHOD)
    {
         ...
         * call it first.
         */
        (*pjsua_var.ua_cfg.cb.on_call_rx_reinvite)(call->index, offer,
                                                   (pjsip_rx_data*)param->rdata,
                                                   NULL, &async, &code, &opt);
        if (async) {
            pjsip_tx_data* response;

            status = pjsip_inv_initial_answer(inv, (pjsip_rx_data*)param->rdata,
                                              100, NULL, NULL, &response);
            if (status != PJ_SUCCESS) {
                PJ_PERROR(
                  3, (THIS_FILE, status, "Failed to create initial answer"));
                goto on_return;
            }
            ...
            if (status != PJ_SUCCESS) {
                PJ_PERROR(3,
                          (THIS_FILE, status, "Failed to send initial answer"));
                goto on_return;
            }

            PJ_LOG(4, (THIS_FILE,
                       "App will manually answer the re-INVITE "
                       "on call %d",
                       call->index));
        }
        if (code != PJSIP_SC_OK) {
            PJ_LOG(4, (THIS_FILE,
                       "Rejecting re-INVITE updated media offer "
                       "on call %d",
                       call->index));
            goto on_return;
        }
    ...
    if (pjsua_var.ua_cfg.cb.on_call_rx_offer && !async) {
        ...
        (*pjsua_var.ua_cfg.cb.on_call_rx_offer)(call->index, offer, NULL, &code,
                                                &opt);
    ...
    for (i = 0; i < answer->media_count; ++i) {
            ...
            pj_strcmp2(&conn->addr, "0") == 0)
        {
            ...
            {
                a_conn =
                  PJ_POOL_ZALLOC_T(call->inv->pool_prov, pjmedia_sdp_conn);
                                       ...
                                       pjmedia_sdp_session** offer)
{
    ...
    if (call->hanging_up || pjsua_call_media_is_changing(call)) {
        *offer = NULL;
        PJ_LOG(1,
               (THIS_FILE, "Unable to create offer%s",
                call->hanging_up ? ", call hanging up" : ERR_MEDIA_CHANGING));
        goto on_return;
    }
    ...
    for (mi = 0; mi < call->med_cnt; ++mi) {
        ...
        pjmedia_transport_media_create(
          call_med->tp, call->inv->pool_prov,
          (call_med->enable_rtcp_mux ? PJMEDIA_TPMED_RTCP_MUX : 0), NULL, mi);
    ...
    if (pjsua_var.ua_cfg.cb.on_call_tx_offer) {
        (*pjsua_var.ua_cfg.cb.on_call_tx_offer)(call->index, NULL, &call->opt);
    }
    ...
    if (call->local_hold) {
        ...
        PJ_LOG(4,
               (THIS_FILE, "Call %d: asked to send a new offer", call->index));

        status = pjsua_media_channel_create_sdp(
          call->index, call->inv->pool_prov, NULL, offer, NULL);
    }
...
static void xfer_client_on_evsub_state(pjsip_evsub* sub, pjsip_event* event)
{
    ...
    if (pjsip_evsub_get_state(sub) == PJSIP_EVSUB_STATE_ACCEPTED) {
        ...
        if (refer_sub && pj_stricmp2(&refer_sub->hvalue, "false") == 0) {
            ...
            if (call && !call->hanging_up &&
                ...
                (*pjsua_var.ua_cfg.cb.on_call_transfer_status)(
                  call->index, 200, &ACCEPTED, PJ_TRUE, &cont);
            }
             ...
             */
            PJ_LOG(4, (THIS_FILE,
                       "Xfer subscription suppressed, terminating "
                       "event subcription..."));
            ...
            if (call && !call->hanging_up &&
                ...
                pj_bool_t cont = PJ_FALSE;
                (*pjsua_var.ua_cfg.cb.on_call_transfer_status)(
                  call->index, 100, &ACCEPTED, PJ_FALSE, &cont);
            }
        }
    }
    /*
             ...
             pjsip_evsub_get_state(sub) == PJSIP_EVSUB_STATE_TERMINATED)
    {
        ...
        cont = !is_last;
        (*pjsua_var.ua_cfg.cb.on_call_transfer_status)(
          call->index, status_line.code, &status_line.reason, is_last, &cont);
        ...
        if (status_line.code / 100 == 2 && !is_last) {
            ...
            status = pjsip_evsub_initiate(sub, pjsip_get_subscribe_method(), 0,
                                          &tdata);
            if (status == PJ_SUCCESS)
 ...
 */
static void on_call_transferred(pjsip_inv_session* inv, pjsip_rx_data* rdata)
{
    pj_status_t status;
    pjsip_tx_data* tdata;
     ...
     * request.
     */
    ref_by_hdr = (pjsip_hdr*)pjsip_msg_find_hdr_by_name(rdata->msg_info.msg,
                                                        &str_ref_by, NULL);

    /* Notify callback */
    code = PJSIP_SC_ACCEPTED;
    if (pjsua_var.ua_cfg.cb.on_call_transfer_request) {
        (*pjsua_var.ua_cfg.cb.on_call_transfer_request)(
          existing_call->index, &refer_to->hvalue, &code);
    }
    ...
    if (pjsua_var.ua_cfg.cb.on_call_transfer_request2) {
        (*pjsua_var.ua_cfg.cb.on_call_transfer_request2)(
          existing_call->index, &refer_to->hvalue, &code, &call_opt);
    }
    ...
    PJ_LOG(3,
           (THIS_FILE, "Call to %.*s is being transferred to %.*s",
            (int)inv->dlg->remote.info_str.slen, inv->dlg->remote.info_str.ptr,
            (int)refer_to->hvalue.slen, refer_to->hvalue.ptr));
    ...
    if (no_refer_sub) {
        ...
        status =
          pjsip_dlg_create_response(inv->dlg, rdata, code, NULL, &tdata2);
        if (status != PJ_SUCCESS) {
        ...
        hdr = (pjsip_hdr*)pjsip_generic_string_hdr_create(
          tdata2->pool, &str_refer_sub, &str_false);
        pjsip_msg_add_hdr(tdata2->msg, hdr);

        /* Send answer */
        status =
          pjsip_dlg_send_response(inv->dlg, pjsip_rdata_get_tsx(rdata), tdata2);
        if (status != PJ_SUCCESS) {
            pjsua_perror(THIS_FILE, "Unable to create 2xx response to REFER",
                         status);
            goto on_return;
        ...
        if (refer_sub) {
            ...
            hdr = (pjsip_hdr*)pjsip_generic_string_hdr_create(
              inv->dlg->pool, &str_refer_sub, &str_true);
        ...
        status =
          pjsip_xfer_notify(sub, PJSIP_EVSUB_STATE_ACTIVE, 100, NULL, &tdata);
        if (status != PJ_SUCCESS) {
    ...
    if (ref_by_hdr != NULL) {
        pjsip_hdr* dup =
          (pjsip_hdr*)pjsip_hdr_clone(rdata->tp_info.pool, ref_by_hdr);
    ...
    tmp = pj_str(uri);
    status =
      pjsua_call_make_call(existing_call->acc_id, &tmp, &call_opt,
                           existing_call->user_data, &msg_data, &new_call);
    if (status != PJ_SUCCESS) {
                                            ...
                                            pjsip_event* e)
{
    ...
    if (tsx->role == PJSIP_ROLE_UAC &&
        (tsx->status_code == 401 || tsx->status_code == 407) && tsx->last_tx &&
        tsx->last_tx->auth_retry)
    {
        goto on_return;
    }
        ...
        if (tsx->status_code >= 200) {
            ...
            if (im_data) {
                ...
                if (pjsua_var.ua_cfg.cb.on_pager_status) {
                    pjsua_var.ua_cfg.cb.on_pager_status(
                      im_data->call_id, &im_data->to, &im_body,
                      im_data->user_data, (pjsip_status_code)tsx->status_code,
                      &tsx->status_text);
                }
                ...
                if (pjsua_var.ua_cfg.cb.on_pager_status2) {
                    ...
                    pjsua_var.ua_cfg.cb.on_pager_status2(
                      im_data->call_id, &im_data->to, &im_body,
                      im_data->user_data, (pjsip_status_code)tsx->status_code,
                      &tsx->status_text, tsx->last_tx, rdata, im_data->acc_id);
                }
            }
        }
                ...
                tsx->status_code != 422))
    {
        if (tsx->status_code / 100 == 2) {
            ...
            if (call->hanging_up) {
                PJ_LOG(3, (THIS_FILE,
                           "Unsuccessful in cancelling the original "
            ...
            if (tsx->last_tx == (pjsip_tx_data*)call->hold_msg) {
                ...
                call->local_hold = PJ_TRUE;
                PJ_LOG(3, (THIS_FILE,
                           "Error releasing hold on call %d "
                           "(reason=%d)",
                           call->index, tsx->status_code));
            }
        }
               ...
               pjsip_method_cmp(&tsx->method, &pjsip_info_method) == 0)
    {
        ...
        if (body && body->len &&
            ...
            pj_stricmp(&body->content_type.subtype, &STR_MEDIA_CONTROL_XML) ==
              0)
        {
            pjsip_tx_data* tdata;
            pj_str_t control_st;
            ...
            if (status == PJ_SUCCESS) {
                status = pjsip_endpt_create_response(tsx->endpt, rdata, 200,
                                                     NULL, &tdata);
                if (status == PJ_SUCCESS)
                    status = pjsip_tsx_send_msg(tsx, tdata);
            } else {
                status = pjsip_endpt_create_response(tsx->endpt, rdata, 400,
                                                     NULL, &tdata);
                if (status == PJ_SUCCESS)
                    status = pjsip_tsx_send_msg(tsx, tdata);
            }
                    ...
                    if (val) {
                        ...
                        if (val && is_handled) {
                            ...
                            if ((count_equal_sign == 1) &&
                                ...
                                is_handled = PJ_FALSE;
                                PJ_LOG(
                                  2, (THIS_FILE, "Invalid dtmf-relay format"));
                            }
                        }
                        ...
                        if (is_handled) {
                            ...
                            if (pjsua_var.ua_cfg.cb.on_dtmf_event) {
                                ...
                                evt.timestamp =
                                  pj_elapsed_msec(&begin_of_time, &timestamp);
                            ...
                            status = pjsip_endpt_create_response(
                              tsx->endpt, rdata, 200, NULL, &tdata);
                            if (status == PJ_SUCCESS)
                                status = pjsip_tsx_send_msg(tsx, tdata);
                        }
                    }
                 ...
                 e->body.tsx_state.prev_state != PJSIP_TSX_STATE_COMPLETED)))
    {
            ...
            pj_stricmp(&body->content_type.subtype, &STR_DTMF_RELAY) == 0)
        {
            ...
            if (tsx->status_code >= 200 && tsx->status_code < 300) {
                PJ_LOG(4,
                       (THIS_FILE, "Call %d: DTMF sent successfully with INFO",
                        call->index));
                                                  ...
                                                  const pjsip_event* e)
{
    ...
    if (!call->hanging_up && pjsua_var.ua_cfg.cb.on_call_redirected) {
        op = (*pjsua_var.ua_cfg.cb.on_call_redirected)(call->index, target, e);
    } else {
        if (!call->hanging_up) {
            PJ_LOG(4, (THIS_FILE,
                       "Unhandled redirection for call %d "
