...
static void pjsua_media_config_dup(pj_pool_t* pool, pjsua_media_config* dst,
...
pj_status_t pjsua_media_subsys_init(const pjsua_media_config* cfg)
{
    ...
    pjsua_var.aud_svmask &=
      ~(PJMEDIA_AUD_DEV_CAP_EC | PJMEDIA_AUD_DEV_CAP_EC_TAIL);
    ...
    if (pjsua_var.media_cfg.has_ioqueue && pjsua_var.media_cfg.thread_cnt == 0)
    {
    ...
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Media stack initialization has returned error",
                     status);
        goto on_error;
    }
                                        ...
                                        pjmedia_sock_info* skinfo)
{
    ...
    for (i = 0; i < RTP_RETRY; ++i, acc->next_rtp_port += 2) {
        ...
        status = pj_sock_apply_qos2(sock[0], cfg->qos_type, &cfg->qos_params, 2,
                                    THIS_FILE, "RTP socket");
        ...
        status =
          pj_sock_bind(sock[0], &bound_addr, pj_sockaddr_get_len(&bound_addr));
        if (status != PJ_SUCCESS) {
        ...
        status = pj_sock_apply_qos2(sock[1], cfg->qos_type, &cfg->qos_params, 2,
                                    THIS_FILE, "RTCP socket");
        ...
        /* Bind RTCP socket */
        pj_sockaddr_set_port(&bound_addr,
                             (pj_uint16_t)(acc->next_rtp_port + 1));
        status =
          pj_sock_bind(sock[1], &bound_addr, pj_sockaddr_get_len(&bound_addr));
        if (status != PJ_SUCCESS) {
            pj_sock_close(sock[0]);
            sock[0] = PJ_INVALID_SOCKET;
  ...
  PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT != 0
                   ...
                   PJSUA_STUN_RETRY_ON_FAILURE))
            {
                ...
                PJ_LOG(4, (THIS_FILE,
                           "Failed to get STUN mapped address, "
                ...
                if (pjsua_var.stun_srv_idx < pjsua_var.ua_cfg.stun_srv_cnt - 1)
                {
                     ...
                     */
                    pj_array_erase(pjsua_var.ua_cfg.stun_srv, sizeof(pj_str_t),
                                   pjsua_var.ua_cfg.stun_srv_cnt,
                                   pjsua_var.stun_srv_idx);

                    pj_array_insert(pjsua_var.ua_cfg.stun_srv, sizeof(pj_str_t),
                                    pjsua_var.ua_cfg.stun_srv_cnt - 1,
                                    pjsua_var.ua_cfg.stun_srv_cnt - 1, &srv);

                    PJSUA_UNLOCK();
                }
                status =
                  pjsua_update_stun_servers(pjsua_var.ua_cfg.stun_srv_cnt,
                                            pjsua_var.ua_cfg.stun_srv, PJ_TRUE);
                if (status == PJ_SUCCESS) {
                    if (pjsua_var.stun_srv.addr.sa_family != 0) {
                        pj_sockaddr_print(&pjsua_var.stun_srv, ip_addr,
                                          sizeof(ip_addr), 0);
                        stun_srv = pj_str(ip_addr);
                    } else {
                        stun_srv.slen = 0;
                    ...
                    stun_opt.port1 = stun_opt.port2 =
                      pj_sockaddr_get_port(&pjsua_var.stun_srv);
                    status = pjstun_get_mapped_addr2(
                      &pjsua_var.cp.factory, &stun_opt, 2, sock, resolved_addr);
                }
            }
            ...
            if (status != PJ_SUCCESS) {
                ...
                PJ_LOG(4,
                       (THIS_FILE, "Ignoring STUN resolve error %d", status));
                                              ...
                                              pjsua_call_media* call_med)
{
    ...
    status = pjmedia_transport_udp_attach(pjsua_var.med_endpt, NULL, &skinfo, 0,
                                          &call_med->tp);
    if (status != PJ_SUCCESS) {
...
static pj_status_t create_loop_media_transport(
  const pjsua_transport_config* cfg, pjsua_call_media* call_med)
{
    pj_status_t status;
    ...
    status =
      pjmedia_transport_loop_create2(pjsua_var.med_endpt, &opt, &call_med->tp);
    if (status != PJ_SUCCESS) {
...
static void on_ice_complete(pjmedia_transport* tp, pj_ice_strans_op op,
                            pj_status_t result)
{
    ...
    case PJ_ICE_STRANS_OP_NEGOTIATION:
        if (result == PJ_SUCCESS) {
            ...
            pj_sockaddr_cp(&call_med->rtp_addr,
        ...
        if (call->trickle_ice.trickling < PJSUA_OP_STATE_DONE) {
            ...
            PJ_LOG(4, (THIS_FILE,
                       "Call %d: ICE trickle stopped trickling as "
                       "ICE nego completed",
                       call->index));
        }
    ...
    case PJ_ICE_STRANS_OP_ADDR_CHANGE:
        ...
        if (!call->hanging_up &&
            ...
            info.status = result;
            info.ext_info = &op;
            (*pjsua_var.ua_cfg.cb.on_call_media_transport_state)(call->index,
                                                                 &info);
        }
...
static pj_status_t parse_host_port(const pj_str_t* host_port, pj_str_t* host,
                                   pj_uint16_t* port)
{
    pj_str_t str_port;
...
static pj_status_t create_ice_media_transport(const pjsua_transport_config* cfg,
                                              pjsua_call_media* call_med,
                                              pj_bool_t async)
{
    ...
    if (ice_cfg.stun_tp_cnt) {
        ...
        for (i = 0; i < ice_cfg.stun_tp_cnt; ++i) {
            ...
            pj_sockaddr_init(
              ice_cfg.stun_tp[i].af, &ice_cfg.stun_tp[i].cfg.bound_addr,
              (ice_cfg.stun_tp[i].af == pj_AF_INET() ? &cfg->bound_addr :
                                                       &IN6_ADDR_ANY),
    ...
    if (acc_cfg->turn_cfg.enable_turn) {
        ...
        status =
          parse_host_port(&acc_cfg->turn_cfg.turn_server,
                          &ice_cfg.turn_tp[0].server, &ice_cfg.turn_tp[0].port);
        ...
        for (i = 0; i < ice_cfg.turn_tp_cnt; i++) {
            ...
            /* Configure binding address */
            pj_sockaddr_init(
              ice_cfg.turn_tp[i].af, &ice_cfg.turn_tp[i].cfg.bound_addr,
              (ice_cfg.turn_tp[i].af == pj_AF_INET() ? &cfg->bound_addr :
                                                       &IN6_ADDR_ANY),
              (pj_uint16_t)cfg->port);
    ...
    status = pjmedia_ice_create3(pjsua_var.med_endpt, name, comp_cnt, &ice_cfg,
                                 &ice_cb, PJSUA_ICE_TRANSPORT_OPTION, call_med,
                                 &call_med->tp);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Unable to create ICE media transport", status);
        goto on_error;
    }
    ...
    if (!async && !trickle) {
        ...
        pjsip_dialog* dlg =
          call_med->call->inv ? call_med->call->inv->dlg : NULL;
        if (has_pjsua_lock)
            PJSUA_UNLOCK();
        if (dlg) {
    ...
    if (!call_med->tp) {
         ...
         */
        PJ_LOG(4, (THIS_FILE,
                   "Media transport initialization cancelled "
 ...
 */
static void sort_media(const pjmedia_sdp_session* sdp, const pj_str_t* type,
                       pjmedia_srtp_use use_srtp, pj_uint8_t midx[],
                       unsigned* p_count, unsigned* p_total_count)
{
    unsigned i;
    unsigned count = 0;
 ...
 * first in the array.
 */
static void sort_media2(const pjsua_call_media* call_med, pj_bool_t check_tp,
                        unsigned call_med_cnt, pjmedia_type type,
                        pj_uint8_t midx[], unsigned* p_count,
...
void call_med_event_cb(void* user_data)
{
    ...
    (*pjsua_var.ua_cfg.cb.on_call_media_event)(eve->call_id, eve->med_idx,
                                               &eve->event);
...
pj_status_t call_media_on_event(pjmedia_event* event, void* user_data)
{
    pjsua_call_media* call_med = (pjsua_call_media*)user_data;
    ...
    pjmedia_fourcc_name(event->type, ev_name);
    PJ_LOG(5, (THIS_FILE,
               "Call %d: Media %d: Received media event, type=%s, "
               "src=%p, epub=%p",
               call->index, call_med->idx, ev_name, event->src, event->epub));
    ...
    case PJMEDIA_EVENT_FMT_CHANGED:
        ...
        if (call_med->strm.v.strm_dec_slot != PJSUA_INVALID_ID) {
            ...
            status = pjmedia_vid_stream_get_port(
              call_med->strm.v.stream, PJMEDIA_DIR_DECODING, &strm_dec);
            if (status != PJ_SUCCESS)
                break;
    ...
    case PJMEDIA_EVENT_VID_DEV_ERROR: {
        PJ_PERROR(3, (THIS_FILE, event->data.vid_dev_err.status,
                      ...
                      event->data.vid_dev_err.id, call->index));
    } break;
#endif
...
static void on_srtp_nego_complete(pjmedia_transport* tp, pj_status_t result)
{
    pjsua_call_media* call_med = (pjsua_call_media*)tp->user_data;
    pjsua_call* call;
    ...
    call = call_med->call;
    PJ_PERROR(
      4, (THIS_FILE, result, "Call %d: Media %d: SRTP negotiation completes",
          call->index, call_med->idx));
                                      ...
                                      int* sip_err_code)
{
    ...
    if (!call_med->tp_orig && pjsua_var.ua_cfg.cb.on_create_media_transport) {
    ...
    if (!call_med->tp_orig) {
        ...
        if (pjsua_var.ua_cfg.cb.on_create_media_transport_srtp) {
            ...
            PJ_LOG(1, (THIS_FILE,
                       "Warning: on_create_media_transport_srtp "
...
pj_status_t pjsua_call_media_init(pjsua_call_media* call_med, pjmedia_type type,
                                  const pjsua_transport_config* tcfg,
                                  int security_level, int* sip_err_code,
                                  pj_bool_t async, pjsua_med_tp_state_cb cb)
{
    pj_status_t status = PJ_SUCCESS;
    ...
    for (mi = 0; mi < call->med_prov_cnt; ++mi) {
        ...
        if (call_med->tp && call_med->tp_st == PJSUA_MED_TP_IDLE) {
            ...
            if (call_med->use_custom_med_tp) {
                ...
                call_med->tp = (*pjsua_var.ua_cfg.cb.on_create_media_transport)(
                  call_id, mi, call_med->tp, custom_med_tp_flags);
                if (!call_med->tp) {
            ...
            if (call_med->tp) {
                unsigned options =
                  (call_med->enable_rtcp_mux ? PJMEDIA_TPMED_RTCP_MUX : 0);
                status = pjmedia_transport_media_create(
                  call_med->tp, tmp_pool, options, call->async_call.rem_sdp,
                  mi);
            }
            if (status != PJ_SUCCESS) {
                call->med_ch_info.status = status;
...
void pjsua_media_prov_clean_up(pjsua_call_id call_id)
{
    ...
    if (call->med_prov_cnt > call->med_cnt) {
        PJ_LOG(4, (THIS_FILE,
                   "Call %d: cleaning up provisional media, "
...
pj_status_t pjsua_media_channel_init(pjsua_call_id call_id, pjsip_role_e role,
                                     int security_level, pj_pool_t* tmp_pool,
                                     const pjmedia_sdp_session* rem_sdp,
                                     int* sip_err_code, pj_bool_t async,
                                     pjsua_med_tp_state_cb cb)
{
    ...
    if (async) {
        pj_pool_t* tmppool =
          (call->inv ? call->inv->pool_prov : call->async_call.dlg->pool);
    ...
    PJ_LOG(4, (THIS_FILE, "Call %d: %sinitializing media..", call_id,
               (reinit ? "re-" : "")));
    ...
    if (rem_sdp) {
         ...
         * from the remote SDP.
         */
        sort_media(rem_sdp, &STR_AUDIO, acc->cfg.use_srtp, maudidx, &maudcnt,
                   &mtotaudcnt);

#if PJMEDIA_HAS_VIDEO
        sort_media(rem_sdp, &STR_VIDEO, acc->cfg.use_srtp, mvididx, &mvidcnt,
                   &mtotvidcnt);
#else
        ...
        if (call->med_prov_cnt < rem_sdp->media_count)
            call->med_prov_cnt =
              PJ_MIN(rem_sdp->media_count, PJSUA_MAX_CALL_MEDIA);
        ...
        if (reinit) {
            ...
            if (maudcnt < call->opt.aud_cnt && mtotaudcnt < call->opt.aud_cnt) {
            ...
            if (mvidcnt < call->opt.vid_cnt && mtotvidcnt < call->opt.vid_cnt) {
    ...
    for (mi = 0; mi < call->med_prov_cnt; ++mi) {
        ...
        if (call->opt.flag & PJSUA_CALL_SET_MEDIA_DIR) {
            ...
            PJ_LOG(4, (THIS_FILE,
                       "Call %d: setting media direction "
                       "#%d to %d.",
        ...
        if (enabled) {
            ...
            status = pjsua_call_media_init(
              call_med, media_type, &acc->cfg.rtp_cfg, security_level,
              sip_err_code, async, (async ? &media_channel_init_cb : NULL));
                                           ...
                                           int* sip_err_code)
{
    enum
    {
        MAX_MEDIA = PJSUA_MAX_CALL_MEDIA
    };
    ...
    for (mi = 0; mi < call->med_prov_cnt; ++mi) {
        ...
        if (call_med->tp == NULL || call_med->tp_st == PJSUA_MED_TP_DISABLED) {
            ...
            if (rem_sdp) {
                ...
                m =
                  pjmedia_sdp_media_clone_deactivate(pool, rem_sdp->media[mi]);
            } else {
        ...
        case PJMEDIA_TYPE_AUDIO:
            status = pjmedia_endpt_create_audio_sdp(
              pjsua_var.med_endpt, pool, &tpinfo.sock_info, &param, &m);
            break;
#if defined(PJMEDIA_HAS_VIDEO) && (PJMEDIA_HAS_VIDEO != 0)
        case PJMEDIA_TYPE_VIDEO:
            status = pjmedia_endpt_create_video_sdp(
              pjsua_var.med_endpt, pool, &tpinfo.sock_info, &param, &m);
            break;
#endif
        default:
        ...
        m->attr[m->attr_count++] =
          pjmedia_sdp_attr_create_ssrc(pool, call_med->ssrc, &call->cname);

        /* Give to transport */
        status =
          pjmedia_transport_encode_sdp(call_med->tp, pool, sdp, rem_sdp, mi);
        if (status != PJ_SUCCESS) {
            if (sip_err_code)
        ...
        /* Setup RTCP-FB */
        {
            ...
            if (rem_sdp) {
                ...
                rtcp_cfg.dont_use_avpf = !PJMEDIA_TP_PROTO_HAS_FLAG(
                  rem_proto, PJMEDIA_TP_PROFILE_RTCP_FB);
            } else {
            ...
            if (acc->cfg.rtcp_fb_cfg.cap_count) {
                ...
                status = pjmedia_rtcp_fb_encode_sdp(
                  pool, pjsua_var.med_endpt, &rtcp_cfg, sdp, mi, rem_sdp);
                if (status != PJ_SUCCESS) {
                    PJ_PERROR(3, (THIS_FILE, status,
    ...
    if (pjsua_var.ua_cfg.nat_type_in_sdp) {
        ...
        if (pjsua_var.ua_cfg.nat_type_in_sdp == 1) {
            value.slen = pj_ansi_snprintf(nat_info, sizeof(nat_info), "%d",
                                          pjsua_var.nat_type);
        } else {
            const char* type_name = pj_stun_get_nat_name(pjsua_var.nat_type);
            value.slen = pj_ansi_snprintf(nat_info, sizeof(nat_info), "%d %s",
                                          pjsua_var.nat_type, type_name);
        }
...
#if DISABLED_FOR_TICKET_1185 && defined(PJMEDIA_HAS_SRTP) && \
  (PJMEDIA_HAS_SRTP != 0)
    ...
    if (!call->hanging_up && pjsua_var.ua_cfg.cb.on_call_sdp_created) {
        (*pjsua_var.ua_cfg.cb.on_call_sdp_created)(call_id, sdp, pool, rem_sdp);
    }
...
static void stop_media_stream(pjsua_call* call, unsigned med_idx)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Media stream call%02d:%d is destroyed", call->index,
               med_idx));
...
pj_status_t pjsua_media_channel_deinit(pjsua_call_id call_id)
{
    ...
    for (mi = 0; mi < call->med_cnt; ++mi) {
        ...
        if (call_med->tp_st > PJSUA_MED_TP_IDLE) {
            ...
            call_med->prev_srtp_use =
              (srtp_info && srtp_info->active) ? PJ_TRUE : PJ_FALSE;
            if (call_med->prev_srtp_use)
                call_med->prev_srtp_info = *srtp_info;
            call_med->prev_ice_use =
              (ice_info && ice_info->active) ? PJ_TRUE : PJ_FALSE;
...
static pj_bool_t is_ice_running(pjmedia_transport* tp)
{
    ...
    ice_info = (pjmedia_ice_transport_info*)pjmedia_transport_info_get_spc_info(
      &tpinfo, PJMEDIA_TRANSPORT_TYPE_ICE);
...
static void check_srtp_roc(pjsua_call* call, unsigned med_idx,
...
#        endif
    ...
    if (ice_info && call_med->prev_ice_use) {
        ...
        if (local_change) {
            attr = pjmedia_sdp_attr_find(local_sdp->attr_count, local_sdp->attr,
                                         &STR_ICE_UFRAG, NULL);
            if (!pj_strcmp(&call_med->prev_ice_info.loc_ufrag, &attr->value)) {
                PJ_LOG(4, (THIS_FILE,
                           "ICE unchanged, SRTP TX ROC "
                           "maintained"));
        ...
        if (rem_change) {
            attr = pjmedia_sdp_attr_find(
              remote_sdp->attr_count, remote_sdp->attr, &STR_ICE_UFRAG, NULL);
            if (!pj_strcmp(&call_med->prev_ice_info.rem_ufrag, &attr->value)) {
                PJ_LOG(4, (THIS_FILE,
                           "ICE unchanged, SRTP RX ROC "
                           "maintained"));
                rem_change = PJ_FALSE;
            }
        }
    ...
    if (rem_change) {
        PJ_LOG(4, (THIS_FILE,
                   "Remote address change detected, "
...
static pj_bool_t is_media_changed(const pjsua_call* call, unsigned med_idx,
                                  const pjsua_stream_info* new_si_)
{
    ...
    if (call_med->type == PJMEDIA_TYPE_AUDIO) {
        ...
        if (pj_stricmp(&old_ci->encoding_name, &new_ci->encoding_name) ||
            ...
            old_si->rx_pt != new_si->rx_pt || old_si->tx_pt != new_si->tx_pt ||
                                       ...
                                       const pjmedia_sdp_session* remote_sdp)
{
    ...
    sort_media(local_sdp, &STR_AUDIO, acc->cfg.use_srtp, maudidx, &maudcnt,
               &mtotaudcnt);
#if PJMEDIA_HAS_VIDEO
    sort_media(local_sdp, &STR_VIDEO, acc->cfg.use_srtp, mvididx, &mvidcnt,
               &mtotvidcnt);
#else
    PJ_UNUSED_ARG(STR_VIDEO);
    mvidcnt = mtotvidcnt = 0;
         ...
         (acc->cfg.rtcp_fb_cfg.cap_count)))
    {
        ...
        if (acc->cfg.rtcp_fb_cfg.cap_count) {
            for (mi = 0; mi < local_sdp_renego->media_count; ++mi) {
                status = pjmedia_rtcp_fb_encode_sdp(
                  tmp_pool, pjsua_var.med_endpt, &acc->cfg.rtcp_fb_cfg,
    ...
    for (mi = 0; mi < call->med_cnt; ++mi) {
        ...
        if (mi >= local_sdp->media_count || mi >= remote_sdp->media_count) {
        ...
        if (call_med->type == PJMEDIA_TYPE_AUDIO) {
            ...
            status = pjmedia_stream_info_from_sdp(
              si, tmp_pool, pjsua_var.med_endpt, local_sdp, remote_sdp, mi);
            if (status != PJ_SUCCESS) {
            ...
            if (pjsua_var.media_cfg.ptime != 0 && si->param) {
                si->param->setting.frm_per_pkt =
                  (pj_uint8_t)(pjsua_var.media_cfg.ptime /
...
#    endif
            ...
            if (local_sdp->media[mi]->desc.port == 0) {
                ...
                if (call->inv->following_fork) {
                    ...
                    status = pjmedia_transport_media_create(
                      call_med->tp, tmp_pool, options, NULL, mi);
                    if (status != PJ_SUCCESS) {
                        PJ_PERROR(1, (THIS_FILE, status,
                ...
                status = pjmedia_transport_media_start(
                  call_med->tp, tmp_pool, local_sdp, remote_sdp, mi);
                if (status != PJ_SUCCESS) {
                    PJ_PERROR(1, (THIS_FILE, status,
                ...
                srtp_info =
                  (pjmedia_srtp_info*)pjmedia_transport_info_get_spc_info(
                ...
                if (media_changed) {
                    status = pjsua_aud_channel_update(
                      call_med, call->inv->pool, si, local_sdp, remote_sdp);
                    if (status != PJ_SUCCESS) {
                        PJ_PERROR(1, (THIS_FILE, status,
            ...
            if (status == PJ_SUCCESS) {
                ...
                len =
                  pj_ansi_sprintf(info + info_len, ", stream #%d: %.*s (%s)",
                                  mi, (int)si->fmt.encoding_name.slen,
                                  si->fmt.encoding_name.ptr, dir);
                if (len > 0)
                    info_len += len;
            ...
            if (si->dir == PJMEDIA_DIR_NONE) {
                ...
                if (media_changed) {
                    status = pjsua_vid_channel_update(
                      call_med, call->inv->pool, si, local_sdp, remote_sdp);
                    if (status != PJ_SUCCESS) {
                        PJ_PERROR(1, (THIS_FILE, status,
            ...
            /* Print info. */
            {
                ...
                len =
                  pj_ansi_sprintf(info + info_len, ", stream #%d: %.*s (%s)",
                                  mi, (int)si->codec_info.encoding_name.slen,
                                  si->codec_info.encoding_name.ptr, dir);
                if (len > 0)
                    info_len += len;
    ...
    on_check_med_status:
        if (status != PJ_SUCCESS) {
            ...
            if (status != PJMEDIA_EUNSUPMEDIATYPE) {
                ...
                PJ_PERROR(3, (THIS_FILE, status,
                              "Skipped updating media "
                              "call%02d:%d (media type=%s)",
 ...
 */
PJ_DEF(pj_status_t) pjsua_enum_codecs(pjsua_codec_info id[], unsigned* p_count)
{
...
pjsua_codec_set_priority(const pj_str_t* codec_id, pj_uint8_t priority)
{
    ...
    return pjmedia_codec_mgr_set_codec_priority(codec_mgr, codec_id, priority);
}
                      ...
                      const pjmedia_codec_param* param)
{
    ...
    if (count > 1 && pj_strnicmp2(codec_id, "G7221/16", 8) != 0 &&
