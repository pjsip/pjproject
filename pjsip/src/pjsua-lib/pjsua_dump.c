...
const char* good_number(char* buf, pj_int32_t val)
{
    if (val < 1000) {
        ...
        pj_ansi_sprintf(buf, "%d.%02dM", val / 1000000,
                        (val % 1000000) / 10000);
    }
...
static unsigned dump_media_stat(const char* indent, char* buf, unsigned maxlen,
                                ...
                                const char* rx_info, const char* tx_info)
{
    ...
    if (stat->rx.update_cnt == 0)
        ...
        sprintf(last_update, "%02ldh:%02ldm:%02ld.%03lds ago", now.sec / 3600,
    ...
    len = pj_ansi_snprintf(
      ...
      "%s        pkt loss=%d (%3.1f%%), discrd=%d (%3.1f%%), dup=%d (%2.1f%%), "
      "reord=%d (%3.1f%%)\n"
...
#endif
      "%s",
      indent, rx_info ? rx_info : "", last_update,

      indent, good_number(packets, stat->rx.pkt),
      good_number(bytes, stat->rx.bytes),
      good_number(ipbytes, stat->rx.bytes + stat->rx.pkt * 40),
      good_number(avg_bps, (pj_int32_t)((pj_int64_t)stat->rx.bytes * 8 * 1000 /
                                        PJ_TIME_VAL_MSEC(media_duration))),
      good_number(
        avg_ipbps,
        (pj_int32_t)(((pj_int64_t)stat->rx.bytes + stat->rx.pkt * 40) * 8 *
                     1000 / PJ_TIME_VAL_MSEC(media_duration))),
      indent, stat->rx.loss,
      (stat->rx.loss ? stat->rx.loss * 100.0 / (stat->rx.pkt + stat->rx.loss) :
                       0),
      stat->rx.discard,
      (stat->rx.discard ?
         ...
         0),
      stat->rx.dup,
      (stat->rx.dup ? stat->rx.dup * 100.0 / (stat->rx.pkt + stat->rx.loss) :
                      0),
      stat->rx.reorder,
      (stat->rx.reorder ?
         stat->rx.reorder * 100.0 / (stat->rx.pkt + stat->rx.loss) :
         0),
      indent, indent, stat->rx.loss_period.min / 1000.0,
      stat->rx.loss_period.mean / 1000.0, stat->rx.loss_period.max / 1000.0,
      stat->rx.loss_period.last / 1000.0,
      pj_math_stat_get_stddev(&stat->rx.loss_period) / 1000.0, indent,
      stat->rx.jitter.min / 1000.0, stat->rx.jitter.mean / 1000.0,
      stat->rx.jitter.max / 1000.0, stat->rx.jitter.last / 1000.0,
      pj_math_stat_get_stddev(&stat->rx.jitter) / 1000.0,
#if defined(PJMEDIA_RTCP_STAT_HAS_RAW_JITTER) && \
  PJMEDIA_RTCP_STAT_HAS_RAW_JITTER != 0
      indent, stat->rx_raw_jitter.min / 1000.0,
      stat->rx_raw_jitter.mean / 1000.0, stat->rx_raw_jitter.max / 1000.0,
      stat->rx_raw_jitter.last / 1000.0,
      pj_math_stat_get_stddev(&stat->rx_raw_jitter) / 1000.0,
#endif
#if defined(PJMEDIA_RTCP_STAT_HAS_IPDV) && PJMEDIA_RTCP_STAT_HAS_IPDV != 0
      indent, stat->rx_ipdv.min / 1000.0, stat->rx_ipdv.mean / 1000.0,
      stat->rx_ipdv.max / 1000.0, stat->rx_ipdv.last / 1000.0,
    ...
    len = pj_ansi_snprintf(
      ...
      "%s        total %spkt %sB (%sB +IP hdr) @avg=%sbps/%sbps\n"
      "%s        pkt loss=%d (%3.1f%%), dup=%d (%3.1f%%), reorder=%d "
      "(%3.1f%%)\n"
      "%s              (msec)    min     avg     max     last    dev \n"
      "%s        loss period: %7.3f %7.3f %7.3f %7.3f %7.3f\n"
      "%s        jitter     : %7.3f %7.3f %7.3f %7.3f %7.3f\n",
      indent, tx_info, last_update,

      indent, good_number(packets, stat->tx.pkt),
      good_number(bytes, stat->tx.bytes),
      good_number(ipbytes, stat->tx.bytes + stat->tx.pkt * 40),
      good_number(avg_bps, (pj_int32_t)((pj_int64_t)stat->tx.bytes * 8 * 1000 /
                                        PJ_TIME_VAL_MSEC(media_duration))),
      good_number(
        avg_ipbps,
        (pj_int32_t)(((pj_int64_t)stat->tx.bytes + stat->tx.pkt * 40) * 8 *
                     1000 / PJ_TIME_VAL_MSEC(media_duration))),

      indent, stat->tx.loss,
      (stat->tx.loss ? stat->tx.loss * 100.0 / stat->tx.pkt : 0), stat->tx.dup,
      (stat->tx.dup ? stat->tx.dup * 100.0 / stat->tx.pkt : 0),
      stat->tx.reorder,
      (stat->tx.reorder ? stat->tx.reorder * 100.0 / stat->tx.pkt : 0),

      indent, indent, stat->tx.loss_period.min / 1000.0,
      stat->tx.loss_period.mean / 1000.0, stat->tx.loss_period.max / 1000.0,
      stat->tx.loss_period.last / 1000.0,
      pj_math_stat_get_stddev(&stat->tx.loss_period) / 1000.0, indent,
      stat->tx.jitter.min / 1000.0, stat->tx.jitter.mean / 1000.0,
      stat->tx.jitter.max / 1000.0, stat->tx.jitter.last / 1000.0,
    ...
    len = pj_ansi_snprintf(
      p, end - p, "%s     RTT msec      : %7.3f %7.3f %7.3f %7.3f %7.3f\n",
      indent, stat->rtt.min / 1000.0, stat->rtt.mean / 1000.0,
      stat->rtt.max / 1000.0, stat->rtt.last / 1000.0,
...
static void dump_media_session(const char* indent, char* buf, unsigned maxlen,
                               pjsua_call* call)
{
    unsigned i;
    char *p = buf, *end = buf + maxlen;
    ...
    for (i = 0; i < call->med_cnt; ++i) {
        ...
        {
            len = pj_ansi_snprintf(p, end - p, "%s  #%d %s deactivated\n",
        ...
        if (call_med->type == PJMEDIA_TYPE_AUDIO) {
            ...
            pj_ansi_snprintf(rx_info, sizeof(rx_info), "pt=%d,", info.rx_pt);
            pj_ansi_snprintf(
              tx_info, sizeof(tx_info), "pt=%d, ptime=%d,", info.tx_pt,
              info.param->setting.frm_per_pkt * info.param->info.frm_ptime);
...
#    if defined(PJMEDIA_HAS_VIDEO) && (PJMEDIA_HAS_VIDEO != 0)
            ...
            if (call_med->dir & PJMEDIA_DIR_DECODING) {
                ...
                pj_ansi_snprintf(rx_info, sizeof(rx_info),
                                 "pt=%d, size=%dx%d, fps=%.2f,", info.rx_pt,
            ...
            if (call_med->dir & PJMEDIA_DIR_ENCODING) {
                ...
                pj_ansi_snprintf(tx_info, sizeof(tx_info),
                                 "pt=%d, size=%dx%d, fps=%.2f,", info.tx_pt,
                                 vfd->size.w, vfd->size.h,
                                 vfd->fps.num * 1.0 / vfd->fps.denum);
            }
#    endif /* PJMEDIA_HAS_VIDEO */
        ...
        len = pj_ansi_snprintf(p, end - p, "%s  #%d %s%s, %s, peer=%s\n",
                               indent, call_med->idx, media_type_str,
                               codec_info, dir_str, rem_addr);
        if (len < 1 || len >= end - p) {
            *p = '\0';
            return;
        }
            ...
            if (pjsua_snd_is_active()) {
                ...
                if (status == PJ_SUCCESS) {
                    len = pj_ansi_snprintf(p, end - p, "   %s  EC stat: %.*s\n",
                                           indent, (int)ec_stat.stat_info.slen,
            ...
            if (tp_info.specific_info_cnt > 0) {
                ...
                for (j = 0; j < tp_info.specific_info_cnt; ++j) {
                    if (tp_info.spc_info[j].type == PJMEDIA_TRANSPORT_TYPE_SRTP)
                    {
                        ...
                        len = pj_ansi_snprintf(
                          p, end - p, "   %s  SRTP status: %s Crypto-suite: %s",
                          indent, (srtp_info->active ? "Active" : "Not active"),
                          policy_name);
                        if (len > 0 && len < end - p) {
                            p += len;
                            *p++ = '\n';
                        ...
                        ii =
                          (const pjmedia_ice_transport_info*)tp_info.spc_info[j]
                            .buffer;

                        len = pj_ansi_snprintf(
                          p, end - p,
                          "   %s  ICE role: %s, state: %s, comp_cnt: %u",
                          indent, pj_ice_sess_role_name(ii->role),
                        ...
                        for (jj = 0;
                             ii->sess_state == PJ_ICE_STRANS_STATE_RUNNING &&
                             jj < 2;
                             ++jj) {
                            const char* type1 = pj_ice_get_cand_type_name(
                              ii->comp[jj].lcand_type);
                            const char* type2 = pj_ice_get_cand_type_name(
                              ii->comp[jj].rcand_type);
                            ...
                            if (pj_sockaddr_has_addr(&ii->comp[jj].lcand_addr))
                                pj_sockaddr_print(&ii->comp[jj].lcand_addr,
                                                  addr1, sizeof(addr1), 3);
                            else
                                strcpy(addr1, "0.0.0.0:0");
                            if (pj_sockaddr_has_addr(&ii->comp[jj].rcand_addr))
                                pj_sockaddr_print(&ii->comp[jj].rcand_addr,
                                                  addr2, sizeof(addr2), 3);
                            else
                                strcpy(addr2, "0.0.0.0:0");
                            len = pj_ansi_snprintf(
                              p, end - p,
                              "   %s     [%d]: L:%s (%c) --> R:%s (%c)\n",
                              indent, jj, addr1, type1[0], addr2, type2[0]);
                            if (len > 0 && len < end - p) {
                                p += len;
                                *p = '\0';
                            }
...
#        define VALIDATE_PRINT_BUF() \
        ...
        if (call_med->type == PJMEDIA_TYPE_AUDIO) {
            ...
            if (pjmedia_stream_get_stat_xr(call_med->strm.a.stream, &xr_stat) !=
                PJ_SUCCESS) {
                continue;
            }
            ...
            /* Statistics Summary */
            len =
              pj_ansi_snprintf(p, end - p, "%s   Statistics Summary", indent);
            VALIDATE_PRINT_BUF();
            ...
            if (xr_stat.rx.stat_sum.j) {
                ...
                SAMPLES_TO_USEC(jmean, xr_stat.rx.stat_sum.jitter.mean,
                                clock_rate);
                SAMPLES_TO_USEC(
                  jdev, pj_math_stat_get_stddev(&xr_stat.rx.stat_sum.jitter),
                  clock_rate);
                sprintf(jitter, "%7.3f %7.3f %7.3f %7.3f", jmin / 1000.0,
                        jmean / 1000.0, jmax / 1000.0, jdev / 1000.0);
            } else
                sprintf(jitter, "(report not available)");

            if (xr_stat.rx.stat_sum.t) {
                sprintf(toh, "%11d %11d %11d %11d", xr_stat.rx.stat_sum.toh.min,
            ...
            if (xr_stat.rx.stat_sum.update.sec == 0)
                ...
                sprintf(last_update, "%02ldh:%02ldm:%02ld.%03lds ago",
                        now.sec / 3600, (now.sec % 3600) / 60, now.sec % 60,
                        now.msec);
            }
            ...
            len = pj_ansi_snprintf(
              ...
              "%s        jitter     : %s\n"
              "%s        toh        : %s",
              indent, last_update, indent, xr_stat.rx.stat_sum.begin_seq,
              xr_stat.rx.stat_sum.end_seq, indent, loss, dup, indent, indent,
              jitter, indent, toh);
            VALIDATE_PRINT_BUF();
            ...
            if (xr_stat.tx.stat_sum.j) {
                ...
                SAMPLES_TO_USEC(jmean, xr_stat.tx.stat_sum.jitter.mean,
                                clock_rate);
                SAMPLES_TO_USEC(
                  jdev, pj_math_stat_get_stddev(&xr_stat.tx.stat_sum.jitter),
                  clock_rate);
                sprintf(jitter, "%7.3f %7.3f %7.3f %7.3f", jmin / 1000.0,
                        jmean / 1000.0, jmax / 1000.0, jdev / 1000.0);
            } else
            ...
            if (xr_stat.tx.stat_sum.t) {
                sprintf(toh, "%11d %11d %11d %11d", xr_stat.tx.stat_sum.toh.min,
            ...
            len = pj_ansi_snprintf(
              ...
              "%s        jitter     : %s\n"
              "%s        toh        : %s",
              indent, last_update, indent, xr_stat.tx.stat_sum.begin_seq,
              xr_stat.tx.stat_sum.end_seq, indent, loss, dup, indent, indent,
              jitter, indent, toh);
            VALIDATE_PRINT_BUF();
            ...
            len = pj_ansi_snprintf(
              p, end - p,
              "%s     RX last update: %s\n"
              "%s        packets    : loss rate=%d (%.2f%%), discard rate=%d "
              "(%.2f%%)\n"
              ...
              indent, last_update,
              /* packets */
              indent, xr_stat.rx.voip_mtc.loss_rate,
              xr_stat.rx.voip_mtc.loss_rate * 100.0 / 256,
              xr_stat.rx.voip_mtc.discard_rate,
              xr_stat.rx.voip_mtc.discard_rate * 100.0 / 256,
              /* burst */
              indent, xr_stat.rx.voip_mtc.burst_den,
              xr_stat.rx.voip_mtc.burst_den * 100.0 / 256,
              xr_stat.rx.voip_mtc.burst_dur, "ms",
              /* gap */
              indent, xr_stat.rx.voip_mtc.gap_den,
              xr_stat.rx.voip_mtc.gap_den * 100.0 / 256,
              xr_stat.rx.voip_mtc.gap_dur, "ms",
              /* delay */
              indent, xr_stat.rx.voip_mtc.rnd_trip_delay, "ms",
              xr_stat.rx.voip_mtc.end_sys_delay, "ms",
              /* level */
              indent, signal_lvl, "dB", noise_lvl, "dB", rerl, "",
              /* quality */
              indent, r_factor, ext_r_factor, indent, mos_lq, mos_cq,
              /* config */
              indent, plc, jba, jbr, xr_stat.rx.voip_mtc.gmin,
              /* JB delay */
              indent, xr_stat.rx.voip_mtc.jb_nom, "ms",
              xr_stat.rx.voip_mtc.jb_max, "ms", xr_stat.rx.voip_mtc.jb_abs_max,
              "ms");
            VALIDATE_PRINT_BUF();
            ...
            len = pj_ansi_snprintf(
              ...
              indent, last_update,
              /* pakcets */
              indent, xr_stat.tx.voip_mtc.loss_rate,
              xr_stat.tx.voip_mtc.loss_rate * 100.0 / 256,
              xr_stat.tx.voip_mtc.discard_rate,
              xr_stat.tx.voip_mtc.discard_rate * 100.0 / 256,
              /* burst */
              indent, xr_stat.tx.voip_mtc.burst_den,
              xr_stat.tx.voip_mtc.burst_den * 100.0 / 256,
              xr_stat.tx.voip_mtc.burst_dur, "ms",
              /* gap */
              indent, xr_stat.tx.voip_mtc.gap_den,
              xr_stat.tx.voip_mtc.gap_den * 100.0 / 256,
              xr_stat.tx.voip_mtc.gap_dur, "ms",
              /* delay */
              indent, xr_stat.tx.voip_mtc.rnd_trip_delay, "ms",
              ...
              indent, r_factor, ext_r_factor, indent, mos_lq, mos_cq,
              /* config */
              indent, plc, jba, jbr, xr_stat.tx.voip_mtc.gmin,
              /* JB delay */
              indent, xr_stat.tx.voip_mtc.jb_nom, "ms",
              xr_stat.tx.voip_mtc.jb_max, "ms", xr_stat.tx.voip_mtc.jb_abs_max,
              "ms");
            VALIDATE_PRINT_BUF();
            ...
            len = pj_ansi_snprintf(
              p, end - p,
              "%s     RTT msec      : %7.3f %7.3f %7.3f %7.3f %7.3f", indent,
              xr_stat.rtt.min / 1000.0, xr_stat.rtt.mean / 1000.0,
              xr_stat.rtt.max / 1000.0, xr_stat.rtt.last / 1000.0,
...
/* Print call info */
void print_call(const char* title, int call_id, char* buf, pj_size_t size)
{
    int len;
    ...
    len = pj_ansi_snprintf(buf, size, "%s[%s] %s", title,
                ...
                unsigned maxlen, const char* indent)
{
    ...
    *p = '\0';
    ...
    len = pj_ansi_snprintf(
      ...
      indent, (int)(duration.sec / 3600), (int)((duration.sec % 3600) / 60),
      (int)(duration.sec % 60), (int)PJ_TIME_VAL_MSEC(res_delay),
