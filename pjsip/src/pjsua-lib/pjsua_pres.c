...
static pjsua_buddy_id find_buddy(const pjsip_uri* uri)
{
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(pjsua_var.buddy); ++i) {
        ...
        if (pj_stricmp(&sip_uri->user, &b->name) == 0 &&
            ...
            (sip_uri->port == (int)b->port ||
             (sip_uri->port == 0 && b->port == 5060)))
        {
            /* Match */
            return i;
        }
...
static pj_status_t lock_buddy(const char* title, pjsua_buddy_id buddy_id,
                              struct buddy_lock* lck, unsigned _unused_)
{
    enum
    {
        MAX_RETRY = 50
 ...
 * Enum buddy IDs.
 */
PJ_DEF(pj_status_t) pjsua_enum_buddies(pjsua_buddy_id ids[], unsigned* count)
{
    unsigned i, c;
 ...
 */
PJ_DEF(pj_status_t)
pjsua_buddy_add(const pjsua_buddy_config* cfg, pjsua_buddy_id* p_buddy_id)
{
    pjsip_name_addr* url;
    pjsua_buddy* buddy;
    ...
    PJ_LOG(4,
           (THIS_FILE, "Adding buddy: %.*s", (int)cfg->uri.slen, cfg->uri.ptr));
    pj_log_push_indent();
    ...
    buddy->term_reason.ptr =
      (char*)pj_pool_alloc(buddy->pool, PJSUA_BUDDY_SUB_TERM_REASON_LEN);
...
PJ_DEF(pj_status_t) pjsua_buddy_del(pjsua_buddy_id buddy_id)
{
    ...
    PJ_ASSERT_RETURN(
      buddy_id >= 0 && buddy_id < (int)PJ_ARRAY_SIZE(pjsua_var.buddy),
      PJ_EINVAL);
...
PJ_DEF(void) pjsua_pres_dump(pj_bool_t verbose)
{
    ...
    if (verbose == PJ_FALSE) {
        ...
        PJ_LOG(3, (THIS_FILE, "Number of server/UAS subscriptions: %d", count));
        ...
        PJ_LOG(3, (THIS_FILE, "Number of client/UAC subscriptions: %d", count));
        PJSUA_UNLOCK();
        return;
    }
    ...
    for (acc_id = 0; acc_id < (int)PJ_ARRAY_SIZE(pjsua_var.acc); ++acc_id) {
        ...
        PJ_LOG(3, (THIS_FILE, "  %.*s", (int)pjsua_var.acc[acc_id].cfg.id.slen,
        ...
        if (pj_list_empty(&pjsua_var.acc[acc_id].pres_srv_list)) {
            ...
            while (uapres != &pjsua_var.acc[acc_id].pres_srv_list) {
                PJ_LOG(
                  3, (THIS_FILE, "    %10s %s",
                      pjsip_evsub_get_state_name(uapres->sub), uapres->remote));
...
static void pres_evsub_on_srv_state(pjsip_evsub* sub, pjsip_event* event)
{
    ...
    if (uapres) {
        ...
        PJ_LOG(4, (THIS_FILE, "Server subscription to %s is %s", uapres->remote,
        ...
        if (pjsua_var.ua_cfg.cb.on_srv_subscribe_state) {
            ...
            (*pjsua_var.ua_cfg.cb.on_srv_subscribe_state)(
              uapres->acc_id, uapres, &from, state, event);
        }
...
static pj_bool_t pres_on_rx_request(pjsip_rx_data* rdata)
{
    ...
    PJ_LOG(
      4, (THIS_FILE, "Creating server subscription, using account %d", acc_id));
    pj_log_push_indent();
    ...
    status = pjsip_dlg_create_uas_and_inc_lock(pjsip_ua_instance(), rdata,
                                               &contact, &dlg);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Unable to create UAS dialog for subscription",
                     status);
        PJSUA_UNLOCK();
        pjsip_endpt_respond_stateless(pjsua_var.endpt, rdata, 400, NULL, NULL,
                                      NULL);
        pj_log_pop_indent();
        return PJ_TRUE;
    }
    ...
    if (status != PJ_SUCCESS) {
        ...
        pjsua_perror(THIS_FILE, "Unable to create server subscription", status);
    ...
    if (pjsua_var.ua_cfg.cb.on_incoming_subscribe) {
        ...
        (*pjsua_var.ua_cfg.cb.on_incoming_subscribe)(
          acc_id, uapres, buddy_id, &dlg->remote.info_str, rdata, &st_code,
          &reason, &msg_data);
    }
    ...
    if (st_code >= 300) {
        ...
        /* Create response */
        status =
          pjsip_dlg_create_response(dlg, rdata, st_code, &reason, &tdata);
        if (status != PJ_SUCCESS) {
    ...
    if (st_code == 200) {
        pjsua_pres_notify(acc_id, uapres, PJSIP_EVSUB_STATE_ACTIVE, NULL, NULL,
                          PJ_TRUE, &msg_data);
    }
...
pjsua_pres_notify(pjsua_acc_id acc_id, pjsua_srv_pres* srv_pres,
                  pjsip_evsub_state ev_state, const pj_str_t* state_str,
                  ...
                  const pjsua_msg_data* msg_data)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Acc %d: sending NOTIFY for srv_pres=0x%p..", acc_id,
               (int)(pj_ssize_t)srv_pres));
    pj_log_push_indent();
    ...
    // Both pjsua_var.local_uri and pjsua_var.contact_uri are enclosed in "<"
    // and ">" causing XML parsing to fail. pres_status.info[0].contact =
    // pjsua_var.local_uri;
    /* add RPID information */
    pj_memcpy(&pres_status.info[0].rpid, &acc->rpid, sizeof(pjrpid_element));
    ...
    status =
      pjsip_pres_notify(srv_pres->sub, ev_state, state_str, reason, &tdata);
    if (status == PJ_SUCCESS) {
    ...
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Unable to create/send NOTIFY", status);
    ...
    if (buddy_id != PJSUA_INVALID_ID) {
        ...
        if (b->monitor && b->sub == NULL) {
            PJ_LOG(4, (THIS_FILE,
                       "Received SUBSCRIBE from buddy %d, "
                       "activating outgoing subscription",
                       buddy_id));
            subscribe_buddy_presence(buddy_id);
        }
    }
...
static void publish_cb(struct pjsip_publishc_cbparam* param)
{
    ...
    if (param->code / 100 != 2 || param->status != PJ_SUCCESS) {
        ...
        if (param->status != PJ_SUCCESS) {
            ...
            PJ_LOG(1,
                   (THIS_FILE, "Client publication (PUBLISH) failed (%d/%.*s)",
...
static pj_status_t send_publish(int acc_id, pj_bool_t active)
{
    ...
    PJ_LOG(5, (THIS_FILE, "Acc %d: sending %sPUBLISH..", acc_id,
               (active ? "" : "un-")));
    pj_log_push_indent();
    ...
    if (active) {
        ...
        status = pjsip_pres_create_pidf(tdata->pool, &pres_status, &entity,
                                        &tdata->msg->body);
        if (status != PJ_SUCCESS) {
               ...
               !pjsua_sip_acc_is_using_upnp(acc_id))
    {
        ...
        if (pjsua_acc_get_uac_addr(acc_id, acc->pool, &acc_cfg->id, &via_addr,
...
pj_status_t pjsua_pres_init_publish_acc(int acc_id)
{
    ...
    if (acc_cfg->publish_enabled) {
        ...
        status = pjsip_publishc_create(pjsua_var.endpt, &acc_cfg->publish_opt,
                                       acc, &publish_cb, &acc->publish_sess);
        if (status != PJ_SUCCESS) {
            acc->publish_sess = NULL;
            return status;
        }
        ...
        status = pjsip_publishc_init(acc->publish_sess, &STR_PRESENCE,
                                     &acc_cfg->id, &acc_cfg->id, &acc_cfg->id,
...
void pjsua_pres_delete_acc(int acc_id, unsigned flags)
{
    ...
    while (uapres != &acc->pres_srv_list) {
        ...
        if ((flags & PJSUA_DESTROY_NO_TX_MSG) == 0) {
            if (pjsip_pres_notify(uapres->sub, PJSIP_EVSUB_STATE_TERMINATED,
                                  NULL, &reason, &tdata) == PJ_SUCCESS)
            {
                              ...
                              unsigned msec_interval)
{
    ...
    if (resched) {
        ...
        PJ_LOG(4,
               (THIS_FILE, "Resubscribing buddy id %u in %u ms (reason: %.*s)",
                buddy->index, msec_interval, (int)buddy->term_reason.slen,
                buddy->term_reason.ptr));
...
static void pjsua_evsub_on_state(pjsip_evsub* sub, pjsip_event* event)
{
    ...
    if (buddy) {
        PJ_LOG(4, (THIS_FILE, "Presence subscription to %.*s is %s",
        ...
        if (pjsip_evsub_get_state(sub) == PJSIP_EVSUB_STATE_TERMINATED) {
            ...
            if (buddy->term_reason.ptr == NULL) {
                buddy->term_reason.ptr = (char*)pj_pool_alloc(
                  buddy->pool, PJSUA_BUDDY_SUB_TERM_REASON_LEN);
            }
            ...
            if (event && event->type == PJSIP_EVENT_TSX_STATE) {
                ...
                if (pjsip_method_cmp(&tsx->method,
                        ...
                        pj_stricmp2(&buddy->term_reason, "timeout") == 0)
                    {
                         ...
                         */
                        resub_delay = 500;
                    } else if (pj_stricmp2(&buddy->term_reason, "probation") ==
                                 0 ||
                               pj_stricmp2(&buddy->term_reason, "giveup") == 0)
                    {
                        ...
                        sub_hdr = (const pjsip_sub_state_hdr*)
                          pjsip_msg_find_hdr_by_name(msg, &sub_state, NULL);
            ...
            if (resub_delay == -1) {
                ...
                resub_delay =
                  PJSUA_PRES_TIMER * 1000 - 2500 + (pj_rand() % 5000);
            }
...
static void pjsua_evsub_on_tsx_state(pjsip_evsub* sub, pjsip_transaction* tsx,
                                     pjsip_event* event)
{
    pjsua_buddy* buddy;
    ...
    contact_hdr = (pjsip_contact_hdr*)pjsip_msg_find_hdr(
      event->body.rx_msg.rdata->msg_info.msg, PJSIP_H_CONTACT, NULL);
    ...
    buddy->contact.slen =
      pjsip_uri_print(PJSIP_URI_IN_CONTACT_HDR, contact_hdr->uri,
                      buddy->contact.ptr, PJSIP_MAX_URL_SIZE);
    if (buddy->contact.slen < 0)
        buddy->contact.slen = 0;
}

/* Callback called when we receive NOTIFY */
static void pjsua_evsub_on_rx_notify(pjsip_evsub* sub, pjsip_rx_data* rdata,
...
static void subscribe_buddy_presence(pjsua_buddy_id buddy_id)
{
    ...
    if (acc->contact.slen) {
        ...
        status =
          pjsua_acc_create_uac_contact(tmp_pool, &contact, acc_id, &buddy->uri);
        if (status != PJ_SUCCESS) {
            pjsua_perror(THIS_FILE, "Unable to generate Contact header",
                         status);
    ...
    if (status != PJ_SUCCESS) {
        buddy->sub = NULL;
        pjsua_perror(THIS_FILE, "Unable to create presence client", status);
    ...
    /* Set credentials */
    if (acc->cred_cnt) {
        pjsip_auth_clt_set_credentials(&buddy->dlg->auth_sess, acc->cred_cnt,
                                       acc->cred);
    }
    ...
    status =
      pjsip_pres_initiate(buddy->sub, PJSIP_EXPIRES_NOT_SPECIFIED, &tdata);
    if (status != PJ_SUCCESS) {
        if (buddy->dlg)
            pjsip_dlg_dec_lock(buddy->dlg);
        if (buddy->sub) {
        ...
        buddy->sub = NULL;
        pjsua_perror(THIS_FILE, "Unable to create initial SUBSCRIBE", status);
        if (tmp_pool)
            pj_pool_release(tmp_pool);
        pj_log_pop_indent();
        return;
    ...
    if (status != PJ_SUCCESS) {
        ...
        buddy->sub = NULL;
        pjsua_perror(THIS_FILE, "Unable to send initial SUBSCRIBE", status);
        if (tmp_pool)
            pj_pool_release(tmp_pool);
        pj_log_pop_indent();
        return;
...
static void mwi_evsub_on_state(pjsip_evsub* sub, pjsip_event* event)
{
    ...
    PJ_LOG(4,
           (THIS_FILE, "MWI subscription for %.*s is %s", (int)acc->cfg.id.slen,
            acc->cfg.id.ptr, pjsip_evsub_get_state_name(sub)));
...
/* Callback called when we receive NOTIFY */
static void mwi_evsub_on_rx_notify(pjsip_evsub* sub, pjsip_rx_data* rdata,
                                   int* p_st_code, pj_str_t** p_st_text,
...
pj_status_t pjsua_start_mwi(pjsua_acc_id acc_id, pj_bool_t force_renew)
{
    ...
    if (acc->contact.slen) {
        ...
        status = pjsua_acc_create_uac_contact(tmp_pool, &contact, acc->index,
                                              &acc->cfg.id);
        if (status != PJ_SUCCESS) {
            pjsua_perror(THIS_FILE, "Unable to generate Contact header",
                         status);
    ...
    /* Set credentials */
    if (acc->cred_cnt) {
        pjsip_auth_clt_set_credentials(&acc->mwi_dlg->auth_sess, acc->cred_cnt,
                                       acc->cred);
    }
    ...
    if (status != PJ_SUCCESS) {
        ...
        acc->mwi_sub = NULL;
        acc->mwi_dlg = NULL;
        pjsua_perror(THIS_FILE, "Unable to send initial MWI SUBSCRIBE", status);
        goto on_return;
    }
...
static pj_bool_t unsolicited_mwi_on_rx_request(pjsip_rx_data* rdata)
{
    ...
    pjsip_endpt_respond(pjsua_get_pjsip_endpt(), NULL, rdata, 200, NULL, NULL,
                        NULL, NULL);
...
static void pres_timer_cb(pj_timer_heap_t* th, pj_timer_entry* entry)
{
    unsigned i;
