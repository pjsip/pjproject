...
static void init_outbound_setting(pjsua_acc* acc)
{
    ...
    if (acc_cfg->rfc5626_instance_id.slen == 0) {
        ...
        pj_size_t pos;
        char instprm[] =
          ";+sip.instance=\"<urn:uuid:00000000-0000-0000-0000-0000CCDDEEFF>\"";
        ...
        pj_ansi_snprintf(acc->rfc5626_instprm.ptr, len + 1, "%s%.*s\"", prmname,
    ...
    if (acc_cfg->rfc5626_reg_id.slen == 0) {
        ...
        pj_ansi_snprintf(acc->rfc5626_regprm.ptr, len + 1, "%s%.*s\"", prmname,
...
static pj_status_t initialize_acc(unsigned acc_id)
{
    ...
    name_addr = (pjsip_name_addr*)pjsip_parse_uri(acc->pool, acc_cfg->id.ptr,
                                                  acc_cfg->id.slen,
    ...
    for (i = 0; i < acc_cfg->proxy_cnt; ++i) {
        ...
        r = (pjsip_route_hdr*)pjsip_parse_hdr(acc->pool, &hname, tmp.ptr,
                                              tmp.slen, NULL);
        if (r == NULL) {
    ...
    for (i = 0; i < pjsua_var.ua_cfg.cred_count &&
                acc->cred_cnt < PJ_ARRAY_SIZE(acc->cred);
         ++i)
    {
 ...
 */
PJ_DEF(pj_status_t)
pjsua_acc_add(const pjsua_acc_config* cfg, pj_bool_t is_default,
              pjsua_acc_id* p_acc_id)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Adding account: id=%.*s", (int)cfg->id.slen,
               cfg->id.ptr));
    pj_log_push_indent();
    ...
    if (acc->pool)
        ...
        acc->pool =
          pjsua_pool_create("acc%p", PJSUA_POOL_LEN_ACC, PJSUA_POOL_INC_ACC);
    ...
    PJ_LOG(4, (THIS_FILE, "Account %.*s added with id %d", (int)cfg->id.slen,
               cfg->id.ptr, id));
                    ...
                    pjsua_acc_id* p_acc_id)
{
    ...
    pj_ansi_snprintf(uri, PJSIP_MAX_URL_SIZE, "<sip:%s%s>",
...
{
    PJ_ASSERT_RETURN(acc_id >= 0 &&
                       acc_id < (int)PJ_ARRAY_SIZE(pjsua_var.acc) &&
                       pjsua_var.acc[acc_id].valid,
                     PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t)
pjsua_acc_modify(pjsua_acc_id acc_id, const pjsua_acc_config* cfg)
{
    pjsua_acc* acc;
    ...
    if (pj_strcmp(&acc->cfg.id, &cfg->id)) {
        ...
        id_name_addr = (pjsip_name_addr*)pjsip_parse_uri(
          acc->pool, cfg->id.ptr, cfg->id.slen, PJSIP_PARSE_URI_AS_NAMEADDR);
    ...
    if (pj_strcmp(&acc->cfg.reg_uri, &cfg->reg_uri) && cfg->reg_uri.slen) {
        ...
        reg_uri =
          pjsip_parse_uri(acc->pool, cfg->reg_uri.ptr, cfg->reg_uri.slen, 0);
        if (reg_uri == NULL) {
            status = PJSIP_EINVALIDURI;
    ...
    if (local_route_crc != acc->local_route_crc) {
        ...
        for (i = 0; i < cfg->proxy_cnt; ++i) {
            ...
            if (status != PJ_SUCCESS)
                goto on_return;
            r = (pjsip_route_hdr*)pjsip_parse_hdr(
              acc->pool, &hname, acc_proxy[i].ptr, acc_proxy[i].slen, NULL);
            if (r == NULL) {
                status = PJSIP_EINVALIDURI;
                pjsua_perror(THIS_FILE, "Invalid URI in account route set",
                             status);
    ...
    if (acc->cfg.priority != cfg->priority) {
        ...
        pj_array_insert(pjsua_var.acc_ids, sizeof(acc_id), pjsua_var.acc_cnt, i,
                        &acc_id);
    }
        ...
        pj_strcmp(&acc->cfg.rfc5626_reg_id, &cfg->rfc5626_reg_id))
    {
        ...
        if (pj_strcmp(&acc->cfg.rfc5626_instance_id, &cfg->rfc5626_instance_id))
        {
    ...
    {
        pjsua_transport_config_dup(acc->pool, &acc->cfg.rtp_cfg, &cfg->rtp_cfg);
    } else {
    ...
    case PJSUA_TURN_CONFIG_USE_CUSTOM:
        pjsua_turn_config_dup(acc->pool, &acc->cfg.turn_cfg, &cfg->turn_cfg);
        break;
    }
        ...
        if (acc->regc) {
            ...
            if (status != PJ_SUCCESS) {
                pjsua_perror(THIS_FILE,
                             "Ignored failure in unregistering the "
    ...
    if (update_reg) {
        ...
        if (acc->cfg.reg_uri.slen) {
            ...
            if (status != PJ_SUCCESS) {
                pjsua_perror(THIS_FILE,
                             "Failed to register with new account "
                             "setting in modifying account",
                             status);
                goto on_return;
            }
        }
    ...
    if (update_mwi) {
        ...
        if (status != PJ_SUCCESS) {
            pjsua_perror(THIS_FILE,
                         "Failed in starting MWI subscription for "
                         "new account setting in modifying account",
                         status);
        }
    }
    ...
    /* SRTP setting */
    pjsua_srtp_opt_dup(acc->pool, &acc->cfg.srtp_opt, &cfg->srtp_opt, PJ_TRUE);
...
pjsua_acc_set_online_status(pjsua_acc_id acc_id, pj_bool_t is_online)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Acc %d: setting online status to %d..", acc_id,
               is_online));
    pj_log_push_indent();
                             ...
                             const pjrpid_element* pr)
{
    ...
    PJSUA_LOCK();
    pjsua_var.acc[acc_id].online_status = is_online;
    pjrpid_element_dup(pjsua_var.acc[acc_id].pool, &pjsua_var.acc[acc_id].rpid,
                       pr);
    PJSUA_UNLOCK();
...
static void update_regc_contact(pjsua_acc* acc)
{
    ...
    acc->rfc5626_status = OUTBOUND_WANTED;
    len =
      acc->contact.slen + acc->cfg.contact_params.slen +
      acc->cfg.reg_contact_params.slen + acc->cfg.reg_contact_uri_params.slen +
      (need_outbound ? (acc->rfc5626_instprm.slen + acc->rfc5626_regprm.slen) :
                       0);
    if (len > acc->contact.slen) {
        ...
        if (acc->cfg.reg_contact_uri_params.slen) {
            ...
            contact_hdr = (pjsip_contact_hdr*)pjsip_parse_hdr(
              pool, &STR_CONTACT, reg_contact.ptr, reg_contact.slen, NULL);
            pj_assert(contact_hdr != NULL);
            uri = (pjsip_sip_uri*)contact_hdr->uri;
            pj_assert(uri != NULL);
            uri = (pjsip_sip_uri*)pjsip_uri_get_uri(uri);
            tmp_len = pjsip_uri_print(PJSIP_URI_IN_CONTACT_HDR, uri, tmp_uri,
                                      sizeof(tmp_uri));
            pj_assert(tmp_len > 0);
            ...
            reg_contact.slen = pj_ansi_snprintf(
              reg_contact.ptr, len, "<%.*s%.*s>%.*s", (int)tmp_len, tmp_uri,
              (int)uri_param.slen, uri_param.ptr,
              (int)acc->cfg.contact_params.slen, acc->cfg.contact_params.ptr);
        ...
        PJ_LOG(4, (THIS_FILE, "Contact for acc %d updated: %.*s", acc->index,
                   (int)acc->reg_contact.slen, acc->reg_contact.ptr));
...
static pj_bool_t is_private_ip(const pj_str_t* addr)
{
    const pj_str_t private_net[] = {
        { "10.", 3 },     { "127.", 4 },    { "172.16.", 7 }, { "172.17.", 7 },
        { "172.18.", 7 }, { "172.19.", 7 }, { "172.20.", 7 }, { "172.21.", 7 },
        { "172.22.", 7 }, { "172.23.", 7 }, { "172.24.", 7 }, { "172.25.", 7 },
        { "172.26.", 7 }, { "172.27.", 7 }, { "172.28.", 7 }, { "172.29.", 7 },
        { "172.30.", 7 }, { "172.31.", 7 }, { "192.168.", 8 }
    };
    unsigned i;
...
static pj_bool_t acc_check_nat_addr(pjsua_acc* acc, int contact_rewrite_method,
                                    struct pjsip_regc_cbparam* param)
{
    ...
    if (acc->rfc5626_status == OUTBOUND_ACTIVE) {
        PJ_LOG(4, (THIS_FILE,
                   "Acc %d has SIP outbound active, no need to "
    ...
    pool = pjsua_pool_create("tmp", 512, 512);
    contact_hdr = (pjsip_contact_hdr*)pjsip_parse_hdr(
      pool, &STR_CONTACT, acc->contact.ptr, acc->contact.slen, NULL);
    pj_assert(contact_hdr != NULL);
    uri = (pjsip_sip_uri*)contact_hdr->uri;
    pj_assert(uri != NULL);
    uri = (pjsip_sip_uri*)pjsip_uri_get_uri(uri);
    ...
    pj_addr_str_print(via_addr, rport, via_addr_buf, sizeof(via_addr_buf), 1);
    PJ_LOG(3,
           (THIS_FILE,
            "IP address change detected for account %d "
            "(%s --> %s). Updating registration "
            "(using method %d)",
            acc->index, host_addr_buf, via_addr_buf, contact_rewrite_method));
     ...
     */
    {
        ...
        len = pj_ansi_snprintf(
          tmp, PJSIP_MAX_URL_SIZE, "<%s:%.*s%s%s%.*s%s:%d%s%.*s%s>%.*s",
          ((secure && acc->is_sips) ? "sips" : "sip"), (int)acc->user_part.slen,
          acc->user_part.ptr, (acc->user_part.slen ? "@" : ""), beginquote,
          (int)via_addr->slen, via_addr->ptr, endquote, rport, transport_param,
          (int)acc->cfg.contact_uri_params.slen,
          acc->cfg.contact_uri_params.ptr, (acc->cfg.use_rfc5626 ? ob : ""),
          (int)acc->cfg.contact_params.slen, acc->cfg.contact_params.ptr);
...
static void update_service_route(pjsua_acc* acc, pjsip_rx_data* rdata)
{
    ...
    for (;;) {
        ...
        hsr = (pjsip_generic_string_hdr*)pjsip_msg_find_hdr_by_name(
          rdata->msg_info.msg, &HNAME, hsr);
        if (!hsr)
            break;
        ...
        hr = (pjsip_route_hdr*)pjsip_parse_hdr(rdata->tp_info.pool, &HROUTE,
                                               hsr->hvalue.ptr,
        ...
        do {
            ...
            {
                PJ_LOG(1,
                       (THIS_FILE, "Error: non SIP URI in Service-Route: %.*s",
...
static void keep_alive_timer_cb(pj_timer_heap_t* th, pj_timer_entry* te)
{
    ...
    PJ_LOG(5,
           (THIS_FILE, "Sending %d bytes keep-alive packet for acc %d to %s",
            acc->cfg.ka_data.slen, acc->index,
            pj_sockaddr_print(&acc->ka_target, addrtxt, sizeof(addrtxt), 3)));

    /* Send raw packet */
    status = pjsip_tpmgr_send_raw(
      pjsip_endpt_get_tpmgr(pjsua_var.endpt), acc->ka_transport->key.type,
      &tp_sel, NULL, acc->cfg.ka_data.ptr, acc->cfg.ka_data.slen,
      &acc->ka_target, acc->ka_target_len, NULL, NULL);
    ...
    ka_timer =
      acc->rfc5626_flowtmr ? acc->rfc5626_flowtmr : acc->cfg.ka_interval;
                              ...
                              struct pjsip_regc_cbparam* param)
{
    ...
    if (start) {
        ...
        hsr = (pjsip_generic_string_hdr*)pjsip_msg_find_hdr_by_name(
          param->rdata->msg_info.msg, &STR_FLOW_TIMER, hsr);
        ...
        delay.msec = 0;
        status =
          pjsip_endpt_schedule_timer(pjsua_var.endpt, &acc->ka_timer, &delay);
        if (status == PJ_SUCCESS) {
            char addr[PJ_INET6_ADDRSTRLEN + 10];
            pj_str_t input_str = pj_str(param->rdata->pkt_info.src_name);
            acc->ka_timer.id = PJ_TRUE;

            pj_addr_str_print(&input_str, param->rdata->pkt_info.src_port, addr,
                              sizeof(addr), 1);
            PJ_LOG(4, (THIS_FILE,
                       "Keep-alive timer started for acc %d, "
...
on_return:
    ...
    PJ_LOG(
      4, (THIS_FILE, "SIP outbound status for acc %d is %s", acc->index,
          (acc->rfc5626_status == OUTBOUND_ACTIVE ? "active" : "not active")));
}
    ...
    if (!acc->contact_rewritten &&
        (acc->cfg.contact_rewrite_method &
         PJSUA_CONTACT_REWRITE_ALWAYS_UPDATE) ==
          PJSUA_CONTACT_REWRITE_ALWAYS_UPDATE &&
        param->cbparam.code >= 400 && param->cbparam.rdata)
    {
...
static void regc_cb(struct pjsip_regc_cbparam* param)
{
    ...
    if (param->status != PJ_SUCCESS) {
        ...
        if (status == PJ_SUCCESS) {
            ...
            pjsua_perror(THIS_FILE, "Deleting registration failed", status);
        }
        ...
        if (param->expiration < 1) {
            ...
            PJ_LOG(3, (THIS_FILE,
                       ...
                       pjsua_var.acc[acc->index].cfg.id.ptr, param->code,
        ...
        PJ_LOG(4,
               (THIS_FILE, "SIP registration updated status=%d", param->code));
    }
    ...
    if (acc->ip_change_op == PJSUA_IP_CHANGE_OP_ACC_UPDATE_CONTACT) {
        if (pjsua_var.ua_cfg.cb.on_ip_change_progress) {
            ...
            (*pjsua_var.ua_cfg.cb.on_ip_change_progress)(
              acc->ip_change_op, param->status, &ip_chg_info);
        }
        ...
        if (PJSIP_IS_STATUS_IN_CLASS(param->code, 200)) {
            if (param->expiration < 1) {
                ...
                /* Send re-register. */
                PJ_LOG(3, (THIS_FILE,
                           "%.*s: send registration triggered by IP"
                           " change",
                           pjsua_var.acc[acc->index].cfg.id.slen,
                           pjsua_var.acc[acc->index].cfg.id.ptr));
                ...
                if (status != PJ_SUCCESS) {
                    if (pjsua_var.ua_cfg.cb.on_ip_change_progress) {
                        ...
                        (*pjsua_var.ua_cfg.cb.on_ip_change_progress)(
                          acc->ip_change_op, status, &ip_chg_info);
                    }
                    pjsua_acc_end_ip_change(acc);
                }
            } else {
                /* Avoid deadlock issue when sending BYE or Re-INVITE.  */
                pjsua_schedule_timer2(&handle_call_on_ip_change_cb, (void*)acc,
                                      0);
            }
        } else {
            pjsua_acc_end_ip_change(acc);
...
static pj_status_t pjsua_regc_init(int acc_id)
{
    ...
    if (acc->contact.slen == 0) {
        ...
        status = pjsua_acc_create_uac_contact(pool, &tmp_contact, acc_id,
                                              &acc->cfg.reg_uri);
        if (status != PJ_SUCCESS) {
            pjsua_perror(THIS_FILE,
    ...
    status =
      pjsip_regc_init(acc->regc, &acc->cfg.reg_uri, &acc->cfg.id, &acc->cfg.id,
                      1, &acc->reg_contact, acc->cfg.reg_timeout);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Client registration initialization error",
                     status);
        pjsip_regc_destroy(acc->regc);
        pj_pool_release(pool);
        acc->regc = NULL;
...
pj_bool_t pjsua_sip_acc_is_using_upnp(pjsua_acc_id acc_id)
{
    ...
    return acc->cfg.sip_upnp_use != PJSUA_UPNP_USE_DISABLED &&
           pjsua_var.ua_cfg.enable_upnp && pjsua_var.upnp_status == PJ_SUCCESS;
}
...
pjsua_acc_set_registration(pjsua_acc_id acc_id, pj_bool_t renew)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Acc %d: setting %sregistration..", acc_id,
               (renew ? "" : "un")));
    pj_log_push_indent();
    ...
    if (renew) {
        ...
        status = pjsip_regc_register(pjsua_var.acc[acc_id].regc,
                                     PJSUA_REG_AUTO_REG_REFRESH, &tdata);
    ...
    if (status == PJ_SUCCESS) {
                   ...
                   !pjsua_sip_acc_is_using_upnp(acc_id))
        {
             ...
             * STUN nor UPnP.
             */
            pjsua_acc_get_uac_addr(acc_id, tdata->pool, &acc->cfg.reg_uri,
                                   &tdata->via_addr, NULL, NULL,
                                   &tdata->via_tp);
        }
 ...
 */
PJ_DEF(pj_status_t) pjsua_enum_accs(pjsua_acc_id ids[], unsigned* count)
{
    unsigned i, c;
 ...
 * Enum accounts info.
 */
PJ_DEF(pj_status_t) pjsua_acc_enum_info(pjsua_acc_info info[], unsigned* count)
{
    unsigned i, c;
...
PJ_DEF(pjsua_acc_id) pjsua_acc_find_for_outgoing(const pj_str_t* url)
{
    ...
    for (i = 0; i < pjsua_var.acc_cnt; ++i) {
        ...
        if (pj_stricmp(&pjsua_var.acc[acc_id].srv_domain, &sip_uri->host) ==
              0 &&
                         ...
                         const pj_str_t* target, pjsip_tx_data** p_tdata)
{
    ...
    status =
      pjsip_endpt_create_request(pjsua_var.endpt, method, target, &acc->cfg.id,
                                 target, NULL, NULL, -1, NULL, &tdata);
    if (status != PJ_SUCCESS) {
               ...
               !pjsua_sip_acc_is_using_upnp(acc_id))
    {
         ...
         * STUN nor UPnP.
         */
        pjsua_acc_get_uac_addr(acc_id, tdata->pool, target, &tdata->via_addr,
                               NULL, NULL, &tdata->via_tp);
    }
 ...
 */
pj_status_t pjsua_acc_get_uac_addr(pjsua_acc_id acc_id, pj_pool_t* pool,
                                   const pj_str_t* dst_uri,
                                   pjsip_host_port* addr,
                                   pjsip_transport_type_e* p_tp_type,
                                   int* secure, const void** p_tp)
{
    pjsua_acc* acc;
    pjsip_sip_uri* sip_uri;
    pj_status_t status;
    ...
    if (!pj_list_empty(&acc->route_set)) {
        sip_uri =
          (pjsip_sip_uri*)pjsip_uri_get_uri(acc->route_set.next->name_addr.uri);
    } else {
        pj_str_t tmp;
        pjsip_uri* uri;
    ...
    if (pj_strchr(&sip_uri->host, ':') || pjsua_sip_acc_is_using_ipv6(acc_id))
        tp_type =
          (pjsip_transport_type_e)(((int)tp_type) | PJSIP_TRANSPORT_IPV6);
    ...
    if (acc->cfg.nat64_opt != PJSUA_NAT64_DISABLED) {
        ...
        if ((flag & PJSIP_TRANSPORT_DATAGRAM) && tfla2_prm.local_if &&
            ...
            for (i = 0; i < sizeof(pjsua_var.tpdata); i++) {
                if (tfla2_prm.ret_tp ==
                    (const void*)pjsua_var.tpdata[i].data.tp) {
                    if (pjsua_var.tpdata[i].has_bound_addr) {
                        pj_strdup(
                          pool, &addr->host,
                          &pjsua_var.tpdata[i].data.tp->local_name.host);
                        addr->port = (pj_uint16_t)pjsua_var.tpdata[i]
                                       .data.tp->local_name.port;
                    }
                    break;
                }
            }
    ...
    if (acc->cfg.contact_use_src_port) {
        ...
        status = pjsip_get_dest_info((pjsip_uri*)sip_uri, NULL, pool, &dinfo);

        if (status == PJ_SUCCESS &&
            (dinfo.flag & PJSIP_TRANSPORT_RELIABLE) == 0) {
            /* Not TCP or TLS. No need to do this */
            status = PJ_EINVALIDOP;
            log_written = PJ_TRUE;
        }

        if (status == PJ_SUCCESS && get_ip_addr_ver(&dinfo.addr.host) == 0 &&
            pjsua_var.ua_cfg.nameserver_count)
        {
             ...
             */
            PJ_LOG(4, (THIS_FILE,
                       "Warning: cannot use source TCP/TLS socket"
        ...
        if (status == PJ_SUCCESS) {
            ...
            status = pjsip_endpt_acquire_transport2(pjsua_var.endpt, dinfo.type,
                                                    &ai.ai_addr, addr_len,
                                                    &tp_sel, &tdata, &tp);
        }

        if (status == PJ_SUCCESS &&
            (tp->local_name.port == 0 || tp->local_name.host.slen == 0 ||
             *tp->local_name.host.ptr == '0'))
        {
             ...
             */
            PJ_LOG(4, (THIS_FILE,
                       "Unable to get transport local port "
...
pjsua_acc_create_uac_contact(pj_pool_t* pool, pj_str_t* contact,
                             pjsua_acc_id acc_id, const pj_str_t* suri)
{
    pjsua_acc* acc;
    pj_status_t status;
    pjsip_transport_type_e tp_type;
    ...
    status = pjsua_acc_get_uac_addr(acc_id, pool, suri, &addr, &tp_type,
                                    &secure, NULL);
    if (status != PJ_SUCCESS)
        return status;
    ...
    contact->slen = pj_ansi_snprintf(
      ...
      (acc->display.slen ? "\"" : ""), (int)acc->display.slen, acc->display.ptr,
      (acc->display.slen ? "\" " : ""),
      ((secure && acc->is_sips) ? "sips" : "sip"), (int)acc->user_part.slen,
      acc->user_part.ptr, (acc->user_part.slen ? "@" : ""), beginquote,
      (int)addr.host.slen, addr.host.ptr, endquote, addr.port, transport_param,
      (int)acc->cfg.contact_uri_params.slen, acc->cfg.contact_uri_params.ptr,
      (acc->cfg.use_rfc5626 ? ob : ""), (int)acc->cfg.contact_params.slen,
                             ...
                             pjsua_acc_id acc_id, pjsip_rx_data* rdata)
{
    ...
    if (rdata->msg_info.record_route) {
        sip_uri = (pjsip_sip_uri*)pjsip_uri_get_uri(
          rdata->msg_info.record_route->name_addr.uri);
    } else {
        pjsip_hdr* pos = NULL;
         ...
         */
        do {
            h_contact = (pjsip_contact_hdr*)pjsip_msg_find_hdr(
              rdata->msg_info.msg, PJSIP_H_CONTACT, pos);
            if (h_contact) {
                if (h_contact->uri)
    ...
    contact->slen = pj_ansi_snprintf(
      ...
      ((secure && acc->is_sips) ? "sips" : "sip"), (int)acc->user_part.slen,
      acc->user_part.ptr, (acc->user_part.slen ? "@" : ""), beginquote,
      (int)local_addr.slen, local_addr.ptr, endquote, local_port,
      transport_param, (int)acc->cfg.contact_uri_params.slen,
      acc->cfg.contact_uri_params.ptr, (int)acc->cfg.contact_params.slen,
      acc->cfg.contact_params.ptr);
    if (contact->slen < 1 || contact->slen >= (int)PJSIP_MAX_URL_SIZE)
        return PJ_ETOOSMALL;
...
static void schedule_reregistration(pjsua_acc* acc)
{
    ...
    if (acc->cfg.drop_calls_on_reg_fail && acc->auto_rereg.attempt_cnt >= 1) {
        ...
        if (cnt) {
            PJ_LOG(3, (THIS_FILE,
                       "Disconnecting %d call(s) of account #%d "
        ...
        if (acc->regc) {
            ...
            if (pjsua_var.acc[i].ip_change_op ==
                ...
                if (pjsua_var.ua_cfg.cb.on_ip_change_progress) {
                    ...
                    pjsua_var.ua_cfg.cb.on_ip_change_progress(
                      acc->ip_change_op, PJ_SUCCESS, &ch_info);
                }
...
pj_status_t pjsua_acc_update_contact_on_ip_change(pjsua_acc* acc)
{
    ...
    PJ_LOG(3, (THIS_FILE,
               "%.*s: send %sregistration triggered "
               "by IP change",
               acc->cfg.id.slen, acc->cfg.id.ptr, (need_unreg ? "un-" : "")));
        ...
        (acc->ip_change_op == PJSUA_IP_CHANGE_OP_ACC_UPDATE_CONTACT))
    {
        ...
        pjsua_var.ua_cfg.cb.on_ip_change_progress(acc->ip_change_op, status,
                                                  &info);
        ...
        acc->cfg.ip_change_cfg.reinvite_flags)
    {
        for (i = 0; i < (int)pjsua_var.ua_cfg.max_calls; ++i) {
                       ...
                       (call_info.state == PJSIP_INV_STATE_CONFIRMED))
            {
                ...
                call_info.setting.flag |= acc->cfg.ip_change_cfg.reinvite_flags;
                ...
                PJ_LOG(3, (THIS_FILE,
                           ...
                           call_info.remote_info.ptr, call_info.setting.flag,
                           acc->cfg.ip_change_cfg.reinvite_flags));
...
void pjsua_acc_end_ip_change(pjsua_acc* acc)
{
    ...
    if (all_done && pjsua_var.ua_cfg.cb.on_ip_change_progress) {
        ...
        pjsua_var.ua_cfg.cb.on_ip_change_progress(PJSUA_IP_CHANGE_OP_COMPLETED,
                                                  PJ_SUCCESS, NULL);
    }
    PJSUA_UNLOCK();
}
