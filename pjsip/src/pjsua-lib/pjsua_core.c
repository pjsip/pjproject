...
PJ_DEF(void) pjsua_logging_config_default(pjsua_logging_config* cfg)
{
    ...
    cfg->level = 5;
    cfg->console_level = 4;
    cfg->decor = PJ_LOG_HAS_SENDER | PJ_LOG_HAS_TIME | PJ_LOG_HAS_MICRO_SEC |
                 PJ_LOG_HAS_NEWLINE | PJ_LOG_HAS_SPACE | PJ_LOG_HAS_THREAD_SWC |
                 PJ_LOG_HAS_INDENT;
                   ...
                   const pjsua_srtp_opt* src, pj_bool_t check_str)
{
    ...
    if (check_str)
        pj_memcpy(&backup_dst, dst, sizeof(*dst));
    pj_memcpy(dst, src, sizeof(*src));
...
static pj_bool_t logging_on_rx_msg(pjsip_rx_data* rdata)
{
    ...
    PJ_LOG(4, (THIS_FILE,
               "RX %d bytes %s from %s %s:\n"
               "%.*s\n"
               "--end msg--",
               rdata->msg_info.len, pjsip_rx_data_get_info(rdata),
               rdata->tp_info.transport->type_name,
               pj_addr_str_print(&input_str, rdata->pkt_info.src_port, addr,
                                 sizeof(addr), 1),
...
static pj_status_t logging_on_tx_msg(pjsip_tx_data* tdata)
{
    ...
    PJ_LOG(4,
           (THIS_FILE,
            ...
            "%.*s\n"
            "--end msg--",
            (tdata->buf.cur - tdata->buf.start), pjsip_tx_data_get_info(tdata),
            tdata->tp_info.transport->type_name,
            pj_addr_str_print(&input_str, tdata->tp_info.dst_port, addr,
                              sizeof(addr), 1),
            (int)(tdata->buf.cur - tdata->buf.start), tdata->buf.start));
...
static pj_bool_t options_on_rx_request(pjsip_rx_data* rdata)
{
    ...
    status =
      pjsip_endpt_create_response(pjsua_var.endpt, rdata, 200, NULL, &tdata);
    ...
    cap_hdr =
      pjsip_endpt_get_capability(pjsua_var.endpt, PJSIP_H_SUPPORTED, NULL);
    if (cap_hdr) {
        pjsip_msg_add_hdr(tdata->msg,
                          (pjsip_hdr*)pjsip_hdr_clone(tdata->pool, cap_hdr));
    }
    ...
    if (pjsua_var.ua_cfg.user_agent.slen) {
        ...
        h = (pjsip_hdr*)pjsip_generic_string_hdr_create(
          tdata->pool, &USER_AGENT, &pjsua_var.ua_cfg.user_agent);
    ...
    status =
      pjsip_endpt_send_response(pjsua_var.endpt, &res_addr, tdata, NULL, NULL);
    if (status != PJ_SUCCESS)
...
PJ_DEF(pj_status_t) pjsua_reconfigure_logging(const pjsua_logging_config* cfg)
{
    ...
    if (pjsua_var.log_cfg.log_filename.slen) {
        ...
        status =
          pj_file_open(pjsua_var.pool, pjsua_var.log_cfg.log_filename.ptr,
                       flags, &pjsua_var.log_file);
...
PJ_DEF(void) pjsua_stop_worker_threads(void)
{
    ...
    for (i = 0; i < (int)pjsua_var.ua_cfg.thread_cnt; ++i) {
        if (pjsua_var.thread[i]) {
            ...
            if (status != PJ_SUCCESS) {
                PJ_PERROR(4,
                          (THIS_FILE, status, "Error joining worker thread"));
...
PJ_DEF(pj_status_t) pjsua_create(void)
{
    ...
    /* Create mutex */
    status =
      pj_mutex_create_recursive(pjsua_var.pool, "pjsua", &pjsua_var.mutex);
    if (status != PJ_SUCCESS) {
        pj_log_pop_indent();
     ...
     */
    status = pjsip_endpt_create(&pjsua_var.cp.factory, pj_gethostname()->ptr,
                                &pjsua_var.endpt);
    if (status != PJ_SUCCESS) {
        pj_log_pop_indent();
           ...
           const pjsua_media_config* media_cfg)
{
  ...
  PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT != 0
    ...
    if (ua_cfg->nameserver_count) {
        ...
        /* Create DNS resolver */
        status =
          pjsip_endpt_create_resolver(pjsua_var.endpt, &pjsua_var.resolver);
        if (status != PJ_SUCCESS) {
        ...
        status =
          pj_dns_resolver_set_ns(pjsua_var.resolver, ua_cfg->nameserver_count,
...
#else
        PJ_LOG(2,
               (THIS_FILE, "DNS resolver is disabled (PJSIP_HAS_RESOLVER==0)"));
#endif
    }
    ...
    for (i = 0; i < ua_cfg->outbound_proxy_cnt; ++i) {
        ...
        r = (pjsip_route_hdr*)pjsip_parse_hdr(pjsua_var.pool, &hname, tmp.ptr,
    ...
    if (pjsua_var.ua_cfg.thread_cnt) {
        ...
        for (ii = 0; ii < pjsua_var.ua_cfg.thread_cnt; ++ii) {
            ...
            if (ii == 0) {
                status =
                  pj_thread_create(pjsua_var.pool, tname, &worker_thread_timer,
                                   NULL, 0, 0, &pjsua_var.thread[ii]);
            } else {
                status = pj_thread_create(pjsua_var.pool, tname,
                                          &worker_thread_ioqueue, NULL, 0, 0,
                                          &pjsua_var.thread[ii]);
            }
#else
...
static void stun_resolve_complete(pjsua_stun_resolve* sess)
{
    ...
    if (result.status == PJ_SUCCESS) {
        ...
        PJ_LOG(
          4, (THIS_FILE, "STUN resolution success, using %.*s, address is %s",
              (int)sess->srv[sess->idx].slen, sess->srv[sess->idx].ptr, addr));
    } else {
        char errmsg[PJ_ERR_MSG_SIZE];
...
static pj_bool_t test_stun_on_status(pj_stun_sock* stun_sock,
                                     pj_stun_sock_op op, pj_status_t status)
{
    ...
    if (status != PJ_SUCCESS) {
        ...
        PJ_LOG(4, (THIS_FILE, "STUN resolution for %.*s failed: %s",
                   (int)sess->srv[sess->idx].slen, sess->srv[sess->idx].ptr,
                   errmsg));
...
static void resolve_stun_entry(pjsua_stun_resolve* sess)
{
           ...
           (++sess->idx, sess->af = pj_AF_INET()))
    {
            ...
            sess->af == pj_AF_INET())
        {
            ...
            PJ_LOG(4, (THIS_FILE,
                       "Skipping IPv4 resolution of STUN server "
                       "%s (%d of %d)",
        ...
        PJ_LOG(4, (THIS_FILE, "Trying STUN server %s %s (%d of %d)..", target,
                   (sess->af == pj_AF_INET() ? "IPv4" : "IPv6"), sess->idx + 1,
                   sess->count));
        ...
        status =
          pj_stun_sock_create(&pjsua_var.stun_cfg, "stunresolve", sess->af,
                              &stun_sock_cb, NULL, sess, &sess->stun_sock);
        if (status != PJ_SUCCESS) {
            char errmsg[PJ_ERR_MSG_SIZE];
            pj_strerror(status, errmsg, sizeof(errmsg));
            PJ_LOG(4, (THIS_FILE, "Error creating STUN socket for %s: %s",
                       target, errmsg));
        ...
        if (status != PJ_SUCCESS) {
            char errmsg[PJ_ERR_MSG_SIZE];
            pj_strerror(status, errmsg, sizeof(errmsg));
            PJ_LOG(4, (THIS_FILE, "Error starting STUN socket for %s: %s",
                       target, errmsg));
...
static void internal_stun_resolve_cb(const pj_stun_resolve_result* result)
{
    ...
    if ((result->status == PJ_SUCCESS) && (pjsua_var.ua_cfg.stun_srv_cnt > 0)) {
        ...
        if (pjsua_var.nat_type == PJ_STUN_NAT_TYPE_UNKNOWN &&
            !pjsua_var.nat_in_progress && pjsua_var.ua_cfg.nat_type_in_sdp)
        {
                                ...
                                unsigned options)
{
    ...
    if (pjsua_var.stun_status != PJ_EPENDING &&
        pjsua_var.stun_status != PJ_SUCCESS && retry_if_cur_error)
    {
        pjsua_var.stun_status = PJ_EUNKNOWN;
    }
    ...
    if (pjsua_var.stun_status == PJ_EUNKNOWN) {
        ...
        if (pjsua_var.ua_cfg.stun_srv_cnt) {
            pjsua_var.stun_status = PJ_EPENDING;
            status = pjsua_resolve_stun_servers(
              pjsua_var.ua_cfg.stun_srv_cnt, pjsua_var.ua_cfg.stun_srv, wait,
              NULL, &internal_stun_resolve_cb);
...
{
    const char* state_name[] = { "NULL",     "CREATED", "INIT",
...
static const char* addr_string(const pj_sockaddr_t* addr)
{
    ...
    pj_inet_ntop(((const pj_sockaddr*)addr)->addr.sa_family,
                 pj_sockaddr_get_addr(addr), str, sizeof(str));
    return str;
}
                                       ...
                                       pj_sockaddr* p_pub_addr)
{
    ...
    if (cfg->bound_addr.slen) {
        status =
          pj_sockaddr_init(af, &bind_addr, &cfg->bound_addr, (pj_uint16_t)port);
        if (status != PJ_SUCCESS) {
            pjsua_perror(THIS_FILE, "Unable to resolve transport bound address",
                         status);
            return status;
        }
    } else {
    ...
    status = pj_sock_apply_qos2(sock, cfg->qos_type, &cfg->qos_params, 2,
                                THIS_FILE, "SIP UDP socket");
    ...
    if (pjsua_var.stun_srv.addr.sa_family != 0) {
        pj_sockaddr_print(&pjsua_var.stun_srv, stun_ip_addr,
                          sizeof(stun_ip_addr), 0);
               ...
               (af == pj_AF_INET() || pjsua_var.ua_cfg.stun_try_ipv6))
    {
        ...
        status = pjstun_get_mapped_addr2(&pjsua_var.cp.factory, &stun_opt, 1,
                                         &sock, &p_pub_addr->ipv4);
        if (status != PJ_SUCCESS) {
    ...
    *p_sock = sock;
    ...
    PJ_LOG(4, (THIS_FILE, "SIP UDP socket reachable at %s:%d",
               addr_string(p_pub_addr), (int)pj_sockaddr_get_port(p_pub_addr)));
                       ...
                       pjsua_transport_id* p_id)
{
    ...
    if (type == PJSIP_TRANSPORT_UDP || type == PJSIP_TRANSPORT_UDP6) {
        ...
        pj_sockaddr_init(pjsip_transport_type_get_af(type), &pub_addr, NULL,
         ...
         */
        status = create_sip_udp_sock(pjsip_transport_type_get_af(type), cfg,
                                     &sock, &pub_addr);
        if (status != PJ_SUCCESS)
            goto on_return;
        ...
        if (cfg->bound_addr.slen) {
            status = pj_sockaddr_set_str_addr(tcp_cfg.af, &tcp_cfg.bind_addr,
                                              &cfg->bound_addr);
            if (status != PJ_SUCCESS) {
                pjsua_perror(THIS_FILE,
        ...
        if (cfg->bound_addr.slen) {
            status =
              pj_sockaddr_set_str_addr(af, &local_addr, &cfg->bound_addr);
            if (status != PJ_SUCCESS) {
                pjsua_perror(THIS_FILE,
                             "Unable to resolve transport bound address",
                             status);
 ...
 */
PJ_DEF(pj_status_t)
pjsua_enum_transports(pjsua_transport_id id[], unsigned* p_count)
{
    unsigned i, count;
    ...
    for (i = 0, count = 0;
         i < PJ_ARRAY_SIZE(pjsua_var.tpdata) && count < *p_count; ++i)
    {
        if (!pjsua_var.tpdata[i].data.ptr)
            continue;
...
pjsua_transport_close(pjsua_transport_id id, pj_bool_t force)
{
    ...
    if (force) {
         ...
         */
        PJ_LOG(1, (THIS_FILE,
                   "pjsua_transport_close(force=PJ_TRUE) is "
                   "deprecated."));
...
#if defined(PJLIB_UTIL_HAS_UPNP) && (PJLIB_UTIL_HAS_UPNP != 0)
        if (pjsua_var.ua_cfg.enable_upnp && pjsua_var.upnp_status == PJ_SUCCESS)
        {
            status = pj_upnp_del_port_mapping(&pjsua_var.tpdata[id].pub_addr);
            if (status != PJ_SUCCESS) {
                pjsua_perror(THIS_FILE,
                             "Error deleting pjsua_transport "
                             "UPnP port mapping",
                             status);
            }
        }
                          ...
                          const pjsua_transport_config* cfg)
{
    ...
    if ((tp_type == PJSIP_TRANSPORT_TLS) || (tp_type == PJSIP_TRANSPORT_TCP)) {
        ...
        {
            status =
              pjsip_tls_transport_lis_start(factory, &bind_addr, &addr_name);
        }
#endif
                            ...
                            const pjsua_msg_data* msg_data)
{
    ...
    if (pjsua_var.ua_cfg.user_agent.slen &&
        ...
        pjsip_hdr* h;
        h = (pjsip_hdr*)pjsip_generic_string_hdr_create(
          tdata->pool, &STR_USER_AGENT, &pjsua_var.ua_cfg.user_agent);
        pjsip_msg_add_hdr(tdata->msg, h);
    }
        ...
        msg_data->multipart_ctype.type.slen)
    {
        ...
        bodies = pjsip_multipart_create(tdata->pool, &msg_data->multipart_ctype,
                                        boundary);
 ...
 */
void pjsua_parse_media_type(pj_pool_t* pool, const pj_str_t* mime,
...
void pjsua_init_tpselector(pjsua_transport_id tp_id, pjsip_tpselector* sel)
{
    ...
    PJ_ASSERT_RETURN(
      tp_id >= 0 && tp_id < (int)PJ_ARRAY_SIZE(pjsua_var.tpdata), );
...
PJ_DEF(pj_status_t) pjsua_detect_nat_type()
{
    ...
    status = pj_stun_detect_nat_type2(&pjsua_var.stun_srv, &pjsua_var.stun_cfg,
                                      NULL, &nat_detect_cb);
...
pj_status_t normalize_route_uri(pj_pool_t* pool, pj_str_t* uri)
{
    ...
    if (!uri_obj) {
        PJ_LOG(
          1, (THIS_FILE, "Invalid route URI: %.*s", (int)uri->slen, uri->ptr));
    ...
    {
        PJ_LOG(1, (THIS_FILE, "Route URI must be SIP URI: %.*s", (int)uri->slen,
                   uri->ptr));
        pj_pool_release(tmp_pool);
    ...
    if (tmp_uri.slen < 1) {
        PJ_LOG(1, (THIS_FILE, "Route URI is too long: %.*s", (int)uri->slen,
                   uri->ptr));
        pj_pool_release(tmp_pool);
...
static pj_status_t handle_ip_change_on_acc()
{
    ...
    if (pjsua_var.acc_cnt == 0) {
        PJ_LOG(3,
               (THIS_FILE, "No account is set, IP change handling will stop"));
    ...
    for (i = 0; i < (int)PJ_ARRAY_SIZE(pjsua_var.acc); ++i) {
            ...
            for (; j < PJ_ARRAY_SIZE(pjsua_var.tpdata); ++j) {
                    ...
                    pjsua_var.tpdata[j].type == regc_info.transport->key.type)
                {
                    if ((pjsua_var.tpdata[j].data.factory ==
                        ...
                        (pjsua_var.tpdata[j].data.tp == regc_info.transport))
                    {
        ...
        {
            PJ_LOG(3, (THIS_FILE,
                       "Permanent registration failure, "
                       "IP change handling will stop for acc %d",
                       acc->index));
        ...
        if (shutdown_transport) {
            ...
            PJ_LOG(3, (THIS_FILE,
                       "Shutdown transport %s used by account %s "
                       "triggered by IP change",
                       transport->obj_name, acc_id));
                                    ...
                                    unsigned restart_lis_delay)
{
    ...
    pj_sockaddr_init(pjsip_transport_type_get_af(tp_info.type), &bind_addr,
                     NULL, pj_sockaddr_get_port(&tp_info.local_addr));
    ...
    case PJSIP_TRANSPORT_UDP6: {
        ...
        status = pjsip_udp_transport_restart2(
          pjsua_var.tpdata[id].data.tp, PJSIP_UDP_TRANSPORT_DESTROY_SOCKET,
          PJ_INVALID_SOCKET, &bind_addr, NULL);
    ...
    case PJSIP_TRANSPORT_TLS6:
        status = pjsip_tls_transport_restart(pjsua_var.tpdata[id].data.factory,
                                             &bind_addr, NULL);
        break;
#endif
    case PJSIP_TRANSPORT_TCP:
    case PJSIP_TRANSPORT_TCP6:
        status = pjsip_tcp_transport_restart(pjsua_var.tpdata[id].data.factory,
                                             &bind_addr, NULL);
        break;
    ...
    PJ_PERROR(3, (THIS_FILE, status, "Listener %.*s restart", tp_info.info.slen,
                  tp_info.info.ptr));

    if (status != PJ_SUCCESS && (restart_lis_delay > 0)) {
        /* Try restarting again, with delay. */
        pjsua_schedule_timer2(&restart_listener_cb, (void*)(pj_size_t)id,
                              restart_lis_delay);
        ...
        if (pjsua_var.ua_cfg.cb.on_ip_change_progress) {
            ...
            pjsua_var.ua_cfg.cb.on_ip_change_progress(
              PJSUA_IP_CHANGE_OP_RESTART_LIS, status, &info);
        }
