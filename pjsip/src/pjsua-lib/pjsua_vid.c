...
pj_status_t pjsua_vid_subsys_init(void)
{
    ...
    status = pjmedia_vid_conf_create(pjsua_var.pool, NULL, &pjsua_var.vid_conf);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE,
                     "Error creating PJMEDIA video conference bridge", status);
        goto on_error;
    }
...
#        if PJMEDIA_HAS_VIDEO && PJMEDIA_HAS_OPENH264_CODEC
    ...
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Error initializing OpenH264 library", status);
        goto on_error;
    }
#        endif
...
#        if PJMEDIA_HAS_VIDEO && PJMEDIA_HAS_VPX_CODEC
    ...
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Error initializing VPX library", status);
        goto on_error;
    }
#        endif
...
{
    const char* win_type_names[] = { "none", "preview", "stream" };
                          ...
                          const void* pval, pj_bool_t keep)
{
    ...
    if (keep) {
        ...
        id = info.id;
        status =
          pjmedia_vid_dev_param_set_cap(&pjsua_var.vid_param[id], cap, pval);
        if (status == PJ_SUCCESS) {
  ...
  const pjmedia_vid_codec_info* p_info[])
{
    ...
    status = pjmedia_vid_codec_mgr_find_codecs_by_id(NULL, codec_id, &count_,
                                                     info, NULL);
    if (status != PJ_SUCCESS)
        return status;
 ...
 */
PJ_DEF(pj_status_t)
pjsua_vid_enum_codecs(pjsua_codec_info id[], unsigned* p_count)
{
    ...
    for (i = 0, j = 0; i < count && j < *p_count; ++i) {
        if (info[i].packings & PJMEDIA_VID_PACKING_PACKETS) {
            ...
            pjmedia_vid_codec_info_to_id(&info[i], id[j].buf_,
                                         sizeof(id[j].buf_));
...
pjsua_vid_codec_set_priority(const pj_str_t* codec_id, pj_uint8_t priority)
{
    ...
    return pjmedia_vid_codec_mgr_set_codec_priority(NULL, codec_id, priority);
}
...
static pj_status_t create_vid_win(
  pjsua_vid_win_type type, const pjmedia_format* fmt,
  pjmedia_vid_dev_index rend_id, pjmedia_vid_dev_index cap_id, pj_bool_t show,
  unsigned wnd_flags, const pjmedia_vid_dev_hwnd* wnd, pjsua_vid_win_id* id)
{
    ...
    PJ_LOG(4,
           (THIS_FILE, "Creating video window: type=%s, cap_id=%d, rend_id=%d",
    ...
    if (type == PJSUA_WND_TYPE_PREVIEW) {
        ...
        if (wid != PJSUA_INVALID_ID) {
            ...
            status = pjmedia_vid_dev_stream_set_cap(
              strm, PJMEDIA_VID_DEV_CAP_OUTPUT_HIDE, &hide);
            if (status != PJ_SUCCESS) {
            ...
            status = pjmedia_vid_dev_stream_set_cap(
              strm, PJMEDIA_VID_DEV_CAP_OUTPUT_WINDOW_FLAGS, &wnd_flags);
            if (status != PJ_SUCCESS) {
                PJ_PERROR(4, (THIS_FILE, status,
    ...
    if (w->type == PJSUA_WND_TYPE_PREVIEW) {
        ...
        status =
          pjmedia_vid_dev_default_param(w->pool, cap_id, &vp_param.vidparam);
        if (status != PJ_SUCCESS)
            goto on_error;
...
#        undef update_param
        ...
        status = pjsua_vid_conf_add_port(
          w->pool, pjmedia_vid_port_get_passive_port(w->vp_cap), NULL,
          &w->cap_slot);
        if (status != PJ_SUCCESS)
            goto on_error;
        ...
        if (w->is_native) {
            ...
            cap_dev = pjmedia_vid_port_get_stream(w->vp_cap);
            status = pjmedia_vid_dev_stream_set_cap(
              cap_dev, PJMEDIA_VID_DEV_CAP_INPUT_PREVIEW, &enabled);
            if (status != PJ_SUCCESS) {
                pjsua_perror(THIS_FILE,
    ...
    if (!w->is_native) {
        status =
          pjmedia_vid_dev_default_param(w->pool, rend_id, &vp_param.vidparam);
        if (status != PJ_SUCCESS)
            goto on_error;
        ...
        status = pjsua_vid_conf_add_port(
          w->pool, pjmedia_vid_port_get_passive_port(w->vp_rend), NULL,
          &w->rend_slot);
        if (status != PJ_SUCCESS)
            goto on_error;
        ...
        PJ_LOG(4,
               (THIS_FILE, "%s window id %d created for cap_dev=%d rend_dev=%d",
...
static void free_vid_win(pjsua_vid_win_id wid)
{
    ...
    if (w->vp_cap) {
        ...
        pjmedia_event_unsubscribe(NULL, &call_media_on_event, NULL, w->vp_cap);
...
static pj_status_t setup_vid_capture(pjsua_call_media* call_med)
{
    ...
    if (wid == PJSUA_INVALID_ID) {
        ...
        status = create_vid_win(PJSUA_WND_TYPE_PREVIEW, &media_port->info.fmt,
                                call_med->strm.v.rdr_dev,
                                call_med->strm.v.cap_dev, PJSUA_HIDE_WINDOW,
                                acc_enc->cfg.vid_wnd_flags, NULL, &wid);
        if (status != PJ_SUCCESS)
            goto on_error;
...
#        endif
    ...
    status =
      pjsua_vid_conf_connect(w->cap_slot, call_med->strm.v.strm_enc_slot, NULL);
    if (status != PJ_SUCCESS)
        goto on_error;
    ...
    PJ_LOG(
      4, (THIS_FILE,
          "Call %d media %d: video capture set up with "
          "dev %d, wid=%d",
          call_med->call->index, call_med->idx, call_med->strm.v.cap_dev, wid));
...
#        endif
        ...
        if (si->dir & PJMEDIA_DIR_ENCODING) {
            ...
            status =
              pjmedia_vid_dev_get_info(call_med->strm.v.cap_dev, &dev_info);
            if (status != PJ_SUCCESS)
                goto on_error;
...
#        endif
            si->rtcp_sdes_bye_disabled =
              prm.stream_info.info.vid.rtcp_sdes_bye_disabled;
            si->codec_param->enc_fmt =
              prm.stream_info.info.vid.codec_param->enc_fmt;
        }

        /* Create session based on session info. */
        status =
          pjmedia_vid_stream_create(pjsua_var.med_endpt, NULL, si, call_med->tp,
                                    NULL, &call_med->strm.v.stream);
        if (status != PJ_SUCCESS)
            goto on_error;
        ...
        if (si->dir & PJMEDIA_DIR_DECODING) {
            ...
            status = pjmedia_vid_stream_get_port(
              call_med->strm.v.stream, PJMEDIA_DIR_DECODING, &media_port);
            if (status != PJ_SUCCESS) {
                pj_log_pop_indent();
                goto on_error;
            }
            ...
            PJSUA_LOCK();
            status =
              create_vid_win(PJSUA_WND_TYPE_STREAM, &media_port->info.fmt,
                             call_med->strm.v.rdr_dev,
                             // acc->cfg.vid_rend_dev,
                             PJSUA_INVALID_ID, acc->cfg.vid_in_auto_show,
                             acc->cfg.vid_wnd_flags, NULL, &wid);
            if (status != PJ_SUCCESS) {
                PJSUA_UNLOCK();
                pj_log_pop_indent();
                goto on_error;
...
void pjsua_vid_stop_stream(pjsua_call_media* call_med)
{
    ...
    if (call_med->strm.v.rdr_win_id != PJSUA_INVALID_ID) {
        ...
        /* Retrieve stream decoding port */
        status =
          pjmedia_vid_stream_get_port(strm, PJMEDIA_DIR_DECODING, &media_port);
        if (status == PJ_SUCCESS) {
            pjmedia_event_unsubscribe(NULL, &call_media_on_event, call_med,
                                      media_port);
    ...
    if (call_med->strm.v.cap_win_id != PJSUA_INVALID_ID) {
        ...
        PJ_LOG(4, (THIS_FILE,
                   "Call %d media %d: Preview video window "
                   "released",
    ...
    if (call_med->strm.v.rdr_win_id != PJSUA_INVALID_ID) {
        ...
        PJ_LOG(4, (THIS_FILE,
                   "Call %d media %d: Stream video window "
                   "released",
                   call_med->call->index, call_med->idx));
    }
    PJSUA_UNLOCK();
...
PJ_DEF(pj_bool_t) pjsua_vid_preview_has_native(pjmedia_vid_dev_index id)
{
    ...
    return (pjmedia_vid_dev_get_info(id, &vdi) == PJ_SUCCESS) ?
             ((vdi.caps & PJMEDIA_VID_DEV_CAP_INPUT_PREVIEW) != 0) :
             PJ_FALSE;
}
                        ...
                        const pjsua_vid_preview_param* prm)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Starting preview for cap_dev=%d, show=%d", id,
               prm->show));
    pj_log_push_indent();
    ...
    if (prm->format.detail_type == PJMEDIA_FORMAT_DETAIL_VIDEO)
        fmt = &prm->format;
    status = create_vid_win(PJSUA_WND_TYPE_PREVIEW, fmt, rend_id, id, prm->show,
                            prm->wnd_flags,
    ...
    if (w->is_native) {
        ...
        if (!w->vp_cap) {
            ...
            if (!cap_strm) {
                ...
                status =
                  pjmedia_vid_dev_stream_get_cap(cap_strm, cap, &wi->hwnd);
            }
        }
        PJSUA_UNLOCK();
        return status;
 ...
 * Show or hide window.
 */
PJ_DEF(pj_status_t) pjsua_vid_win_set_show(pjsua_vid_win_id wid, pj_bool_t show)
{
    pjsua_vid_win* w;
    pjmedia_vid_dev_stream* s;
    pj_bool_t hide;
    ...
    hide = !show;
    status =
      pjmedia_vid_dev_stream_set_cap(s, PJMEDIA_VID_DEV_CAP_OUTPUT_HIDE, &hide);
...
pjsua_vid_win_set_pos(pjsua_vid_win_id wid, const pjmedia_coord* pos)
{
    ...
    status = pjmedia_vid_dev_stream_set_cap(
      s, PJMEDIA_VID_DEV_CAP_OUTPUT_POSITION, pos);
...
pjsua_vid_win_set_size(pjsua_vid_win_id wid, const pjmedia_rect_size* size)
{
    ...
    status = pjmedia_vid_dev_stream_set_cap(
      s, PJMEDIA_VID_DEV_CAP_OUTPUT_RESIZE, size);
...
pjsua_vid_win_set_win(pjsua_vid_win_id wid, const pjmedia_vid_dev_hwnd* win)
{
    ...
    status =
      pjmedia_vid_dev_stream_set_cap(s, PJMEDIA_VID_DEV_CAP_OUTPUT_WINDOW, win);
 ...
 */
PJ_DEF(pj_status_t) pjsua_vid_win_rotate(pjsua_vid_win_id wid, int angle)
{
    pjsua_vid_win* w;
    pjmedia_vid_dev_stream* s;
    ...
    status = pjmedia_vid_dev_stream_set_cap(s, PJMEDIA_VID_DEV_CAP_ORIENTATION,
                                            &orient);
                             ...
                             pjmedia_vid_dev_fullscreen_flag mode)
{
    ...
    status = pjmedia_vid_dev_stream_set_cap(
      s, PJMEDIA_VID_DEV_CAP_OUTPUT_FULLSCREEN, &mode);
...
static void call_get_vid_strm_info(pjsua_call* call, int* first_active,
                                   int* first_inactive, unsigned* active_cnt,
                                   unsigned* cnt)
{
                                  ...
                                  pjmedia_dir dir)
{
    ...
    status =
      pjsua_call_media_init(call_med, PJMEDIA_TYPE_VIDEO, &acc_cfg->rtp_cfg,
                            call->secure_level, NULL, PJ_FALSE, NULL);
    if (status != PJ_SUCCESS)
        goto on_error;
    ...
    status = pjmedia_transport_media_create(call_med->tp, pool, options, NULL,
                                            call_med->idx);
    if (status != PJ_SUCCESS)
        goto on_error;
    ...
    status = pjmedia_transport_encode_sdp(call_med->tp, pool, sdp, NULL,
                                          call_med->idx);
    if (status != PJ_SUCCESS)
        goto on_error;
 ...
 * remove/disable.
 */
static pj_status_t call_modify_video(pjsua_call* call, int med_idx,
                                     pjmedia_dir dir, pj_bool_t remove)
{
    pjsua_call_media* call_med;
    const pjmedia_sdp_session* current_sdp;
    pjmedia_sdp_session* sdp;
    ...
    if ((!remove && call_med->dir == dir) ||
        (remove &&
         (call_med->tp_st == PJSUA_MED_TP_DISABLED || call_med->tp == NULL)))
    {
        return PJ_SUCCESS;
    }
    ...
    if (!remove) {
        ...
        if (sdp->media[med_idx]->desc.port == 0) {
            ...
            status = pjmedia_endpt_create_video_sdp(
              pjsua_var.med_endpt, pool, &tpinfo.sock_info, 0, &sdp_m);
            if (status != PJ_SUCCESS)
                goto on_error;
        }
...
static pj_status_t call_change_cap_dev(pjsua_call* call, int med_idx,
                                       pjmedia_vid_dev_index cap_dev)
{
    pjsua_call_media* call_med;
    ...
    status = pjmedia_vid_dev_stream_set_cap(old_dev, PJMEDIA_VID_DEV_CAP_SWITCH,
                                            &switch_prm);
    if (status == PJ_SUCCESS) {
    ...
    status =
      pjsua_vid_conf_disconnect(w->cap_slot, call_med->strm.v.strm_enc_slot);
    if (status != PJ_SUCCESS) {
        PJSUA_UNLOCK();
        return status;
    }
    ...
    if (new_wid == PJSUA_INVALID_ID) {
        ...
        /* Create preview video window */
        status =
          create_vid_win(PJSUA_WND_TYPE_PREVIEW, &media_port->info.fmt,
                         call_med->strm.v.rdr_dev, cap_dev, PJSUA_HIDE_WINDOW,
                         acc->cfg.vid_wnd_flags, NULL, &new_wid);
        if (status != PJ_SUCCESS)
            goto on_error;
    }
...
on_error:
    PJ_PERROR(
      4, (THIS_FILE, status, "Call %d: error changing capture device to %d",
          call->index, cap_dev));
...
static pj_status_t call_set_tx_video(pjsua_call* call, int med_idx,
                                     pj_bool_t enable)
{
    pjsua_call_media* call_med;
    pj_status_t status;
...
static pj_status_t call_send_vid_keyframe(pjsua_call* call, int med_idx)
{
    pjsua_call_media* call_med;
                        ...
                        const pjsua_call_vid_strm_op_param* param)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Call %d: set video stream, op=%d", call_id, op));
    pj_log_push_indent();
    ...
    case PJSUA_CALL_VID_STRM_REMOVE:
        status =
          call_modify_video(call, param_.med_idx, PJMEDIA_DIR_NONE, PJ_TRUE);
        break;
...
{
    return pjmedia_vid_conf_enum_ports(pjsua_var.vid_conf, (unsigned*)id,
                                       count);
}
                        ...
                        pjsua_conf_port_id* p_id)
{
    ...
    status = pjmedia_vid_conf_add_port(pjsua_var.vid_conf, pool, port, NULL,
                                       NULL, (unsigned*)p_id);
    if (status != PJ_SUCCESS) {
        if (p_id)
