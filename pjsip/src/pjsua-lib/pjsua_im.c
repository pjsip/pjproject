...
const pjsip_method pjsip_message_method = {
    (pjsip_method_e)PJSIP_MESSAGE_METHOD, { "MESSAGE", 7 }
};
                            ...
                            const pj_str_t* to, pjsip_rx_data* rdata)
{
    ...
    if (contact_hdr && contact_hdr->uri) {
        contact.ptr =
          (char*)pj_pool_alloc(rdata->tp_info.pool, PJSIP_MAX_URL_SIZE);
        contact.slen =
        ...
        pj_stricmp(&body->content_type.subtype, &STR_MIME_ISCOMPOSING) == 0)
    {
        ...
        status =
          pjsip_iscomposing_parse(rdata->tp_info.pool, (char*)body->data,
                                  body->len, &is_typing, NULL, NULL, NULL);
        ...
        if (body) {
            ...
            mime_type.ptr = buf;
            mime_type.slen = pj_ansi_snprintf(
              buf, sizeof(buf), "%.*s/%.*s", (int)m->type.slen, m->type.ptr,
...
static pj_bool_t im_on_rx_request(pjsip_rx_data* rdata)
{
     ...
     */
    pjsip_endpt_respond(pjsua_var.endpt, NULL, rdata, 200, NULL, NULL, NULL,
                        NULL);
    ...
    from.slen =
      pjsip_uri_print(PJSIP_URI_IN_FROMTO_HDR, rdata->msg_info.from->uri,
    ...
    to.slen = pjsip_uri_print(PJSIP_URI_IN_FROMTO_HDR, rdata->msg_info.to->uri,
    ...
    if (e->type == PJSIP_EVENT_TSX_STATE) {
            ...
            (tsx->status_code == 401 || tsx->status_code == 407))
        {
            ...
            pjsip_auth_clt_set_credentials(
              &auth, pjsua_var.acc[im_data->acc_id].cred_cnt,
              pjsua_var.acc[im_data->acc_id].cred);

            pjsip_auth_clt_set_prefs(
              &auth, &pjsua_var.acc[im_data->acc_id].cfg.auth_pref);

            status =
              pjsip_auth_clt_reinit_req(&auth, rdata, tsx->last_tx, &tdata);
            if (status == PJ_SUCCESS) {
        ...
        if (tsx->status_code / 100 == 2) {
            PJ_LOG(4, (THIS_FILE, "Message \'%s\' delivered successfully",
                       im_data->body.ptr));
        } else {
            PJ_LOG(3, (THIS_FILE, "Failed to deliver message \'%s\': %d/%.*s",
                       im_data->body.ptr, tsx->status_code,
        ...
        if (pjsua_var.ua_cfg.cb.on_pager_status) {
            ...
            pjsua_var.ua_cfg.cb.on_pager_status(
              im_data->call_id, &im_data->to, &im_body, im_data->user_data,
              (pjsip_status_code)tsx->status_code, &tsx->status_text);
        }
        ...
        if (pjsua_var.ua_cfg.cb.on_pager_status2) {
            ...
            pjsua_var.ua_cfg.cb.on_pager_status2(
              im_data->call_id, &im_data->to, &im_body, im_data->user_data,
              (pjsip_status_code)tsx->status_code, &tsx->status_text,
              tsx->last_tx, rdata, im_data->acc_id);
        }
              ...
              const pjsua_msg_data* msg_data, void* user_data)
{
    ...
    status = pjsip_endpt_create_request(
      pjsua_var.endpt, &pjsip_message_method,
      (msg_data && msg_data->target_uri.slen ? &msg_data->target_uri : to),
      &acc->cfg.id, to, NULL, NULL, -1, NULL, &tdata);
    if (status != PJ_SUCCESS) {
    ...
    if (acc->contact.slen) {
        ...
        status = pjsua_acc_create_uac_contact(tdata->pool, &contact, acc_id,
    to); if (status != PJ_SUCCESS) { pjsua_perror(THIS_FILE, "Unable to generate
    Contact header", status); pjsip_tx_data_dec_ref(tdata); return status;
        }
    }
    ...
    */
    ...
    status = pjsip_endpt_send_request(pjsua_var.endpt, tdata, -1, im_data,
                                      &im_callback);
    if (status != PJ_SUCCESS) {
