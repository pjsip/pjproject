/* $Header: /pjproject/pjsip/src/pjsip_mod_ua/sip_dialog.c 28    12/02/05 9:05p Bennylp $ */
/* 
 * PJSIP - SIP Stack
...
pj_status_t pjsip_dlg_init_from_rdata( pjsip_dlg *dlg, pjsip_rx_data *rdata )
{
    ...
    dlg->remote.target = dlg->remote.contact->uri;
					    ...
					    pjsip_event *event)
{
	...
	case PJSIP_TSX_STATE_COMPLETED:
		...
		tdata = pjsip_dlg_create_request(dlg, &invite_method, -1);
		if (tdata) {
		    tdata = pjsip_auth_reinit_req( dlg->ua->endpt,
						   dlg->pool, &dlg->auth_sess,
						   dlg->cred_count, dlg->cred_info,
						   tdata, event->src.rdata);
		}
		 */
		if (tdata) {
