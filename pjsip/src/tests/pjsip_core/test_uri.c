/* $Header: /pjproject/pjsip/src/tests/pjsip_core/test_uri.c 7     6/19/05 6:12p Bennylp $ */
/* 
 * PJSIP - SIP Stack
...
static pj_status_t test_entry(struct uri_test *entry)
{
	 ...
	 * return OK.
	 */
	status = entry->status==ERR_SYNTAX_ERR ? PJ_OK : ERR_SYNTAX_ERR;
	goto on_return;
    }
	...
	status = entry->status==ERR_NOT_EQUAL ? PJ_OK : ERR_NOT_EQUAL;
	goto on_return;

    } else {
	/* Equal. See if this is the expected status. */
	status = entry->status==PJ_OK ? PJ_OK : -1;
	if (status != PJ_OK) {
	    goto on_return;
	}
    }
