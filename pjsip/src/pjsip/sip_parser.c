...
static void int_parse_status_line(pj_scanner* scanner, pjsip_status_line* line);
static void int_parse_user_pass(pj_scanner* scanner, pj_pool_t* pool,
                                pj_str_t* user, pj_str_t* pass);
static void int_parse_uri_host_port(pj_scanner* scanner, pj_str_t* p_host,
                                    int* p_port);
static pjsip_uri* int_parse_uri_or_name_addr(pj_scanner* scanner,
                                             pj_pool_t* pool, unsigned option);
static void* int_parse_sip_url(pj_scanner* scanner, pj_pool_t* pool,
                               pj_bool_t parse_params);
static pjsip_name_addr* int_parse_name_addr(pj_scanner* scanner,
                                            pj_pool_t* pool);
static void* int_parse_other_uri(pj_scanner* scanner, pj_pool_t* pool,
                                 pj_bool_t parse_params);
...
#define parser_stricmp(s1, s2) \
    (s1.slen != s2.slen || pj_stricmp_alnum(&s1, &s2))

/* Get a token and unescape */
PJ_INLINE(void)
parser_get_and_unescape(pj_scanner* scanner, pj_pool_t* pool,
                        const pj_cis_t* spec, const pj_cis_t* unesc_spec,
                        pj_str_t* token)
{
...
static void on_str_parse_error(const pj_str_t* str, int rc)
{
    ...
    case PJ_EINVAL:
        s =
          "NULL input string, invalid input string, or NULL return "
          "value pointer";
        break;
    case PJ_ETOOSMALL:
    ...
    if (str) {
        PJ_LOG(1, (THIS_FILE, "Error parsing '%.*s': %s", (int)str->slen,
                   str->ptr, s));
    } else {
...
static void strtoi_validate(const pj_str_t* str, int min_val, int max_val,
                            int* value)
{
    long retval;
    pj_status_t status;
...
PJ_DEF(void)
pjsip_concat_param_imp(pj_str_t* param, pj_pool_t* pool, const pj_str_t* pname,
                       const pj_str_t* pvalue, int sepchar)
{
    char *new_param, *p;
    pj_size_t len;
...
static pj_status_t init_parser()
{
    ...
    status =
      pj_exception_id_alloc("PJSIP syntax error", &PJSIP_SYN_ERR_EXCEPTION);
    ...
    */
    ...
    /*
    status = pj_cis_dup(&pconst.pjsip_VIA_PARAM_SPEC_ESC,
    &pconst.pjsip_TOKEN_SPEC_ESC); PJ_ASSERT_RETURN(status == PJ_SUCCESS,
    status); pj_cis_add_str(&pconst.pjsip_VIA_PARAM_SPEC_ESC, "[:]");
    */
    ...
    status =
      pj_cis_dup(&pconst.pjsip_PARAM_CHAR_SPEC, &pconst.pjsip_ALNUM_SPEC);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    pj_cis_add_str(&pconst.pjsip_PARAM_CHAR_SPEC, PARAM_CHAR);

    status = pj_cis_dup(&pconst.pjsip_PARAM_CHAR_SPEC_ESC,
    ...
    status =
      pj_cis_dup(&pconst.pjsip_HDR_CHAR_SPEC_ESC, &pconst.pjsip_HDR_CHAR_SPEC);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    ...
    status =
      pj_cis_dup(&pconst.pjsip_USER_SPEC_LENIENT, &pconst.pjsip_USER_SPEC);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    pj_cis_add_str(&pconst.pjsip_USER_SPEC_LENIENT, "#");

    status = pj_cis_dup(&pconst.pjsip_USER_SPEC_LENIENT_ESC,
                        &pconst.pjsip_USER_SPEC_ESC);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    ...
    status =
      pj_cis_dup(&pconst.pjsip_PASSWD_SPEC_ESC, &pconst.pjsip_PASSWD_SPEC);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    ...
    status =
      pj_cis_dup(&pconst.pjsip_OTHER_URI_CONTENT, &pconst.pjsip_ALNUM_SPEC);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    ...
    status =
      pjsip_register_hdr_parser("Content-Length", "l", &parse_hdr_content_len);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);

    status =
      pjsip_register_hdr_parser("Content-Type", "c", &parse_hdr_content_type);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    ...
    status =
      pjsip_register_hdr_parser("Max-Forwards", NULL, &parse_hdr_max_forwards);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);

    status =
      pjsip_register_hdr_parser("Min-Expires", NULL, &parse_hdr_min_expires);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    ...
    status =
      pjsip_register_hdr_parser("Retry-After", NULL, &parse_hdr_retry_after);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);

    status = pjsip_register_hdr_parser("Supported", "k", &parse_hdr_supported);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    ...
    status =
      pjsip_register_hdr_parser("Unsupported", NULL, &parse_hdr_unsupported);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
 ...
 */
PJ_INLINE(int)
compare_handler(const handler_rec* r1, const char* name, pj_size_t name_len,
                pj_uint32_t hash)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_register_hdr_parser(const char* hname, const char* hshortname,
                          pjsip_parse_hdr_func* fptr)
{
    unsigned i;
    pj_size_t len;
...
/* Register URI parser. */
PJ_DEF(pj_status_t)
pjsip_register_uri_parser(char* scheme, pjsip_parse_uri_func* func)
{
...
PJ_DEF(pjsip_msg*)
pjsip_parse_msg(pj_pool_t* pool, char* buf, pj_size_t size,
                pjsip_parser_err_report* err_list)
{
    pjsip_msg* msg = NULL;
    pj_scanner scanner;
...
PJ_DEF(pjsip_msg*)
pjsip_parse_rdata(char* buf, pj_size_t size, pjsip_rx_data* rdata)
{
    pj_scanner scanner;
    pjsip_parse_ctx context;
...
PJ_DEF(pj_status_t)
pjsip_find_msg(const char* buf, pj_size_t size, pj_bool_t is_datagram,
               pj_size_t* msg_size)
{
#if PJ_HAS_TCP
...
PJ_DEF(pjsip_uri*)
pjsip_parse_uri(pj_pool_t* pool, char* buf, pj_size_t size, unsigned option)
{
    pj_scanner scanner;
    pjsip_uri* uri = NULL;
    PJ_USE_EXCEPTION;
...
PJ_DEF(void)
pjsip_parse_param_imp(pj_scanner* scanner, pj_pool_t* pool, pj_str_t* pname,
                      pj_str_t* pvalue, unsigned option)
{
...
PJ_DEF(void)
pjsip_parse_uri_param_imp(pj_scanner* scanner, pj_pool_t* pool, pj_str_t* pname,
                          pj_str_t* pvalue, unsigned option)
{
                                ...
                                unsigned option)
{
    ...
    /* Get pname and optionally pvalue */
    pjsip_parse_uri_param_imp(scanner, pool, pname, pvalue, option);
}
...
static void int_parse_uri_host_port(pj_scanner* scanner, pj_str_t* host,
                                    int* p_port)
{
                                             ...
                                             pj_pool_t* pool, unsigned opt)
{
    ...
    if (*scanner->curptr == '"' || *scanner->curptr == '<') {
        ...
        if (next_ch == ':') {
            ...
            uri = (pjsip_uri*)(*func)(
              scanner, pool, (opt & PJSIP_PARSE_URI_IN_FROM_TO_HDR) == 0);
                               ...
                               pj_bool_t parse_params)
{
    ...
    if (parse_params) {
        while (*scanner->curptr == ';') {
            ...
            } else if (!parser_stricmp(pname, pconst.pjsip_METHOD_STR) &&
                       pvalue.slen) {
                url->method_param = pvalue;

            } else if (!parser_stricmp(pname, pconst.pjsip_TRANSPORT_STR) &&
                       pvalue.slen) {
                url->transport_param = pvalue;

            } else if (!parser_stricmp(pname, pconst.pjsip_TTL_STR) &&
                       pvalue.slen) {
                strtoi_validate(&pvalue, PJSIP_MIN_TTL, PJSIP_MAX_TTL,
                                &url->ttl_param);
            } else if (!parser_stricmp(pname, pconst.pjsip_MADDR_STR) &&
                       pvalue.slen) {
 ...
 */
PJ_DEF(pj_status_t)
pjsip_parse_status_line(char* buf, pj_size_t size,
                        pjsip_status_line* status_line)
{
    pj_scanner scanner;
    PJ_USE_EXCEPTION;
...
/* Parse generic array header. */
PJ_DEF(void)
pjsip_parse_generic_array_hdr_imp(pjsip_generic_array_hdr* hdr,
                                  pj_scanner* scanner)
{
                                     ...
                                     pjsip_parse_ctx* ctx)
{
    ...
    while (pj_cis_match(&pconst.pjsip_NOT_NEWLINE, *scanner->curptr)) {
        ...
        /* concatenate */
        tmp.ptr =
          (char*)pj_pool_alloc(ctx->pool, hdr->hvalue.slen + next.slen + 2);
        tmp.slen = 0;
...
static void int_parse_contact_param(pjsip_contact_hdr* hdr, pj_scanner* scanner,
                                    pj_pool_t* pool)
{
    while (*scanner->curptr == ';') {
        pj_str_t pname, pvalue;
...
static pjsip_hdr* parse_hdr_contact(pjsip_parse_ctx* ctx)
{
    ...
    do {
        ...
        if (*scanner->curptr == '*') {
            ...
            hdr->star = 0;
            hdr->uri = int_parse_uri_or_name_addr(
              scanner, ctx->pool,
              PJSIP_PARSE_URI_AS_NAMEADDR | PJSIP_PARSE_URI_IN_FROM_TO_HDR);
...
static void parse_hdr_fromto(pj_scanner* scanner, pj_pool_t* pool,
                             pjsip_from_hdr* hdr)
{
    hdr->uri = int_parse_uri_or_name_addr(
      scanner, pool,
...
static pjsip_hdr* parse_hdr_require(pjsip_parse_ctx* ctx)
{
    ...
    pj_bool_t new_hdr =
      (ctx->rdata == NULL || ctx->rdata->msg_info.require == NULL);
...
static pjsip_hdr* parse_hdr_supported(pjsip_parse_ctx* ctx)
{
    ...
    pj_bool_t new_hdr =
      (ctx->rdata == NULL || ctx->rdata->msg_info.supported == NULL);
                                ...
                                pj_pool_t* pool)
{
    while (*scanner->curptr == ';') {
        ...
        } else if (!parser_stricmp(pname, pconst.pjsip_RECEIVED_STR) &&
                   pvalue.slen) {
...
PJ_DEF(void*)
pjsip_parse_hdr(pj_pool_t* pool, const pj_str_t* hname, char* buf,
                pj_size_t size, int* parsed_len)
{
    pj_scanner scanner;
    pjsip_hdr* hdr = NULL;
    pjsip_parse_ctx context;
...
PJ_DEF(pj_status_t)
pjsip_parse_headers(pj_pool_t* pool, char* input, pj_size_t size,
                    pjsip_hdr* hlist, unsigned options)
{
    enum
    {
        STOP_ON_ERROR = 1
