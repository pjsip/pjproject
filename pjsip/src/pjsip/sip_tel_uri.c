...
static pj_ssize_t tel_uri_print(pjsip_uri_context_e context,
                                const pjsip_tel_uri* url, char* buf,
                                pj_size_t size);
static int tel_uri_cmp(pjsip_uri_context_e context, const pjsip_tel_uri* url1,
                       const pjsip_tel_uri* url2);
...
static pjsip_uri_vptr tel_uri_vptr = { (P_GET_SCHEME)&tel_uri_get_scheme,
...
/* Print tel: URI */
static pj_ssize_t tel_uri_print(pjsip_uri_context_e context,
                                const pjsip_tel_uri* uri, char* buf,
                                pj_size_t size)
{
    int printed;
    char* startbuf = buf;
...
{
    const char *s1 = number1->ptr, *e1 = number1->ptr + number1->slen,
               *s2 = number2->ptr, *e2 = number2->ptr + number2->slen;
        ...
        do {
            ...
            if (*scanner->curptr == '=') {
                ...
                pj_scan_get_unescape(
                  scanner, &pjsip_TEL_PARSING_PVALUE_SPEC_ESC, &pvalue);
#else
