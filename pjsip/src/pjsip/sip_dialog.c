...
static pj_status_t create_dialog(pjsip_user_agent* ua, pj_grp_lock_t* grp_lock,
                                 pjsip_dialog** p_dlg)
{
    ...
    pool = pjsip_endpt_create_pool(endpt, "dlg%p", PJSIP_POOL_LEN_DIALOG,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_create_uac(pjsip_user_agent* ua, const pj_str_t* local_uri,
                     const pj_str_t* local_contact, const pj_str_t* remote_uri,
                     const pj_str_t* target, pjsip_dialog** p_dlg)
{
...
PJ_DEF(pj_status_t)
pjsip_dlg_create_uac2(const pjsip_dlg_create_uac_param* create_param,
                      pjsip_dialog** p_dlg)
{
    pj_status_t status;
    pj_str_t tmp;
    ...
    PJ_ASSERT_RETURN(create_param->ua && create_param->local_uri.slen &&
                       create_param->remote_uri.slen && p_dlg,
                     PJ_EINVAL);
    ...
    pj_strdup_with_null(dlg->pool, &tmp,
                        create_param->target.slen ? &create_param->target :
        ...
        PJSIP_URI_SCHEME_IS_SIPS(dlg->target))
    {
        ...
        while (param != &uri->header_param) {
            if (param->value.ptr) {
                ...
                hdr = (pjsip_hdr*)pjsip_parse_hdr(dlg->pool, &param->name,
                                                  param->value.ptr,
    ...
    dlg->local.info->uri = pjsip_parse_uri(dlg->pool, dlg->local.info_str.ptr,
    ...
    pj_strdup_with_null(dlg->pool, &tmp,
                        create_param->local_contact.slen ?
                          &create_param->local_contact :
                          &create_param->local_uri);
    dlg->local.contact = (pjsip_contact_hdr*)pjsip_parse_hdr(
      dlg->pool, &HCONTACT, tmp.ptr, tmp.slen, NULL);
    ...
    pj_strdup_with_null(dlg->pool, &dlg->remote.info_str,
                        &create_param->remote_uri);
    dlg->remote.info->uri = pjsip_parse_uri(dlg->pool, dlg->remote.info_str.ptr,
        ...
        PJSIP_URI_SCHEME_IS_SIPS(dlg->remote.info->uri))
    {
        ...
        if (!pj_list_empty(&sip_uri->header_param)) {
            ...
            /* Print URI */
            tmp2.ptr =
              (char*)pj_pool_alloc(dlg->pool, dlg->remote.info_str.slen);
            tmp2.slen = pjsip_uri_print(PJSIP_URI_IN_FROMTO_HDR, sip_uri,
                                        tmp2.ptr, dlg->remote.info_str.slen);
 ...
 * Create UAS dialog.
 */
pj_status_t create_uas_dialog(pjsip_user_agent* ua, pjsip_rx_data* rdata,
                              const pj_str_t* contact, pj_bool_t inc_lock,
                              pjsip_dialog** p_dlg)
{
    pj_status_t status;
    pjsip_hdr* pos = NULL;
    ...
    dlg->local.info =
      (pjsip_fromto_hdr*)pjsip_hdr_clone(dlg->pool, rdata->msg_info.to);
    ...
    len = pjsip_uri_print(PJSIP_URI_IN_FROMTO_HDR, dlg->local.info->uri,
                          tmp.ptr, TMP_LEN);
    if (len < 1) {
    ...
    if (contact) {
        ...
        dlg->local.contact = (pjsip_contact_hdr*)pjsip_parse_hdr(
          dlg->pool, &HCONTACT, tmp2.ptr, tmp2.slen, NULL);
        if (!dlg->local.contact) {
            status = PJSIP_EINVALIDURI;
            goto on_error;
        }
    ...
    dlg->remote.info =
      (pjsip_fromto_hdr*)pjsip_hdr_clone(dlg->pool, rdata->msg_info.from);
    pjsip_fromto_hdr_set_to(dlg->remote.info);

    /* Print the remote info. */
    len = pjsip_uri_print(PJSIP_URI_IN_FROMTO_HDR, dlg->remote.info->uri,
                          tmp.ptr, TMP_LEN);
    if (len < 1) {
        pj_ansi_strcpy(tmp.ptr, "<-error: uri too long->");
        tmp.slen = pj_ansi_strlen(tmp.ptr);
    ...
    dlg->remote.contact =
      (pjsip_contact_hdr*)pjsip_hdr_clone(dlg->pool, (pjsip_hdr*)contact_hdr);
    ...
    /* Call-ID */
    dlg->call_id =
      (pjsip_cid_hdr*)pjsip_hdr_clone(dlg->pool, rdata->msg_info.cid);
    ...
    dlg->remote.tag_hval =
      pj_hash_calc_tolower(0, NULL, &dlg->remote.info->tag);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_create_uas(pjsip_user_agent* ua, pjsip_rx_data* rdata,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_create_uas_and_inc_lock(pjsip_user_agent* ua, pjsip_rx_data* rdata,
                                  const pj_str_t* contact, pjsip_dialog** p_dlg)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_set_transport(pjsip_dialog* dlg, const pjsip_tpselector* sel)
{
    /* Validate */
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_set_via_sent_by(pjsip_dialog* dlg, pjsip_host_port* via_addr,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_fork(const pjsip_dialog* first_dlg, const pjsip_rx_data* rdata,
               pjsip_dialog** new_dlg)
{
    ...
    PJ_ASSERT_RETURN(
      (status / 100 == 1 && status != 100) || (status / 100 == 2), PJ_EBUG);
    ...
    contact =
      (const pjsip_contact_hdr*)pjsip_msg_find_hdr(msg, PJSIP_H_CONTACT, NULL);
    ...
    /* Clone local info. */
    dlg->local.info =
      (pjsip_fromto_hdr*)pjsip_hdr_clone(dlg->pool, first_dlg->local.info);
    ...
    /* Clone local Contact. */
    dlg->local.contact =
      (pjsip_contact_hdr*)pjsip_hdr_clone(dlg->pool, first_dlg->local.contact);

    /* Clone remote info. */
    dlg->remote.info =
      (pjsip_fromto_hdr*)pjsip_hdr_clone(dlg->pool, first_dlg->remote.info);
    ...
    dlg->call_id =
      (pjsip_cid_hdr*)pjsip_hdr_clone(dlg->pool, first_dlg->call_id);
    ...
    status =
      pjsip_auth_clt_clone(dlg->pool, &dlg->auth_sess, &first_dlg->auth_sess);
    if (status != PJ_SUCCESS)
        goto on_error;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_set_route_set(pjsip_dialog* dlg, const pjsip_route_hdr* route_set)
{
    pjsip_route_hdr* r;
...
PJ_DEF(pj_status_t) pjsip_dlg_try_inc_lock(pjsip_dialog* dlg)
{
    ...
    PJ_LOG(6,
           (dlg->obj_name, "Entering pjsip_dlg_try_inc_lock(), sess_count=%d",
            dlg->sess_count));
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_add_usage(pjsip_dialog* dlg, pjsip_module* mod, void* mod_data)
{
    unsigned index;
    ...
    PJ_LOG(5, (dlg->obj_name, "Module %.*s added as dialog usage, data=%p",
     ...
     */
    pj_array_insert(dlg->usage, sizeof(dlg->usage[0]), dlg->usage_cnt, index,
                    &mod);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_set_mod_data(pjsip_dialog* dlg, int mod_id, void* data)
{
    PJ_ASSERT_RETURN(dlg, PJ_EINVAL);
...
static pj_status_t dlg_create_request_throw(pjsip_dialog* dlg,
                                            ...
                                            int cseq, pjsip_tx_data** p_tdata)
{
    pjsip_tx_data* tdata;
    ...
    status = pjsip_endpt_create_request_from_hdr(
      dlg->endpt, method, dlg->target, dlg->local.info, dlg->remote.info,
      contact, dlg->call_id, cseq, NULL, &tdata);
    if (status != PJ_SUCCESS)
        return status;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_create_request(pjsip_dialog* dlg, const pjsip_method* method,
                         int cseq, pjsip_tx_data** p_tdata)
{
    pj_status_t status;
 ...
 */
static void send_ack_callback(pjsip_send_state* send_state, pj_ssize_t sent,
                              pj_bool_t* cont)
{
    if (sent > 0)
        return;
    ...
    if (*cont) {
        ...
        PJ_PERROR(3, (THIS_FILE, (pj_status_t)-sent, "Failed to send %s!",
                      pjsip_tx_data_get_info(send_state->tdata)));
    }
}
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_send_request(pjsip_dialog* dlg, pjsip_tx_data* tdata, int mod_data_id,
                       void* mod_data)
{
    ...
    if (msg->line.req.method.id != PJSIP_ACK_METHOD) {
        ...
        /* Send request */
        status = pjsip_endpt_send_request_stateless(dlg->endpt, tdata, NULL,
                                                    &send_ack_callback);
        if (status != PJ_SUCCESS)
            goto on_error;
    }
...
static void dlg_beautify_response(pjsip_dialog* dlg, pj_bool_t add_headers,
                                  int st_code, pjsip_tx_data* tdata)
{
    pjsip_cseq_hdr* cseq;
    int st_class;
    ...
    if (add_headers && pjsip_method_creates_dialog(&cseq->method)) {
        ...
        if (st_class == 2 || st_class == 3 ||
            (st_class == 1 && st_code != 100) || st_code == 485)
        {
            ...
            {
                hdr =
                  (pjsip_hdr*)pjsip_hdr_clone(tdata->pool, dlg->local.contact);
        ...
        {
            c_hdr =
              pjsip_endpt_get_capability(dlg->endpt, PJSIP_H_SUPPORTED, NULL);
            if (c_hdr) {
                hdr = (pjsip_hdr*)pjsip_hdr_clone(tdata->pool, c_hdr);
                pjsip_msg_add_hdr(tdata->msg, hdr);
            }
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_create_response(pjsip_dialog* dlg, pjsip_rx_data* rdata, int st_code,
                          const pj_str_t* st_text, pjsip_tx_data** p_tdata)
{
     ...
     */
    status =
      pjsip_endpt_create_response(dlg->endpt, rdata, st_code, st_text, &tdata);
    if (status != PJ_SUCCESS)
        return status;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_modify_response(pjsip_dialog* dlg, pjsip_tx_data* tdata, int st_code,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_send_response(pjsip_dialog* dlg, pjsip_transaction* tsx,
                        pjsip_tx_data* tdata)
{
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_respond(pjsip_dialog* dlg, pjsip_rx_data* rdata, int st_code,
...
void pjsip_dlg_on_rx_request(pjsip_dialog* dlg, pjsip_rx_data* rdata)
{
        ...
        rdata->msg_info.msg->line.req.method.id != PJSIP_CANCEL_METHOD)
    {
        ...
        pjsip_endpt_respond_stateless(dlg->endpt, rdata, 500, &warn_text, NULL,
                                      NULL);
        pj_log_pop_indent();
        return;
    }
        ...
        rdata->msg_info.msg->line.req.method.id != PJSIP_ACK_METHOD)
    {
        ...
        if (status != PJ_SUCCESS) {
            ...
            pjsip_endpt_respond_stateless(dlg->endpt, rdata, 500, &reason, NULL,
                                          NULL);
            goto on_return;
        }
    ...
    if (pjsip_method_creates_dialog(&rdata->msg_info.cseq->method)) {
        ...
        if (contact && contact->uri &&
            (dlg->remote.contact == NULL ||
             pjsip_uri_cmp(PJSIP_URI_IN_REQ_URI, dlg->remote.contact->uri,
                           contact->uri)))
        {
            dlg->remote.contact =
              (pjsip_contact_hdr*)pjsip_hdr_clone(dlg->pool, contact);
            dlg->target = dlg->remote.contact->uri;
        }
    }
...
void pjsip_dlg_on_rx_response(pjsip_dialog* dlg, pjsip_rx_data* rdata)
{
    ...
    if ((dlg->state == PJSIP_DIALOG_STATE_NULL &&
         ...
         (res_code > 100 && res_code < 300) && rdata->msg_info.to->tag.slen) ||
        (dlg->role == PJSIP_ROLE_UAC && !dlg->uac_has_2xx && res_code > 100 &&
         res_code / 100 <= 2 &&
         pjsip_method_creates_dialog(&rdata->msg_info.cseq->method) &&
    ...
    if (dlg->usage_cnt == 0) {
                   ...
                   rdata->msg_info.msg->line.status.code == 407)
        {
            ...
            status = pjsip_auth_clt_reinit_req(&dlg->auth_sess, rdata,
                                               tsx->last_tx, &tdata);
 ...
 * state notification.
 */
void pjsip_dlg_on_tsx_state(pjsip_dialog* dlg, pjsip_transaction* tsx,
                            pjsip_event* e)
{
    unsigned i;
...
pjsip_dlg_remote_has_cap(pjsip_dialog* dlg, int htype, const pj_str_t* hname,
                         const pj_str_t* token)
{
    ...
    hdr = (const pjsip_generic_array_hdr*)pjsip_dlg_get_remote_cap_hdr(
      dlg, htype, hname);
    if (!hdr) {
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_update_remote_cap(pjsip_dialog* dlg, const pjsip_msg* msg,
                            pj_bool_t strict)
{
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(htypes); ++i) {
        ...
        hdr = (const pjsip_generic_array_hdr*)pjsip_msg_find_hdr(msg, htypes[i],
                                                                 NULL);
        if (!hdr) {
            /* Not found.
            ...
            while (hdr) {
                ...
                for (j = 0; j < hdr->count &&
                            tmp_hdr.count < PJSIP_GENERIC_ARRAY_MAX_COUNT;
                     ++j)
                {
                ...
                hdr = (const pjsip_generic_array_hdr*)pjsip_msg_find_hdr(
                  msg, htypes[i], hdr->next);
            }
 ...
 */
PJ_DEF(const pjsip_hdr*)
pjsip_dlg_get_remote_cap_hdr(pjsip_dialog* dlg, int htype,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_set_remote_cap_hdr(pjsip_dialog* dlg,
                             const pjsip_generic_array_hdr* cap_hdr)
{
    ...
    /* Find the header. */
    hdr = (pjsip_generic_array_hdr*)pjsip_dlg_get_remote_cap_hdr(
      dlg, cap_hdr->type, &cap_hdr->name);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_dlg_remove_remote_cap_hdr(pjsip_dialog* dlg, int htype,
                                const pj_str_t* hname)
{
    pjsip_generic_array_hdr* hdr;
    ...
    hdr =
      (pjsip_generic_array_hdr*)pjsip_dlg_get_remote_cap_hdr(dlg, htype, hname);
    if (!hdr) {
        pjsip_dlg_dec_lock(dlg);
        return PJ_ENOTFOUND;
    }
