...
static const struct
{
    ...
    PJ_BUILD_ERR(PJSIP_ENOTSX,
                 "No transaction is associated with the object "
    ...
    PJ_BUILD_ERR(PJSIP_EINVALIDALGORITHM,
                 "Invalid/unsupported digest algorithm"),
    ...
    PJ_BUILD_ERR(PJSIP_EAUTHSTALECOUNT,
                 "Maximum number of stale retries exceeded"),
    PJ_BUILD_ERR(PJSIP_EAUTHINNONCE,
                 "Invalid nonce value in authentication challenge"),
    ...
    PJ_BUILD_ERR(PJSIP_ENOREFERSESSION,
    ...
    PJ_BUILD_ERR(PJSIP_TLS_ECERTFILE,
                 "Error loading SSL certificate chain file"),
    PJ_BUILD_ERR(PJSIP_TLS_EKEYFILE,
    ...
    PJ_BUILD_ERR(PJSIP_TLS_ECONNECT,
                 "Unknown error when performing SSL connect()"),
    PJ_BUILD_ERR(PJSIP_TLS_EACCEPT,
 ...
 */
PJ_DEF(pj_str_t)
pjsip_strerror(pj_status_t statcode, char* buf, pj_size_t bufsize)
{
    pj_str_t errstr;
    ...
    errstr.ptr = buf;
    errstr.slen =
      pj_ansi_snprintf(buf, bufsize, "Unknown pjsip error %d", statcode);
