...
#define MD5_APPEND(pms, buf, len) \
    pj_md5_update(pms, (const pj_uint8_t*)buf, (unsigned)len)
...
PJ_DEF(void)
pjsip_cred_info_dup(pj_pool_t* pool, pjsip_cred_info* dst,
...
PJ_DEF(int)
pjsip_cred_info_cmp(const pjsip_cred_info* cred1, const pjsip_cred_info* cred2)
{
    int result;
...
PJ_DEF(void)
pjsip_auth_clt_pref_dup(pj_pool_t* pool, pjsip_auth_clt_pref* dst,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_auth_create_digest(pj_str_t* result, const pj_str_t* nonce,
                         const pj_str_t* nc, const pj_str_t* cnonce,
                         const pj_str_t* qop, const pj_str_t* uri,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_auth_create_digestSHA256(pj_str_t* result, const pj_str_t* nonce,
                               const pj_str_t* nc, const pj_str_t* cnonce,
                               const pj_str_t* qop, const pj_str_t* uri,
                               const pj_str_t* realm,
                               const pjsip_cred_info* cred_info,
...
#    else
        ...
        EVP_DigestUpdate(mdctx, cred_info->username.ptr,
...
static pj_status_t respond_digest(pj_pool_t* pool,
                                  ...
                                  const pj_str_t* uri,
                                  const pjsip_cred_info* cred_info,
                                  const pj_str_t* cnonce, pj_uint32_t nc,
                                  const pj_str_t* method)
{
    ...
    if (chal->algorithm.slen == 0 ||
        (algo_sha256 || pj_stricmp(&chal->algorithm, &pjsip_MD5_STR) == 0 ||
    ...
    if (chal->qop.slen == 0) {
        ...
        if ((cred_info->data_type & EXT_MASK) == PJSIP_CRED_DATA_EXT_AKA) {
            ...
            return (*cred_info->ext.aka.cb)(pool, chal, cred_info, method,
                                            cred);
        } else {
            /* Convert digest to string and store in chal->response. */
            if (algo_sha256) {
                status = pjsip_auth_create_digestSHA256(
                  &cred->response, &cred->nonce, NULL, NULL, NULL, uri,
        ...
        if ((cred_info->data_type & EXT_MASK) == PJSIP_CRED_DATA_EXT_AKA) {
            ...
            if (algo_sha256) {
                status = pjsip_auth_create_digestSHA256(
                  &cred->response, &cred->nonce, &cred->nc, &cred->cnonce,
                  &pjsip_AUTH_STR, uri, &chal->realm, cred_info, method);
            } else {
                status = pjsip_auth_create_digest(
                  &cred->response, &cred->nonce, &cred->nc, &cred->cnonce,
                  &pjsip_AUTH_STR, uri, &chal->realm, cred_info, method);
        ...
        PJ_LOG(4, (THIS_FILE, "Unsupported qop offer %.*s", chal->qop.slen,
                   chal->qop.ptr));
...
#    if PJSIP_AUTH_AUTO_SEND_NEXT != 0
        if (!cached_auth->last_chal || pj_stricmp2(&hdr->scheme, "digest")) {
            cached_auth->last_chal =
              (pjsip_www_authenticate_hdr*)pjsip_hdr_clone(cached_auth->pool,
                                                           hdr);
        } else {
            ...
            {
                cached_auth->last_chal =
                  (pjsip_www_authenticate_hdr*)pjsip_hdr_clone(
                    cached_auth->pool, hdr);
            }
        }
#    endif
        return;
    ...
    if (cached_auth->cnonce.slen == 0) {
        ...
        cached_auth->last_chal =
          (pjsip_www_authenticate_hdr*)pjsip_hdr_clone(cached_auth->pool, hdr);
...
/* Init client session. */
PJ_DEF(pj_status_t)
pjsip_auth_clt_init(pjsip_auth_clt_sess* sess, pjsip_endpoint* endpt,
...
/* Clone session. */
PJ_DEF(pj_status_t)
pjsip_auth_clt_clone(pj_pool_t* pool, pjsip_auth_clt_sess* sess,
                     const pjsip_auth_clt_sess* rhs)
{
    ...
    sess->cred_info = (pjsip_cred_info*)pj_pool_alloc(
      pool, sess->cred_cnt * sizeof(pjsip_cred_info));
...
PJ_DEF(pj_status_t)
pjsip_auth_clt_set_credentials(pjsip_auth_clt_sess* sess, int cred_cnt,
                               const pjsip_cred_info* c)
{
    ...
    if (cred_cnt == 0) {
        ...
        int i;
        sess->cred_info =
          (pjsip_cred_info*)pj_pool_alloc(sess->pool, cred_cnt * sizeof(*c));
 ...
 */
PJ_DEF(pj_status_t)
pjsip_auth_clt_set_prefs(pjsip_auth_clt_sess* sess,
...
/*
 * Create Authorization/Proxy-Authorization response header based on the
 * challege in WWW-Authenticate/Proxy-Authenticate header.
 */
static pj_status_t auth_respond(
  pj_pool_t* req_pool, const pjsip_www_authenticate_hdr* hdr,
  const pjsip_uri* uri, const pjsip_cred_info* cred_info,
  const pjsip_method* method, pj_pool_t* sess_pool,
  pjsip_cached_auth* cached_auth, pjsip_authorization_hdr** p_h_auth)
{
    ...
    PJ_ASSERT_RETURN(req_pool && hdr && uri && cred_info && method &&
                       sess_pool && cached_auth && p_h_auth,
                     PJ_EINVAL);
...
#if PJSIP_AUTH_HEADER_CACHING
    {
        if (hauth && cached_auth &&
            cached_auth->qop_value == PJSIP_AUTH_QOP_NONE) {
                                    ...
                                    pjsip_authorization_hdr** p_h_auth)
{
    ...
    status =
      auth_respond(tdata->pool, auth->last_chal, tdata->msg->line.req.uri, cred,
                   &tdata->msg->line.req.method, sess->pool, auth, &hauth);
    if (status != PJ_SUCCESS)
        return status;
...
PJ_DEF(pj_status_t)
pjsip_auth_clt_init_req(pjsip_auth_clt_sess* sess, pjsip_tx_data* tdata)
{
...
#if defined(PJSIP_AUTH_HEADER_CACHING) && PJSIP_AUTH_HEADER_CACHING != 0
            {
...
#    endif
...
#elif defined(PJSIP_AUTH_AUTO_SEND_NEXT) && PJSIP_AUTH_AUTO_SEND_NEXT != 0
            {
                new_auth_for_req(tdata, sess, auth, NULL);
            }
#endif
...
#if defined(PJSIP_AUTH_QOP_SUPPORT) && defined(PJSIP_AUTH_AUTO_SEND_NEXT) && \
  (PJSIP_AUTH_QOP_SUPPORT && PJSIP_AUTH_AUTO_SEND_NEXT)
        else if (auth->qop_value == PJSIP_AUTH_QOP_AUTH)
        {
            ...
            status = auth_respond(
              tdata->pool, auth->last_chal, tdata->msg->line.req.uri, cred,
              &tdata->msg->line.req.method, sess->pool, auth, &hauth);
            if (status != PJ_SUCCESS)
                return status;
    ...
    if (sess->pref.initial_auth == PJ_FALSE) {
        ...
        uri.slen =
          pjsip_uri_print(PJSIP_URI_IN_REQ_URI, tdata->msg->line.req.uri,
...
{
    pj_pool_t* auth_pool =
      pjsip_endpt_create_pool(endpt, "auth_cli%p", 1024, 1024);
    ...
    if (auth->last_chal) {
        auth->last_chal = (pjsip_www_authenticate_hdr*)pjsip_hdr_clone(
          auth_pool, auth->last_chal);
    }
...
static pj_status_t process_auth(pj_pool_t* req_pool,
                                ...
                                const pjsip_uri* uri, pjsip_tx_data* tdata,
                                pjsip_auth_clt_sess* sess,
                                ...
                                pjsip_authorization_hdr** h_auth)
{
             ...
             hdr->type == PJSIP_H_PROXY_AUTHORIZATION))
        {
                ...
                pj_stricmp(&hchal->scheme, &sent_auth->scheme) == 0)
            {
                    ...
                    sent_auth->credential.digest.response.slen == 0)
                {
                    ...
                    continue;
                } else if (pj_stricmp(&sent_auth->scheme, &pjsip_DIGEST_STR) ==
                             0 &&
    ...
    if (!cred) {
        ...
        PJ_LOG(
          ...
          (THIS_FILE,
           ...
           tdata->obj_name, realm->slen, realm->ptr, hchal->scheme.slen,
           hchal->scheme.ptr));
        return PJSIP_ENOCREDENTIAL;
    }
    ...
    status =
      auth_respond(req_pool, hchal, uri, cred, &tdata->msg->line.req.method,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_auth_clt_reinit_req(pjsip_auth_clt_sess* sess, const pjsip_rx_data* rdata,
                          ...
                          pjsip_tx_data** new_request)
{
    ...
    while (hdr != &rdata->msg_info.msg->hdr) {
        ...
        if (!cached_auth) {
            ...
            cached_auth->pool =
              pjsip_endpt_create_pool(sess->endpt, "auth_cli%p", 1024, 1024);
            pj_strdup(cached_auth->pool, &cached_auth->realm,
