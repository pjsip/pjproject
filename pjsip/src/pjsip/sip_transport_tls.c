...
static pj_bool_t on_data_read(pj_ssl_sock_t* ssock, void* data, pj_size_t size,
...
static pj_status_t lis_create_transport(pjsip_tpfactory* factory,
                                        pjsip_tpmgr* mgr, pjsip_endpoint* endpt,
                                        const pj_sockaddr* rem_addr,
                                        int addr_len, pjsip_tx_data* tdata,
                                        pjsip_transport** transport);

/* Common function to create and initialize transport */
static pj_status_t tls_create(struct tls_listener* listener, pj_pool_t* pool,
                              pj_ssl_sock_t* ssock, pj_bool_t is_server,
                              const pj_sockaddr* local,
                              const pj_sockaddr* remote,
                              const pj_str_t* remote_name, pj_grp_lock_t* glock,
...
{
    PJ_PERROR(
      3, (sender, status, "%s: [code=%d]%s%.*s", title, status,
          remote_name ? " peer: " : "", remote_name ? remote_name->slen : 0,
          remote_name ? remote_name->ptr : ""));
}

static void sockaddr_to_host_port(pj_pool_t* pool, pjsip_host_port* host_port,
                            ...
                            struct tls_listener* listener)
{
    ...
    ssock_param->require_client_cert =
      listener->tls_setting.require_client_cert;
                                       ...
                                       const pjsip_host_port* addr_name)
{
    ...
    pj_ansi_snprintf(listener->factory.obj_name,
                     sizeof(listener->factory.obj_name), "tlstp:%d",
...
static void update_transport_info(struct tls_listener* listener)
{
    ...
    if (listener->factory.info == NULL) {
        listener->factory.info =
          (char*)pj_pool_alloc(listener->factory.pool, INFO_LEN);
    }
    ...
    pj_addr_str_print(&listener->factory.addr_name.host,
                      ...
                      sizeof(pub_addr), 1);
    len = pj_ansi_snprintf(listener->factory.info, INFO_LEN,
                           "tls %s [published as %s]", local_addr, pub_addr);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tls_transport_start(pjsip_endpoint* endpt, const pjsip_tls_setting* opt,
...
PJ_DEF(pj_status_t)
pjsip_tls_transport_lis_start(pjsip_tpfactory* factory,
                              const pj_sockaddr* local,
                              const pjsip_host_port* a_name)
{
    ...
    if (listener->cert) {
        status = pj_ssl_sock_set_certificate(
          listener->ssock, listener->factory.pool, listener->cert);
        if (status != PJ_SUCCESS)
            return status;
    }
    ...
    status = pj_ssl_sock_start_accept2(
      listener->ssock, listener->factory.pool, (pj_sockaddr_t*)listener_addr,
      pj_sockaddr_get_len((pj_sockaddr_t*)listener_addr), &newsock_param);
...
PJ_DEF(pj_status_t)
pjsip_tls_transport_start2(pjsip_endpoint* endpt, const pjsip_tls_setting* opt,
                           const pj_sockaddr* local,
                           const pjsip_host_port* a_name, unsigned async_cnt,
                           pjsip_tpfactory** p_factory)
{
    ...
    listener->factory.type_name =
      (char*)pjsip_transport_get_type_name(listener->factory.type);
        ...
        listener->tls_setting.privkey_file.slen)
    {
        status = pj_ssl_cert_load_from_files2(
          pool, &listener->tls_setting.ca_list_file,
          &listener->tls_setting.ca_list_path, &listener->tls_setting.cert_file,
          &listener->tls_setting.privkey_file, &listener->tls_setting.password,
          &listener->cert);
        if (status != PJ_SUCCESS)
            goto on_error;
               ...
               listener->tls_setting.privkey_buf.slen)
    {
        status = pj_ssl_cert_load_from_buffer(
          pool, &listener->tls_setting.ca_buf, &listener->tls_setting.cert_buf,
          &listener->tls_setting.privkey_buf, &listener->tls_setting.password,
          &listener->cert);
        if (status != PJ_SUCCESS)
            goto on_error;
    }
...
#    endif
    ...
    status =
      pjsip_tpmgr_register_tpfactory(listener->tpmgr, &listener->factory);
    if (status != PJ_SUCCESS) {
...
PJ_DEF(pj_status_t)
pjsip_tls_transport_restart(pjsip_tpfactory* factory, const pj_sockaddr* local,
                            const pjsip_host_port* a_name)
{
    pj_status_t status = PJ_SUCCESS;
    struct tls_listener* listener = (struct tls_listener*)factory;
                              ...
                              struct tls_transport** p_tls)
{
    ...
    if (pool == NULL) {
        pool = pjsip_endpt_create_pool(listener->endpt, "tls", POOL_TP_INIT,
                                       POOL_TP_INC);
    ...
    pj_ansi_snprintf(
      tls->base.info, 64, "%s to %s", tls->base.type_name,
      pj_sockaddr_print(remote, print_addr, sizeof(print_addr), 3));
...
static void tls_flush_pending_tx(struct tls_transport* tls)
{
    ...
    while (!pj_list_empty(&tls->delayed_list)) {
        ...
        status =
          pj_ssl_sock_send(tls->ssock, op_key, tdata->buf.start, &size, 0);
...
static pj_status_t tls_start_read(struct tls_transport* tls)
{
    ...
    /* Init rdata */
    pool = pjsip_endpt_create_pool(tls->base.endpt, "rtd%p",
    ...
    status =
      pj_ssl_sock_start_read2(tls->ssock, tls->base.pool, size, readbuf, 0);
    if (status != PJ_SUCCESS && status != PJ_EPENDING) {
        PJ_PERROR(
          4, (tls->base.obj_name, status, "pj_ssl_sock_start_read() error"));
        return status;
    }
                                        ...
                                        pjsip_transport** p_transport)
{
    ...
    /* Sanity checks */
    PJ_ASSERT_RETURN(
      factory && mgr && endpt && rem_addr && addr_len && p_transport,
      PJ_EINVAL);
    ...
    pj_ssl_sock_param_default(&ssock_param);
    ssock_param.sock_af =
      (factory->type & PJSIP_TRANSPORT_IPV6) ? pj_AF_INET6() : pj_AF_INET();
    ...
    status = tls_create(listener, pool, ssock, PJ_FALSE, &local_addr, rem_addr,
                        &remote_name, glock, &tls);
    if (status != PJ_SUCCESS)
        return status;
    ...
    status = pj_ssl_sock_start_connect(tls->ssock, tls->base.pool,
                                       ...
                                       (pj_sockaddr_t*)rem_addr, addr_len);
    if (status == PJ_SUCCESS) {
    ...
    if (tls->has_pending_connect) {
        ...
        PJ_LOG(4,
               (tls->base.obj_name, "TLS transport %s is connecting to %s...",
                                     ...
                                     pj_status_t accept_status)
{
    ...
    PJ_LOG(4, (listener->factory.obj_name,
               ...
               pj_addr_str_print(&listener->factory.addr_name.host,
                                 ...
                                 sizeof(addr_buf), 1),
               pj_sockaddr_print(src_addr, addr, sizeof(addr), 3), new_ssock));
     ...
     */
    status =
      tls_create(listener, NULL, new_ssock, PJ_TRUE, &ssl_info.local_addr,
                 &tmp_src_addr, NULL, ssl_info.grp_lock, &tls);
 ...
 */
static pj_bool_t on_data_sent(pj_ssl_sock_t* ssock, pj_ioqueue_op_key_t* op_key,
                              pj_ssize_t bytes_sent)
{
    struct tls_transport* tls =
      (struct tls_transport*)pj_ssl_sock_get_user_data(ssock);
    ...
    if (bytes_sent <= 0) {
        ...
        PJ_LOG(5,
               (tls->base.obj_name, "TLS send() error, sent=%d", bytes_sent));
...
static pj_bool_t on_verify_cb(pj_ssl_sock_t* ssock, pj_bool_t is_server)
{
    ...
    if (verify_cb) {
        ...
        param.tp_dir =
          is_server ? PJSIP_TP_DIR_INCOMING : PJSIP_TP_DIR_OUTGOING;
                                ...
                                void* token, pjsip_transport_callback callback)
{
    ...
    if (!delayed) {
        ...
        size = tdata->buf.cur - tdata->buf.start;
        status =
          pj_ssl_sock_send(tls->ssock, (pj_ioqueue_op_key_t*)&tdata->op_key,
        ...
        if (status != PJ_EPENDING) {
            ...
            if (size <= 0) {
                PJ_LOG(5,
                       (tls->base.obj_name, "TLS send() error, sent=%d", size));
                              ...
                              pj_status_t status, pj_size_t* remainder)
{
    ...
    if (status == PJ_SUCCESS) {
         ...
         */
        size_eaten =
          pjsip_tpmgr_receive_packet(rdata->tp_info.transport->tpmgr, rdata);
        ...
        *remainder = size - size_eaten;
        if (*remainder > 0 && *remainder != size) {
            pj_memmove(rdata->pkt_info.packet,
                       rdata->pkt_info.packet + size_eaten, *remainder);
        }
...
static pj_bool_t on_connect_complete(pj_ssl_sock_t* ssock, pj_status_t status)
{
        ...
        for (i = 0; i < serv_cert->subj_alt_name.cnt && !matched; ++i) {
            ...
            case PJ_SSL_CERT_NAME_URI:
                    ...
                    pj_strnicmp2(cert_name, "sips:", 5) == 0)
                {
                    ...
                    pj_strset(&host_part, p,
                              cert_name->slen - (p - cert_name->ptr));
    ...
    PJ_LOG(4, (tls->base.obj_name, "TLS transport %s is connected to %s",
               pj_addr_str_print(&tls->base.local_name.host,
                                 tls->base.local_name.port, local_addr_buf,
                                 sizeof(local_addr_buf), 1),
               pj_addr_str_print(&tls->base.remote_name.host,
...
static void tls_keep_alive_timer(pj_timer_heap_t* th, pj_timer_entry* e)
{
    ...
    PJ_LOG(5, (tls->base.obj_name, "Sending %d byte(s) keep-alive to %s",
               (int)tls->ka_pkt.slen,
               pj_addr_str_print(&tls->base.remote_name.host,
                                 tls->base.remote_name.port, addr, sizeof(addr),
                                 1)));

    /* Send the data */
    size = tls->ka_pkt.slen;
    status = pj_ssl_sock_send(tls->ssock, &tls->ka_op_key.key, tls->ka_pkt.ptr,
                              &size, 0);

    if (status != PJ_SUCCESS && status != PJ_EPENDING) {
        tls_perror(tls->base.obj_name, "Error sending keep-alive packet",
                   status, &tls->remote_name);
