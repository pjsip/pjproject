...
PJ_DEF_DATA(const pjsip_method)
...
static const pj_str_t* method_names[] = {
    &pjsip_invite_method.name,   &pjsip_cancel_method.name,
    &pjsip_ack_method.name,      &pjsip_bye_method.name,
    &pjsip_register_method.name, &pjsip_options_method.name
};
...
static int init_status_phrase()
{
    ...
    pj_strset2(&status_phrase[703],
               "Error sending message to destination server");
...
PJ_DEF(void)
pjsip_method_init(pjsip_method* m, pj_pool_t* pool, const pj_str_t* str)
{
    pj_str_t dup;
...
PJ_DEF(void) pjsip_method_init_np(pjsip_method* m, pj_str_t* str)
{
    unsigned i;
...
PJ_DEF(void)
pjsip_method_copy(pj_pool_t* pool, pjsip_method* method,
...
PJ_DEF(pjsip_msg*) pjsip_msg_clone(pj_pool_t* pool, const pjsip_msg* src)
{
    ...
    if (src->type == PJSIP_REQUEST_MSG) {
        ...
        dst->line.req.uri =
          (pjsip_uri*)pjsip_uri_clone(pool, src->line.req.uri);
    } else {
...
PJ_DEF(void*)
pjsip_hdr_find(const void* hdr_list, pjsip_hdr_e hdr_type, const void* start)
{
...
PJ_DEF(void*)
pjsip_hdr_find_by_name(const void* hdr_list, const pj_str_t* name,
                       const void* start)
{
    const pjsip_hdr *hdr = (const pjsip_hdr*)start, *end = hdr_list;
...
PJ_DEF(void*)
pjsip_hdr_find_by_names(const void* hdr_list, const pj_str_t* name,
                        const pj_str_t* sname, const void* start)
{
    const pjsip_hdr *hdr = (const pjsip_hdr*)start, *end = hdr_list;
...
PJ_DEF(void*)
pjsip_msg_find_hdr(const pjsip_msg* msg, pjsip_hdr_e hdr_type,
                   const void* start)
{
...
PJ_DEF(void*)
pjsip_msg_find_hdr_by_name(const pjsip_msg* msg, const pj_str_t* name,
                           const void* start)
{
...
PJ_DEF(void*)
pjsip_msg_find_hdr_by_names(const pjsip_msg* msg, const pj_str_t* name,
                            const pj_str_t* sname, const void* start)
{
...
PJ_DEF(void*)
pjsip_msg_find_remove_hdr(pjsip_msg* msg, pjsip_hdr_e hdr_type, void* start)
{
...
PJ_DEF(pj_ssize_t)
pjsip_msg_print(const pjsip_msg* msg, char* buf, pj_size_t size)
{
    ...
    for (hdr = msg->hdr.next; hdr != &msg->hdr; hdr = hdr->next) {
        ...
        if (len < 0) {
            if (len == -2) {
                PJ_LOG(5, ("sip_msg",
                           "Header with no vptr encountered!! "
                           "Current buffer: %.*s",
                           (int)(p - buf), buf));
            }
            return len;
 ...
 */
PJ_DEF(int)
pjsip_media_type_cmp(const pjsip_media_type* mt1, const pjsip_media_type* mt2,
                     pj_bool_t cmp_param)
{
    int rc;
...
PJ_DEF(void)
pjsip_media_type_cp(pj_pool_t* pool, pjsip_media_type* dst,
...
static pjsip_generic_string_hdr* pjsip_generic_string_hdr_clone(
  pj_pool_t* pool, const pjsip_generic_string_hdr* hdr);
static pjsip_generic_string_hdr* pjsip_generic_string_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_generic_string_hdr* hdr);
...
PJ_DEF(void)
pjsip_generic_string_hdr_init2(pjsip_generic_string_hdr* hdr, pj_str_t* hname,
                               pj_str_t* hvalue)
{
...
pjsip_generic_string_hdr_init(pj_pool_t* pool, void* mem,
                              const pj_str_t* hnames, const pj_str_t* hvalue)
{
...
PJ_DEF(pjsip_generic_string_hdr*)
pjsip_generic_string_hdr_create(pj_pool_t* pool, const pj_str_t* hnames,
                                          ...
                                          char* buf, pj_size_t size)
{
    ...
    const pj_str_t* hname =
      pjsip_cfg()->endpt.use_compact_form ? &hdr->sname : &hdr->name;
...
static pjsip_generic_string_hdr* pjsip_generic_string_hdr_clone(
  pj_pool_t* pool, const pjsip_generic_string_hdr* rhs)
{
...
{
    pjsip_generic_string_hdr* hdr =
      PJ_POOL_ALLOC_T(pool, pjsip_generic_string_hdr);
...
static int pjsip_generic_int_hdr_print(pjsip_generic_int_hdr* hdr, char* buf,
                                       pj_size_t size);
static pjsip_generic_int_hdr* pjsip_generic_int_hdr_clone(
  pj_pool_t* pool, const pjsip_generic_int_hdr* hdr);
static pjsip_generic_int_hdr* pjsip_generic_int_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_generic_int_hdr* hdr);
...
PJ_DEF(pjsip_generic_int_hdr*)
pjsip_generic_int_hdr_init(pj_pool_t* pool, void* mem, const pj_str_t* hnames,
                           unsigned value)
{
...
PJ_DEF(pjsip_generic_int_hdr*)
pjsip_generic_int_hdr_create(pj_pool_t* pool, const pj_str_t* hnames,
                             unsigned value)
{
...
static pjsip_generic_int_hdr* pjsip_generic_int_hdr_clone(
  pj_pool_t* pool, const pjsip_generic_int_hdr* rhs)
{
...
static int pjsip_generic_array_hdr_print(pjsip_generic_array_hdr* hdr,
                                         char* buf, pj_size_t size);
static pjsip_generic_array_hdr* pjsip_generic_array_hdr_clone(
  pj_pool_t* pool, const pjsip_generic_array_hdr* hdr);
static pjsip_generic_array_hdr* pjsip_generic_array_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_generic_array_hdr* hdr);
...
PJ_DEF(pjsip_generic_array_hdr*)
pjsip_generic_array_hdr_init(pj_pool_t* pool, void* mem, const pj_str_t* hnames)
{
...
PJ_DEF(pjsip_generic_array_hdr*)
pjsip_generic_array_hdr_create(pj_pool_t* pool, const pj_str_t* hnames)
{
...
static pjsip_generic_array_hdr* pjsip_generic_array_hdr_clone(
  pj_pool_t* pool, const pjsip_generic_array_hdr* rhs)
{
    unsigned i;
    pjsip_generic_array_hdr* hdr =
      PJ_POOL_ALLOC_T(pool, pjsip_generic_array_hdr);
 ...
 * Accept header.
 */
PJ_DEF(pjsip_accept_hdr*) pjsip_accept_hdr_init(pj_pool_t* pool, void* mem)
{
...
PJ_DEF(pjsip_allow_hdr*) pjsip_allow_hdr_init(pj_pool_t* pool, void* mem)
{
...
PJ_DEF(pjsip_cid_hdr*) pjsip_cid_hdr_init(pj_pool_t* pool, void* mem)
{
...
static pjsip_clen_hdr* pjsip_clen_hdr_clone(pj_pool_t* pool,
                                            const pjsip_clen_hdr* hdr);
...
PJ_DEF(pjsip_clen_hdr*) pjsip_clen_hdr_init(pj_pool_t* pool, void* mem)
{
...
PJ_DEF(pjsip_cseq_hdr*) pjsip_cseq_hdr_init(pj_pool_t* pool, void* mem)
{
...
static pjsip_contact_hdr* pjsip_contact_hdr_clone(pj_pool_t* pool,
                                                  const pjsip_contact_hdr* hdr);
static pjsip_contact_hdr* pjsip_contact_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_contact_hdr*);
...
PJ_DEF(pjsip_contact_hdr*) pjsip_contact_hdr_init(pj_pool_t* pool, void* mem)
{
                                   ...
                                   pj_size_t size)
{
    ...
    if (hdr->star) {
        ...
        *p++ = '*';
        ...
        printed = pjsip_uri_print(PJSIP_URI_IN_CONTACT_HDR, hdr->uri, buf,
                                  endbuf - buf);
        if (printed < 1)
            return -1;
        ...
        printed = (int)pjsip_param_print_on(&hdr->other_param, buf,
                                            endbuf - buf, &pc->pjsip_TOKEN_SPEC,
                                            &pc->pjsip_TOKEN_SPEC, ';');
        if (printed < 0)
            return printed;
        buf += printed;
...
static pjsip_contact_hdr* pjsip_contact_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_contact_hdr* rhs)
{
...
PJ_DEF(pjsip_ctype_hdr*) pjsip_ctype_hdr_init(pj_pool_t* pool, void* mem)
{
                            ...
                            const pjsip_media_type* media)
{
    ...
    *p++ = '/';
    ...
    printed =
      pjsip_param_print_on(&media->param, p, buf + len - p,
                           &pc->pjsip_TOKEN_SPEC, &pc->pjsip_TOKEN_SPEC, ';');
    if (printed < 0)
        return -1;
...
PJ_DEF(int)
pjsip_media_type_print(char* buf, unsigned len, const pjsip_media_type* media)
{
                                 ...
                                 pj_size_t size)
{
    ...
    if ((pj_ssize_t)size <
        hname->slen + hdr->media.type.slen + hdr->media.subtype.slen + 8)
    {
        return -1;
    }
 ...
 * Expires header.
 */
PJ_DEF(pjsip_expires_hdr*)
pjsip_expires_hdr_init(pj_pool_t* pool, void* mem, unsigned value)
{
...
PJ_DEF(pjsip_expires_hdr*)
pjsip_expires_hdr_create(pj_pool_t* pool, unsigned value)
{
 ...
 * To or From header.
 */
static int pjsip_fromto_hdr_print(pjsip_fromto_hdr* hdr, char* buf,
                                  pj_size_t size);
static pjsip_fromto_hdr* pjsip_fromto_hdr_clone(pj_pool_t* pool,
                                                const pjsip_fromto_hdr* hdr);
static pjsip_fromto_hdr* pjsip_fromto_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_fromto_hdr* hdr);
...
PJ_DEF(pjsip_from_hdr*) pjsip_from_hdr_init(pj_pool_t* pool, void* mem)
{
...
PJ_DEF(pjsip_to_hdr*) pjsip_to_hdr_init(pj_pool_t* pool, void* mem)
{
                                  ...
                                  pj_size_t size)
{
    ...
    *buf++ = ' ';
    ...
    printed =
      pjsip_uri_print(PJSIP_URI_IN_FROMTO_HDR, hdr->uri, buf, endbuf - buf);
    if (printed < 1)
        return -1;

    buf += printed;

    copy_advance_pair_escape(buf, ";tag=", 5, hdr->tag, pc->pjsip_TOKEN_SPEC);

    printed =
      pjsip_param_print_on(&hdr->other_param, buf, endbuf - buf,
                           &pc->pjsip_TOKEN_SPEC, &pc->pjsip_TOKEN_SPEC, ';');
    if (printed < 0)
        return -1;
    buf += printed;
...
static pjsip_fromto_hdr* pjsip_fromto_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_fromto_hdr* rhs)
{
 ...
 * Min-Expires header.
 */
PJ_DEF(pjsip_min_expires_hdr*)
pjsip_min_expires_hdr_init(pj_pool_t* pool, void* mem, unsigned value)
{
...
PJ_DEF(pjsip_min_expires_hdr*)
pjsip_min_expires_hdr_create(pj_pool_t* pool, unsigned value)
{
 ...
 */
static int pjsip_routing_hdr_print(pjsip_routing_hdr* r, char* buf,
                                   pj_size_t size);
static pjsip_routing_hdr* pjsip_routing_hdr_clone(pj_pool_t* pool,
                                                  const pjsip_routing_hdr* r);
static pjsip_routing_hdr* pjsip_routing_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_routing_hdr* r);
...
PJ_DEF(pjsip_rr_hdr*) pjsip_rr_hdr_init(pj_pool_t* pool, void* mem)
{
...
PJ_DEF(pjsip_route_hdr*) pjsip_route_hdr_init(pj_pool_t* pool, void* mem)
{
...
static int pjsip_routing_hdr_print(pjsip_routing_hdr* hdr, char* buf,
                                   pj_size_t size)
{
    pj_ssize_t printed;
    char* startbuf = buf;
...
static pjsip_routing_hdr* pjsip_routing_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_routing_hdr* rhs)
{
    pjsip_routing_hdr* hdr = PJ_POOL_ALLOC_T(pool, pjsip_routing_hdr);
    pj_memcpy(hdr, rhs, sizeof(*hdr));
    pjsip_param_shallow_clone(pool, &hdr->other_param, &rhs->other_param);
 ...
 * Require header.
 */
PJ_DEF(pjsip_require_hdr*) pjsip_require_hdr_init(pj_pool_t* pool, void* mem)
{
 ...
 * Retry-After header.
 */
static int pjsip_retry_after_hdr_print(pjsip_retry_after_hdr* r, char* buf,
                                       pj_size_t size);
static pjsip_retry_after_hdr* pjsip_retry_after_hdr_clone(
  pj_pool_t* pool, const pjsip_retry_after_hdr* r);
static pjsip_retry_after_hdr* pjsip_retry_after_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_retry_after_hdr* r);
...
PJ_DEF(pjsip_retry_after_hdr*)
pjsip_retry_after_hdr_init(pj_pool_t* pool, void* mem, int value)
{
...
PJ_DEF(pjsip_retry_after_hdr*)
pjsip_retry_after_hdr_create(pj_pool_t* pool, int value)
{
    void* mem = pj_pool_alloc(pool, sizeof(pjsip_retry_after_hdr));
    return pjsip_retry_after_hdr_init(pool, mem, value);
}

static int pjsip_retry_after_hdr_print(pjsip_retry_after_hdr* hdr, char* buf,
                                       pj_size_t size)
{
    char* p = buf;
    char* endbuf = buf + size;
    ...
    *p++ = ' ';
    ...
    printed =
      pjsip_param_print_on(&hdr->param, p, endbuf - p, &pc->pjsip_TOKEN_SPEC,
                           &pc->pjsip_TOKEN_SPEC, ';');
    if (printed < 0)
        return (int)printed;
...
static pjsip_retry_after_hdr* pjsip_retry_after_hdr_clone(
  pj_pool_t* pool, const pjsip_retry_after_hdr* rhs)
{
    pjsip_retry_after_hdr* hdr =
      pjsip_retry_after_hdr_create(pool, rhs->ivalue);
 ...
 * Supported header.
 */
PJ_DEF(pjsip_supported_hdr*)
pjsip_supported_hdr_init(pj_pool_t* pool, void* mem)
{
 ...
 * Unsupported header.
 */
PJ_DEF(pjsip_unsupported_hdr*)
pjsip_unsupported_hdr_init(pj_pool_t* pool, void* mem)
{
...
PJ_DEF(pjsip_via_hdr*) pjsip_via_hdr_init(pj_pool_t* pool, void* mem)
{
...
static int pjsip_via_hdr_print(pjsip_via_hdr* hdr, char* buf, pj_size_t size)
{
    ...
    if ((pj_ssize_t)size < hname->slen + sip_ver.slen + hdr->transport.slen +
                             hdr->sent_by.host.slen + 12)
    {
        return -1;
    }
...
pjsip_warning_hdr_create(pj_pool_t* pool, int code, const pj_str_t* host,
                         const pj_str_t* text)
{
    ...
    hvalue.ptr = (char*)pj_pool_alloc(pool, 10 +               /* code */
                                              ...
                                              text->slen + 2); /* text */
    hvalue.slen =
      pj_ansi_sprintf(hvalue.ptr, "%u %.*s \"%.*s\"", code, (int)host->slen,
                      host->ptr, (int)text->slen, text->ptr);

    return pjsip_generic_string_hdr_create(pool, &str_warning, &hvalue);
}

PJ_DEF(pjsip_warning_hdr*)
pjsip_warning_hdr_create_from_status(pj_pool_t* pool, const pj_str_t* host,
                                     pj_status_t status)
{
 ...
 */
PJ_DEF(int)
pjsip_print_text_body(pjsip_msg_body* msg_body, char* buf, pj_size_t size)
{
...
PJ_DEF(void*)
pjsip_clone_text_data(pj_pool_t* pool, const void* data, unsigned len)
{
    char* newdata = "";
...
PJ_DEF(pj_status_t)
pjsip_msg_body_copy(pj_pool_t* pool, pjsip_msg_body* dst_body,
                    const pjsip_msg_body* src_body)
{
    ...
    pjsip_media_type_cp(pool, &dst_body->content_type, &src_body->content_type);

    /* Duplicate data. */
    dst_body->data =
      (*src_body->clone_data)(pool, src_body->data, src_body->len);
...
PJ_DEF(pjsip_msg_body*)
pjsip_msg_body_clone(pj_pool_t* pool, const pjsip_msg_body* body)
{
...
PJ_DEF(pjsip_msg_body*)
pjsip_msg_body_create(pj_pool_t* pool, const pj_str_t* type,
                      const pj_str_t* subtype, const pj_str_t* text)
{
    pjsip_msg_body* body;
