...
static const char* state_str[] = {
    "Null",      "Calling",   "Trying",     "Proceeding",
    "Completed", "Confirmed", "Terminated", "Destroyed",
};

/* Role names */
static const char* role_name[] = { "UAC", "UAS" };
...
static void tsx_set_state(pjsip_transaction* tsx, pjsip_tsx_state_e state,
                          pjsip_event_id_e event_src_type, void* event_src,
                          int flag);
static void tsx_set_status_code(pjsip_transaction* tsx, int code,
                                const pj_str_t* reason);
static pj_status_t tsx_create(pjsip_module* tsx_user, pj_grp_lock_t* grp_lock,
...
static void tsx_update_transport(pjsip_transaction* tsx, pjsip_transport* tp);
...
static int (*tsx_state_handler_uas[PJSIP_TSX_STATE_MAX])(pjsip_transaction*,
                                                         pjsip_event*) = {
    &tsx_on_state_null,          NULL,
    &tsx_on_state_trying,        &tsx_on_state_proceeding_uas,
    &tsx_on_state_completed_uas, &tsx_on_state_confirmed,
    &tsx_on_state_terminated,    &tsx_on_state_destroyed,
};
 ...
 */
static pj_status_t create_tsx_key_2543(pj_pool_t* pool, pj_str_t* str,
                                       pjsip_role_e role,
 ...
 */
static pj_status_t create_tsx_key_3261(pj_pool_t* pool, pj_str_t* key,
                                       pjsip_role_e role,
                                       const pjsip_method* method,
                                       const pj_str_t* branch)
{
    ...
    p = key->ptr =
      (char*)pj_pool_alloc(pool, branch->slen + method->name.slen + 4);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_create_key(pj_pool_t* pool, pj_str_t* key, pjsip_role_e role,
                     const pjsip_method* method, const pjsip_rx_data* rdata)
{
 ...
 */
PJ_DEF(void)
pjsip_tsx_set_timers(unsigned t1, unsigned t2, unsigned t4, unsigned td)
{
    if (t1 != 0) {
...
PJ_DEF(pj_status_t) pjsip_tsx_layer_init_module(pjsip_endpoint* endpt)
{
    ...
    pool = pjsip_endpt_create_pool(endpt, "tsxlayer", PJSIP_POOL_TSX_LAYER_LEN,
...
static pj_status_t mod_tsx_layer_register_tsx(pjsip_transaction* tsx)
{
     ...
     * twice!
     */
    if (pj_hash_get_lower(mod_tsx_layer.htable, tsx->transaction_key.ptr,
                          (unsigned)tsx->transaction_key.slen, NULL))
    {
        pj_mutex_unlock(mod_tsx_layer.mutex);
        PJ_LOG(2,
               (THIS_FILE, "Unable to register %.*s transaction (key exists)",
    ...
    TSX_TRACE_(
      (THIS_FILE, "Transaction %p registered with hkey=0x%p and key=%.*s", tsx,
       tsx->hashed_key, tsx->transaction_key.slen, tsx->transaction_key.ptr));

    /* Register the transaction to the hash table. */
#ifdef PRECALC_HASH
    pj_hash_set_lower(tsx->pool, mod_tsx_layer.htable, tsx->transaction_key.ptr,
...
static void mod_tsx_layer_unregister_tsx(pjsip_transaction* tsx)
{
    ...
    TSX_TRACE_(
      (THIS_FILE, "Transaction %p unregistered, hkey=0x%p and key=%.*s", tsx,
       tsx->hashed_key, tsx->transaction_key.slen, tsx->transaction_key.ptr));
                                   ...
                                   pj_bool_t add_ref)
{
    ...
    TSX_TRACE_((THIS_FILE, "Finding tsx with hkey=0x%p and key=%.*s: found %p",
...
pjsip_tsx_layer_find_tsx(const pj_str_t* key, pj_bool_t lock)
{
    return find_tsx(key, lock, PJ_FALSE);
}

PJ_DEF(pjsip_transaction*)
pjsip_tsx_layer_find_tsx2(const pj_str_t* key, pj_bool_t add_ref)
{
...
static pj_status_t mod_tsx_layer_stop(void)
{
    ...
    while (it) {
        pjsip_transaction* tsx =
          (pjsip_transaction*)pj_hash_this(mod_tsx_layer.htable, it);
...
{
    return (pjsip_transaction*)rdata->endpt_info.mod_data[mod_tsx_layer.mod.id];
}
    ...
    if (detail) {
        ...
        if (it == NULL) {
            ...
            while (it != NULL) {
                ...
                PJ_LOG(3,
                       (THIS_FILE, " %s %s|%d|%s", tsx->obj_name,
                        (tsx->last_tx ? pjsip_tx_data_get_info(tsx->last_tx) :
                                        "none"),
...
static pj_status_t tsx_schedule_timer(pjsip_transaction* tsx,
                                      ...
                                      const pj_time_val* delay, int active_id)
{
    pj_timer_heap_t* timer_heap = pjsip_endpt_get_timer_heap(tsx->endpt);
    pj_status_t status;

    pj_assert(active_id != 0);
    status = pj_timer_heap_schedule_w_grp_lock(timer_heap, entry, delay,
                                               active_id, tsx->grp_lock);

    return status;
}

/* Utility: cancel a timer */
static int tsx_cancel_timer(pjsip_transaction* tsx, pj_timer_entry* entry)
{
    pj_timer_heap_t* timer_heap = pjsip_endpt_get_timer_heap(tsx->endpt);
                              ...
                              pjsip_transaction** p_tsx)
{
    ...
    pj_ansi_snprintf(tsx->obj_name, sizeof(tsx->obj_name), "tsx%p", tsx);
...
static pj_status_t tsx_shutdown(pjsip_transaction* tsx)
{
    ...
    if (tsx->transport_flag & TSX_HAS_PENDING_TRANSPORT) {
        ...
        PJ_LOG(4, (tsx->obj_name,
                   "Will destroy later because transport is "
                   "in progress"));
    }
    ...
    if (entry->id == TRANSPORT_ERR_TIMER || entry->id == TRANSPORT_DISC_TIMER) {
        ...
        if (tsx->state < PJSIP_TSX_STATE_TERMINATED) {
            ...
            if (tsx->tsx_user && tsx->tsx_user->on_tsx_state) {
                pjsip_event e;
                PJSIP_EVENT_INIT_TSX_STATE(e, tsx, PJSIP_EVENT_TRANSPORT_ERROR,
                                           NULL, prev_state);
                          ...
                          int flag)
{
    ...
    if (tsx->tsx_user && tsx->tsx_user->on_tsx_state && (flag & NO_NOTIFY) == 0)
    {
        pjsip_event e;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_create_uac(pjsip_module* tsx_user, pjsip_tx_data* tdata,
                     pjsip_transaction** p_tsx)
{
...
PJ_DEF(pj_status_t)
pjsip_tsx_create_uac2(pjsip_module* tsx_user, pjsip_tx_data* tdata,
                      pj_grp_lock_t* grp_lock, pjsip_transaction** p_tsx)
{
    pjsip_transaction* tsx;
    pjsip_msg* msg;
    pjsip_cseq_hdr* cseq;
    ...
    if (via->branch_param.slen == 0) {
        ...
        via->branch_param.ptr =
          (char*)pj_pool_alloc(tsx->pool, PJSIP_MAX_BRANCH_LEN);
    ...
    create_tsx_key_3261(tsx->pool, &tsx->transaction_key, PJSIP_ROLE_UAC,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_create_uas(pjsip_module* tsx_user, pjsip_rx_data* rdata,
                     pjsip_transaction** p_tsx)
{
...
PJ_DEF(pj_status_t)
pjsip_tsx_create_uas2(pjsip_module* tsx_user, pjsip_rx_data* rdata,
                      pj_grp_lock_t* grp_lock, pjsip_transaction** p_tsx)
{
    pjsip_transaction* tsx;
    pjsip_msg* msg;
    ...
    if (pjsip_method_cmp(&msg->line.req.method,
        ...
        PJ_LOG(4, (THIS_FILE,
                   "Error: CSeq header contains different "
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_set_transport(pjsip_transaction* tsx, const pjsip_tpselector* sel)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_set_timeout(pjsip_transaction* tsx, unsigned millisec)
{
    pj_time_val timeout;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_send_msg(pjsip_transaction* tsx, pjsip_tx_data* tdata)
{
    pjsip_event event;
    pj_status_t status;
    ...
    PJ_LOG(5, (tsx->obj_name, "Sending %s in state %s",
               pjsip_tx_data_get_info(tdata), state_str[tsx->state]));
    pj_log_push_indent();
 ...
 */
PJ_DEF(void) pjsip_tsx_recv_msg(pjsip_transaction* tsx, pjsip_rx_data* rdata)
{
    pjsip_event event;
...
static void send_msg_callback(pjsip_send_state* send_state, pj_ssize_t sent,
                              pj_bool_t* cont)
{
    ...
    if (sent > 0) {
        ...
        if (tsx->transport_flag & TSX_HAS_PENDING_DESTROY) {
            tsx_set_state(tsx, PJSIP_TSX_STATE_DESTROYED, PJSIP_EVENT_UNKNOWN,
                          NULL, 0);
            pj_grp_lock_release(tsx->grp_lock);
            return;
        }
        ...
        if ((!*cont) || (tsx->transport_flag & TSX_HAS_PENDING_DESTROY)) {
            ...
            PJ_LOG(3,
                   (tsx->obj_name, "Failed to send %s! err=%d (%s)",
                    pjsip_tx_data_get_info(send_state->tdata), -sent, errmsg));
                               ...
                               pj_ssize_t sent)
{
    ...
    if (sent < 0) {
        ...
        PJ_PERROR(
          2, (tsx->obj_name, (pj_status_t)-sent, "Transport failed to send %s!",
              pjsip_tx_data_get_info(tdata)));
 ...
 */
static pj_status_t tsx_send_msg(pjsip_transaction* tsx, pjsip_tx_data* tdata)
{
    ...
    if (tsx->transport) {
        ...
        status = pjsip_transport_send(tsx->transport, tdata, &tsx->addr,
                                      tsx->addr_len, tsx, &transport_callback);
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(2, (tsx->obj_name, status, "Error sending %s",
                          pjsip_tx_data_get_info(tdata)));
    ...
    if (tdata->msg->type == PJSIP_REQUEST_MSG) {
        ...
        tsx->transport_flag |= TSX_HAS_PENDING_TRANSPORT;
        status = pjsip_endpt_send_response(tsx->endpt, &tsx->res_addr, tdata,
                                           tsx, &send_msg_callback);
        if (status == PJ_EPENDING)
            status = PJ_SUCCESS;
        if (status != PJ_SUCCESS) {
            tsx->transport_flag &= ~(TSX_HAS_PENDING_TRANSPORT);
...
static pj_status_t tsx_retransmit(pjsip_transaction* tsx, int resched)
{
    ...
    PJ_LOG(5, (tsx->obj_name, "Retransmiting %s, count=%d, restart?=%d",
               pjsip_tx_data_get_info(tsx->last_tx), tsx->retransmit_count,
               resched));
 ...
 */
static pj_status_t tsx_on_state_null(pjsip_transaction* tsx, pjsip_event* event)
{
    pj_status_t status;
    ...
    if (tsx->role == PJSIP_ROLE_UAS) {
        ...
        if (!tsx->is_reliable) {
            ...
            if (tsx->transport_flag & TSX_HAS_PENDING_TRANSPORT) {
                ...
                tsx_schedule_timer(tsx, &tsx->retransmit_timer, &t1_timer_val,
                                   RETRANSMIT_TIMER);
            }
        }

        /* Move state. */
        tsx_set_state(tsx, PJSIP_TSX_STATE_CALLING, PJSIP_EVENT_TX_MSG, tdata,
                      0);
    }
                                        ...
                                        pjsip_event* event)
{
               ...
               event->body.timer.entry == &tsx->timeout_timer)
    {
        ...
        /* Inform TU. */
        tsx_set_state(tsx, PJSIP_TSX_STATE_TERMINATED, PJSIP_EVENT_TIMER,
                      &tsx->timeout_timer, 0);
    ...
    if (event->type == PJSIP_EVENT_RX_MSG) {
        ...
        if (PJSIP_IS_STATUS_IN_CLASS(tsx->status_code, 100)) {
            ...
            if (tsx->method.id == PJSIP_INVITE_METHOD) {
                ...
                tsx_schedule_timer(tsx, &tsx->timeout_timer, &zero_time,
                                   TIMEOUT_TIMER);
            }
            unlock_timer(tsx);
            ...
            /* Inform TU */
            tsx_set_state(tsx, PJSIP_TSX_STATE_COMPLETED, PJSIP_EVENT_TX_MSG,
                          tdata, 0);
                                               ...
                                               pjsip_event* event)
{
    ...
    if (PJSIP_IS_STATUS_IN_CLASS(tsx->status_code, 100)) {
        ...
        tsx_set_state(tsx, PJSIP_TSX_STATE_PROCEEDING, PJSIP_EVENT_RX_MSG,
                      event->body.rx_msg.rdata, 0);
         ...
         */
        if (tsx->method.id == PJSIP_INVITE_METHOD) {
            tsx_set_state(tsx, PJSIP_TSX_STATE_TERMINATED, PJSIP_EVENT_RX_MSG,
                          event->body.rx_msg.rdata, 0);
            // return PJSIP_ETSXDESTROYED;
            ...
            tsx_set_state(tsx, PJSIP_TSX_STATE_COMPLETED, PJSIP_EVENT_RX_MSG,
                          event->body.rx_msg.rdata, 0);
        }
	...
	/* XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX */
        ...
        if (tsx->method.id == PJSIP_INVITE_METHOD) {
            ...
            status = pjsip_endpt_create_ack(
              tsx->endpt, tsx->last_tx, event->body.rx_msg.rdata, &ack_tdata);
            if (status != PJ_SUCCESS)
                return status;
                                              ...
                                              pjsip_event* event)
{
    ...
    if (event->type == PJSIP_EVENT_RX_MSG) {
        ...
        if (msg->line.req.method.id != PJSIP_ACK_METHOD) {
            ...
            tsx_set_state(tsx, PJSIP_TSX_STATE_CONFIRMED, PJSIP_EVENT_RX_MSG,
                          event->body.rx_msg.rdata, 0);
        }
        ...
        PJ_ASSERT_RETURN(event->type == PJSIP_EVENT_TX_MSG, PJ_EINVALIDOP);
                                              ...
                                              pjsip_event* event)
{
    ...
    if (event->type == PJSIP_EVENT_TIMER) {
        ...
        if (tsx->method.id == PJSIP_INVITE_METHOD) {
            ...
            if (msg->type == PJSIP_RESPONSE_MSG && msg->line.status.code >= 200)
            {
                pj_status_t status;
                                           ...
                                           pjsip_event* event)
{
    ...
    tsx_set_state(tsx, PJSIP_TSX_STATE_DESTROYED, event->type,
                  event->body.user.user1, 0);
