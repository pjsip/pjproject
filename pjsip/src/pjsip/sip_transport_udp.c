                                 ...
                                 pj_ssize_t bytes_read)
{
    ...
    for (i = 0;; ++i) {
        ...
        if (bytes_read > MIN_SIZE) {
            ...
            size_eaten = pjsip_tpmgr_receive_packet(
              rdata->tp_info.transport->tpmgr, rdata);
         ...
         */
        {
            ...
            rdata_index =
              (unsigned)(unsigned long)(pj_ssize_t)rdata->tp_info.tp_data;
        ...
        status = pj_ioqueue_recvfrom(
          key, op_key, rdata->pkt_info.packet, &bytes_read, flags,
                                  ...
                                  pj_ssize_t bytes_sent)
{
    struct udp_transport* tp =
      (struct udp_transport*)pj_ioqueue_get_user_data(key);
                                ...
                                void* token, pjsip_transport_callback callback)
{
    ...
    status = pj_ioqueue_sendto(tp->key, (pj_ioqueue_op_key_t*)&tdata->op_key,
                               tdata->buf.start, &size, 0, rem_addr, addr_len);
...
static pj_status_t udp_destroy(pjsip_transport* transport)
{
    ...
    */
    ...
    for (i = 0; i < 50 && tp->is_closing < 1 + tp->rdata_cnt; ++i) {
        ...
        cnt =
          pj_ioqueue_poll(pjsip_endpt_get_ioqueue(transport->endpt), &timeout);
        if (cnt == 0)
            break;
    }
...
static pj_status_t get_published_name(pj_sock_t sock, char hostbuf[],
                                      int hostbufsz,
                                      pjsip_host_port* bound_name)
{
    ...
    if (tmp_addr.addr.sa_family == pj_AF_INET()) {
        ...
        if (tmp_addr.ipv4.sin_addr.s_addr == PJ_INADDR_ANY) {
            ...
            status = pj_inet_ntop(pj_AF_INET(), &hostip.ipv4.sin_addr, hostbuf,
                                  hostbufsz);
        } else {
        ...
        status =
          pj_inet_ntop(tmp_addr.addr.sa_family, pj_sockaddr_get_addr(&tmp_addr),
                       hostbuf, hostbufsz);
    }
    if (status == PJ_SUCCESS) {
                             ...
                             const pjsip_host_port* a_name)
{
    ...
    pj_addr_str_print(&tp->base.local_name.host, tp->base.local_name.port,
                      pub_addr, sizeof(pub_addr), 1),

      len = pj_ansi_snprintf(tp->base.info, INFO_LEN,
                             "udp %s [published as %s]", local_addr, pub_addr);
    PJ_CHECK_TRUNC_STR(len, tp->base.info, INFO_LEN);
}

/* Set the socket handle of the transport */
static void udp_set_socket(struct udp_transport* tp, pj_sock_t sock,
                           const pjsip_host_port* a_name)
{
...
static pj_status_t start_async_read(struct udp_transport* tp)
{
    ...
    for (i = 0; i < tp->rdata_cnt; ++i) {
        ...
        tp->rdata[i]->pkt_info.src_addr_len =
          sizeof(tp->rdata[i]->pkt_info.src_addr);
        status = pj_ioqueue_recvfrom(
          tp->key, &tp->rdata[i]->tp_info.op_key.op_key,
          tp->rdata[i]->pkt_info.packet, &size, PJ_IOQUEUE_ALWAYS_ASYNC,
...
static pj_status_t transport_attach(pjsip_endpoint* endpt,
                                    pjsip_transport_type_e type, pj_sock_t sock,
                                    const pjsip_host_port* a_name,
                                    unsigned async_cnt,
                                    pjsip_transport** p_transport)
{
    ...
    PJ_ASSERT_RETURN(
      endpt && sock != PJ_INVALID_SOCKET && a_name && async_cnt > 0, PJ_EINVAL);
    ...
    /* Init lock. */
    status =
      pj_lock_create_recursive_mutex(pool, pool->obj_name, &tp->base.lock);
    if (status != PJ_SUCCESS)
        goto on_error;
    ...
    tp->base.key.rem_addr.addr.sa_family =
      (pj_uint16_t)((type & PJSIP_TRANSPORT_IPV6) ? pj_AF_INET6() :
                                                    pj_AF_INET());
    ...
    /* Init local address. */
    status =
      pj_sock_getsockname(sock, &tp->base.local_addr, &tp->base.addr_len);
    if (status != PJ_SUCCESS)
        goto on_error;
    ...
    tp->rdata_cnt = 0;
    tp->rdata = (pjsip_rx_data**)pj_pool_calloc(tp->base.pool, async_cnt,
                                                sizeof(pjsip_rx_data*));
    for (i = 0; i < async_cnt; ++i) {
        pj_pool_t* rdata_pool = pjsip_endpt_create_pool(
          endpt, "rtd%p", PJSIP_POOL_RDATA_LEN, PJSIP_POOL_RDATA_INC);
        if (!rdata_pool) {
    ...
    PJ_LOG(
      4,
      (tp->base.obj_name, "SIP %s started, published address is %s%.*s%s:%d",
       pjsip_transport_get_type_desc((pjsip_transport_type_e)tp->base.key.type),
       ipv6_quoteb, (int)tp->base.local_name.host.slen,
       tp->base.local_name.host.ptr, ipv6_quotee, tp->base.local_name.port));
...
PJ_DEF(pj_status_t)
pjsip_udp_transport_attach(pjsip_endpoint* endpt, pj_sock_t sock,
                           const pjsip_host_port* a_name, unsigned async_cnt,
                           pjsip_transport** p_transport)
{
    return transport_attach(endpt, PJSIP_TRANSPORT_UDP, sock, a_name, async_cnt,
                            p_transport);
}

PJ_DEF(pj_status_t)
pjsip_udp_transport_attach2(pjsip_endpoint* endpt, pjsip_transport_type_e type,
                            pj_sock_t sock, const pjsip_host_port* a_name,
                            unsigned async_cnt, pjsip_transport** p_transport)
{
    return transport_attach(endpt, type, sock, a_name, async_cnt, p_transport);
}
 ...
 */
PJ_DEF(void)
pjsip_udp_transport_cfg_default(pjsip_udp_transport_cfg* cfg, int af)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_udp_transport_start2(pjsip_endpoint* endpt,
                           ...
                           pjsip_transport** p_transport)
{
    ...
    pj_sock_apply_qos2(sock, cfg->qos_type, &cfg->qos_params, 2, THIS_FILE,
                       "SIP UDP transport");
    ...
    if (cfg->addr_name.host.slen == 0) {
         ...
         */
        status =
          get_published_name(sock, addr_buf, sizeof(addr_buf), &addr_name);
        if (status != PJ_SUCCESS) {
            pj_sock_close(sock);
            return status;
        }
    ...
    return pjsip_udp_transport_attach2(endpt, transport_type, sock, &addr_name,
                                       cfg->async_cnt, p_transport);
}
 ...
 */
PJ_DEF(pj_status_t)
pjsip_udp_transport_start(pjsip_endpoint* endpt, const pj_sockaddr_in* local_a,
                          const pjsip_host_port* a_name, unsigned async_cnt,
                          pjsip_transport** p_transport)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_udp_transport_start6(pjsip_endpoint* endpt,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_udp_transport_pause(pjsip_transport* transport, unsigned option)
{
    struct udp_transport* tp;
    unsigned i;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_udp_transport_restart(pjsip_transport* transport, unsigned option,
                            pj_sock_t sock, const pj_sockaddr_in* local,
                            const pjsip_host_port* a_name)
{
...
PJ_DEF(pj_status_t)
pjsip_udp_transport_restart2(pjsip_transport* transport, unsigned option,
                             pj_sock_t sock, const pj_sockaddr* local,
                             const pjsip_host_port* a_name)
{
    struct udp_transport* tp;
    pj_status_t status;
    ...
    if (option & PJSIP_UDP_TRANSPORT_DESTROY_SOCKET) {
        ...
        if (sock == PJ_INVALID_SOCKET) {
            status = create_socket(
              local ? local->addr.sa_family : pj_AF_UNSPEC(), local,
              local ? pj_sockaddr_get_len(local) : 0, &sock);
            if (status != PJ_SUCCESS)
                return status;
        }
        ...
        if (a_name == NULL) {
            status =
              get_published_name(sock, addr_buf, sizeof(addr_buf), &bound_name);
            if (status != PJ_SUCCESS) {
                pj_sock_close(sock);
                return status;
            }
