 ...
 */
static pjsip_authorization_hdr* pjsip_authorization_hdr_clone(
  pj_pool_t* pool, const pjsip_authorization_hdr* hdr);
static pjsip_authorization_hdr* pjsip_authorization_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_authorization_hdr* hdr);
...
PJ_DEF(pjsip_proxy_authorization_hdr*)
...
static int print_digest_credential(pjsip_digest_credential* cred, char* buf,
                                   pj_size_t size)
{
    pj_ssize_t printed;
    char* startbuf = buf;
    ...
    printed =
      pjsip_param_print_on(&cred->other_param, buf, endbuf - buf,
...
static int print_pgp_credential(pjsip_pgp_credential* cred, char* buf,
                                pj_size_t size)
{
    PJ_UNUSED_ARG(cred);
    PJ_UNUSED_ARG(buf);
                                         ...
                                         char* buf, pj_size_t size)
{
    ...
    *buf++ = ' ';
    ...
    if (pj_stricmp(&hdr->scheme, &pjsip_DIGEST_STR) == 0) {
        printed =
          print_digest_credential(&hdr->credential.digest, buf, endbuf - buf);
        ...
        printed =
          print_oauth_credential(&hdr->credential.oauth, buf, endbuf - buf);
    } else {
        pj_assert(0);
        return -1;
    }
...
static pjsip_authorization_hdr* pjsip_authorization_hdr_clone(
  pj_pool_t* pool, const pjsip_authorization_hdr* rhs)
{
    ...
    if (pj_stricmp2(&hdr->scheme, "digest") == 0) {
        pj_strdup(pool, &hdr->credential.digest.username,
                  &rhs->credential.digest.username);
        pj_strdup(pool, &hdr->credential.digest.realm,
                  &rhs->credential.digest.realm);
        pj_strdup(pool, &hdr->credential.digest.nonce,
                  &rhs->credential.digest.nonce);
        pj_strdup(pool, &hdr->credential.digest.uri,
                  &rhs->credential.digest.uri);
        pj_strdup(pool, &hdr->credential.digest.response,
                  &rhs->credential.digest.response);
        pj_strdup(pool, &hdr->credential.digest.algorithm,
                  &rhs->credential.digest.algorithm);
        pj_strdup(pool, &hdr->credential.digest.cnonce,
                  &rhs->credential.digest.cnonce);
        pj_strdup(pool, &hdr->credential.digest.opaque,
                  &rhs->credential.digest.opaque);
        pj_strdup(pool, &hdr->credential.digest.qop,
                  &rhs->credential.digest.qop);
        pj_strdup(pool, &hdr->credential.digest.nc, &rhs->credential.digest.nc);
        pjsip_param_clone(pool, &hdr->credential.digest.other_param,
                          &rhs->credential.digest.other_param);
...
static pjsip_www_authenticate_hdr* pjsip_www_authenticate_hdr_clone(
  pj_pool_t* pool, const pjsip_www_authenticate_hdr* hdr);
static pjsip_www_authenticate_hdr* pjsip_www_authenticate_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_www_authenticate_hdr* hdr);
...
PJ_DEF(pjsip_www_authenticate_hdr*)
...
PJ_DEF(pjsip_proxy_authenticate_hdr*)
...
static int print_digest_challenge(pjsip_digest_challenge* chal, char* buf,
                                  pj_size_t size)
{
    pj_ssize_t printed;
    char* startbuf = buf;
    ...
    printed =
      pjsip_param_print_on(&chal->other_param, buf, endbuf - buf,
                           &pc->pjsip_TOKEN_SPEC, &pc->pjsip_TOKEN_SPEC, ',');
    if (printed < 0)
        return -1;
    buf += printed;
...
static int print_pgp_challenge(pjsip_pgp_challenge* chal, char* buf,
                               pj_size_t size)
{
    PJ_UNUSED_ARG(chal);
    PJ_UNUSED_ARG(buf);
                                            ...
                                            char* buf, pj_size_t size)
{
    ...
    *buf++ = ' ';
    ...
    if (pj_stricmp2(&hdr->scheme, "digest") == 0)
        printed =
          print_digest_challenge(&hdr->challenge.digest, buf, endbuf - buf);
    else if (pj_stricmp2(&hdr->scheme, "pgp") == 0)
...
static pjsip_www_authenticate_hdr* pjsip_www_authenticate_hdr_clone(
  pj_pool_t* pool, const pjsip_www_authenticate_hdr* rhs)
{
    ...
    if (pj_stricmp2(&hdr->scheme, "digest") == 0) {
        pj_strdup(pool, &hdr->challenge.digest.realm,
                  &rhs->challenge.digest.realm);
        pj_strdup(pool, &hdr->challenge.digest.domain,
                  &rhs->challenge.digest.domain);
        pj_strdup(pool, &hdr->challenge.digest.nonce,
                  &rhs->challenge.digest.nonce);
        pj_strdup(pool, &hdr->challenge.digest.opaque,
                  &rhs->challenge.digest.opaque);
        hdr->challenge.digest.stale = rhs->challenge.digest.stale;
        pj_strdup(pool, &hdr->challenge.digest.algorithm,
                  &rhs->challenge.digest.algorithm);
