                ...
                pj_bool_t ig_nf)
{
    ...
    if ((ig_nf & 1) == 0 &&
        pj_list_size(param_list1) != pj_list_size(param_list2))
        return 1;
...
pjsip_param_print_on(const pjsip_param* param_list, char* buf, pj_size_t size,
                     const pj_cis_t* pname_spec, const pj_cis_t* pvalue_spec,
                     int sep)
{
    const pjsip_param* p;
    char* startbuf;
...
static pj_ssize_t pjsip_name_addr_print(pjsip_uri_context_e context,
                                        const pjsip_name_addr* name, char* buf,
                                        pj_size_t size);
...
static pj_ssize_t pjsip_url_print(pjsip_uri_context_e context,
                                  const pjsip_sip_uri* url, char* buf,
                                  pj_size_t size);
...
typedef pj_ssize_t (*P_PRINT_URI)(pjsip_uri_context_e, const void*, char*,
                                  pj_size_t);
typedef int (*P_CMP_URI)(pjsip_uri_context_e, const void*, const void*);
typedef void* (*P_CLONE)(pj_pool_t*, const void*);

static pjsip_uri_vptr sip_url_vptr = { (P_GET_SCHEME)&pjsip_url_get_scheme,
                                       (P_GET_URI)&pjsip_get_uri,
                                       (P_PRINT_URI)&pjsip_url_print,
                                       (P_CMP_URI)&pjsip_url_compare,
                                       (P_CLONE)&pjsip_url_clone };

static pjsip_uri_vptr sips_url_vptr = { (P_GET_SCHEME)&pjsips_url_get_scheme,
                                        (P_GET_URI)&pjsip_get_uri,
                                        (P_PRINT_URI)&pjsip_url_print,
                                        (P_CMP_URI)&pjsip_url_compare,
                                        (P_CLONE)&pjsip_url_clone };
...
static pjsip_uri_vptr name_addr_vptr = {
    ...
    (P_GET_URI)&pjsip_name_addr_get_uri, (P_PRINT_URI)&pjsip_name_addr_print,
    (P_CMP_URI)&pjsip_name_addr_compare, (P_CLONE)&pjsip_name_addr_clone
};
...
PJ_DEF(void) pjsip_sip_uri_set_secure(pjsip_sip_uri* url, pj_bool_t secure)
{
...
PJ_DEF(pjsip_sip_uri*) pjsip_sip_uri_create(pj_pool_t* pool, pj_bool_t secure)
{
                                  ...
                                  pj_size_t size)
{
    ...
    *buf = '\0';
    ...
    if (context == PJSIP_URI_IN_CONTACT_HDR || context == PJSIP_URI_IN_OTHER) {
        printed = (int)pjsip_param_print_on(
          &url->header_param, buf, endbuf - buf, &pc->pjsip_HDR_CHAR_SPEC,
                                        ...
                                        pj_size_t size)
{
    ...
    if (context != PJSIP_URI_IN_REQ_URI) {
        if (name->display.slen) {
            if (endbuf - buf < name->display.slen + 3)
                return -1;
            copy_advance_char_check(buf, '"');
...
PJ_DEF(void)
pjsip_name_addr_assign(pj_pool_t* pool, pjsip_name_addr* dst,
...
static pj_ssize_t other_uri_print(pjsip_uri_context_e context,
                                  const pjsip_other_uri* url, char* buf,
                                  pj_size_t size);
...
static pjsip_uri_vptr other_uri_vptr = { (P_GET_SCHEME)&other_uri_get_scheme,
...
static pj_ssize_t other_uri_print(pjsip_uri_context_e context,
                                  const pjsip_other_uri* uri, char* buf,
                                  pj_size_t size)
{
    char* startbuf = buf;
    char* endbuf = buf + size;
