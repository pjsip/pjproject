...
pjsip_ua_register_dlg(pjsip_user_agent* ua, pjsip_dialog* dlg)
{
    ...
    PJ_ASSERT_RETURN(
      dlg->local.info && dlg->local.info->tag.slen && dlg->local.tag_hval != 0,
      PJ_EBUG);
    ...
    if (dlg->role == PJSIP_ROLE_UAC) {
        ...
        dlg_set = (struct dlg_set*)pj_hash_get_lower(
          mod_ua.dlg_table, dlg->local.info->tag.ptr,
          (unsigned)dlg->local.info->tag.slen, &dlg->local.tag_hval);
        ...
        if (dlg_set) {
            ...
            pj_hash_set_np_lower(mod_ua.dlg_table, dlg_set->ht_key.ptr,
...
pjsip_ua_unregister_dlg(pjsip_user_agent* ua, pjsip_dialog* dlg)
{
    ...
    if (pj_list_empty(&dlg_set->dlg_list)) {
        ...
        pj_hash_set_lower(NULL, mod_ua.dlg_table, dlg_set->ht_key.ptr,
                          (unsigned)dlg_set->ht_key.slen, dlg->local.tag_hval,
                          NULL);
                     ...
                     const pj_str_t* remote_tag, pj_bool_t lock_dialog)
{
    ...
    dlg_set = (struct dlg_set*)pj_hash_get_lower(
      mod_ua.dlg_table, local_tag->ptr, (unsigned)local_tag->slen, NULL);
    if (dlg_set == NULL) {
        /* Not found */
        pj_mutex_unlock(mod_ua.mutex);
        return NULL;
    ...
    if (pj_strcmp(&dlg->call_id->id, call_id) != 0) {
        PJ_LOG(6, (THIS_FILE,
                   "Dialog not found: local and remote tags "
...
static struct dlg_set* find_dlg_set_for_msg(pjsip_rx_data* rdata)
{
    ...
    if (rdata->msg_info.cseq->method.id == PJSIP_CANCEL_METHOD) {
        ...
        /* Lookup the dialog set. */
        dlg_set = (struct dlg_set*)pj_hash_get_lower(mod_ua.dlg_table, tag->ptr,
    ...
    if (dlg == (pjsip_dialog*)&dlg_set->dlg_list) {
        ...
        if (first_dlg->remote.info->tag.slen != 0) {
            ...
            if (rdata->msg_info.msg->line.req.method.id != PJSIP_ACK_METHOD) {
                ...
                PJ_LOG(5, (THIS_FILE, "Unable to find dialog for %s",
                           pjsip_rx_data_get_info(rdata)));
            }
            return PJ_TRUE;
        }
...
retry_on_deadlock:
    ...
    if (tsx) {
        ...
        /* Get the dialog set. */
        dlg_set = (struct dlg_set*)pj_hash_get_lower(
          mod_ua.dlg_table, rdata->msg_info.from->tag.ptr,
          (unsigned)rdata->msg_info.from->tag.slen, NULL);
    ...
    if (rdata->msg_info.cseq->method.id == PJSIP_INVITE_METHOD) {
        ...
        {
            PJ_LOG(5, (THIS_FILE, "Received forked %s for existing dialog %s",
                       pjsip_rx_data_get_info(rdata),
            ...
            if (mod_ua.param.on_dlg_forked) {
                dlg =
                  (*mod_ua.param.on_dlg_forked)(dlg_set->dlg_list.next, rdata);
                if (dlg == NULL) {
                    pj_mutex_unlock(mod_ua.mutex);
                    return PJ_TRUE;
                }
...
static void print_dialog(const char* title, pjsip_dialog* dlg, char* buf,
                         pj_size_t size)
{
    int len;
    ...
    len = pj_ansi_snprintf(
      buf, size, "%s[%s]  %s", title,
      (dlg->state == PJSIP_DIALOG_STATE_NULL ? " - " : "est"), userinfo);
...
PJ_DEF(void) pjsip_ua_dump(pj_bool_t detail)
{
    ...
    if (detail && pj_hash_count(mod_ua.dlg_table)) {
        ...
        for (; it != NULL; it = pj_hash_next(mod_ua.dlg_table, it)) {
            ...
            if (!dlg_set || pj_list_empty(&dlg_set->dlg_list))
                continue;
