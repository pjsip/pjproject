...
static pj_status_t tcp_create(struct tcp_listener* listener, pj_pool_t* pool,
                                       ...
                                       const pjsip_host_port* addr_name)
{
    ...
    pj_ansi_snprintf(listener->factory.obj_name,
                     sizeof(listener->factory.obj_name), "tcptp:%d",
...
static void update_transport_info(struct tcp_listener* listener)
{
    ...
    len = pj_ansi_snprintf(listener->factory.info, INFO_LEN,
                           "tcp %s [published as %s]", local_addr, pub_addr);
                           ...
                           pjsip_tpfactory** p_factory)
{
    ...
    pool = pjsip_endpt_create_pool(endpt, "tcptp", POOL_LIS_INIT, POOL_LIS_INC);
    PJ_ASSERT_RETURN(pool, PJ_ENOMEM);

    listener = PJ_POOL_ZALLOC_T(pool, struct tcp_listener);
    listener->factory.pool = pool;
    listener->factory.type =
      cfg->af == pj_AF_INET() ? PJSIP_TRANSPORT_TCP : PJSIP_TRANSPORT_TCP6;
    ...
    pj_memcpy(&listener->qos_params, &cfg->qos_params, sizeof(cfg->qos_params));
...
static pj_status_t tcp_destroy(pjsip_transport* transport, pj_status_t reason);
                              ...
                              struct tcp_transport** p_tcp)
{
    ...
    if (pool == NULL) {
        pool = pjsip_endpt_create_pool(listener->endpt, "tcp", POOL_TP_INIT,
                                       POOL_TP_INC);
    ...
    pj_ansi_snprintf(
      tcp->base.info, 64, "%s to %s", tcp->base.type_name,
    ...
    if (is_server && listener->initial_timeout) {
        ...
        pjsip_endpt_schedule_timer(listener->endpt, &tcp->initial_timer,
                                   &delay);
...
static void tcp_flush_pending_tx(struct tcp_transport* tcp)
{
    ...
    while (!pj_list_empty(&tcp->delayed_list)) {
        ...
        status =
          pj_activesock_send(tcp->asock, op_key, tdata->buf.start, &size, 0);
...
static void tcp_on_destroy(void* arg)
{
    ...
    if (tcp->base.pool) {
        ...
        if (tcp->close_reason != PJ_SUCCESS) {
            ...
            PJ_LOG(4, (tcp->base.obj_name, "TCP transport destroyed normally"));
        }
...
static pj_status_t tcp_start_read(struct tcp_transport* tcp)
{
    ...
    /* Init rdata */
    pool = pjsip_endpt_create_pool(tcp->base.endpt, "rtd%p",
    ...
    status =
      pj_activesock_start_read2(tcp->asock, tcp->base.pool, size, readbuf, 0);
    if (status != PJ_SUCCESS && status != PJ_EPENDING) {
        PJ_PERROR(
          4, (tcp->base.obj_name, status, "pj_activesock_start_read() error"));
        return status;
    }
                                        ...
                                        pjsip_transport** p_transport)
{
    ...
    /* Create socket */
    status =
      pj_sock_socket(rem_addr->addr.sa_family, pj_SOCK_STREAM(), 0, &sock);
    if (status != PJ_SUCCESS)
        return status;

    /* Apply QoS, if specified */
    status =
      pj_sock_apply_qos2(sock, listener->qos_type, &listener->qos_params, 2,
                         listener->factory.obj_name, "outgoing SIP TCP socket");
    ...
    status =
      tcp_create(listener, NULL, sock, PJ_FALSE, &local_addr, rem_addr, &tcp);
    if (status != PJ_SUCCESS)
        return status;
    ...
    if (tcp->has_pending_connect) {
        ...
        PJ_LOG(4,
               (tcp->base.obj_name, "TCP transport %s is connecting to %s...",
 ...
 * has completed.
 */
static pj_bool_t on_accept_complete(pj_activesock_t* asock, pj_sock_t sock,
                                    const pj_sockaddr_t* src_addr,
                                    int src_addr_len)
{
    struct tcp_listener* listener;
    ...
    PJ_LOG(4, (listener->factory.obj_name,
               ...
               pj_addr_str_print(&listener->factory.addr_name.host,
                                 ...
                                 sizeof(addr_buf), 1),
               pj_sockaddr_print(src_addr, addr, sizeof(addr), 3), sock));

    /* Apply QoS, if specified */
    status =
      pj_sock_apply_qos2(sock, listener->qos_type, &listener->qos_params, 2,
                         listener->factory.obj_name, "incoming SIP TCP socket");
     ...
     */
    status = tcp_create(listener, NULL, sock, PJ_TRUE, &tmp_dst_addr,
                        &tmp_src_addr, &tcp);
    if (status == PJ_SUCCESS) {
        ...
        if (status != PJ_SUCCESS) {
            ...
            if (pjsip_cfg()->tcp.keep_alive_interval) {
                ...
                pjsip_endpt_schedule_timer(listener->endpt, &tcp->ka_timer,
                                           &delay);
                              ...
                              pj_ssize_t bytes_sent)
{
    ...
    if (bytes_sent <= 0) {
        ...
        PJ_LOG(5,
               (tcp->base.obj_name, "TCP send() error, sent=%d", bytes_sent));
                                ...
                                void* token, pjsip_transport_callback callback)
{
    ...
    if (!delayed) {
        ...
        size = tdata->buf.cur - tdata->buf.start;
        status =
          pj_activesock_send(tcp->asock, (pj_ioqueue_op_key_t*)&tdata->op_key,
        ...
        if (status != PJ_EPENDING) {
            ...
            if (size <= 0) {
                PJ_LOG(5,
                       (tcp->base.obj_name, "TCP send() error, sent=%d", size));
...
static pj_bool_t on_connect_complete(pj_activesock_t* asock, pj_status_t status)
{
    ...
    PJ_LOG(4, (tcp->base.obj_name, "TCP transport %s is connected to %s",
               pj_addr_str_print(&tcp->base.local_name.host,
                                 tcp->base.local_name.port, local_addr_buf,
                                 sizeof(local_addr_buf), 1),
               pj_addr_str_print(&tcp->base.remote_name.host,
    ...
    if (pj_sock_getsockname(tcp->sock, &addr, &addrlen) == PJ_SUCCESS) {
        ...
        if (pj_sockaddr_has_addr(&addr) && pj_sockaddr_cmp(&addr, tp_addr) != 0)
        {
    ...
    if (pjsip_cfg()->tcp.keep_alive_interval) {
        pj_time_val delay = { 0 };
        delay.sec = pjsip_cfg()->tcp.keep_alive_interval;
        pjsip_endpt_schedule_timer(tcp->base.endpt, &tcp->ka_timer, &delay);
        tcp->ka_timer.id = PJ_TRUE;
        pj_gettimeofday(&tcp->last_activity);
    }
...
static void tcp_keep_alive_timer(pj_timer_heap_t* th, pj_timer_entry* e)
{
    ...
    PJ_LOG(5, (tcp->base.obj_name, "Sending %d byte(s) keep-alive to %s",
               (int)tcp->ka_pkt.slen,
               pj_addr_str_print(&tcp->base.remote_name.host,
                                 tcp->base.remote_name.port, addr, sizeof(addr),
                                 1)));
    ...
    if (status != PJ_SUCCESS && status != PJ_EPENDING) {
        tcp_perror(tcp->base.obj_name, "Error sending keep-alive packet",
                   status);
        tcp_init_shutdown(tcp, status);
        return;
    }
                              ...
                              const pjsip_host_port* a_name)
{
    ...
    status =
      pj_sock_apply_qos2(sock, listener->qos_type, &listener->qos_params, 2,
                         listener->factory.obj_name, "SIP TCP listener socket");
    ...
    status =
      pj_sock_getsockname(sock, &listener->factory.local_addr, &addr_len);
    if (status != PJ_SUCCESS)
        goto on_error;
    ...
    status =
      pj_activesock_create(listener->factory.pool, sock, pj_SOCK_STREAM(),
                           &asock_cfg, pjsip_endpt_get_ioqueue(listener->endpt),
                           &listener_cb, listener, &listener->asock);

    /* Start pending accept() operations */
    status =
      pj_activesock_start_accept(listener->asock, listener->factory.pool);
