                                                ...
                                                pjsip_tx_data* tdata)
{
    ...
    pool =
      pjsip_endpt_create_pool(loop->base.endpt, "rdata", PJSIP_POOL_RDATA_LEN,
...
static pj_status_t add_notification(
  struct loop_transport* loop, pjsip_tx_data* tdata, pj_ssize_t sent,
  void* token, void (*callback)(pjsip_transport*, void*, pj_ssize_t))
{
...
static pj_status_t loop_send_msg(pjsip_transport* tp, pjsip_tx_data* tdata,
                                 const pj_sockaddr_t* rem_addr, int addr_len,
                                 void* token, pjsip_transport_callback cb)
{
    ...
    PJ_ASSERT_RETURN(tp && (tp->key.type == PJSIP_TRANSPORT_LOOP ||
                            tp->key.type == PJSIP_TRANSPORT_LOOP_DGRAM),
                     PJ_EINVAL);
    ...
    if (loop->recv_delay == 0) {
        ...
        size_eaten =
          pjsip_tpmgr_receive_packet(loop->base.tpmgr, &recv_pkt->rdata);
    ...
    if (loop->send_delay != 0) {
        add_notification(loop, tdata, tdata->buf.cur - tdata->buf.start, token,
                         cb);
        return PJ_EPENDING;
    } else {
        return PJ_SUCCESS;
...
static pj_status_t loop_destroy(pjsip_transport* tp)
{
    ...
    while (!pj_list_empty(&loop->recv_list)) {
        ...
        pj_list_erase(node);
        pjsip_endpt_release_pool(loop->base.endpt, node->rdata.tp_info.pool);
    }
...
static int loop_transport_worker_thread(void* arg)
{
    ...
    while (!loop->thread_quit_flag) {
        ...
        while (!pj_list_empty(&r)) {
            ...
            size_eaten =
              pjsip_tpmgr_receive_packet(loop->base.tpmgr, &node->rdata);
...
pjsip_loop_start(pjsip_endpoint* endpt, pjsip_transport** transport)
{
    ...
    pj_ansi_snprintf(loop->base.obj_name, sizeof(loop->base.obj_name), "loop%p",
                     loop);
    loop->base.pool = pool;
    ...
    status = pj_thread_create(pool, "loop", &loop_transport_worker_thread, loop,
                              0, PJ_THREAD_SUSPENDED, &loop->thread);
    if (status != PJ_SUCCESS)
        goto on_error;
