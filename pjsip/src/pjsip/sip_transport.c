...
static void tp_state_callback(pjsip_transport* tp, pjsip_transport_state state,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_transport_register_type(unsigned tp_flag, const char* tp_name,
                              int def_port, int* p_tp_type)
{
    unsigned i;
    ...
    PJ_ASSERT_RETURN(
      pj_ansi_strlen(tp_name) < PJ_ARRAY_SIZE(transport_names[0].name_buf),
      PJ_ENAMETOOLONG);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tx_data_create(pjsip_tpmgr* mgr, pjsip_tx_data** p_tdata)
{
    pj_pool_t* pool;
    pjsip_tx_data* tdata;
    pj_status_t status;
    ...
    pool = pjsip_endpt_create_pool(mgr->endpt, "tdta%p", PJSIP_POOL_LEN_TDATA,
...
PJ_DEF(pj_status_t) pjsip_tx_data_encode(pjsip_tx_data* tdata)
{
    ...
    if (tdata->buf.start == NULL) {
        ...
        {
            tdata->buf.start =
              (char*)pj_pool_alloc(tdata->pool, PJSIP_MAX_PKT_LEN);
        }
        PJ_CATCH_ANY
        {
            return PJ_ENOMEM;
                          ...
                          const pjsip_msg* msg)
{
    ...
    if (msg->type == PJSIP_REQUEST_MSG) {
        len = pj_ansi_snprintf(
          info_buf, sizeof(info_buf), "Request msg %.*s/cseq=%d (%s)",
          (int)msg->line.req.method.name.slen, msg->line.req.method.name.ptr,
          cseq->cseq, obj_name);
    } else {
        len = pj_ansi_snprintf(
          info_buf, sizeof(info_buf), "Response msg %d/%.*s/cseq=%d (%s)",
          msg->line.status.code, (int)cseq->method.name.slen,
          cseq->method.name.ptr, cseq->cseq, obj_name);
    }
...
PJ_DEF(pj_status_t)
pjsip_tx_data_set_transport(pjsip_tx_data* tdata, const pjsip_tpselector* sel)
{
...
/* Clone pjsip_tx_data. */
PJ_DEF(pj_status_t)
pjsip_tx_data_clone(const pjsip_tx_data* src, unsigned flags,
...
PJ_DEF(char*) pjsip_rx_data_get_info(pjsip_rx_data* rdata)
{
    ...
    rdata->msg_info.info =
      get_msg_info(rdata->tp_info.pool, obj_name, rdata->msg_info.msg);
    return rdata->msg_info.info;
}

/* Clone pjsip_rx_data. */
PJ_DEF(pj_status_t)
pjsip_rx_data_clone(const pjsip_rx_data* src, unsigned flags,
                    pjsip_rx_data** p_rdata)
{
    ...
    pool = pj_pool_create(src->tp_info.pool->factory, "rtd%p",
                          PJSIP_POOL_RDATA_LEN, PJSIP_POOL_RDATA_INC, NULL);
    if (!pool)
        return PJ_ENOMEM;
    ...
    dst->msg_info.msg_buf =
      dst->pkt_info.packet + (src->msg_info.msg_buf - src->pkt_info.packet);
...
static void transport_send_callback(pjsip_transport* transport, void* token,
                                    pj_ssize_t size)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_transport_send(pjsip_transport* tr, pjsip_tx_data* tdata,
                     const pj_sockaddr_t* addr, int addr_len, void* token,
                     pjsip_tp_send_callback cb)
{
    ...
    pj_inet_ntop(((pj_sockaddr*)addr)->addr.sa_family,
                 pj_sockaddr_get_addr(addr), tdata->tp_info.dst_name,
...
/* send_raw() callback */
static void send_raw_callback(pjsip_transport* transport, void* token,
                              pj_ssize_t size)
{
    pjsip_tx_data* tdata = (pjsip_tx_data*)token;
...
/* Send raw data */
PJ_DEF(pj_status_t)
pjsip_tpmgr_send_raw(pjsip_tpmgr* mgr, pjsip_transport_type_e tp_type,
                     const pjsip_tpselector* sel, pjsip_tx_data* tdata,
                     const void* raw_data, pj_size_t data_len,
                     const pj_sockaddr_t* addr, int addr_len, void* token,
                     pjsip_tp_send_callback cb)
{
    pjsip_transport* tr;
    ...
    status =
      pjsip_tpmgr_acquire_transport(mgr, tp_type, addr, addr_len, sel, &tr);
    if (status != PJ_SUCCESS)
        return status;
...
static pj_bool_t is_transport_valid(pjsip_transport* tp, pjsip_tpmgr* tpmgr,
                                    const pjsip_transport_key* key, int key_len)
{
    transport* tp_entry;
    ...
    if (pj_atomic_dec_and_get(tp->ref_cnt) == 0) {
            ...
            !tp->is_destroying && pj_atomic_get(tp->ref_cnt) == 0)
        {
            ...
            pjsip_endpt_schedule_timer_w_grp_lock(
              tp->tpmgr->endpt, &tp->idle_timer, &delay, PJ_TRUE, tp->grp_lock);
        }
        pj_lock_release(tpmgr->lock);
    }
 ...
 */
PJ_DEF(pj_status_t)
pjsip_transport_register(pjsip_tpmgr* mgr, pjsip_transport* tp)
{
    int key_len;
    pj_uint32_t hval;
    ...
    if (tp_ref) {
        ...
        TRACE_((THIS_FILE,
                "Remote address already registered, "
    ...
    TRACE_((THIS_FILE, "Transport %s registered: type=%s, remote=%s:%d",
            tp->obj_name, pjsip_transport_get_type_name(tp->key.type),
...
static pj_status_t destroy_transport(pjsip_tpmgr* mgr, pjsip_transport* tp)
{
    ...
    if (entry) {
            ...
            if (tp_iter->tp == tp) {
                ...
                if (tp_iter == tp_ref) {
                    ...
                    if (tp_ref->next != tp_ref) {
                        ...
                        TRACE_((THIS_FILE,
                                "Hash entry updated after "
                                "transport %d being destroyed",
                                tp->obj_name));
                    } else {
                        TRACE_((THIS_FILE,
                                "Hash entry deleted after "
                                "transport %d being destroyed",
                                tp->obj_name));
                    }
                }
        ...
        if (tp_iter->tp != tp) {
            PJ_LOG(3, (THIS_FILE,
                       "Warning: transport %s being destroyed is "
                       "not registered",
                       tp->obj_name));
        }
    } else {
 ...
 */
PJ_DEF(pj_status_t)
pjsip_transport_shutdown2(pjsip_transport* tp, pj_bool_t force)
{
    pjsip_tpmgr* mgr;
    pj_status_t status;
    pjsip_tp_state_callback state_cb;

    PJ_LOG(4, (THIS_FILE, "Transport %s shutting down, force=%d", tp->obj_name,
               force));
    ...
    if (state_cb) {
        ...
        (*state_cb)(
          tp, (force ? PJSIP_TP_STATE_DISCONNECTED : PJSIP_TP_STATE_SHUTDOWN),
          &state_info);
    }
...
PJ_DEF(pj_status_t)
pjsip_tpmgr_register_tpfactory(pjsip_tpmgr* mgr, pjsip_tpfactory* tpf)
{
    pjsip_tpfactory* p;
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tpmgr_unregister_tpfactory(pjsip_tpmgr* mgr, pjsip_tpfactory* tpf)
{
    pj_lock_acquire(mgr->lock);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tpmgr_create(pj_pool_t* pool, pjsip_endpoint* endpt,
                   pjsip_rx_callback rx_cb, pjsip_tx_callback tx_cb,
                   pjsip_tpmgr** p_mgr)
{
    pjsip_tpmgr* mgr;
    pj_status_t status;
    ...
    mgr_pool = pjsip_endpt_create_pool(endpt, "tpmgr", TPMGR_POOL_INIT_SIZE,
                                       TPMGR_POOL_INC_SIZE);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tpmgr_find_local_addr2(pjsip_tpmgr* tpmgr, pj_pool_t* pool,
...
PJ_DEF(pj_status_t)
pjsip_tpmgr_find_local_addr(pjsip_tpmgr* tpmgr, pj_pool_t* pool,
                            pjsip_transport_type_e type,
                            const pjsip_tpselector* sel, pj_str_t* ip_addr,
                            int* port)
{
    pjsip_tpmgr_fla2_param prm;
    pj_status_t status;
 ...
 */
PJ_DEF(pj_ssize_t)
pjsip_tpmgr_receive_packet(pjsip_tpmgr* mgr, pjsip_rx_data* rdata)
{
    ...
    while (remaining_len > 0) {
        ...
        if (msg == NULL || !pj_list_empty(&rdata->msg_info.parse_err)) {
            ...
            while (err != &rdata->msg_info.parse_err) {
                int len;
                len = pj_ansi_snprintf(
                  tmp.ptr + tmp.slen, sizeof(buf) - tmp.slen,
                  ": %s exception when parsing '%.*s' "
                  "header on line %d col %d",
                  pj_exception_id_name(err->except_code), (int)err->hname.slen,
                  err->hname.ptr, err->line, err->col);
            ...
            if (tmp.slen) {
                PJ_LOG(1,
                       (THIS_FILE,
                        ...
                        "%.*s\n"
                        "-- end of packet.",
                        msg_fragment_size, rdata->tp_info.transport->type_name,
                        rdata->pkt_info.src_name, rdata->pkt_info.src_port,
                        (int)tmp.slen, tmp.ptr, (int)msg_fragment_size,
        ...
        if (rdata->msg_info.cid == NULL || rdata->msg_info.cid->id.slen == 0 ||
            rdata->msg_info.from == NULL || rdata->msg_info.to == NULL ||
            rdata->msg_info.via == NULL || rdata->msg_info.cseq == NULL)
        {
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tpmgr_acquire_transport(pjsip_tpmgr* mgr, pjsip_transport_type_e type,
                              const pj_sockaddr_t* remote, int addr_len,
                              const pjsip_tpselector* sel, pjsip_transport** tp)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tpmgr_acquire_transport2(pjsip_tpmgr* mgr, pjsip_transport_type_e type,
                               const pj_sockaddr_t* remote, int addr_len,
                               const pjsip_tpselector* sel,
                               pjsip_tx_data* tdata, pjsip_transport** tp)
{
    pjsip_tpfactory* factory;
    pj_status_t status;

    TRACE_((THIS_FILE, "Acquiring transport type=%s, sel=%s remote=%s:%d",
            pjsip_transport_get_type_name(type), print_tpsel_info(sel),
            addr_string(remote), pj_sockaddr_get_port(remote)));
        ...
        if (sel && sel->type == PJSIP_TPSELECTOR_LISTENER && sel->u.listener) {
            ...
            */
            ...
            if (!pjsip_tpmgr_is_tpfactory_valid(mgr, factory)) {
                pj_lock_release(mgr->lock);
                PJ_LOG(3, (THIS_FILE,
                           "Specified factory for creating "
    ...
    if (factory->create_transport2) {
        ...
        status = factory->create_transport(
          factory, mgr, mgr->endpt, (const pj_sockaddr*)remote, addr_len, tp);
    }
    if (status == PJ_SUCCESS) {
...
#    endif
    ...
    factory = mgr->factory_list.next;
    while (factory != &mgr->factory_list) {
        PJ_LOG(3, (THIS_FILE, "  %s %s:%.*s:%d", factory->obj_name,
                   factory->type_name, (int)factory->addr_name.host.slen,
                   factory->addr_name.host.ptr, (int)factory->addr_name.port));
        factory = factory->next;
    }
        ...
        do {
            ...
            if (tp_entry) {
                ...
                do {
                    ...
                    PJ_LOG(3,
                           (THIS_FILE, "  %s %s%s%s%s(refcnt=%d%s)",
                            tp_ref->obj_name, tp_ref->info,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tpmgr_set_state_cb(pjsip_tpmgr* mgr, pjsip_tp_state_callback cb)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_transport_add_state_listener(pjsip_transport* tp,
                                   pjsip_tp_state_callback cb, void* user_data,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_transport_remove_state_listener(pjsip_transport* tp,
                                      pjsip_tp_state_listener_key* key,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tpmgr_set_drop_data_cb(pjsip_tpmgr* mgr, pjsip_tp_on_rx_dropped_cb cb)
{
    PJ_ASSERT_RETURN(mgr, PJ_EINVAL);
