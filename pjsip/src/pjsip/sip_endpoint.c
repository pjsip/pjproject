 ...
 * Prototypes.
 */
static void endpt_on_rx_msg(pjsip_endpoint*, pj_status_t, pjsip_rx_data*);
static pj_status_t endpt_on_tx_msg(pjsip_endpoint* endpt, pjsip_tx_data* tdata);
static pj_status_t unload_module(pjsip_endpoint* endpt, pjsip_module* mod);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_endpt_register_module(pjsip_endpoint* endpt, pjsip_module* mod)
{
    ...
    PJ_ASSERT_ON_FAIL(
      pj_list_search(&endpt->module_list, &mod->name, &cmp_mod_name) == NULL, {
          status = PJ_EEXISTS;
          goto on_return;
      });
    ...
    PJ_LOG(4, (THIS_FILE, "Module \"%.*s\" registered", (int)mod->name.slen,
               mod->name.ptr));
 ...
 */
PJ_DEF(pj_status_t)
pjsip_endpt_unregister_module(pjsip_endpoint* endpt, pjsip_module* mod)
{
    pj_status_t status;
...
static pj_status_t unload_module(pjsip_endpoint* endpt, pjsip_module* mod)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Module \"%.*s\" unregistered", (int)mod->name.slen,
               mod->name.ptr));
                           ...
                           const pj_str_t* hname, const pj_str_t* token)
{
    ...
    hdr = (const pjsip_generic_array_hdr*)pjsip_endpt_get_capability(
      endpt, htype, hname);
    if (!hdr)
        return PJ_FALSE;
                           ...
                           const pj_str_t tags[])
{
    ...
    PJ_ASSERT_RETURN(htype == PJSIP_H_ACCEPT || htype == PJSIP_H_ALLOW ||
                       htype == PJSIP_H_SUPPORTED,
                     PJ_EINVAL);

    /* Find the header. */
    hdr =
      (pjsip_generic_array_hdr*)pjsip_endpt_get_capability(endpt, htype, hname);
                   ...
                   pjsip_endpoint** p_endpt)
{
    ...
    *p_endpt = NULL;
    ...
    /* Create pool */
    pool = pj_pool_create(pf, "pept%p", PJSIP_POOL_LEN_ENDPT,
                          PJSIP_POOL_INC_ENDPT, &pool_callback);
    if (!pool)
        return PJ_ENOMEM;
    ...
    status = pjsip_tpmgr_create(endpt->pool, endpt, &endpt_on_rx_msg,
                                &endpt_on_tx_msg, &endpt->transport_mgr);
    if (status != PJ_SUCCESS) {
        goto on_error;
    }
    ...
    if (status != PJ_SUCCESS) {
        PJ_PERROR(4, (THIS_FILE, status, "Error creating resolver instance"));
        goto on_error;
    }
    ...
    mf_hdr = pjsip_max_fwd_hdr_create(endpt->pool, PJSIP_MAX_FORWARDS_VALUE);
                        ...
                        pj_size_t initial, pj_size_t increment)
{
    ...
    /* Create pool */
    pool =
      pj_pool_create(endpt->pf, pool_name, initial, increment, &pool_callback);
...
pjsip_endpt_handle_events2(pjsip_endpoint* endpt,
                           const pj_time_val* max_timeout, unsigned* p_count)
{
    enum
    {
        MAX_TIMEOUT_ON_ERR = 10
    };
 ...
 */
PJ_DEF(pj_status_t)
pjsip_endpt_handle_events(pjsip_endpoint* endpt, const pj_time_val* max_timeout)
{
                               ...
                               int src_line)
{
    ...
    return pj_timer_heap_schedule_dbg(endpt->timer_heap, entry, delay, src_file,
                                      src_line);
}
#else
PJ_DEF(pj_status_t)
                                          ...
                                          const char* src_file, int src_line)
{
    ...
    return pj_timer_heap_schedule_w_grp_lock_dbg(
      endpt->timer_heap, entry, delay, id_val, grp_lock, src_file, src_line);
}
#else
PJ_DEF(pj_status_t)
                                      ...
                                      pj_grp_lock_t* grp_lock)
{
    PJ_LOG(6, (THIS_FILE,
               ...
               "(entry=%p, delay=%u.%u, grp_lock=%p)",
               entry, delay->sec, delay->msec, grp_lock));
    return pj_timer_heap_schedule_w_grp_lock(endpt->timer_heap, entry, delay,
                                             id_val, grp_lock);
}
#endif
                            ...
                            pjsip_process_rdata_param* p, pj_bool_t* p_handled)
{
    ...
    if (p->start_mod) {
        mod =
          (pjsip_module*)pj_list_find_node(&endpt->module_list, p->start_mod);
        if (!mod) {
            status = PJ_ENOTFOUND;
            goto on_return;
        }
 ...
 */
static void endpt_on_rx_msg(pjsip_endpoint* endpt, pj_status_t status,
                            pjsip_rx_data* rdata)
{
    ...
    if (status != PJ_SUCCESS) {
        ...
        PJ_LOG(
          1, (THIS_FILE,
              ...
              "%.*s\n"
              "-- end of packet.",
              rdata->pkt_info.src_name, rdata->pkt_info.src_port, errmsg, info,
              status, (int)rdata->msg_info.len, rdata->msg_info.msg_buf));
        return;
    }
    ...
    if (msg->type == PJSIP_RESPONSE_MSG) {
        ...
        if (pj_strcmp(&rdata->msg_info.via->sent_by.host, local_addr) != 0) {
            ...
            if (rdata->msg_info.via->rport_param !=
                ...
                PJ_LOG(
                  4, (THIS_FILE,
                      "Message %s from %s has mismatch port in "
                      "sent-by but the rport parameter is "
                      "correct",
                      pjsip_rx_data_get_info(rdata), rdata->pkt_info.src_name));
            }
        }
        ...
        if (mismatch) {
            ...
            PJ_LOG(4, (THIS_FILE,
                       "Dropping response %s from %s:%d because "
                       "sent-by is mismatch",
                       pjsip_rx_data_get_info(rdata), rdata->pkt_info.src_name,
    ...
    if (!handled) {
        PJ_LOG(4, (THIS_FILE,
                   "%s from %s:%d was dropped/unhandled by"
                   " any modules",
                   pjsip_rx_data_get_info(rdata), rdata->pkt_info.src_name,
                   rdata->pkt_info.src_port));
    }
...
{
    return pjsip_tpmgr_acquire_transport(endpt->transport_mgr, type, remote,
                                         addr_len, sel, transport);
}
                      ...
                      pj_status_t error_code, const char* format, ...)
{
    ...
    if (len < (int)sizeof(newformat) - 30) {
        ...
        errstr =
          pj_strerror(error_code, newformat + len, sizeof(newformat) - len - 1);
