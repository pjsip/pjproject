                               ...
                               pjsip_digest_credential* auth)
{
    ...
    if (chal->algorithm.slen == 0 || pj_stricmp2(&chal->algorithm, "md5") == 0)
    {
        ...
        status = pjsip_auth_create_digest(
          &auth->response, &auth->nonce, &auth->nc, &auth->cnonce, &auth->qop,
    ...
    if (aka_version == 1) {
        ...
        status = pjsip_auth_create_digest(
          &auth->response, &chal->nonce, &auth->nc, &auth->cnonce, &auth->qop,
        ...
        pj_memcpy(resikck.ptr + PJSIP_AKA_RESLEN + PJSIP_AKA_IKLEN, ck,
                  PJSIP_AKA_CKLEN);

        pj_hmac_md5((const pj_uint8_t*)AKAv2_Passwd.ptr, AKAv2_Passwd.slen,
                    (const pj_uint8_t*)resikck.ptr, resikck.slen, hmac_digest);
