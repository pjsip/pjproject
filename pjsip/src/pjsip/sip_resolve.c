...
static void srv_resolver_cb(void* user_data, pj_status_t status,
                            const pj_dns_srv_record* rec);
static void dns_a_callback(void* user_data, pj_status_t status,
                           pj_dns_parsed_packet* response);
static void dns_aaaa_callback(void* user_data, pj_status_t status,
                              pj_dns_parsed_packet* response);
              ...
              pjsip_resolver_callback* cb)
{
    ...
    if (ip_addr_ver || resolver->res == NULL) {
        ...
        if (ip_addr_ver != 0) {
            ...
            if (ip_addr_ver == 4) {
                if (af == pj_AF_INET6()) {
                    ...
                    status2 = pj_getaddrinfo(pj_AF_INET6(), &target->addr.host,
                                             &count, ai);
                ...
                pj_sockaddr_init(pj_AF_INET6(), &svr_addr.entry[0].addr, NULL,
                                 0);
            ...
            PJ_LOG(5, (THIS_FILE,
                       ...
                       target->addr.host.slen, target->addr.host.ptr,
                       target->addr.port,
        ...
        for (i = 0; i < svr_addr.count; i++) {
            ...
            if (target->addr.port == 0) {
                srv_port =
                  (pj_uint16_t)pjsip_transport_get_default_port_for_type(type);
            } else {
    ...
    PJ_LOG(5, (query->objname,
               ...
               (int)query->naptr[0].res_type.slen, query->naptr[0].res_type.ptr,
               (int)query->naptr[0].name.slen, query->naptr[0].name.ptr,
               pjsip_transport_get_type_name(target->type), target->addr.port));
    ...
    if (query->query_type == PJ_DNS_TYPE_SRV) {
        ...
        status = pj_dns_srv_resolve(
          &query->naptr[0].name, &query->naptr[0].res_type, query->req.def_port,
          pool, resolver->res, opt, query, &srv_resolver_cb, NULL);
        ...
        if (af != pj_AF_INET6()) {
            ...
            status = pj_dns_resolver_start_query(
              resolver->res, &query->naptr[0].name, PJ_DNS_TYPE_A, 0,
              &dns_a_callback, query, &query->object);
        }

        /* Resolve DNS AAAA record if address family is not fixed to IPv4 */
        if (af != pj_AF_INET() && status == PJ_SUCCESS) {
            status = pj_dns_resolver_start_query(
              resolver->res, &query->naptr[0].name, PJ_DNS_TYPE_AAAA, 0,
              &dns_aaaa_callback, query, &query->object6);
        }
                           ...
                           pj_dns_parsed_packet* pkt)
{
    ...
    if (status == PJ_SUCCESS) {
        ...
        for (i = 0;
             i < rec.addr_count && srv->count < PJSIP_MAX_RESOLVED_ADDRESSES;
             ++i)
        {
            ...
            pj_sockaddr_in_init(&srv->entry[srv->count].addr.ipv4, 0,
    ...
    if (status != PJ_SUCCESS) {
        PJ_PERROR(4,
                  (query->objname, status, "DNS A record resolution failed"));
                              ...
                              pj_dns_parsed_packet* pkt)
{
    ...
    if (status == PJ_SUCCESS) {
             ...
             ++i)
        {
            ...
            srv->entry[srv->count].type =
              query->naptr[0].type | PJSIP_TRANSPORT_IPV6;
            srv->entry[srv->count].priority = 0;
            srv->entry[srv->count].weight = 0;
            srv->entry[srv->count].addr_len = sizeof(pj_sockaddr_in6);
            pj_sockaddr_init(pj_AF_INET6(), &srv->entry[srv->count].addr, 0,
                             (pj_uint16_t)query->req.def_port);
    ...
    if (status != PJ_SUCCESS) {
        PJ_PERROR(
          4, (query->objname, status, "DNS AAAA record resolution failed"));
                            ...
                            const pj_dns_srv_record* rec)
{
    ...
    if (status != PJ_SUCCESS) {
        PJ_PERROR(
          4, (query->objname, status, "DNS A/AAAA record resolution failed"));
    ...
    for (i = 0; i < rec->count; ++i) {
        ...
        for (j = 0;
             j < s->addr_count && srv.count < PJSIP_MAX_RESOLVED_ADDRESSES; ++j)
        {
            srv.entry[srv.count].type = query->naptr[0].type;
            srv.entry[srv.count].priority = rec->entry[i].priority;
            srv.entry[srv.count].weight = rec->entry[i].weight;
            pj_sockaddr_init(s->addr[j].af, &srv.entry[srv.count].addr, 0,
