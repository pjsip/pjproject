                                     ...
                                     const pjsip_cred_info* cred_info)
{
    if (pj_stricmp(&hdr->scheme, &pjsip_DIGEST_STR) == 0) {
        ...
        status = pjsip_auth_create_digest(
          &digest, &hdr->credential.digest.nonce, &hdr->credential.digest.nc,
          &hdr->credential.digest.cnonce, &hdr->credential.digest.qop,
          &hdr->credential.digest.uri, &cred_info->realm, cred_info, method);
                      ...
                      int* status_code)
{
    ...
    htype =
      auth_srv->is_proxy ? PJSIP_H_PROXY_AUTHORIZATION : PJSIP_H_AUTHORIZATION;
    ...
    *status_code = 200;
    ...
    while (h_auth) {
        ...
        h_auth =
          (pjsip_authorization_hdr*)pjsip_msg_find_hdr(msg, htype, h_auth);
    }
