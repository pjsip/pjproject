...
static int multipart_print_body(struct pjsip_msg_body* msg_body, char* buf,
                                pj_size_t size)
{
...
#define SIZE_LEFT() (end - p)
    ...
    while (part != &m_data->part_head) {
        ...
        *p++ = 10;
        ...
        while (hdr != &part->hdr) {
            int printed =
              pjsip_hdr_print_on((pjsip_hdr*)hdr, p, SIZE_LEFT() - 2);
            if (printed < 0)
                return -1;
            p += printed;
            *p++ = '\r';
                       ...
                       const pj_str_t* boundary)
{
    ...
    if (ctype && ctype->type.slen) {
        ...
        pjsip_media_type_init(&body->content_type, &STR_MULTIPART, &STR_MIXED);
    }
...
pjsip_multipart_clone_part(pj_pool_t* pool, const pjsip_multipart_part* src)
{
                              ...
                              pjsip_multipart_part* part)
{
    ...
    PJ_ASSERT_RETURN(pj_list_find_node(&m_data->part_head, part) != NULL, NULL);
...
#define SEPARATOR_LEN      2
    ...
    while (part != &m_data->part_head) {
        found_hdr = NULL;
        while ((found_hdr = pjsip_hdr_find_by_name(
                  &part->hdr, hdr_name,
                  (found_hdr ? found_hdr->next : NULL))) != NULL)
        {
            found_hdr_str.slen =
              pjsip_hdr_print_on((void*)found_hdr, found_hdr_str.ptr, buf_size);
            /*
                                    ...
                                    const pjsip_multipart_part* start)
{
    ...
    search_buf.slen =
      pjsip_hdr_print_on(search_hdr, search_buf.ptr, PJSIP_MAX_URL_SIZE - 1);
    ...
    return pjsip_multipart_find_part_by_header_str(pool, mp, &search_hdr->name,
                                                   &search_buf, start);
}
...
static pj_str_t cid_uri_to_hdr_value(pj_pool_t* pool, pj_str_t* cid_uri)
{
    ...
    pj_size_t alloc_len =
      cid_len + 2 /* for the leading and trailing angle brackets */;
...
static pjsip_multipart_part* parse_multipart_part(pj_pool_t* pool, char* start,
                                                  pj_size_t len,
                                                  const pjsip_media_type* pct)
{
    ...
    if (end_hdr - start > 0) {
        ...
        status =
          pjsip_parse_headers(pool, start, end_hdr - start, &part->hdr, 0);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(2, (THIS_FILE, status,
                          "Warning: error parsing multipart"
                          " header"));
        }
                      ...
                      const pjsip_media_type* ctype, unsigned options)
{
    ...
    if (!boundary.slen) {
        ...
        PJ_LOG(4, (THIS_FILE,
                   "Warning: boundary parameter not found or "
        ...
        if (p == end) {
             ...
             */
            PJ_LOG(4, (THIS_FILE,
                       "Error: multipart boundary not specified and"
        ...
        TRACE_((THIS_FILE, "Boundary is calculated: '%.*s'", (int)boundary.slen,
                boundary.ptr));
    }
    ...
    for (;;) {
        ...
        while (curptr != endptr && IS_SPACE(*curptr))
            ++curptr;
        /* Mandatory CRLF */
        if (*curptr == '\r')
            ++curptr;
         ...
         */
        part =
          parse_multipart_part(pool, start_body, end_body - start_body, ctype);
        if (part) {
