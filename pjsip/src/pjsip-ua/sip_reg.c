...
PJ_DEF(pj_status_t)
pjsip_regc_create(pjsip_endpoint* endpt, void* token, pjsip_regc_cb* cb,
                  pjsip_regc** p_regc)
{
    pj_pool_t* pool;
    pjsip_regc* regc;
    ...
    status = pj_lock_create_recursive_mutex(pool, pool->obj_name, &regc->lock);
...
PJ_DEF(pj_status_t) pjsip_regc_get_info(pjsip_regc* regc, pjsip_regc_info* info)
{
...
static pj_status_t set_contact(pjsip_regc* regc, int contact_cnt,
                               const pj_str_t contact[])
{
    ...
    for (i = 0; i < contact_cnt; ++i) {
        ...
        hdr = (pjsip_contact_hdr*)pjsip_parse_hdr(regc->pool, &CONTACT, tmp.ptr,
                                                  tmp.slen, NULL);
        if (hdr == NULL) {
            PJ_LOG(4, (THIS_FILE, "Invalid Contact: \"%.*s\"", (int)tmp.slen,
                       tmp.ptr));
        ...
        while (h != &regc->removed_contact_hdr_list) {
            ...
            rc = pjsip_uri_cmp(PJSIP_URI_IN_CONTACT_HDR, h->uri, hdr->uri);
            if (rc == 0) {
                /* Match */
                pj_list_erase(h);
                break;
...
PJ_DEF(pj_status_t)
pjsip_regc_init(pjsip_regc* regc, const pj_str_t* srv_url,
                const pj_str_t* from_url, const pj_str_t* to_url,
                int contact_cnt, const pj_str_t contact[], pj_uint32_t expires)
{
    pj_str_t tmp;
    pj_status_t status;

    PJ_ASSERT_RETURN(regc && srv_url && from_url && to_url && expires,
                     PJ_EINVAL);
    ...
    if (!regc->from_hdr->uri) {
        PJ_LOG(4, (THIS_FILE, "regc: invalid source URI %.*s", from_url->slen,
                   from_url->ptr));
        return PJSIP_EINVALIDURI;
    }
    ...
    regc->to_hdr->uri = pjsip_parse_uri(regc->pool, tmp.ptr, tmp.slen,
                                        PJSIP_PARSE_URI_AS_NAMEADDR);
    if (!regc->to_hdr->uri) {
        PJ_LOG(4, (THIS_FILE, "regc: invalid target URI %.*s", to_url->slen,
                   to_url->ptr));
        return PJSIP_EINVALIDURI;
    }
...
PJ_DEF(pj_status_t)
pjsip_regc_set_credentials(pjsip_regc* regc, int count,
...
PJ_DEF(pj_status_t)
pjsip_regc_set_prefs(pjsip_regc* regc, const pjsip_auth_clt_pref* pref)
{
...
PJ_DEF(pj_status_t)
pjsip_regc_set_route_set(pjsip_regc* regc, const pjsip_route_hdr* route_set)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_regc_set_transport(pjsip_regc* regc, const pjsip_tpselector* sel)
{
...
PJ_DEF(pj_status_t)
pjsip_regc_add_headers(pjsip_regc* regc, const pjsip_hdr* hdr_list)
{
    const pjsip_hdr* hdr;
...
static pj_status_t create_request(pjsip_regc* regc, pjsip_tx_data** p_tdata)
{
    pj_status_t status;
    pjsip_tx_data* tdata;
    ...
    status = pjsip_endpt_create_request_from_hdr(
      regc->endpt, pjsip_get_register_method(), regc->srv_url, regc->from_hdr,
      regc->to_hdr, NULL, regc->cid_hdr, regc->cseq_hdr->cseq, NULL, &tdata);
    if (status != PJ_SUCCESS)
        return status;
    ...
    if (!pj_list_empty(&regc->route_set)) {
        ...
        route_pos =
          (pjsip_hdr*)pjsip_msg_find_hdr(tdata->msg, PJSIP_H_VIA, NULL);
        if (!route_pos)
        ...
        while (route != &regc->route_set) {
            pjsip_hdr* new_hdr =
              (pjsip_hdr*)pjsip_hdr_clone(tdata->pool, route);
    ...
    if (!pj_list_empty(&regc->hdr_list)) {
        ...
        while (hdr != &regc->hdr_list) {
            pjsip_hdr* new_hdr = (pjsip_hdr*)pjsip_hdr_clone(tdata->pool, hdr);
...
PJ_DEF(pj_status_t)
pjsip_regc_register(pjsip_regc* regc, pj_bool_t autoreg,
                    pjsip_tx_data** p_tdata)
{
    ...
    if (regc->expires_hdr)
        pjsip_msg_add_hdr(
          msg, (pjsip_hdr*)pjsip_hdr_clone(tdata->pool, regc->expires_hdr));
    ...
    if (h_allow) {
        pjsip_msg_add_hdr(msg,
                          (pjsip_hdr*)pjsip_hdr_clone(tdata->pool, h_allow));
    }
...
PJ_DEF(pj_status_t)
pjsip_regc_unregister(pjsip_regc* regc, pjsip_tx_data** p_tdata)
{
    pjsip_tx_data* tdata;
    pjsip_msg* msg;
    pjsip_hdr* hdr;
...
PJ_DEF(pj_status_t)
pjsip_regc_unregister_all(pjsip_regc* regc, pjsip_tx_data** p_tdata)
{
    pjsip_tx_data* tdata;
...
PJ_DEF(pj_status_t)
pjsip_regc_update_contact(pjsip_regc* regc, int contact_cnt,
                          const pj_str_t contact[])
{
    pj_status_t status;
...
PJ_DEF(pj_status_t)
pjsip_regc_update_expires(pjsip_regc* regc, pj_uint32_t expires)
{
    PJ_ASSERT_RETURN(regc, PJ_EINVAL);
...
static void cbparam_init(struct pjsip_regc_cbparam* cbparam, pjsip_regc* regc,
                         pj_status_t status, int st_code,
                         const pj_str_t* reason, pjsip_rx_data* rdata,
                         pj_uint32_t expiration, int contact_cnt,
                         pjsip_contact_hdr* contact[], pj_bool_t is_unreg)
{
    cbparam->regc = regc;
...
PJ_DEF(pj_status_t)
pjsip_regc_set_reg_tsx_cb(pjsip_regc* regc, pjsip_regc_tsx_cb* tsx_cb)
{
    PJ_ASSERT_RETURN(regc, PJ_EINVAL);
    regc->tsx_cb = tsx_cb;
    return PJ_SUCCESS;
...
PJ_DEF(pj_status_t)
pjsip_regc_set_via_sent_by(pjsip_regc* regc, pjsip_host_port* via_addr,
...
PJ_DEF(pj_status_t)
pjsip_regc_set_delay_before_refresh(pjsip_regc* regc, pj_uint32_t delay)
{
    PJ_ASSERT_RETURN(regc, PJ_EINVAL);
                                                 ...
                                                 pjsip_contact_hdr* contacts[])
{
    ...
    *contact_cnt = 0;
    for (hdr = msg->hdr.next; hdr != &msg->hdr; hdr = hdr->next) {
        if (hdr->type == PJSIP_H_CONTACT && *contact_cnt < max_contact) {
    ...
    if (regc->current_op == REGC_REGISTERING) {
        ...
        if (pjsip_cfg()->regc.check_contact || regc->add_xuid_param) {
            ...
            for (i = 0; i < *contact_cnt; ++i) {
                ...
                our_hdr = (const pjsip_contact_hdr*)regc->contact_hdr_list.next;
                ...
                while ((void*)our_hdr != (void*)&regc->contact_hdr_list) {
                     ...
                     */
                    uri1 =
                      (const pjsip_uri*)pjsip_uri_get_uri(contacts[i]->uri);
                    uri2 = (const pjsip_uri*)pjsip_uri_get_uri(our_hdr->uri);

                    /* First try with exact matching, according to RFC 3261
                     * Section 19.1.4 URI Comparison
                     */
                    if (pjsip_cfg()->regc.check_contact) {
                        matched = pjsip_uri_cmp(PJSIP_URI_IN_CONTACT_HDR, uri1,
                                                uri2) == 0;
                    }
                         ...
                         PJSIP_URI_SCHEME_IS_SIPS(uri2)))
                    {
                        ...
                        p2 = pjsip_param_cfind(&sip_uri2->other_param,
                                               &XUID_PARAM_NAME);
                        matched =
                          p1 && p2 && pj_strcmp(&p1->value, &p2->value) == 0;
                    }

                    if (matched) {
                        has_our_contact = PJ_TRUE;

                        if (contacts[i]->expires !=
...
static void regc_tsx_callback(void* token, pjsip_event* event)
{
        ...
        tsx->status_code == PJSIP_SC_UNAUTHORIZED)
    {
        ...
        if (update_contact) {
            ...
            while (chdr != &regc->contact_hdr_list) {
                pj_list_insert_before(ins_hdr, (pjsip_hdr*)pjsip_hdr_clone(
                                                 tsx->last_tx->pool, chdr));
                chdr = chdr->next;
            }
        ...
        status = pjsip_auth_clt_reinit_req(&regc->auth_sess, rdata,
                                           tsx->last_tx, &tdata);
        ...
        if (status != PJ_SUCCESS) {
            ...
            if (regc->_delete_flag == 0) {
                ...
                call_callback(regc, status, tsx->status_code,
                              &rdata->msg_info.msg->line.status.reason, rdata,
                              NOEXP, 0, NULL, is_unreg);
                pj_lock_acquire(regc->lock);
            }
        }
               ...
               regc->current_op == REGC_REGISTERING)
    {
        ...
        me_hdr = (pjsip_min_expires_hdr*)pjsip_msg_find_hdr(
          rdata->msg_info.msg, PJSIP_H_MIN_EXPIRES, NULL);
        if (me_hdr) {
            ...
            if (!regc->expires_hdr ||
                ...
                handled = PJ_FALSE;
                PJ_LOG(4, (THIS_FILE,
                           "Registration failed: 423 response "
...
handle_err:
    if (!handled) {
        ...
        if (tsx->status_code / 100 == 2) {
            ...
            expiration = calculate_response_expiration(
              regc, rdata, &contact_cnt, PJSIP_REGC_MAX_CONTACT, contact);
            ...
            if (expiration == 0 && regc->current_op != REGC_UNREGISTERING) {
                ...
                PJ_LOG(4, (THIS_FILE,
                           "Modify response's expiration from 0 "
                           "to %d",
                           expiration));
            }
#endif
        ...
        call_callback(regc, PJ_SUCCESS, tsx->status_code,
                      (rdata ? &rdata->msg_info.msg->line.status.reason :
                               &tsx->status_text),
                      rdata, expiration, contact_cnt, contact, is_unreg);
        pj_lock_acquire(regc->lock);
    }
...
PJ_DEF(pj_status_t) pjsip_regc_send(pjsip_regc* regc, pjsip_tx_data* tdata)
{
    ...
    if (regc->has_tsx) {
        PJ_LOG(4, (THIS_FILE,
                   "Unable to send request, regc has another "
                   "transaction pending"));
    ...
    cseq_hdr =
      (pjsip_cseq_hdr*)pjsip_msg_find_hdr(tdata->msg, PJSIP_H_CSEQ, NULL);
    cseq_hdr->cseq = cseq;

    /* Find Expires header */
    expires_hdr =
      (pjsip_expires_hdr*)pjsip_msg_find_hdr(tdata->msg, PJSIP_H_EXPIRES, NULL);
