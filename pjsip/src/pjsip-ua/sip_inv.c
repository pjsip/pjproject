				       ...
				       pjsip_rx_data *rdata)
{
     ...
     */
    if (status != PJ_SUCCESS) {
	status = pjsip_dlg_create_response(inv->dlg, rdata, PJSIP_SC_OK, NULL, &tdata);
	goto on_return;
    }
    ...
    if (neg_state == PJMEDIA_SDP_NEG_STATE_LOCAL_OFFER) {
	status = inv_check_sdp_in_incoming_msg(inv, pjsip_rdata_get_tsx(rdata), rdata);
	...
	if (status == PJ_SUCCESS && inv->invite_tsx) {
	    ...
	    /* Get/attach invite session's transaction data */
	    tsx_inv_data = (struct tsx_inv_data*)inv->invite_tsx->mod_data[mod_inv.mod.id];
	    if (tsx_inv_data == NULL) {
		tsx_inv_data = PJ_POOL_ZALLOC_T(inv->invite_tsx->pool, struct tsx_inv_data);
		tsx_inv_data->inv = inv;
