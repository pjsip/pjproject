...
static const char* inv_state_names[] = {
    ...
    "CONNECTING", "CONFIRMED", "DISCONNCTD", "TERMINATED",
};
...
static pj_status_t process_answer(pjsip_inv_session* inv, int st_code,
                                  pjsip_tx_data* tdata,
...
static pj_bool_t inv_check_secure_dlg(pjsip_inv_session* inv, pjsip_event* e);
...
static void (*inv_state_handler[])(pjsip_inv_session* inv, pjsip_event* e) = {
    &inv_on_state_null,         &inv_on_state_calling,
    &inv_on_state_incoming,     &inv_on_state_early,
    &inv_on_state_connecting,   &inv_on_state_confirmed,
...
static pj_status_t mod_inv_load(pjsip_endpoint* endpt)
{
    ...
    pjsip_endpt_add_capability(endpt, &mod_inv.mod, PJSIP_H_ACCEPT, NULL, 1,
                               &accepted);
                          ...
                          pjsip_event* e)
{
    ...
    if (state == PJSIP_INV_STATE_CONFIRMED) {
        ...
        if (inv->invite_tsx) {
            tsx_inv_data =
              (struct tsx_inv_data*)inv->invite_tsx->mod_data[mod_inv.mod.id];
        }
             ...
             pjmedia_sdp_neg_get_state(inv->neg) != PJMEDIA_SDP_NEG_STATE_DONE))
        {
            ...
            PJ_LOG(4, (inv->obj_name,
                       "SDP offer/answer incomplete, ending the "
                       "session"));

            status =
              pjsip_inv_end_session(inv, PJSIP_SC_NOT_ACCEPTABLE, NULL, &bye);
    ...
    pj_assert(inv->state != PJSIP_INV_STATE_DISCONNECTED || inv->cause != 0);
...
static void inv_perform_pending_bye(pjsip_inv_session* inv)
{
    if (inv->pending_bye) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(
              1, (inv->dlg->obj_name, status, "Failed sending pending BYE"));
        }
    }
}
...
static pj_status_t inv_send_ack(pjsip_inv_session* inv, pjsip_event* e)
{
    ...
    if (inv->last_ack && rdata->msg_info.cseq->cseq == inv->last_ack_cseq &&
        inv->last_ack->tp_info.transport != NULL && !inv->last_ack->is_pending)
    {
...
static pj_bool_t mod_inv_on_rx_request(pjsip_rx_data* rdata)
{
    ...
    if (method->id == PJSIP_BYE_METHOD || method->id == PJSIP_CANCEL_METHOD) {
        if (inv == NULL)
            return PJ_FALSE;
    ...
    if (method->id == PJSIP_ACK_METHOD && inv) {
        ...
        if (inv->invite_tsx->state <= PJSIP_TSX_STATE_COMPLETED) {
            ...
            pjsip_tsx_terminate(inv->invite_tsx, inv->invite_tsx->status_code);
            inv->invite_tsx = NULL;
            if (inv->last_answer) {
...
static pj_bool_t tx_data_has_sdp(const pjsip_tx_data* tdata)
{
    ...
    if (body && pj_stricmp(&body->content_type.type, &app_sdp.type) == 0 &&
        pj_stricmp(&body->content_type.subtype, &app_sdp.subtype) == 0)
    {
        return PJ_TRUE;

    } else if (body && pj_stricmp2(&body->content_type.type, "multipart") &&
...
{
    PJ_ASSERT_RETURN(
      state >= PJSIP_INV_STATE_NULL && state <= PJSIP_INV_STATE_DISCONNECTED,
      "??");
                     ...
                     unsigned options, pjsip_inv_session** p_inv)
{
    ...
    if (local_sdp) {
        status =
          pjmedia_sdp_neg_create_w_local_offer(inv->pool, local_sdp, &inv->neg);
        if (status != PJ_SUCCESS) {
            pjsip_dlg_dec_lock(dlg);
            return status;
        }
                   ...
                   const pjsip_media_type* search_media_type)
{
    ...
    sdp_info = PJ_POOL_ZALLOC_T(pool, pjsip_sdp_info);
    ...
    } else if (pjsip_media_type_cmp(&multipart_mixed, msg_type, PJ_FALSE) ==
                 0 ||
               pjsip_media_type_cmp(&multipart_alternative, msg_type,
                                    PJ_FALSE) == 0)
    {
        pjsip_multipart_part* part;
     ...
     */
    status = pjmedia_sdp_parse(pool, sdp_info->body.ptr, sdp_info->body.slen,
                               &sdp_info->sdp);
    if (status == PJ_SUCCESS)
    ...
    if (status != PJ_SUCCESS) {
        sdp_info->sdp = NULL;
        PJ_PERROR(1, (THIS_FILE, status, "Error parsing/validating SDP body"));
    }
                          ...
                          const pjsip_media_type* search_media_type)
{
    ...
    sdp_info =
      pjsip_get_sdp_info(rdata->tp_info.pool, rdata->msg_info.msg->body,
                         msg_media_type, search_media_type);
                          ...
                          const pjsip_media_type* search_media_type)
{
    ...
    sdp_info = pjsip_get_sdp_info(tdata->pool, tdata->msg->body, msg_media_type,
                                  search_media_type);
...
pjsip_inv_verify_request3(pjsip_rx_data* rdata, pj_pool_t* tmp_pool,
                          unsigned* options, const pjmedia_sdp_session* r_sdp,
                          const pjmedia_sdp_session* l_sdp, pjsip_dialog* dlg,
                          pjsip_endpoint* endpt, pjsip_tx_data** p_tdata)
{
    pjsip_msg* msg = NULL;
    ...
    if (msg && (!c_hdr || !c_hdr->uri)) {
        ...
        w = pjsip_warning_hdr_create(tmp_pool, 399, pjsip_endpt_name(endpt),
                                     &warn_text);
        if (w) {
    ...
    if (pjsip_cfg()->endpt.disable_secure_dlg_check == PJ_FALSE && msg &&
        PJSIP_URI_SCHEME_IS_SIPS(msg->line.req.uri))
    {
        ...
        if (status == PJ_SUCCESS) {
            pjsip_rr_hdr* r = (pjsip_rr_hdr*)pjsip_msg_find_hdr(
              msg, PJSIP_H_RECORD_ROUTE, NULL);
            if (r && !PJSIP_URI_SCHEME_IS_SIPS(&r->name_addr)) {
                ...
                if (PJSIP_URI_SCHEME_IS_SIP(&r->name_addr)) {
                    pjsip_sip_uri* sip_uri =
                      (pjsip_sip_uri*)pjsip_uri_get_uri(r->name_addr.uri);
        ...
        if (status != PJ_SUCCESS) {
            ...
            w = pjsip_warning_hdr_create(tmp_pool, 381, pjsip_endpt_name(endpt),
                                         &warn_text);
            if (w) {
                pj_list_push_back(&res_hdr_list, w);
            }
        ...
        if (sdp_info->sdp_err != PJ_SUCCESS) {
            ...
            if (p_tdata) {
                ...
                w = pjsip_warning_hdr_create_from_status(
                  tmp_pool, pjsip_endpt_name(endpt), sdp_info->sdp_err);
    ...
    if (r_sdp) {
        ...
        if (l_sdp) {
            ...
            status = pjmedia_sdp_neg_create_w_remote_offer(tmp_pool, l_sdp,
                                                           r_sdp, &neg);
            if (status != PJ_SUCCESS) {
            ...
            if (status != PJ_SUCCESS) {
                ...
                if (p_tdata) {
                    ...
                    /* Add Warning header. */
                    w = pjsip_warning_hdr_create_from_status(
                      tmp_pool, pjsip_endpt_name(endpt), status);
                    PJ_ASSERT_RETURN(w, PJ_ENOMEM);
    ...
    if (msg) {
        sup_hdr = (pjsip_supported_hdr*)pjsip_msg_find_hdr(
          msg, PJSIP_H_SUPPORTED, NULL);
    }
    if (sup_hdr) {
        unsigned i;
    ...
    if (msg) {
        req_hdr =
          (pjsip_require_hdr*)pjsip_msg_find_hdr(msg, PJSIP_H_REQUIRE, NULL);
    }
    if (req_hdr) {
        unsigned i;
        const pj_str_t STR_100REL = { "100rel", 6 };
        ...
        if (unsupp_cnt) {
            ...
            if (p_tdata) {
                ...
                h = pjsip_endpt_get_capability(endpt, PJSIP_H_SUPPORTED, NULL);
                pj_assert(h);
                if (h) {
                    sup_hdr =
                      (pjsip_supported_hdr*)pjsip_hdr_clone(tmp_pool, h);
                 ...
                 (rem_option & PJSIP_INV_SUPPORT_TRICKLE_ICE) == 0)))
    {
        ...
        if (p_tdata) {
            ...
            pj_assert(h);
            if (h) {
                sup_hdr = (pjsip_supported_hdr*)pjsip_hdr_clone(tmp_pool, h);
                pj_list_push_back(&res_hdr_list, sup_hdr);
            }
        }
...
on_return:
    ...
    if (code != 200 && p_tdata) {
        ...
        if (dlg) {
            ...
            status =
              pjsip_endpt_create_response(endpt, rdata, code, NULL, &tdata);
        }
...
pjsip_inv_verify_request2(pjsip_rx_data* rdata, unsigned* options,
                          ...
                          pjsip_endpoint* endpt, pjsip_tx_data** p_tdata)
{
    return pjsip_inv_verify_request3(rdata, rdata->tp_info.pool, options, r_sdp,
                                     l_sdp, dlg, endpt, p_tdata);
}
...
pjsip_inv_verify_request(pjsip_rx_data* rdata, unsigned* options,
                         ...
                         pjsip_endpoint* endpt, pjsip_tx_data** p_tdata)
{
    return pjsip_inv_verify_request3(rdata, rdata->tp_info.pool, options, NULL,
                                     l_sdp, dlg, endpt, p_tdata);
}
                     ...
                     pjsip_inv_session** p_inv)
{
    ...
    if (sdp_info->sdp) {
        status = pjmedia_sdp_neg_create_w_remote_offer(
          inv->pool, local_sdp, sdp_info->sdp, &inv->neg);
...
pjsip_inv_terminate(pjsip_inv_session* inv, int st_code, pj_bool_t notify)
{
     ...
     * disconnected.
     */
    if (inv->invite_tsx && inv->invite_tsx->state <= PJSIP_TSX_STATE_COMPLETED)
    {
...
static pjsip_multipart_part* create_sdp_part(pj_pool_t* pool,
                                       ...
                                       const pjmedia_sdp_session* c_sdp)
{
    ...
    status = pjsip_create_sdp_body(pool, pjmedia_sdp_session_clone(pool, c_sdp),
                                   &body);
 ...
 */
static void cleanup_allow_sup_hdr(unsigned inv_option, pjsip_tx_data* tdata,
                                  ...
                                  pjsip_supported_hdr* sup_hdr)
{
    ...
    if (!allow_hdr && tdata) {
        allow_hdr =
          (pjsip_allow_hdr*)pjsip_msg_find_hdr(tdata->msg, PJSIP_H_ALLOW, NULL);
    }
    if (!sup_hdr && tdata) {
        sup_hdr = (pjsip_supported_hdr*)pjsip_msg_find_hdr(
          tdata->msg, PJSIP_H_SUPPORTED, NULL);
    }
...
pjsip_inv_invite(pjsip_inv_session* inv, pjsip_tx_data** p_tdata)
{
    ...
    status =
      pjsip_dlg_create_request(inv->dlg, pjsip_get_invite_method(), -1, &tdata);
    if (status != PJ_SUCCESS)
        goto on_return;
    ...
    if (inv->state == PJSIP_INV_STATE_NULL) {
        ...
        while (hdr != &inv->dlg->inv_hdr) {
            pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*)pjsip_hdr_shallow_clone(
                                            tdata->pool, hdr));
            hdr = hdr->next;
        }
    }
    ...
    if (inv->dlg->add_allow) {
        ...
        if (hdr) {
            allow_hdr =
              (pjsip_allow_hdr*)pjsip_hdr_shallow_clone(tdata->pool, hdr);
    ...
    if (hdr) {
        sup_hdr =
          (pjsip_supported_hdr*)pjsip_hdr_shallow_clone(tdata->pool, hdr);
...
static pj_status_t inv_negotiate_sdp(pjsip_inv_session* inv)
{
    ...
    PJ_ASSERT_RETURN(
      pjmedia_sdp_neg_get_state(inv->neg) == PJMEDIA_SDP_NEG_STATE_WAIT_NEGO,
                                                 ...
                                                 pjsip_rx_data* rdata)
{
    ...
    if (sdp_info->sdp_err) {
        PJ_PERROR(4, (THIS_FILE, sdp_info->sdp_err, "Error parsing SDP in %s",
                      pjsip_rx_data_get_info(rdata)));
        return PJMEDIA_SDP_EINSDP;
    }
    ...
    if (tsx_inv_data->sdp_done) {
               ...
               pj_stricmp(&tsx_inv_data->done_tag, &res_tag))))
        {
            ...
            status = pjmedia_sdp_neg_modify_local_offer2(
              inv->pool_prov, inv->neg, inv->sdp_neg_flags, reoffer_sdp);

            if (status != PJ_SUCCESS) {
                PJ_LOG(1, (inv->obj_name,
                           "Error updating local offer for "
                           "forked 2xx/18x response (err=%d)",
                           status));
                return status;
            }
        ...
        pjmedia_sdp_neg_get_state(inv->neg) == PJMEDIA_SDP_NEG_STATE_DONE)
    {
        ...
        if (inv->neg == NULL) {
            status = pjmedia_sdp_neg_create_w_remote_offer(
              inv->pool, NULL, sdp_info->sdp, &inv->neg);
        } else {
        ...
        if (pjmedia_sdp_neg_get_state(inv->neg) !=
                ...
                msg->line.req.method.id == PJSIP_INVITE_METHOD)
            {
                 ...
                 */
                PJ_LOG(5, (inv->obj_name,
                           "Ignoring on_rx_offer() status "
               ...
               PJMEDIA_SDP_NEG_STATE_LOCAL_OFFER)
    {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4,
                      (THIS_FILE, status, "Error processing SDP answer in %s",
                       pjsip_rx_data_get_info(rdata)));
            return PJMEDIA_SDP_EINSDP;
        }
        ...
        tsx_inv_data->done_early_rel =
          tsx_inv_data->done_early && pjsip_100rel_is_reliable(rdata);
        ...
        PJ_LOG(
          5, (THIS_FILE, "Ignored SDP in %s: negotiator state is %s",
              pjsip_rx_data_get_info(rdata),
                                  ...
                                  const pjmedia_sdp_session* local_sdp)
{
    ...
    /* If SDP negotiator is ready, start negotiation. */
    if (st_code / 100 == 2 ||
        (st_code / 10 == 18 && st_code != 180 && st_code != 181))
    {
        pjmedia_sdp_neg_state neg_state;
                         ...
                         pjsip_tx_data** p_tdata)
{
    ...
    /* Create response */
    status =
      pjsip_dlg_create_response(inv->dlg, rdata, st_code, st_text, &tdata);
    if (status != PJ_SUCCESS)
        goto on_return;
...
pjsip_inv_set_local_sdp(pjsip_inv_session* inv, const pjmedia_sdp_session* sdp)
{
    ...
    if (inv->neg) {
            ...
            pjmedia_sdp_neg_get_neg_remote(inv->neg, &offer) == PJ_SUCCESS)
        {
            ...
            status = pjmedia_sdp_neg_modify_local_offer2(
              inv->pool, inv->neg, inv->sdp_neg_flags, sdp);
        } else
            return PJMEDIA_SDPNEG_EINSTATE;
    } else {
        status =
          pjmedia_sdp_neg_create_w_local_offer(inv->pool, sdp, &inv->neg);
    }
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_set_sdp_answer(pjsip_inv_session* inv, const pjmedia_sdp_session* sdp)
{
    pj_status_t status;
                      ...
                      const pj_str_t* st_text, pjsip_tx_data** p_tdata)
{
    ...
    case PJSIP_INV_STATE_INCOMING:
        ...
        if (inv->role == PJSIP_ROLE_UAC) {
            ...
            status = pjsip_endpt_create_cancel(
              inv->dlg->endpt, inv->invite_tsx->last_tx, &tdata);
            if (status != PJ_SUCCESS) {
                pj_log_pop_indent();
                return status;
            }
    ...
    case PJSIP_INV_STATE_CONFIRMED:
        ...
        status = pjsip_dlg_create_request(inv->dlg, pjsip_get_bye_method(), -1,
                                          &tdata);
        break;
...
PJ_DEF(pj_status_t)
pjsip_inv_process_redirect(pjsip_inv_session* inv, pjsip_redirect_op op,
                           pjsip_event* e)
{
    ...
    case PJSIP_REDIRECT_ACCEPT_REPLACE:
         ...
         */
        {
            ...
            /* Set target */
            tdata->msg->line.req.uri = (pjsip_uri*)pjsip_uri_clone(
              tdata->pool, inv->dlg->target_set.current->uri);

            /* Remove branch param in Via header. */
            via =
              (pjsip_via_hdr*)pjsip_msg_find_hdr(tdata->msg, PJSIP_H_VIA, NULL);
            ...
            if (op == PJSIP_REDIRECT_ACCEPT_REPLACE) {
                ...
                to->uri = (pjsip_uri*)pjsip_uri_clone(
                  tdata->pool, dlg->target_set.current->uri);
                to->tag.slen = 0;
                pj_list_init(&to->other_param);

                /* Re-init dialog remote info */
                dlg->remote.info =
                  (pjsip_to_hdr*)pjsip_hdr_clone(dlg->pool, to);
                ...
                {
                    pjsip_sip_uri* sip_uri =
                      (pjsip_sip_uri*)pjsip_uri_get_uri(dlg->remote.info->uri);
                   ...
                   pjsip_tx_data** p_tdata)
{
    ...
    if (new_contact) {
        ...
        contact_hdr = (pjsip_contact_hdr*)pjsip_parse_hdr(
          inv->dlg->pool, &STR_CONTACT, tmp.ptr, tmp.slen, NULL);
        if (!contact_hdr) {
    ...
    if (new_offer) {
        if (!inv->neg) {
            ...
            case PJMEDIA_SDP_NEG_STATE_DONE:
                status = pjmedia_sdp_neg_modify_local_offer2(
                  inv->pool_prov, inv->neg, inv->sdp_neg_flags, new_offer);
                if (status != PJ_SUCCESS)
                    goto on_return;
                break;
            }
                 ...
                 const pjmedia_sdp_session* offer, pjsip_tx_data** p_tdata)
{
    ...
    PJ_ASSERT_RETURN(inv->state < PJSIP_INV_STATE_DISCONNECTED, PJ_EINVALIDOP);
    ...
    /* Create request */
    status =
      pjsip_dlg_create_request(inv->dlg, &pjsip_update_method, -1, &tdata);
    if (status != PJ_SUCCESS)
        goto on_error;
...
pjsip_inv_create_ack(pjsip_inv_session* inv, int cseq, pjsip_tx_data** p_tdata)
{
    ...
    status = pjsip_dlg_create_request(inv->dlg, pjsip_get_ack_method(), cseq,
                                      &inv->last_ack);
    if (status != PJ_SUCCESS) {
        pjsip_dlg_dec_lock(inv->dlg);
        return status;
...
pjsip_inv_send_msg(pjsip_inv_session* inv, pjsip_tx_data* tdata)
{
    ...
    if (tdata->msg->type == PJSIP_REQUEST_MSG) {
        ...
        status =
          pjsip_dlg_send_request(inv->dlg, tdata, mod_inv.mod.id, tsx_inv_data);
        if (status != PJ_SUCCESS) {
            goto on_error;
        }
         ...
         * request.
         */
        PJ_ASSERT_RETURN((cseq = (pjsip_cseq_hdr*)pjsip_msg_find_hdr(
                            tdata->msg, PJSIP_H_CSEQ, NULL)) != NULL &&
                                        ...
                                        pjsip_event* e)
{
    ...
    if (invite_tsx == NULL) {
         ...
         */
        status = pjsip_dlg_create_response(inv->dlg, rdata, 481, NULL, &tdata);
    ...
    if (invite_tsx && invite_tsx->status_code < 200) {
        ...
        if (status == PJ_SUCCESS) {
            ...
            if (inv->options & PJSIP_INV_REQUIRE_100REL) {
                ...
                status = pjsip_dlg_send_response(inv->dlg, invite_tsx, tdata);
            }
        }
    }
...
static void inv_respond_incoming_bye(pjsip_inv_session* inv,
                                     ...
                                     pjsip_rx_data* rdata, pjsip_event* e)
{
    pj_status_t status;
    pjsip_tx_data* tdata;
                                    ...
                                    pjsip_rx_data* rdata, pjsip_event* e)
{
    ...
    if (tsx->status_code == 401 || tsx->status_code == 407) {
        ...
        status = pjsip_auth_clt_reinit_req(&inv->dlg->auth_sess, rdata,
                                           tsx->last_tx, &tdata);
 ...
 */
static void inv_respond_incoming_update(pjsip_inv_session* inv, pjsip_event* e)
{
    pjmedia_sdp_neg_state neg_state;
    pj_status_t status;
    pjsip_tx_data* tdata = NULL;
    ...
    if (status != PJ_SUCCESS) {
        status =
          pjsip_dlg_create_response(inv->dlg, rdata, st_code, NULL, &tdata);
        goto on_return;
    }
    ...
    else if (neg_state == PJMEDIA_SDP_NEG_STATE_LOCAL_OFFER)
    {
        status = pjsip_dlg_create_response(
          inv->dlg, rdata, PJSIP_SC_REQUEST_PENDING, NULL, &tdata);
    }
             ...
             neg_state == PJMEDIA_SDP_NEG_STATE_WAIT_NEGO)
    {
        ...
        status = pjsip_dlg_create_response(
          inv->dlg, rdata, PJSIP_SC_INTERNAL_SERVER_ERROR, NULL, &tdata);
            ...
            (status = inv_negotiate_sdp(inv)) != PJ_SUCCESS)
        {
            ...
            status = pjsip_dlg_create_response(
              inv->dlg, rdata, PJSIP_SC_NOT_ACCEPTABLE_HERE, NULL, &tdata);
        } else {
                                            ...
                                            pjsip_event* e)
{
    ...
    else if (tsx->state == PJSIP_TSX_STATE_COMPLETED && tsx->status_code == 422)
    {
        status =
          handle_timer_response(inv, e->body.tsx_state.src.rdata, PJ_FALSE);
        handled = PJ_TRUE;
    }
        ...
        if (inv_check_secure_dlg(inv, e)) {
            ...
            if (rdata->msg_info.msg->body) {
                                  ...
                                  PJMEDIA_SDP_NEG_STATE_LOCAL_OFFER)
                {
                    ...
                    PJ_LOG(5, (THIS_FILE,
                               "Ignored message body in %s as no "
                                       ...
                                       pjsip_rx_data* rdata)
{
    ...
    if (status == PJ_SUCCESS && inv->invite_tsx) {
        ...
        if (tsx_inv_data == NULL) {
            tsx_inv_data =
              PJ_POOL_ZALLOC_T(inv->invite_tsx->pool, struct tsx_inv_data);
            tsx_inv_data->inv = inv;
...
static pj_bool_t inv_check_secure_dlg(pjsip_inv_session* inv, pjsip_event* e)
{
         ...
         pjsip_method_cmp(&tsx->method, &pjsip_update_method) == 0))
    {
        ...
        /* Check Contact header */
        c = (pjsip_contact_hdr*)pjsip_msg_find_hdr(msg, PJSIP_H_CONTACT, NULL);
        ...
        if (status == PJSIP_ESESSIONINSECURE) {
            ...
            if (tsx->role == PJSIP_ROLE_UAC) {
                ...
                if (status == PJ_SUCCESS && bye) {
                    pjsip_warning_hdr* w;
                    w = pjsip_warning_hdr_create(
                      bye->pool, 381, pjsip_endpt_name(dlg->endpt), &warn_text);
                    if (w)
                ...
                status =
                  pjsip_dlg_create_response(inv->dlg, rdata, 480, NULL, &tdata);
                if (status == PJ_SUCCESS) {
                    pjsip_warning_hdr* w;
 ...
 */
static pj_bool_t handle_uac_tsx_response(pjsip_inv_session* inv, pjsip_event* e)
{
          ...
          !pjsip_cfg()->endpt.keep_inv_after_tsx_timeout)))
    {
        ...
        /* Send BYE */
        status =
          pjsip_dlg_create_request(inv->dlg, pjsip_get_bye_method(), -1, &bye);
        if (status == PJ_SUCCESS) {
...
static void handle_uac_call_rejection(pjsip_inv_session* inv, pjsip_event* e)
{
    ...
    if (PJSIP_IS_STATUS_IN_CLASS(tsx->status_code, 300)) {
        if (mod_inv.cb.on_redirected == NULL) {
             ...
             * duplicate URI's.
             */
            pjsip_target_set_add_from_msg(&inv->dlg->target_set, inv->dlg->pool,
                                          res_msg);
        ...
        case PJSIP_TSX_STATE_PROCEEDING:
            ...
            if (tsx->status_code != 100) {
                if (inv->role == PJSIP_ROLE_UAC) {
                    ...
                    if (msg) {
                        allow = (pjsip_allow_hdr*)pjsip_msg_find_hdr(
                          msg, PJSIP_H_ALLOW, NULL);
                    }
                    if (allow) {
                        unsigned i;
                        const pj_str_t STR_UPDATE = { "UPDATE", 6 };
...
static void inv_on_state_early(pjsip_inv_session* inv, pjsip_event* e)
{
        ...
        case PJSIP_TSX_STATE_COMPLETED:
            if (tsx->status_code / 100 == 2) {
                ...
                if (e->body.tsx_state.type == PJSIP_EVENT_RX_MSG) {
                    ...
                    /* Process session timer response. */
                    status = handle_timer_response(
                      inv, e->body.tsx_state.src.rdata, PJ_TRUE);
                    if (status != PJ_SUCCESS)
                        break;
    ...
    } else if (inv->role == PJSIP_ROLE_UAS && tsx->role == PJSIP_ROLE_UAS &&
               tsx->method.id == PJSIP_CANCEL_METHOD &&
               tsx->state < PJSIP_TSX_STATE_COMPLETED &&
               e->body.tsx_state.type == PJSIP_EVENT_RX_MSG)
    {
               ...
               pjsip_method_cmp(&tsx->method, &pjsip_prack_method) == 0)
    {
        ...
        handle_uac_tsx_response(inv, e);
               ...
               tsx->method.id == PJSIP_BYE_METHOD && tsx->status_code < 200 &&
               e->body.tsx_state.type == PJSIP_EVENT_RX_MSG)
    {
            ...
            if (tsx->status_code / 100 != 2) {
                if (tsx->role == PJSIP_ROLE_UAC) {
                    ...
                    status = pjsip_dlg_create_request(
                      inv->dlg, pjsip_get_bye_method(), -1, &bye);
                    if (status == PJ_SUCCESS) {
                        pjsip_inv_send_msg(inv, bye);
...
static void inv_on_state_confirmed(pjsip_inv_session* inv, pjsip_event* e)
{
    ...
    if (tsx->method.id == PJSIP_BYE_METHOD && tsx->role == PJSIP_ROLE_UAC &&
        (tsx->state == PJSIP_TSX_STATE_COMPLETED ||
         tsx->state == PJSIP_TSX_STATE_TERMINATED))
    {
        /*
               ...
               e->body.tsx_state.type == PJSIP_EVENT_RX_MSG)
    {
        ...
        if (tsx->state == PJSIP_TSX_STATE_TRYING) {
            ...
            if (status == PJ_SUCCESS && mod_inv.cb.on_rx_reinvite &&
                ...
                sdp_info = pjsip_rdata_get_sdp_info(rdata);
                rc = (*mod_inv.cb.on_rx_reinvite)(inv, sdp_info->sdp, rdata);
                if (rc == PJ_SUCCESS) {
                    /* Application will send its own response.
                     * Our job is done. */
                    PJ_LOG(5,
                           (inv->obj_name, "on_rx_reinvite() returns %d", rc));
                    return;
                }
                 ...
                 */
                if (sdp_info->sdp && (pjmedia_sdp_neg_get_state(inv->neg) !=
                                      PJMEDIA_SDP_NEG_STATE_WAIT_NEGO))
                {
                    status = PJ_EINVALIDOP;
                }
            ...
            if (status != PJ_SUCCESS) {
                ...
                if (reject_message) {
                    ...
                    status = pjsip_dlg_create_response(
                      inv->dlg, rdata,
                      (status == PJMEDIA_SDP_EINSDP) ? 415 : 488, NULL, &tdata);
                    ...
                    if (accept) {
                        pjsip_msg_add_hdr(
                          tdata->msg,
                          (pjsip_hdr*)pjsip_hdr_clone(tdata->pool, accept));
                    }
            ...
            if (sdp_info->sdp != NULL) {
                ...
                if (mod_inv.cb.on_create_offer) {
                    ...
                    if (sdp) {
                         ...
                         */
                        status = pjmedia_sdp_neg_modify_local_offer2(
                          inv->pool_prov, inv->neg, inv->sdp_neg_flags, sdp);
                ...
                if (sdp == NULL) {
                    ...
                    status = pjmedia_sdp_neg_send_local_offer(
                      inv->pool_prov, inv->neg, &active_sdp);
                    if (status == PJ_SUCCESS)
            ...
            if (status != PJ_SUCCESS) {
                ...
                rc =
                  pjsip_dlg_create_response(dlg, rdata, 500, &reason, &tdata);
                if (rc == PJ_SUCCESS) {
                    pjsip_warning_hdr* w;
                   ...
                   tsx->status_code / 100 == 2)
        {
            ...
            /* Process session timer response. */
            status =
              handle_timer_response(inv, e->body.tsx_state.src.rdata, PJ_TRUE);
            if (status != PJ_SUCCESS)
                return;
...
static void inv_on_state_disconnected(pjsip_inv_session* inv, pjsip_event* e)
{
    ...
    if (tsx->role == PJSIP_ROLE_UAS && tsx->status_code < 200 &&
        e->body.tsx_state.type == PJSIP_EVENT_RX_MSG)
    {
        pjsip_rx_data* rdata = e->body.tsx_state.src.rdata;
