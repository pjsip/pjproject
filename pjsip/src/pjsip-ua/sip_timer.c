...
const pjsip_method pjsip_update_method = { PJSIP_OTHER_METHOD,
                                           { "UPDATE", 6 } };
/*
 * Session-Expires header vptr.
 */
static int se_hdr_print(pjsip_sess_expires_hdr* hdr, char* buf, pj_size_t size);
...
static pjsip_sess_expires_hdr* se_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_sess_expires_hdr* hdr);
 ...
 * Min-SE header vptr.
 */
static int min_se_hdr_print(pjsip_min_se_hdr* hdr, char* buf, pj_size_t size);
static pjsip_min_se_hdr* min_se_hdr_clone(pj_pool_t* pool,
                                          const pjsip_min_se_hdr* hdr);
static pjsip_min_se_hdr* min_se_hdr_shallow_clone(pj_pool_t* pool,
                                                  const pjsip_min_se_hdr* hdr);
...
static int se_hdr_print(pjsip_sess_expires_hdr* hdr, char* buf, pj_size_t size)
{
    ...
    *p++ = ' ';
    ...
    printed =
      pjsip_param_print_on(&hdr->other_param, p, endbuf - p,
...
static pjsip_sess_expires_hdr* se_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_sess_expires_hdr* hsrc)
{
...
static void timer_cb(pj_timer_heap_t* timer_heap, struct pj_timer_entry* entry)
{
    ...
    if (as_refresher && (entry_id != REFRESHER_EXPIRE_TIMER_ID)) {
        ...
        if (inv->timer->use_update) {
            ...
            if (status == PJ_SUCCESS) {
                status = pjsip_create_sdp_body(
                  tdata->pool, (pjmedia_sdp_session*)offer, &body);
                tdata->msg->body = body;
            }
        }
        ...
        pj_gettimeofday(&now);
        PJ_LOG(
          4, (obj_name, "Refreshing session after %ds (expiration period=%ds)",
        ...
        status =
          pjsip_inv_end_session(inv, PJSIP_SC_REQUEST_TIMEOUT, NULL, &tdata);
    ...
    if (status != PJ_SUCCESS) {
        PJ_PERROR(2, (obj_name, status, "Error in %s session timer",
...
static void start_timer(pjsip_inv_session* inv)
{
    ...
    inv->timer->use_update =
      (pjsip_dlg_remote_has_cap(inv->dlg, PJSIP_H_ALLOW, NULL, &UPDATE) ==
    ...
    pj_timer_entry_init(&timer->timer, 1, /* id */
                        inv,              /* user data */
                        timer_cb);        /* callback */
        ...
        (timer->refresher == TR_UAS && inv->timer->role == PJSIP_ROLE_UAS))
    {
         ...
         */
        delay.sec =
          timer->setting.sess_expires - timer->setting.sess_expires / 3;
...
PJ_DEF(pj_status_t) pjsip_timer_init_module(pjsip_endpoint* endpt)
{
    ...
    status =
      pjsip_register_hdr_parser(STR_SE.ptr, STR_SHORT_SE.ptr, &parse_hdr_se);
    if (status != PJ_SUCCESS)
        return status;

    /* Register Min-SE header parser */
    status = pjsip_register_hdr_parser(STR_MIN_SE.ptr, NULL, &parse_hdr_min_se);
    if (status != PJ_SUCCESS)
        return status;

    /* Register 'timer' capability to endpoint */
    status = pjsip_endpt_add_capability(endpt, NULL, PJSIP_H_SUPPORTED, NULL, 1,
                                        &STR_TIMER);
    if (status != PJ_SUCCESS)
        return status;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_timer_init_session(pjsip_inv_session* inv,
                         const pjsip_timer_setting* setting)
{
    ...
    if (setting) {
        PJ_ASSERT_RETURN(setting->min_se >= ABS_MIN_SE, PJ_ETOOSMALL);
        PJ_ASSERT_RETURN(setting->sess_expires >= setting->min_se, PJ_EINVAL);
 ...
 */
PJ_DEF(pjsip_sess_expires_hdr*) pjsip_sess_expires_hdr_create(pj_pool_t* pool)
{
    pjsip_sess_expires_hdr* hdr =
      PJ_POOL_ZALLOC_T(pool, pjsip_sess_expires_hdr);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_timer_update_req(pjsip_inv_session* inv, pjsip_tx_data* tdata)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_timer_process_resp(pjsip_inv_session* inv, const pjsip_rx_data* rdata,
                         pjsip_status_code* st_code)
{
    ...
    if (msg->line.status.code == PJSIP_SC_SESSION_TIMER_TOO_SMALL) {
        ...
        min_se_hdr =
          (pjsip_min_se_hdr*)pjsip_msg_find_hdr_by_name(msg, &STR_MIN_SE, NULL);
        if (min_se_hdr == NULL) {
        ...
        inv->timer->setting.min_se =
          PJ_MAX(min_se_hdr->min_se, inv->timer->setting.min_se);
        ...
        hdr =
          (pjsip_hdr*)pjsip_msg_find_hdr_by_name(tdata->msg, &STR_MIN_SE, NULL);
        if (hdr != NULL)
            pj_list_erase(hdr);
        ...
        if (se_hdr == NULL) {
            ...
            if (inv->options & PJSIP_INV_REQUIRE_TIMER) {
                ...
                pjsip_timer_end_session(inv);
                return PJSIP_ERRNO_FROM_SIP_STATUS(PJSIP_SC_EXTENSION_REQUIRED);
            }
         ...
         * Min-SE.
         */
        if (se_hdr && se_hdr->sess_expires < inv->timer->setting.min_se) {
...
pjsip_timer_handle_refresh_error(pjsip_inv_session* inv, pjsip_event* event)
{
    ...
    if (inv->timer && inv->timer->active) {
            ...
            (inv->timer->refresh_tdata == event->body.tsx_state.tsx->last_tx))
        {
            ...
            st_code = (pjsip_status_code)event->body.tsx_state.tsx->status_code;
            ...
            if (st_code == 503 && PJSIP_SESS_TIMER_RETRY_DELAY >= 0) {
                ...
                PJ_LOG(3, (inv->pool->obj_name,
                           "Scheduling to retry refresh "
                           "request after %d second(s)",
                           delay.sec));
                ...
                status = pjsip_inv_end_session(
                  inv, event->body.tsx_state.tsx->status_code,
                  pjsip_get_status_text(st_code), &bye);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_timer_process_req(pjsip_inv_session* inv, const pjsip_rx_data* rdata,
                        pjsip_status_code* st_code)
{
    pjsip_min_se_hdr* min_se_hdr;
    pjsip_sess_expires_hdr* se_hdr;
    ...
    if (se_hdr && pj_stricmp(&se_hdr->refresher, &STR_UAC) == 0)
        ...
        if (inv->timer->refresher != TR_UNKNOWN) {
            ...
            inv->timer->refresher =
              (uac_supports_timer && se_hdr) ? TR_UAC : TR_UAS;
        }
    }
 ...
 */
PJ_DEF(pj_status_t)
pjsip_timer_update_resp(pjsip_inv_session* inv, pjsip_tx_data* tdata)
{
    pjsip_msg* msg;
    ...
    if (msg->line.status.code / 100 == 2) {
        if (inv->timer && inv->timer->active) {
            ...
            if (inv->timer->refresher == TR_UAC) {
                ...
                req_hdr = (pjsip_require_hdr*)pjsip_msg_find_hdr(
                  tdata->msg, PJSIP_H_REQUIRE, NULL);
                if (req_hdr == NULL) {
