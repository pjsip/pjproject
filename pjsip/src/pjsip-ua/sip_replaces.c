 ...
 */
static int replaces_hdr_print(pjsip_replaces_hdr* hdr, char* buf,
                              pj_size_t size);
static pjsip_replaces_hdr* replaces_hdr_clone(pj_pool_t* pool,
                                              const pjsip_replaces_hdr* hdr);
static pjsip_replaces_hdr* replaces_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_replaces_hdr*);
...
static pjsip_replaces_hdr* replaces_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_replaces_hdr* rhs)
{
...
PJ_DEF(pj_status_t) pjsip_replaces_init_module(pjsip_endpoint* endpt)
{
    ...
    status = pjsip_register_hdr_parser("Replaces", NULL, &parse_hdr_replaces);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_replaces_verify_request(pjsip_rx_data* rdata, pjsip_dialog** p_dlg,
                              pj_bool_t lock_dlg, pjsip_tx_data** p_tdata)
{
    const pj_str_t STR_REPLACES = { "Replaces", 8 };
    ...
    *p_dlg = NULL;
    ...
    rep_hdr = (pjsip_replaces_hdr*)pjsip_msg_find_hdr_by_name(
      rdata->msg_info.msg, &STR_REPLACES, NULL);
    if (!rep_hdr) {
    ...
    if (inv->state <= PJSIP_INV_STATE_EARLY && inv->role != PJSIP_ROLE_UAC) {
            ...
            pjsip_cfg()->endpt.accept_replace_in_early_state == PJ_FALSE)
        {
            code = PJSIP_SC_CALL_TSX_DOES_NOT_EXIST;
            warn_text =
              "Found early INVITE session but not initiated by "
              "this UA";
            goto on_return;
        }
    }
...
on_return:
    ...
    if (code != 200) {
        ...
        if (p_tdata) {
            ...
            status =
              pjsip_endpt_create_response(the_endpt, rdata, code, NULL, &tdata);
            ...
            if (warn_text) {
                ...
                warn_hdr = pjsip_warning_hdr_create(
                  tdata->pool, 399, pjsip_endpt_name(the_endpt), &warn_value);
