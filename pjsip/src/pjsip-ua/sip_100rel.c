...
pjsip_prack_method = { PJSIP_OTHER_METHOD, { "PRACK", 5 } };
...
static pj_status_t mod_100rel_load(pjsip_endpoint* endpt)
{
    ...
    pjsip_endpt_add_capability(endpt, &mod_100rel.mod, PJSIP_H_ALLOW, NULL, 1,
                               &pjsip_prack_method.name);
    pjsip_endpt_add_capability(endpt, &mod_100rel.mod, PJSIP_H_SUPPORTED, NULL,
                               1, &tag_100rel);
...
static pjsip_require_hdr* find_req_hdr(pjsip_msg* msg)
{
    ...
    hreq = (pjsip_require_hdr*)pjsip_msg_find_hdr(msg, PJSIP_H_REQUIRE, NULL);
    ...
    while (hreq) {
        ...
        hreq = (pjsip_require_hdr*)pjsip_msg_find_hdr(msg, PJSIP_H_REQUIRE,
                                                      hreq->next);
    }
...
PJ_DEF(pj_bool_t) pjsip_100rel_is_reliable(pjsip_rx_data* rdata)
{
    ...
    return msg->line.status.code > 100 && msg->line.status.code < 200 &&
           rdata->msg_info.require != NULL && find_req_hdr(msg) != NULL;
}
                          ...
                          pjsip_tx_data** p_tdata)
{
    ...
    *p_tdata = NULL;
    ...
    pj_assert(tsx->role == PJSIP_ROLE_UAC &&
              ...
              msg->line.status.code > 100 && msg->line.status.code < 200);

    /* Get the RSeq header */
    rseq_hdr =
      (pjsip_generic_string_hdr*)pjsip_msg_find_hdr_by_name(msg, &RSEQ, NULL);
    if (rseq_hdr == NULL) {
    ...
    /* Create PRACK */
    status =
      pjsip_dlg_create_request(dd->inv->dlg, &pjsip_prack_method, -1, &tdata);
    ...
    if (pj_stricmp(&uac_state->tag, &dd->inv->dlg->remote.info->tag)) {
        ...
        mhdr = (const pjsip_contact_hdr*)pjsip_msg_find_hdr(
        ...
        tdata->msg->line.req.uri =
          (pjsip_uri*)pjsip_uri_clone(tdata->pool, mhdr->uri);
    }

    /* Create RAck header */
    rack.ptr = rack_buf;
    rack.slen =
      pj_ansi_snprintf(rack.ptr, sizeof(rack_buf), "%u %u %.*s", rseq,
                       rdata->msg_info.cseq->cseq, (int)tsx->method.name.slen,
                       tsx->method.name.ptr);
...
pjsip_100rel_send_prack(pjsip_inv_session* inv, pjsip_tx_data* tdata)
{
    ...
    return pjsip_dlg_send_request(inv->dlg, tdata, mod_100rel.mod.id,
                                  (void*)dd);
}
...
static void parse_rack(const pj_str_t* rack, pj_uint32_t* p_rseq,
                       pj_int32_t* p_seq, pj_str_t* p_method)
{
...
pjsip_100rel_on_rx_prack(pjsip_inv_session* inv, pjsip_rx_data* rdata)
{
    ...
    if (dd == NULL) {
        ...
        status =
          pjsip_dlg_create_response(inv->dlg, rdata, 400, &reason, &tdata);
        if (status == PJ_SUCCESS) {
    ...
    if (dd->uas_state == NULL || pj_list_empty(&dd->uas_state->tx_data_list)) {
        PJ_LOG(4,
               (dd->inv->dlg->obj_name, "PRACK ignored - no pending response"));
        return PJ_EIGNORED;
    }

    /* Find RAck header */
    rack_hdr =
      (pjsip_generic_string_hdr*)pjsip_msg_find_hdr_by_name(msg, &RACK, NULL);
    if (!rack_hdr) {
                          ...
                          struct pj_timer_entry* entry)
{
    ...
    if (dd->uas_state->retransmit_count >= 7) {
        ...
        */
        ...
        if (status == PJ_SUCCESS && tdata) {
            pjsip_dlg_send_response(dd->inv->dlg, dd->inv->invite_tsx, tdata);
        }
        return;
    }
    ...
    if (dd->uas_state->retransmit_count == 1) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(3, (THIS_FILE, status, "Failed to send message"));
            return;
        }
    } else {
    ...
    if (dd->uas_state->retransmit_count < 6) {
        delay.sec = 0;
        delay.msec =
          (1 << dd->uas_state->retransmit_count) * (long)pjsip_cfg()->tsx.t1;
    ...
    pjsip_endpt_schedule_timer(dd->inv->dlg->endpt,
                               &dd->uas_state->retransmit_timer, &delay);
...
pjsip_100rel_tx_response(pjsip_inv_session* inv, pjsip_tx_data* tdata)
{
    ...
    PJ_ASSERT_RETURN(cseq_hdr->method.id == PJSIP_INVITE_METHOD, PJ_EINVALIDOP);
    ...
    rseq_hdr = (pjsip_generic_string_hdr*)pjsip_msg_find_hdr_by_name(
      tdata->msg, &RSEQ, NULL);
    if (rseq_hdr)
    ...
    if (status_code / 100 == 2) {
        ...
        */
        ...
        if (dd->uas_state == NULL) {
            dd->uas_state = PJ_POOL_ZALLOC_T(inv->dlg->pool, uas_state_t);
        ...
        PJ_ASSERT_RETURN(cseq_hdr->cseq == dd->uas_state->cseq, PJ_EINVALIDOP);
        ...
        /* Add RSeq header */
        pj_ansi_snprintf(rseq_str, sizeof(rseq_str), "%u", dd->uas_state->rseq);
        rseq = pj_str(rseq_str);
        rseq_hdr = pjsip_generic_string_hdr_create(tdata->pool, &RSEQ, &rseq);
        ...
        if (!pj_list_empty(&dd->uas_state->tx_data_list)) {
            ...
            PJ_LOG(4, (dd->inv->dlg->obj_name,
                       "Reliable %d response enqueued (%d pending)", code,
