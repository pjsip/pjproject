...
pjsip_refer_method = { (pjsip_method_e)PJSIP_REFER_METHOD, { "REFER", 5 } };
...
static void xfer_on_evsub_tsx_state(pjsip_evsub* sub, pjsip_transaction* tsx,
                                    pjsip_event* event);
static void xfer_on_evsub_rx_refresh(pjsip_evsub* sub, pjsip_rx_data* rdata,
                                     int* p_st_code, pj_str_t** p_st_text,
                                     pjsip_hdr* res_hdr,
                                     pjsip_msg_body** p_body);
static void xfer_on_evsub_rx_notify(pjsip_evsub* sub, pjsip_rx_data* rdata,
                                    int* p_st_code, pj_str_t** p_st_text,
                                    pjsip_hdr* res_hdr,
                                    pjsip_msg_body** p_body);
...
static pjsip_evsub_user xfer_user = {
    &xfer_on_evsub_state,          &xfer_on_evsub_tsx_state,
    &xfer_on_evsub_rx_refresh,     &xfer_on_evsub_rx_notify,
    &xfer_on_evsub_client_refresh, &xfer_on_evsub_server_timeout,
};
...
PJ_DEF(pj_status_t) pjsip_xfer_init_module(pjsip_endpoint* endpt)
{
    ...
    status = pjsip_endpt_add_capability(endpt, &mod_xfer, PJSIP_H_ALLOW, NULL,
                                        1, &pjsip_get_refer_method()->name);
    if (status != PJ_SUCCESS)
        return status;

    status = pjsip_evsub_register_pkg(&mod_xfer, &STR_REFER, PJSIP_XFER_EXPIRES,
                                      1, &accept);
    if (status != PJ_SUCCESS)
        return status;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_xfer_create_uac(pjsip_dialog* dlg, const pjsip_evsub_user* user_cb,
                      pjsip_evsub** p_evsub)
{
    pj_status_t status;
    pjsip_xfer* xfer;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_xfer_create_uas(pjsip_dialog* dlg, const pjsip_evsub_user* user_cb,
                      pjsip_rx_data* rdata, pjsip_evsub** p_evsub)
{
     ...
     */
    if (pjsip_msg_find_hdr_by_name(rdata->msg_info.msg, &STR_EVENT, NULL) ==
        NULL) {
 ...
 */
PJ_DEF(pj_status_t)
pjsip_xfer_initiate(pjsip_evsub* sub, const pj_str_t* refer_to_uri,
                    pjsip_tx_data** p_tdata)
{
    ...
    hdr = pjsip_generic_string_hdr_create(tdata->pool, &refer_to, refer_to_uri);
    if (!hdr) {
 ...
 */
PJ_DEF(pj_status_t)
pjsip_xfer_accept(pjsip_evsub* sub, pjsip_rx_data* rdata, int st_code,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_xfer_notify(pjsip_evsub* sub, pjsip_evsub_state state, int xfer_st_code,
                  const pj_str_t* xfer_st_text, pjsip_tx_data** p_tdata)
{
    pjsip_tx_data* tdata;
    pjsip_xfer* xfer;
    pjsip_param* param;
    ...
    bodylen = pj_ansi_snprintf(body, 128, "SIP/2.0 %u %.*s\r\n", xfer_st_code,
                               (int)xfer_st_text->slen, xfer_st_text->ptr);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_xfer_current_notify(pjsip_evsub* sub, pjsip_tx_data** p_tdata)
{
    pjsip_xfer* xfer;
    pj_status_t status;
    ...
    status =
      pjsip_xfer_notify(sub, pjsip_evsub_get_state(sub), xfer->last_st_code,
                        &xfer->last_st_text, p_tdata);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_xfer_send_request(pjsip_evsub* sub, pjsip_tx_data* tdata)
{
                                    ...
                                    pjsip_hdr* res_hdr, pjsip_msg_body** p_body)
{
    ...
    if (xfer->user_cb.on_rx_notify)
        (*xfer->user_cb.on_rx_notify)(sub, rdata, p_st_code, p_st_text, res_hdr,
                                      p_body);
}
...
static void xfer_on_evsub_server_timeout(pjsip_evsub* sub)
{
    ...
    if (xfer->user_cb.on_server_timeout) {
        ...
        status =
          pjsip_xfer_notify(sub, PJSIP_EVSUB_STATE_TERMINATED,
                            xfer->last_st_code, &xfer->last_st_text, &tdata);
        if (status == PJ_SUCCESS)
            pjsip_xfer_send_request(sub, tdata);
    }
}
