...
static int uac_tsx_bench(unsigned working_set, pj_timestamp* p_elapsed)
{
    ...
    via = (pjsip_via_hdr*)pjsip_msg_find_hdr(request->msg, PJSIP_H_VIA, NULL);

    /* Create transaction array */
    tsx = (pjsip_transaction**)pj_pool_zalloc(request->pool,
                                              working_set * sizeof(pj_pool_t*));
...
static int uas_tsx_bench(unsigned working_set, pj_timestamp* p_elapsed)
{
    ...
    for (i = 0; i < working_set; ++i) {
        ...
        via->branch_param.slen =
          ...
          pj_ansi_sprintf(branch_buf + PJSIP_RFC3261_BRANCH_LEN, "-%d", i);
...
int tsx_bench(void)
{
    ...
    pj_ansi_sprintf(desc, "Time to create %d UAC transactions, in miliseconds",
                    WORKING_SET);
    report_ival("create-uac-time", (unsigned)(min.u64 * 1000 / freq.u64),
                "msec", desc);
    ...
    report_ival("create-uac-tsx-per-sec", speed, "tsx/sec", desc);
    ...
    pj_ansi_sprintf(desc, "Time to create %d UAS transactions, in miliseconds",
                    WORKING_SET);
    report_ival("create-uas-time", (unsigned)(min.u64 * 1000 / freq.u64),
                "msec", desc);
    ...
    report_ival("create-uas-tsx-per-sec", speed, "tsx/sec", desc);
