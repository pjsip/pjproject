...
typedef struct inv_test_t
{
    ...
    pj_bool_t complete;
    pj_bool_t uas_complete, uac_complete;

    unsigned oa_index;
    unsigned uac_update_cnt, uas_update_cnt;
} inv_test_t;
...
static void on_media_update(pjsip_inv_session* inv_ses, pj_status_t status)
{
    ...
    if (inv_test.uac_update_cnt == inv_test.uas_update_cnt) {
        ...
        if (inv_test.param.count == 1 && inv_test.param.oa[0] == OFFERER_UAC &&
            inv_test.param.need_established)
        {
            jobs[job_cnt].type = ESTABLISH_CALL;
            jobs[job_cnt].who = PJSIP_ROLE_UAS;
...
static pj_bool_t on_rx_request(pjsip_rx_data* rdata)
{
        ...
        rdata->msg_info.msg->line.req.method.id == PJSIP_INVITE_METHOD)
    {
        ...
        status = pjsip_inv_create_uas(dlg, rdata, sdp,
                                      inv_test.param.inv_option, &inv_test.uas);
        pj_assert(status == PJ_SUCCESS);
         ...
         * Answer with 183
         */
        status = pjsip_inv_initial_answer(inv_test.uas, rdata, 183, NULL, NULL,
                                          &tdata);
        pj_assert(status == PJ_SUCCESS);
        ...
        if (sdp && inv_test.param.multipart_body) {
            status = pjsip_create_multipart_sdp_body(
              tdata->pool, pjmedia_sdp_session_clone(tdata->pool, sdp),
              &tdata->msg->body);
        }
        pj_assert(status == PJ_SUCCESS);
...
static int perform_test(inv_test_param_t* param)
{
     ...
     */
    status =
      pjsip_dlg_create_uac(pjsip_ua_instance(), &uri, &uri, &uri, &uri, &dlg);
    ...
    status =
      pjsip_inv_create_uac(dlg, sdp, inv_test.param.inv_option, &inv_test.uac);
    // inv_test.uac->create_multipart = param->multipart_body;
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, -20);

    TRACE_(
      (THIS_FILE, "    Sending INVITE %s offer", (sdp ? "with" : "without")));
...
static pj_bool_t log_on_rx_msg(pjsip_rx_data* rdata)
{
    ...
    if (msg->type == PJSIP_REQUEST_MSG)
        ...
        pj_ansi_snprintf(info, sizeof(info), "%d/%.*s", msg->line.status.code,
 ...
 */
#if 1
    { "Standard INVITE with offer", 0, PJ_TRUE, 1, { OFFERER_UAC }, PJ_FALSE },
 ...
 */
#if 1
    { "INVITE with no offer", 0, PJ_TRUE, 1, { OFFERER_UAS }, PJ_FALSE },
