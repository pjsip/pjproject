...
static pj_status_t multi_listener_test(pjsip_tpfactory* factory[],
                                       ...
                                       pjsip_transport* tp[], unsigned* num_tp)
{
    pj_status_t status;
    unsigned i = 0;
    pj_str_t s;
    ...
    status = pjsip_endpt_acquire_transport(
      endpt, PJSIP_TRANSPORT_TCP, &rem_addr, sizeof(rem_addr), NULL, &tcp);
    ...
    status = pjsip_endpt_acquire_transport(
      endpt, PJSIP_TRANSPORT_TCP, &rem_addr, sizeof(rem_addr), &tp_sel, &tcp);
    if (status != PJ_SUCCESS) {
        app_perror("   Error: unable to acquire TCP transport", status);
        return -18;
    }
...
int transport_tcp_test(void)
{
    ...
    pj_ansi_sprintf(
      url, "sip:alice@%s:%d;transport=tcp",
      pj_inet_ntop2(pj_AF_INET(), &rem_addr.sin_addr, addr, sizeof(addr)),
    ...
    for (i = 0; i < SEND_RECV_LOOP; ++i) {
        status =
          transport_send_recv_test(PJSIP_TRANSPORT_TCP, tcp[0], url, &rtt[i]);
