...
static pj_status_t multi_transport_test(pjsip_transport* tp[], unsigned num_tp)
{
    ...
    status = pjsip_endpt_acquire_transport(
      endpt, PJSIP_TRANSPORT_UDP, &rem_addr, sizeof(rem_addr), NULL, &udp_tp);
    if (status != PJ_SUCCESS)
        return -140;
    ...
    pj_sockaddr_in_init(&rem_addr, pj_cstr(&s, "1.1.1.1"), 80);
    status =
      pjsip_endpt_acquire_transport(endpt, PJSIP_TRANSPORT_UDP, &rem_addr,
                                    sizeof(rem_addr), &tp_sel, &udp_tp);
    if (status != PJ_SUCCESS)
        return -170;
...
int transport_udp_test(void)
{
    ...
    for (i = 0; i < SEND_RECV_LOOP; ++i) {
        status = transport_send_recv_test(
          PJSIP_TRANSPORT_UDP, tp[0], "sip:alice@127.0.0.1:" TEST_UDP_PORT_STR,
          &rtt[i]);
        if (status != 0)
            return status;
    }
    ...
    status =
      transport_rt_test(PJSIP_TRANSPORT_UDP, tp[0],
                        "sip:alice@127.0.0.1:" TEST_UDP_PORT_STR, &pkt_lost);
    if (status != 0)
        return status;
