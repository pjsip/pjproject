...
static int tsx_create_and_send_req(void* arg)
{
    ...
    status = pjsip_endpt_create_request(endpt, &pjsip_options_method, &dst_uri,
                                        &from_uri, &dst_uri, NULL, NULL, -1,
                                        NULL, &tdata);
    if (status != PJ_SUCCESS)
        return status;
...
int tsx_destroy_test()
{
    ...
    struct test_desc
    {
        ...
        { "normal unable to resolve", &tsx_create_and_send_req,
          "sip:user@somehost", 20000, 1 },
        { "resolve and destroy, wait", &tsx_create_and_send_req,
          "sip:user@somehost", 1, 20000 },
        { "tcp connect and destroy", &tsx_create_and_send_req,
          "sip:user@10.125.36.63:58517;transport=tcp", 60000, 1000 },
        { "tcp connect and destroy 2", &tsx_create_and_send_req,
          "sip:user@10.125.36.63:58517;transport=tcp", 1, 60000 },
    };
    int rc;
    unsigned i;
    const int INDENT = 2;
