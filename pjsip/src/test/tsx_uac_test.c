    ...
    if (pj_stricmp2(&tsx->branch, TEST1_BRANCH_ID) == 0) {
        ...
        if (tsx->state == PJSIP_TSX_STATE_TERMINATED) {
            ...
            if (tsx->status_code != PJSIP_SC_TSX_TIMEOUT) {
                PJ_LOG(3,
                       (THIS_FILE, "    error: status code is %d instead of %d",
            ...
            if (tp_flag & PJSIP_TRANSPORT_RELIABLE) {
                ...
                if (tsx->method.id == PJSIP_INVITE_METHOD && recv_count < 6) {
                    ...
                    // if (tsx->method.id==PJSIP_OPTIONS_METHOD && recv_count !=
                    // 11) {
        ...
        /*
         * Transaction with TEST2_BRANCH_ID should terminate with transport
         * error.
         */
        if (tsx->state == PJSIP_TSX_STATE_TERMINATED) {
            /* Test the status code. */
        ...
        if (tsx->state == PJSIP_TSX_STATE_COMPLETED) {
            ...
            if (tsx->last_tx == NULL) {
                PJ_LOG(3, (THIS_FILE, "    error: transaction lost last_tx"));
                test_complete = -738;
            }
        ...
        if (tsx->state == PJSIP_TSX_STATE_COMPLETED) {
            ...
            if (e->body.tsx_state.prev_state != PJSIP_TSX_STATE_PROCEEDING) {
                PJ_LOG(3,
                       (THIS_FILE, "    error: prev state is %s instead of %s",
                        pjsip_tsx_state_str(
                          (pjsip_tsx_state_e)e->body.tsx_state.prev_state),
        ...
        if (tsx->state == PJSIP_TSX_STATE_COMPLETED) {
             ...
             */
            if (tsx->last_tx &&
                tsx->last_tx->msg->line.req.method.id != PJSIP_INVITE_METHOD)
            {
                PJ_LOG(3, (THIS_FILE, "    error: last_tx is not INVITE"));
                test_complete = -748;
            }
        } else if (tsx->state == PJSIP_TSX_STATE_TERMINATED) {
            test_complete = 1;
...
static pj_bool_t msg_receiver_on_rx_request(pjsip_rx_data* rdata)
{
    if (pj_stricmp2(&rdata->msg_info.via->branch_param, TEST1_BRANCH_ID) == 0) {
        ...
        if (recv_count == 0) {
            ...
            if (recv_count > max_received) {
                PJ_LOG(3,
                       (THIS_FILE, "    error: too many messages (%d) received",
                        recv_count));
                test_complete = -620;
            }
    ...
    } else if (pj_stricmp2(&rdata->msg_info.via->branch_param,
                           TEST4_BRANCH_ID) == 0)
    {
        /*
                           ...
                           TEST6_BRANCH_ID) == 0)
    {
        ...
        status =
          pjsip_endpt_respond_stateless(endpt, rdata, 202, NULL, NULL, NULL);
        if (status != PJ_SUCCESS) {
                           ...
                           TEST9_BRANCH_ID) == 0)
    {
        ...
        if (method->id == PJSIP_INVITE_METHOD) {
            ...
            /* Respond with provisional response */
            status =
              pjsip_endpt_create_response(endpt, rdata, 100, NULL, &tdata);
            pj_assert(status == PJ_SUCCESS);
            ...
            status =
              pjsip_endpt_send_response(endpt, &res_addr, tdata, NULL, NULL);
            pj_assert(status == PJ_SUCCESS);

            /* Create the final response. */
            status =
              pjsip_endpt_create_response(endpt, rdata, 302, NULL, &tdata);
            pj_assert(status == PJ_SUCCESS);
...
static int tsx_uac_retransmit_test(void)
{
    ...
    struct
    {
        const pjsip_method* method;
        unsigned delay;
    } sub_test[] = { { &pjsip_invite_method, 0 },
    ...
    for (i = 0; i < (int)PJ_ARRAY_SIZE(sub_test); ++i) {
        PJ_LOG(3, (THIS_FILE, "   variant %c: %s with %d ms network delay",
                   ('a' + i), sub_test[i].method->name.ptr, sub_test[i].delay));
        ...
        /* Do the test. */
        status = perform_tsx_test(-500, TARGET_URI, FROM_URI, TEST1_BRANCH_ID,
...
static int tsx_resolve_error_test(void)
{
    ...
    status = perform_tsx_test(-800, "sip:bob@unresolved-host", FROM_URI,
                              TEST2_BRANCH_ID, 20, &pjsip_options_method);
    if (status != 0)
        return status;
    ...
    if (test_param->type == PJSIP_TRANSPORT_LOOP_DGRAM) {
        ...
        status = perform_tsx_test(-800, TARGET_URI, FROM_URI, TEST2_BRANCH_ID,
...
static int tsx_terminate_resolving_test(void)
{
    ...
    /* Start the test. */
    status = perform_tsx_test(-900, TARGET_URI, FROM_URI, TEST3_BRANCH_ID, 2,
...
static int tsx_retransmit_fail_test(void)
{
    ...
    for (i = 0; i < (int)PJ_ARRAY_SIZE(delay); ++i) {
        PJ_LOG(3, (THIS_FILE, "   variant %c: transport delay %d ms", ('a' + i),
                   delay[i]));
        ...
        /* Start the test. */
        status = perform_tsx_test(-1000, TARGET_URI, FROM_URI, TEST4_BRANCH_ID,
...
static int tsx_terminate_after_retransmit_test(void)
{
    ...
    /* Do the test. */
    status = perform_tsx_test(-1100, TARGET_URI, FROM_URI, TEST5_BRANCH_ID, 6,
                              &pjsip_options_method);
 ...
 *****************************************************************************
 */
static int perform_generic_test(const char* title, char* branch_id,
                                const pjsip_method* method)
{
    int i, status = 0;
    ...
    for (i = 0; i < (int)PJ_ARRAY_SIZE(delay); ++i) {
        ...
        status =
          perform_tsx_test(-1200, TARGET_URI, FROM_URI, branch_id, 10, method);
        if (status != 0)
            return status;
...
int tsx_uac_test(struct tsx_test_param* param)
{
    ...
    /* Get transport flag */
    tp_flag = pjsip_transport_get_flag_from_type(
      (pjsip_transport_type_e)test_param->type);
