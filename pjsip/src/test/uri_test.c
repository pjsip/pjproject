...
struct uri_test
{
    ...
    { PJ_SUCCESS, "sip:localhost", &create_uri0 },
    { PJ_SUCCESS, "sip:user@localhost", &create_uri1 },
    {
      PJ_SUCCESS,
    ...
    { /* Port is specified should not match unspecified port. */
      ERR_NOT_EQUAL, "sip:localhost:5060", &create_uri3 },
    { /* All recognized parameters. */
      PJ_SUCCESS,
      "sip:localhost;transport=tcp;user=ip;ttl=255;lr;maddr=127.0.0.1;method="
      "ACK",
      &create_uri4 },
    ...
    { /* Name address */
      PJ_SUCCESS, "<sip:localhost>", &create_uri7 },
    { /* Name address with display name and SIPS scheme with some redundant
       * whitespaced.
       */
      PJ_SUCCESS, "  Power Administrator  <sips:localhost>", &create_uri8 },
    { /* Name address. */
      PJ_SUCCESS, " \"User\" <sip:user@localhost:5071>", &create_uri9 },
    {
    ...
    { /* 25: Simple tel: URI with global context */
      PJ_SUCCESS, "tel:+1-201-555-0123", &create_uri25, "tel:+1-201-555-0123" },
    { /* 26: Simple tel: URI with local context */
      PJ_SUCCESS, "tel:7042;phone-context=example.com", &create_uri26,
      "tel:7042;phone-context=example.com" },
    { /* 27: Simple tel: URI with local context */
      PJ_SUCCESS, "tel:863-1234;phone-context=+1-914-555", &create_uri27,
      "tel:863-1234;phone-context=+1-914-555" },
    { /* 28: Comparison between local and global number */
      ERR_NOT_EQUAL, "tel:+1", &create_uri28, "tel:+1" },
    { /* 29: tel: with some visual chars and spaces */
      PJ_SUCCESS, "tel:(44).1234-*#+Deaf", &create_uri29,
      "tel:(44).1234-*#+Deaf" },
    { /* 30: isub parameters */
      PJ_SUCCESS, "tel:+1;isub=/:@&$,-_.!~*'()[]/:&$aA1%21+=", &create_uri30,
      "tel:+1;isub=/:@&$,-_.!~*'()[]/:&$aA1!+%3d" },
    { /* 31: extension number parsing and encoding */
      PJ_SUCCESS, "tel:+1;ext=+123", &create_uri31, "tel:+1;ext=%2b123" },
    { /* 32: context parameter parsing and encoding */
      PJ_SUCCESS, "tel:911;phone-context=+1-911", &create_uri32,
      "tel:911;phone-context=+1-911" },
    { /* 33: case-insensitive comparison */
      PJ_SUCCESS, "tel:911;phone-context=emergency.example.com", &create_uri33,
      "tel:911;phone-context=emergency.example.com" },
    { /* 34: parameter only appears in one URL */
      ERR_NOT_EQUAL, "tel:911;p1=p1;p2=p2", &create_uri34,
      "tel:911;p1=p1;p2=p2" },
    { /* 35: IPv6 in host and maddr parameter */
      PJ_SUCCESS, "sip:user@[::1];maddr=[::01]", &create_uri35,
      "sip:user@[::1];maddr=[::01]" },
    { /* 36: IPv6 in host and maddr, without username */
      PJ_SUCCESS, "sip:[::1];maddr=[::01]", &create_uri36,
      "sip:[::1];maddr=[::01]" },
    { /* 37: Non-ASCII UTF-8 in display name, with quote */
      PJ_SUCCESS, "\"\xC0\x81\" <sip:localhost>", &create_uri37,
      "\"\xC0\x81\" <sip:localhost>" },
    { /* 38: Non-ASCII UTF-8 in display name, without quote */
      PJ_SUCCESS, "\xC0\x81 <sip:localhost>", &create_uri38,
      "\"\xC0\x81\" <sip:localhost>" },
    {
...
static pjsip_uri* create_uri5(pj_pool_t* pool)
{
    ...
    // pj_strdup2(pool, &url->other_param,
    // ";pickup=hurry;message=I%20am%20sorry");
    param_add(url->other_param, "pickup", "hurry");
    param_add(url->other_param, "message", "I am sorry");

    // pj_strdup2(pool, &url->header_param,
    // "?Subject=Hello%20There&Server=SIP%20Server");
...
{
    /* "This is -. !% *_+`'~ me
     * <sip:a19A&=+$,;?/%2c:%40a&Zz=+$,@my_proxy09.my-domain.com:9801>" */
    pjsip_name_addr* name_addr = pjsip_name_addr_create(pool);
    pjsip_sip_uri* url;
...
static pjsip_uri* create_uri17(pj_pool_t* pool)
{
    /* "sip:host;user=ip;" PARAM_CHAR "%21=" PARAM_CHAR
     * "%21;lr;other=1;transport=sctp;other2" */
    pjsip_sip_uri* url;
    url = pjsip_sip_uri_create(pool, 0);
    pj_strdup2(pool, &url->host, "host");
    pj_strdup2(pool, &url->user_param, "ip");
...
static pj_status_t do_uri_test(pj_pool_t* pool, struct uri_test* entry)
{
    ...
    pj_get_timestamp(&t1);
    len = pjsip_uri_print(PJSIP_URI_IN_OTHER, parsed_uri, s1.ptr,
                          PJSIP_MAX_URL_SIZE);
    if (len < 1) {
        status = -20;
        goto on_return;
    ...
    len =
      pjsip_uri_print(PJSIP_URI_IN_OTHER, ref_uri, s2.ptr, PJSIP_MAX_URL_SIZE);
    if (len < 1) {
        status = -30;
        goto on_return;
    }
...
static int simple_uri_test(void)
{
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(uri_test_array); ++i) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_LOG(3,
                   (THIS_FILE, "  error %d when testing entry %d", status, i));
            return status;
        }
    }
...
static int uri_benchmark(unsigned* p_parse, unsigned* p_print, unsigned* p_cmp)
{
    ...
    PJ_LOG(3, (THIS_FILE,
               ...
               elapsed.sec, elapsed.msec, (unsigned)avg_parse));
    ...
    *p_parse = (unsigned)avg_parse;
    ...
    PJ_LOG(3, (THIS_FILE,
               ...
               elapsed.sec, elapsed.msec, (unsigned)avg_print));
    ...
    *p_print = (unsigned)avg_print;
    ...
    PJ_LOG(3, (THIS_FILE,
               ...
               (unsigned)(var.cmp_len / 1000000), (unsigned)kbytes, elapsed.sec,
               elapsed.msec, (unsigned)avg_cmp));
...
int uri_test(void)
{
    ...
    report_ival("uri-parse-bandwidth-mb", (int)avg_len * max / 1000000,
                "MB/sec",
