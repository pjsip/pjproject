...
struct test_msg
{
    ...
    { /* 'Normal' message with all headers. */
      ...
      "from: Hi I'm Joe "
      "<sip:joe.user@bar.otherdomain.com>;tag=123457890123456\r"
      ...
      "\r\n",
      &create_msg0, 0, PJ_SUCCESS },
    { /* Typical response message. */
      "SIP/2.0 200 OK\r\n"
      "Via: SIP/2.0/SCTP "
      "server10.biloxi.com;branch=z9hG4bKnashds8;rport;received=192.0.2.1\r\n"
      "Via: SIP/2.0/UDP "
      "bigbox3.site3.atlanta.com;branch=z9hG4bK77ef4c2312983.1;received=192.0."
      "2.2\r\n"
    ...
    { /* Torture message from RFC 4475
      ...
      ";;*:&it+has=1,weird!*pas$wo~d_too.(doesn't-it)@example.com SIP/2.0\n"
      ...
      ";fromParam''~+*_!.-%="
      "\"\xD1\x80\xD0\xB0\xD0\xB1\xD0\xBE\xD1\x82\xD0\xB0\xD1\x8E\xD1\x89\xD0"
      "\xB8\xD0\xB9\";tag=_token~1'+`*%!-.\n"
...
parse_msg:
    ...
    if (parsed_msg->type == PJSIP_REQUEST_MSG) {
        ...
        status = pjsip_uri_cmp(PJSIP_URI_IN_REQ_URI, parsed_msg->line.req.uri,
    ...
    str1.slen =
      parsed_msg->body->print_body(parsed_msg->body, msgbuf1, sizeof(msgbuf1));
    if (str1.slen < 1) {
        status = -110;
        goto on_return;
    }
    ...
    str2.slen =
      ref_msg->body->print_body(ref_msg->body, msgbuf2, sizeof(msgbuf2));
    if (str2.slen < 1) {
        status = -120;
        goto on_return;
    }
                         ...
                         unsigned* p_print)
{
    ...
    PJ_LOG(3, (THIS_FILE,
               ...
               elapsed.sec, elapsed.msec, (unsigned)avg_detect));
    *p_detect = (unsigned)avg_detect;
    ...
    PJ_LOG(
      3, (THIS_FILE, "    %u.%u MB parsed in %d.%03ds (avg=%d msg parsing/sec)",
          (unsigned)(var.parse_len / 1000000), (unsigned)kbytes, elapsed.sec,
          elapsed.msec, (unsigned)avg_parse));
    *p_parse = (unsigned)avg_parse;
    ...
    PJ_LOG(
      3, (THIS_FILE, "    %u.%u MB printed in %d.%03ds (avg=%d msg print/sec)",
          (unsigned)(var.print_len / 1000000), (unsigned)kbytes, elapsed.sec,
          elapsed.msec, (unsigned)avg_print));
...
struct hdr_test_t
{
    ...
    { /* Overflowing generic string header */
      ...
      "a, a, a, a, a, a, a, a, a, a, a, a, a, a, a, a, a, a, a, a, "
      "a, a, a, a, a, a, a, a, a, a, a, a, a, a, a, a, a, a, a, a",
      &hdr_test_success, HDR_FLAG_PARSE_FAIL },

    { /* Normal Accept */
      "Accept", NULL, "application/*, text/plain", &hdr_test_accept1 },

    { /* Accept with params */
      "Accept", NULL, "application/*;p1=v1, text/plain", &hdr_test_accept2 },
    ...
    { /* Parameter belong to hparam */
      "Contact", "m", SIMPLE_ADDR_SPEC ";p1=v1", &hdr_test_contact0,
      HDR_FLAG_DONT_PRINT },

    { /* generic-param in Contact header */
      "Contact", "m", NAME_ADDR ";" GENERIC_PARAM, &hdr_test_contact1 },

    { /* q=0 parameter in Contact header */
      "Contact", "m", NAME_ADDR ";q=0", &hdr_test_contact_q0,
      HDR_FLAG_DONT_PRINT },

    { /* q=0.5 parameter in Contact header */
      "Contact", "m", NAME_ADDR ";q=0.5", &hdr_test_contact_q1 },

    { /* q=1 parameter in Contact header */
      "Contact", "m", NAME_ADDR ";q=1", &hdr_test_contact_q2 },

    { /* q=1.0 parameter in Contact header */
      "Contact", "m", NAME_ADDR ";q=1.0", &hdr_test_contact_q3,
      HDR_FLAG_DONT_PRINT },

    { /* q=1.1 parameter in Contact header */
      "Contact", "m", NAME_ADDR ";q=1.15", &hdr_test_contact_q4 },

    { /* Content-Length */
      "Content-Length", "l", "10", &hdr_test_content_length },

    { /* Content-Type, with generic-param */
      "Content-Type", "c",
      "application/sdp"
      ";" GENERIC_PARAM,
      &hdr_test_content_type, HDR_FLAG_DONT_PRINT },

    { /* From, testing parameters and generic-param */
      "From", "f", NAME_ADDR ";" GENERIC_PARAM, &hdr_test_from },
    ...
    { /* Record-Route, param belong to header */
      "Record-Route", NULL, NAME_ADDR ";" GENERIC_PARAM,
    ...
    { /* To */
      "To", "t", NAME_ADDR ";" GENERIC_PARAM, &hdr_test_to },
    ...
    { /* Via with IPv6 */
      "Via", "v", "SIP/2.0/UDP [::1]", &hdr_test_via_ipv6_1 },

    { /* Via with IPv6 */
      "Via", "v", "SIP/2.0/UDP [::1]:5061", &hdr_test_via_ipv6_2 },

    { /* Via with IPv6 */
      "Via", "v", "SIP/2.0/UDP [::1];rport=5061;received=::2",
      &hdr_test_via_ipv6_3 },

    { /* Retry-After header with comment */
      "Retry-After", NULL, "10(Already Pending Register)",
      &hdr_test_retry_after1 },

    { /* Non-ASCII UTF-8 characters in Subject */
      "Subject", NULL, "\xC0\x81", &hdr_test_subject_utf }
};
...
static int hdr_test_content_type(pjsip_hdr* h)
{
    ...
    if (prm == &hdr->media.param)
        return -1940;
    if (pj_strcmp2(&prm->name, "p0"))
        return -1941;
    if (pj_strcmp2(&prm->value, "a"))
        return -1942;

    prm = prm->next;
    if (prm == &hdr->media.param)
        return -1950;
    if (pj_strcmp2(&prm->name, "p1")) {
        PJ_LOG(3, ("", "%.*s", (int)prm->name.slen, prm->name.ptr));
        return -1951;
    }
    if (pj_strcmp2(&prm->value, "\"ab:;cd\"")) {
        PJ_LOG(3, ("", "%.*s", (int)prm->value.slen, prm->value.ptr));
        return -1952;
    }
    ...
    if (prm == &hdr->media.param)
        return -1960;
    if (pj_strcmp2(&prm->name, "p2"))
        return -1961;
    if (pj_strcmp2(&prm->value, "ab%3acd"))
        return -1962;

    prm = prm->next;
    if (prm == &hdr->media.param)
        return -1970;
    if (pj_strcmp2(&prm->name, "p3"))
        return -1971;
    if (pj_strcmp2(&prm->value, ""))
        return -1972;
...
static int hdr_test(void)
{
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(hdr_test_data); ++i) {
        ...
        parsed_hdr1 =
          (pjsip_hdr*)pjsip_parse_hdr(pool, &hname, hcontent, len, &parsed_len);
        if (parsed_hdr1 == NULL) {
            if (test->flags & HDR_FLAG_PARSE_FAIL) {
                pj_pool_release(pool);
                continue;
            }
            PJ_LOG(3, (THIS_FILE, "    error parsing header %s: %s",
                       test->hname, test->hcontent));
            return -500;
        }

        /* Test the parsing result */
        if (test->test && (rc = test->test(parsed_hdr1)) != 0) {
            PJ_LOG(3, (THIS_FILE, "    validation failed for header %s: %s",
                       test->hname, test->hcontent));
...
#    endif
            ...
            parsed_hdr2 = (pjsip_hdr*)pjsip_parse_hdr(pool, &hname, hcontent,
                                                      len, &parsed_len);
            if (parsed_hdr2 == NULL) {
                PJ_LOG(3, (THIS_FILE, "    error parsing header %s: %s",
                           test->hshort_name, test->hcontent));
                return -510;
            }
        }
#endif
        ...
        len =
          pj_ansi_snprintf(input, 1024, "%s: %s", test->hname, test->hcontent);
        if (len < 1 || len >= 1024)
            return -520;
        ...
        if (len < 0 || len >= 1024) {
            PJ_LOG(3, (THIS_FILE, "    header too long: %s: %s", test->hname,
                       test->hcontent));
            return -530;
        }
        output[len] = 0;
...
int msg_test(void)
{
    ...
    pj_ansi_sprintf(desc,
                    ...
                    "%d bytes)",
                    (int)PJ_ARRAY_SIZE(test_array), avg_len);
