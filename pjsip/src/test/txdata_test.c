...
#define HFIND(msg, h, H) \
    ((pjsip_##h##_hdr*)pjsip_msg_find_hdr(msg, PJSIP_H_##H, NULL))
...
static int core_txdata_test(void)
{
    ...
    status =
      pjsip_endpt_create_request(endpt, &pjsip_invite_method, &target, &from,
                                 &to, &contact, NULL, 10, &body, &invite);
    ...
    status = pjsip_endpt_create_request_from_hdr(
      endpt, &pjsip_invite_method, invite->msg->line.req.uri,
      HFIND(invite->msg, from, FROM), HFIND(invite->msg, to, TO),
      HFIND(invite->msg, contact, CONTACT), HFIND(invite->msg, cid, CALL_ID),
      10, &body, &invite2);
    if (status != PJ_SUCCESS) {
    ...
    */
    ...
    status =
      pjsip_endpt_create_response(endpt, &dummy_rdata, 301, NULL, &response);
    if (status != PJ_SUCCESS) {
...
static int gcc_test()
{
    ...
    pj_str_t target = pj_str(
      "sip:alice@wonderland:5061;x-param=param%201"
      "?X-Hdr-1=Header%201"
      "&X-Empty-Hdr=");
    pjsip_tx_data* tdata;
    ...
    status =
      pjsip_endpt_create_request(endpt, &pjsip_invite_method, &target, &target,
                                 &target, &target, NULL, -1, NULL, &tdata);
    if (status != 0) {
        app_perror("   error: Unable to create request", status);
        return -200;
    }
    ...
    PJ_LOG(5, (THIS_FILE,
               "%d bytes request created:--begin-msg--\n"
               "%s\n"
               "--end-msg--",
               len, msgbuf));
    ...
    if (msg == NULL) {
        ...
        while (e != &err_list) {
            PJ_LOG(3, (THIS_FILE, "     %s in line %d col %d hname=%.*s",
                       pj_exception_id_name(e->except_code), e->line,
                       e->col + 1, (int)e->hname.slen, e->hname.ptr));
            e = e->next;
        }
...
static int txdata_test_uri_params(void)
{
     ...
     */
    pj_ansi_snprintf(msgbuf, sizeof(msgbuf), "<%.*s>", (int)target.slen,
                     target.ptr);
    contact.ptr = msgbuf;
    ...
    status =
      pjsip_endpt_create_request(endpt, &pjsip_invite_method, &target, &target,
                                 &target, &contact, NULL, -1, NULL, &tdata);
    if (status != 0) {
        app_perror("   error: Unable to create request", status);
        return -200;
    }
    ...
    if (param == NULL || pj_strcmp2(&param->value, "param 1") != 0) {
        PJ_LOG(3,
               (THIS_FILE, "   error: x-param not present in Contact header"));
        pjsip_tx_data_dec_ref(tdata);
        return -291;
    }
    ...
    hdr =
      (pjsip_generic_string_hdr*)pjsip_msg_find_hdr_by_name(msg, &hname, NULL);
...
static int create_request_bench(pj_timestamp* p_elapsed)
{
    ...
    pj_str_t str_from =
      pj_str("\"Local User\" <sip:localuser@serviceprovider.com>");
    pj_str_t str_to =
      pj_str("\"Remote User\" <sip:remoteuser@serviceprovider.com>");
    ...
    for (i = 0; i < LOOP; i += COUNT) {
        ...
        for (j = 0; j < COUNT; ++j) {
            status = pjsip_endpt_create_request(
              endpt, &pjsip_invite_method, &str_target, &str_from, &str_to,
              &str_contact, NULL, -1, NULL, &tdata[j]);
            if (status != PJ_SUCCESS) {
                app_perror("    error: unable to create request", status);
                goto on_error;
            }
...
static int create_response_bench(pj_timestamp* p_elapsed)
{
    ...
    status = pjsip_endpt_create_request(endpt, &pjsip_invite_method,
                                        ...
                                        &str_contact, NULL, -1, NULL, &request);
    if (status != PJ_SUCCESS) {
        app_perror("    error: unable to create request", status);
        return status;
    }
    ...
    pjsip_msg_insert_first_hdr(request->msg,
                               (pjsip_hdr*)pjsip_hdr_clone(request->pool, via));
    pjsip_msg_insert_first_hdr(request->msg,
                               (pjsip_hdr*)pjsip_hdr_clone(request->pool, via));
    ...
    rdata.msg_info.from =
      (pjsip_from_hdr*)pjsip_msg_find_hdr(request->msg, PJSIP_H_FROM, NULL);
    rdata.msg_info.to =
      (pjsip_to_hdr*)pjsip_msg_find_hdr(request->msg, PJSIP_H_TO, NULL);
    rdata.msg_info.cseq =
      (pjsip_cseq_hdr*)pjsip_msg_find_hdr(request->msg, PJSIP_H_CSEQ, NULL);
    rdata.msg_info.cid =
      (pjsip_cid_hdr*)pjsip_msg_find_hdr(request->msg, PJSIP_H_FROM, NULL);
    ...
    for (i = 0; i < LOOP; i += COUNT) {
        ...
        for (j = 0; j < COUNT; ++j) {
            status =
              pjsip_endpt_create_response(endpt, &rdata, 200, NULL, &tdata[j]);
            if (status != PJ_SUCCESS) {
                app_perror("    error: unable to create request", status);
                goto on_error;
            }
...
int txdata_test(void)
{
    ...
    for (i = 0; i < REPEAT; ++i) {
        PJ_LOG(3, (THIS_FILE, "    test %d of %d..", i + 1, REPEAT));
    ...
    report_ival("create-request-per-sec", msgs, "msg/sec",
    ...
    report_ival("create-response-per-sec", msgs, "msg/sec",
