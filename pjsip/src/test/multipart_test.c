...
static void init_media_type(pjsip_media_type* mt, char* type, char* subtype,
                            char* boundary)
{
...
static pj_status_t verify2(pj_pool_t* pool, pjsip_msg_body* body)
{
    ...
    rcbase += 10;
    rc = verify_cid_str(pool, body, pj_str("cid:header1@example.org"),
                        "has header1");
    if (rc) {
        return (rc - rcbase);
    }
    ...
    rcbase += 10;
    rc = verify_cid_str(pool, body, pj_str("%22header1%22@example.org"),
                        "has header1");
    if (rc) {
        return (rc - rcbase);
    }

    cid_uri = pjsip_uri_get_uri(
      pjsip_parse_uri(pool, "<cid:%22header1%22@example.org>",
    ...
    rcbase += 10;
    rc = verify_cid_str(pool, body, pj_str("<cid:my%20header2@example.org>"),
                        "has header2");
    if (rc) {
        return (rc - rcbase);
    }
    ...
    rcbase += 10;
    rc = verify_cid_str(pool, body, pj_str("cid:my%ffheader2@example.org"),
                        "has header2");
    if (rc) {
        return (rc - rcbase);
    }
    ...
    cid_uri = pjsip_uri_get_uri(
      pjsip_parse_uri(pool, "<cid:my%ffheader2@example.org>",
    ...
    rcbase += 10;
    rc = verify_cid_str(pool, body, pj_str("cid:my%20header3@example.org"),
                        "has header4");
    if (rc) {
        return (rc - rcbase);
    }
    ...
    rcbase += 10;
    rc = verify_cid_str(pool, body, pj_str("<cid:my%20header4@example.org>"),
                        "has header4");
    if (rc) {
        return (rc - rcbase);
    }
    ...
    cid_uri = pjsip_uri_get_uri(
      pjsip_parse_uri(pool, "<cid:my%20header4@example.org>",
    ...
    rcbase += 10;
    rc = verify_cid_str(pool, body, pj_str("<my%20header3@example.org>"),
                        "has header4");
    if (rc) {
        return (rc - rcbase);
    }
    ...
    */
    ...
    /*
     * This is going to pass but the ' ' in the uri is un-encoded which is
     * invalid so we should never see it.
     */
    rcbase += 10;
    rc = verify_cid_str(pool, body, pj_str("cid:my header3@example.org"),
                        "has header4");
    if (rc) {
        return (rc - rcbase);
    }
...
static int verify_part(pjsip_multipart_part* part, char* h_content_type,
                       char* h_content_subtype, char* boundary,
                       int h_content_length, const char* body)
{
