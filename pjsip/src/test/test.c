...
static struct
{
    ...
    { "uri", 0 },       { "msg", 0 }, { "multipart", 0 },   { "txdata", 0 },
    { "tsx_bench", 0 }, { "udp", 0 }, { "loop", 0 },        { "tcp", 0 },
    { "resolve", 0 },   { "tsx", 0 }, { "tsx_destroy", 0 }, { "inv_oa", 0 },
    { "regc", 0 },
};
enum tests_to_run
{
...
static pj_status_t init_report(void)
{
    ...
    pj_ansi_sprintf(tmp, "pjsip-static-bench-%s-%s.htm", PJ_OS_NAME,
                    PJ_CC_NAME);
    ...
    /* Debug or release */
    report_sval("build-type", (PJ_DEBUG ? "Debug" : "Release"), "",
                "Build type");
    ...
    len = pj_ansi_sprintf(tmp, "%04d-%02d-%02d %02d:%02d:%02d", date_time.year,
                          date_time.mon + 1, date_time.day, date_time.hour,
    ...
    /* Write CC name */
    len = pj_ansi_sprintf(tmp, "%s-%d.%d.%d", PJ_CC_NAME, PJ_CC_VER_1,
                          PJ_CC_VER_2, PJ_CC_VER_2);
...
#    endif /* PJ_HAS_TCP */
    ...
    if (rc != PJ_SUCCESS) {
        app_perror("   error: unable to create datagram loop transport", rc);
        goto on_return;
    }
...
on_return:
    ...
    PJ_LOG(3,
           (THIS_FILE, "Stack max usage: %u, deepest: %s:%u",
            pj_thread_get_stack_max_usage(pj_thread_this()), filename, line));
    if (rc == 0)
        PJ_LOG(3, (THIS_FILE, "Looks like everything is okay!.."));
    else
        PJ_LOG(3, (THIS_FILE, "Test completed with error(s)"));

    report_ival("test-status", rc, "",
