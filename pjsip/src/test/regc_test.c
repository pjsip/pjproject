...
static pj_bool_t regs_rx_request(pjsip_rx_data* rdata)
{
        ...
        pjsip_msg_find_hdr(msg, PJSIP_H_AUTHORIZATION, NULL) == NULL)
    {
        ...
        if (registrar.cfg.contact_op == EXACT ||
            ...
            for (hsrc = msg->hdr.next; hsrc != &msg->hdr; hsrc = hsrc->next) {
                ...
                hdst = (pjsip_contact_hdr*)pjsip_hdr_clone(rdata->tp_info.pool,
                                                           hsrc);
                ...
                if (registrar.cfg.contact_op == MODIFIED) {
                    if (PJSIP_URI_SCHEME_IS_SIP(hdst->uri) ||
                        ...
                        pjsip_sip_uri* sip_uri =
                          (pjsip_sip_uri*)pjsip_uri_get_uri(hdst->uri);
        ...
        if (registrar.cfg.more_contacts.slen) {
            ...
            hcontact = pjsip_generic_string_hdr_create(
              rdata->tp_info.pool, &hname, &registrar.cfg.more_contacts);
    ...
    status = pjsip_endpt_respond(endpt, NULL, rdata, code, NULL, &hdr_list,
                                 NULL, NULL);
...
static int do_test(const char* title, const struct registrar_cfg* srv_cfg,
                   const struct client* client_cfg,
                   const pj_str_t* registrar_uri, unsigned contact_cnt,
                   const pj_str_t contacts[], unsigned expires,
                   pj_bool_t leave_session, pjsip_regc** p_regc)
{
    pjsip_regc* regc;
    unsigned i;
    ...
    if (client_result.code != client_cfg->code && client_cfg->code != 502 &&
        client_cfg->code != 503 && client_result.code != 502 &&
        client_result.code != 503)
    {
...
static int keep_alive_test(const pj_str_t* registrar_uri)
{
    ...
    ret = do_test("register refresh (takes ~40 secs)", &server_cfg, &client_cfg,
                  registrar_uri, 1, &contact, TIMEOUT, PJ_TRUE, &regc);
    if (ret != 0)
        return ret;
    ...
    if (client_result.code != 200) {
        PJ_LOG(3, (THIS_FILE, "    error: expecting code=%d, got code=%d", 200,
                   client_result.code));
        return -420;
    }
    ...
    if (client_result.have_reg == 0) {
        PJ_LOG(
          3, (THIS_FILE, "    error: expecting have_reg=%d, got have_reg=%d", 1,
              client_result.have_reg));
        return -460;
    }
...
static int refresh_error(const pj_str_t* registrar_uri, pj_bool_t destroy_on_cb)
{
    enum
    {
        TIMEOUT = 40
    ...
    ret = do_test("refresh error (takes ~40 secs)", &server_cfg, &client_cfg,
                  registrar_uri, 1, &contact, TIMEOUT, PJ_TRUE, &regc);
    if (ret != 0)
        return ret;
    ...
    if (client_result.error == PJ_FALSE && client_result.code / 100 == 2) {
        PJ_LOG(
          3, (THIS_FILE, "    error: expecting error got successfull result"));
        return -510;
    }
...
static int update_test(const pj_str_t* registrar_uri)
{
    ...
    ret = do_test("update test", &server_cfg, &client_cfg, registrar_uri, 1,
                  &contacts[0], TIMEOUT, PJ_TRUE, &regc);
    if (ret != 0) {
        return -600;
    }
     ...
     */
    h1 =
      (pjsip_contact_hdr*)pjsip_msg_find_hdr(tdata->msg, PJSIP_H_CONTACT, NULL);
    if (!h1) {
        ret = -630;
        goto on_return;
    }
    if ((void*)h1->next == (void*)&tdata->msg->hdr)
        ...
        h2 = (pjsip_contact_hdr*)pjsip_msg_find_hdr(tdata->msg, PJSIP_H_CONTACT,
                                                    h1->next);
    if (!h2) {
        ret = -640;
        goto on_return;
    ...
    for (i = 0; i < 2; ++i) {
         ...
         *  - <sip:c>
         */
        h1 = (pjsip_contact_hdr*)pjsip_msg_find_hdr(tdata->msg, PJSIP_H_CONTACT,
                                                    NULL);
        if (!h1) {
            ret = -730;
            goto on_return;
        ...
        if ((void*)h1->next == (void*)&tdata->msg->hdr)
            ...
            h2 = (pjsip_contact_hdr*)pjsip_msg_find_hdr(
              tdata->msg, PJSIP_H_CONTACT, h1->next);
        if (!h2) {
            ret = -740;
            goto on_return;
        }
                           ...
                           pj_bool_t destroy_on_cb)
{
    ...
    ret = do_test("auth send error", &server_cfg, &client_cfg, registrar_uri, 1,
                  &contact, TIMEOUT, PJ_TRUE, &regc);
...
int regc_test(void)
{
    struct test_rec
    {
        ...
        { OFF,                           /* check_contact	*/
          ...
          /* registrar config: */
          /* respond	code	auth	  contact   exp_prm expires
             more_contacts */
        ...
        { ON_OFF,                               /* check_contact	*/
          ...
          { PJ_TRUE,
            ...
            0,
            65,
            { "<sip:a@A>;expires=75, <sip:b@B;expires=70>", 0 } },
    ...
    rc = pjsip_endpt_acquire_transport(endpt, PJSIP_TRANSPORT_UDP, &addr,
                                       sizeof(addr), NULL, &udp);
    if (rc == PJ_SUCCESS) {
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(test_rec); ++i) {
        ...
        if (t->server_cfg.more_contacts.ptr)
            t->server_cfg.more_contacts.slen =
              strlen(t->server_cfg.more_contacts.ptr);
            ...
            for (y = 1; y <= 2; ++y) {
                ...
                for (z = 0; z <= 1; ++z) {
                    ...
                    rc = do_test(new_title, &t->server_cfg, &t->client_cfg,
                                 &reg_uri, t->contact_cnt, contacts, t->expires,
                                 PJ_FALSE, NULL);
                    if (rc != 0)
                        goto on_return;
                }
