...
int generic_transport_test(pjsip_transport* tp)
{
    ...
    /* Check that local address name is valid. */
    {
        ...
        if (pj_inet_pton(pj_AF_INET(), &tp->local_name.host, &addr) ==
            PJ_SUCCESS) {
...
/* Transport callback. */
static void send_msg_callback(pjsip_send_state* stateless_data, pj_ssize_t sent,
                              pj_bool_t* cont)
{
...
int transport_send_recv_test(pjsip_transport_type_e tp_type,
                             pjsip_transport* ref_tp, char* target_url,
                             int* p_usec_rtt)
{
    ...
    status =
      pjsip_endpt_create_request(endpt, &method, &target, &from, &to, &contact,
                                 &call_id, CSEQ_VALUE, &body, &tdata);
    if (status != PJ_SUCCESS) {
    ...
    PJ_LOG(
      5, (THIS_FILE, "Sending request to %.*s", (int)target.slen, target.ptr));
...
static pj_status_t rt_send_request(int thread_id)
{
    ...
    status =
      pjsip_endpt_create_request(endpt, &pjsip_options_method, &target, &from,
                                 &to, &contact, &call_id, -1, NULL, &tdata);
    if (status != PJ_SUCCESS) {
        app_perror("    error: unable to create request", status);
    ...
    timeout_delay.sec = 100;
    timeout_delay.msec = 0;
...
int transport_rt_test(pjsip_transport_type_e tp_type, pjsip_transport* ref_tp,
                      char* target_url, int* lost)
{
    enum
    {
        THREADS = 4,
    ...
    for (i = 0; i < THREADS; ++i) {
        ...
        rt_test_data[i].call_id.ptr =
          (char*)pj_pool_alloc(pool, rt_call_id.slen + 1);
        ...
        status = pj_thread_create(pool, "rttest%p", &rt_worker_thread,
                                  (void*)(pj_ssize_t)i, 0, PJ_THREAD_SUSPENDED,
                                  &rt_test_data[i].thread);
        if (status != PJ_SUCCESS) {
...
int transport_load_test(char* target_url)
{
    ...
    if (mod_load.next_seq != COUNT) {
        PJ_LOG(1, ("THIS_FILE", "    err: expecting %u msg, got only %u", COUNT,
                   mod_load.next_seq));
        status = -2;
        goto on_return;
    }
