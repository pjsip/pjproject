/* $Header: /pjproject/pjsip/src/pjsua/main.c 38    8/31/05 9:05p Bennylp $ */
/* 
 * PJSIP - SIP Stack
...
static pj_status_t init_sockets()
{
	...
	if (global.stun_port1 == 0) {
	    ...
	    if (pj_sockaddr_set_str_addr( &addr, &hostname) != PJ_OK)
		goto on_error;
...
static pjsip_dlg *make_call(pj_str_t *remote_uri)
{
    ...
    if (pjsip_dlg_init(dlg, &local, &remote, NULL) != PJ_OK) {
    ...
    if (pjsip_dlg_send_msg(dlg, tdata) != PJ_OK) {
	pjsip_ua_destroy_dialog(dlg);
	return NULL;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (parse_args(global.pool, argc, argv) != PJ_OK) {
