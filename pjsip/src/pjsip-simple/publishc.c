...
const pjsip_method pjsip_publish_method = {
    (pjsip_method_e)PJSIP_PUBLISH_METHOD, { "PUBLISH", 7 }
};
...
PJ_DEF(pj_status_t)
pjsip_publishc_create(pjsip_endpoint* endpt, const pjsip_publishc_opt* opt,
                      void* token, pjsip_publishc_cb* cb,
...
PJ_DEF(pj_status_t)
pjsip_publishc_init(pjsip_publishc* pubc, const pj_str_t* event,
                    const pj_str_t* target_uri, const pj_str_t* from_uri,
                    const pj_str_t* to_uri, pj_uint32_t expires)
{
    pj_str_t tmp;

    PJ_ASSERT_RETURN(
      pubc && event && target_uri && from_uri && to_uri && expires, PJ_EINVAL);
...
PJ_DEF(pj_status_t)
pjsip_publishc_set_credentials(pjsip_publishc* pubc, int count,
...
PJ_DEF(pj_status_t)
pjsip_publishc_set_route_set(pjsip_publishc* pubc,
...
PJ_DEF(pj_status_t)
pjsip_publishc_set_headers(pjsip_publishc* pubc, const pjsip_hdr* hdr_list)
{
    const pjsip_hdr* h;
...
PJ_DEF(pj_status_t)
pjsip_publishc_set_via_sent_by(pjsip_publishc* pubc, pjsip_host_port* via_addr,
...
static pj_status_t create_request(pjsip_publishc* pubc, pjsip_tx_data** p_tdata)
{
    ...
    status = pjsip_endpt_create_request_from_hdr(
      pubc->endpt, &pjsip_publish_method, pubc->target_uri, pubc->from_hdr,
      pubc->to_hdr, NULL, pubc->cid_hdr, pubc->cseq_hdr->cseq, NULL, &tdata);
    if (status != PJ_SUCCESS)
        return status;
    ...
    if (!pj_list_empty(&pubc->route_set)) {
        ...
        while (route != &pubc->route_set) {
            pjsip_hdr* new_hdr =
              (pjsip_hdr*)pjsip_hdr_shallow_clone(tdata->pool, route);
    ...
    /* Add Event header */
    hdr =
      pjsip_generic_string_hdr_create(tdata->pool, &STR_EVENT, &pubc->event);
    if (hdr)
    ...
    if (pubc->etag.slen) {
        ...
        hdr =
          pjsip_generic_string_hdr_create(tdata->pool, &STR_HNAME, &pubc->etag);
        if (hdr)
            pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*)hdr);
    }
    ...
    if (!pj_list_empty(&pubc->usr_hdr)) {
        ...
        while (uhdr != &pubc->usr_hdr) {
            pjsip_hdr* new_hdr =
              (pjsip_hdr*)pjsip_hdr_shallow_clone(tdata->pool, uhdr);
...
PJ_DEF(pj_status_t)
pjsip_publishc_publish(pjsip_publishc* pubc, pj_bool_t auto_refresh,
                       pjsip_tx_data** p_tdata)
{
    ...
    if (pubc->expires_hdr) {
        ...
        dup =
          (pjsip_hdr*)pjsip_hdr_shallow_clone(tdata->pool, pubc->expires_hdr);
        if (dup)
...
PJ_DEF(pj_status_t)
pjsip_publishc_unpublish(pjsip_publishc* pubc, pjsip_tx_data** p_tdata)
{
    pjsip_tx_data* tdata;
    pjsip_msg* msg;
...
PJ_DEF(pj_status_t)
pjsip_publishc_update_expires(pjsip_publishc* pubc, pj_uint32_t expires)
{
    PJ_ASSERT_RETURN(pubc, PJ_EINVAL);
    set_expires(pubc, expires);
    return PJ_SUCCESS;
...
static void call_callback(pjsip_publishc* pubc, pj_status_t status, int st_code,
                          const pj_str_t* reason, pjsip_rx_data* rdata,
                          pj_uint32_t expiration)
{
...
static void tsx_callback(void* token, pjsip_event* event)
{
               ...
               tsx->status_code == PJSIP_SC_UNAUTHORIZED)
    {
        ...
        if (tsx->status_code / 100 == 2) {
            ...
            /* Save ETag value */
            etag_hdr = (pjsip_generic_string_hdr*)pjsip_msg_find_hdr_by_name(
              msg, &STR_ETAG, NULL);
            if (etag_hdr) {
        ...
        while (!pj_list_empty(&pubc->pending_reqs)) {
            ...
            if (pubc->etag.slen) {
                ...
                sim_hdr = (pjsip_generic_string_hdr*)pjsip_msg_find_hdr_by_name(
                  tdata->msg, &STR_HNAME, NULL);
                if (!sim_hdr) {
                    /* Create the header */
                    sim_hdr = pjsip_generic_string_hdr_create(
                      tdata->pool, &STR_HNAME, &pubc->etag);
...
PJ_DEF(pj_status_t)
pjsip_publishc_send(pjsip_publishc* pubc, pjsip_tx_data* tdata)
{
    pj_status_t status;
    pjsip_cseq_hdr* cseq_hdr;
    pj_uint32_t cseq;
    ...
    if (pubc->pending_tsx) {
        if (pubc->opt.queue_request) {
            ...
            pj_mutex_unlock(pubc->mutex);
            PJ_LOG(4, (THIS_FILE,
                       "Request is queued, pubc has another "
                       "transaction pending"));
            return PJ_EPENDING;
        } else {
            pjsip_tx_data_dec_ref(tdata);
            pj_mutex_unlock(pubc->mutex);
            PJ_LOG(4, (THIS_FILE,
                       "Unable to send request, pubc has another "
                       "transaction pending"));
            return PJ_EBUSY;
        }
    }
    ...
    ++pubc->pending_tsx;
    status =
      pjsip_endpt_send_request(pubc->endpt, tdata, -1, pubc, &tsx_callback);
    if (status != PJ_SUCCESS) {
