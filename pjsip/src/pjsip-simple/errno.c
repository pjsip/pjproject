...
static const struct
{
    ...
    { PJSIP_SIMPLE_ENOPRESENCE,
      "No presence associated with the subscription" },
    { PJSIP_SIMPLE_ENOPRESENCEINFO,
      "No presence info in the server subscription" },
...
pjsipsimple_strerror(pj_status_t statcode, char* buf, pj_size_t bufsize)
{
    ...
    errstr.ptr = buf;
    errstr.slen =
      pj_ansi_snprintf(buf, bufsize, "Unknown pjsip-simple error %d", statcode);
