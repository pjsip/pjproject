...
static void update_namespaces(pjpidf_pres* pres, pj_pool_t* pool)
{
...
static pj_bool_t substring_match(const pj_xml_node* node, const char* part_name,
                                 pj_ssize_t part_len)
{
    pj_str_t end_name;
...
static pj_xml_node* find_node(const pj_xml_node* parent, const char* part_name)
{
                   ...
                   const pjrpid_element* elem)
{
    ...
    if (elem->id.slen == 0 && elem->activity == PJRPID_ACTIVITY_UNKNOWN) {
...
static pj_status_t get_tuple_note(const pjpidf_pres* pres, pj_pool_t* pool,
                                  pjrpid_element* elem)
{
