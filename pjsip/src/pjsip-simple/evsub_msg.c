 ...
 * Event header.
 */
static int pjsip_event_hdr_print(pjsip_event_hdr* hdr, char* buf,
                                 pj_size_t size);
...
static pjsip_event_hdr* pjsip_event_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_event_hdr* rhs)
{
 ...
 */
static int pjsip_sub_state_hdr_print(pjsip_sub_state_hdr* hdr, char* buf,
                                     pj_size_t size);
static pjsip_sub_state_hdr* pjsip_sub_state_hdr_clone(
  pj_pool_t* pool, const pjsip_sub_state_hdr* hdr);
static pjsip_sub_state_hdr* pjsip_sub_state_hdr_shallow_clone(
  pj_pool_t* pool, const pjsip_sub_state_hdr*);
...
static pjsip_sub_state_hdr* pjsip_sub_state_hdr_clone(
  pj_pool_t* pool, const pjsip_sub_state_hdr* rhs)
{
...
static pjsip_hdr* parse_hdr_sub_state(pjsip_parse_ctx* ctx)
{
    ...
    const pj_str_t reason = { "reason", 6 }, expires = { "expires", 7 },
...
{
    pjsip_register_hdr_parser("Event", "o", &parse_hdr_event);

    pjsip_register_hdr_parser("Subscription-State", NULL, &parse_hdr_sub_state);
}
