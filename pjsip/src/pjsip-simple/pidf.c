...
struct pjpidf_op_desc pjpidf_op = {
    { &pjpidf_pres_construct, &pjpidf_pres_add_tuple,
      &pjpidf_pres_get_first_tuple, &pjpidf_pres_get_next_tuple,
      &pjpidf_pres_find_tuple, &pjpidf_pres_remove_tuple, &pjpidf_pres_add_note,
      &pjpidf_pres_get_first_note, &pjpidf_pres_get_next_note },
    { &pjpidf_tuple_construct, &pjpidf_tuple_get_id, &pjpidf_tuple_set_id,
      &pjpidf_tuple_get_status, &pjpidf_tuple_get_contact,
      &pjpidf_tuple_set_contact, &pjpidf_tuple_set_contact_prio,
      &pjpidf_tuple_get_contact_prio, &pjpidf_tuple_add_note,
      &pjpidf_tuple_get_first_note, &pjpidf_tuple_get_next_note,
      &pjpidf_tuple_get_timestamp, &pjpidf_tuple_set_timestamp,
      &pjpidf_tuple_set_timestamp_np },
    { &pjpidf_status_construct, &pjpidf_status_is_basic_open,
...
static void xml_init_node(pj_pool_t* pool, pj_xml_node* node, pj_str_t* name,
                          const pj_str_t* value)
{
    ...
    node->name = *name;
    if (value)
        pj_strdup(pool, &node->content, value);
    else
        node->content.ptr = NULL, node->content.slen = 0;
}
...
/* Presence */
PJ_DEF(void)
pjpidf_pres_construct(pj_pool_t* pool, pjpidf_pres* pres,
...
PJ_DEF(pjpidf_tuple*)
pjpidf_pres_add_tuple(pj_pool_t* pool, pjpidf_pres* pres, const pj_str_t* id)
{
...
PJ_DEF(pjpidf_tuple*)
pjpidf_pres_get_next_tuple(pjpidf_pres* pres, pjpidf_tuple* tuple)
{
...
PJ_DEF(pjpidf_tuple*)
pjpidf_pres_find_tuple(pjpidf_pres* pres, const pj_str_t* id)
{
...
PJ_DEF(pjpidf_note*)
pjpidf_pres_add_note(pj_pool_t* pool, pjpidf_pres* pres, const pj_str_t* text)
{
...
PJ_DEF(pjpidf_note*)
pjpidf_pres_get_next_note(pjpidf_pres* t, pjpidf_note* note)
{
...
/* Tuple */
PJ_DEF(void)
pjpidf_tuple_construct(pj_pool_t* pool, pjpidf_tuple* t, const pj_str_t* id)
{
    pj_xml_attr* attr;
    pjpidf_status* st;
...
PJ_DEF(void)
pjpidf_tuple_set_id(pj_pool_t* pool, pjpidf_tuple* t, const pj_str_t* id)
{
...
PJ_DEF(void)
pjpidf_tuple_set_contact(pj_pool_t* pool, pjpidf_tuple* t,
...
PJ_DEF(void)
pjpidf_tuple_set_contact_prio(pj_pool_t* pool, pjpidf_tuple* t,
                              const pj_str_t* prio)
{
    pj_xml_node* node = pj_xml_find_node(t, &CONTACT);
    pj_xml_attr* attr;
...
PJ_DEF(pjpidf_note*)
pjpidf_tuple_add_note(pj_pool_t* pool, pjpidf_tuple* t, const pj_str_t* text)
{
    pjpidf_note* note = PJ_POOL_ALLOC_T(pool, pjpidf_note);
    xml_init_node(pool, note, &NOTE, text);
...
PJ_DEF(void)
pjpidf_tuple_set_timestamp(pj_pool_t* pool, pjpidf_tuple* t, const pj_str_t* ts)
{
...
PJ_DEF(void)
pjpidf_tuple_set_timestamp_np(pj_pool_t* pool, pjpidf_tuple* t, pj_str_t* ts)
{
    pj_xml_node* node = pj_xml_find_node(t, &TIMESTAMP);
    if (!node) {
        node = PJ_POOL_ALLOC_T(pool, pj_xml_node);
