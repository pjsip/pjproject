...
pjsip_subscribe_method = { (pjsip_method_e)PJSIP_SUBSCRIBE_METHOD,
                           { "SUBSCRIBE", 9 } };

PJ_DEF_DATA(const pjsip_method)
pjsip_notify_method = { (pjsip_method_e)PJSIP_NOTIFY_METHOD, { "NOTIFY", 6 } };
 ...
 * State names.
 */
static pj_str_t evsub_state_names[] = { { "NULL", 4 },     { "SENT", 4 },
                                        { "ACCEPTED", 8 }, { "PENDING", 7 },
                                        { "ACTIVE", 6 },   { "TERMINATED", 10 },
                                        { "UNKNOWN", 7 } };
...
static const char* timer_names[] = {
    "None",          "UAC_REFRESH",     "UAS_TIMEOUT",
    "UAC_TERMINATE", "UAC_WAIT_NOTIFY", "INVALID_TIMER"
};
 ...
 */
PJ_DECL(pj_str_t)
pjsipsimple_strerror(pj_status_t statcode, char* buf, pj_size_t bufsize);
...
{
    pj_status_t status;
    pj_str_t method_tags[] = { { "SUBSCRIBE", 9 }, { "NOTIFY", 6 } };

    status = pj_register_strerror(PJSIP_SIMPLE_ERRNO_START, PJ_ERRNO_SPACE_SIZE,
    ...
    /* Create pool: */
    mod_evsub.pool = pjsip_endpt_create_pool(
      endpt, "evsub", PJSIP_POOL_EVSUB_LEN, PJSIP_POOL_EVSUB_INC);
    if (!mod_evsub.pool)
        return PJ_ENOMEM;
    ...
    pjsip_endpt_add_capability(endpt, &mod_evsub.mod, PJSIP_H_ALLOW, NULL, 2,
                               method_tags);
 ...
 */
PJ_DEF(void*) pjsip_evsub_get_mod_data(const pjsip_evsub* sub, unsigned mod_id)
{
                         ...
                         const pj_str_t accept[])
{
    ...
    PJ_ASSERT_RETURN(accept_cnt <= PJSIP_GENERIC_ARRAY_MAX_COUNT, PJ_ETOOMANY);
    ...
    pjsip_endpt_add_capability(mod_evsub.endpt, &mod_evsub.mod, PJSIP_H_ACCEPT,
                               NULL, pkg->pkg_accept->count,
                               pkg->pkg_accept->values);

    /* Done */

    PJ_LOG(5, (THIS_FILE, "Event pkg \"%.*s\" registered by %.*s",
               (int)event_name->slen, event_name->ptr, (int)pkg_mod->name.slen,
               pkg_mod->name.ptr));
 ...
 */
PJ_DEF(const pjsip_hdr*) pjsip_evsub_get_allow_events_hdr(const pjsip_module* m)
{
 ...
 * Schedule timer.
 */
static void set_timer(pjsip_evsub* sub, int timer_id, pj_uint32_t seconds)
{
    ...
    if (timer_id != TIMER_TYPE_NONE && seconds != PJSIP_EXPIRES_NOT_SPECIFIED) {
        ...
        PJ_ASSERT_ON_FAIL(
          timer_id > TIMER_TYPE_NONE && timer_id < TIMER_TYPE_MAX, return );

        timeout.sec = seconds;
        timeout.msec = 0;

        pj_timer_heap_schedule_w_grp_lock(
          pjsip_endpt_get_timer_heap(sub->endpt), &sub->timer, &timeout,
          timer_id, sub->grp_lock);
                      ...
                      const pj_str_t* reason)
{
    ...
    PJ_LOG(4, (sub->obj_name, "Subscription state changed %.*s --> %.*s",
               (int)old_state_str.slen, old_state_str.ptr,
               (int)sub->state_str.slen, sub->state_str.ptr));
    pj_log_push_indent();
...
static void on_timer(pj_timer_heap_t* timer_heap, struct pj_timer_entry* entry)
{
    ...
    case TIMER_TYPE_UAC_REFRESH:
        ...
        if (sub->user.on_client_refresh && sub->call_cb) {
            ...
            pj_log_push_indent();
            status =
              pjsip_evsub_initiate(sub, NULL, sub->expires->ivalue, &tdata);
            if (status == PJ_SUCCESS)
    ...
    case TIMER_TYPE_UAC_TERMINATE: {
        ...
        PJ_LOG(5, (sub->obj_name,
                   "Timeout waiting for final NOTIFY. "
                   "Terminating.."));
        pj_log_push_indent();
        set_state(sub, PJSIP_EVSUB_STATE_TERMINATED, NULL, NULL, &timeout);
        pj_log_pop_indent();
    } break;
 ...
 */
static pj_status_t evsub_create(pjsip_dialog* dlg, pjsip_role_e role,
                                ...
                                pjsip_evsub** p_evsub)
{
    ...
    sub->accept =
      (pjsip_accept_hdr*)pjsip_hdr_clone(sub->pool, pkg->pkg_accept);
    ...
    /* Set name. */
    pj_ansi_snprintf(sub->obj_name, PJ_ARRAY_SIZE(sub->obj_name), "evsub%p",
                     sub);
    ...
    PJ_LOG(5, (sub->obj_name, "%s subscription created, using dialog %s",
               (role == PJSIP_ROLE_UAC ? "UAC" : "UAS"), dlg->obj_name));
                       ...
                       pjsip_evsub** p_evsub)
{
     ...
     */
    event_hdr = (pjsip_event_hdr*)pjsip_msg_find_hdr_by_names(
      rdata->msg_info.msg, &STR_EVENT, &STR_EVENT_S, NULL);
    if (event_hdr == NULL) {
    ...
    status = evsub_create(dlg, PJSIP_UAS_ROLE, user_cb, &event_hdr->event_type,
                          option, &sub);
    if (status != PJ_SUCCESS)
        goto on_return;
    ...
    expires_hdr = (pjsip_expires_hdr*)pjsip_msg_find_hdr(rdata->msg_info.msg,
    ...
    accept_hdr = (pjsip_accept_hdr*)pjsip_msg_find_hdr(rdata->msg_info.msg,
                                                       PJSIP_H_ACCEPT, NULL);
    if (accept_hdr)
 ...
 */
PJ_DEF(pj_status_t) pjsip_evsub_terminate(pjsip_evsub* sub, pj_bool_t notify)
{
                     ...
                     pj_uint32_t expires, pjsip_tx_data** p_tdata)
{
    ...
    /* Add Event header: */
    pjsip_msg_add_hdr(
      tdata->msg, (pjsip_hdr*)pjsip_hdr_shallow_clone(tdata->pool, sub->event));
    ...
    {
        const pjsip_hdr* hdr =
          pjsip_endpt_get_capability(sub->endpt, PJSIP_H_SUPPORTED, NULL);
        if (hdr) {
            pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*)pjsip_hdr_shallow_clone(
                                            tdata->pool, hdr));
        }
    ...
    /* Add Accept header: */
    pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*)pjsip_hdr_shallow_clone(
                                    tdata->pool, sub->accept));

    /* Add Allow-Events header: */
    pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*)pjsip_hdr_shallow_clone(
                                    tdata->pool, mod_evsub.allow_events_hdr));
...
pjsip_evsub_add_header(pjsip_evsub* sub, const pjsip_hdr* hdr_list)
{
    ...
    hdr = hdr_list->next;
    while (hdr != hdr_list) {
        pj_list_push_back(&sub->sub_hdr_list,
                          (pjsip_hdr*)pjsip_hdr_clone(sub->pool, hdr));
        hdr = hdr->next;
    }
                   ...
                   const pjsip_hdr* hdr_list)
{
     ...
     */
    tsx = pjsip_rdata_get_tsx(rdata);
    PJ_ASSERT_RETURN(tsx->mod_data[mod_evsub.mod.id] != NULL, PJ_EINVALIDOP);

    /* Lock dialog */
    pjsip_dlg_inc_lock(sub->dlg);

    /* Create response: */
    status = pjsip_dlg_create_response(sub->dlg, rdata, st_code, NULL, &tdata);
    if (status != PJ_SUCCESS)
        goto on_return;
 ...
 * state.
 */
static pjsip_sub_state_hdr* sub_state_create(pj_pool_t* pool, pjsip_evsub* sub,
                   ...
                   pjsip_tx_data** p_tdata)
{
    ...
    status =
      pjsip_dlg_create_request(sub->dlg, pjsip_get_notify_method(), -1, &tdata);
    if (status != PJ_SUCCESS)
        goto on_return;
    ...
    sub_state = sub_state_create(tdata->pool, sub, state, state_str, reason);
...
{
    return pjsip_evsub_notify(sub, sub->state, &sub->state_str, NULL, p_tdata);
}
...
pjsip_evsub_send_request(pjsip_evsub* sub, pjsip_tx_data* tdata)
{
    ...
    if (sub->calling_on_rx_refresh &&
        pjsip_method_cmp(&tdata->msg->line.req.method, &pjsip_notify_method) ==
          0)
    {
     ...
     */
    if (pjsip_method_cmp(&tdata->msg->line.req.method, &pjsip_notify_method) ==
        0) {
        PJ_ASSERT_ON_FAIL(sub->dst_state != PJSIP_EVSUB_STATE_NULL,
                          { goto on_return; });

        set_state(sub, sub->dst_state,
                  (sub->dst_state_str.slen ? &sub->dst_state_str : NULL), NULL,
                  NULL);
                                       ...
                                       pjsip_event* event)
{
    ...
    event_hdr = (pjsip_event_hdr*)pjsip_msg_find_hdr_by_names(
      msg, &STR_EVENT, &STR_EVENT_S, NULL);
    if (!event_hdr) {
    ...
    while (dlgsub != dlgsub_head) {
        if (pj_stricmp(&dlgsub->sub->event->event_type,
                     ...
                     !pjsip_method_cmp(&tsx->method, &pjsip_notify_method))
            {
                ...
                pj_strdup(dlgsub->sub->pool, &dlgsub->sub->event->id_param,
                          &event_hdr->id_param);
    ...
    if (dlgsub == dlgsub_head ||
        (dlgsub->sub && tsx->role == PJSIP_ROLE_UAS &&
         pjsip_evsub_get_state(dlgsub->sub) == PJSIP_EVSUB_STATE_TERMINATED))
    {
        ...
        PJ_LOG(4, (THIS_FILE, "%s for %.*s, event=%.*s;id=%.*s", reason_msg,
                   (int)tsx->method.name.slen, tsx->method.name.ptr,
                   (int)event_hdr->event_type.slen, event_hdr->event_type.ptr,
                   (int)event_hdr->id_param.slen, event_hdr->id_param.ptr));
            ...
            pjsip_method_cmp(&tsx->method, &pjsip_notify_method) == 0)
        {
            ...
            status = pjsip_dlg_create_response(
              dlg, event->body.tsx_state.src.rdata, 481, &reason, &tdata);
            if (status == PJ_SUCCESS) {
     ...
     */
    if (tsx->role == PJSIP_ROLE_UAC && tsx->state == PJSIP_TSX_STATE_CALLING &&
         ...
         pjsip_method_cmp(&tsx->method, &pjsip_subscribe_method) == 0))
    {
        if (sub->pending_sub &&
            ...
            PJ_LOG(4,
                   (sub->obj_name, "Cancelling pending subscription request"));
 ...
 */
static pj_status_t create_response(pjsip_evsub* sub, pjsip_rx_data* rdata,
                                   ...
                                   pjsip_tx_data** p_tdata)
{
    ...
    status =
      pjsip_dlg_create_response(sub->dlg, rdata, st_code, st_text, &tdata);
    if (status != PJ_SUCCESS)
        return status;
...
static void get_hdr_state(pjsip_sub_state_hdr* sub_state,
                          pjsip_evsub_state* state, pj_str_t** state_str)
{
                             ...
                             pjsip_event* event)
{
        ...
        pjsip_method_cmp(&tsx->method, &pjsip_subscribe_method) == 0)
    {
        ...
        if (tsx->status_code / 100 == 2) {
            ...
            if (sub->expires->ivalue != 0) {
                unsigned timeout = (sub->expires->ivalue > TIME_UAC_REFRESH) ?
                                     sub->expires->ivalue - TIME_UAC_REFRESH :
                                     sub->expires->ivalue;
                ...
                PJ_LOG(5,
                       (sub->obj_name, "Will refresh in %d seconds", timeout));
        ...
        sub_state = (pjsip_sub_state_hdr*)pjsip_msg_find_hdr_by_name(
          msg, &STR_SUB_STATE, NULL);
        if (sub_state == NULL) {
        ...
        if (st_code == 200 && sub->user.on_rx_notify && sub->call_cb) {
            (*sub->user.on_rx_notify)(sub, rdata, &st_code, &st_text, &res_hdr,
                                      &body);
        ...
        /* Send response. */
        status =
          create_response(sub, rdata, st_code, st_text, &res_hdr, body, &tdata);
        if (status == PJ_SUCCESS)
            status = pjsip_dlg_send_response(sub->dlg, tsx, tdata);
        ...
        if (status == PJ_SUCCESS) {
            ...
            reason = pj_strerror(status, errmsg, sizeof(errmsg));
            set_state(sub, PJSIP_EVSUB_STATE_TERMINATED, NULL, event, &reason);
        }
                             ...
                             pjsip_event* event)
{
        ...
        pjsip_method_cmp(&tsx->method, &pjsip_subscribe_method) == 0)
    {
         ...
         */
        event_hdr = (pjsip_event_hdr*)pjsip_msg_find_hdr_by_names(
          msg, &STR_EVENT, &STR_EVENT, NULL);
        expires =
          (pjsip_expires_hdr*)pjsip_msg_find_hdr(msg, PJSIP_H_EXPIRES, NULL);
        ...
        if (sub->expires->ivalue == 0) {
            ...
            PJ_LOG(4, (sub->obj_name,
                       "Receiving unsubscription request "
                       "(Expires=0)."));
            set_state(sub, PJSIP_EVSUB_STATE_TERMINATED, NULL, event, &timeout);
        ...
        if (sub->user.on_rx_refresh && sub->call_cb) {
            (*sub->user.on_rx_refresh)(sub, rdata, &st_code, &st_text, &res_hdr,
                                       &body);
        ...
        if (tsx->status_code / 100 != 2) {
            ...
            if (event->body.tsx_state.type == PJSIP_EVENT_RX_MSG) {
                if (tsx->status_code == 481) {
                    ...
                    retry_after =
                      (pjsip_retry_after_hdr*)pjsip_msg_find_hdr_by_name(
                        msg, &STR_RETRY_AFTER, NULL);
...
static void mod_evsub_on_tsx_state(pjsip_transaction* tsx, pjsip_event* event)
{
    ...
    if (tsx->state == PJSIP_TSX_STATE_TERMINATED) {
        ...
        if (sub->state == PJSIP_EVSUB_STATE_TERMINATED && sub->pending_tsx == 0)
        {
            evsub_destroy(sub);
        }
    }
