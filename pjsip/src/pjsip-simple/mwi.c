...
static void mwi_on_evsub_tsx_state(pjsip_evsub* sub, pjsip_transaction* tsx,
                                   pjsip_event* event);
static void mwi_on_evsub_rx_refresh(pjsip_evsub* sub, pjsip_rx_data* rdata,
                                    int* p_st_code, pj_str_t** p_st_text,
                                    pjsip_hdr* res_hdr,
                                    pjsip_msg_body** p_body);
static void mwi_on_evsub_rx_notify(pjsip_evsub* sub, pjsip_rx_data* rdata,
                                   int* p_st_code, pj_str_t** p_st_text,
...
static pjsip_evsub_user mwi_user = {
    &mwi_on_evsub_state,          &mwi_on_evsub_tsx_state,
    &mwi_on_evsub_rx_refresh,     &mwi_on_evsub_rx_notify,
    &mwi_on_evsub_client_refresh, &mwi_on_evsub_server_timeout,
};
...
static const pj_str_t STR_APP_SIMPLE_SMS = {
    "application/simple-message-summary", 34
};
...
pjsip_mwi_init_module(pjsip_endpoint* endpt, pjsip_module* mod_evsub)
{
    ...
    status =
      pjsip_evsub_register_pkg(&mod_mwi, &STR_MWI, PJSIP_MWI_DEFAULT_EXPIRES,
                     ...
                     unsigned options, pjsip_evsub** p_evsub)
{
    ...
    status = pjsip_evsub_create_uac(dlg, &mwi_user, &STR_MWI, options, &sub);
    if (status != PJ_SUCCESS)
        goto on_return;
 ...
 */
PJ_DEF(pj_status_t) pjsip_mwi_terminate(pjsip_evsub* sub, pj_bool_t notify)
{
 ...
 */
static pj_status_t mwi_create_msg_body(pjsip_mwi* mwi, pjsip_tx_data* tdata)
{
    pjsip_msg_body* body;
    pj_str_t dup_text;
                                    ...
                                    pjsip_hdr* res_hdr, pjsip_msg_body** p_body)
{
    ...
    if (mwi->user_cb.on_rx_refresh) {
        (*mwi->user_cb.on_rx_refresh)(sub, rdata, p_st_code, p_st_text, res_hdr,
                                      p_body);
        ...
        if (pjsip_evsub_get_state(sub) == PJSIP_EVSUB_STATE_TERMINATED) {
            status = pjsip_mwi_notify(sub, PJSIP_EVSUB_STATE_TERMINATED, NULL,
                                      &timeout, NULL, NULL, &tdata);
        } else {
                                   ...
                                   pjsip_hdr* res_hdr, pjsip_msg_body** p_body)
{
    ...
    if (mwi->user_cb.on_rx_notify) {
        (*mwi->user_cb.on_rx_notify)(sub, rdata, p_st_code, p_st_text, res_hdr,
                                     p_body);
    }
}
