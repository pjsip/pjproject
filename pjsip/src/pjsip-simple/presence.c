...
static void pres_on_evsub_tsx_state(pjsip_evsub* sub, pjsip_transaction* tsx,
                                    pjsip_event* event);
static void pres_on_evsub_rx_refresh(pjsip_evsub* sub, pjsip_rx_data* rdata,
                                     int* p_st_code, pj_str_t** p_st_text,
                                     pjsip_hdr* res_hdr,
                                     pjsip_msg_body** p_body);
static void pres_on_evsub_rx_notify(pjsip_evsub* sub, pjsip_rx_data* rdata,
                                    int* p_st_code, pj_str_t** p_st_text,
                                    pjsip_hdr* res_hdr,
                                    pjsip_msg_body** p_body);
...
static pjsip_evsub_user pres_user = {
    &pres_on_evsub_state,          &pres_on_evsub_tsx_state,
    &pres_on_evsub_rx_refresh,     &pres_on_evsub_rx_notify,
    &pres_on_evsub_client_refresh, &pres_on_evsub_server_timeout,
};
                      ...
                      unsigned options, pjsip_evsub** p_evsub)
{
    ...
    status =
      pjsip_evsub_create_uac(dlg, &pres_user, &STR_PRESENCE, options, &sub);
    if (status != PJ_SUCCESS)
        goto on_return;
    ...
    pres->status_pool =
      pj_pool_create(dlg->pool->factory, obj_name, 512, 512, NULL);
                      ...
                      pjsip_rx_data* rdata, pjsip_evsub** p_evsub)
{
    ...
    event = (pjsip_event_hdr*)pjsip_msg_find_hdr_by_name(rdata->msg_info.msg,
                                                         &STR_EVENT, NULL);
    if (!event) {
    ...
    accept = (pjsip_accept_hdr*)pjsip_msg_find_hdr(rdata->msg_info.msg,
                                                   PJSIP_H_ACCEPT, NULL);
    if (accept) {
        unsigned i;
 ...
 */
PJ_DEF(pj_status_t) pjsip_pres_terminate(pjsip_evsub* sub, pj_bool_t notify)
{
...
pjsip_pres_set_status(pjsip_evsub* sub, const pjsip_pres_status* status)
{
    ...
    for (i = 0; i < status->info_cnt; ++i) {
        ...
        if (pres->status.info[i].id.slen) {
            ...
            pj_create_unique_string(pres->dlg->pool, &pres->status.info[i].id);
        } else {
            pj_strdup(pres->dlg->pool, &pres->status.info[i].id,
                      &status->info[i].id);
        }
        pj_strdup(pres->tmp_pool, &pres->status.info[i].contact,
                  &status->info[i].contact);

        /* Duplicate <person> */
        pres->status.info[i].rpid.activity = status->info[i].rpid.activity;
        pj_strdup(pres->tmp_pool, &pres->status.info[i].rpid.id,
                  &status->info[i].rpid.id);
        pj_strdup(pres->tmp_pool, &pres->status.info[i].rpid.note,
 ...
 * Create message body.
 */
static pj_status_t pres_create_msg_body(pjsip_pres* pres, pjsip_tx_data* tdata)
{
    pj_str_t entity;
    ...
    entity.slen =
      pjsip_uri_print(PJSIP_URI_IN_REQ_URI, pres->dlg->local.info->uri,
                      entity.ptr, PJSIP_MAX_URL_SIZE);
    if (entity.slen < 1)
        return PJ_ENOMEM;

    if (pres->content_type == CONTENT_TYPE_PIDF) {
        return pjsip_pres_create_pidf(tdata->pool, &pres->status, &entity,
                                      &tdata->msg->body);

    } else if (pres->content_type == CONTENT_TYPE_XPIDF) {
        return pjsip_pres_create_xpidf(tdata->pool, &pres->status, &entity,
                                       &tdata->msg->body);
                  ...
                  pjsip_tx_data** p_tdata)
{
     ...
     */
    PJ_ASSERT_RETURN(
      state == PJSIP_EVSUB_STATE_TERMINATED || pres->status.info_cnt > 0,
                                     ...
                                     pjsip_msg_body** p_body)
{
    ...
    if (pres->user_cb.on_rx_refresh) {
        ...
        if (pjsip_evsub_get_state(sub) == PJSIP_EVSUB_STATE_TERMINATED) {
            status = pjsip_pres_notify(sub, PJSIP_EVSUB_STATE_TERMINATED, NULL,
                                       &timeout, &tdata);
        } else {
...
static pj_status_t pres_process_rx_notify(pjsip_pres* pres,
                                          pjsip_rx_data* rdata, int* p_st_code,
                                          pj_str_t** p_st_text,
                                          pjsip_hdr* res_hdr)
{
    ...
    *p_st_text = NULL;
    ...
    if (pj_stricmp(&ctype_hdr->media.type, &STR_MULTIPART) == 0) {
        ...
        mpart =
          pjsip_multipart_find_part(rdata->msg_info.msg->body, &ctype, NULL);
        if (mpart) {
            status =
              pjsip_pres_parse_pidf2((char*)mpart->body->data, mpart->body->len,
                                     pres->tmp_pool, &pres->tmp_status);
        }

        if (mpart == NULL) {
            pjsip_media_type_init(&ctype, (pj_str_t*)&STR_APPLICATION,
                                  (pj_str_t*)&STR_XPIDF_XML);
            mpart = pjsip_multipart_find_part(rdata->msg_info.msg->body, &ctype,
                                              NULL);
            if (mpart) {
                status = pjsip_pres_parse_xpidf2(
                  (char*)mpart->body->data, mpart->body->len, pres->tmp_pool,
                  &pres->tmp_status);
            } else {
    ...
    {
        status =
          pjsip_pres_parse_pidf(rdata, pres->tmp_pool, &pres->tmp_status);
    } else if (pj_stricmp(&ctype_hdr->media.type, &STR_APPLICATION) == 0 &&
               pj_stricmp(&ctype_hdr->media.subtype, &STR_XPIDF_XML) == 0)
    {
        status =
          pjsip_pres_parse_xpidf(rdata, pres->tmp_pool, &pres->tmp_status);
    } else {
        status = PJSIP_SIMPLE_EBADCONTENT;
    }
        ...
        if (PJSIP_PRES_BAD_CONTENT_RESPONSE >= 300) {
            ...
            *p_st_code = PJSIP_PRES_BAD_CONTENT_RESPONSE;
            ...
            warn_hdr = pjsip_warning_hdr_create_from_status(
              rdata->tp_info.pool, pjsip_endpt_name(pres->dlg->endpt), status);
            pj_list_push_back(res_hdr, warn_hdr);
                                    ...
                                    pjsip_hdr* res_hdr, pjsip_msg_body** p_body)
{
    ...
    if (rdata->msg_info.msg->body) {
        status =
          pres_process_rx_notify(pres, rdata, p_st_code, p_st_text, res_hdr);
        if (status != PJ_SUCCESS)
            return;
    ...
    if (pres->user_cb.on_rx_notify) {
        (*pres->user_cb.on_rx_notify)(sub, rdata, p_st_code, p_st_text, res_hdr,
                                      p_body);
    }
...
static void pres_on_evsub_client_refresh(pjsip_evsub* sub)
{
    ...
    if (pres->user_cb.on_client_refresh) {
        ...
        status = pjsip_pres_initiate(sub, PJSIP_EXPIRES_NOT_SPECIFIED, &tdata);
        if (status == PJ_SUCCESS)
            pjsip_pres_send_request(sub, tdata);
    }
}
