...
static pj_xml_node* xml_create_node(pj_pool_t* pool, pj_str_t* name,
...
PJ_DEF(pjxpidf_pres*) pjxpidf_create(pj_pool_t* pool, const pj_str_t* uri_cstr)
{
    ...
    /* uri attribute */
    uri.ptr =
      (char*)pj_pool_alloc(pool, uri_cstr->slen + STR_SUBSCRIBE_PARAM.slen);
...
PJ_DEF(pj_status_t)
pjxpidf_set_status(pjxpidf_pres* pres, pj_bool_t online_status)
{
    pj_xml_node* atom;
    pj_xml_node* addr;
    pj_xml_node* status;
