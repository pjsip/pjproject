 ...
 */
static int pres_print_body(struct pjsip_msg_body* msg_body, char* buf,
                           pj_size_t size)
{
                       ...
                       const pj_str_t* entity, pjsip_msg_body** p_body)
{
    ...
    for (i = 0; i < status->info_cnt; ++i) {
        ...
        pjpidf_status_set_basic_open(pidf_status, status->info[i].basic_open);
        ...
        if (PJSIP_PRES_PIDF_ADD_TIMESTAMP) {
            ...
            tslen = pj_ansi_snprintf(
              buf, sizeof(buf), "%04d-%02d-%02dT%02d:%02d:%02d.%03dZ", pt.year,
              pt.mon + 1, pt.day, pt.hour, pt.min, pt.sec, pt.msec);
                        ...
                        const pj_str_t* entity, pjsip_msg_body** p_body)
{
    ...
    PJ_LOG(4, (THIS_FILE,
               "Warning: XPIDF format is not fully supported "
               "by PJSIP"));
                      ...
                      pjsip_pres_status* pres_status)
{
    return pjsip_pres_parse_pidf2((char*)rdata->msg_info.msg->body->data,
                                  rdata->msg_info.msg->body->len, pool,
                                  pres_status);
}
                       ...
                       pjsip_pres_status* pres_status)
{
    ...
    while (pidf_tuple && pres_status->info_cnt < PJSIP_PRES_STATUS_MAX_INFO) {
        ...
        pj_strdup(pool, &pres_status->info[pres_status->info_cnt].id,
                  pjpidf_tuple_get_id(pidf_tuple));

        pj_strdup(pool, &pres_status->info[pres_status->info_cnt].contact,
                        ...
                        pjsip_pres_status* pres_status)
{
    ...
    pj_strdup(pool, &pres_status->info[0].contact, pjxpidf_get_uri(xpidf));
