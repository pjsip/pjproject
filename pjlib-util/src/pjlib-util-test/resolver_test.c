...
static int server_thread(void* p)
{
    ...
    while (!thread_quit) {
        ...
        pkt_len = sizeof(pkt);
        rc = pj_sock_recvfrom(srv->sock, pkt, &pkt_len, 0, &src_addr, &src_len);
        if (rc != 0) {
        ...
        PJ_LOG(5,
               (THIS_FILE, "Server %d processing packet", srv - &g_server[0]));
        srv->pkt_count++;
        ...
        if (req->hdr.qdcount != 1) {
            PJ_LOG(5,
                   (THIS_FILE, "server receive multiple queries in a packet"));
            continue;
        }

        if (req->q[0].dnsclass != 1) {
            PJ_LOG(5,
                   (THIS_FILE, "server receive query with invalid DNS class"));
            continue;
        }
...
static int init(pj_bool_t use_ipv6)
{
    ...
    for (i = 0; i < 2; ++i) {
        ...
        pj_sockaddr_init((use_ipv6 ? pj_AF_INET6() : pj_AF_INET()), &addr, NULL,
                         (pj_uint16_t)g_server[i].port);

        status =
          pj_sock_bind(g_server[i].sock, &addr, pj_sockaddr_get_len(&addr));
        if (status != PJ_SUCCESS)
            return -20;

        status = pj_thread_create(pool, NULL, &server_thread, &g_server[i], 0,
                                  0, &g_server[i].thread);
        if (status != PJ_SUCCESS)
            return -30;
    }
    ...
    status =
      pj_dns_resolver_create(mem, NULL, 0, timer_heap, ioqueue, &resolver);
    if (status != PJ_SUCCESS)
        return -40;
    ...
    status = pj_thread_create(pool, NULL, &poll_worker_thread, NULL, 0, 0,
                              &poll_thread);
    pj_assert(status == PJ_SUCCESS);
...
static int a_parser_test(void)
{
    ...
    pkt.ans =
      (pj_dns_parsed_rr*)pj_pool_calloc(pool, 32, sizeof(pj_dns_parsed_rr));
...
static int addr_parser_test(void)
{
    ...
    pj_assert(rec.addr[0].af == pj_AF_INET() &&
              rec.addr[0].ip.v4.s_addr == 0x01020304);
    pj_assert(rec.addr[1].af == pj_AF_INET6() &&
              s6_addr32(rec.addr[1].ip.v6, 0) == 0x01020304);
...
static void dns_callback(void* user_data, pj_status_t status,
                         pj_dns_parsed_packet* resp)
{
...
static void dns_callback_1b(void* user_data, pj_status_t status,
                            pj_dns_parsed_packet* resp)
{
    PJ_UNUSED_ARG(user_data);
    PJ_UNUSED_ARG(resp);
...
static int dns_test(void)
{
    ...
    /* Wait to allow active & bad period to complete, both will be in probing
     * state */
    PJ_LOG(3, (THIS_FILE, "  waiting for active NS to expire (%d sec)",
               set.good_ns_ttl));
    pj_thread_sleep(set.good_ns_ttl * 1000);
    ...
    /* Wait to allow probing period to complete, server 0 remains active, server
     * 1 will be bad */
                      ...
                      pj_dns_parsed_packet** p_res)
{
    ...
    if (res->ans == NULL) {
        res->ans =
          (pj_dns_parsed_rr*)pj_pool_calloc(pool, 4, sizeof(pj_dns_parsed_rr));
    }
...
static void srv_cb_1(void* user_data, pj_status_t status,
                     const pj_dns_srv_record* rec)
{
    ...
    PJ_ASSERT_ON_FAIL(
      pj_strcmp2(&rec->entry[0].server.name, "sip.somedomain.com") == 0,
      return );
    PJ_ASSERT_ON_FAIL(
      pj_strcmp2(&rec->entry[0].server.alias, "sipalias.somedomain.com") == 0,
      return );

    /* IPv4 only */
    PJ_ASSERT_ON_FAIL(rec->entry[0].server.addr_count == 1, return );
    PJ_ASSERT_ON_FAIL(rec->entry[0].server.addr[0].ip.v4.s_addr == IP_ADDR1,
                      return );
    PJ_ASSERT_ON_FAIL(rec->entry[0].port == PORT1, return );
}

static void srv_cb_1b(void* user_data, pj_status_t status,
                      const pj_dns_srv_record* rec)
{
    PJ_UNUSED_ARG(user_data);
...
static void srv_cb_1c(void* user_data, pj_status_t status,
                      const pj_dns_srv_record* rec)
{
    PJ_UNUSED_ARG(user_data);
    ...
    PJ_ASSERT_ON_FAIL(rec->entry[0].server.addr[0].af == pj_AF_INET() &&
                        rec->entry[0].server.addr[0].ip.v4.s_addr == IP_ADDR1,
                      return );
    PJ_ASSERT_ON_FAIL(
      rec->entry[0].server.addr[1].af == pj_AF_INET6() &&
        s6_addr32(rec->entry[0].server.addr[1].ip.v6, 0) == IP_ADDR1,
      return );
}

static void srv_cb_1d(void* user_data, pj_status_t status,
                      const pj_dns_srv_record* rec)
{
    PJ_UNUSED_ARG(user_data);
    ...
    PJ_ASSERT_ON_FAIL(rec->entry[0].server.addr_count == 1, return );
    PJ_ASSERT_ON_FAIL(
      rec->entry[0].server.addr[0].af == pj_AF_INET6() &&
        s6_addr32(rec->entry[0].server.addr[0].ip.v6, 0) == IP_ADDR1,
      return );
}
...
static int srv_resolver_test(void)
{
    ...
    status = pj_dns_srv_resolve(&domain, &res_name, 5061, pool, resolver,
                                PJ_TRUE, NULL, &srv_cb_1, NULL);
    pj_assert(status == PJ_SUCCESS);

    pj_sem_wait(sem);

    /* Because of previous tests, only NS 1 should get the request */
    pj_assert(g_server[0].pkt_count ==
              2); /* 2 because of SRV and A resolution */
    pj_assert(g_server[1].pkt_count == 0);
    ...
    status =
      pj_dns_srv_resolve(&domain, &res_name, 5061, pool, resolver,
                         PJ_DNS_SRV_RESOLVE_AAAA, NULL, &srv_cb_1c, NULL);
    pj_assert(status == PJ_SUCCESS);
    ...
    PJ_LOG(3,
           (THIS_FILE, "  srv_resolve(): option PJ_DNS_SRV_RESOLVE_AAAA_ONLY"));
    ...
    status =
      pj_dns_srv_resolve(&domain, &res_name, 5061, pool, resolver,
                         PJ_DNS_SRV_RESOLVE_AAAA_ONLY, NULL, &srv_cb_1d, NULL);
    pj_assert(status == PJ_SUCCESS);
...
#define SRV_CB_CHECK(cond, err) \
    if (!(cond)) { \
        *cb_err = err; \
        goto on_return; \
    }

static void srv_cb_2(void* user_data, pj_status_t status,
                     const pj_dns_srv_record* rec)
{
    ...
    SRV_CB_CHECK(
      pj_strcmp2(&rec->entry[0].server.alias, "sipalias01." TARGET) == 0, -60);
    ...
    SRV_CB_CHECK(rec->entry[0].server.addr[0].af == pj_AF_INET() &&
                   rec->entry[0].server.addr[0].ip.v4.s_addr == IP_ADDR2,
                 -90);
...
static void srv_cb_2a(void* user_data, pj_status_t status,
                      const pj_dns_srv_record* rec)
{
    int* cb_err = (int*)user_data;
...
static void srv_cb_2b(void* user_data, pj_status_t status,
                      const pj_dns_srv_record* rec)
{
    int* cb_err = (int*)user_data;
...
static int srv_resolver_fallback_test(void)
{
    ...
    status =
      pj_dns_srv_resolve(&domain, &res_name, PORT2, pool, resolver,
    ...
    status =
      pj_dns_srv_resolve(&domain, &res_name, PORT2, pool, resolver,
                         PJ_DNS_SRV_FALLBACK_AAAA, &cb_err, &srv_cb_2b, NULL);
    if (status != PJ_SUCCESS) {
        app_perror("   srv_resolve error", status);
        return -80;
    }
                      ...
                      pj_dns_parsed_packet** p_res)
{
    ...
    if (pkt->q[0].type == PJ_DNS_TYPE_SRV) {
        ...
        res->ans = (pj_dns_parsed_rr*)pj_pool_calloc(pool, SRV_COUNT3,
        ...
        res->ans = (pj_dns_parsed_rr*)pj_pool_calloc(pool, A_COUNT3,
                                                     sizeof(pj_dns_parsed_rr));
...
static void srv_cb_3(void* user_data, pj_status_t status,
                     const pj_dns_srv_record* rec)
{
    unsigned i;
    int* cb_err = (int*)user_data;
    ...
    for (i = 0; i < PJ_DNS_SRV_MAX_ADDR; ++i) {
        ...
        for (j = 0; j < PJ_DNS_MAX_IP_IN_A_REC; ++j) {
            SRV_CB_CHECK(
              rec->entry[i].server.addr[j].ip.v4.s_addr == IP_ADDR3 + j, -80);
        }
    }
