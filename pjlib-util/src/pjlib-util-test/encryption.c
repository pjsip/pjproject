...
static int sha1_test2(void)
{
    ...
    for (i = 0; i < 4; ++i) {
        ...
        for (j = 0; j < repeatcount[i]; ++j) {
            pj_sha1_update(&sha, (const pj_uint8_t*)testarray[i],
...
struct rfc2202_test rfc2202_test_vector[] = {
    ...
    { "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
      ...
      "\xdd\xdd\xdd\xdd\xdd\xdd\xdd\xdd\xdd\xdd"
      "\xdd\xdd\xdd\xdd\xdd\xdd\xdd\xdd\xdd\xdd",
      50, "\x56\xbe\x34\x52\x1d\x14\x4c\x88\xdb\xb8\xc7\x33\xf0\xe8\xb3\xf6",
      NULL },
    { "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
      "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa",
      20,
    ...
    { "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
      ...
      "Data",
      73, "\x6f\x63\x0f\xad\x67\xcd\xa0\xee\x1f\xb1\xf5\x62\xdb\x3a\xa5\x3e",
...
static int rfc2202_test(void)
{
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(rfc2202_test_vector); ++i) {
        ...
        PJ_ASSERT_RETURN(pj_ansi_strlen(rfc2202_test_vector[i].key) ==
                         ...
                         -52);
        PJ_ASSERT_RETURN(
          rfc2202_test_vector[i].md5_digest == NULL ||
            pj_ansi_strlen(rfc2202_test_vector[i].md5_digest) <= 16,
          -54);
        PJ_ASSERT_RETURN(
          rfc2202_test_vector[i].sha1_digest == NULL ||
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(rfc2202_test_vector); ++i) {
        ...
        pj_hmac_md5((pj_uint8_t*)rfc2202_test_vector[i].input,
                    ...
                    rfc2202_test_vector[i].key_len, digest);

        /* Check for overwrites */
        if (digest_buf[0] != '\0' || digest_buf[17] != '\0') {
            PJ_LOG(3, (THIS_FILE,
                       "    error: overwriting outside buffer on test %d", i));
            return -60;
        }
...
struct crc32_test_t
{
    ...
    pj_uint32_t crc;
                        ...
                        { /* Something read from /dev/random */
                          "\x21\x21\x98\x10\x62\x59\xbc\x58\x42\x24\xe5\xf3\x92"
                          "\x0a\x68\x3c\xa7\x67\x73\xc3",
                          0x506693be },
                        { "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", 0xcab11777 },
...
static int crc32_test(void)
{
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(crc32_test_data); ++i) {
        ...
        if (len / 2 > 0) {
            pj_crc32_update(&ctx,
                            (pj_uint8_t*)crc32_test_data[i].input + len / 2,
                            len - len / 2);
        }
        ...
        if (crc0 != crc1) {
            PJ_LOG(3,
                   (THIS_FILE, "    error: crc algorithm error on test %d", i));
            return -85;
        }
    }
    return 0;
...
static struct base64_test_vec
{
    ...
    { "foo", "Zm9v", ENCODE_DECODE },
    { "foob", "Zm9vYg==", ENCODE_DECODE },
    { "fooba", "Zm9vYmE=", ENCODE_DECODE },
    { "foobar", "Zm9vYmFy", ENCODE_DECODE },
    { "\x14\xfb\x9c\x03\xd9\x7e", "FPucA9l+", ENCODE_DECODE },
    { "\x14\xfb\x9c\x03\xd9", "FPucA9k=", ENCODE_DECODE },
    { "\x14\xfb\x9c\x03", "FPucAw==", ENCODE_DECODE },
    /* with whitespaces */
    { "foobar", "Zm9v\r\nYmFy", DECODE },
    { "foobar", "\nZ\r\nm 9\tv\nYm\nF\ny\n", DECODE },
};
