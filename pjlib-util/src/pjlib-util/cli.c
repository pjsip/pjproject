...
{
    const pj_str_t msg;
} arg_type[3] = { { { "Text", 4 } }, { { "Int", 3 } }, { { "Choice", 6 } } };
 ...
 */
static pj_status_t get_available_cmds(pj_cli_sess* sess, pj_cli_cmd_spec* cmd,
                                      pj_str_t* cmd_val, unsigned argc,
                                      pj_pool_t* pool, pj_bool_t get_cmd,
...
static pj_status_t cmd_handler(pj_cli_cmd_val* cval)
{
    ...
    case CLI_CMD_CHANGE_LOG:
        ...
        if (!level && cval->argv[1].slen > 0 &&
            (cval->argv[1].ptr[0] < '0' || cval->argv[1].ptr[0] > '9'))
        {
...
PJ_DEF(pj_status_t) pj_cli_create(pj_cli_cfg* cfg, pj_cli_t** p_cli)
{
    pj_pool_t* pool;
    pj_cli_t* cli;
    unsigned i;
    ...
    pool =
      pj_pool_create(cfg->pf, "cli", PJ_CLI_POOL_SIZE, PJ_CLI_POOL_INC, NULL);
    if (!pool)
        return PJ_ENOMEM;
...
PJ_DEF(void) pj_cli_register_front_end(pj_cli_t* cli, pj_cli_front_end* fe)
{
    pj_list_push_back(&cli->fe_head, fe);
}

PJ_DEF(void) pj_cli_quit(pj_cli_t* cli, pj_cli_sess* req, pj_bool_t restart)
{
    pj_cli_front_end* fe;
 ...
 **/
static pj_status_t add_choice_node(pj_cli_t* cli, pj_xml_node* xml_node,
                                   pj_cli_arg_spec* arg,
                                   pj_cli_get_dyn_choice get_choice)
{
    ...
    if (arg->stat_choice_cnt > 0) {
        ...
        arg->stat_choice_val = (pj_cli_arg_choice_val*)pj_pool_zalloc(
          cli->pool, arg->stat_choice_cnt * sizeof(pj_cli_arg_choice_val));
 ...
 **/
static pj_status_t add_arg_node(pj_cli_t* cli, pj_xml_node* xml_node,
                                pj_cli_cmd_spec* cmd, pj_cli_arg_spec* arg,
                                pj_cli_get_dyn_choice get_choice)
{
    pj_xml_attr* attr;
    pj_status_t status = PJ_SUCCESS;
 ...
 **/
static pj_status_t add_cmd_node(pj_cli_t* cli, pj_cli_cmd_spec* group,
                                ...
                                pj_cli_get_dyn_choice get_choice)
{
    ...
    while (attr != &root->attr_head) {
        if (!pj_stricmp2(&attr->name, "name")) {
            ...
            if (!attr->value.slen || (get_cmd_name(cli, group, &attr->value))) {
    ...
    while (sub_node != (pj_xml_node*)&root->node_head) {
        if (!pj_stricmp2(&sub_node->name, "CMD")) {
            status =
              add_cmd_node(cli, cmd, sub_node, handler, NULL, get_choice);
            if (status != PJ_SUCCESS)
                return status;
        } else if (!pj_stricmp2(&sub_node->name, "ARG")) {
            /* Get argument attribute */
            status =
              add_arg_node(cli, sub_node, cmd, &args[cmd->arg_cnt], get_choice);
    ...
    if (cmd->arg_cnt) {
        ...
        cmd->arg = (pj_cli_arg_spec*)pj_pool_zalloc(
          cli->pool, cmd->arg_cnt * sizeof(pj_cli_arg_spec));
    ...
    if (cmd->sc_cnt) {
        ...
        cmd->sc =
          (pj_str_t*)pj_pool_zalloc(cli->pool, cmd->sc_cnt * sizeof(pj_str_t));
    ...
    pj_hash_set(cli->pool, cli->cmd_id_hash, &cmd->id, sizeof(pj_cli_cmd_id), 0,
                cmd);
                  ...
                  pj_pool_t* pool, pj_cli_exec_info* info)
{
    ...
    } else if (len > 0 && (cmdline[len - 1] == '\t' || cmdline[len - 1] == '?'))
    {
        cmdline[--len] = 0;
        if (len == 0) {
    ...
    if (len > 0) {
        ...
        PJ_TRY
        {
            ...
            while (!pj_scan_is_eof(&scanner)) {
                ...
                status =
                  get_available_cmds(sess, cmd, &str, val->argc - 1, pool,
                                     PJ_TRUE, parse_mode, &next_cmd, info);
                 ...
                 pj_cli_exec_info* info)
{
    ...
    if ((val.argc > 0) && (val.cmd->handler)) {
        ...
        if (info->cmd_ret == PJ_CLI_EINVARG || info->cmd_ret == PJ_CLI_EEXIT) {
            return info->cmd_ret;
        }
    }
...
static pj_bool_t hint_inserted(const pj_str_t* name, const pj_str_t* desc,
                               const pj_str_t* type, pj_cli_exec_info* info)
{
    unsigned i;
...
/** This will insert new hint with the option to check for the same
    previous entry **/
static pj_status_t insert_new_hint2(pj_pool_t* pool, pj_bool_t unique_insert,
                                    const pj_str_t* name, const pj_str_t* desc,
                                    const pj_str_t* type,
                                    pj_cli_exec_info* info)
{
...
static pj_status_t insert_new_hint(pj_pool_t* pool, const pj_str_t* name,
                                   const pj_str_t* desc, const pj_str_t* type,
                                   pj_cli_exec_info* info)
{
...
static pj_status_t get_comp_match_cmds(const pj_cli_t* cli,
                                       const pj_cli_cmd_spec* group,
                                       const pj_str_t* cmd_val, pj_pool_t* pool,
                                       pj_cli_cmd_spec** p_cmd,
                                       pj_cli_exec_info* info)
{
    pj_cli_cmd_spec* cmd;
                                              ...
                                              pj_cli_exec_info* info)
{
    ...
    while (it) {
        unsigned i;
        pj_cli_cmd_spec* cmd =
          (pj_cli_cmd_spec*)pj_hash_this(cli->cmd_name_hash, it);
...
static pj_status_t get_pattern_match_cmds(
  pj_cli_cmd_spec* cmd, const pj_str_t* cmd_val, pj_pool_t* pool,
  pj_cli_cmd_spec** p_cmd, pj_cli_parse_mode parse_mode, pj_cli_exec_info* info)
{
    pj_status_t status;
...
static pj_status_t get_match_args(pj_cli_sess* sess, pj_cli_cmd_spec* cmd,
                                  const pj_str_t* cmd_val, unsigned argc,
                                  pj_pool_t* pool, pj_cli_parse_mode parse_mode,
                                  pj_cli_exec_info* info)
{
    pj_cli_arg_spec* arg;
    pj_status_t status = PJ_SUCCESS;
            ...
            for (j = 0; j < arg->stat_choice_cnt; ++j) {
                ...
                if (!pj_strncmp(&choice_val->value, cmd_val, cmd_val->slen)) {
                    status = insert_new_hint(
                      pool, &choice_val->value, &choice_val->desc,
                      &arg_type[PJ_CLI_ARG_CHOICE].msg, info);
                    if (status != PJ_SUCCESS)
                        return status;
                }
            }
            ...
            if (cmd_val->slen == 0) {
                if (info->hint_cnt == 0) {
                    if (!((parse_mode == PARSE_EXEC) && (arg->optional))) {
                        ...
                        status =
                          insert_new_hint(pool, &arg->name, &arg->desc,
                                      ...
                                      pj_cli_exec_info* info)
{
    ...
    if (get_cmd) {
        status =
          get_comp_match_cmds(sess->fe->cli, cmd, prefix, pool, p_cmd, info);
        if (status != PJ_SUCCESS)
            return status;
    ...
    if (argc > 0)
        status =
          get_match_args(sess, cmd, prefix, argc, pool, parse_mode, info);
