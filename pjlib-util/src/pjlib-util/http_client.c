...
static const char* http_protocol_names[NUM_PROTOCOL] = { "HTTP", "HTTPS" };

static const unsigned int http_default_port[NUM_PROTOCOL] = { 80, 443 };
...
static const char* http_method_names[3] = { "GET", "PUT", "DELETE" };
...
/* Parse the response */
static pj_status_t http_response_parse(pj_pool_t* pool, pj_http_resp* response,
...
static pj_bool_t http_on_connect(pj_activesock_t* asock, pj_status_t status)
{
                                   ...
                                   pj_ioqueue_op_key_t* op_key, pj_ssize_t sent)
{
    ...
    if (hreq->tcp_state.current_send_size == hreq->tcp_state.send_size) {
        ...
        if (hreq->param.reqdata.total_size > 0 || hreq->param.reqdata.size > 0)
        {
            ...
            if (hreq->param.reqdata.total_size > 0 && hreq->cb.on_send_data) {
...
static pj_bool_t http_on_data_read(pj_activesock_t* asock, void* data,
                                   ...
                                   pj_size_t* remainder)
{
    ...
    if (hreq->state == READING_RESPONSE) {
        ...
        /* Parse the response. */
        st = http_response_parse(hreq->pool, &hreq->response, data, size, &rem);
            ...
            *remainder = size;
            ...
            if (hreq->response.status_code == 401 ||
                ...
                for (i = 0; i < hdrs->count; i++) {
                    ...
                    {
                        status =
                          parse_auth_chal(hreq->pool, &hdrs->header[i].value,
                                          &response->auth_chal);
                        break;
                    }
                }
                ...
                if (status == PJ_SUCCESS && hreq->auth_state == AUTH_NONE &&
            ...
            if (rem > 0 || hreq->response.content_length == 0)
                return http_on_data_read(
                  asock, (rem == 0 ? NULL : (char*)data + size - rem), rem,
                  PJ_SUCCESS, NULL);
        }
    ...
    if (hreq->cb.on_data_read) {
        ...
        if (hreq->response.size == 0) {
            ...
            hreq->response.data =
              pj_pool_alloc(hreq->pool, hreq->response.size);
        }

        /* If the size of data received exceeds its current size,
         * grow the buffer by a factor of 2.
         */
        if (hreq->tcp_state.current_read_size + size > hreq->response.size) {
            void* olddata = hreq->response.data;

            hreq->response.data =
              pj_pool_alloc(hreq->pool, hreq->response.size << 1);
        ...
        pj_memcpy(
          (char*)hreq->response.data + hreq->tcp_state.current_read_size, data,
          size);
    }
...
{
    pj_scanner scanner;
    const pj_str_t REALM_STR = { "realm", 5 }, NONCE_STR = { "nonce", 5 },
                   ALGORITHM_STR = { "algorithm", 9 },
                   STALE_STR = { "stale", 5 }, QOP_STR = { "qop", 3 },
                   OPAQUE_STR = { "opaque", 6 };
    ...
    PJ_TRY
    {
        ...
        for (;;) {
            ...
            if (!pj_stricmp(&name, &REALM_STR)) {
                ...
                chal->stale = value.slen && (*value.ptr != '0') &&
                              (*value.ptr != 'f') && (*value.ptr != 'F');
            }
                                       ...
                                       pj_size_t* remainder)
{
    ...
    *remainder = size - 1 - i;
    ...
    if (size > 0) {
        status = http_headers_parse(end_status + 1, size, &response->headers);
    } else {
        status = PJ_SUCCESS;
    }
    ...
    for (i = 0; i < response->headers.count; i++) {
        if (!pj_stricmp(&response->headers.header[i].name, &STR_CONTENT_LENGTH))
        {
                   ...
                   const pj_http_req_callback* hcb, pj_http_req** http_req)
{
    ...
    PJ_ASSERT_RETURN(pool && url && timer && ioqueue && hcb && http_req,
                     PJ_EINVAL);
    ...
    *http_req = NULL;
    ...
    if ((at_pos = get_url_at_pos(hreq->url.ptr, hreq->url.slen)) != NULL) {
        ...
        pj_memmove(user_pos, at_pos + 1,
                   hreq->url.ptr + hreq->url.slen - at_pos - 1);
                                  ...
                                  pj_bool_t notify_on_fail)
{
    ...
    if (!http_req->resolved) {
        ...
        status = pj_sockaddr_init(http_req->param.addr_family, &http_req->addr,
                                  &http_req->hurl.host, http_req->hurl.port);
        if (status != PJ_SUCCESS || !pj_sockaddr_has_addr(&http_req->addr) ||
    ...
    status =
      pj_sock_socket(http_req->param.addr_family, pj_SOCK_STREAM(), 0, &sock);
    if (status != PJ_SUCCESS)
    ...
    if (status != PJ_SUCCESS) {
        PJ_PERROR(1,
                  (THIS_FILE, status, "Unable to bind to the requested port"));
        pj_sock_close(sock);
        goto on_return;
    }
    ...
    status = pj_activesock_create(http_req->pool, sock, pj_SOCK_STREAM(), NULL,
                                  http_req->ioqueue, &asock_cb, http_req,
                                  &http_req->asock);
    if (status != PJ_SUCCESS) {
        pj_sock_close(sock);
...
static void auth_create_digest_response(
  pj_str_t* result, const pj_http_auth_cred* cred, const pj_str_t* nonce,
  const pj_str_t* nc, const pj_str_t* cnonce, const pj_str_t* qop,
  const pj_str_t* uri, const pj_str_t* realm, const pj_str_t* method)
{
    char ha1[MD5_STRLEN];
    char ha2[MD5_STRLEN];
...
static pj_status_t auth_respond_digest(pj_http_req* hreq)
{
    ...
    if (chal->qop.slen == 0) {
        ...
        auth_create_digest_response(&digest_response, cred, &chal->nonce, NULL,
                                    NULL, NULL, &hreq->hurl.path, &chal->realm,
                                    &hreq->param.method);

        max_len = len;
        len = pj_ansi_snprintf(phdr->value.ptr, max_len,
                               "Digest username=\"%.*s\", "
                               "realm=\"%.*s\", "
                               "nonce=\"%.*s\", "
                               "uri=\"%.*s\", "
                               "algorithm=%.*s, "
                               "response=\"%.*s\"",
                               STR_PREC(cred->username), STR_PREC(chal->realm),
                               STR_PREC(chal->nonce), STR_PREC(hreq->hurl.path),
                               STR_PREC(STR_MD5), STR_PREC(digest_response));
        ...
        auth_create_digest_response(&digest_response, cred, &chal->nonce, &nc,
                                    &cnonce, &qop, &hreq->hurl.path,
                                    &chal->realm, &hreq->param.method);
        max_len = len;
        len = pj_ansi_snprintf(phdr->value.ptr, max_len,
                               "Digest username=\"%.*s\", "
                               "realm=\"%.*s\", "
                               ...
                               STR_PREC(chal->nonce), STR_PREC(hreq->hurl.path),
                               STR_PREC(STR_MD5), STR_PREC(digest_response),
                               STR_PREC(qop), STR_PREC(nc), STR_PREC(cnonce));
        if (len < 0 || len >= max_len)
            return PJ_ETOOSMALL;
        phdr->value.slen = len;
        ...
        TRACE_((THIS_FILE, "Error: Unsupported qop offer %.*s", chal->qop.slen,
                chal->qop.ptr));
        return PJ_ENOTSUP;
    }
 ...
 */
static void str_snprintf(pj_str_t* s, size_t size, pj_bool_t append,
                         const char* format, ...)
{
    va_list arg;
    int retval;
    ...
    if (!append)
        s->slen = 0;
    size -= s->slen;
    retval = pj_ansi_vsnprintf(s->ptr + s->slen, size, format, arg);
...
static pj_status_t http_req_start_sending(pj_http_req* hreq)
{
    ...
    PJ_ASSERT_RETURN(
      hreq->state == SENDING_REQUEST || hreq->state == SENDING_REQUEST_BODY,
      PJ_EBUG);
    ...
    if (hreq->state == SENDING_REQUEST) {
        ...
        str_snprintf(&pkt, BUF_SIZE, PJ_TRUE, "%.*s %.*s %s/%.*s\r\n",
                     STR_PREC(hreq->param.method), STR_PREC(hreq->hurl.path),
        ...
        if (!pj_strcmp2(&hreq->param.method, http_method_names[HTTP_PUT])) {
            ...
            pj_utoa(hreq->param.reqdata.total_size ?
                      ...
                      (unsigned long)hreq->param.reqdata.size,
                    buf);
    ...
    status = pj_activesock_send(hreq->asock, &hreq->op_key, pkt.ptr, &len, 0);
...
PJ_DEF(pj_status_t) pj_http_req_cancel(pj_http_req* http_req, pj_bool_t notify)
{
    ...
    if (notify && http_req->cb.on_complete) {
        (*http_req->cb.on_complete)(
          http_req, (!http_req->error ? PJ_ECANCELLED : http_req->error), NULL);
    }
