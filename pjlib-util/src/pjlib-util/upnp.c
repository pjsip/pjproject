...
static const char* check_error_response(IXML_Document* doc)
{
    ...
    if (error_code) {
        ...
        PJ_LOG(3, (THIS_FILE, "Response error code: %s (%s)", error_code,
                   error_desc));
    }
...
static const char* action_get_external_ip(struct igd* igd)
{
    ...
    pj_sockaddr_parse(pj_AF_UNSPEC(), 0, &igd->public_ip, &igd->public_ip_addr);
...
static void download_igd_xml(unsigned dev_idx)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        pj_strdup2_with_null(upnp_mgr.pool, &igd_dev->service_type,
                             service_type);
        ...
        PJ_LOG(4,
               (THIS_FILE,
                ...
                "\tControl URL  : %s\n"
                "\tPublic IP    : %s",
                igd_dev->dev_id.ptr, friendly_name, igd_dev->service_type.ptr,
                igd_dev->control_url.ptr, public_ip));
                     ...
                     void* user_data)
{
    ...
    if (!pj_thread_is_registered()) {
        ...
        pj_thread_register("upnp_cb", upnp_mgr.thread_desc, &upnp_mgr.thread);
    }
    ...
    case UPNP_CONTROL_ACTION_COMPLETE: {
        ...
        if (err_code != UPNP_E_SUCCESS) {
            PJ_LOG(4, (THIS_FILE,
                       "Port mapping deletion action complete "
                       "error: %d (%s)",
                       err_code, UpnpGetErrorMessage(err_code)));
            break;
        }
        ...
        if (!response) {
            PJ_LOG(4, (THIS_FILE,
                       "Failed to get response to delete port "
                       "mapping"));
        } else {
            if (!check_error_response(response)) {
...
static void search_igd(int search_time)
{
    ...
    err = UpnpSearchAsync(upnp_mgr.client_hnd, search_time, UPNP_ROOT_DEVICE,
                          &upnp_mgr);
    ...
    err = UpnpSearchAsync(upnp_mgr.client_hnd, search_time, UPNP_IGD_DEVICE,
                          &upnp_mgr);
    if (err != UPNP_E_SUCCESS) {
    ...
    err = UpnpSearchAsync(upnp_mgr.client_hnd, search_time, UPNP_WANIP_SERVICE,
                          &upnp_mgr);
    if (err != UPNP_E_SUCCESS) {
    ...
    err = UpnpSearchAsync(upnp_mgr.client_hnd, search_time, UPNP_WANPPP_SERVICE,
                          &upnp_mgr);
    if (err != UPNP_E_SUCCESS) {
...
#    endif
    ...
    if (upnp_err != UPNP_E_SUCCESS) {
        PJ_LOG(1, (THIS_FILE,
                   "Failed to initialize libupnp with "
                   "interface %s: %s",
...
#    endif
    if (param->if_name) {
        PJ_LOG(
          4, (THIS_FILE, "UPnP initialized with interface %s", param->if_name));
    }
    if (ip_address6 && port6) {
        PJ_LOG(4, (THIS_FILE,
                   "UPnP initialized on %s:%u (IPv4) and "
                   "%s:%u (IPv6)",
                   ip_address, port, ip_address6, port6));
    } else {
                         ...
                         unsigned ext_port[], pj_sockaddr mapped_addr[])
{
    ...
    for (i = 0; i < sock_cnt; i++) {
        ...
        /* Create action XML. */
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr,
                        "NewRemoteHost", "");
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr,
                        "NewExternalPort", pext_port);
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr,
                        "NewProtocol", "UDP");
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr,
                        "NewInternalPort", int_port_buf);
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr,
                        "NewInternalClient", addr_buf);
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr,
                        "NewEnabled", "1");
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr,
        ...
        /* Send the action XML. */
        upnp_err =
          UpnpSendAction(upnp_mgr.client_hnd, igd->control_url.ptr,
                         igd->service_type.ptr, NULL, action, &response);
        if (upnp_err != UPNP_E_SUCCESS || !response) {
            PJ_LOG(3, (THIS_FILE,
                       "Failed to %s IGD %s to add port mapping "
                       "for %s:%s -> %s:%s: %d (%s)",
                       response ? "send action to" : "get response from",
                       igd->dev_id.ptr, igd->public_ip.ptr, pext_port, addr_buf,
                       int_port_buf, upnp_err, UpnpGetErrorMessage(upnp_err)));
            status = PJ_ETIMEDOUT;
        }

        TRACE_("Add port mapping XML action:\n%s", ixmlPrintDocument(action));
        ...
        pj_sockaddr_set_str_addr(bound_addr.addr.sa_family, &mapped_addr[i],
                                 &igd->public_ip);
        ...
        PJ_LOG(4, (THIS_FILE,
                   "Successfully add port mapping to IGD %s: "
                   "%s:%s -> %s:%s",
                   igd->dev_id.ptr, igd->public_ip.ptr, pext_port, addr_buf,
                   int_port_buf));
    }
...
PJ_DEF(pj_status_t) pj_upnp_del_port_mapping(const pj_sockaddr* mapped_addr)
{
    ...
    if (upnp_err == UPNP_E_SUCCESS) {
        PJ_LOG(4, (THIS_FILE,
                   "Successfully sending async action to "
                   "delete port mapping to IGD %s for "
                   "%s:%s",
                   igd->dev_id.ptr, igd->public_ip.ptr, ext_port_buf));
    } else {
        PJ_LOG(3, (THIS_FILE,
                   "Failed to send action to IGD %s to delete "
                   "port mapping for %s:%s: %d (%s)",
                   igd->dev_id.ptr, igd->public_ip.ptr, ext_port_buf, upnp_err,
                   UpnpGetErrorMessage(upnp_err)));
        status = PJ_EINVALIDOP;
    }
