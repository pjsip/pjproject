...
static pj_status_t telnet_sess_send(cli_telnet_sess* sess, const pj_str_t* str);
 ...
 */
static pj_bool_t recv_buf_insert(telnet_recv_buf* recv_buf, unsigned char* data)
{
 ...
 */
static pj_status_t insert_history(cli_telnet_sess* sess, char* cmd_val)
{
    ...
    if (!in_history) {
        if (pj_list_size(sess->history) < PJ_CLI_MAX_CMD_HISTORY) {
            ...
            data_history = (char*)pj_pool_calloc(sess->pool, sizeof(char),
                                                 PJ_CLI_MAX_CMDBUF);
 ...
 * referenced - (RFC-854).
 */
static pj_bool_t send_telnet_cmd(cli_telnet_sess* sess, cli_telnet_command cmd,
 ...
 * negotiation process.
 */
static pj_bool_t send_enable_option(cli_telnet_sess* sess, pj_bool_t is_local,
                                    unsigned char option)
{
...
static pj_bool_t send_cmd_do(cli_telnet_sess* sess, unsigned char option)
{
    return send_enable_option(sess, PJ_FALSE, option);
}

static pj_bool_t send_cmd_will(cli_telnet_sess* sess, unsigned char option)
{
...
static pj_bool_t receive_enable_option(cli_telnet_sess* sess,
                                       pj_bool_t is_local, unsigned char option)
{
...
static void set_local_option(cli_telnet_sess* sess, unsigned char option,
                             pj_bool_t enable)
{
    sess->telnet_option[option].local_is_enable = enable;
}

static void set_peer_option(cli_telnet_sess* sess, unsigned char option,
                            pj_bool_t enable)
{
 ...
 */
static void send_err_arg(cli_telnet_sess* sess, const pj_cli_exec_info* info,
...
static void send_inv_arg(cli_telnet_sess* sess, const pj_cli_exec_info* info,
                         pj_bool_t with_return, pj_bool_t with_last_cmd)
{
...
static void send_hint_arg(cli_telnet_sess* sess, pj_str_t* send_data,
                          const pj_str_t* desc, pj_ssize_t cmd_len,
                          pj_ssize_t max_len)
{
    if ((desc) && (desc->slen > 0)) {
        int j;
 ...
 */
static void send_ambi_arg(cli_telnet_sess* sess, const pj_cli_exec_info* info,
                          pj_bool_t with_return, pj_bool_t with_last_cmd)
{
    unsigned i;
    pj_size_t len;
    ...
    for (i = 0; i < info->hint_cnt; ++i) {
            ...
            (pj_strncmp(&hint[i].desc, &hint[i + 1].desc, hint[i].desc.slen)))
        {
            ...
            send_hint_arg(sess, &send_data, &hint[i].desc, cmd_length,
                          max_length);
 ...
 */
static void send_comp_arg(cli_telnet_sess* sess, pj_cli_exec_info* info)
{
    pj_str_t send_data;
    char data[128];
...
static pj_bool_t handle_tab(cli_telnet_sess* sess)
{
    ...
    pool =
      pj_pool_create(sess->pool->factory, "handle_tab", PJ_CLI_TELNET_POOL_SIZE,
                     PJ_CLI_TELNET_POOL_INC, NULL);
    ...
    case PJ_SUCCESS:
        if (len > sess->rcmd->cur_pos) {
            ...
            unsigned char* data_sent =
              &sess->rcmd->rbuf[sess->rcmd->cur_pos - 1];
        ...
        if (info.hint_cnt > 0) {
            ...
            if (get_last_token(&cmd, &last_token) == PJ_SUCCESS) {
                ...
                if (hint_info->slen >= last_token.slen) {
                    ...
                    pj_memmove(hint_info->ptr, &hint_info->ptr[last_token.slen],
                               hint_info->slen);
                }
...
static pj_bool_t handle_return(cli_telnet_sess* sess)
{
    ...
    pool =
      pj_pool_create(sess->pool->factory, "handle_return",
                     PJ_CLI_TELNET_POOL_SIZE, PJ_CLI_TELNET_POOL_INC, NULL);

    status =
      pj_cli_sess_exec(&sess->base, (char*)&sess->rcmd->rbuf, pool, &info);
...
static pj_status_t process_vt100_cmd(cli_telnet_sess* sess, unsigned char* cmd)
{
...
static pj_status_t telnet_sess_send(cli_telnet_sess* sess, const pj_str_t* str)
{
    ...
    if (sess->buf_len == 0)
        status =
          pj_activesock_send(sess->asock, &sess->op_key, str->ptr, &sz, 0);
    /* If we cannot send now, append it at the end of the buffer
     * to be sent later.
     */
    if (sess->buf_len > 0 || (status != PJ_SUCCESS && status != PJ_EPENDING)) {
        int clen = (int)sz;
                                                ...
                                                const pj_str_t* str)
{
    ...
    pj_scan_init(&scanner, str->ptr, str->slen, PJ_SCAN_AUTOSKIP_WS,
                 &on_syntax_error);
                                          ...
                                          pj_ssize_t sent)
{
    cli_telnet_sess* sess =
      (cli_telnet_sess*)pj_activesock_get_user_data(asock);
...
static pj_bool_t telnet_sess_on_data_read(pj_activesock_t* asock, void* data,
...
static pj_bool_t telnet_fe_on_accept(pj_activesock_t* asock, pj_sock_t newsock,
                                     ...
                                     int src_addr_len, pj_status_t status)
{
    ...
    if (status != PJ_SUCCESS && status != PJ_EPENDING) {
        ...
        if (status == PJ_ESOCKETSTOP) {
            ...
            if (sstatus != PJ_SUCCESS) {
                ...
                TRACE_(
                  (THIS_FILE, "Error restarting telnet (status=%d)", status));
            }
        }
    ...
    pool =
      pj_pool_create(fe->pool->factory, "telnet_sess", PJ_CLI_TELNET_POOL_SIZE,
                     PJ_CLI_TELNET_POOL_INC, NULL);
    if (!pool) {
        TRACE_((THIS_FILE, "Not enough memory to create a new telnet session"));
        return PJ_TRUE;
    }
    ...
    sstatus =
      pj_mutex_create_recursive(pool, "mutex_telnet_sess", &sess->smutex);
    if (sstatus != PJ_SUCCESS)
        goto on_exit;

    sstatus =
      pj_activesock_create(pool, newsock, pj_SOCK_STREAM(), NULL,
                           fe->cfg.ioqueue, &asock_cb, sess, &sess->asock);
    if (sstatus != PJ_SUCCESS) {
                     ...
                     pj_cli_front_end** p_fe)
{
    ...
    pool =
      pj_pool_create(pj_cli_get_param(cli)->pf, "telnet_fe",
                     PJ_CLI_TELNET_POOL_SIZE, PJ_CLI_TELNET_POOL_INC, NULL);
...
static pj_status_t telnet_start(cli_telnet_fe* fe)
{
    ...
    val = 1;
    status =
      pj_sock_setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &val, sizeof(val));
    ...
    for (msec = MIN_WAIT_ON_TELNET_RESTART, restart_retry = 0;
         restart_retry < MAX_RETRY_ON_TELNET_RESTART; ++restart_retry,
        msec = (msec < MAX_WAIT_ON_TELNET_RESTART ? msec * 2 :
                                                    MAX_WAIT_ON_TELNET_RESTART))
    {
    ...
    if (status == PJ_SUCCESS) {
        ...
        if (fe->cfg.prompt_str.slen == 0) {
            ...
            char* prompt_data =
              pj_pool_alloc(fe->pool, pj_gethostname()->slen + 2);
    ...
    status = pj_activesock_create(fe->pool, sock, pj_SOCK_STREAM(), NULL,
                                  fe->cfg.ioqueue, &asock_cb, fe, &fe->asock);
    if (status != PJ_SUCCESS)
        goto on_exit;
    ...
    if (fe->own_ioqueue) {
        ...
        status =
          pj_thread_create(fe->pool, "worker_telnet_fe", &poll_worker_thread,
                           fe, 0, 0, &fe->worker_thread);
        if (status != PJ_SUCCESS)
            goto on_exit;
    }
