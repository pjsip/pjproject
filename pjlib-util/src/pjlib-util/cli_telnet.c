...
static pj_bool_t recv_buf_delete(telnet_recv_buf *recv_buf)
{
    if ((recv_buf->len == 0) || (recv_buf_right_len(recv_buf) == 0)) {
	return PJ_FALSE;
    } else {
	unsigned rlen;
	unsigned cur_pos = recv_buf->cur_pos;
	rlen = recv_buf_right_len(recv_buf)-1;
	pj_memmove(&recv_buf->rbuf[cur_pos], &recv_buf->rbuf[cur_pos+1], 
		   rlen);
	--recv_buf->cur_pos;
	--recv_buf->len;
	recv_buf->rbuf[recv_buf->len] = 0;
...
/**
 * This method will handle sending telnet's DISABLE option negotiation.
 * For local option: send WON'T.
 * For remote option: send DON'T.
 * This method also handle the state transition of the DISABLE 
 * negotiation process. 
 */
static pj_bool_t send_disable_option(cli_telnet_sess *sess, 
				     pj_bool_t is_local, 
				     unsigned char option)
{
    cli_telnet_sess_option *sess_option;
...
static pj_bool_t send_cmd_dont(cli_telnet_sess *sess, 
			       unsigned char option)
{
    return send_disable_option(sess, PJ_FALSE, option);
}
...
static pj_bool_t send_cmd_wont(cli_telnet_sess *sess, 
			       unsigned char option)
{
    return send_disable_option(sess, PJ_TRUE, option);
}
...
static pj_bool_t is_peer_option_state_ena(cli_telnet_sess *sess, 
					  unsigned char option)
{    
    return (sess->telnet_option[option].peer_state == OPT_ENABLE);
}
...
static void send_bell(cli_telnet_sess *sess) {
    unsigned char bell[1] = {0x07};
    telnet_sess_send2(sess, &bell[0], 1);
}
...
static pj_bool_t handle_left_key(cli_telnet_sess *sess)
{
	...
	telnet_sess_send2(sess, &BACK_SPACE, 1);
	--sess->rcmd->cur_pos;
	return PJ_TRUE;
    }
    return PJ_FALSE;
...
static pj_bool_t handle_up_down(cli_telnet_sess *sess, pj_bool_t is_up)
{
	...
	if (sess->rcmd->cur_pos > 0) {
	    pj_memset(send_data.ptr, 0x08, sess->rcmd->cur_pos);
        ...
        (status != PJ_SUCCESS && status != PJ_EPENDING))
    {
        ...
        if (clen < sz) {
            ...
            sess->buf_len = CLI_TELNET_BUF_SIZE +
                            pj_ansi_strlen((char *)sess->buf+CLI_TELNET_BUF_SIZE);
        } else
            sess->buf_len += clen;
		                    ...
		                    const char *data, int len)
{
	...
	if (tsess->base.log_level > level)
            telnet_sess_send2(tsess, (unsigned char *)data, len);
    }
