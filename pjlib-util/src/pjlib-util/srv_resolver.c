...
pj_dns_srv_resolve(const pj_str_t* domain_name, const pj_str_t* res_name,
                   ...
                   pj_dns_srv_resolver_cb* cb, pj_dns_srv_async_query** p_query)
{
    pj_size_t len;
    pj_str_t target_name;
    ...
    PJ_ASSERT_RETURN(domain_name && domain_name->slen && res_name &&
                       res_name->slen && pool && resolver && cb,
                     PJ_EINVAL);
    ...
    PJ_LOG(
      5, (query_job->objname, "Starting async DNS %s query_job: target=%.*s:%d",
          pj_dns_get_type_name(query_job->dns_state), (int)target_name.slen,
          target_name.ptr, def_port));

    status = pj_dns_resolver_start_query(resolver, &target_name,
                                         query_job->dns_state, 0, &dns_callback,
                                 ...
                                 pj_dns_parsed_packet* response)
{
    ...
    query_job->srv_cnt = 0;
    for (i = 0;
         i < response->hdr.anscount && query_job->srv_cnt < PJ_DNS_SRV_MAX_ADDR;
         ++i)
    {
    ...
    if (query_job->srv_cnt == 0) {
        PJ_LOG(
          4, (query_job->objname, "Could not find SRV record in DNS answer!"));
        return;
    }
    ...
    for (i = 0; i < query_job->srv_cnt; ++i) {
        ...
        for (j = i + 1; j < query_job->srv_cnt && query_job->srv[j].priority ==
                                                    query_job->srv[i].priority;
             ++j)
        {
    ...
    for (i = 0; i < response->hdr.arcount; ++i) {
        ...
        for (j = 0; j < query_job->srv_cnt; ++j) {
                ...
                query_job->srv[j].addr_cnt < ADDR_MAX_COUNT)
            {
                ...
                if (rr->type == PJ_DNS_TYPE_A) {
                    pj_sockaddr_init(pj_AF_INET(), &query_job->srv[j].addr[cnt],
                                     NULL, query_job->srv[j].port);
    ...
    for (i = 0; i < query_job->srv_cnt; ++i) {
        ...
        if ((query_job->option & PJ_DNS_SRV_RESOLVE_AAAA_ONLY) == 0 &&
            pj_inet_pton(pj_AF_INET(), &query_job->srv[i].target_name, &addr) ==
              PJ_SUCCESS)
        {
            pj_sockaddr_init(pj_AF_INET(), &query_job->srv[i].addr[cnt], NULL,
            ...
            ++query_job->host_resolved;
                   ...
                   pj_inet_pton(pj_AF_INET6(), &query_job->srv[i].target_name,
                                &addr6) == PJ_SUCCESS)
        {
            pj_sockaddr_init(pj_AF_INET6(), &query_job->srv[i].addr[cnt], NULL,
                             query_job->srv[i].port);
    ...
    PJ_LOG(5, (query_job->objname,
               ...
               (int)query_job->full_name.slen, query_job->full_name.ptr,
               query_job->srv_cnt, response->hdr.anscount,
               (query_job->srv_cnt ? ':' : ' ')));
    ...
    for (i = 0; i < query_job->srv_cnt; ++i) {
        ...
        if (query_job->srv[i].addr_cnt != 0) {
            pj_sockaddr_print(&query_job->srv[i].addr[0], addr, sizeof(addr),
                              2);
        } else
            pj_ansi_strcpy(addr, "-");

        PJ_LOG(5,
               (query_job->objname, " %d: SRV %d %d %d %.*s (%s)", i,
                query_job->srv[i].priority, query_job->srv[i].weight,
                query_job->srv[i].port, (int)query_job->srv[i].target_name.slen,
                query_job->srv[i].target_name.ptr, addr));
    }
}
...
static pj_status_t resolve_hostnames(pj_dns_srv_async_query* query_job)
{
    ...
    for (i = 0; i < query_job->srv_cnt; ++i) {
        ...
        PJ_LOG(5,
               (query_job->objname, "Starting async DNS A query_job for %.*s",
                (int)srv->target_name.slen, srv->target_name.ptr));
        ...
        if ((query_job->option & PJ_DNS_SRV_RESOLVE_AAAA_ONLY) == 0) {
            ...
            status = pj_dns_resolver_start_query(
              query_job->resolver, &srv->target_name, PJ_DNS_TYPE_A, 0,
              &dns_callback, &srv->common, &srv->q_a);
        }
        ...
        if (status == PJ_SUCCESS &&
            (query_job->option & PJ_DNS_SRV_RESOLVE_AAAA) != 0) {
            status = pj_dns_resolver_start_query(
              query_job->resolver, &srv->target_name, PJ_DNS_TYPE_AAAA, 0,
              &dns_callback, &srv->common_aaaa, &srv->q_aaaa);
        }

        /* See also #1809: dns_callback() will be invoked synchronously when
         * response is available in the cache, and var
         * 'query_job->host_resolved' will get incremented within the
         * dns_callback(), which will cause this function returning false error,
         * so don't use that variable for counting errors.
         */
        if (status != PJ_SUCCESS) {
            query_job->host_resolved++;
            err_cnt++;
                         ...
                         pj_dns_parsed_packet* pkt)
{
    ...
    if (query_job->dns_state == PJ_DNS_TYPE_SRV) {
        ...
        if (status == PJ_SUCCESS) {
            ...
            PJ_LOG(
              4, (query_job->objname, "DNS SRV resolution failed for %.*s: %s",
                  (int)query_job->full_name.slen, query_job->full_name.ptr,
                  errmsg));
        ...
        if (query_job->srv_cnt == 0 && query_job->domain_part.slen > 0) {
            ...
            if (query_job->option & PJ_DNS_SRV_FALLBACK_AAAA)
                new_option |=
                  (PJ_DNS_SRV_RESOLVE_AAAA | PJ_DNS_SRV_RESOLVE_AAAA_ONLY);
        ...
        if (status == PJ_SUCCESS && pkt->hdr.anscount != 0) {
            ...
            for (i = 0; i < rec.addr_count && srv->addr_cnt < ADDR_MAX_COUNT;
                 ++i) {
                ...
                if (is_type_a && rec.addr[i].af == pj_AF_INET()) {
                    pj_sockaddr_init(pj_AF_INET(), &srv->addr[srv->addr_cnt],
                                     NULL, srv->port);
                    srv->addr[srv->addr_cnt].ipv4.sin_addr = rec.addr[i].ip.v4;
                    added = PJ_TRUE;
                } else if (!is_type_a && rec.addr[i].af == pj_AF_INET6()) {
                    pj_sockaddr_init(pj_AF_INET6(), &srv->addr[srv->addr_cnt],
                                     NULL, srv->port);
                    srv->addr[srv->addr_cnt].ipv6.sin6_addr = rec.addr[i].ip.v6;
                    added = PJ_TRUE;
                } else {
                ...
                if (added) {
                    PJ_LOG(5, (query_job->objname, "DNS %s for %.*s: %s",
                               (is_type_a ? "A" : "AAAA"),
                               (int)srv->target_name.slen, srv->target_name.ptr,
            ...
            /* Log error */
            PJ_PERROR(
              4, (query_job->objname, status, "DNS %s record resolution failed",
                  (is_type_a ? "A" : "AAAA")));
        }
...
on_error:
    ...
    if (status != PJ_SUCCESS) {
        PJ_PERROR(
          4, (query_job->objname, status,
              "DNS %s record resolution error for '%.*s'.",
              pj_dns_get_type_name(query_job->dns_state),
              (int)query_job->domain_part.slen, query_job->domain_part.ptr));
