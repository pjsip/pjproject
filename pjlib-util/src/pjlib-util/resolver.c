...
static const char* state_names[3] = { "Probing", "Active", "Bad" };
...
static void on_read_complete(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
...
static pj_status_t select_nameservers(pj_dns_resolver* resolver,
                                      unsigned* count, unsigned servers[]);
...
static pj_status_t init_sock(pj_dns_resolver* resv)
{
    ...
    status = pj_ioqueue_register_sock2(resv->pool, resv->ioqueue,
                                       resv->udp_sock, resv->grp_lock, resv,
                                       &socket_cb, &resv->udp_key);
    if (status != PJ_SUCCESS)
        return status;
    ...
    status = pj_ioqueue_recvfrom(
      resv->udp_key, &resv->udp_op_rx_key, resv->udp_rx_pkt, &rx_pkt_size,
      PJ_IOQUEUE_ALWAYS_ASYNC, &resv->udp_src_addr, &resv->udp_addr_len);
    ...
    /* Create the UDP socket */
    status =
      pj_sock_socket(pj_AF_INET6(), pj_SOCK_DGRAM(), 0, &resv->udp6_sock);
    if (status != PJ_SUCCESS) {
    ...
    socket_cb.on_read_complete = &on_read_complete;
    status = pj_ioqueue_register_sock2(resv->pool, resv->ioqueue,
                                       resv->udp6_sock, resv->grp_lock, resv,
                                       &socket_cb, &resv->udp6_key);
    if (status != PJ_SUCCESS)
        return status;

    pj_ioqueue_op_key_init(&resv->udp6_op_rx_key, sizeof(resv->udp6_op_rx_key));
    pj_ioqueue_op_key_init(&resv->udp6_op_tx_key, sizeof(resv->udp6_op_tx_key));

    /* Start asynchronous read to the UDP socket */
    rx_pkt_size = sizeof(resv->udp6_rx_pkt);
    resv->udp6_addr_len = sizeof(resv->udp6_src_addr);
    status = pj_ioqueue_recvfrom(
      resv->udp6_key, &resv->udp6_op_rx_key, resv->udp6_rx_pkt, &rx_pkt_size,
      PJ_IOQUEUE_ALWAYS_ASYNC, &resv->udp6_src_addr, &resv->udp6_addr_len);
    if (status != PJ_EPENDING)
        return status;
#else
                       ...
                       pj_dns_resolver** p_resolver)
{
    ...
    /* Create group lock */
    status = pj_grp_lock_create_w_handler(
      pool, NULL, resv, &dns_resolver_on_destroy, &resv->grp_lock);
    if (status != PJ_SUCCESS)
        goto on_error;
...
pj_dns_resolver_destroy(pj_dns_resolver* resolver, pj_bool_t notify)
{
    ...
    if (notify) {
        ...
        while (it) {
            pj_dns_async_query* q =
              (pj_dns_async_query*)pj_hash_this(resolver->hquerybyid, it);
    ...
    while (it) {
        ...
        pj_hash_set(NULL, resolver->hrescache, &cache->key, sizeof(cache->key),
                    0, NULL);
                                  ...
                                  pj_dns_async_query* q)
{
    ...
    status = pj_timer_heap_schedule_w_grp_lock(resolver->timer, &q->timer_entry,
                                               &delay, 1, resolver->grp_lock);
    if (status != PJ_SUCCESS) {
        return status;
    }
    ...
    if (pj_ioqueue_is_pending(resolver->udp_key, &resolver->udp_op_tx_key)
        ...
        ||
        (resolver->udp6_key &&
         pj_ioqueue_is_pending(resolver->udp6_key, &resolver->udp6_op_tx_key))
#endif
    )
    {
        ++q->transmit_cnt;
    ...
    status = pj_dns_make_query(resolver->udp_tx_pkt, &pkt_size, q->id,
                               q->key.qtype, &name);
    if (status != PJ_SUCCESS) {
    ...
    for (i = 0; i < server_cnt; ++i) {
        ...
        if (ns->addr.addr.sa_family == pj_AF_INET()) {
            status = pj_ioqueue_sendto(
              resolver->udp_key, &resolver->udp_op_tx_key, resolver->udp_tx_pkt,
              &sent, 0, &ns->addr, pj_sockaddr_get_len(&ns->addr));
        ...
        {
            status =
              pj_ioqueue_sendto(resolver->udp6_key, &resolver->udp6_op_tx_key,
                                resolver->udp_tx_pkt, &sent, 0, &ns->addr,
...
static struct cached_res* alloc_entry(pj_dns_resolver* resolver)
{
    ...
    pool = pj_pool_create(resolver->pool->factory, "dnscache", RES_BUF_SZ, 256,
                          NULL);
                            ...
                            void* user_data, pj_dns_async_query** p_query)
{
    ...
    if (cache) {
        ...
        if (PJ_TIME_VAL_GT(cache->expiry_time, now)) {
            ...
            PJ_LOG(5, (resolver->name.ptr,
                       ...
                       pj_dns_get_type_name(type), (int)name->slen, name->ptr,
    ...
    pj_hash_set_np(resolver->hquerybyid, &q->id, sizeof(q->id), 0, q->hbufid,
                   q);
                           ...
                           pj_dns_addr_record* rec)
{
    ...
    /* Keep following CNAME records. */
    while (pkt->ans[ansidx].type == PJ_DNS_TYPE_CNAME && cnt++ < MAX_SEARCH) {
        resname = &pkt->ans[ansidx].rdata.cname.name;
...
static void set_nameserver_state(pj_dns_resolver* resolver, unsigned index,
                                 enum ns_state state, const pj_time_val* now)
{
    ...
    PJ_LOG(5, (resolver->name.ptr, "Nameserver %s:%d state changed %s --> %s",
               pj_sockaddr_print(&ns->addr, addr, sizeof(addr), 2),
               pj_sockaddr_get_port(&ns->addr), state_names[old_state],
               state_names[state]));
}
                                     ...
                                     const pj_dns_parsed_packet* pkt)
{
    ...
    if (!pkt || /* rcode == PJ_DNS_RCODE_SERVFAIL || */
        rcode == PJ_DNS_RCODE_REFUSED || rcode == PJ_DNS_RCODE_NOTAUTH)
    {
        is_good = PJ_FALSE;
    } else {
        is_good = PJ_TRUE;
...
static void update_res_cache(pj_dns_resolver* resolver,
                             const struct res_key* key, pj_status_t status,
                             ...
                             const pj_dns_parsed_packet* pkt)
{
     ...
     */
    pj_dns_packet_dup(cache->pool, pkt, PJ_DNS_NO_NS | PJ_DNS_NO_AR,
                      &cache->pkt);
                       ...
                       struct pj_timer_entry* entry)
{
    ...
    if (q->transmit_cnt < resolver->settings.qretr_count) {
        ...
        if (status == PJ_SUCCESS) {
            ...
            /* Error occurs */
            PJ_PERROR(
              4, (resolver->name.ptr, status, "Error transmitting request"));
                             ...
                             pj_ssize_t bytes_read)
{
    ...
    PJ_LOG(5,
           (resolver->name.ptr, "Received %d bytes DNS response from %s:%d",
            (int)bytes_read, pj_sockaddr_print(src_addr, addr, sizeof(addr), 2),
    ...
    {
        status =
          pj_dns_parse_packet(pool, rx_pkt, (unsigned)bytes_read, &dns_pkt);
    }
    PJ_CATCH_ANY
    {
        status = PJ_ENOMEM;
    ...
    q = (pj_dns_async_query*)pj_hash_get(resolver->hquerybyid, &dns_pkt->hdr.id,
                                         sizeof(dns_pkt->hdr.id), NULL);
    if (!q) {
        PJ_LOG(5,
               (resolver->name.ptr, "DNS response from %s:%d id=%d discarded",
                pj_sockaddr_print(src_addr, addr, sizeof(addr), 2),
                pj_sockaddr_get_port(src_addr), (unsigned)dns_pkt->hdr.id));
        goto read_next_packet;
    }
...
read_next_packet:
    ...
    status =
      pj_ioqueue_recvfrom(key, op_key, rx_pkt, &rx_pkt_size,
                          PJ_IOQUEUE_ALWAYS_ASYNC, src_addr, src_addr_len);

    if (status != PJ_EPENDING && status != PJ_ECANCELLED) {
        PJ_PERROR(
          4, (resolver->name.ptr, status, "DNS resolver ioqueue read error"));
                          ...
                          const pj_dns_parsed_packet* pkt, pj_bool_t set_ttl)
{
    ...
    PJ_ASSERT_RETURN(
      (pkt->hdr.anscount && pkt->ans) || (pkt->hdr.qdcount && pkt->q),
 ...
 */
PJ_DEF(void) pj_dns_resolver_dump(pj_dns_resolver* resolver, pj_bool_t detail)
{
    ...
    for (i = 0; i < resolver->ns_count; ++i) {
        ...
        PJ_LOG(
          3, (resolver->name.ptr,
              "   NS %d: %s:%d (state=%s until %ds, rtt=%d ms)", i,
              pj_sockaddr_print(&ns->addr, addr, sizeof(addr), 2),
              pj_sockaddr_get_port(&ns->addr), state_names[ns->state],
              ns->state_expiry.sec - now.sec, PJ_TIME_VAL_MSEC(ns->rt_delay)));
    }
    ...
    if (detail) {
        ...
        while (it) {
            ...
            cache = (struct cached_res*)pj_hash_this(resolver->hrescache, it);
            PJ_LOG(3,
                   (resolver->name.ptr, "   Type %s: %s",
                    pj_dns_get_type_name(cache->key.qtype), cache->key.name));
    ...
    if (detail) {
        ...
        while (it) {
            ...
            PJ_LOG(3, (resolver->name.ptr, "   Type %s: %s",
                       pj_dns_get_type_name(q->key.qtype), q->key.name));
            it = pj_hash_next(resolver->hquerybyid, it);
        }
    }
