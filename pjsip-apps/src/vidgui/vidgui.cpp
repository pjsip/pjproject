...
void MainWin::initLayout()
{
    ...
    vbox_right->addWidget(
      (previewButton_ = new QPushButton(tr("Start &Preview"))));
    ...
    connect(vidEnabled_, SIGNAL(stateChanged(int)), this,
            SLOT(onVidEnabledChanged(int)));

    // UI updates must be done in the UI thread!
    connect(this, SIGNAL(signalNewCall(int, bool)), this,
            SLOT(onNewCall(int, bool)));
    connect(this, SIGNAL(signalCallReleased()), this, SLOT(onCallReleased()));
    connect(this, SIGNAL(signalInitVideoWindow()), this,
            SLOT(initVideoWindow()));
    connect(this, SIGNAL(signalShowStatus(const QString&)), this,
            SLOT(doShowStatus(const QString&)));
}
...
void MainWin::call()
{
    if (callButton_->text() == "Answer") {
        ...
        status = pjsua_call_make_call(accountId_, &uri2, &call_setting, NULL,
                                      NULL, &currentCall_);
        if (status != PJ_SUCCESS) {
...
void MainWin::on_call_state(pjsua_call_id call_id, pjsip_event* e)
{
    ...
    if (ci.state == PJSIP_INV_STATE_DISCONNECTED) {
        snprintf(status, sizeof(status), "Call is %s (%s)", ci.state_text.ptr,
                 ci.last_status_text.ptr);
        showStatus(status);
        emit signalCallReleased();
    } else {
        snprintf(status, sizeof(status), "Call is %s",
                 pjsip_inv_state_name(ci.state));
        showStatus(status);
    }
}
...
#    if SIP_TCP
    acc_cfg.proxy[acc_cfg.proxy_cnt++] =
      pj_str((char*)"<sip:" SIP_DOMAIN ";transport=tcp>");
#    endif

#else
    char sip_id[80];
    snprintf(sip_id, sizeof(sip_id), "sip:%s@%.*s:%u", SIP_USERNAME,
             (int)udp_info.local_name.host.slen, udp_info.local_name.host.ptr,
...
int main(int argc, char* argv[])
{
     ...
     * in SDL_CreateWindow(). Here's a stack trace if you're interested:
     ...
     Segmentation fault.) 13 XCreateIC() 12 SetupWindowData() 11
     X11_CreateWindow() 10 SDL_CreateWindow()
        ..
     */
