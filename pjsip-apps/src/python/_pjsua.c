...
static void cb_log_cb(int level, const char* data, int len)
{
    ...
    if (PyCallable_Check(g_obj_log_cb)) {
        ...
        PyObject_CallFunction(g_obj_log_cb, "iOi", level, param_data, len,
                              NULL);
...
static void cb_on_call_state(pjsua_call_id call_id, pjsip_event* e)
{
    ...
    if (PyCallable_Check(g_obj_callback->on_call_state)) {
        ...
        PyObject_CallFunction(g_obj_callback->on_call_state, "iO", call_id, obj,
                              NULL);
                                ...
                                pjsip_rx_data* rdata)
{
    if (PyCallable_Check(g_obj_callback->on_incoming_call)) {
        ...
        obj = (PyObj_pjsip_rx_data*)PyObj_pjsip_rx_data_new(
          &PyTyp_pjsip_rx_data, NULL, NULL);
        PyObj_pjsip_rx_data_import(obj, rdata);

        PyObject_CallFunction(g_obj_callback->on_incoming_call, "iiO", acc_id,
                              call_id, obj, NULL);
...
static void cb_on_dtmf_digit(pjsua_call_id call_id, int digit)
{
    if (PyCallable_Check(g_obj_callback->on_dtmf_digit)) {
        ...
        PyObject_CallFunction(g_obj_callback->on_dtmf_digit, "is", call_id,
                              digit_str, NULL);
                                        ...
                                        pjsip_status_code* code)
{
    if (PyCallable_Check(g_obj_callback->on_call_transfer_request)) {
        ...
        ret = PyObject_CallFunction(g_obj_callback->on_call_transfer_request,
                                    "iOi", call_id, param_dst, *code, NULL);
                                       ...
                                       pj_bool_t final, pj_bool_t* p_cont)
{
    if (PyCallable_Check(g_obj_callback->on_call_transfer_status)) {
        ...
        ret = PyObject_CallFunction(g_obj_callback->on_call_transfer_status,
                                    "iiOii", call_id, status_code, param_reason,
                                    final, *p_cont, NULL);
                                       ...
                                       pj_str_t* st_text)
{
    ...
    if (PyCallable_Check(g_obj_callback->on_call_replace_request)) {
        ...
        ret = PyObject_CallFunction(g_obj_callback->on_call_replace_request,
                                    "iOiO", call_id, param_rdata, *st_code,
                                    param_reason, NULL);
                                ...
                                pjsua_call_id new_call_id)
{
    if (PyCallable_Check(g_obj_callback->on_call_replaced)) {
        ...
        PyObject_CallFunction(g_obj_callback->on_call_replaced, "ii",
                              old_call_id, new_call_id, NULL);
                                     ...
                                     pjsua_msg_data* msg_data)
{
    ...
    if (PyCallable_Check(g_obj_callback->on_incoming_subscribe)) {
        ...
        ret = PyObject_CallFunction(g_obj_callback->on_incoming_subscribe,
                                    "iiOOO", acc_id, buddy_id, param_from,
                                    param_contact, param_srv_pres, NULL);
                        ...
                        pjsip_rx_data* rdata, pjsua_acc_id acc_id)
{
    ...
    if (PyCallable_Check(g_obj_callback->on_pager)) {
        ...
        PyObject_CallFunction(g_obj_callback->on_pager, "iOOOOOi", call_id,
                              param_from, param_to, param_contact,
                              param_mime_type, param_body, acc_id, NULL);
...
{
    if (PyCallable_Check(g_obj_callback->on_pager)) {
        PyObject *param_call_id, *param_to, *param_body, *param_user_data,
          *param_status, *param_reason, *param_acc_id;
                         ...
                         pjsua_acc_id acc_id)
{
    if (PyCallable_Check(g_obj_callback->on_typing)) {
        ...
        PyObject_CallFunctionObjArgs(
          ...
          param_call_id = Py_BuildValue("i", call_id),
          param_from = PyString_FromPJ(from), param_to = PyString_FromPJ(to),
          param_contact = PyString_FromPJ(contact),
...
static void cb_on_mwi_info(pjsua_acc_id acc_id, pjsua_mwi_info* mwi_info)
{
    if (PyCallable_Check(g_obj_callback->on_mwi_info)) {
        ...
        PyObject_CallFunctionObjArgs(g_obj_callback->on_mwi_info,
                                     param_acc_id = Py_BuildValue("i", acc_id),
                                     param_body = PyString_FromPJ(&body), NULL);
...
static PyObject* py_pjsua_thread_register(PyObject* pSelf, PyObject* pArgs)
{
    ...
    thread_desc = (struct py_thread_desc*)malloc(sizeof(struct py_thread_desc));
...
static PyObject* py_pjsua_msg_data_init(PyObject* pSelf, PyObject* pArgs)
{
    ...
    return (PyObject*)PyObj_pjsua_msg_data_new(&PyTyp_pjsua_msg_data, NULL,
                                               NULL);
}
                                                 ...
                                                 PyObject* pArgs)
{
    ...
    if (!PyArg_ParseTuple(pArgs, "iiiss", &acc_id, &is_online, &activity_id,
                          &activity_text, &rpid_id))
    {
        return NULL;
    }
 ...
 */
static PyObject* py_pjsua_acc_pres_notify(PyObject* pSelf, PyObject* pArgs)
{
    int acc_id, state;
    ...
    if (!PyArg_ParseTuple(pArgs, "iOiOO", &acc_id, &arg_pres, &state,
                          &arg_reason, &arg_msg_data))
    {
        return NULL;
    }
    ...
    status = pjsua_pres_notify(acc_id, (pjsua_srv_pres*)srv_pres,
                               (pjsip_evsub_state)state, NULL, &reason,
                               with_body, &msg_data);
...
static PyObject* py_pjsua_im_send(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (!PyArg_ParseTuple(pArgs, "iOOOOi", &acc_id, &pTo, &pMimeType, &pContent,
                          &pMsgData, &user_data))
    {
        return NULL;
    }
...
static PyObject* py_pjsua_im_typing(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (!PyArg_ParseTuple(pArgs, "iOiO", &acc_id, &pTo, &is_typing, &pMsgData))
    {
        return NULL;
    }
...
static PyObject* py_pjsua_conf_get_port_info(PyObject* pSelf, PyObject* pArgs)
{
    ...
    ret = (PyObj_pjsua_conf_port_info*)conf_port_info_new(
...
static PyObject* py_pjsua_playlist_create(PyObject* pSelf, PyObject* pArgs)
{
    ...
    count = 0;
    for (count = 0;
         count < PyList_Size(pFileList) && count < PJ_ARRAY_SIZE(files);
         ++count)
    {
...
static PyObject* py_pjsua_recorder_create(PyObject* pSelf, PyObject* pArgs)
{
    ...
    status =
      pjsua_recorder_create(&filename, enc_type, NULL, max_size, options, &id);
...
static PyObject* py_pjsua_codec_get_param(PyObject* pSelf, PyObject* pArgs)
{
    ...
    ret = (PyObj_pjmedia_codec_param*)pjmedia_codec_param_new(
...
static PyObject* py_pjsua_call_make_call(PyObject* pSelf, PyObject* pArgs)
{
    ...
    status = pjsua_call_make_call(acc_id, &dst_uri, &option, (void*)pUserData,
                                  &msg_data, &call_id);
    if (pool != NULL)
        pj_pool_release(pool);
...
static PyObject* py_pjsua_call_get_info(PyObject* pSelf, PyObject* pArgs)
{
    ...
    ret =
      (PyObj_pjsua_call_info*)call_info_new(&PyTyp_pjsua_call_info, NULL, NULL);
    ret->acc_id = info.acc_id;
    Py_XDECREF(ret->call_id);
    ret->call_id = PyString_FromPJ(&info.call_id);
    ret->conf_slot = info.conf_slot;
    ret->connect_duration =
      info.connect_duration.sec * 1000 + info.connect_duration.msec;
    ret->total_duration =
      info.total_duration.sec * 1000 + info.total_duration.msec;
    ret->id = info.id;
...
static PyObject* py_pjsua_call_dump(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (!PyArg_ParseTuple(pArgs, "iiIO", &call_id, &with_media, &maxlen,
                          &pIndent)) {
        return NULL;
    }
...
static PyMethodDef py_pjsua_methods[] = {
    ...
    { "conf_get_signal_level", py_pjsua_conf_get_signal_level, METH_VARARGS,
      "Get last signal level transmitted to or received from the specified "
      "port" },
    ...
    { "call_update", py_pjsua_call_update, METH_VARARGS, "Send UPDATE" },
...
#define ADD_CONSTANT(mod, name) PyModule_AddIntConstant(mod, #name, name)
    ...
    m =
      Py_InitModule3("_pjsua", py_pjsua_methods, "PJSUA-lib module for python");
