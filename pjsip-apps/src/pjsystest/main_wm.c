    ...
    for (i = 0; i < menu->submenu_cnt; ++i) {
        if (menu->submenus[i] == NULL) {
            ...
            pj_ansi_to_unicode(menu->submenus[i]->title,
                               pj_ansi_strlen(menu->submenus[i]->title), buf,
                               64);

            hMenu = CreatePopupMenu();
            AppendMenu(parent, MF_STRING | MF_ENABLED | MF_POPUP, (UINT)hMenu,
                       buf);
...
pj_status_t gui_init(gui_menu* menu)
{
    ...
    dwStyle = WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_READONLY |
              ES_AUTOVSCROLL | ES_LEFT;
    g_hWndLog = CreateWindow(TEXT("EDIT"),  // Class name
                             NULL,          // Window text
                             dwStyle,       // Window style
                             0,  // x-coordinate of the upper-left corner
                             0,  // y-coordinate of the upper-left corner
                             r.right - r.left,  // Width of the window for the
                                                // edit control
                             r.bottom - r.top,  // Height of the window for the
                                                // edit control
