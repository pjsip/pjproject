...
static gui_menu menu_tests = {
    ...
    NULL,
    10,
    { &menu_wizard, &menu_audtest, &menu_playtn, &menu_playwv1, &menu_playwv2,
      &menu_recaud, &menu_calclat, &menu_sndaec, NULL, &menu_exit }
};

static gui_menu menu_options = { "Options",
                                 NULL,
                                 2,
                                 {
                                   &menu_listdev,
...
static gui_menu root_menu = { "Root", NULL, 2, { &menu_tests, &menu_options } };
...
#define RINGBACK_INTERVAL 4000 /* 2000 */
    ...
    status = pjmedia_tonegen_create2(pool, &name, systest.media_cfg.clock_rate,
                                     systest.media_cfg.channel_count,
                                     samples_per_frame, 16,
                                     PJMEDIA_TONEGEN_LOOP, &ringback_port);
    if (status != PJ_SUCCESS)
        goto on_return;
...
on_return:
    ...
    if (status != PJ_SUCCESS) {
        systest_perror(
          "Sorry we encounter error when initializing "
          "the tone generator: ",
          status);
        ti->success = PJ_FALSE;
...
static void systest_play_wav(unsigned path_cnt, const char* paths[])
{
    ...
    key = gui_msgbox(title, textbuf, WITH_OKCANCEL);
    if (key != KEY_OK) {
        ti->skipped = PJ_TRUE;
        return;
    }
...
static void systest_audio_test(void)
{
    ...
    if (status != PJ_SUCCESS) {
        systest_perror("Sorry we had error in pjmedia_aud_dev_default_param()",
                       status);
    ...
    if (result.rec.frame_cnt == 0) {
        ...
        pj_ansi_snprintf(textbuf + textbufpos, sizeof(textbuf) - textbufpos,
                         "Rec : interval (min/max/avg/dev)=\r\n"
                         "         %u/%u/%u/%u (ms)\r\n"
                         "      max burst=%u\r\n",
                         result.rec.min_interval, result.rec.max_interval,
                         result.rec.avg_interval, result.rec.dev_interval,
                         result.rec.max_burst);
        textbufpos = strlen(textbuf);

        if (result.rec.max_burst > GOOD_MAX_INTERVAL) {
            problems[problem_count++] = "Recording max burst is quite high";
        }
    }
    ...
    if (result.play.frame_cnt == 0) {
        ...
        pj_ansi_snprintf(textbuf + textbufpos, sizeof(textbuf) - textbufpos,
                         ...
                         "         %u/%u/%u/%u (ms)\r\n"
                         "      burst=%u\r\n",
                         result.play.min_interval, result.play.max_interval,
                         result.play.avg_interval, result.play.dev_interval,
                         result.play.max_burst);
        textbufpos = strlen(textbuf);

        if (result.play.max_burst > GOOD_MAX_INTERVAL) {
            problems[problem_count++] = "Playback max burst is quite high";
        }
    }
...
static void systest_latency_test(void)
{
    const char* ref_wav_paths[] = { add_path(res_path, WAV_TOCK8_PATH),
                                    ALT_PATH1 WAV_TOCK8_PATH };
    ...
    status =
      create_player(PJ_ARRAY_SIZE(ref_wav_paths), ref_wav_paths, &play_id);
    if (status != PJ_SUCCESS)
        goto on_return;
    ...
    status = pjmedia_wav_player_port_create(pool, rec_wav_file.ptr, 0, 0, 0,
                                            &wav_port);
    if (status != PJ_SUCCESS)
        goto on_return;

    status =
      calculate_latency(pool, wav_port, &lat_sum, &lat_cnt, &lat_min, &lat_max);
    if (status != PJ_SUCCESS)
        goto on_return;
...
static void systest_aec_test(void)
{
     ...
     */
    status =
      create_player(PJ_ARRAY_SIZE(ref_wav_paths), ref_wav_paths, &player_id);
    if (status != PJ_SUCCESS) {
    ...
    status = pjsua_recorder_create(
      pj_cstr(&tmp, add_path(doc_path, AEC_REC_PATH)), 0, 0, -1, 0, &writer_id);
    if (status != PJ_SUCCESS) {
        PJ_PERROR(
          1, (THIS_FILE, status, "Error writing WAV file %s", AEC_REC_PATH));
        goto on_return;
    }
    ...
    status = pjsua_player_create(
      pj_cstr(&tmp, add_path(doc_path, AEC_REC_PATH)), 0, &player_id);
    if (status != PJ_SUCCESS) {
        PJ_PERROR(
          1, (THIS_FILE, status, "Error opening WAV file %s", AEC_REC_PATH));
        goto on_return;
    }
    pjsua_conf_connect(pjsua_player_get_conf_port(player_id), 0);
    ...
    /* Wait user signal */
    gui_msgbox(title,
               "We are now playing the captured audio from the mic. "
               "Check if echo (of the audio played back previously) is "
               "present in the audio. The recording is stored in " AEC_REC_PATH
    ...
    key = gui_msgbox(title, "Did you notice any echo in the recording?",
                     WITH_YESNO);
...
static void systest_list_audio_devs()
{
    ...
    if (dev_count == 0) {
        key = gui_msgbox(title, "No audio devices are found", WITH_OK);
        ti->success = PJ_FALSE;
...
static void systest_display_settings(void)
{
    ...
    pj_ansi_snprintf(
      textbuf + len, sizeof(textbuf) - len, "Audio switching: %s\r\n",
...
void systest_save_result(const char* filename)
{
    ...
    if (status != PJ_SUCCESS) {
        pj_ansi_snprintf(textbuf, sizeof(textbuf), "Error opening file %s",
                         filename);
    ...
    if (pj_time_decode(&tv, &pt) == PJ_SUCCESS) {
        pj_ansi_snprintf(textbuf, sizeof(textbuf),
                         ...
                         pt.mon + 1, pt.day, pt.hour, pt.min, pt.sec);
        size = strlen(textbuf);
    ...
    pj_ansi_snprintf(textbuf, sizeof(textbuf),
                     "Test result successfully appended to file %s", filename);
