...
static void cb_log_cb(int level, const char* data, int len)
{
    ...
    if (PyCallable_Check(obj_log_cb)) {
        ...
        PyObject_CallFunctionObjArgs(obj_log_cb, Py_BuildValue("i", level),
                                     PyString_FromString(data),
                                     Py_BuildValue("i", len), NULL);
...
static void cb_on_call_state(pjsua_call_id call_id, pjsip_event* e)
{
    if (PyCallable_Check(g_obj_callback->on_call_state)) {
        ...
        obj =
          (PyObj_pjsip_event*)PyType_GenericNew(&PyTyp_pjsip_event, NULL, NULL);

        obj->event = e;

        PyObject_CallFunctionObjArgs(g_obj_callback->on_call_state,
                                     Py_BuildValue("i", call_id), obj, NULL);
                                ...
                                pjsip_rx_data* rdata)
{
    if (PyCallable_Check(g_obj_callback->on_incoming_call)) {
        ...
        PyObject_CallFunctionObjArgs(g_obj_callback->on_incoming_call,
...
static void cb_on_dtmf_digit(pjsua_call_id call_id, int digit)
{
    if (PyCallable_Check(g_obj_callback->on_dtmf_digit)) {
        ...
        PyObject_CallFunctionObjArgs(g_obj_callback->on_dtmf_digit,
                                     Py_BuildValue("i", call_id),
                                     PyString_FromString(digit_str), NULL);
                                        ...
                                        pjsip_status_code* code)
{
    if (PyCallable_Check(g_obj_callback->on_call_transfer_request)) {
        ...
        ret = PyObject_CallFunctionObjArgs(
          g_obj_callback->on_call_transfer_request, Py_BuildValue("i", call_id),
          PyString_FromStringAndSize(dst->ptr, dst->slen),
          Py_BuildValue("i", *code), NULL);
        if (ret != NULL) {
            if (ret != Py_None) {
 ...
 * !modified @061206
 */
static void cb_on_call_transfer_status(pjsua_call_id call_id, int status_code,
                                       ...
                                       pj_bool_t final, pj_bool_t* p_cont)
{
    if (PyCallable_Check(g_obj_callback->on_call_transfer_status)) {
        ...
        ret = PyObject_CallFunctionObjArgs(
          g_obj_callback->on_call_transfer_status, Py_BuildValue("i", call_id),
          Py_BuildValue("i", status_code),
          PyString_FromStringAndSize(status_text->ptr, status_text->slen),
          Py_BuildValue("i", final), Py_BuildValue("i", *p_cont), NULL);
        if (ret != NULL) {
            if (ret != Py_None) {
                                       ...
                                       pj_str_t* st_text)
{
    if (PyCallable_Check(g_obj_callback->on_call_replace_request)) {
        ...
        ret = PyObject_CallFunctionObjArgs(
          g_obj_callback->on_call_replace_request, Py_BuildValue("i", call_id),
          obj, Py_BuildValue("i", *st_code),
          PyString_FromStringAndSize(st_text->ptr, st_text->slen), NULL);
        if (ret != NULL) {
            if (ret != Py_None) {
                                ...
                                pjsua_call_id new_call_id)
{
    if (PyCallable_Check(g_obj_callback->on_call_replaced)) {
        ...
        PyObject_CallFunctionObjArgs(g_obj_callback->on_call_replaced,
                                     Py_BuildValue("i", old_call_id),
                                     Py_BuildValue("i", new_call_id), NULL);
...
static void cb_on_reg_state(pjsua_acc_id acc_id)
{
    if (PyCallable_Check(g_obj_callback->on_reg_state)) {
        ...
        PyObject_CallFunction(g_obj_callback->on_reg_state, "i", acc_id, NULL);
                               ...
                               pjsip_status_code status, const pj_str_t* reason)
{
    if (PyCallable_Check(g_obj_callback->on_pager)) {
        ...
        PyObject_CallFunctionObjArgs(
          g_obj_callback->on_pager_status, Py_BuildValue("i", call_id),
          PyString_FromStringAndSize(to->ptr, to->slen),
          PyString_FromStringAndSize(body->ptr, body->slen), obj_user_data,
          Py_BuildValue("i", status),
          PyString_FromStringAndSize(reason->ptr, reason->slen), NULL);
    ...
    if (PyList_Check(py_hdr_list)) {
        ...
        for (i = 0; i < PyList_Size(py_hdr_list); i++) {
            ...
            if (PyTuple_Check(tuple)) {
                ...
                hname.slen =
                  strlen(PyString_AsString(PyTuple_GetItem(tuple, 0)));
                hvalue.ptr = PyString_AsString(PyTuple_GetItem(tuple, 1));
                hvalue.slen =
                  strlen(PyString_AsString(PyTuple_GetItem(tuple, 1)));
            } else {
                hname.ptr = "";
                hname.slen = 0;
                hvalue.ptr = "";
...
void translate_hdr_rev(pjsip_generic_string_hdr* hdr, PyObject* py_hdr_list)
{
    ...
    if (len > 0) {
        ...
        for (i = 0; i < len && p_hdr != NULL; i++) {
            ...
            str =
              PyString_FromStringAndSize(hdr->hvalue.ptr, p_hdr->hvalue.slen);
...
static PyObject* py_pjsua_config_default(PyObject* pSelf, PyObject* pArgs)
{
    ...
    obj = (PyObj_pjsua_config*)PyObj_pjsua_config_new(&PyTyp_pjsua_config, NULL,
                                                      NULL);
 ...
 * !modified @ 051206
 */
static PyObject* py_pjsua_media_config_default(PyObject* pSelf, PyObject* pArgs)
{
    ...
    obj = (PyObj_pjsua_media_config*)PyType_GenericNew(
      &PyTyp_pjsua_media_config, NULL, NULL);
...
static PyObject* py_pjsua_msg_data_init(PyObject* pSelf, PyObject* pArgs)
{
    ...
    obj = (PyObj_pjsua_msg_data*)PyObj_pjsua_msg_data_new(&PyTyp_pjsua_msg_data,
                                                          NULL, NULL);
    Py_XDECREF(obj->content_type);
    obj->content_type =
      PyString_FromStringAndSize(msg.content_type.ptr, msg.content_type.slen);
    Py_XDECREF(obj->msg_body);
    obj->msg_body =
      PyString_FromStringAndSize(msg.msg_body.ptr, msg.msg_body.slen);
...
static PyObject* py_pjsua_get_pjsip_endpt(PyObject* pSelf, PyObject* pArgs)
{
    ...
    endpt = (PyObj_pjsip_endpoint*)PyType_GenericNew(&PyTyp_pjsip_endpoint,
                                                     NULL, NULL);
    endpt->endpt = e;
...
static PyObject* py_pjsua_get_pjmedia_endpt(PyObject* pSelf, PyObject* pArgs)
{
    ...
    endpt =
      (PyObj_pjmedia_endpt*)PyType_GenericNew(&PyTyp_pjmedia_endpt, NULL, NULL);
    endpt->endpt = e;
    return (PyObject*)endpt;
}
...
static PyObject* py_pjsua_get_pool_factory(PyObject* pSelf, PyObject* pArgs)
{
    ...
    pool = (PyObj_pj_pool_factory*)PyType_GenericNew(&PyTyp_pj_pool_factory,
                                                     NULL, NULL);
    pool->pool_fact = p;
    return (PyObject*)pool;
}
...
static PyObject* py_pjsua_handle_events(PyObject* pSelf, PyObject* pArgs)
{
     ...
     */
    Py_BEGIN_ALLOW_THREADS ret = pjsua_handle_events(msec);
                                                   ...
                                                   PyObject* pArgs)
{
    ...
    obj = (PyObj_pjsua_transport_config*)PyObj_pjsua_transport_config_new(
...
static PyObject* py_pjsua_transport_get_info(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (status == PJ_SUCCESS) {
        ...
        obj = (PyObj_pjsua_transport_info*)PyObj_pjsua_transport_info_new(
          &PyTyp_pjsua_transport_info, NULL, NULL);
 ...
 */
static PyObject* py_pjsua_transport_set_enable(PyObject* pSelf, PyObject* pArgs)
{
    pj_status_t status;
    int id;
    int enabled;
...
static PyObject* py_pjsua_acc_config_default(PyObject* pSelf, PyObject* pArgs)
{
    ...
    obj = (PyObj_pjsua_acc_config*)PyObj_pjsua_acc_config_new(
      &PyTyp_pjsua_acc_config, NULL, NULL);
                                                 ...
                                                 PyObject* pArgs)
{
    ...
    if (!PyArg_ParseTuple(pArgs, "iiis", &acc_id, &is_online, &activity_id,
                          &activity_text))
    {
        return NULL;
    }
 ...
 */
static PyObject* py_pjsua_acc_set_registration(PyObject* pSelf, PyObject* pArgs)
{
    int renew;
    int acc_id;
    int status;
...
static PyObject* py_pjsua_acc_get_info(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (status == PJ_SUCCESS) {
        obj = (PyObj_pjsua_acc_info*)PyObj_pjsua_acc_info_new(
          &PyTyp_pjsua_acc_info, NULL, NULL);
 ...
 */
static PyObject* py_pjsua_buddy_config_default(PyObject* pSelf, PyObject* pArgs)
{
    ...
    obj = (PyObj_pjsua_buddy_config*)PyObj_pjsua_buddy_config_new(
      &PyTyp_pjsua_buddy_config, NULL, NULL);
...
static PyObject* py_pjsua_buddy_get_info(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (status == PJ_SUCCESS) {
        obj = (PyObj_pjsua_buddy_info*)PyObj_pjsua_buddy_config_new(
          &PyTyp_pjsua_buddy_info, NULL, NULL);
...
static PyObject* py_pjsua_im_send(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (!PyArg_ParseTuple(pArgs, "iOOOOi", &acc_id, &st, &smt, &sc, &omdObj,
                          &user_data))
    {
        return NULL;
    }
    ...
    if (omdObj != Py_None) {
        ...
        status = pjsua_im_send(acc_id, &to, mime_type, &content, &msg_data,
                               (void*)user_data);
        pj_pool_release(pool);
    } else {
        status = pjsua_im_send(acc_id, &to, mime_type, &content, NULL, NULL);
    }
...
static PyMemberDef pjmedia_codec_param_setting_members[] = {
    ...
    { "reserved", T_INT, offsetof(PyObj_pjmedia_codec_param_setting, reserved),
 ...
 */
static PyObject* py_pjsua_conf_add_port(PyObject* pSelf, PyObject* pArgs)
{
    int p_id;
    PyObject* oportObj;
 ...
 */
static PyObject* py_pjsua_player_get_conf_port(PyObject* pSelf, PyObject* pArgs)
{
    int id, port_id;
...
static PyObject* py_pjsua_recorder_create(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (!PyArg_ParseTuple(pArgs, "OiOii", &filename, &enc_type, &enc_param,
                          &max_size, &options))
    {
        return NULL;
    }
    ...
    if (enc_param != Py_None) {
        ...
        status =
          pjsua_recorder_create(&str, enc_type, NULL, max_size, options, &p_id);
    } else {
        status =
          pjsua_recorder_create(&str, enc_type, NULL, max_size, options, &p_id);
    }
 ...
 */
static PyObject* py_pjsua_set_no_snd_dev(PyObject* pSelf, PyObject* pArgs)
{
    ...
    obj =
      (PyObj_pjmedia_port*)PyType_GenericNew(&PyTyp_pjmedia_port, NULL, NULL);
...
static PyObject* py_pjsua_enum_codecs(PyObject* pSelf, PyObject* pArgs)
{
    ...
    for (i = 0; i < c; i++) {
        ...
        obj = (PyObj_pjsua_codec_info*)codec_info_new(&PyTyp_pjsua_codec_info,
                                                      NULL, NULL);
        obj->codec_id = PyString_FromStringAndSize(info[i].codec_id.ptr,
                                                   info[i].codec_id.slen);
...
static PyMemberDef pj_time_val_members[] = {
    ...
    { "sec", T_INT, offsetof(PyObj_pj_time_val, sec), 0,
      "The seconds part of the time" },
    { "msec", T_INT, offsetof(PyObj_pj_time_val, sec), 0,
                               ...
                               PyObject* kwds)
{
    ...
    if (self != NULL) {
        ...
        self->connect_duration =
          (PyObj_pj_time_val*)PyType_GenericNew(&PyTyp_pj_time_val, NULL, NULL);
...
static PyObject* py_pjsua_call_make_call(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (!PyArg_ParseTuple(pArgs, "iOIiO", &acc_id, &sd, &options, &user_data,
                          &omdObj))
    {
        return NULL;
    }
    ...
    if (omdObj != Py_None) {
        ...
        msg_data.content_type.slen =
          strlen(PyString_AsString(omd->content_type));
        msg_data.msg_body.ptr = PyString_AsString(omd->msg_body);
        msg_data.msg_body.slen = strlen(PyString_AsString(omd->msg_body));
        pool = pjsua_pool_create("pjsua", POOL_SIZE, POOL_SIZE);
        translate_hdr(pool, &msg_data.hdr_list, omd->hdr_list);
        status = pjsua_call_make_call(acc_id, &dst_uri, options,
                                      (void*)user_data, &msg_data, &call_id);
        pj_pool_release(pool);
    } else {
        status = pjsua_call_make_call(acc_id, &dst_uri, options,
                                      (void*)user_data, NULL, &call_id);
    }
...
static PyObject* py_pjsua_call_get_info(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (status == PJ_SUCCESS) {
        oi = (PyObj_pjsua_call_info*)call_info_new(&PyTyp_pjsua_call_info, NULL,
                                                   NULL);
        oi->acc_id = info.acc_id;
        pj_ansi_snprintf(oi->buf_.call_id, sizeof(oi->buf_.call_id), "%.*s",
                         (int)info.call_id.slen, info.call_id.ptr);
        pj_ansi_snprintf(
          oi->buf_.last_status_text, sizeof(oi->buf_.last_status_text), "%.*s",
          (int)info.last_status_text.slen, info.last_status_text.ptr);
        pj_ansi_snprintf(oi->buf_.local_contact, sizeof(oi->buf_.local_contact),
                         "%.*s", (int)info.local_contact.slen,
                         info.local_contact.ptr);
        pj_ansi_snprintf(oi->buf_.local_info, sizeof(oi->buf_.local_info),
                         "%.*s", (int)info.local_info.slen,
                         info.local_info.ptr);
        pj_ansi_snprintf(
          oi->buf_.remote_contact, sizeof(oi->buf_.remote_contact), "%.*s",
          (int)info.remote_contact.slen, info.remote_contact.ptr);
        pj_ansi_snprintf(oi->buf_.remote_info, sizeof(oi->buf_.remote_info),
                         "%.*s", (int)info.remote_info.slen,
                         info.remote_info.ptr);

        oi->call_id =
          PyString_FromStringAndSize(info.call_id.ptr, info.call_id.slen);
        ...
        oi->local_contact = PyString_FromStringAndSize(info.local_contact.ptr,
                                                       info.local_contact.slen);
        oi->local_info =
          PyString_FromStringAndSize(info.local_info.ptr, info.local_info.slen);
        oi->remote_contact = PyString_FromStringAndSize(
          info.remote_contact.ptr, info.remote_contact.slen);
        oi->remote_info = PyString_FromStringAndSize(info.remote_info.ptr,
                                                     info.remote_info.slen);
        oi->media_dir = info.media_dir;
        oi->media_status = info.media_status;
        oi->role = info.role;
        oi->state = info.state;
        oi->state_text =
          PyString_FromStringAndSize(info.state_text.ptr, info.state_text.slen);
...
static PyObject* py_pjsua_call_xfer_replaces(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (!PyArg_ParseTuple(pArgs, "iiIO", &call_id, &dest_call_id, &options,
                          &omdObj))
    {
        return NULL;
    }
    ...
    if (omdObj != Py_None) {
        ...
        translate_hdr(pool, &msg_data.hdr_list, omd->hdr_list);
        status =
          pjsua_call_xfer_replaces(call_id, dest_call_id, options, &msg_data);
        pj_pool_release(pool);
    } else {
...
static PyObject* py_pjsua_call_send_im(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (!PyArg_ParseTuple(pArgs, "iOOOi", &call_id, &sm, &sc, &omdObj,
                          &user_data)) {
        return NULL;
    }
    if (sm == Py_None) {
    ...
    if (omdObj != Py_None) {
        ...
        pool = pjsua_pool_create("pjsua", POOL_SIZE, POOL_SIZE);
        translate_hdr(pool, &msg_data.hdr_list, omd->hdr_list);
        status = pjsua_call_send_im(call_id, mime_type, &content, &msg_data,
                                    (void*)user_data);
        pj_pool_release(pool);
    } else {
        status = pjsua_call_send_im(call_id, mime_type, &content, NULL,
                                    (void*)user_data);
    }
 ...
 */
static PyObject* py_pjsua_call_send_typing_ind(PyObject* pSelf, PyObject* pArgs)
{
    int status;
    int call_id;
    int is_typing;
...
static PyMethodDef py_pjsua_methods[] = {
    ...
    { "logging_config_default", py_pjsua_logging_config_default, METH_VARARGS,
      pjsua_logging_config_default_doc },
    ...
    { "acc_add", py_pjsua_acc_add, METH_VARARGS, pjsua_acc_add_doc },
    { "acc_add_local", py_pjsua_acc_add_local, METH_VARARGS,
      pjsua_acc_add_local_doc },
    { "acc_del", py_pjsua_acc_del, METH_VARARGS, pjsua_acc_del_doc },
    { "acc_modify", py_pjsua_acc_modify, METH_VARARGS, pjsua_acc_modify_doc },
    ...
    { "enum_accs", py_pjsua_enum_accs, METH_VARARGS, pjsua_enum_accs_doc },
    ...
    { "buddy_add", py_pjsua_buddy_add, METH_VARARGS, pjsua_buddy_add_doc },
    { "buddy_del", py_pjsua_buddy_del, METH_VARARGS, pjsua_buddy_del_doc },
    { "buddy_subscribe_pres", py_pjsua_buddy_subscribe_pres, METH_VARARGS,
      pjsua_buddy_subscribe_pres_doc },
    { "pres_dump", py_pjsua_pres_dump, METH_VARARGS, pjsua_pres_dump_doc },
    { "im_send", py_pjsua_im_send, METH_VARARGS, pjsua_im_send_doc },
    { "im_typing", py_pjsua_im_typing, METH_VARARGS, pjsua_im_typing_doc },
    ...
    { "set_ec", py_pjsua_set_ec, METH_VARARGS, pjsua_set_ec_doc },
    ...
    { "enum_calls", py_pjsua_enum_calls, METH_VARARGS, pjsua_enum_calls_doc },
    ...
    { "call_xfer", py_pjsua_call_xfer, METH_VARARGS, pjsua_call_xfer_doc },
    ...
    { "call_dump", py_pjsua_call_dump, METH_VARARGS, pjsua_call_dump_doc },
...
#define ADD_CONSTANT(mod, name) PyModule_AddIntConstant(mod, #name, name)
    ...
    m = Py_InitModule3("py_pjsua", py_pjsua_methods,
    ...
    PyModule_AddObject(m, "Logging_Config",
                       (PyObject*)&PyTyp_pjsua_logging_config);
    ...
    PyModule_AddObject(m, "Pj_Pool_Factory", (PyObject*)&PyTyp_pj_pool_factory);

    Py_INCREF(&PyTyp_pjsip_cred_info);
    PyModule_AddObject(m, "Pjsip_Cred_Info", (PyObject*)&PyTyp_pjsip_cred_info);

    /* LIB TRANSPORT */

    Py_INCREF(&PyTyp_pjsua_transport_config);
    PyModule_AddObject(m, "Transport_Config",
                       (PyObject*)&PyTyp_pjsua_transport_config);

    Py_INCREF(&PyTyp_pjsua_transport_info);
    PyModule_AddObject(m, "Transport_Info",
                       (PyObject*)&PyTyp_pjsua_transport_info);
    ...
    PyModule_AddObject(m, "Conf_Port_Info",
                       (PyObject*)&PyTyp_pjsua_conf_port_info);
