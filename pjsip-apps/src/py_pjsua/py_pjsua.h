                                         ...
                                         const pjsip_cred_info* cfg)
{
    ...
    obj->username =
      PyString_FromStringAndSize(cfg->username.ptr, cfg->username.slen);
                                      ...
                                      const pjsua_config* cfg)
{
    ...
    obj->outbound_proxy = PyString_FromStringAndSize(
      cfg->outbound_proxy[0].ptr, cfg->outbound_proxy[0].slen);
    Py_XDECREF(obj->stun_domain);
    obj->stun_domain =
      PyString_FromStringAndSize(cfg->stun_domain.ptr, cfg->stun_domain.slen);
    Py_XDECREF(obj->stun_host);
    obj->stun_host =
      PyString_FromStringAndSize(cfg->stun_host.ptr, cfg->stun_host.slen);
    Py_XDECREF(obj->stun_relay_host);
    obj->stun_relay_host =
      PyString_FromStringAndSize(cfg->stun_host.ptr, cfg->stun_host.slen);
    Py_XDECREF(obj->user_agent);
    obj->user_agent =
      PyString_FromStringAndSize(cfg->user_agent.ptr, cfg->user_agent.slen);
}
                                              ...
                                              const pjsua_transport_info* info)
{
    ...
    obj->type = info->type;
    obj->type_name =
      PyString_FromStringAndSize(info->type_name.ptr, info->type_name.slen);
...
static PyMemberDef PyObj_pjsua_transport_info_members[] = {
    ...
    { "type", T_INT, offsetof(PyObj_pjsua_transport_info, id), 0,
      "Transport type." },
                                          ...
                                          PyObj_pjsua_acc_config* obj)
{
    ...
    for (i = 0; i < cfg->proxy_cnt; i++) {
        ...
        cfg->proxy[i] =
          PyString_to_pj_str(PyList_GetItem((PyObject*)obj->proxy, i));
    }
                                        ...
                                        const pjsua_acc_info* info)
{
    ...
    obj->acc_uri =
      PyString_FromStringAndSize(info->acc_uri.ptr, info->acc_uri.slen);
    obj->has_registration = info->has_registration;
    obj->expires = info->expires;
    obj->status = info->status;
    obj->status_text =
      PyString_FromStringAndSize(info->status_text.ptr, info->status_text.slen);
    obj->online_status = info->online_status;
    obj->online_status_text = PyString_FromStringAndSize(
      info->online_status_text.ptr, info->online_status_text.slen);
}
                                          ...
                                          const pjsua_buddy_info* info)
{
    ...
    obj->contact =
      PyString_FromStringAndSize(info->contact.ptr, info->contact.slen);
    obj->status = info->status;
