...
#    define UNIMPLEMENTED(func) \
        PJ_LOG( \
          2, (THIS_FILE, "*** Call to unimplemented function %s ***", #func));
...
static const struct alt_codec_desc* find_codec_desc_by_info(
...
static pj_status_t alt_vid_codec_default_attr(
  pjmedia_vid_codec_factory* factory, const pjmedia_vid_codec_info* info,
  pjmedia_vid_codec_param* attr)
{
    ...
    /* Encoded format */
    pjmedia_format_init_video(&attr->enc_fmt, desc->info.fmt_id, 720, 480,
                              30000, 1001);
...
static pj_status_t alt_vid_codec_dealloc_codec(
  pjmedia_vid_codec_factory* factory, pjmedia_vid_codec* codec)
{
    /* This will never get called since we won't be using this codec */
...
static pjmedia_vid_codec_factory_op alt_vid_codec_factory_op = {
    &alt_vid_codec_test_alloc, &alt_vid_codec_default_attr,
    &alt_vid_codec_enum_codecs, &alt_vid_codec_alloc_codec,
...
pj_status_t pjsua_vid_subsys_init(void)
{
    ...
    if (status != PJ_SUCCESS) {
        PJ_PERROR(
          1, (THIS_FILE, status, "Error creating PJMEDIA video codec manager"));
        return status;
    }
    ...
    status =
      pjmedia_vid_codec_mgr_register_factory(mgr, &alt_vid_codec_factory.base);
    if (status != PJ_SUCCESS)
        return status;
                                     ...
                                     const pjmedia_sdp_session* remote_sdp)
{
    ...
    if (si->dir != PJMEDIA_DIR_NONE) {
        ...
        status = pjmedia_transport_attach(
          call_med->tp, call_med, &si->rem_addr, &si->rem_rtcp,
          pj_sockaddr_get_len(&si->rem_addr), &vid_rtp_cb, &vid_rtcp_cb);
        /*
         * TODO:
