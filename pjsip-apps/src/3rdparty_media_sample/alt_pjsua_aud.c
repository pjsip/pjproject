...
#define UNIMPLEMENTED(func) \
    PJ_LOG(2, (THIS_FILE, "*** Call to unimplemented function %s ***", #func));
...
static pjmedia_codec_factory_op alt_codec_factory_op = {
    &alt_codec_test_alloc,  &alt_codec_default_attr,  &alt_codec_enum_codecs,
    &alt_codec_alloc_codec, &alt_codec_dealloc_codec, &alt_codec_deinit
};
                                     ...
                                     const pjmedia_sdp_session* remote_sdp)
{
    ...
    if (si->dir != PJMEDIA_DIR_NONE) {
        ...
        status = pjmedia_transport_attach(
          ...
          pj_sockaddr_get_len(&si->rem_addr), &aud_rtp_cb, &aud_rtcp_cb);
