...
} codec_fmts[] = { { PJMEDIA_FORMAT_MJPEG, "mjpeg", PJ_TRUE,
                     PJMEDIA_FORMAT_I420 },
...
static pj_status_t avi_event_cb(pjmedia_event* event, void* user_data)
{
...
static pj_status_t codec_get_frame(pjmedia_port* port, pjmedia_frame* frame)
{
...
static int aviplay(pj_pool_t* pool, const char* fname)
{
    ...
    if (vid_port) {
        ...
        status = pjmedia_vid_dev_default_param(
          pool, PJMEDIA_VID_DEFAULT_RENDER_DEV, &param.vidparam);
        if (status != PJ_SUCCESS) {
            rc = 220;
            goto on_return;
        }
        ...
        param.vidparam.window_flags =
          PJMEDIA_VID_DEV_WND_BORDER | PJMEDIA_VID_DEV_WND_RESIZABLE;
        vfd =
          pjmedia_format_get_video_format_detail(&vid_port->info.fmt, PJ_TRUE);
        pjmedia_format_init_video(&param.vidparam.fmt, vid_port->info.fmt.id,
                                  vfd->size.w, vfd->size.h, vfd->fps.num,
                                  vfd->fps.denum);
        ...
        if (!vfi || vfi->bpp == 0) {
            ...
            status = pjmedia_vid_codec_mgr_find_codecs_by_id(
              NULL, &codec_id_st, &info_cnt, &codec_info, NULL);
            if (status != PJ_SUCCESS) {
                rc = 245;
                goto on_return;
            }
            ...
            /* Open codec */
            status =
              pjmedia_vid_codec_mgr_alloc_codec(NULL, codec_info, &codec);
            if (status != PJ_SUCCESS) {
                rc = 250;
                goto on_return;
            }
            ...
            status = pjmedia_vid_port_connect(renderer, &codec_port, PJ_FALSE);
        } else {
            status = pjmedia_vid_port_create(pool, &param, &renderer);
            if (status != PJ_SUCCESS) {
                rc = 230;
            ...
            status = pjmedia_vid_port_connect(renderer, vid_port, PJ_FALSE);
        }
    ...
    aud_stream = pjmedia_avi_streams_get_stream_by_media(avi_streams, 0,
                                                         PJMEDIA_TYPE_AUDIO);
    ...
    if (vid_port) {
        ...
        /* subscribe events */
        pjmedia_event_subscribe(NULL, &avi_event_cb, &avi_port, renderer);

        if (snd_port) {
            /* Synchronize video rendering and audio playback */
            pjmedia_vid_port_set_clock_src(
              renderer,
              pjmedia_snd_port_get_clock_src(snd_port, PJMEDIA_DIR_PLAYBACK));
        }
...
on_return:
    ...
    if (renderer) {
        pjmedia_event_unsubscribe(NULL, &avi_event_cb, &avi_port, renderer);
