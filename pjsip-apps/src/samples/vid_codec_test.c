...
static void diff_file()
{
    ...
    for (frame_cnt = 0;; ++frame_cnt) {
        unsigned nal_len[2];
        for (i = 0; i < size[0]; i++) {
            if (memcmp(buf[0] + pos[0] + i, start_nal, sizeof(start_nal)) ==
                  0 &&
                i > 0) {
                break;
            }
        ...
        nal_len[0] = i;
        for (i = 0; i < size[1]; i++) {
            if (memcmp(buf[1] + pos[1] + i, start_nal, sizeof(start_nal)) ==
                  0 &&
                i > 0) {
                break;
            }
...
int main(int argc, char* argv[])
{
    ...
    unsigned read_cnt = 0, pkt_cnt = 0, dec_cnt = 0, enc_cnt;
    ...
    while (fread(yuv_frame, 1, YUV_SIZE, fyuv) == YUV_SIZE) {
        ...
        status = pjmedia_vid_codec_encode_begin(codec, NULL, &frm_yuv, MTU,
                                                &frm_enc[enc_cnt], &has_more);
        if (status != PJ_SUCCESS) {
        ...
        while (has_more) {
            ...
            status = pjmedia_vid_codec_encode_more(
              codec, MTU, &frm_enc[enc_cnt], &has_more);
            if (status != PJ_SUCCESS) {
                PJ_PERROR(3, (THIS_FILE, status, "Codec encode error"));
                goto on_exit;
            }
        ...
        if (enc_cnt) {
            ...
            status = pjmedia_vid_codec_decode(codec, enc_cnt, frm_enc,
                                              YUV_BUF_SIZE, &frm_yuv);
            if (status != PJ_SUCCESS) {
...
int main(int argc, char* argv[])
{
    ...
    PJ_UNUSED_ARG(argv);
    puts(
      "Error: this sample requires video capability "
      "(PJMEDIA_HAS_VIDEO == 1)");
    return -1;
}
