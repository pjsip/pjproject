...
static const char* USAGE =
  ...
  ""
  "  --codec=codec_id	  The codec ID formatted "
  "\"name/clock-rate/channel-count\"\n"
  "                         must be specified for codec with dynamic PT,\n"
  "                         e.g: \"Speex/8000\"\n"
  "  --srtp-crypto=TAG, -c  Set crypto to be used to decrypt SRTP packets. "
  "Valid\n"
  "                         tags are: \n"
...
{
    if (app.srtp)
        pjmedia_transport_close(app.srtp);
    if (app.wav) {
...
static void read_rtp(pj_uint8_t* buf, pj_size_t bufsize, pjmedia_rtp_hdr** rtp,
                     pj_uint8_t** payload, unsigned* payload_size,
                     pj_bool_t check_pt)
{
    pj_status_t status;
    ...
    for (;;) {
         ...
         * SRTP decoding
         */
        status = pjmedia_rtp_decode_rtp(&app.rtp_sess, buf, (int)sz, &r, &p,
                                        payload_size);
        if (status != PJ_SUCCESS) {
            char errmsg[PJ_ERR_MSG_SIZE];
            pj_strerror(status, errmsg, sizeof(errmsg));
        ...
        if (app.srtp) {
            int len = (int)sz;
            status =
              pjmedia_transport_srtp_decrypt_pkt(app.srtp, PJ_TRUE, buf, &len);
            if (status != PJ_SUCCESS) {
                char errmsg[PJ_ERR_MSG_SIZE];
                pj_strerror(status, errmsg, sizeof(errmsg));
...
static void pcap2wav(const pj_str_t* codec, const pj_str_t* wav_filename,
                     pjmedia_aud_dev_index dev_id, const pj_str_t* srtp_crypto,
                     const pj_str_t* srtp_key)
{
    ...
    /* Read first packet */
    read_rtp(pkt0.buffer, sizeof(pkt0.buffer), &pkt0.rtp, &pkt0.payload,
             &pkt0.payload_len, PJ_FALSE);
    ...
    if (app.pt < 96) {
        ...
        T(pjmedia_codec_mgr_find_codecs_by_id(cmgr, codec, &cnt, info, NULL));
        if (cnt != 1)
    ...
    if (pj_strcmp2(wav_filename, "-") == 0) {
        ...
        T(pjmedia_aud_stream_create(&aud_param, NULL, &play_cb, NULL,
                                    &app.aud_strm));
        T(pjmedia_aud_stream_start(app.aud_strm));
    } else if (pj_stristr(wav_filename, &WAV)) {
        /* Open WAV file */
        T(pjmedia_wav_writer_port_create(
          app.pool, wav_filename->ptr, ci->clock_rate, ci->channel_cnt,
          samples_per_frame, param.info.pcm_bits_per_sample, 0, 0, &app.wav));
    } else {
    ...
    for (;;) {
        ...
        T(pjmedia_codec_parse(app.codec, pkt0.payload, pkt0.payload_len, &ts,
                              &frame_cnt, frames));
        ...
        /* Read next packet */
        read_rtp(pkt1.buffer, sizeof(pkt1.buffer), &pkt1.rtp, &pkt1.payload,
                 &pkt1.payload_len, PJ_TRUE);

        /* Fill in the gap (if any) between pkt0 and pkt1 */
        ts_gap = pj_ntohl(pkt1.rtp->ts) - pj_ntohl(pkt0.rtp->ts) - samples_cnt;
...
int main(int argc, char* argv[])
{
    ...
    /* Parse arguments */
    pj_optind = 0;
    while ((c = pj_getopt_long(argc, argv, "c:k:", long_options,
                               &option_index)) != -1)
    {
        switch (c) {
        case 'c':
