                          ...
                          unsigned src_addr_len)
{
    ...
    PJ_LOG(3,
           (THIS_FILE, "Component %d: received %d bytes data from %s: \"%.*s\"",
...
static pj_status_t icedemo_init(void)
{
    ...
    icedemo.pool =
      pj_pool_create(&icedemo.cp.factory, "icedemo", 512, 512, NULL);
    ...
    CHECK(
      pj_ioqueue_create(icedemo.pool, 16, &icedemo.ice_cfg.stun_cfg.ioqueue));
...
static void icedemo_init_session(unsigned rolechar)
{
    pj_ice_sess_role role =
      (pj_tolower((pj_uint8_t)rolechar) == 'o' ? PJ_ICE_SESS_ROLE_CONTROLLING :
                      ...
                      const pj_ice_sess_cand* cand)
{
    ...
    PRINT("a=candidate:%.*s %u UDP %u %s %u typ ", (int)cand->foundation.slen,
...
static int encode_session(char buffer[], unsigned maxlen)
{
    ...
    pj_ice_strans_get_ufrag_pwd(icedemo.icest, &local_ufrag, &local_pwd, NULL,
                                NULL);

    /* Write the a=ice-ufrag and a=ice-pwd attributes */
    PRINT("a=ice-ufrag:%.*s\na=ice-pwd:%.*s\n", (int)local_ufrag.slen,
          local_ufrag.ptr, (int)local_pwd.slen, local_pwd.ptr);
    ...
    for (comp = 0; comp < icedemo.opt.comp_cnt; ++comp) {
        ...
        status =
          pj_ice_strans_enum_cands(icedemo.icest, comp + 1, &cand_cnt, cand);
        if (status != PJ_SUCCESS)
            return -status;
...
static void icedemo_show_ice(void)
{
    ...
    printf("Role               : %s\n", pj_ice_strans_get_role(icedemo.icest) ==
                                            PJ_ICE_SESS_ROLE_CONTROLLED ?
                                          "controlled" :
                                          "controlling");
...
static void icedemo_input_remote(void)
{
    ...
    while (!done) {
        ...
        case 'a': {
            ...
            if (strcmp(attr, "ice-ufrag") == 0) {
                ...
                pj_sockaddr_set_port(&icedemo.rem.def_addr[1],
                                     (pj_uint16_t)port);
                ...
                cnt = sscanf(sdpcand, "%s %d %s %d %s %d typ %s", foundation,
                ...
                if (status != PJ_SUCCESS) {
                    PJ_LOG(
                      1, (THIS_FILE, "Error: invalid IP address '%s'", ipaddr));
                    goto on_error;
                }
    ...
    if (comp0_port == 0 || comp0_addr[0] == '\0') {
        PJ_LOG(1,
               (THIS_FILE, "Error: default address for component 0 not found"));
        goto on_error;
    } else {
        int af;
        pj_str_t tmp_addr;
        ...
        status =
          pj_sockaddr_set_str_addr(af, &icedemo.rem.def_addr[0], &tmp_addr);
        if (status != PJ_SUCCESS) {
...
static void icedemo_start_nego(void)
{
    ...
    if (icedemo.rem.cand_cnt == 0) {
        PJ_LOG(1,
               (THIS_FILE, "Error: No remote info, input remote info first"));
        return;
    }
    ...
    status = pj_ice_strans_start_ice(
      icedemo.icest, pj_cstr(&rufrag, icedemo.rem.ufrag),
...
static void icedemo_send_data(unsigned comp_id, const char* data)
{
    ...
    /*
    if (!pj_ice_strans_sess_is_complete(icedemo.icest)) {
        PJ_LOG(1,(THIS_FILE, "Error: ICE negotiation has not been started or is
    in progress")); return;
    }
    */

    if (comp_id < 1 ||
        comp_id > pj_ice_strans_get_running_comp_cnt(icedemo.icest)) {
        PJ_LOG(1, (THIS_FILE, "Error: invalid component ID"));
        return;
    }

    status = pj_ice_strans_sendto2(
      icedemo.icest, comp_id, data, strlen(data),
...
static void icedemo_help_menu(void)
{
    ...
    puts("");
    puts(
      "This application demonstrates how to use ICE in pjnath without having\n"
...
static void icedemo_usage()
{
    ...
    puts(
      " --stun-srv, -s HOSTDOM    Enable srflx candidate by resolving to STUN "
      "server.");
    puts(
      "                           HOSTDOM may be a \"host_or_ip[:port]\" or a "
      "domain");
    puts("                           name if DNS SRV resolution is used.");
    puts("");
    puts("TURN related options:");
    puts(
      " --turn-srv, -t HOSTDOM    Enable relayed candidate by using this TURN "
      "server.");
    puts(
      "                           HOSTDOM may be a \"host_or_ip[:port]\" or a "
      "domain");
...
int main(int argc, char* argv[])
{
    struct pj_getopt_option long_options[] = {
        { "comp-cnt", 1, 0, 'c' },      { "nameserver", 1, 0, 'n' },
        { "max-host", 1, 0, 'H' },      { "help", 0, 0, 'h' },
        { "stun-srv", 1, 0, 's' },      { "turn-srv", 1, 0, 't' },
        { "turn-tcp", 0, 0, 'T' },      { "turn-username", 1, 0, 'u' },
        { "turn-password", 1, 0, 'p' }, { "turn-fingerprint", 0, 0, 'F' },
        { "regular", 0, 0, 'R' },       { "log-file", 1, 0, 'L' },
    };
    int c, opt_id;
    pj_status_t status;
    ...
    while ((c = pj_getopt_long(argc, argv, "c:n:s:t:u:p:H:L:hTFR", long_options,
                               &opt_id)) != -1)
    {
        switch (c) {
        case 'c':
