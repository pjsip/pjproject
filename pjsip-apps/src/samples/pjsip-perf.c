...
#define DEFAULT_COUNT \
    (pjsip_cfg()->tsx.max_count / 2 > 10000 ? 10000 : \
                                              pjsip_cfg()->tsx.max_count / 2)
...
static pj_bool_t mod_stateful_on_rx_request(pjsip_rx_data* rdata)
{
    ...
    case PJSIP_INVITE_METHOD: {
        ...
        body = pjsip_msg_body_create(rdata->tp_info.pool, &mime_application,
                                     &mime_sdp, &dummy_sdp_str);
        pjsip_endpt_respond(app.sip_endpt, &mod_stateful_server, rdata, 200,
                            NULL, NULL, body, NULL);
    } break;
...
static pj_status_t send_response(pjsip_inv_session* inv, pjsip_rx_data* rdata,
                                 int code, pj_bool_t* has_initial)
{
    pjsip_tx_data* tdata;
    pj_status_t status;
    ...
    if (status != PJ_SUCCESS) {
        if (*has_initial) {
            status = pjsip_inv_answer(inv, PJSIP_SC_NOT_ACCEPTABLE, NULL, NULL,
                                      &tdata);
        } else {
            status = pjsip_inv_initial_answer(
              inv, rdata, PJSIP_SC_NOT_ACCEPTABLE, NULL, NULL, &tdata);
        }
    ...
    if (app.real_sdp) {
        ...
        if (status != PJ_SUCCESS) {
            ...
            if (tdata) {
                ...
                status = pjsip_endpt_send_response(app.sip_endpt, &res_addr,
                                                   tdata, NULL, NULL);
                if (status != PJ_SUCCESS)
                    pjsip_tx_data_dec_ref(tdata);
            } else {
    ...
    /* Add UDP/TCP transport. */
    {
        ...
        if (0) {
            ...
            status = pjsip_udp_transport_start(
              app.sip_endpt, &addr, (app.local_addr.slen ? &addrname : NULL),
              app.thread_count, &tp);
            if (status == PJ_SUCCESS) {
        ...
        app.local_uri.slen = pj_ansi_sprintf(
          app.local_uri.ptr, "<sip:pjsip-perf@%.*s:%d;transport=%s>",
          (int)app.local_addr.slen, app.local_addr.ptr, app.local_port,
          transport_type);
...
static pj_status_t init_media()
{
    ...
    app.skinfo_cnt = 0;
    for (i = 0, rtp_port = 4000; i < PJ_ARRAY_SIZE(app.skinfo);
         ++i, rtp_port += 2) {
...
static void usage(void)
{
    printf(
      ...
      "\n"
      "Client options:\n"
      "   --method=METHOD, -m     Set test method (set to INVITE for call "
      "benchmark)\n"
      "                           [default: OPTIONS]\n"
      ...
      "   --real-sdp              Generate real SDP from pjmedia, and also "
      "perform\n"
      "                           proper SDP negotiation [default: dummy]\n"
      "\n"
      "Client and Server options:\n"
      "   --local-port=PORT, -p   Set local port [default: 5060]\n"
      "   --use-tcp, -T           Use TCP instead of UDP. Note that when "
      "started as\n"
      "                           client, you must add ;transport=tcp "
      "parameter to URL\n"
      "                           [default: no]\n"
      "   --thread-count=N        Set number of worker threads [default=1]\n"
      "   --trying                Send 100/Trying response (server, default "
      "no)\n"
      "   --ringing               Send 180/Ringing response (server, default "
      "no)\n"
      "   --delay=MS, -d          Delay answering call by MS (server, default "
      "no)\n"
      "\n"
      "Misc options:\n"
      "   --help, -h              Display this screen\n"
      "   --verbose, -v           Verbose logging (put more than once for even "
      "more)\n"
      "\n"
      "When started as server, pjsip-perf can be contacted on the following "
      "URIs:\n"
...
static pj_status_t init_options(int argc, char* argv[])
{
    ...
    /* Parse options */
    pj_optind = 0;
    while ((c = pj_getopt_long(argc, argv, "p:c:m:t:w:d:hsv", long_options,
                               &option_index)) != -1)
    {
        switch (c) {
        case 'p':
        ...
        case 'c':
            ...
            if (app.client.job_count > pjsip_cfg()->tsx.max_count)
                PJ_LOG(3, (THIS_FILE,
                           ...
                           "transaction count (%d)",
                           app.client.job_count, pjsip_cfg()->tsx.max_count));
            break;
        ...
        case 'd':
            ...
            if (app.server.delay > 3600) {
                PJ_LOG(
                  3, (THIS_FILE, "I think --delay %s is too long", pj_optarg));
                return -1;
            }
            break;
...
static pj_status_t submit_stateless_job(void)
{
    ...
    status = pjsip_endpt_create_request(
      app.sip_endpt, &app.client.method, &app.client.dst_uri, &app.local_uri,
      &app.client.dst_uri, &app.local_contact, NULL, -1, NULL, &tdata);
    if (status != PJ_SUCCESS) {
    ...
    status =
      pjsip_endpt_send_request_stateless(app.sip_endpt, tdata, NULL, NULL);
    if (status != PJ_SUCCESS) {
        pjsip_tx_data_dec_ref(tdata);
...
static int client_thread(void* arg)
{
    ...
    while (PJ_TIME_VAL_LT(now, end_time) &&
           app.client.job_finished < app.client.job_count && !app.thread_quit)
    {
        pj_time_val timeout = { 0, 1 };
        unsigned i;
    ...
    while (!app.thread_quit) {
        ...
        if (thread_index == 0) {
            ...
            if (PJ_TIME_VAL_GTE(now, next_report)) {
                ...
                stateless = app.server.cur_state.stateless_cnt -
                            app.server.prev_state.stateless_cnt;
                stateful = app.server.cur_state.stateful_cnt -
                           app.server.prev_state.stateful_cnt;
                call = app.server.cur_state.call_cnt -
                       app.server.prev_state.call_cnt;
                ...
                printf(
                  "Total(rate): stateless:%s (%d/s), statefull:%s (%d/s), "
                  "call:%s (%d/s)       \r",
                  str_stateless, stateless * 1000 / msec, str_stateful,
                  stateful * 1000 / msec, str_call, call * 1000 / msec);
                fflush(stdout);
...
int main(int argc, char* argv[])
{
    ...
    printf(
      "PJSIP Performance Measurement Tool v%s\n"
      "(c)2006 pjsip.org\n\n",
      PJ_VERSION);
    ...
    if (app.client.dst_uri.slen) {
        ...
        printf(
          "Sending %d %s to '%.*s' with %d maximum outstanding jobs, please "
          "wait..\n",
          app.client.job_count, test_type, (int)app.client.dst_uri.slen,
          app.client.dst_uri.ptr, app.client.job_window);

        for (i = 0; i < app.thread_count; ++i) {
            status =
              pj_thread_create(app.pool, NULL, &client_thread,
                               (void*)(pj_ssize_t)i, 0, 0, &app.thread[i]);
            if (status != PJ_SUCCESS) {
        ...
        for (i = 0; i < PJ_ARRAY_SIZE(app.client.response_codes); ++i) {
            ...
            pj_ansi_snprintf(
              report, sizeof(report), " - %d responses:  %7d     (%.*s)", i,
              app.client.response_codes[i], (int)reason->slen, reason->ptr);
            write_report(report);
        }
        ...
        printf(
          ...
          (int)app.local_addr.slen, app.local_addr.ptr, app.local_port,
          (app.use_tcp ? ";transport=tcp" : ""), (int)app.local_addr.slen,
          app.local_addr.ptr, app.local_port,
          (app.use_tcp ? ";transport=tcp" : ""), (int)app.local_addr.slen,
          app.local_addr.ptr, app.local_port,
          (app.use_tcp ? ";transport=tcp" : ""));
        printf(
          "INVITE with non-matching user part will be handled "
          "call-statefully\n");

        for (i = 0; i < app.thread_count; ++i) {
            status =
              pj_thread_create(app.pool, NULL, &server_thread,
                               (void*)(pj_ssize_t)i, 0, 0, &app.thread[i]);
            if (status != PJ_SUCCESS) {
                app_perror(THIS_FILE, "Unable to create thread", status);
                return 1;
