...
static pj_status_t create_file_player(pj_pool_t* pool, const char* file_name,
                                      pjmedia_port** p_play_port)
{
    ...
    status =
      pjmedia_avi_player_create_streams(pool, file_name, 0, &avi_streams);
    if (status != PJ_SUCCESS)
        return status;

    vid_stream = pjmedia_avi_streams_get_stream_by_media(avi_streams, 0,
                                                         PJMEDIA_TYPE_VIDEO);
    if (!vid_stream)
        return PJ_ENOTFOUND;
...
static pj_status_t create_stream(
  ...
  pjmedia_vid_codec_param* codec_param, pjmedia_dir dir, pj_int8_t rx_pt,
  pj_int8_t tx_pt, pj_uint16_t local_port, const pj_sockaddr_in* rem_addr,
#    if defined(PJMEDIA_HAS_SRTP) && (PJMEDIA_HAS_SRTP != 0)
...
#    endif
    ...
    status =
      pjmedia_transport_udp_create(med_endpt, NULL, local_port, 0, &transport);
    if (status != PJ_SUCCESS)
        return status;
...
#    endif
    ...
    status = pjmedia_vid_stream_create(med_endpt, pool, &info, transport, NULL,
                                       p_stream);
...
static void clock_cb(const pj_timestamp* ts, void* user_data)
{
    ...
    if (play_file->decoder) {
        ...
        status = pjmedia_vid_codec_decode(
          decoder, 1, &read_frame, (unsigned)write_frame.size, &write_frame);
        if (status != PJ_SUCCESS)
            return;
    } else {
...
#    endif
    ...
    if (codec_id) {
        ...
        status = pjmedia_vid_codec_mgr_find_codecs_by_id(
          NULL, &str_codec_id, &count, &codec_info, NULL);
        if (status != PJ_SUCCESS) {
    ...
    status =
      pjmedia_vid_codec_mgr_get_default_param(NULL, codec_info, &codec_param);
...
#    endif
    ...
    if (play_file.file_name) {
        ...
        /* Collect format info */
        file_vfd =
          pjmedia_format_get_video_format_detail(&play_port->info.fmt, PJ_TRUE);
        ...
        if (codec_info->fmt_id != (pjmedia_format_id)play_port->info.fmt.id) {
            ...
            /* Find decoder */
            status = pjmedia_vid_codec_mgr_get_codec_info2(
              NULL, play_port->info.fmt.id, &codec_info2);
            if (status != PJ_SUCCESS)
                goto on_exit;
            ...
            dec_vfi =
              pjmedia_get_video_format_info(NULL, codec_info2->dec_fmt_id[0]);
        ...
        if (dir & PJMEDIA_DIR_ENCODING) {
            ...
            status = pjmedia_vid_dev_default_param(
              pool, PJMEDIA_VID_DEFAULT_CAPTURE_DEV, &vpp.vidparam);
            if (status != PJ_SUCCESS)
                goto on_exit;
        ...
        if (dir & PJMEDIA_DIR_DECODING) {
            /* Create renderer */
            status = pjmedia_vid_dev_default_param(
              pool, PJMEDIA_VID_DEFAULT_RENDER_DEV, &vpp.vidparam);
            if (status != PJ_SUCCESS)
                goto on_exit;
    ...
    status =
      create_stream(pool, med_endpt, codec_info, &codec_param, dir, rx_pt,
                    tx_pt, local_port, &remote_addr,
#    if defined(PJMEDIA_HAS_SRTP) && (PJMEDIA_HAS_SRTP != 0)
                    use_srtp, &srtp_crypto_suite, &srtp_tx_key, &srtp_rx_key,
#    endif
                    &stream);
    if (status != PJ_SUCCESS)
        goto on_exit;
    ...
    status =
      pjmedia_vid_stream_get_port(stream, PJMEDIA_DIR_ENCODING, &enc_port);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, 1);

    status =
      pjmedia_vid_stream_get_port(stream, PJMEDIA_DIR_DECODING, &dec_port);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, 1);
...
#    endif
    ...
    if (dir & PJMEDIA_DIR_ENCODING)
        PJ_LOG(2,
               (THIS_FILE, "Sending %dx%d %.*s @%.2ffps",
                codec_param.enc_fmt.det.vid.size.w,
                codec_param.enc_fmt.det.vid.size.h,
                codec_info->encoding_name.slen, codec_info->encoding_name.ptr,
