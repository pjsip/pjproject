...
int main(int argc, char* argv[])
{
    ...
    status = pjmedia_wav_player_port_create(pool, argv[pj_optind], 0, 0, 0,
                                            &file_port);
    ...
    if (PJMEDIA_PIA_CCNT(&file_port->info) != (unsigned)channel_count) {
        PJ_LOG(3, (THIS_FILE,
                   "Error: file has different number of channels. "
    ...
    status = pjmedia_resample_port_create(pool, file_port, sampling_rate, 0,
                                          &resample_port);
    if (status != PJ_SUCCESS) {
    ...
    status =
      pjmedia_snd_port_create(pool,              /* pool			    */
                              dev_id,            /* device		    */
                              dev_id,            /* device		    */
    ...
    printf("Playing %s at sampling rate %d (original file sampling rate=%d)\n",
           argv[pj_optind], sampling_rate, PJMEDIA_PIA_SRATE(&file_port->info));
    puts("");
