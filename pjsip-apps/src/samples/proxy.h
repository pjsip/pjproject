...
static pj_status_t init_stack(void)
{
    ...
    global.pool =
      pj_pool_create(&global.cp.factory, "proxyapp", 4000, 4000, NULL);
...
static pj_status_t init_proxy(void)
{
    ...
    for (i = 0; i < global.name_cnt; ++i) {
        PJ_LOG(3, (THIS_FILE, " %.*s:%d", (int)global.name[i].host.slen,
                   global.name[i].host.ptr, global.name[i].port));
    }
...
static pj_status_t proxy_verify_request(pjsip_rx_data* rdata)
{
    ...
    if (rdata->msg_info.max_fwd && rdata->msg_info.max_fwd->ivalue <= 1) {
        pjsip_endpt_respond_stateless(global.endpt, rdata,
                                      PJSIP_SC_TOO_MANY_HOPS, NULL, NULL, NULL);
...
static pj_status_t proxy_process_routing(pjsip_tx_data* tdata)
{
    ...
    if (is_uri_local(target)) {
        ...
        while ((r = (pjsip_route_hdr*)pjsip_msg_find_hdr(
                  tdata->msg, PJSIP_H_ROUTE, r->next)) != NULL)
        {
            hroute = r;
        }
...
static void proxy_postprocess(pjsip_tx_data* tdata)
{
    ...
    if (global.record_route) {
        ...
        pj_ansi_snprintf(uribuf, sizeof(uribuf), "<sip:%.*s:%d;lr>",
                         (int)global.name[0].host.slen, global.name[0].host.ptr,
                         global.name[0].port);
        uri = pj_str(uribuf);
                                          ...
                                          pjsip_tx_data* tdata)
{
    ...
    if (rdata->msg_info.msg->line.req.method.id != PJSIP_ACK_METHOD) {
        pjsip_endpt_respond_stateless(global.endpt, rdata, PJSIP_SC_NOT_FOUND,
                                      NULL, NULL, NULL);
    }
