...
static int calculate_latency(pj_pool_t* pool, pjmedia_port* wav)
{
    ...
    unsigned lat_sum = 0, lat_cnt = 0, lat_min = 10000, lat_max = 0;
    ...
    while (start_pos < len / 2 - PJMEDIA_PIA_SRATE(&wav->info)) {
        ...
        for (i = start_pos;
             i < start_pos + PJMEDIA_PIA_SRATE(&wav->info) * 700 / 1000; ++i)
        {
        ...
        lat = (max_echo_pos - max_signal_pos) * 1000 /
              PJMEDIA_PIA_SRATE(&wav->info);
