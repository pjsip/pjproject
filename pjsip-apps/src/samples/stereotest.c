...
int main(int argc, char* argv[])
{
    ...
    /* Parse arguments */
    pj_optind = 0;
    while ((c = pj_getopt_long(argc, argv, "m:C:c:", long_options,
                               &option_index)) != -1)
    {
        switch (c) {
        case OPT_MODE:
    ...
    if (mode == MODE_PLAY) {
        ...
        status = pjmedia_wav_player_port_create(pool, wav_file, PTIME, 0, 0,
                                                &file_port);
        ...
        if (snd_ch_cnt != PJMEDIA_PIA_CCNT(&file_port->info)) {
            status = pjmedia_stereo_port_create(pool, file_port, snd_ch_cnt, 0,
                                                &stereo_port);
            if (status != PJ_SUCCESS) {
        ...
        status = pjmedia_wav_writer_port_create(
          pool, wav_file, REC_CLOCK_RATE, rec_ch_cnt,
          rec_ch_cnt * PTIME * REC_CLOCK_RATE / 1000, NBITS, 0, 0, &file_port);
        if (status != PJ_SUCCESS) {
            app_perror(THIS_FILE, "Unable to open file", status);
            return 1;
        }
        ...
        status = pjmedia_snd_port_create_rec(
          ...
          snd_ch_cnt,                                 /* # of channels.	    */
          snd_ch_cnt * PTIME * REC_CLOCK_RATE / 1000, /* samples per frame.   */
