...
/**
 * \page page_pjmedia_samples_confbench_c Samples: Benchmarking Conference
 * Bridge
 *
 ...
 */
static pj_status_t create_sine_port(pj_pool_t* pool, unsigned sampling_rate,
                                    ...
                                    pjmedia_port** p_port)
{
    ...
    pjmedia_port_info_init(&port->info, &port_name, 12345, sampling_rate,
                           channel_count, 16,
    ...
    for (i = 0; i < count; i++) {
        sine->samples[i] =
          (pj_int16_t)(10000.0 * sin(((double)i / (double)count) * M_PI * 8.));
    }
...
int main()
{
    ...
    status =
      pjmedia_conf_create(pool, PORT_COUNT, CLOCK_RATE, 1, SAMPLES_PER_FRAME,
    ...
    for (i = 0; i < NULL_COUNT; ++i) {
        status = pjmedia_null_port_create(pool, CLOCK_RATE, 1,
                                          SAMPLES_PER_FRAME * 2, 16, &nulls[i]);
        PJ_ASSERT_RETURN(status == PJ_SUCCESS, 1);

        status =
          pjmedia_conf_add_port(conf, pool, nulls[i], NULL, &null_slots[i]);
        PJ_ASSERT_RETURN(status == PJ_SUCCESS, 1);
    }
    ...
    /* Create null port */
    status = pjmedia_null_port_create(pool, CLOCK_RATE, 1, SAMPLES_PER_FRAME,
                                      16, &null_port);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, 1);
    ...
    /* Create master port */
    status =
      pjmedia_master_port_create(pool, null_port, conf_port, 0, &master_port);

    pjmedia_master_port_start(master_port);

    puts("Waiting to settle..");
    fflush(stdout);
