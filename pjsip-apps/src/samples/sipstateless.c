...
static pj_bool_t on_rx_request(pjsip_rx_data* rdata)
{
    ...
    if (rdata->msg_info.msg->line.req.method.id != PJSIP_ACK_METHOD) {
        pjsip_endpt_respond_stateless(sip_endpt, rdata, code, NULL, &hdr_list,
                                      NULL);
    }
    return PJ_TRUE;
}
...
int main(int argc, char* argv[])
{
    ...
    while ((c = pj_getopt(argc, argv, "H:")) != -1) {
        ...
        case 'H':
            if (pool == NULL) {
                pool =
                  pj_pool_create(&cp.factory, "sipstateless", 1000, 1000, NULL);
            }
     ...
     */
    {
        ...
        if (status != PJ_SUCCESS) {
            PJ_LOG(3,
                   (THIS_FILE, "Error starting TCP transport (port in use?)"));
            return 1;
        }
    }
#endif
