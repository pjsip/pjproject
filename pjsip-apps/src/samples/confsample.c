...
int main(int argc, char* argv[])
{
...
#if RECORDER
    status = pjmedia_wav_writer_port_create(pool, "confrecord.wav", clock_rate,
    ...
    for (;;) {
        ...
        case 't':
            ...
            if (!input("Enter level (-128 to >127, 0 for normal)", tmp2,
                       sizeof(tmp2)))
                continue;
        ...
        case 'v':
            ...
            if (!input("Enter r for rx level or t for tx level", tmp2,
                       sizeof(tmp2)))
                continue;
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (!detail) {
            printf("Port #%02d %-25.*s  transmitting to: %s\n", port_info->slot,
                   (int)port_info->name.slen, port_info->name.ptr, txlist);
        } else {
            unsigned tx_level, rx_level;

            pjmedia_conf_get_signal_level(conf, port_info->slot, &tx_level,
                                          &rx_level);
            ...
            printf(
              ...
              port_info->slot, (int)port_info->name.slen, port_info->name.ptr,
              port_info->clock_rate, port_info->samples_per_frame,
              port_info->samples_per_frame * 1000 / port_info->clock_rate,
              port_info->tx_adj_level, port_info->rx_adj_level, tx_level,
              rx_level, txlist);
        }
    }
    puts("");
...
static void monitor_level(pjmedia_conf* conf, int slot, int dir, int dur)
{
    ...
    for (i = 0; i < total_count; ++i) {
        ...
        status =
          pjmedia_conf_get_signal_level(conf, slot, &tx_level, &rx_level);
        if (status != PJ_SUCCESS) {
        ...
        printf("Port #%02d %cx level: [%s] %d  \r", slot, dir, meter, level);
