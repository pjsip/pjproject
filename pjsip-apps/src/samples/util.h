 ...
 * common sound options.
 */
pj_status_t get_snd_options(const char* app_name, int argc, char* argv[],
                            int* dev_id, int* clock_rate, int* channel_count,
                            int* samples_per_frame, int* bits_per_sample)
{
    struct pj_getopt_option long_options[] = {
        { "dev", 1, 0, 'd' },   { "rate", 1, 0, 'r' }, { "channel", 1, 0, 'c' },
        { "frame", 1, 0, 'f' }, { "bit", 1, 0, 'b' },  { NULL, 0, 0, 0 },
    };
    int c;
    int option_index;
    long val;
    ...
    *samples_per_frame = 0;
    ...
    pj_optind = 0;
    while ((c = pj_getopt_long(argc, argv, "d:r:c:f:b:", long_options,
                               &option_index)) != -1)
    {
        switch (c) {
        case 'd':
...
void dump_pool_usage(const char* app_name, pj_caching_pool* cp)
{
    ...
    PJ_LOG(3, (app_name, "Total pool memory allocated=%d KB, used=%d KB",
               total_alloc / 1000, total_used / 1000));
#endif
}
