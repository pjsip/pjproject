...
/* Settings */
#define AF \
    pj_AF_INET() /* Change to pj_AF_INET6() for IPv6. \
...
#define MAX_MEDIA_CNT \
    2 /* Media count, set to 1 for audio \
...
static pj_bool_t logging_on_rx_msg(pjsip_rx_data* rdata)
{
    PJ_LOG(4, (THIS_FILE,
               ...
               rdata->tp_info.transport->type_name, rdata->pkt_info.src_name,
...
static pj_status_t logging_on_tx_msg(pjsip_tx_data* tdata)
{
    ...
    PJ_LOG(4,
           (THIS_FILE,
            ...
            tdata->tp_info.transport->type_name, tdata->tp_info.dst_name,
            tdata->tp_info.dst_port, (int)(tdata->buf.cur - tdata->buf.start),
            tdata->buf.start));
...
int main(int argc, char* argv[])
{
     ...
     */
    {
        ...
        if (af == pj_AF_INET()) {
            status =
              pjsip_udp_transport_start(g_endpt, &addr.ipv4, NULL, 1, NULL);
        } else if (af == pj_AF_INET6()) {
            status =
              pjsip_udp_transport_start6(g_endpt, &addr.ipv6, NULL, 1, NULL);
        } else {
            status = PJ_EAFNOTSUP;
        }
...
#else
    status = pjmedia_endpt_create(&cp.factory, pjsip_endpt_get_ioqueue(g_endpt),
                                  0, &g_med_endpt);
#endif
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, 1);
...
#    endif /* PJMEDIA_HAS_FFMPEG_VID_CODEC */
    ...
    if (argc > 1) {
        ...
        pj_ansi_sprintf(temp, "<sip:simpleuac@%s:%d>", hostip, SIP_PORT);
        local_uri = pj_str(temp);

        /* Create UAC dialog */
        status =
          pjsip_dlg_create_uac(pjsip_ua_instance(), &local_uri, /* local URI */
         ...
         */
        status =
          pjmedia_endpt_create_sdp(g_med_endpt,   /* the media endpt	*/
                                   dlg->pool,     /* pool.		*/
...
static pj_bool_t on_rx_request(pjsip_rx_data* rdata)
{
    ...
    if (rdata->msg_info.msg->line.req.method.id != PJSIP_INVITE_METHOD) {
        if (rdata->msg_info.msg->line.req.method.id != PJSIP_ACK_METHOD) {
            pj_str_t reason = pj_str(
              "Simple UA unable to handle "
              "this request");

            pjsip_endpt_respond_stateless(g_endpt, rdata, 500, &reason, NULL,
                                          NULL);
        }
        return PJ_TRUE;
    }
    ...
    status =
      pjsip_inv_verify_request(rdata, &options, NULL, NULL, g_endpt, NULL);
    if (status != PJ_SUCCESS) {
     ...
     */
    status = pjsip_inv_answer(g_inv, 200, NULL, /* st_code and st_text */
...
static void call_on_media_update(pjsip_inv_session* inv, pj_status_t status)
{
    ...
    status = pjmedia_stream_info_from_sdp(
      &stream_info, inv->dlg->pool, g_med_endpt, local_sdp, remote_sdp, 0);
    if (status != PJ_SUCCESS) {
    ...
    pjmedia_snd_port_create(
      inv->pool, PJMEDIA_AUD_DEFAULT_CAPTURE_DEV,
    ...
    if (local_sdp->media_count > 1) {
        ...
        status = pjmedia_vid_stream_info_from_sdp(
          &vstream_info, inv->dlg->pool, g_med_endpt, local_sdp, remote_sdp, 1);
        if (status != PJ_SUCCESS) {
         ...
         */
        status =
          pjmedia_vid_stream_create(g_med_endpt, NULL, &vstream_info,
                                    g_med_transport[1], NULL, &g_med_vstream);
        if (status != PJ_SUCCESS) {
        ...
        if (vstream_info.dir & PJMEDIA_DIR_DECODING) {
            status = pjmedia_vid_dev_default_param(
              inv->pool, PJMEDIA_VID_DEFAULT_RENDER_DEV, &vport_param.vidparam);
            if (status != PJ_SUCCESS) {
                app_perror(THIS_FILE,
                           "Unable to get default param of video "
                           "renderer device",
                           status);
                return;
            }
            ...
            /* Create renderer */
            status =
              pjmedia_vid_port_create(inv->pool, &vport_param, &g_vid_renderer);
            if (status != PJ_SUCCESS) {
            ...
            status =
              pjmedia_vid_port_connect(g_vid_renderer, media_port, PJ_FALSE);
            if (status != PJ_SUCCESS) {
        ...
        if (vstream_info.dir & PJMEDIA_DIR_ENCODING) {
            ...
            /* Create capturer */
            status =
              pjmedia_vid_port_create(inv->pool, &vport_param, &g_vid_capturer);
            if (status != PJ_SUCCESS) {
            ...
            status =
              pjmedia_vid_port_connect(g_vid_capturer, media_port, PJ_FALSE);
            if (status != PJ_SUCCESS) {
        ...
        if (g_vid_renderer) {
            ...
            if (status != PJ_SUCCESS) {
                app_perror(THIS_FILE, "Unable to start video renderer", status);
                return;
            }
        }
        if (g_vid_capturer) {
            ...
            if (status != PJ_SUCCESS) {
                app_perror(THIS_FILE, "Unable to start video capturer", status);
                return;
            }
        }
    }
