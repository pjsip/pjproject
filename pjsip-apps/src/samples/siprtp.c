...
static const char* USAGE =
  ...
  "   --auto-quit,      -q    Quit when calls have been completed "
  "(default:no)\n"
  "   --call-report     -R    Display report on call termination "
  "(default:yes)\n"
  "\n"
  ...
  "                           try to determine local IP address from "
  "hostname)\n"
  "\n"
  " Logging Options:\n"
  ...
  "   --a-clock=RATE          Set audio codec rate to RATE Hz
  (default=8000Hz)\n" "   --a-bitrate=BPS         Set audio codec bitrate to BPS
  (default=64000bps)\n" "   --a-ptime=MS            Set audio frame time to MS
  msec (default=20ms)\n"
  */
  ;
...
static pj_status_t init_sip()
{
    ...
    /* Add UDP transport. */
    {
        ...
        status = pjsip_udp_transport_start(
          app.sip_endpt, &addr, (app.local_addr.slen ? &addrname : NULL), 1,
          &tp);
        if (status != PJ_SUCCESS) {
    ...
    for (i = 0, count = 0; i < app.max_calls; ++i, ++count) {
        ...
        for (j = 0; j < PJ_ARRAY_SIZE(app.call[0].media); ++j) {
            ...
            for (retry = 0; retry < 100; ++retry, rtp_port += 2) {
                ...
                status = pjmedia_transport_udp_create2(
                  app.med_endpt, "siprtp", &app.local_addr, rtp_port, 0,
                  &m->transport);
                if (status == PJ_SUCCESS) {
                    rtp_port += 2;
                    break;
...
static void process_incoming_call(pjsip_rx_data* rdata)
{
    ...
    if (status != PJ_SUCCESS) {
        ...
        if (tdata) {
            ...
            pjsip_endpt_respond_stateless(app.sip_endpt, rdata, 500, NULL, NULL,
                                          NULL);
        }
    ...
    /* Create 200 response .*/
    status =
      pjsip_inv_initial_answer(call->inv, rdata, 200, NULL, NULL, &tdata);
    if (status != PJ_SUCCESS) {
        status = pjsip_inv_initial_answer(
          call->inv, rdata, PJSIP_SC_NOT_ACCEPTABLE, NULL, NULL, &tdata);
        if (status == PJ_SUCCESS)
...
static void call_on_state_changed(pjsip_inv_session* inv, pjsip_event* e)
{
    ...
    if (inv->state == PJSIP_INV_STATE_DISCONNECTED) {
        ...
        PJ_LOG(
          3, (THIS_FILE, "Call #%d disconnected. Reason=%d (%.*s)", call->index,
              inv->cause, (int)inv->cause_text.slen, inv->cause_text.ptr));
...
static pj_status_t init_options(int argc, char* argv[])
{
    ...
    /* Parse options */
    pj_optind = 0;
    while ((c = pj_getopt_long(argc, argv, "c:d:p:r:i:l:g:qR", long_options,
                               &option_index)) != -1)
    {
        switch (c) {
        case 'c':
...
#    define POLICY SCHED_FIFO
    ...
    if (rc != 0) {
        app_perror(THIS_FILE, "sched_getparam error", PJ_RETURN_OS_ERROR(rc));
        return;
    }
    ...
    PJ_LOG(4, (THIS_FILE, "New process policy=%d, priority=%d", policy,
               tp.sched_priority));
    ...
    PJ_LOG(4, (THIS_FILE, "Old thread policy=%d, priority=%d", policy,
               tp.sched_priority));
    ...
    PJ_LOG(4, (THIS_FILE, "New thread policy=%d, priority=%d", policy,
               tp.sched_priority));
}
...
static void on_rx_rtp(void* user_data, void* pkt, pj_ssize_t size)
{
    ...
    /* Decode RTP packet. */
    status = pjmedia_rtp_decode_rtp(&strm->in_sess, pkt, (int)size, &hdr,
                                    &payload, &payload_len);
    if (status != PJ_SUCCESS) {
    ...
    pjmedia_rtcp_rx_rtp(&strm->rtcp, pj_ntohs(hdr->seq), pj_ntohl(hdr->ts),
                        payload_len);
...
static void on_rx_rtcp(void* user_data, void* pkt, pj_ssize_t size)
{
    ...
    /* Check for errors */
    if (size < 0) {
        app_perror(THIS_FILE, "Error receiving RTCP packet",
                   (pj_status_t)-size);
        return;
    }
...
static int media_thread(void* arg)
{
    ...
    next_rtcp = next_rtp;
    next_rtcp.u64 +=
      (freq.u64 * (RTCP_INTERVAL + (pj_rand() % RTCP_RAND)) / 1000);
    ...
    while (!strm->thread_quit_flag) {
        ...
        if (send_rtp || next_rtp.u64 <= now.u64) {
            ...
            status = pjmedia_rtp_encode_rtp(
              &strm->out_sess, strm->si.tx_pt, 0, /* marker bit */
              strm->bytes_per_frame, strm->samples_per_frame, &p_hdr, &hdrlen);
            if (status == PJ_SUCCESS) {
                ...
                status =
                  pjmedia_transport_send_rtp(strm->transport, packet, size);
                if (status != PJ_SUCCESS)
            ...
            /* Update RTCP SR */
            pjmedia_rtcp_tx_rtp(&strm->rtcp,
                                (pj_uint16_t)strm->bytes_per_frame);
        ...
        if (send_rtcp || next_rtcp.u64 <= now.u64) {
            ...
            size = rtcp_len;
            status =
              pjmedia_transport_send_rtcp(strm->transport, rtcp_pkt, size);
            if (status != PJ_SUCCESS) {
...
static void call_on_media_update(pjsip_inv_session* inv, pj_status_t status)
{
    ...
    audio->bytes_per_frame =
      codec_desc->bit_rate * codec_desc->ptime / 1000 / 8;
    ...
    status = pjmedia_transport_attach(
      audio->transport, audio, &audio->si.rem_addr, &audio->si.rem_rtcp,
      sizeof(pj_sockaddr_in), &on_rx_rtp, &on_rx_rtcp);
    if (status != PJ_SUCCESS) {
...
#if PJ_HAS_THREADS
    status = pj_thread_create(inv->pool, "media", &media_thread, audio, 0, 0,
                              &audio->thread);
    if (status != PJ_SUCCESS) {
...
#define BIGVAL         0x7FFFFFFFL
    ...
    printf(
      ...
      count, call_dur.min / 1000, call_dur.avg / 1000, call_dur.max / 1000,
      "seconds",

      call_pdd.min, call_pdd.avg, call_pdd.max, "ms",

      /* rx */

      good_number(srx_min, min_stat.rx.pkt),
      good_number(srx_avg, avg_stat.rx.pkt),
      good_number(srx_max, max_stat.rx.pkt), "packets",

      good_number(brx_min, min_stat.rx.bytes),
      good_number(brx_avg, avg_stat.rx.bytes),
      good_number(brx_max, max_stat.rx.bytes), "bytes",

      min_stat.rx.loss, avg_stat.rx.loss, max_stat.rx.loss, "packets",
      ...
      min_stat.rx.dup, avg_stat.rx.dup, max_stat.rx.dup, "packets",

      min_stat.rx.reorder, avg_stat.rx.reorder, max_stat.rx.reorder, "packets",

      min_stat.rx.jitter.min / 1000.0, avg_stat.rx.jitter.mean / 1000.0,
      max_stat.rx.jitter.max / 1000.0, "ms",

      /* tx */

      good_number(stx_min, min_stat.tx.pkt),
      good_number(stx_avg, avg_stat.tx.pkt),
      good_number(stx_max, max_stat.tx.pkt), "packets",

      good_number(btx_min, min_stat.tx.bytes),
      good_number(btx_avg, avg_stat.tx.bytes),
      good_number(btx_max, max_stat.tx.bytes), "bytes",

      min_stat.tx.loss, avg_stat.tx.loss, max_stat.tx.loss, "packets",
      ...
      min_stat.tx.dup, avg_stat.tx.dup, max_stat.tx.dup, "packets",

      min_stat.tx.reorder, avg_stat.tx.reorder, max_stat.tx.reorder, "packets",

      min_stat.tx.jitter.min / 1000.0, avg_stat.tx.jitter.mean / 1000.0,
      max_stat.tx.jitter.max / 1000.0, "ms",

      /* rtt */
      min_stat.rtt.min / 1000.0, avg_stat.rtt.mean / 1000.0,
      max_stat.rtt.max / 1000.0, "ms");
}
...
static pj_status_t logger_on_tx_msg(pjsip_tx_data* tdata)
{
    ...
    PJ_LOG(
      4, (THIS_FILE,
          ...
          tdata->tp_info.dst_name, tdata->tp_info.dst_port, tdata->buf.start));
...
pj_status_t app_logging_init(void)
{
    ...
    if (app.log_filename) {
        ...
        if (log_file == NULL) {
            PJ_LOG(1,
                   (THIS_FILE, "Unable to open log file %s", app.log_filename));
            return -1;
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    if (app.auto_quit && app.uri_to_call.slen == 0) {
        printf(
          "Error: --auto-quit option only valid for outgoing "
          "mode (UAC) only\n");
        return 1;
    }
...
#if PJ_HAS_THREADS
    for (i = 0; i < app.thread_count; ++i) {
        pj_thread_create(app.pool, "app", &sip_worker_thread, NULL, 0, 0,
                         &app.sip_thread[i]);
    }
#endif
    ...
    if (app.uri_to_call.slen) {
        ...
        PJ_LOG(3,
               (THIS_FILE, "Ready for incoming calls (max=%d)", app.max_calls));
