...
static void hangup_all(void)
{
    ...
    for (i = 0; i < MAX_CALLS; ++i) {
        ...
        if (call->inv && call->inv->state <= PJSIP_INV_STATE_CONFIRMED) {
            ...
            status = pjsip_inv_end_session(call->inv, PJSIP_SC_BUSY_HERE, NULL,
                                           &tdata);
...
static pj_status_t init_stack()
{
    ...
    if (sip_af == pj_AF_INET()) {
        if (sip_tcp) {
            status =
              pjsip_tcp_transport_start(app.sip_endpt, &addr.ipv4, 1, NULL);
        } else {
        ...
        status =
          pjsip_udp_transport_start6(app.sip_endpt, &addr.ipv6, NULL, 1, NULL);
    } else {
        status = PJ_EAFNOTSUP;
    }
...
static pjmedia_sdp_session* create_answer(int call_num, pj_pool_t* pool,
                                          const pjmedia_sdp_session* offer)
{
    const char* dir_attrs[] = { "sendrecv", "sendonly", "recvonly",
                                "inactive" };
    ...
    sess_dir_attr = find_remove_sdp_attrs(&answer->attr_count, answer->attr,
                                          PJ_ARRAY_SIZE(dir_attrs), dir_attrs);
    ...
    for (mi = 0; mi < answer->media_count; ++mi) {
        ...
        find_remove_sdp_attrs(&m->attr_count, m->attr, PJ_ARRAY_SIZE(ice_attrs),
                              ice_attrs);

        /* Done */
        c = m->conn ? m->conn : answer->conn;
        PJ_LOG(3, (THIS_FILE, "  Media %d, %.*s: %s <--> %.*s:%d", mi,
                   (int)m->desc.media.slen, m->desc.media.ptr,
                   (our_dir ? our_dir : "sendrecv"), (int)c->addr.slen,
                   c->addr.ptr, m->desc.port));
    }
...
static void call_on_state_changed(pjsip_inv_session* inv, pjsip_event* e)
{
    ...
    if (inv->state == PJSIP_INV_STATE_DISCONNECTED) {
        ...
        PJ_LOG(3, (THIS_FILE, "Call %d: state changed to %s", call - app.call,
                             ...
                             const pjmedia_sdp_session* offer)
{
    ...
    pjsip_inv_set_sdp_answer(
      inv, create_answer((int)(call - app.call), inv->pool_prov, offer));
}
...
static pj_bool_t on_rx_request(pjsip_rx_data* rdata)
{
    ...
    if (rdata->msg_info.msg->line.req.method.id != PJSIP_INVITE_METHOD) {
        if (rdata->msg_info.msg->line.req.method.id != PJSIP_ACK_METHOD) {
            ...
            pjsip_endpt_respond_stateless(app.sip_endpt, rdata, 400, &reason,
                                          NULL, NULL);
        }
        return PJ_TRUE;
    }
    ...
    if (!sdp_info || !sdp_info->sdp) {
        ...
        pjsip_endpt_respond_stateless(app.sip_endpt, rdata, 400, &reason, NULL,
                                      NULL);
    }
    ...
    if (i == MAX_CALLS) {
        ...
        pjsip_endpt_respond_stateless(app.sip_endpt, rdata, PJSIP_SC_BUSY_HERE,
                                      &reason, NULL, NULL);
        return PJ_TRUE;
    }
    ...
    if (status == PJ_SUCCESS)
        answer =
          create_answer((int)(call - app.call), dlg->pool, sdp_info->sdp);
    ...
    if (status == PJ_SUCCESS)
        status =
          pjsip_inv_initial_answer(call->inv, rdata, 100, NULL, NULL, &tdata);
    if (status == PJ_SUCCESS)
...
{
    struct pj_getopt_option long_options[] = { { "local-port", 1, 0, 'p' },
                                               { "tcp", 0, 0, 't' },
                                               { "ipv6", 0, 0, '6' },
                                               { "help", 0, 0, 'h' } };
    int c, option_index;
