...
static pj_status_t enc_dec_test(const char* codec_id, const char* filein,
                                const char* fileout)
{
    pj_pool_t* pool;
...
#define T file_msec_duration / 1000, file_msec_duration % 1000
    ...
    cnt = 1;
    CHECK(pjmedia_codec_mgr_find_codecs_by_id(cm, pj_cstr(&tmp, codec_id), &cnt,
                                              &pci, NULL));
    ...
    CHECK(pjmedia_wav_player_port_create(pool, filein, param.info.frm_ptime,
                                         PJMEDIA_FILE_NO_LOOP, 0, &wavin));

    /* Open wav for writing */
    CHECK(pjmedia_wav_writer_port_create(pool, fileout, param.info.clock_rate,
                                         param.info.channel_cnt,
                                         samples_per_frame, 16, 0, 0, &wavout));
    ...
    for (;;) {
        ...
        /* Update duration */
        file_msec_duration += samples_per_frame * 1000 / param.info.clock_rate;

        /* Encode */
        frm_bit.buf = bitstream;
        frm_bit.size = sizeof(bitstream);
        CHECK(
          pjmedia_codec_encode(codec, &frm_pcm, sizeof(bitstream), &frm_bit));
        ...
        if (frm_bit.size == 0 || frm_bit.type != PJMEDIA_FRAME_TYPE_AUDIO) {
            ...
            TRACE_((THIS_FILE, "%d.%03d read: %u, enc: %u", T, frm_pcm.size,
                    frm_bit.size));
            continue;
        }
        ...
        TRACE_((THIS_FILE, "%d.%03d read: %u, enc: %u, dec/write: %u", T,
