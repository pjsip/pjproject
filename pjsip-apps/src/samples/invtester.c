...
/* Send request */
static void send_request(const pjsip_method* method, int cseq,
                         const pj_str_t* branch, pj_bool_t with_offer)
{
    pjsip_tx_data* tdata;
    ...
    if (with_offer) {
        ...
        body = pjsip_msg_body_create(tdata->pool, &mime_application, &mime_sdp,
                                     &dummy_sdp_str);
        tdata->msg->body = body;
    }
...
static void on_tsx_state(pjsip_transaction* tsx, pjsip_event* event)
{
    if (tsx->role == PJSIP_ROLE_UAC) {
        ...
        {
#if SAME_BRANCH
            send_request(&pjsip_ack_method, tsx->cseq, &tsx->branch,
                         ACK_HAS_SDP);
#else
        ...
        if (event->type == PJSIP_EVENT_RX_MSG &&
            ...
            pjsip_dlg_create_response(dlg, event->body.tsx_state.src.rdata, 200,
                                      NULL, &tdata);
...
void make_call(char* uri, pj_bool_t with_offer)
{
    ...
    status = pjsip_dlg_create_uac(pjsip_ua_instance(), &local, &local, &remote,
                                  &remote, &dlg);
    pj_assert(status == PJ_SUCCESS);
...
int main(int argc, char* argv[])
{
     ...
     */
    {
        ...
        if (status != PJ_SUCCESS) {
            PJ_LOG(3,
                   (THIS_FILE, "Error starting UDP transport (port in use?)"));
            return 1;
        }
    }
