...
int main(int argc, char* argv[])
{
    ...
    status = pjmedia_wav_player_port_create(pool, argv[pj_optind], PTIME,
                                            PJMEDIA_FILE_NO_LOOP, 0, &wav_play);
    ...
    status = pjmedia_wav_player_port_create(pool, argv[pj_optind + 1], PTIME,
                                            PJMEDIA_FILE_NO_LOOP, 0, &wav_rec);
    if (status != PJ_SUCCESS) {
    ...
    status = pjmedia_wav_writer_port_create(
      pool, argv[pj_optind + 2], PJMEDIA_PIA_SRATE(&wav_play->info),
      PJMEDIA_PIA_CCNT(&wav_play->info), PJMEDIA_PIA_SPF(&wav_play->info),
      PJMEDIA_PIA_BITS(&wav_play->info), 0, 0, &wav_out);
    if (status != PJ_SUCCESS) {
    ...
    status = pjmedia_echo_create2(pool, PJMEDIA_PIA_SRATE(&wav_play->info),
                                  ...
                                  PJMEDIA_PIA_SPF(&wav_play->info), tail_ms,
                                  latency_ms, opt, &ec);
    if (status != PJ_SUCCESS) {
    ...
    i = (int)pjmedia_wav_writer_port_get_pos(wav_out) / sizeof(pj_int16_t) *
        1000 /
        (PJMEDIA_PIA_SRATE(&wav_out->info) * PJMEDIA_PIA_CCNT(&wav_out->info));
    PJ_LOG(3, (THIS_FILE, "Processed %3d.%03ds audio", i / 1000, i % 1000));
