...
static void list_devices(void)
{
    ...
    for (i = 0; i < dev_count; ++i) {
        ...
        PJ_LOG(3, (THIS_FILE, " %2d: %s [%s] (%d/%d)", i, info.driver,
...
static const char* decode_caps(unsigned caps)
{
    ...
    for (i = 0; i < 31; ++i) {
        if ((1 << i) & caps) {
            const char* capname;
            capname =
              pjmedia_aud_dev_cap_name((pjmedia_aud_dev_cap)(1 << i), NULL);
            strcat(text, capname);
            strcat(text, " ");
        }
    }
                        ...
                        unsigned clock_rate, unsigned ptime, unsigned chnum)
{
    ...
    if (dir & PJMEDIA_DIR_CAPTURE) {
        if (result.rec.frame_cnt == 0) {
            ...
            PJ_LOG(3,
                   (THIS_FILE,
                    "  %-20s: interval (min/max/avg/dev)=%u/%u/%u/%u, burst=%u",
                    "Recording result", result.rec.min_interval,
                    result.rec.max_interval, result.rec.avg_interval,
                    result.rec.dev_interval, result.rec.max_burst));
        }
    }
    ...
    if (dir & PJMEDIA_DIR_PLAYBACK) {
        if (result.play.frame_cnt == 0) {
            ...
            PJ_LOG(3,
                   (THIS_FILE,
                    "  %-20s: interval (min/max/avg/dev)=%u/%u/%u/%u, burst=%u",
                    "Playback result", result.play.min_interval,
                    result.play.max_interval, result.play.avg_interval,
                    result.play.dev_interval, result.play.max_burst));
        }
    }
    ...
    if (dir == PJMEDIA_DIR_CAPTURE_PLAYBACK) {
        if (result.rec_drift_per_sec == 0) {
            ...
            const char* which =
              result.rec_drift_per_sec >= 0 ? "faster" : "slower";
...
static void record(unsigned rec_index, const char* filename)
{
    ...
    pool = pj_pool_create(pjmedia_aud_subsys_get_pool_factory(), "wav", 1000,
                          1000, NULL);

    status = pjmedia_wav_writer_port_create(pool, filename, 16000, 1, 320, 16,
                                            0, 0, &wav);
    if (status != PJ_SUCCESS) {
...
static void play_file(unsigned play_index, const char* filename)
{
    ...
    status = pjmedia_aud_stream_create(&param, NULL, &wav_play_cb, wav, &strm);
    if (status != PJ_SUCCESS) {
        app_perror("Error opening the sound device", status);
        goto on_return;
    }
        ...
        case 'd':
            /* latencies */
            {
                ...
                if (sscanf(line + 2, "%d %d", &rec_lat, &play_lat) == 2) {
                    capture_lat = (unsigned)(rec_lat >= 0 ?
                                               rec_lat :
