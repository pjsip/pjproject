...
static pj_bool_t proxy_on_rx_request(pjsip_rx_data* rdata)
{
    ...
    if (rdata->msg_info.msg->line.req.method.id != PJSIP_CANCEL_METHOD) {
         ...
         */
        status = pjsip_endpt_create_request_fwd(global.endpt, rdata, NULL, NULL,
                                                0, &tdata);
        if (status != PJ_SUCCESS) {
         ...
         */
        uac_data = (struct uac_data*)pj_pool_alloc(uac_tsx->pool,
                                                   sizeof(struct uac_data));
         ...
         */
        uas_data = (struct uas_data*)pj_pool_alloc(uas_tsx->pool,
                                                   sizeof(struct uas_data));
        ...
        if (status != PJ_SUCCESS) {
            ...
            pjsip_endpt_create_response(global.endpt, rdata, 500, NULL,
                                        &err_res);
        ...
        if (!invite_uas) {
            ...
            pjsip_endpt_respond_stateless(global.endpt, rdata, 481, NULL, NULL,
                                          NULL);
            return PJ_TRUE;
        }

        /* Respond 200 OK to CANCEL */
        pjsip_endpt_respond(global.endpt, NULL, rdata, 200, NULL, NULL, NULL,
                            NULL);
...
static pj_bool_t proxy_on_rx_response(pjsip_rx_data* rdata)
{
    ...
    /* Forward response */
    status =
      pjsip_endpt_send_response(global.endpt, &res_addr, tdata, NULL, NULL);
    if (status != PJ_SUCCESS) {
        pjsip_tx_data_dec_ref(tdata);
...
static void tu_on_tsx_state(pjsip_transaction* tsx, pjsip_event* event)
{
    ...
    if (tsx->role == PJSIP_ROLE_UAS) {
        if (tsx->state == PJSIP_TSX_STATE_TERMINATED) {
            ...
            if (uas_data->uac_tsx) {
                uac_data =
                  (struct uac_data*)uas_data->uac_tsx->mod_data[mod_tu.id];
    ...
    if (event->body.tsx_state.type == PJSIP_EVENT_RX_MSG) {
         ...
         */
        status =
          pjsip_endpt_create_response_fwd(global.endpt, rdata, 0, &tdata);
        if (status != PJ_SUCCESS) {
            app_perror("Error creating response", status);
            return;
        }
...
#if PJ_HAS_THREADS
    status = pj_thread_create(global.pool, "sproxy", &worker_thread, NULL, 0, 0,
                              &global.thread);
    if (status != PJ_SUCCESS) {
