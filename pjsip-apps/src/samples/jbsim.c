...
static void write_log(struct log_entry* entry, pj_bool_t to_stdout)
{
    /* Format (CSV): */
    const char* format =
      "TIME;EVENT;#RX packets;#packets lost;#JB prefetch;#JB "
      "size;#JBDISCARD;#JBEMPTY;Log Message";
    static char log[2000];
    enum
    {
        D = 20
    };
    char s_jbprefetch[D], s_jbsize[D], s_rxpkt[D], s_losspkt[D], s_jbdiscard[D],
      s_jbempty[D];
    static pj_bool_t header_written;

    if (!header_written) {
        pj_ansi_snprintf(log, sizeof(log), "%s\n", format);
    ...
    pj_ansi_snprintf(log, sizeof(log),
                     ...
                     (int)entry->wall_clock.sec,
                     (int)entry->wall_clock.msec, /* time */
                     entry->event, s_rxpkt, s_losspkt, s_jbprefetch, s_jbsize,
                     s_jbdiscard, s_jbempty, entry->log);
    if (g_app.log_fd != NULL) {
        pj_ssize_t size = strlen(log);
        pj_file_write(g_app.log_fd, log, &size);
    }
...
static pj_status_t stream_init(const struct stream_cfg* cfg,
                               struct stream** p_stream)
{
    ...
    count = 1;
    status =
      pjmedia_codec_mgr_find_codecs_by_id(cm, &cfg->codec, &count, &ci, NULL);
    ...
    if (cfg->ptime) {
        si.param->setting.frm_per_pkt =
          (pj_uint8_t)((cfg->ptime + si.param->info.frm_ptime - 1) /
    ...
    /* Create stream */
    status = pjmedia_stream_create(g_app.endpt, pool, &si, g_app.loop, NULL,
                                   &stream->strm);
    if (status != PJ_SUCCESS) {
...
static pj_status_t test_init(void)
{
    ...
    if (g_app.cfg.log_file) {
        status = pj_file_open(g_app.pool, g_app.cfg.log_file, PJ_O_WRONLY,
                              &g_app.log_fd);
        if (status != PJ_SUCCESS) {
        ...
        pj_log_set_decor(PJ_LOG_HAS_SENDER | PJ_LOG_HAS_COLOR |
    ...
    status = pjmedia_wav_player_port_create(
      g_app.pool, g_app.cfg.tx_wav_in, g_app.cfg.tx_ptime, 0, 0, &g_app.tx_wav);
    if (status != PJ_SUCCESS) {
    ...
    if (PJMEDIA_PIA_SRATE(&g_app.tx_wav->info) !=
          PJMEDIA_PIA_SRATE(&g_app.tx->port->info) ||
        PJMEDIA_PIA_CCNT(&g_app.tx_wav->info) !=
          PJMEDIA_PIA_CCNT(&g_app.tx->port->info))
    {
        jbsim_perror(
          "Error: Input WAV file has different clock rate "
          "or number of channels than the codec",
          PJ_SUCCESS);
        goto on_error;
    }
    ...
    /* Frame buffer */
    g_app.framebuf = (pj_int16_t*)pj_pool_alloc(
      g_app.pool, MAX(PJMEDIA_PIA_SPF(&g_app.rx->port->info),
                      PJMEDIA_PIA_SPF(&g_app.tx->port->info)) *
                    sizeof(pj_int16_t));
...
static void tx_tick(const pj_time_val* t)
{
    ...
    pkt_interval =
      PJMEDIA_PIA_SPF(&port->info) * 1000 / PJMEDIA_PIA_SRATE(&port->info);
    ...
    while (PJ_TIME_VAL_GTE(*t, strm->state.tx.next_schedule)) {
        ...
        if (strm->state.tx.cur_lost_burst) {
            ...
            if (!drop_this_pkt &&
                ...
                MAX(strm->state.tx.total_lost - LOSS_EXTRA, 0) * 100 /
                    MAX(strm->state.tx.total_tx, 1) <
                  g_app.cfg.tx_pct_avg_lost)
            {
                strm->state.tx.drop_prob =
                  ((g_app.cfg.tx_pct_loss_corr * strm->state.tx.drop_prob) +
        ...
        if (!drop_this_pkt && MAX(strm->state.tx.total_lost - LOSS_EXTRA, 0) *
                                  100 / MAX(strm->state.tx.total_tx, 1) <
                                g_app.cfg.tx_pct_avg_lost)
        {
        ...
        if (drop_this_pkt) {
            /* Drop the frame */
            pjmedia_transport_simulate_lost(g_app.loop, PJMEDIA_DIR_ENCODING,
                                            100);
            ...
            if (jstate.discard > last_discard)
                strcat(log_msg,
                       "** Note: packet was discarded by jitter buffer **");
        ...
        strm->state.tx.next_schedule.msec =
          (strm->state.tx.total_tx + 1) * pkt_interval;
        ...
        if (g_app.cfg.tx_max_jitter || g_app.cfg.tx_min_jitter) {
            if (g_app.cfg.tx_max_jitter == g_app.cfg.tx_min_jitter) {
                ...
                int jitter_range;
                jitter_range =
                  (g_app.cfg.tx_max_jitter - g_app.cfg.tx_min_jitter) * 2;
        ...
        sprintf(
          log_msg, "** Packet #%u tick is at %d.%03d, %d ms jitter applied **",
          strm->state.tx.total_tx + 1, (int)strm->state.tx.next_schedule.sec,
          (int)strm->state.tx.next_schedule.msec, jitter);
...
static void usage(void)
{
    ...
    printf("  --log-file, -%c FILE    Save simulation log file to FILE\n",
           OPT_LOG_FILE);
    printf(
      "                         Note: FILE will be in CSV format with "
      "semicolon separator\n");
    printf("                         Default: %s\n", LOG_FILE);
    ...
    printf("                         Default: 0\n");
    printf(
      "  --loss-corr PCT        Set the loss correlation to PCT percent. "
      "Default: 0\n");
    printf(
      "  --min-lost-burst N     Set minimum packet lost burst (default:%d)\n",
      MIN_LOST_BURST);
    printf(
      "  --max-lost-burst N     Set maximum packet lost burst (default:%d)\n",
      MAX_LOST_BURST);
    ...
    printf("                         Default: 0\n");
    printf(
      "  --snd-burst, -%c VAL    Set RX sound burst value to VAL frames.\n",
      OPT_SND_BURST);
    printf("                         Default: 1\n");
    ...
    printf(
      "  --jb-prefetch, -%c      Enable prefetch bufferring in jitter buffer\n",
      OPT_JB_PREFETCH);
    printf(
      "  --jb-min-pre, -%c MSEC  Jitter buffer minimum prefetch delay in "
      "msec\n",
      OPT_JB_MIN_PRE);
    printf(
      "  --jb-max-pre, -%c MSEC  Jitter buffer maximum prefetch delay in "
      "msec\n",
      OPT_JB_MAX_PRE);
    printf(
      "  --jb-max, -%c MSEC      Set maximum delay that can be accomodated by "
      "the\n",
      OPT_JB_MAX);
...
static int init_options(int argc, char* argv[])
{
    ...
    /* Parse options */
    pj_optind = 0;
    while ((c = pj_getopt_long(argc, argv, format, long_options,
                               &option_index)) != -1)
    {
        switch (c) {
        case OPT_CODEC:
        ...
        case OPT_LOSS_CORR:
            ...
            if (g_app.cfg.tx_pct_avg_lost > 100) {
                puts(
                  "Error: Loss correlation is in percentage, value is not "
                  "valid?");
                return 1;
            }
            break;
...
int main(int argc, char* argv[])
{
    ...
    PJ_LOG(3, (THIS_FILE, "  Codec=%.*s, tx_ptime=%d, rx_ptime=%d",
               (int)g_app.cfg.codec.slen, g_app.cfg.codec.ptr,
               g_app.cfg.tx_ptime, g_app.cfg.rx_ptime));
    PJ_LOG(3, (THIS_FILE, " Loss avg=%d%%, min_burst=%d, max_burst=%d",
               g_app.cfg.tx_pct_avg_lost, g_app.cfg.tx_min_lost_burst,
               g_app.cfg.tx_max_lost_burst));
    PJ_LOG(3, (THIS_FILE, " TX jitter min=%dms, max=%dms",
               g_app.cfg.tx_min_jitter, g_app.cfg.tx_max_jitter));
    PJ_LOG(3,
           (THIS_FILE, " RX jb init:%dms, min_pre=%dms, max_pre=%dms, max=%dms",
            g_app.cfg.rx_jb_init, g_app.cfg.rx_jb_min_pre,
            g_app.cfg.rx_jb_max_pre, g_app.cfg.rx_jb_max));
    PJ_LOG(3, (THIS_FILE, " RX sound burst:%d frames", g_app.cfg.rx_snd_burst));
    PJ_LOG(3,
           (THIS_FILE, " DTX=%d, PLC=%d", g_app.cfg.tx_dtx, g_app.cfg.rx_plc));
    ...
    PJ_LOG(3, (THIS_FILE, " TX packets=%u, dropped=%u/%5.1f%%",
               g_app.tx->state.tx.total_tx, g_app.tx->state.tx.total_lost,
               (float)(g_app.tx->state.tx.total_lost * 100.0 /
