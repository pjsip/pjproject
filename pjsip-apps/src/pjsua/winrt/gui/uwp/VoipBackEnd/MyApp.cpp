...
#define DEFAULT_DESTRUCTOR() delete inPtr;
...
#define CHECK_EXCEPTION(expr) \
    do { \
        try { \
            ...
            Platform::Exception ^ exp = \
              ref new Platform::COMException(GenHresultFromITF(e.status)); \
        } \
    } while (0)
...
std::string make_string(const std::wstring& wstring)
{
    ...
    int bufferSize =
      WideCharToMultiByte(CP_UTF8, 0, wideData, -1, nullptr, 0, NULL, NULL);

    std::unique_ptr<char[]> utf8;
    utf8.reset(new char[bufferSize]);

    if (WideCharToMultiByte(CP_UTF8, 0, wideData, -1, utf8.get(), bufferSize,
                            NULL, NULL) == 0)
    {
        return std::string();
    }
...
std::wstring make_wstring(const std::string& string)
{
    ...
    if (MultiByteToWideChar(CP_UTF8, 0, utf8Data, -1, wide.get(), bufferSize) ==
        0) {
        return std::wstring();
    }
...
Platform::String ^
  to_platform_str(const std::string& in_str) {
...
///////////////////////////////////////////////////////////////////////////////
ImpCall::ImpCall(pj::Account& account, int call_id)
...
void MyAppRT::init(IntAccount ^ iAcc, IntCall ^ iCall)
{
    ...
    log_cfg->decor =
      log_cfg->decor & ~(::pj_log_decoration::PJ_LOG_HAS_CR |
...
void MyAppRT::makeCall(Platform::String ^ dst_uri)
{
    ...
    CHECK_EXCEPTION(
      activeCall->makeCall(to_std_str(dst_uri), *param->getPtr()));
};
...
{
    // Some threads are registered using PJLIB API, ep.libIsThreadRegistered()
    // will return false on those threads.
...
void MyAppRT::modifyAccount(Platform::String ^ id, Platform::String ^ registrar,
                            ...
                            Platform::String ^ password)
{
    if (id->IsEmpty()) {
        ...
        pj::AuthCredInfo info = pj::AuthCredInfo(
          "Digest", "*", to_std_str(username), 0, to_std_str(password));
