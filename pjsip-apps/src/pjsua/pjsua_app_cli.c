...
PJ_DEF(void) cli_get_info(char* info, pj_size_t size)
{
    ...
    pj_ansi_snprintf(info, size, "Telnet to %.*s:%d",
                     ...
                     telnet_info.ip_address.ptr, telnet_info.port);
}
...
pj_status_t cli_init(void)
{
    ...
    if (app_config.cli_cfg.cli_fe & CLI_FE_CONSOLE) {
        ...
        status = pj_cli_console_create(cli, fe_cfg, &cli_cons_sess, NULL);
        if (status != PJ_SUCCESS)
            goto on_error;
    }
...
/* Get input URL */
static void get_input_url(char* buf, pj_size_t len, pj_cli_cmd_val* cval,
    ...
    if (param->cnt < param->max_cnt) {
        ...
        if (count > 0) {
            for (i = 0; i < (int)count; ++i) {
                ...
                pj_ansi_snprintf(data_out, sizeof(data_out), "<%.*s>  %.*s",
                                 (int)info.status_text.slen,
                                 info.status_text.ptr, (int)info.uri.slen,
                                 info.uri.ptr);
...
static void get_account_id(pj_cli_dyn_choice_param* param)
{
    if (param->cnt < param->max_cnt) {
        ...
        for (i = 0; i < (int)count; ++i) {
            ...
            pj_ansi_snprintf(buf_out, sizeof(buf_out), "%c%.*s",
...
static void get_media_port(pj_cli_dyn_choice_param* param)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        pj_ansi_snprintf(slot_id, sizeof(slot_id), "%d", info.slot_id);
        ...
        len = pj_ansi_snprintf(
          desc, sizeof(desc), "[%2dKHz/%dms/%d] %20.*s  transmitting to: %s",
          info.clock_rate / 1000,
          info.samples_per_frame * 1000 / info.channel_count / info.clock_rate,
          info.channel_count, (int)info.name.slen, info.name.ptr, txlist);
...
static void get_audio_codec_id(pj_cli_dyn_choice_param* param)
{
    if (param->cnt < param->max_cnt) {
        ...
        for (i = 0; i <= count; ++i) {
            ...
            pj_ansi_snprintf(codec_id, sizeof(codec_id), "%.*s", (int)cid.slen,
                             cid.ptr);

            if (i < count) {
                pj_ansi_snprintf(desc, sizeof(desc), "Audio, prio: %d%s%.*s",
                                 c[i].priority, c[i].desc.slen ? " - " : "",
...
static void get_video_dev_hint(pj_cli_dyn_choice_param* param,
                               pjmedia_vid_dev_info* vdi, unsigned vid_dev_id)
{
    char desc[128];
    char dev_id[8];
    pj_ansi_snprintf(dev_id, sizeof(dev_id), "%d", vid_dev_id);
    pj_ansi_snprintf(desc, sizeof(desc), "%s [%s]", vdi->name, vdi->driver);

    pj_strdup2(param->pool, &param->choice[param->cnt].value, dev_id);
    pj_strdup2(param->pool, &param->choice[param->cnt].desc, desc);
}

static void get_video_dev_id(pj_cli_dyn_choice_param* param, pj_bool_t all,
                             pj_bool_t capture)
{
    if (param->cnt < param->max_cnt) {
        unsigned i, count;
        ...
        for (i = 0; i < count; ++i) {
            ...
            if (status == PJ_SUCCESS) {
                if ((all) || ((capture) && (vdi.dir == PJMEDIA_DIR_CAPTURE)) ||
    ...
    if (param->cnt < param->max_cnt) {
        ...
        for (i = 0; i <= count; ++i) {
            ...
            pj_ansi_snprintf(codec_id, sizeof(codec_id), "%.*s",
                             (int)cur_ci.slen, cur_ci.ptr);

            if (i < count) {
                pj_ansi_snprintf(
                  desc, sizeof(desc),
                  "Video, p[%d], f[%.2f], b[%d/%d], s[%dx%d]", ci[i].priority,
                  (vfd->fps.num * 1.0 / vfd->fps.denum), vfd->avg_bps / 1000,
                  vfd->max_bps / 1000, vfd->size.w, vfd->size.h);
            } else {
...
static void get_video_window_id(pj_cli_dyn_choice_param* param)
{
    if (param->cnt < param->max_cnt) {
        ...
        for (i = 0; i < cnt; ++i) {
            ...
            pj_ansi_snprintf(
              desc, sizeof(desc), "Show:%c Pos(%d,%d)  Size(%dx%d)",
              (wi.show ? 'Y' : 'N'), wi.pos.x, wi.pos.y, wi.size.w, wi.size.h);
...
static void get_choice_value(pj_cli_dyn_choice_param* param)
{
    ...
    case DYN_CHOICE_VID_DEV_ID:
        get_video_dev_id(param, (param->arg_id == DYN_CHOICE_VID_DEV_ID),
...
static pj_status_t cmd_del_account(pj_cli_cmd_val* cval)
{
    ...
    if (!pjsua_acc_is_valid(i)) {
        pj_ansi_snprintf(out_str, sizeof(out_str), "Invalid account id %d\n",
                         i);
        str_len = (unsigned)pj_ansi_strlen(out_str);
        pj_cli_sess_write_msg(cval->sess, out_str, str_len);
    } else {
        pjsua_acc_del(i);
        pj_ansi_snprintf(out_str, sizeof(out_str), "Account %d deleted\n", i);
        str_len = (unsigned)pj_ansi_strlen(out_str);
        pj_cli_sess_write_msg(cval->sess, out_str, str_len);
    }
    return PJ_SUCCESS;
...
static pj_status_t cmd_show_account(pj_cli_cmd_val* cval)
{
    ...
    for (i = 0; i < (int)count; ++i) {
        ...
        if (!info.has_registration) {
            ...
            pj_ansi_snprintf(acc_info, sizeof(acc_info), "%d/%.*s (expires=%d)",
                             info.status, (int)info.status_text.slen,
                             info.status_text.ptr, info.expires);
        }
        ...
        pj_ansi_snprintf(
          out_str, sizeof(out_str), "       Online status: %.*s\n",
...
static pj_status_t cmd_add_buddy(pj_cli_cmd_val* cval)
{
    ...
    if (pjsua_verify_url(cval->argv[1].ptr) != PJ_SUCCESS) {
        pj_ansi_snprintf(out_str, sizeof(out_str), "Invalid URI '%s'\n",
                         cval->argv[1].ptr);
    } else {
...
static pj_status_t cmd_del_buddy(pj_cli_cmd_val* cval)
{
    ...
    if (!pjsua_buddy_is_valid(i)) {
        pj_ansi_snprintf(out_str, sizeof(out_str), "Invalid buddy id %d\n", i);
    } else {
        pjsua_buddy_del(i);
        pj_ansi_snprintf(out_str, sizeof(out_str), "Buddy %d deleted\n", i);
    }
    pj_cli_sess_write_msg(cval->sess, out_str, pj_ansi_strlen(out_str));
    return PJ_SUCCESS;
}
...
static pj_status_t cmd_show_buddy(pj_cli_cmd_val* cval)
{
    ...
    if (count == 0) {
        ...
        for (i = 0; i < (int)count; ++i) {
            ...
            pj_ansi_snprintf(out_str, sizeof(out_str), " [%2d] <%.*s>  %.*s\n",
                             ids[i] + 1, (int)info.status_text.slen,
                             info.status_text.ptr, (int)info.uri.slen,
                             info.uri.ptr);
...
pj_status_t cmd_presence_handler(pj_cli_cmd_val* cval)
{
    ...
    case CMD_PRESENCE_UNSUB:
        status =
          cmd_subs_pres(cval, pj_cli_get_cmd_id(cval->cmd) == CMD_PRESENCE_SUB);
        break;
...
static pj_status_t cmd_set_codec_prio(pj_cli_cmd_val* cval)
{
    ...
    status = pjsua_codec_set_priority(&cval->argv[1], (pj_uint8_t)new_prio);
#if PJSUA_HAS_VIDEO
    if (status != PJ_SUCCESS) {
        status =
          pjsua_vid_codec_set_priority(&cval->argv[1], (pj_uint8_t)new_prio);
    }
#endif
    if (status != PJ_SUCCESS)
...
pj_status_t cmd_media_handler(pj_cli_cmd_val* cval)
{
    ...
    case CMD_MEDIA_CONF_DISCONNECT:
        status = cmd_media_connect(
          cval, pj_cli_get_cmd_id(cval->cmd) == CMD_MEDIA_CONF_CONNECT);
        break;
    ...
    case CMD_MEDIA_SPEAKER_TOGGLE: {
        ...
        status =
          pjsua_snd_get_setting(PJMEDIA_AUD_DEV_CAP_OUTPUT_ROUTE, &route);
        if (status != PJ_SUCCESS) {
        ...
        PJ_LOG(4, (THIS_FILE, "Setting output route to %s %s",
                   (route == PJMEDIA_AUD_DEV_ROUTE_DEFAULT ? "default" :
                                                             "loudspeaker"),
                   (status ? "anyway" : "")));

        status = pjsua_snd_set_setting(PJMEDIA_AUD_DEV_CAP_OUTPUT_ROUTE, &route,
                                       PJ_TRUE);
...
static pj_status_t cmd_make_single_call(pj_cli_cmd_val* cval)
{
    ...
    pjsua_call_make_call(current_acc, &tmp, &call_opt, NULL, &msg_data,
                         &current_call);
    return PJ_SUCCESS;
}
...
static pj_status_t cmd_make_multi_call(pj_cli_cmd_val* cval)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        status =
          pjsua_call_make_call(current_acc, &tmp, &call_opt, NULL, NULL, NULL);
        if (status != PJ_SUCCESS)
            break;
    }
    return PJ_SUCCESS;
...
static pj_status_t cmd_answer_call(pj_cli_cmd_val* cval)
{
    ...
    if (current_call == PJSUA_INVALID_ID || call_info.role != PJSIP_ROLE_UAS ||
...
static pj_status_t cmd_transfer_replace_call(pj_cli_cmd_val* cval)
{
    if (current_call == -1) {
        ...
        /* Check if call is still there. */
        if (call != current_call) {
            static pj_str_t err_call_dc = { "Call has been disconnected\n",
                                            27 };
        ...
        pjsua_call_xfer_replaces(call, dst_call, PJSUA_XFER_NO_REQUIRE_REPLACES,
                                 &msg_data_);
    }
    return PJ_SUCCESS;
}
...
static pj_status_t cmd_dtmf_2833(pj_cli_cmd_val* cval)
{
    if (current_call == PJSUA_INVALID_ID) {
        ...
        if (status != PJ_SUCCESS) {
            ...
            static const pj_str_t msg = {
                "DTMF digits enqueued "
                "for transmission\n",
                39
            };
...
static pj_status_t cmd_call_info(pj_cli_cmd_val* cval)
{
    if (current_call == PJSUA_INVALID_ID) {
        ...
        for (i = 0; i < cval->argv[1].slen; ++i) {
            ...
            status =
              pjsua_call_send_request(current_call, &SIP_INFO, &msg_data);
            if (status != PJ_SUCCESS) {
                break;
            }
        }
...
static pj_status_t cmd_send_arbitrary(pj_cli_cmd_val* cval)
{
    if (pjsua_acc_get_count() == 0) {
        ...
        pj_str_t tmp = pj_str(dest);
        struct input_result result;
        static const pj_str_t header = {
            "Send arbitrary request to "
            "remote host\n",
            39
        };
...
static pj_status_t cmd_show_current_call(pj_cli_cmd_val* cval)
{
    ...
    pj_ansi_snprintf(out_str, sizeof(out_str), "You have %d active call%s\n", i,
                     (i > 1 ? "s" : ""));
    ...
    if (current_call != PJSUA_INVALID_ID) {
        ...
        if (pjsua_call_get_info(current_call, &ci) == PJ_SUCCESS) {
            pj_ansi_snprintf(
              out_str, sizeof(out_str), "Current call id=%d to %.*s [%.*s]\n",
              current_call, (int)ci.remote_info.slen, ci.remote_info.ptr,
...
static pj_status_t modify_video_account(pjsua_acc_config* acc_cfg)
{
    ...
    if (status != PJ_SUCCESS)
        PJ_PERROR(
          1, (THIS_FILE, status, "Error modifying account %d", current_acc));
...
{
    return pjsua_call_set_vid_strm(current_call, PJSUA_CALL_VID_STRM_ADD, NULL);
}
...
static pj_status_t cmd_enable_vid_rx(pj_cli_cmd_val* cval)
{
    ...
    status = pjsua_call_set_vid_strm(current_call,
                                     PJSUA_CALL_VID_STRM_CHANGE_DIR, &param);
    return status;
}
...
static pj_status_t cmd_enable_vid_stream(pj_cli_cmd_val* cval, pj_bool_t enable)
{
    pjsua_call_vid_strm_op_param param;
    pjsua_call_vid_strm_op op =
      enable ? PJSUA_CALL_VID_STRM_CHANGE_DIR : PJSUA_CALL_VID_STRM_REMOVE;

    pjsua_call_vid_strm_op_param_default(&param);

    param.med_idx = cval->argc > 1 ? (int)pj_strtol(&cval->argv[1]) : -1;
...
static pj_status_t cmd_set_cap_dev_id(pj_cli_cmd_val* cval)
{
    ...
    pjsua_call_vid_strm_op_param_default(&param);
    param.med_idx = cval->argc > 1 ? (int)pj_strtol(&cval->argv[1]) : -1;
    param.cap_dev = cval->argc > 2 ? (int)pj_strtol(&cval->argv[2]) :
    ...
    if (status != PJ_SUCCESS) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            PJ_LOG(3, (THIS_FILE, "%.*s%.*s %3d %7.2f  %4d/%4d  %dx%d",
                       ...
                       ci[i].priority, (vfd->fps.num * 1.0 / vfd->fps.denum),
                       vfd->avg_bps / 1000, vfd->max_bps / 1000, vfd->size.w,
                       vfd->size.h));
        }
    }
    return PJ_SUCCESS;
...
static pj_status_t cmd_vid_win_list()
{
    ...
    for (i = 0; i < cnt; ++i) {
        ...
        pjsua_vid_win_get_info(wids[i], &wi);
        PJ_LOG(
          3, (THIS_FILE, "%3d   %c  (%d,%d)  %dx%d", wids[i],
              (wi.show ? 'Y' : 'N'), wi.pos.x, wi.pos.y, wi.size.w, wi.size.h));
    }
    return PJ_SUCCESS;
}
...
static pj_status_t cmd_vid_conf_list()
{
    ...
    if (status != PJ_SUCCESS) {
        PJ_PERROR(
          1, (THIS_FILE, status, "Failed enumerating video conf bridge ports"));
        return status;
    }

    PJ_LOG(3, (THIS_FILE, " Video conference has %d ports:\n", count));
    PJ_LOG(3, (THIS_FILE,
               " id name                   format               rx-from      "
               "tx-to \n"));
    PJ_LOG(3, (THIS_FILE,
               " --------------------------------------------------------------"
               "----\n"));
    for (i = 0; i < count; ++i) {
        ...
        s[4] = ' ';
        pj_ansi_snprintf(s + 5, sizeof(s) - 5, "%dx%d@%.1f", size->w, size->h,
                         (float)(fps->num * 1.0 / fps->denum));
        PJ_LOG(3, (THIS_FILE, "%3d %.*s%.*s %s%.*s %s%.*s %s\n", id[i],
                   (int)info.name.slen, info.name.ptr, 22 - (int)info.name.slen,
                   "                   ", s, 20 - pj_ansi_strlen(s),
                   "                    ", tr_list,
                   12 - pj_ansi_strlen(tr_list), "            ", li_list));
    }
    return PJ_SUCCESS;
}
...
static pj_status_t add_call_command(pj_cli_t* c)
{
    ...
    return pj_cli_add_cmd_from_xml(c, NULL, &xml, cmd_call_handler, NULL,
                                   get_choice_value);
}
...
static pj_status_t add_presence_command(pj_cli_t* c)
{
    ...
    return pj_cli_add_cmd_from_xml(c, NULL, &xml, cmd_presence_handler, NULL,
                                   get_choice_value);
}
...
static pj_status_t add_account_command(pj_cli_t* c)
{
    ...
    return pj_cli_add_cmd_from_xml(c, NULL, &xml, cmd_account_handler, NULL,
                                   get_choice_value);
}
...
static pj_status_t add_media_command(pj_cli_t* c)
{
    ...
    return pj_cli_add_cmd_from_xml(c, NULL, &xml, cmd_media_handler, NULL,
                                   get_choice_value);
}
...
static pj_status_t add_config_command(pj_cli_t* c)
{
    ...
    return pj_cli_add_cmd_from_xml(c, NULL, &xml, cmd_config_handler, NULL,
                                   get_choice_value);
}
...
static pj_status_t add_video_command(pj_cli_t* c)
{
    char* video_command =
      ...
      "    <CMD name='list' id='60081' desc='List all ports in video "
      "conference'/>"
      ...
      "    </CMD>"
      "    <CMD name='cd' id='60083' desc='Disconnect ports in video "
      "conference'>"
      "      <ARG name='source' type='int' desc='Source port ID'/>"
      "      <ARG name='sink' type='int' desc='Sink port ID'/>"
      "    </CMD>"
    ...
    return pj_cli_add_cmd_from_xml(c, NULL, &xml, cmd_video_handler, NULL,
                                   get_choice_value);
}
#endif
...
static pj_status_t add_other_command(pj_cli_t* c)
{
    ...
    status = pj_cli_add_cmd_from_xml(c, NULL, &sleep_xml, cmd_sleep_handler,
                                     NULL, NULL);
    if (status != PJ_SUCCESS)
        return status;

    status = pj_cli_add_cmd_from_xml(c, NULL, &network_xml, cmd_network_handler,
                                     NULL, NULL);
    if (status != PJ_SUCCESS)
        return status;

    status = pj_cli_add_cmd_from_xml(c, NULL, &shutdown_xml, cmd_quit_handler,
                                     NULL, NULL);

    if (status != PJ_SUCCESS)
        return status;

    status = pj_cli_add_cmd_from_xml(c, NULL, &restart_xml, cmd_restart_handler,
                                     NULL, NULL);

    if (status != PJ_SUCCESS)
        return status;

    status = pj_cli_add_cmd_from_xml(c, NULL, &ip_change_xml,
                                     cmd_ip_change_handler, NULL, NULL);
