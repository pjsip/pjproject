...
static void print_acc_status(int acc_id)
{
    ...
    if (!info.has_registration) {
        pj_ansi_snprintf(buf, sizeof(buf), "%.*s", (int)info.status_text.slen,
                         info.status_text.ptr);

    } else {
        pj_ansi_snprintf(buf, sizeof(buf), "%d/%.*s (expires=%d)", info.status,
                         (int)info.status_text.slen, info.status_text.ptr,
                         info.expires);
    }

    printf(" %c[%2d] %.*s: %s\n", (acc_id == current_acc ? '*' : ' '), acc_id,
           (int)info.acc_uri.slen, info.acc_uri.ptr, buf);
...
static void vid_show_help()
{
    ...
    puts(
      ...
      "      |");
    puts(
      "| vid enable|disable        Enable or disable video in next "
      "offer/answer      |");
    puts(
      "| vid acc show              Show current account video settings         "
      "      |");
    puts(
      "| vid acc autorx on|off     Automatically show incoming video on/off    "
      "      |");
    puts(
      "| vid acc autotx on|off     Automatically offer video on/off            "
      "      |");
    puts(
      "| vid acc cap ID            Set default capture device for current acc  "
      "      |");
    puts(
      "| vid acc rend ID           Set default renderer device for current acc "
      "      |");
    puts(
      "| vid call rx on|off N      Enable/disable video RX for stream N in "
      "curr call |");
    puts(
      "| vid call tx on|off N      Enable/disable video TX for stream N in "
      "curr call |");
    puts(
      "| vid call add              Add video stream for current call           "
      "      |");
    puts(
      "| vid call enable|disable N Enable/disable stream #N in current call    "
      "      |");
    puts(
      "| vid call cap N ID         Set capture dev ID for stream #N in current "
      "call  |");
    puts(
      "| vid dev list              List all video devices                      "
      "      |");
    puts(
      "| vid dev refresh           Refresh video device list                   "
      "      |");
    puts(
      "| vid dev prev on|off ID    Enable/disable preview for specified device "
      "ID    |");
    puts(
      "| vid codec list            List video codecs                           "
      "      |");
    puts(
      "| vid codec prio ID PRIO    Set codec ID priority to PRIO               "
      "      |");
    puts(
      "| vid codec fps ID NUM DEN  Set codec ID framerate to (NUM/DEN) fps     "
      "      |");
    puts(
      "| vid codec bw ID AVG MAX   Set codec ID bitrate to AVG & MAX kbps      "
      "      |");
    puts(
      "| vid codec size ID W H     Set codec ID size/resolution to W x H       "
      "      |");
    puts(
      "| vid win list              List all active video windows               "
      "      |");
    puts(
      "| vid win arrange           Auto arrange windows                        "
      "      |");
    puts(
      "| vid win show|hide ID      Show/hide the specified video window ID     "
      "      |");
    puts(
      "| vid win move ID X Y       Move window ID to position X,Y              "
      "      |");
    puts(
      "| vid win resize ID w h     Resize window ID to the specified width, "
      "height   |");
    puts(
      "| vid win full off|on|dt ID Set fullscreen off/on/desktop for window "
      "ID	|");
    puts(
      "| vid conf list             List all video ports in video conference "
      "bridge   |");
    puts(
      "| vid conf cc P Q           Connect port P to Q in the video conf "
      "bridge      |");
    puts(
      "| vid conf cd P Q           Disconnect port P to Q in the video conf "
      "bridge   |");
    puts(
      "+======================================================================="
      "======+");
    printf(
      "| Video will be %s in the next offer/answer %s                          "
      "  |\n",
                             ...
                             strcmp(argv[1], "disable") == 0))
    {
        ...
        if (argc == 5 && strcmp(argv[2], "rx") == 0) {
            ...
            status = pjsua_call_set_vid_strm(
              current_call, PJSUA_CALL_VID_STRM_CHANGE_DIR, &param);
        } else if (argc == 5 && strcmp(argv[2], "tx") == 0) {
            pj_bool_t on = (strcmp(argv[3], "on") == 0);
            pjsua_call_vid_strm_op op = on ?
                                 ...
                                 strcmp(argv[2], "enable") == 0))
        {
            ...
            pjsua_call_vid_strm_op op = enable ?
            ...
            param.med_idx = argc >= 4 ? atoi(argv[3]) : -1;
            param.cap_dev =
              argc >= 5 ? atoi(argv[4]) : PJMEDIA_VID_DEFAULT_CAPTURE_DEV;
            status = pjsua_call_set_vid_strm(
              current_call, PJSUA_CALL_VID_STRM_CHANGE_CAP_DEV, &param);
        } else
            goto on_error;
        ...
        if (argc == 3 && strcmp(argv[2], "list") == 0) {
            ...
            for (i = 0; i < cnt; ++i) {
                ...
                PJ_LOG(3, (THIS_FILE, "%3d   %c  (%d,%d)  %dx%d", wids[i],
                           (wi.show ? 'Y' : 'N'), wi.pos.x, wi.pos.y, wi.size.w,
                           wi.size.h));
            }
        ...
        if (argc == 3 && strcmp(argv[2], "list") == 0) {
            ...
            if (status != PJ_SUCCESS) {
                ...
                PJ_LOG(
                  3, (THIS_FILE, "codec id      prio  fps    bw(kbps)   size"));
                PJ_LOG(
                  3, (THIS_FILE, "------------------------------------------"));
                for (i = 0; i < count; ++i) {
                    ...
                    vfd = pjmedia_format_get_video_format_detail(&cp.enc_fmt,
                                                                 PJ_TRUE);
                    PJ_LOG(
                      3, (THIS_FILE, "%.*s%.*s %3d %7.2f  %4d/%4d  %dx%d",
            ...
            if (status != PJ_SUCCESS) {
                ...
                for (i = 0; i < count; ++i) {
                    ...
                    for (j = 0; j < info.listener_cnt; ++j) {
                        char str_info[10];
                        pj_ansi_snprintf(
                          str_info, sizeof(str_info), "%d%s", info.listeners[j],
                    ...
                    printf("%3d %.*s%.*s %s%.*s %s%.*s %s\n", id[i],
                           ...
                           22 - (int)info.name.slen, "                   ", s,
                           20 - (int)pj_ansi_strlen(s), "                    ",
                           tr_list, 12 - (int)pj_ansi_strlen(tr_list),
                           "            ", li_list);
                }
            }
...
static void ui_make_new_call()
{
    ...
    pjsua_call_make_call(current_acc, &tmp, &call_opt, NULL, &msg_data_,
                         &current_call);
}
...
static void ui_answer_call()
{
    ...
    if (current_call == -1 || call_info.role != PJSIP_ROLE_UAS ||
        call_info.state >= PJSIP_INV_STATE_CONNECTING)
    {
        ...
        if (st_code / 100 == 3) {
            if (!simple_input("Enter URL to be put in Contact", contact,
                              sizeof(contact)))
                return;
...
static void ui_manage_codec_prio()
{
    ...
    for (i = 0; i < count; ++i) {
        printf("  %d\t%.*s%s%.*s\n", c[i].priority, (int)c[i].codec_id.slen,
               c[i].codec_id.ptr, c[i].desc.slen ? " - " : "",
    ...
    puts("");
    puts(
      "Enter codec id and its new priority (e.g. \"speex/16000 200\", "
      ""
      "\"H263 200\"),");
    ...
    status =
      pjsua_codec_set_priority(pj_cstr(&id, codec), (pj_uint8_t)new_prio);
#if PJSUA_HAS_VIDEO
    if (status != PJ_SUCCESS) {
...
static void ui_call_transfer_replaces(pj_bool_t no_refersub)
{
    if (current_call == -1) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            printf("%d  %.*s [%.*s]\n", ids[i], (int)call_info.remote_info.slen,
                   call_info.remote_info.ptr, (int)call_info.state_text.slen,
        ...
        if (dst_call == call) {
            puts(
              "Destination call number must not be the same "
...
static void ui_send_dtmf_2833()
{
    if (current_call == -1) {
        ...
        if (!simple_input("DTMF strings to send (0-9*R#A-B)", buf, sizeof(buf)))
#else
        if (!simple_input("DTMF strings to send (0-9*#A-B)", buf, sizeof(buf)))
#endif
        {
            return;
        }
...
static void ui_subscribe(char menuin[])
{
    ...
    if (result.nb_result != PJSUA_APP_NO_NB) {
        if (result.nb_result == -1) {
            ...
            puts(
              "Sorry, can only subscribe to buddy's presence, "
              "not from existing call");
        } else {
            pjsua_buddy_subscribe_pres(result.nb_result - 1,
                                       (menuin[0] == 's'));
        }
...
static void ui_change_online_status()
{
    ...
    struct opt
    {
        int id;
        char* name;
    } opts[] = { { AVAILABLE, "Available" }, { BUSY, "Busy" },
                 { OTP, "On the phone" },    { IDLE, "Idle" },
                 { AWAY, "Away" },           { BRB, "Be right back" },
                 { OFFLINE, "Offline" } };
...
static void ui_conf_connect(char menuin[])
{
    ...
    if (cnt != 3) {
        ...
        src_title =
          (menuin[1] == 'c' ? "Connect src port #" : "Disconnect src port #");
