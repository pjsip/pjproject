...
static void usage(void)
{
    ...
    puts(
      "  --log-level=N       Set log max level to N (0(none) to 6(trace)) "
      "(default=5)");
    puts(
    ...
    puts(
      "  --light-bg          Use dark colors for light background (default is "
      "dark bg)");
    ...
    puts(
      "  --contact-params=S  Append the specified parameters S in Contact "
      "header");
    puts(
      "  --contact-uri-params=S  Append the specified parameters S in Contact "
      "URI");
    ...
    printf(
      ...
      PJSUA_REG_INTERVAL);
    printf(
      "  --rereg-delay=SEC   Optional auto retry registration interval "
      "(default %d)\n",
      PJSUA_REG_RETRY_INTERVAL);
    puts(
      "  --reg-use-proxy=N   Control the use of proxy settings in REGISTER.");
    puts(
      "                      0=no proxy, 1=outbound only, 2=acc only, 3=all "
      "(default)");
    ...
    puts(
      "  --srtp-secure=N     SRTP require secure SIP? 0:no, 1:tls, 2:sips "
      "(def:1)");
#endif
    puts(
    ...
    printf(
      "  --timer-se=N        Session timers expiration period, in secs "
      "(def:%d)\n",
      PJSIP_SESS_TIMER_DEF_SE);
    puts(
      "  --timer-min-se=N    Session timers minimum expiration period, in secs "
      "(def:90)");
    puts("  --outb-rid=string   Set SIP outbound reg-id (default:1)");
    puts(
      "  --auto-update-nat=N Where N is 0 or 1 to enable/disable SIP traversal "
      "behind");
    ...
    puts(
      "                      TCP and UDP transports on the specified port, "
      "unless");
    puts("                      if TCP or UDP is disabled.");
    puts(
      "  --ip-addr=IP        Use the specifed address as SIP and RTP "
      "addresses.");
    puts(
      "                      (Hint: the IP may be the public IP of the "
      "NAT/router)");
    ...
    puts(
      "  --nameserver=NS     Add the specified nameserver to enable SRV "
      "resolution");
    puts("                      This option can be specified multiple times.");
    puts("  --outbound=url      Set the URL of global outbound proxy server");
    puts("                      May be specified multiple times");
    puts(
      "  --stun-srv=FORMAT   Set STUN server host or domain. This option may "
      "be");
    puts(
      "                      specified more than once. FORMAT is "
      "hostdom[:PORT]");
    puts(
      "  --upnp=if_name      Enable UPnP using the interface name. If "
      "interface is");
    puts(
      "                      not specified, the first interface found will be "
      "used");
    ...
    puts(
      "  --tls-password      Specify TLS password to private key file "
      "(default=none)");
    ...
    puts(
      "  --stereo            Audio device and conference bridge opened in "
      "stereo mode");
    ...
    puts("                      This can be specified multiple times.");
    puts(
      "  --auto-play         Automatically play the file (to incoming calls "
      "only)");
    puts(
      "  --auto-play-hangup  Automatically hangup the file after file play "
      "completes");
    puts(
      "  --auto-loop         Automatically loop incoming RTP to outgoing RTP");
    puts(
      "  --auto-conf         Automatically put calls in conference with "
      "others");
    puts(
      "  --rec-file=file     Open file recorder (extension can be .wav or "
      ".mp3");
    puts("  --auto-rec          Automatically record conversation");
    puts("  --quality=N         Specify media quality (0-10, default=" xstr(
      PJSUA_DEFAULT_CODEC_QUALITY) ")");
    puts(
      "  --ptime=MSEC        Override codec ptime to MSEC (default=codec "
      "specific)");
    puts(
      "  --no-vad            Disable VAD/silence detector (default=codec "
      "specific)");
    puts("  --ec-tail=MSEC      Set echo canceller tail length (default=" xstr(
      PJSUA_DEFAULT_EC_TAIL_LEN) ")");
    puts("  --ec-opt=OPT        Select echo canceller algorithm (0=default, ");
    puts(
      "                        1=speex, 2=suppressor, 3=WebRtc, 4=WebRtc "
      "AEC3)");
    puts(
      "  --ilbc-mode=MODE    Set iLBC codec mode (20 or 30, default is " xstr(
    ...
    puts("  --capture-lat=N     Audio capture latency, in ms (default=" xstr(
      PJMEDIA_SND_DEFAULT_REC_LATENCY) ")");
    puts("  --playback-lat=N    Audio playback latency, in ms (default=" xstr(
      PJMEDIA_SND_DEFAULT_PLAY_LATENCY) ")");
    puts(
      "  --snd-auto-close=N  Auto close audio device when idle for N secs "
      "(default=1)");
    ...
    puts(
      "  --jb-max-size       Specify jitter buffer maximum size, in frames "
      "(default=-1)");
    ...
    puts(
      "  --ice-trickle=N     Use trickle ICE? 0:disabled, 1:half, 2:full "
      "(default=0)");
    ...
    puts(
      "  --rx-drop-pct=PCT   Drop PCT percent of RX RTP (for pkt lost sim, "
      "default: 0)");
    puts(
      "  --tx-drop-pct=PCT   Drop PCT percent of TX RTP (for pkt lost sim, "
      "default: 0)");
    ...
    puts(
      "  --turn-tls-cert-file  Specify TURN TLS certificate file "
      "(default=none)");
    puts(
      "  --turn-tls-privkey-file  Specify TURN TLS private key file "
      "(default=none)");
    puts(
      "  --turn-tls-privkey-pwd Specify TURN TLS password to private key file "
      "(default=none)");
    puts(
      "  --turn-tls-neg-timeout Specify TURN TLS negotiation timeout "
      "(default=no)");
    ...
    puts(
      "  --auto-answer=code  Automatically answer incoming calls with code "
      "(e.g. 200)");
    puts(
      "  --max-calls=N       Maximum number of concurrent calls (default:4, "
      "max:255)");
    puts("  --thread-cnt=N      Number of worker threads (default:1)");
    puts("  --duration=SEC      Set maximum call duration (default:no limit)");
    puts(
      "  --norefersub        Suppress event subscription when transferring "
      "calls");
    puts("  --use-compact-form  Minimize SIP message size");
    puts(
      "  --no-force-lr       Allow strict-route to be used (i.e. do not force "
      "lr)");
    puts(
      "  --accept-redirect=N Specify how to handle call redirect (3xx) "
      "response.");
    ...
    puts("");
    puts(
      "When URL is specified, pjsua will immediately initiate call to that "
      "URL");
    puts("");
...
static pj_status_t parse_args(int argc, char* argv[], pj_str_t* uri_to_call)
{
    ...
    enum
    {
        ...
        OPT_TURN_PASSWD,
        OPT_TURN_TLS,
        OPT_TURN_TLS_CA_FILE,
        OPT_TURN_TLS_CERT_FILE,
        OPT_TURN_TLS_NEG_TIMEOUT,
        OPT_TURN_TLS_CIPHER,
        OPT_TURN_TLS_PRIV_FILE,
        OPT_TURN_TLS_PASSWORD,
        OPT_RTCP_MUX,
        OPT_SRTP_KEYING,
        OPT_PLAY_FILE,
        OPT_PLAY_TONE,
    ...
    while ((c = pj_getopt_long(argc, argv, "", long_options, &option_index)) !=
        ...
        case OPT_APP_LOG_LEVEL:
            cfg->log_cfg.console_level =
              (unsigned)pj_strtoul(pj_cstr(&tmp, pj_optarg));
        ...
        case OPT_NO_UDP: /* no-udp */
            if (cfg->no_tcp && !cfg->use_tls) {
                PJ_LOG(1,
                       (THIS_FILE, "Error: cannot disable both TCP and UDP"));
                return PJ_EINVAL;
            }
        ...
        case OPT_AUTO_UPDATE_NAT: /* OPT_AUTO_UPDATE_NAT */
            cur_acc->allow_contact_rewrite =
              (pj_bool_t)pj_strtoul(pj_cstr(&tmp, pj_optarg));
            break;
        ...
        case OPT_ACCEPT_REDIRECT:
            ...
            if (cfg->redir_op > PJSIP_REDIRECT_STOP) {
                PJ_LOG(1, (THIS_FILE, "Error: accept-redirect value '%s' ",
                           pj_optarg));
                return PJ_EINVAL;
            }
            break;
        ...
        case OPT_USERNAME: /* Default authentication user */
            cur_acc->cred_info[cur_acc->cred_count].username =
              pj_str(pj_optarg);
        ...
        case OPT_PASSWORD: /* authentication password */
            ...
            cur_acc->cred_info[cur_acc->cred_count].ext.aka.k =
              pj_str(pj_optarg);
            cur_acc->cred_info[cur_acc->cred_count].ext.aka.cb =
              &pjsip_auth_create_aka_response;
#endif
            break;

        case OPT_REG_RETRY_INTERVAL:
            cur_acc->reg_retry_interval =
              (unsigned)pj_strtoul(pj_cstr(&tmp, pj_optarg));
            break;

        case OPT_REG_USE_PROXY:
            cur_acc->reg_use_proxy =
              (unsigned)pj_strtoul(pj_cstr(&tmp, pj_optarg));
            if (cur_acc->reg_use_proxy > 3) {
                PJ_LOG(1,
                       (THIS_FILE, "Error: invalid --reg-use-proxy value '%s'",
                        pj_optarg));
                return PJ_EINVAL;
            }
            break;
        ...
        case OPT_NAMESERVER: /* nameserver */
            cfg->cfg.nameserver[cfg->cfg.nameserver_count++] =
              pj_str(pj_optarg);
        ...
        case OPT_ICE_TRICKLE:
            cfg->media_cfg.ice_opt.trickle = cur_acc->ice_cfg.ice_opt.trickle =
              my_atoi(pj_optarg);

            if (!pj_isdigit(*pj_optarg) || cfg->media_cfg.ice_opt.trickle > 2) {
                PJ_LOG(1,
                       (THIS_FILE, "Invalid value for --ice-trickle option"));
                return -1;
            }
        ...
        case OPT_TURN_SRV:
            cfg->media_cfg.turn_server = cur_acc->turn_cfg.turn_server =
              pj_str(pj_optarg);
            break;
        ...
        case OPT_TURN_TLS_CERT_FILE:
            cfg->media_cfg.turn_tls_setting.cert_file =
              cur_acc->turn_cfg.turn_tls_setting.cert_file = pj_str(pj_optarg);
            break;
        ...
        case OPT_TURN_TLS_PASSWORD:
            cfg->media_cfg.turn_tls_setting.password =
              cur_acc->turn_cfg.turn_tls_setting.password = pj_str(pj_optarg);
            break;
        ...
        case OPT_EC_TAIL:
            ...
            if (cfg->media_cfg.ec_tail_len > 1000) {
                PJ_LOG(1, (THIS_FILE,
                           "I think the ec-tail length setting "
                           "is too big"));
                return -1;
            }
            break;
        ...
        case OPT_MAX_CALLS:
            ...
            {
                PJ_LOG(1, (THIS_FILE,
                           "Error: maximum call setting exceeds "
...
static void write_account_settings(int acc_index, pj_str_t* result)
{
    ...
    if (acc_cfg->srtp_secure_signaling != PJSUA_DEFAULT_SRTP_SECURE_SIGNALING) {
