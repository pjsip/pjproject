...
static void on_call_state(pjsua_call_id call_id, pjsip_event* e)
{
    ...
    if (call_info.state == PJSIP_INV_STATE_DISCONNECTED) {
        ...
        PJ_LOG(
          3, (THIS_FILE, "Call %d is DISCONNECTED [reason=%d (%.*s)]", call_id,
        ...
        if (call_info.state == PJSIP_INV_STATE_EARLY) {
            ...
            PJ_LOG(
              3, (THIS_FILE, "Call %d state changed to %.*s (%d %.*s)", call_id,
                  (int)call_info.state_text.slen, call_info.state_text.ptr,
                  code, (int)reason.slen, reason.ptr));
        } else {
            PJ_LOG(3,
                   (THIS_FILE, "Call %d state changed to %.*s", call_id,
                    (int)call_info.state_text.slen, call_info.state_text.ptr));
        }
 ...
 */
static void on_stream_destroyed(pjsua_call_id call_id, pjmedia_stream* strm,
                                unsigned stream_idx)
{
    PJ_UNUSED_ARG(strm);
    ...
    if (0) {
        PJ_LOG(5,
               (THIS_FILE, "Call %d stream %d destroyed, dumping media stats..",
                call_id, stream_idx));
        log_call_dump(call_id);
    }
}
    ...
    if (app_config.auto_answer < 200) {
        ...
        PJ_LOG(3,
               (THIS_FILE,
                ...
                acc_id, call_info.rem_aud_cnt, call_info.rem_vid_cnt, notif_st,
                (int)call_info.remote_info.slen, call_info.remote_info.ptr,
                (int)call_info.local_info.slen, call_info.local_info.ptr,
...
static void on_call_generic_media_state(pjsua_call_info* ci, unsigned mi,
                                        pj_bool_t* has_error)
{
    const char* status_name[] = { "None", "Active", "Local hold", "Remote hold",
                                  "Error" };
    ...
    PJ_LOG(4, (THIS_FILE, "Call %d media %d [type=%s], status is %s", ci->id,
...
static void on_incoming_subscribe(pjsua_acc_id acc_id, pjsua_srv_pres* srv_pres,
                                  ...
                                  pj_str_t* reason, pjsua_msg_data* msg_data_)
{
...
static void on_buddy_state(pjsua_buddy_id buddy_id)
{
    ...
    PJ_LOG(3, (THIS_FILE,
               ...
               (int)info.uri.slen, info.uri.ptr, (int)info.status_text.slen,
               info.status_text.ptr, info.sub_state_name, info.sub_term_code,
                                 ...
                                 pjsip_event* event)
{
        ...
        event->body.tsx_state.type == PJSIP_EVENT_RX_MSG)
    {
        ...
        snprintf(event_info, sizeof(event_info), " (RX %s)",
                     ...
                     const pj_str_t* mime_type, const pj_str_t* text)
{
    ...
    PJ_LOG(3, (THIS_FILE, "MESSAGE from %.*s: %.*s (%.*s)", (int)from->slen,
               from->ptr, (int)text->slen, text->ptr, (int)mime_type->slen,
               mime_type->ptr));
}
                      ...
                      pj_bool_t is_typing)
{
    ...
    PJ_LOG(3, (THIS_FILE, "IM indication: %.*s %s", (int)from->slen, from->ptr,
 ...
 */
static void on_call_transfer_status(pjsua_call_id call_id, int status_code,
                                    const pj_str_t* status_text,
                                    pj_bool_t final, pj_bool_t* p_cont)
{
    PJ_LOG(3, (THIS_FILE, "Call %d: transfer status=%d (%.*s) %s", call_id,
               status_code, (int)status_text->slen, status_text->ptr,
               (final ? "[final]" : "")));
                               ...
                               const pjsip_transport_state_info* info)
{
    ...
    pj_addr_str_print(&tp->remote_name.host, tp->remote_name.port, host_port,
                      sizeof(host_port), 1);
    switch (state) {
             ...
             ->ssl_sock_info->verify_status != PJ_SUCCESS))
    {
        ...
        PJ_LOG(4,
               (THIS_FILE, "TLS cipher used: 0x%06X/%s", ssl_sock_info->cipher,
                pj_ssl_cipher_name(ssl_sock_info->cipher)));

        /* Dump server TLS certificate */
        pj_ssl_cert_info_dump(ssl_sock_info->remote_cert_info, "  ", buf,
                              sizeof(buf));
                                   ...
                                   pj_status_t status, void* param)
{
    ...
    PJ_UNUSED_ARG(param);
    PJ_PERROR(
      1, (THIS_FILE, status, "ICE keep alive failure for transport %d", index));
}
...
static void on_call_media_event(pjsua_call_id call_id, unsigned med_idx,
                                pjmedia_event* event)
{
    char event_name[5];

    PJ_LOG(
      5, (THIS_FILE, "Event %s", pjmedia_fourcc_name(event->type, event_name)));
                                                    ...
                                                    unsigned flags)
{
    ...
    /* Create the adapter */
    status =
      pjmedia_tp_adapter_create(pjsua_get_pjmedia_endpt(), NULL, base_tp,
                                (flags & PJSUA_MED_TP_CLOSE_MEMBER), &adapter);
...
void on_playfile_done(pjmedia_port* port, void* usr_data)
{
    ...
    pjsip_endpt_schedule_timer(pjsua_get_pjsip_endpt(),
                               &app_config.auto_hangup_timer, &delay);
}
    ...
    if (status == PJ_SUCCESS) {
        ...
        case PJSUA_IP_CHANGE_OP_RESTART_LIS:
            ...
            pj_ansi_snprintf(info_str, sizeof(info_str),
                             "restart transport %.*s", (int)tp_info.info.slen,
                             tp_info.info.ptr);
            break;
        ...
        case PJSUA_IP_CHANGE_OP_ACC_UPDATE_CONTACT:
            ...
            if (info->acc_update_contact.code) {
                ...
                pj_ansi_snprintf(
                  info_str, sizeof(info_str), "update contact for account %.*s",
                  (int)acc_info.acc_uri.slen, acc_info.acc_uri.ptr);
            }
            break;
...
static void simple_registrar(pjsip_rx_data* rdata)
{
    ...
    status = pjsip_endpt_create_response(pjsua_get_pjsip_endpt(), rdata, 200,
                                         NULL, &tdata);
    if (status != PJ_SUCCESS)
        return;
    ...
    while (h != &rdata->msg_info.msg->hdr) {
        if (h->type == PJSIP_H_CONTACT) {
            ...
            if (e > 0) {
                pjsip_contact_hdr* nc =
                  (pjsip_contact_hdr*)pjsip_hdr_clone(tdata->pool, h);
                nc->expires = e;
...
static pj_bool_t default_mod_on_rx_request(pjsip_rx_data* rdata)
{
    ...
    if (status_code == PJSIP_SC_METHOD_NOT_ALLOWED) {
        ...
        if (cap_hdr) {
            pjsip_msg_add_hdr(
              tdata->msg, (pjsip_hdr*)pjsip_hdr_clone(tdata->pool, cap_hdr));
        }
    }
    ...
    /* Add User-Agent header */
    {
        ...
        h = (pjsip_hdr*)pjsip_generic_string_hdr_create(
          tdata->pool, &USER_AGENT, &user_agent);
...
static pj_status_t app_init(void)
{
    ...
    /* Initialize pjsua */
    status =
      pjsua_init(&app_config.cfg, &app_config.log_cfg, &app_config.media_cfg);
    if (status != PJ_SUCCESS) {
        pj_pool_release(tmp_pool);
        return status;
    }
    ...
    for (i = 0; i < app_config.wav_count; ++i) {
        ...
        status =
          pjsua_player_create(&app_config.wav_files[i], play_options, &wav_id);
        if (status != PJ_SUCCESS)
            goto on_error;
        ...
        if (app_config.wav_id == PJSUA_INVALID_ID) {
            ...
            if (app_config.auto_play_hangup) {
                ...
                status =
                  pjmedia_wav_player_set_eof_cb2(port, NULL, &on_playfile_done);
                if (status != PJ_SUCCESS)
                    goto on_error;
    ...
    for (i = 0; i < app_config.tone_count; ++i) {
        ...
        pj_ansi_snprintf(name, sizeof(name), "tone-%d,%d",
                         app_config.tones[i].freq1, app_config.tones[i].freq2);
        label = pj_str(name);
        status2 = pjmedia_tonegen_create2(app_config.pool, &label, 8000, 1, 160,
                                          16, PJMEDIA_TONEGEN_LOOP, &tport);
    ...
    if (app_config.no_tones == PJ_FALSE) {
        ...
        status = pjmedia_tonegen_create2(
          app_config.pool, &name, app_config.media_cfg.clock_rate,
          app_config.media_cfg.channel_count, samples_per_frame, 16,
          PJMEDIA_TONEGEN_LOOP, &app_config.ringback_port);
        if (status != PJ_SUCCESS)
            goto on_error;
        ...
        status = pjmedia_tonegen_create2(
          app_config.pool, &name, app_config.media_cfg.clock_rate,
          app_config.media_cfg.channel_count, samples_per_frame, 16,
          PJMEDIA_TONEGEN_LOOP, &app_config.ring_port);
        if (status != PJ_SUCCESS)
            goto on_error;
    ...
    if (app_config.avi_cnt) {
        ...
        status = pjmedia_avi_dev_create_factory(
          pjsua_get_pool_factory(), app_config.avi_cnt, &avi_factory);
        if (status != PJ_SUCCESS) {
        ...
        for (i = 0; i < app_config.avi_cnt; ++i) {
            ...
            if (status != PJ_SUCCESS) {
                PJ_PERROR(
                  1, (THIS_FILE, status, "Error creating AVI player for %.*s",
            ...
            for (strm_idx = 0; strm_idx < strm_cnt; ++strm_idx) {
                ...
                aud =
                  pjmedia_avi_streams_get_stream(avdp.avi_streams, strm_idx);
                fmt = &aud->info.fmt;

                pjmedia_fourcc_name(fmt->id, fmt_name);

                if (fmt->id == PJMEDIA_FORMAT_PCM) {
                    status = pjsua_conf_add_port(app_config.pool, aud, &slot);
                    if (status == PJ_SUCCESS) {
                        PJ_LOG(4,
                               (THIS_FILE, "AVI %.*s: audio added to slot %d",
                                (int)avdp.title.slen, avdp.title.ptr, slot));
                        app_config.avi[i].slot = slot;
                    }
                } else {
                    PJ_LOG(4, (THIS_FILE, "AVI %.*s: audio ignored, format=%s",
                               (int)avdp.title.slen, avdp.title.ptr, fmt_name));
                }
            }
        }
#else
    ...
    if (!app_config.no_tcp && app_config.ipv6) {
        ...
        status = pjsua_transport_create(type, &tcp_cfg, &transport_id);
        if (status != PJ_SUCCESS)
            goto on_error;
    ...
    if (app_config.use_tls) {
        ...
        tcp_cfg.port++;
        status =
          pjsua_transport_create(PJSIP_TRANSPORT_TLS, &tcp_cfg, &transport_id);
        tcp_cfg.port--;
        if (status != PJ_SUCCESS)
            goto on_error;
...
pj_status_t pjsua_app_run(pj_bool_t wait_telnet_cli)
{
    ...
    if (stdout_refresh > 0) {
        pj_thread_create(app_config.pool, "stdout", &stdout_refresh_proc, NULL,
                         0, 0, &stdout_refresh_thread);
    }
    ...
    if (uri_arg.slen) {
        ...
        pjsua_call_make_call(current_acc, &uri_arg, &call_opt, NULL, NULL,
                             NULL);
    }
...
static void stereo_demo()
{
    ...
    status = pjmedia_splitcomb_create(
      app_config.pool, PJMEDIA_PIA_SRATE(&conf->info) /* clock rate */,
      2 /* stereo */, 2 * PJMEDIA_PIA_SPF(&conf->info),
      PJMEDIA_PIA_BITS(&conf->info), 0 /* options */, &app_config.sc);
    ...
    status = pjmedia_splitcomb_create_rev_channel(
      app_config.pool, app_config.sc, 1 /* ch1 */, 0 /* options */,
      &app_config.sc_ch1);
    pj_assert(status == PJ_SUCCESS);
    ...
    status = pjmedia_snd_port_create(
      app_config.pool, -1, -1, PJMEDIA_PIA_SRATE(&conf->info), 2 /* stereo */,
      2 * PJMEDIA_PIA_SPF(&conf->info), PJMEDIA_PIA_BITS(&conf->info), 0,
      &app_config.snd);
