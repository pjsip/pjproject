...
void app_config_init_video(pjsua_acc_config* acc_cfg)
{
    ...
    if (app_config.avi_auto_play &&
        ...
        app_config.avi[app_config.avi_def_idx].dev_id !=
...
void add_multipart(pjsua_msg_data* msg_data)
{
    ...
    if (!alt_part) {
        ...
        alt_part->body =
          pjsip_msg_body_create(app_config.pool, &type, &subtype, &content);
    }
...
void app_config_show_video(int acc_id, const pjsua_acc_config* acc_cfg)
{
    PJ_LOG(3,
           (THIS_FILE,
            ...
            "  Capture dev:      %d\n"
            "  Render dev:       %d",
            acc_id, acc_cfg->vid_in_auto_show, acc_cfg->vid_out_auto_transmit,
            acc_cfg->vid_cap_dev, acc_cfg->vid_rend_dev));
}
