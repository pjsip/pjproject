...
pj_status_t gui_start(gui_menu* menu)
{
    ...
    for (int i = 0; i < menu->submenu_cnt; i++) {
        NSString* str =
          [NSString stringWithFormat:@"%s", menu->submenus[i]->title];
        [view.titles addObject:str];
        smenu =
          [NSMutableArray arrayWithCapacity:menu->submenus[i]->submenu_cnt];
        /* We do not need the last two menus of the "Tests" menu (NULL and
         * "Exit"), so subtract by 2
         */
        for (int j = 0; j < menu->submenus[i]->submenu_cnt - (i == 0 ? 2 : 0);
             j++) {
            str = [NSString
              stringWithFormat:@"%s", menu->submenus[i]->submenus[j]->title];
            [smenu addObject:str];
        }
                        ...
                        enum gui_flag flag)
{
    ...
    cmsg = message;
    cflag = flag;
    [view performSelectorOnMainThread:@selector(showMsg)
                           withObject:nil
                        waitUntilDone:YES];
...
- (void)viewDidLoad
{
    ...
    [documentsDirectory getCString:doc_path
        ...
        cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    // Configure the cell.
    cell.textLabel.text =
      [[menus objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
...
- (void)showMsg
{
    if (cmsg == NULL) {
        self.testView.testDesc.text =
          [self.testView.testDesc.text stringByAppendingString:@"Finished"];
        [self.testView.testDesc
          scrollRangeToVisible:NSMakeRange(
                                 [self.testView.testDesc.text length] - 1, 1)];
    ...
    self.testView.testDesc.text = [self.testView.testDesc.text
      stringByAppendingString:[NSString stringWithFormat:@"%s\n\n", cmsg]];
    [self.testView.testDesc
      scrollRangeToVisible:NSMakeRange([self.testView.testDesc.text length] - 1,
                                       1)];
    ...
    if (cflag == WITH_YESNO) {
        ...
        [self.testView.button1 setTitle:@"OK" forState:UIControlStateNormal];
        [self.testView.button2 setTitle:@"Cancel"
  ...
  didSelectRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    if (self.testView == nil) {
        self.testView =
          [[TestViewController alloc] initWithNibName:@"TestViewController"
