...
{
	// The only thing that changes is to make mixable linear PCM into the HAL's virtual linear PCM format, which is currently Float32.
	if((ioDescription.mFormatID == kAudioFormatLinearPCM) && ((ioDescription.mFormatFlags & kIsNonMixableFlag) == 0))
	{
...
bool	operator<(const AudioStreamBasicDescription& x, const AudioStreamBasicDescription& y)
{
	...
	if((!isDone) && ((x.mFormatID != 0) && (y.mFormatID != 0)))
	{
		...
		{
			// Formats are sorted numerically except that linear PCM is always first.
...
bool CAStreamBasicDescription::FromText(const char *inTextDesc, AudioStreamBasicDescription &fmt)
{
	...
	if (p[0] == '-') // Previously a leading dash on PCM formats was required.
		++p;
