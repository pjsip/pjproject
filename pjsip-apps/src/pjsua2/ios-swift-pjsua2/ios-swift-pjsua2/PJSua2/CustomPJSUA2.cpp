...
/**
 Dispatch queue to manage ios thread serially or concurrently on app's main
 thread for more information please visit:
...
class MyAccount : public Account {
   public:
    ...
    // This is getting for incoming call (We can either answer or hangup the
    // incoming call)
                           ...
                           std::string ip, std::string port)
{
    ...
    //  TODO:: GET ID -1 IS EXPERIMENTAL, NOT SURE THAT, IT IS GOOD WAY TO CHECK
    //  ACC IS CREATED. FIX IT!
    if (acc->getId() == -1) {
        // Create the account
        try {
            acc->create(acfg);
