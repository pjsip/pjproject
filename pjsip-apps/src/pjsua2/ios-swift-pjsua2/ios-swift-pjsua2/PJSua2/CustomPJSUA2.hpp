...
class PJSua2 {
   public:
    ...
    // Account
    /**
     Create Account via following config(string username, string password,
    ...
    /**
     Listener (When we have incoming call, this function pointer will notify
     swift.)
     */
    void incoming_call(void (*function)());

    /**
     Listener (When we have changes on the call state, this function pointer
     will notify swift.)
     */
