                          ...
                          pj_ioqueue_key_t **p_key)
{
    ...
    if (status == -1)
    {
        ...
        key = NULL;
        TRACE_((THIS_FILE,
                "pj_ioqueue_register_sock error: kevent rc=%d", status));
        goto on_return;
    }
                                ...
                                enum ioqueue_event_type event_type )
{
    ...
    case READABLE_EVENT:
        ...
        kevent(ioqueue->kfd, &event, 1, NULL, 0, NULL);
        break;
    case WRITEABLE_EVENT:
...
PJ_DEF(int) pj_ioqueue_poll( pj_ioqueue_t *ioqueue, const pj_time_val *timeout)
{
    ...
    TRACE_((THIS_FILE, "start kqueue wait, msec=%d", xtimeout.tv_sec * 1000 + xtimeout.tv_nsec / 1000));
    count = os_kqueue_wait(ioqueue->kfd, NULL, 0, events, MAX_EVENTS, &xtimeout);
    if (count == 0)
    {
#if PJ_IOQUEUE_HAS_SAFE_UNREG
    ...
    for (event_cnt = 0, i = 0; i < count; ++i)
    {
        ...
        if ((events[i].filter & EVFILT_WRITE) &&
            (key_has_pending_write(h) || key_has_pending_connect(h)) && !IS_CLOSING(h))
        {
