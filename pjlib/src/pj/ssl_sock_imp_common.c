...
static pj_status_t circ_init(pj_pool_factory* factory, circ_buf_t* cb,
                             pj_size_t cap)
{
    cb->cap = cap;
    cb->readp = 0;
...
static pj_status_t circ_write(circ_buf_t* cb, const pj_uint8_t* src,
                              pj_size_t len)
{
    ...
    if (ssock->is_server) {
        ...
        if (status != PJ_SUCCESS) {
            ...
            if (ssock->param.timer_heap) {
                ...
                status1 = pj_timer_heap_schedule_w_grp_lock(
                  ssock->param.timer_heap, &ssock->timer, &interval,
                  TIMER_CLOSE, ssock->param.grp_lock);
        ...
        if (ssock->param.cb.on_accept_complete2) {
            ret = (*ssock->param.cb.on_accept_complete2)(
              ssock->parent, ssock, (pj_sockaddr_t*)&ssock->rem_addr,
              pj_sockaddr_get_len((pj_sockaddr_t*)&ssock->rem_addr), status);
...
static void free_send_data(pj_ssl_sock_t* ssock, write_data_t* wdata)
{
    ...
    if (spl->next == wdata && spl->prev == wdata) {
        ...
        if (wdata->prev < wdata) {
            pj_size_t jump_len;
            jump_len =
              (char*)wdata - ((char*)wdata->prev + wdata->prev->record_len);
                                         ...
                                         pj_size_t orig_len, unsigned flags)
{
    ...
    if (ssock->param.sock_type == pj_SOCK_STREAM()) {
        status = pj_activesock_send(ssock->asock, &wdata->key,
                                    wdata->data.content, &len, flags);
    } else {
        status = pj_activesock_sendto(
          ssock->asock, &wdata->key, wdata->data.content, &len, flags,
          (pj_sockaddr_t*)&ssock->rem_addr, ssock->addr_len);
    }
#endif
...
#define OFFSET_OF_READ_DATA_PTR(ssock, asock_rbuf) \
    (read_data_t**)((pj_int8_t*)(asock_rbuf) + ssock->param.read_buffer_size)

static pj_bool_t ssock_on_data_read(pj_ssl_sock_t* ssock, void* data,
                                    pj_size_t size, pj_status_t status,
                                    pj_size_t* remainder)
{
    if (status != PJ_SUCCESS)
        ...
        do {
            ...
            if (size_ > 0 || status != PJ_SUCCESS) {
                if (ssock->param.cb.on_data_read) {
                    ...
                    ret = (*ssock->param.cb.on_data_read)(
                      ssock, buf->data, buf->len, status, &remainder_);
                    if (!ret) {
...
on_error:
    ...
    if (ssock->read_started && ssock->param.cb.on_data_read) {
        pj_bool_t ret;
        ret =
          (*ssock->param.cb.on_data_read)(ssock, NULL, 0, status, remainder);
        if (!ret) {
            /* We've been destroyed */
            return PJ_FALSE;
        }
...
static pj_status_t get_localaddr(pj_ssl_sock_t* ssock, pj_sockaddr_t* addr,
                                 int* namelen)
{
    PJ_UNUSED_ARG(addr);
...
static pj_bool_t ssock_on_accept_complete(pj_ssl_sock_t* ssock_parent,
                                          pj_sock_t newsock, void* newconn,
                                          ...
                                          pj_status_t accept_status)
{
    ...
    if (accept_status != PJ_SUCCESS) {
        if (ssock_parent->param.cb.on_accept_complete2) {
            (*ssock_parent->param.cb.on_accept_complete2)(
              ssock_parent, NULL, src_addr, src_addr_len, accept_status);
        }
        return PJ_TRUE;
    }
    ...
    if (ssock_parent->cert) {
        status =
          pj_ssl_sock_set_certificate(ssock, ssock->pool, ssock_parent->cert);
        if (status != PJ_SUCCESS)
            goto on_return;
    }
    ...
    /* Prepare read buffer */
    ssock->asock_rbuf = (void**)pj_pool_calloc(
      ssock->pool, ssock->param.async_cnt, sizeof(void*));
    if (!ssock->asock_rbuf) {
        status = PJ_ENOMEM;
        goto on_return;
    }
    ...
    for (i = 0; i < ssock->param.async_cnt; ++i) {
        ssock->asock_rbuf[i] = (void*)pj_pool_alloc(
          ssock->pool, ssock->param.read_buffer_size + sizeof(read_data_t*));
    ...
    if (ssock->param.sockopt_params.cnt) {
        status =
          pj_sock_setsockopt_params(ssock->sock, &ssock->param.sockopt_params);
    ...
    status = pj_activesock_create(
      ssock->pool, ssock->sock, ssock->param.sock_type, &asock_cfg,
      ssock->param.ioqueue, &asock_cb, ssock, &ssock->asock);
    ...
    status = pj_activesock_start_read2(
      ssock->asock, ssock->pool, (unsigned)ssock->param.read_buffer_size,
      ssock->asock_rbuf, PJ_IOQUEUE_ALWAYS_ASYNC);
    if (status != PJ_SUCCESS)
        goto on_return;
#endif
    ...
    ssock->send_buf.buf =
      (char*)pj_pool_alloc(ssock->pool, ssock->param.send_buffer_size);
        ...
        (ssock->param.timeout.sec != 0 || ssock->param.timeout.msec != 0))
    {
        ...
        status = pj_timer_heap_schedule_w_grp_lock(
          ssock->param.timer_heap, &ssock->timer, &ssock->param.timeout,
          TIMER_HANDSHAKE_TIMEOUT, ssock->param.grp_lock);
        if (status != PJ_SUCCESS) {
            ssock->timer.id = TIMER_NONE;
            status = PJ_SUCCESS;
        }
                                           ...
                                           int src_addr_len, pj_status_t status)
{
    ...
    return ssock_on_accept_complete(ssock, newsock, NULL, src_addr,
                                    src_addr_len, status);
}
...
PJ_DEF(pj_status_t)
pj_ssl_cipher_get_availables(pj_ssl_cipher ciphers[], unsigned* cipher_num)
{
    unsigned i;
...
PJ_DEF(pj_status_t)
pj_ssl_curve_get_availables(pj_ssl_curve curves[], unsigned* curve_num)
{
    unsigned i;
 ...
 */
PJ_DEF(pj_status_t)
pj_ssl_sock_create(pj_pool_t* pool, const pj_ssl_sock_param* param,
 ...
 */
PJ_DEF(pj_status_t)
pj_ssl_sock_start_read(pj_ssl_sock_t* ssock, pj_pool_t* pool,
                       unsigned buff_size, pj_uint32_t flags)
{
    ...
    readbuf =
      (void**)pj_pool_calloc(pool, ssock->param.async_cnt, sizeof(void*));
    if (!readbuf)
        return PJ_ENOMEM;
 ...
 */
PJ_DEF(pj_status_t)
pj_ssl_sock_start_read2(pj_ssl_sock_t* ssock, pj_pool_t* pool,
                        unsigned buff_size, void* readbuf[], pj_uint32_t flags)
{
    unsigned i;
    ...
    ssock->ssock_rbuf = (read_data_t*)pj_pool_calloc(
      pool, ssock->param.async_cnt, sizeof(read_data_t));
    if (!ssock->ssock_rbuf)
        return PJ_ENOMEM;
    ...
    for (i = 0; i < ssock->param.async_cnt; ++i) {
        if (ssock->asock_rbuf[i]) {
            ...
            ssock_on_data_read(ssock, ssock->asock_rbuf[i], 0, PJ_SUCCESS,
                               &remainder);
        }
    }
 ...
 */
PJ_DEF(pj_status_t)
pj_ssl_sock_start_recvfrom(pj_ssl_sock_t* ssock, pj_pool_t* pool,
                           unsigned buff_size, pj_uint32_t flags)
{
    PJ_UNUSED_ARG(ssock);
    PJ_UNUSED_ARG(pool);
 ...
 */
PJ_DEF(pj_status_t)
pj_ssl_sock_start_recvfrom2(pj_ssl_sock_t* ssock, pj_pool_t* pool,
...
static pj_status_t ssl_send(pj_ssl_sock_t* ssock, pj_ioqueue_op_key_t* send_key,
...
static pj_status_t flush_delayed_send(pj_ssl_sock_t* ssock)
{
    ...
    while (!pj_list_empty(&ssock->write_pending)) {
        ...
        status = ssl_send(ssock, &wp->key, wp->data.ptr, wp->plain_data_len,
                          wp->flags);
        if (status != PJ_SUCCESS) {
...
static pj_status_t delay_send(pj_ssl_sock_t* ssock,
                              pj_ioqueue_op_key_t* send_key, const void* data,
                              pj_ssize_t size, unsigned flags)
{
    write_data_t* wp;
 ...
 */
PJ_DEF(pj_status_t)
pj_ssl_sock_send(pj_ssl_sock_t* ssock, pj_ioqueue_op_key_t* send_key,
                 const void* data, pj_ssize_t* size, unsigned flags)
{
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pj_ssl_sock_sendto(pj_ssl_sock_t* ssock, pj_ioqueue_op_key_t* send_key,
                   const void* data, pj_ssize_t* size, unsigned flags,
 ...
 */
PJ_DEF(pj_status_t)
pj_ssl_sock_start_accept(pj_ssl_sock_t* ssock, pj_pool_t* pool,
                         const pj_sockaddr_t* localaddr, int addr_len)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_ssl_sock_start_accept2(pj_ssl_sock_t* ssock, pj_pool_t* pool,
                          const pj_sockaddr_t* localaddr, int addr_len,
                          const pj_ssl_sock_param* newsock_param)
{
...
#ifdef SSL_SOCK_IMP_USE_OWN_NETWORK
    status =
      network_start_accept(ssock, pool, localaddr, addr_len, newsock_param);
    if (status != PJ_SUCCESS)
        goto on_error;
#else
    /* Create socket */
    ...
    if (ssock->param.reuse_addr) {
        int enabled = 1;
        status =
          pj_sock_setsockopt(ssock->sock, pj_SOL_SOCKET(), pj_SO_REUSEADDR(),
    ...
    status = pj_activesock_create(pool, ssock->sock, ssock->param.sock_type,
                                  &asock_cfg, ssock->param.ioqueue, &asock_cb,
                                  ssock, &ssock->asock);
    ...
    status =
      pj_sock_getsockname(ssock->sock, &ssock->local_addr, &ssock->addr_len);
    if (status != PJ_SUCCESS)
 ...
 */
PJ_DEF(pj_status_t)
pj_ssl_sock_start_connect(pj_ssl_sock_t* ssock, pj_pool_t* pool,
...
PJ_DEF(pj_status_t)
pj_ssl_sock_start_connect2(pj_ssl_sock_t* ssock,
...
PJ_DEF(pj_status_t)
pj_ssl_cert_load_from_files(pj_pool_t* pool, const pj_str_t* CA_file,
...
PJ_DEF(pj_status_t)
pj_ssl_cert_load_from_files2(pj_pool_t* pool, const pj_str_t* CA_file,
                             const pj_str_t* CA_path, const pj_str_t* cert_file,
                             const pj_str_t* privkey_file,
                             const pj_str_t* privkey_pass,
                             pj_ssl_cert_t** p_cert)
{
    ...
    PJ_ASSERT_RETURN(pool && (CA_file || CA_path) && cert_file && privkey_file,
                     PJ_EINVAL);
...
PJ_DEF(pj_status_t)
pj_ssl_cert_load_from_buffer(pj_pool_t* pool, const pj_ssl_cert_buffer* CA_buf,
...
PJ_DEF(pj_status_t)
pj_ssl_sock_set_certificate(pj_ssl_sock_t* ssock, pj_pool_t* pool,
