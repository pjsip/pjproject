...
#if PJ_TIMER_DEBUG
        PJ_LOG(3, (THIS_FILE,
                   "Bug! Trying to remove entry %p from %s "
                   "line %d, which has been deallocated "
                   "without being cancelled",
                   GET_ENTRY(removed_node), removed_node->src_file,
                   removed_node->src_line));
#else
        PJ_LOG(3, (THIS_FILE,
                   "Bug! Trying to remove entry %p "
                   "which has been deallocated "
                   "without being cancelled",
                   GET_ENTRY(removed_node)));
#endif
#if ASSERT_IF_ENTRY_DESTROYED
        pj_assert(removed_node->dup._timer_id ==
                  removed_node->entry->_timer_id);
#endif
    }
...
static pj_status_t grow_heap(pj_timer_heap_t* ht)
{
    ...
    PJ_LOG(6, (THIS_FILE, "Growing heap size from %d to %d", ht->max_size,
               new_size));

    // First grow the heap itself.
    new_heap = (pj_timer_entry_dup**)pj_pool_calloc(
      ht->pool, new_size, sizeof(pj_timer_entry_dup*));
    if (!new_heap)
        return PJ_ENOMEM;
    ...
    new_timer_dups = (pj_timer_entry_dup*)pj_pool_alloc(
      ht->pool, sizeof(pj_timer_entry_dup) * new_size);
    if (!new_timer_dups)
        return PJ_ENOMEM;
    ...
    new_timer_ids = 0;
    new_timer_ids =
      (pj_timer_id_t*)pj_pool_alloc(ht->pool, new_size * sizeof(pj_timer_id_t));
    if (!new_timer_ids)
        return PJ_ENOMEM;
...
static pj_status_t insert_node(pj_timer_heap_t* ht, pj_timer_entry* new_node,
                               const pj_time_val* future_time)
{
    ...
    } else if (PJ_TIME_VAL_GTE(*future_time, ht->head_list.prev->_timer_value))
    {
...
static pj_status_t schedule_entry(pj_timer_heap_t* ht, pj_timer_entry* entry,
                                  const pj_time_val* future_time)
{
...
pj_timer_heap_create(pj_pool_t* pool, pj_size_t size, pj_timer_heap_t** p_heap)
{
    ...
    *p_heap = NULL;
    ...
    ht->heap = (pj_timer_entry_dup**)pj_pool_calloc(
      pool, size, sizeof(pj_timer_entry_dup*));
    if (!ht->heap)
        return PJ_ENOMEM;

#if PJ_TIMER_USE_COPY
    // Create the timer entry copies array.
    ht->timer_dups = (pj_timer_entry_dup*)pj_pool_alloc(
      pool, sizeof(pj_timer_entry_dup) * size);
    if (!ht->timer_dups)
        return PJ_ENOMEM;
#endif
    ...
    // Create the parallel
    ht->timer_ids =
      (pj_timer_id_t*)pj_pool_alloc(pool, sizeof(pj_timer_id_t) * size);
    if (!ht->timer_ids)
        return PJ_ENOMEM;
...
pj_timer_entry_init(pj_timer_entry* entry, int id, void* user_data,
...
#if PJ_TIMER_DEBUG
PJ_DEF(pj_status_t)
pj_timer_heap_schedule_dbg(pj_timer_heap_t* ht, pj_timer_entry* entry,
...
PJ_DEF(pj_status_t)
pj_timer_heap_schedule_w_grp_lock_dbg(pj_timer_heap_t* ht,
                                      pj_timer_entry* entry,
                                      const pj_time_val* delay, int id_val,
                                      pj_grp_lock_t* grp_lock,
                                      const char* src_file, int src_line)
{
    return schedule_w_grp_lock_dbg(ht, entry, delay, PJ_TRUE, id_val, grp_lock,
                                   src_file, src_line);
}

#else
PJ_DEF(pj_status_t)
pj_timer_heap_schedule(pj_timer_heap_t* ht, pj_timer_entry* entry,
                       const pj_time_val* delay)
{
...
PJ_DEF(pj_status_t)
pj_timer_heap_schedule_w_grp_lock(pj_timer_heap_t* ht, pj_timer_entry* entry,
                                  const pj_time_val* delay, int id_val,
                                  pj_grp_lock_t* grp_lock)
{
...
static int cancel_timer(pj_timer_heap_t* ht, pj_timer_entry* entry,
...
PJ_DEF(int) pj_timer_heap_cancel(pj_timer_heap_t* ht, pj_timer_entry* entry)
{
    return cancel_timer(ht, entry, 0, 0);
}

PJ_DEF(int)
pj_timer_heap_cancel_if_active(pj_timer_heap_t* ht, pj_timer_entry* entry,
                               int id_val)
{
...
PJ_DEF(unsigned)
pj_timer_heap_poll(pj_timer_heap_t* ht, pj_time_val* next_delay)
{
    pj_time_val now;
    ...
    while (ht->cur_size && PJ_TIME_VAL_LTE(min_time_node, now) &&
...
#if PJ_TIMER_DEBUG
            PJ_LOG(3, (THIS_FILE,
                       "Bug! Polling entry %p from %s line %d has "
                       "been deallocated without being cancelled",
                       GET_ENTRY(node), node->src_file, node->src_line));
#else
            PJ_LOG(3, (THIS_FILE,
                       "Bug! Polling entry %p has "
                       "been deallocated without being cancelled",
                       GET_ENTRY(node)));
#endif
#if ASSERT_IF_ENTRY_DESTROYED
...
PJ_DEF(pj_status_t)
pj_timer_heap_earliest_time(pj_timer_heap_t* ht, pj_time_val* timeval)
{
...
PJ_DEF(void) pj_timer_heap_dump(pj_timer_heap_t* ht)
{
    ...
    PJ_LOG(3, (THIS_FILE, "  Cur size: %d entries, max: %d", (int)ht->cur_size,
               (int)ht->max_size));
...
#    endif
            ...
            PJ_LOG(3,
                   (THIS_FILE, "    %d\t%d\t%d.%03d\t%s:%d",
                    GET_FIELD(e, _timer_id), GET_FIELD(e, id), (int)delta.sec,
                    (int)delta.msec, e->src_file, e->src_line));
        }
    }
