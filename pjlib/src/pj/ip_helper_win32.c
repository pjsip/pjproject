...
static DWORD MyGetIpAddrTable(PMIB_IPADDRTABLE pIpAddrTable, PULONG pdwSize,
                              BOOL bOrder)
{
    if (NULL == s_pfnGetIpAddrTable) {
        s_pfnGetIpAddrTable =
          (PFN_GetIpAddrTable)GetIpHlpApiProc(GPA_TEXT("GetIpAddrTable"));
    }
...
static DWORD MyGetAdapterAddresses(ULONG Family, ULONG Flags, PVOID Reserved,
                                   PIP_ADAPTER_ADDRESSES AdapterAddresses,
                                   PULONG SizePointer)
{
    if (NULL == s_pfnGetAdapterAddresses) {
        s_pfnGetAdapterAddresses = (PFN_GetAdapterAddresses)GetIpHlpApiProc(
          GPA_TEXT("GetAdaptersAddresses"));
    }
...
static DWORD MyGetIfEntry(MIB_IFROW* pIfRow)
{
    if (NULL == s_pfnGetIfEntry) {
        s_pfnGetIfEntry =
          (PFN_GetIfEntry)GetIpHlpApiProc(GPA_TEXT("GetIfEntry"));
    }
                                 ...
                                 PULONG pdwSize, BOOL bOrder)
{
    if (NULL == s_pfnGetIpForwardTable) {
        s_pfnGetIpForwardTable =
          (PFN_GetIpForwardTable)GetIpHlpApiProc(GPA_TEXT("GetIpForwardTable"));
    }
 ...
 */
static pj_status_t enum_ipv4_interface(unsigned* p_cnt, pj_sockaddr ifs[])
{
    char ipTabBuff[512];
 ...
 */
static pj_status_t enum_ipv4_ipv6_interface(int af, unsigned* p_cnt,
                                            pj_sockaddr ifs[])
{
    ...
    flags = GAA_FLAG_SKIP_FRIENDLY_NAME | GAA_FLAG_SKIP_DNS_SERVER |
...
pj_enum_ip_interface(int af, unsigned* p_cnt, pj_sockaddr ifs[])
{
    ...
    PJ_ASSERT_RETURN(p_cnt && ifs, PJ_EINVAL);
    PJ_ASSERT_RETURN(
      af == PJ_AF_UNSPEC || af == PJ_AF_INET || af == PJ_AF_INET6,
      PJ_EAFNOTSUP);
