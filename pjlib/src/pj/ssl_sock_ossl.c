...
static void update_certs_info(pj_ssl_sock_t* ssock, X509_STORE_CTX* ctx,
...
#        else
#            define strerror_r(err, buf, len) \
                pj_ansi_strncpy(buf, strerror(err), len)
#        endif
#    endif
...
#        define ERROR_LOG(msg, err, ssock) \
            { \
                ...
                PJ_LOG( \
                  2, ("SSL", \
                      "%s (%s): Level: %d err: <%lu> <%s> len: %d peer: %s", \
                      msg, action, level, err, err_str, len, \
                      (ssock && pj_sockaddr_has_addr(&ssock->rem_addr) ? \
                         pj_sockaddr_print(&ssock->rem_addr, buf, sizeof(buf), \
                                           3) : \
                         "???"))); \
            }
#    else
#        define ERROR_LOG(msg, err, ssock) \
            { \
                ...
                PJ_LOG( \
                  2, \
                  ("SSL", \
                   "%s (%s): Level: %d err: <%lu> <%s-%s-%s> len: %d " \
                   "peer: %s", \
                         ...
                         pj_ssl_sock_t* ssock)
{
    ...
    case SSL_ERROR_SYSCALL: {
        ...
        if (err2) {
            ...
            PJ_LOG(4, ("SSL",
                       "BIO error, SSL_ERROR_SYSCALL (%s): "
                       "errno: <%d> <%s> len: %d",
                       action, errno, errStr, len));
        } else {
            /* ret!=0 & ret!=-1 & nothing on error stack - is this valid??? */
            PJ_LOG(2, ("SSL", "SSL_ERROR_SYSCALL (%s) ret: %d len: %d", action,
                       ret, len));
        }
        break;
    }
    ...
    default:
        PJ_LOG(2, ("SSL", "%lu [%s] (%s) ret: %d len: %d", ssl_err, ssl_err_str,
                   action, ret, len));
        break;
    }
}
...
#    endif /* PJ_HAS_ERROR_STRING */
    ...
    errstr.ptr = buf;
    errstr.slen =
      pj_ansi_snprintf(buf, bufsize, "Unknown OpenSSL error %lu", ssl_err);
...
static const struct ssl_ciphers_t ADDITIONAL_CIPHERS[] = {
    { 0xFF000000, "DEFAULT" },     { 0xFF000001, "@SECLEVEL=1" },
    { 0xFF000002, "@SECLEVEL=2" }, { 0xFF000003, "@SECLEVEL=3" },
    { 0xFF000004, "@SECLEVEL=4" }, { 0xFF000005, "@SECLEVEL=5" }
};
...
static void io_read(pj_ssl_sock_t* ssock, circ_buf_t* cb, pj_uint8_t* dst,
                    pj_size_t len)
{
    ossl_sock_t* ossock = (ossl_sock_t*)ssock;
    char* data;
...
#    if defined(PJ_SSL_SOCK_OSSL_USE_THREAD_CB) && \
      PJ_SSL_SOCK_OSSL_USE_THREAD_CB != 0 && \
...
static void ossl_set_thread_id(CRYPTO_THREADID* id)
{
    CRYPTO_THREADID_set_numeric(
      id, (unsigned long)pj_thread_get_os_handle(pj_thread_this()));
}
...
static pj_status_t init_ossl_lock()
{
    ...
    if (ossl_locks) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(
              1, (THIS_FILE, status, "Fail creating mutex for OpenSSL lock"));
            release_thread_cb();
            return status;
        }
...
static pj_status_t init_openssl(void)
{
    ...
    status = pj_register_strerror(PJ_SSL_ERRNO_START, PJ_SSL_ERRNO_SPACE_SIZE,
                                  &ssl_strerror);
...
#    endif
        ...
        for (i = 0; i < n; ++i) {
            ...
            ssl_ciphers[i].id =
              (pj_ssl_cipher)(pj_uint32_t)SSL_CIPHER_get_id(c) & 0x00FFFFFF;
...
static int verify_cb(int preverify_ok, X509_STORE_CTX* x509_ctx)
{
    ...
    if (ssock->param.cb.on_verify_cb) {
        ...
        preverify_ok = (*ssock->param.cb.on_verify_cb)(ssock, ssock->is_server);
    ...
    if (dctx != NULL) {
        if (pass->slen) {
            OSSL_DECODER_CTX_set_passphrase(
              dctx, (const unsigned char*)pass->ptr, pass->slen);
        }
...
#    endif
        ...
        if (!SSL_CTX_set_session_id_context(ctx, (const unsigned char*)&sid_ctx,
                                            sizeof(sid_ctx)))
        {
            PJ_LOG(1, (THIS_FILE,
                       "Warning! Unable to set server session id "
    ...
    if (cert) {
        ...
        if (cert->CA_file.slen || cert->CA_path.slen) {
            rc = SSL_CTX_load_verify_locations(
              ctx, cert->CA_file.slen == 0 ? NULL : cert->CA_file.ptr,
            ...
            if (rc != 1) {
                ...
                if (cert->CA_path.slen) {
                    PJ_PERROR(
                      1, (ssock->pool->obj_name, status,
                          "Error loading CA path '%s'", cert->CA_path.ptr));
                }
                SSL_CTX_free(ctx);
                ...
                PJ_LOG(
                  4, (ssock->pool->obj_name,
                      "CA certificates loaded from '%s%s%s'", cert->CA_file.ptr,
        ...
        if (cert->privkey_file.slen) {
            ...
            if (rc != 1) {
                ...
                PJ_LOG(4,
                       (ssock->pool->obj_name, "Private key loaded from '%s'",
                        cert->privkey_file.ptr));
            }
...
#    endif
        ...
        if (cert->cert_buf.slen) {
            ...
            cbio =
              BIO_new_mem_buf((void*)cert->cert_buf.ptr, cert->cert_buf.slen);
            if (cbio != NULL) {
        ...
        if (cert->CA_buf.slen) {
            BIO* cbio =
              BIO_new_mem_buf((void*)cert->CA_buf.ptr, cert->CA_buf.slen);
            X509_STORE* cts = SSL_CTX_get_cert_store(ctx);

            if (cbio && cts) {
                STACK_OF(X509_INFO)* inf =
                  PEM_X509_INFO_read_bio(cbio, NULL, NULL, NULL);
...
#    if PJ_LOG_MAX_LEVEL >= 4
                            ...
                            PJ_LOG(4, (ssock->pool->obj_name,
                                       ...
                                       X509_NAME_oneline(
                                         X509_get_subject_name(itmp->x509), buf,
                                         sizeof(buf))));
#    endif
                        }
                    }
                    PJ_LOG(
                      4, (ssock->pool->obj_name,
                          "CA certificates loaded from buffer (cnt=%d)", cnt));
                }
    ...
    if (ssock->is_server) {
        ...
        if (p) {
            ...
            for (i = 0; i < PJ_ARRAY_SIZE(cert_types); ++i) {
                ...
                if (err == 1)
                    err =
                      SSL_CTX_use_PrivateKey_file(ctx, cf, SSL_FILETYPE_PEM);
                if (err == 1) {
                    PJ_LOG(4, (ssock->pool->obj_name,
...
#    endif
        ...
        if (ca_dn != NULL) {
            ...
            PJ_LOG(4, (ssock->pool->obj_name, "CA certificates loaded from %s",
...
static pj_status_t set_cipher_list(pj_ssl_sock_t* ssock)
{
    ...
    if (ssock->param.ciphers_num == 0) {
        ret =
          SSL_CTX_set_cipher_list(ossock->ossl_ctx, PJ_SSL_SOCK_OSSL_CIPHERS);
        if (ret < 1) {
            return GET_SSL_STATUS(ssock);
        }
...
#    if !USING_LIBRESSL && !defined(OPENSSL_NO_EC) && \
    ...
    if (SSL_is_server(ossock->ossl_ssl)) {
        ret =
          SSL_set1_curves(ossock->ossl_ssl, curves, ssock->param.curves_num);
        if (ret < 1)
            return GET_SSL_STATUS(ssock);
    } else {
...
#        if !USING_BORINGSSL
        if (ssock->is_server) {
            ...
            ret =
              SSL_set1_sigalgs_list(ossock->ossl_ssl, ssock->param.sigalgs.ptr);
        }
#        else
                          ...
                          pj_bool_t get_pem)
{
    ...
    if (ci->version >= 3) {
        names =
          (GENERAL_NAMES*)X509_get_ext_d2i(x, NID_subject_alt_name, NULL, NULL);
    }
    if (names) {
        unsigned i, cnt;
        ...
        ci->subj_alt_name.entry =
          pj_pool_calloc(pool, cnt, sizeof(*ci->subj_alt_name.entry));
                                              ...
                                              pj_bool_t get_pem)
{
    ...
    ci->raw_chain.cert_raw =
      (pj_str_t*)pj_pool_calloc(pool, sk_X509_num(chain), sizeof(pj_str_t));
                              ...
                              pj_bool_t is_verify)
{
    ...
    if (chain) {
        ...
        ssl_update_remote_cert_chain_info(ssock->info_pool, remote_cert_info,
                                          chain, PJ_TRUE);
    } else {
...
#    if OPENSSL_VERSION_NUMBER >= 0x10100000L
        if (ssock->is_server && ssock->ssl_state != SSL_STATE_ESTABLISHED) {
            ...
            PJ_LOG(5, (THIS_FILE,
                       "Session info: reused=%d, resumable=%d, "
                       "timeout=%d",
