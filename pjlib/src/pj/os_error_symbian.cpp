...
static const struct
{
    ...
    PJ_BUILD_ERR(KErrNoProtocolOpt,
                 "The specified server refuses the selected protocol"),
    PJ_BUILD_ERR(KErrUrgentData, ""),
    PJ_BUILD_ERR(KErrWouldBlock,
                 "Conflicts with KErrExtended, but cannot occur in practice"),
...
PJ_DECL(int)
platform_strerror(pj_os_err_type os_errcode, char* buf, pj_size_t bufsize);
PJ_END_DECL
