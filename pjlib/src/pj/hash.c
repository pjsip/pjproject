...
PJ_DEF(pj_uint32_t)
pj_hash_calc(pj_uint32_t hash, const void* key, unsigned keylen)
{
    PJ_CHECK_STACK();
...
PJ_DEF(pj_hash_table_t*) pj_hash_create(pj_pool_t* pool, unsigned size)
{
    ...
    PJ_LOG(6, ("hashtbl", "hash table %p created from pool %s", h,
    ...
    h->rows = table_size;
    h->table = (pj_hash_entry**)pj_pool_calloc(pool, table_size + 1,
                                               sizeof(pj_hash_entry*));
    return h;
}

static pj_hash_entry** find_entry(pj_pool_t* pool, pj_hash_table_t* ht,
                                  const void* key, unsigned keylen, void* val,
                                  pj_uint32_t* hval, void* entry_buf,
                                  pj_bool_t lower)
{
    pj_uint32_t hash;
    ...
    for (p_entry = &ht->table[hash & ht->rows], entry = *p_entry; entry;
    ...
    if (entry_buf) {
        ...
        PJ_LOG(
          6, ("hashtbl", "%p: New p_entry %p created, pool used=%u, cap=%u", ht,
              entry, pj_pool_get_used_size(pool), pj_pool_get_capacity(pool)));
    }
    entry->next = NULL;
    entry->hash = hash;
    if (pool) {
...
static void hash_set(pj_pool_t* pool, pj_hash_table_t* ht, const void* key,
                     unsigned keylen, pj_uint32_t hval, void* value,
                     void* entry_buf, pj_bool_t lower)
{
    pj_hash_entry** p_entry;

    p_entry = find_entry(pool, ht, key, keylen, value, &hval, entry_buf, lower);
    if (*p_entry) {
        if (value == NULL) {
            /* delete entry */
