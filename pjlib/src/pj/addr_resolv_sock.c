...
#    endif
    ...
    PJ_ASSERT_RETURN(
      af == PJ_AF_INET || af == PJ_AF_INET6 || af == PJ_AF_UNSPEC, PJ_EINVAL);
...
#    if PJ_WIN32_WINCE
    ...
    if ((af == PJ_AF_INET || af == PJ_AF_UNSPEC) &&
        pj_inet_pton(PJ_AF_INET, nodename, &ai[0].ai_addr.ipv4.sin_addr) ==
          PJ_SUCCESS)
    {
        af = PJ_AF_INET;
        has_addr = PJ_TRUE;
...
#    if defined(PJ_GETADDRINFO_USE_CFHOST) && PJ_GETADDRINFO_USE_CFHOST != 0
    hostname = CFStringCreateWithCStringNoCopy(
      kCFAllocatorDefault, nodecopy, kCFStringEncodingASCII, kCFAllocatorNull);
    ...
    if (CFHostStartInfoResolution(hostRef, kCFHostAddresses, nil)) {
        ...
        if (addrRef != nil) {
            ...
            for (idx = 0; idx < naddr && i < *count; idx++) {
                ...
                addr = (struct sockaddr*)CFDataGetBytePtr(
                  CFArrayGetValueAtIndex(addrRef, idx));
...
#    endif
        ...
        *count = 0;
        ...
        for (i = 0; he.h_addr_list[i] && *count < max_count; ++i) {
            ...
            pj_memcpy(&ai[*count].ai_addr.ipv4.sin_addr, he.h_addr_list[i],
                      he.h_length);
