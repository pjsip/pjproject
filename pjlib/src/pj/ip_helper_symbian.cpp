...
static pj_status_t rsock_enum_interface(int af, unsigned* p_cnt,
                                        pj_sockaddr ifs[])
{
    TInt rc;
    RSocket rSock;
    ...
    if (PjSymbianOS::Instance()->Connection()) {
        rc =
          rSock.Open(PjSymbianOS::Instance()->SocketServ(), af, PJ_SOCK_DGRAM,
                     KProtocolInetUdp, *PjSymbianOS::Instance()->Connection());
    } else {
        rc = rSock.Open(PjSymbianOS::Instance()->SocketServ(), af,
                        PJ_SOCK_DGRAM, KProtocolInetUdp);
    }
    ...
    for (i = 0; i < *p_cnt && rSock.GetOpt(KSoInetNextInterface, KSolInetIfCtrl,
                                           info) == KErrNone;)
    {
        ...
        if (1) {
            ...
            if (PjSymbianOS::Addr2pj(iAddress, a, &namelen, PJ_FALSE) ==
                PJ_SUCCESS) {
        ...
        if (PjSymbianOS::Addr2pj(iAddress, ifs[i], &namelen, PJ_TRUE) !=
            PJ_SUCCESS) {
            continue;
        }
