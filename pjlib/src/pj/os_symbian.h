...
class CPjSocketReader : public CActive {
   public:
    // Construct.
    static CPjSocketReader* NewL(CPjSocket& sock,
                                 unsigned max_len = CPjSocket::MAX_LEN);

    // Destroy;
    ~CPjSocketReader();

    // Start asynchronous read from the socket.
    void StartRecv(void (*cb)(void* key) = NULL, void* key = NULL,
                   TDes8* aDesc = NULL, TUint flags = 0);

    // Start asynchronous read from the socket.
    void StartRecvFrom(void (*cb)(void* key) = NULL, void* key = NULL,
                       TDes8* aDesc = NULL, TUint flags = 0,
...
class PjSymbianOS {
   public:
    ...
    static inline pj_status_t Addr2pj(
      const TInetAddr& sym_addr, pj_sockaddr& pj_addr, int* addr_len,
      pj_bool_t convert_ipv4_mapped_addr = PJ_FALSE)
    {
        TUint fam = sym_addr.Family();
        pj_bzero(&pj_addr, *addr_len);
        if (fam == PJ_AF_INET || (convert_ipv4_mapped_addr &&
                                  fam == PJ_AF_INET6 && sym_addr.IsV4Mapped()))
        {
            pj_addr.addr.sa_family = PJ_AF_INET;
            PJ_ASSERT_RETURN(*addr_len >= (int)sizeof(pj_sockaddr_in),
                             PJ_ETOOSMALL);
            ...
            *addr_len = sizeof(pj_sockaddr_in);
            ...
            PJ_ASSERT_RETURN(*addr_len >= (int)sizeof(pj_sockaddr_in6),
                             PJ_ETOOSMALL);
    ...
    static inline pj_status_t pj2Addr(const pj_sockaddr& pj_addr, int addrlen,
                                      TInetAddr& sym_addr)
    {
        if (pj_addr.addr.sa_family == PJ_AF_INET) {
            ...
            sym_addr.SetAddress(
              (TUint32)pj_ntohl(pj_addr.ipv4.sin_addr.s_addr));
