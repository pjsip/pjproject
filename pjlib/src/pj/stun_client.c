					      ...
					      pj_sockaddr_in mapped_addr[])
{
    ...
    /* Resolve servers. */
    if (pj_sockaddr_init(&srv_addr[0], srv1, port1) != 0) {
	mapped_status = PJ_STUN_ERR_RESOLVE; 
	goto on_error;
    }
    if (pj_sockaddr_init(&srv_addr[1], srv2, port2) != 0) {
	mapped_status = PJ_STUN_ERR_RESOLVE;
	goto on_error;
    }
	...
	for (i=0; i<sock_cnt && mapped_status==0; ++i) {
		...
		/* Send! */
		rc = pj_sock_sendto(sock[i], out_msg, out_msg_len, 0,
				    (pj_sockaddr_t*)&srv_addr[j], 
				    sizeof(pj_sockaddr_in));
		if (rc != (int)out_msg_len) {
		    PJ_LOG(4,(THIS_FILE, 
	     ...
	     pj_gettimeofday(&now)) 
	{
		...
		if (!FD_ISSET(sock[i], &r))
		    continue;

		len = pj_sock_recvfrom( sock[i], recv_buf, 
					sizeof(recv_buf), 0,
					(pj_sockaddr_t*)&addr,
					&addrlen);
