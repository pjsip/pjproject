...
static char* ver_info(pj_uint32_t ver, char* buf)
{
    ...
    sprintf(buf, "-%u.%u", (ver & 0xFF000000) >> 24, (ver & 0x00FF0000) >> 16);
    len = strlen(buf);
...
static pj_uint32_t parse_version(char* str)
{
    ...
    for (found_idx = pj_strtok(&in_str, &delim, &token, 0), i = 0;
         found_idx != in_str.slen && i < maxtok; ++i,
        found_idx = pj_strtok(&in_str, &delim, &token, found_idx + token.slen))
    {
        int n;
...
#    if _WIN32_WCE <= 500
    si.sdk_ver = ((_WIN32_WCE / 100) << 24) |
                 (((_WIN32_WCE % 100) / 10) << 16) | ((_WIN32_WCE % 10) << 8);
#    else
...
#    endif
     ...
     */
    si.sdk_ver = ((_MSC_VER / 100) << 24) | (((_MSC_VER % 100) / 10) << 16) |
                 ((_MSC_VER % 10) << 8);
     ...
     */
    {
        ...
        cnt = pj_ansi_snprintf(tmp, sizeof(tmp), "%s%s%s%s%s%s%s",
                               si.os_name.ptr, ver_info(si.os_ver, os_ver),
                               (si.machine.slen ? "/" : ""), si.machine.ptr,
                               (si.sdk_name.slen ? "/" : ""), si.sdk_name.ptr,
