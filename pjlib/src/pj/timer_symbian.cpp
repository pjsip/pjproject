...
class CPjTimerEntry : public CActive {
   public:
    ...
    static CPjTimerEntry* NewL(pj_timer_heap_t* timer_heap,
                               pj_timer_entry* entry, const pj_time_val* delay);
...
static pj_status_t add_entry(pj_timer_heap_t* th, CPjTimerEntry* entry)
{
    ...
    PJ_ASSERT_RETURN(
      (slot >= 0) && (slot < (int)th->max_size) && (th->entries[slot] == NULL),
      PJ_EBUG);
...
CPjTimerEntry::CPjTimerEntry(pj_timer_heap_t* timer_heap, pj_timer_entry* entry)
    : CActive(PJ_SYMBIAN_TIMER_PRIORITY),
      entry_(entry),
