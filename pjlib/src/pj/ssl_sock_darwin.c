...
static pj_status_t pj_status_from_err(darwinssl_sock_t* dssock, const char* msg,
                                      OSStatus err)
{
    ...
    if (__builtin_available(macOS 10.3, iOS 11.3, *)) {
        ...
        PJ_LOG(3, (THIS_FILE, "Darwin SSL error %s [%d]: %s", (msg ? msg : ""),
                   err, CFStringGetCStringPtr(errmsg, kCFStringEncodingUTF8)));
        CFRelease(errmsg);
    }
...
static OSStatus SocketWrite(SSLConnectionRef connection, const void* data,
...
static pj_status_t set_cert(darwinssl_sock_t* dssock, pj_ssl_cert_t* cert)
{
    ...
    {
        PJ_LOG(3, (THIS_FILE,
                   "Ignoring supplied private key. Private key "
...
#    endif
        ...
        if (!identity) {
            PJ_LOG(2, (THIS_FILE,
                       "Failed extracting identity from "
                       "the cert file"));
            return PJ_EINVAL;
        }
...
static pj_status_t ssl_create(pj_ssl_sock_t* ssock)
{
    ...
    ssl_ctx = SSLCreateContext(
      NULL, (ssock->is_server ? kSSLServerSide : kSSLClientSide),
      kSSLStreamType);
    if (ssl_ctx == NULL)
        return PJ_ENOMEM;
    ...
    if (ssock->param.proto == PJ_SSL_SOCK_PROTO_DEFAULT) {
        ...
        ssock->param.proto = PJ_SSL_SOCK_PROTO_ALL & ~PJ_SSL_SOCK_PROTO_SSL2;
    }
...
static pj_status_t verify_cert(darwinssl_sock_t* dssock, pj_ssl_cert_t* cert)
{
    ...
    if (ca_data) {
        ...
        if (!ca_cert) {
            PJ_LOG(2, (THIS_FILE,
                       "Failed creating certificate from "
    ...
    err = SecTrustEvaluateAsync(
      trust, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),
...
static pj_status_t ssl_do_handshake(pj_ssl_sock_t* ssock)
{
    ...
    if (ret == errSSLServerAuthCompleted || ret == errSSLClientAuthCompleted) {
