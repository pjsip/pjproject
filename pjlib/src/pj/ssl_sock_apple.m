...
pj_status_t ssl_network_event_poll()
{
    ...
    while (!pj_list_empty(&event_mgr->event_list)) {
        ...
        {
            PJ_LOG(3, (THIS_FILE,
                       "Warning: Discarding SSL event type %d of "
                       "a closing socket %p",
                       event->type, ssock));
            ...
            if (pj_grp_lock_get_ref(ssock->param.grp_lock) > 0) {
                ...
                PJ_LOG(3, (THIS_FILE,
                           "Warning: Discarding SSL event type %d "
                           " of a destroyed socket %p",
                           event->type, ssock));
                event->type = EVENT_DISCARD;
            }
        }
        ...
        switch (event->type) {
        case EVENT_ACCEPT:
            ret = ssock_on_accept_complete(
              event->ssock, PJ_INVALID_SOCKET, event->body.accept_ev.newconn,
              &event->body.accept_ev.src_addr,
              event->body.accept_ev.src_addr_len, event->body.accept_ev.status);
            break;
        case EVENT_CONNECT:
        ...
        case EVENT_DATA_READ:
            ret = ssock_on_data_read(
              event->ssock, event->body.data_read_ev.data,
              event->body.data_read_ev.size, event->body.data_read_ev.status,
...
static pj_status_t pj_status_from_err(applessl_sock_t* assock, const char* msg,
                                      OSStatus err)
{
    ...
    PJ_LOG(3, (THIS_FILE, "Apple SSL error %s [%d]: %s", (msg ? msg : ""), err,
...
static pj_status_t create_data_from_file(CFDataRef* data, pj_str_t* fname,
                                         pj_str_t* path)
{
    CFURLRef file;
    ...
    if (path) {
        ...
        path_str =
          CFStringCreateWithBytes(NULL, (const UInt8*)path->ptr, path->slen,
        ...
        path_str =
          CFStringCreateWithBytes(NULL, (const UInt8*)fname->ptr, fname->slen,
                                  kCFStringEncodingUTF8, false);
        if (!path_str) {
            CFRelease(filepath);
            return PJ_ENOMEM;
        ...
        file = CFURLCreateCopyAppendingPathComponent(NULL, filepath, path_str,
                                                     false);
        CFRelease(path_str);
        CFRelease(filepath);
    } else {
        file = CFURLCreateFromFileSystemRepresentation(
          NULL, (const UInt8*)fname->ptr, fname->slen, false);
    }
    ...
    nbytes = CFReadStreamRead(read_stream, data_buf, sizeof(data_buf));
    if (nbytes > 0)
    ...
    *p_identity = NULL;
    ...
    if (cert_data) {
        if (cert->privkey_pass.slen) {
            password = CFStringCreateWithBytes(
              NULL, (const UInt8*)cert->privkey_pass.ptr,
              cert->privkey_pass.slen, kCFStringEncodingUTF8, false);
...
#    else
        {
            ...
            for (i = 0; i < PJ_ARRAY_SIZE(ext_format); i++) {
                items = NULL;
                err = SecItemImport(cert_data, NULL, &ext_format[i], &ext_type,
                                    0, &key_params, NULL, &items);
                if (err == noErr && items) {
                    break;
                }
            }
...
#    if !TARGET_OS_IPHONE
            ...
            {
                err = SecIdentityCreateWithCertificate(
                  NULL, (SecCertificateRef)item, &identity);
                if (err != noErr) {
...
static pj_status_t verify_cert(applessl_sock_t* assock, pj_ssl_cert_t* cert)
{
    ...
    if (trust && cert && cert->CA_file.slen) {
        status =
          create_data_from_file(&ca_data, &cert->CA_file,
                                (cert->CA_path.slen ? &cert->CA_path : NULL));
        if (status != PJ_SUCCESS)
            PJ_LOG(2, (THIS_FILE, "Failed reading CA file"));
    } else if (trust && cert && cert->CA_buf.slen) {
        ca_data =
          CFDataCreate(NULL, (const UInt8*)cert->CA_buf.ptr, cert->CA_buf.slen);
        if (!ca_data)
                                ...
                                pj_ssize_t* size, unsigned flags)
{
    ...
    nw_connection_send(
      assock->connection, content, NW_CONNECTION_DEFAULT_MESSAGE_CONTEXT, true,
      ^(nw_error_t error) {
        event_t event;
...
static pj_status_t network_start_read(pj_ssl_sock_t* ssock,
                                      unsigned async_count, unsigned buff_size,
                                      void* readbuf[], pj_uint32_t flags)
{
    applessl_sock_t* assock = (applessl_sock_t*)ssock;
    unsigned i;
          ...
          ^(dispatch_data_t content, nw_content_context_t context,
            ...
            dispatch_block_t schedule_next_receive = ^{
              ...
              if (!error && !is_complete && assock->connection) {
                  network_start_read(ssock, async_count, buff_size, readbuf,
                                     flags);
              }
            };
            ...
            if (content) {
                dispatch_data_apply(
                  content, ^(dispatch_data_t region, size_t offset,
...
static pj_status_t network_get_localaddr(pj_ssl_sock_t* ssock,
                                         pj_sockaddr_t* addr, int* namelen)
{
    applessl_sock_t* assock = (applessl_sock_t*)ssock;
    nw_path_t path;
                                         ...
                                         nw_parameters_t* p_params)
{
    ...
    if (ssock->param.proto == PJ_SSL_SOCK_PROTO_DEFAULT) {
        ssock->param.proto = PJ_SSL_SOCK_PROTO_TLS1 | PJ_SSL_SOCK_PROTO_TLS1_1 |
    ...
    if (ssock->cert) {
        pj_status_t status =
          create_identity_from_cert(assock, ssock->cert, &assock->identity);
        if (status != PJ_SUCCESS)
            return status;
    }
    ...
    configure_tls = ^(nw_protocol_options_t tls_options) {
      ...
      sec_protocol_options_set_min_tls_protocol_version(sec_options, min_proto);
      sec_protocol_options_set_max_tls_protocol_version(sec_options, max_proto);
      ...
      if (ssock->param.ciphers_num > 0) {
          ...
          for (i = 0; i < ssock->param.ciphers_num; i++) {
              sec_protocol_options_append_tls_ciphersuite(
                sec_options, (tls_ciphersuite_t)ssock->param.ciphers[i]);
          }
      }

      if (!ssock->is_server && ssock->param.server_name.slen) {
          sec_protocol_options_set_tls_server_name(
            sec_options, ssock->param.server_name.ptr);
      }
                                            ...
                                            void* connection)
{
    ...
    nw_connection_set_state_changed_handler(
      assock->connection, ^(nw_connection_state_t state, nw_error_t error) {
        pj_status_t status = PJ_SUCCESS;
...
#    endif
        ...
        if (state == nw_connection_state_ready) {
            if (ssock->is_server) {
                ...
                prot_meta =
                  nw_connection_copy_protocol_metadata(connection, tls_def);
        ...
        if (call_cb) {
            ...
            if (ssock->is_server && status == PJ_SUCCESS) {
                status =
                  network_start_read(ssock, ssock->param.async_cnt,
...
static pj_status_t network_start_accept(pj_ssl_sock_t* ssock, pj_pool_t* pool,
                                        const pj_sockaddr_t* localaddr,
                                        int addr_len,
                                        const pj_ssl_sock_param* newsock_param)
{
    ...
    nw_listener_set_state_changed_handler(
      assock->listener, ^(nw_listener_state_t state, nw_error_t error) {
    ...
    nw_listener_set_new_connection_handler(
      assock->listener, ^(nw_connection_t connection) {
...
static pj_status_t
network_start_connect(pj_ssl_sock_t* ssock,
                      pj_ssl_start_connect_param* connect_param)
{
    ...
    status = network_create_params(ssock, localaddr, port_range, &parameters);
    if (status != PJ_SUCCESS)
        return status;
...
static void ssl_destroy(pj_ssl_sock_t* ssock)
{
    ...
    if (assock->listener) {
        ...
        if (assock->lis_state != nw_listener_state_cancelled) {
            PJ_LOG(3, (THIS_FILE,
                       "Warning: Failed to cancel SSL listener "
                       "%p %d",
                       assock, assock->lis_state));
        }
        nw_release(assock->listener);
        assock->listener = nil;
    }
                                    ...
                                    CFTypeRef* value)
{
    ...
    key[0] = (void*)oid;
    key_arr = CFArrayCreate(NULL, (const void**)key, 1, &kCFTypeArrayCallBacks);
...
#    if !TARGET_OS_IPHONE
    {
        ...
        dict =
          get_cert_oid(cert, kSecOIDX509V1IssuerName, (CFTypeRef*)&issuer_vals);
        if (dict) {
...
#    if !TARGET_OS_IPHONE
    {
        ...
        dict = get_cert_oid(cert, kSecOIDX509V1Version, (CFTypeRef*)&version);
        if (dict) {
...
#    if !TARGET_OS_IPHONE
    {
        ...
        dict =
          get_cert_oid(cert, kSecOIDX509V1SubjectName, (CFTypeRef*)&subject);
        if (dict) {
            subject_info = CFStringCreateMutable(NULL, 0);

            get_info_and_cn(subject, subject_info, &str);

            CFStringGetCString(subject_info, buf, bufsize,
...
#    if !TARGET_OS_IPHONE
    {
        ...
        ci->subj_alt_name.entry = pj_pool_calloc(
          pool, CFArrayGetCount(altname), sizeof(*ci->subj_alt_name.entry));
        ...
        for (i = 0; i < CFArrayGetCount(altname); ++i) {
            ...
            label =
              (CFStringRef)CFDictionaryGetValue(item, kSecPropertyKeyLabel);
            if (CFGetTypeID(label) != CFStringGetTypeID())
                continue;

            value =
              (CFStringRef)CFDictionaryGetValue(item, kSecPropertyKeyValue);
