                                       ...
                                       ioqueue_accept_rec* accept_overlapped)
{
     ...
     */
    status =
      setsockopt(accept_overlapped->newsock, SOL_SOCKET,
                 SO_UPDATE_ACCEPT_CONTEXT, (char*)&key->hnd, sizeof(SOCKET));
    ...
    if (accept_overlapped->addrlen) {
        GetAcceptExSockaddrs(accept_overlapped->accept_buf, 0, ACCEPT_ADDR_LEN,
                             ACCEPT_ADDR_LEN, &local, &locallen, &remote,
                             &remotelen);
...
static int check_connecting(pj_ioqueue_t* ioqueue)
{
    if (ioqueue->connecting_count) {
        ...
        for (count = 0; count < PJ_IOQUEUE_MAX_EVENTS_IN_SINGLE_POLL - 1;
             ++count) {
            DWORD result;

            result = WaitForMultipleObjects(
              ioqueue->connecting_count, ioqueue->connecting_handles, FALSE, 0);
 ...
 */
static pj_bool_t poll_iocp(HANDLE hIocp, DWORD dwTimeout, pj_ssize_t* p_bytes,
                           pj_ioqueue_key_t** p_key)
{
    ...
    rcGetQueued = GetQueuedCompletionStatus(hIocp, &dwBytesTransferred, &dwKey,
                                            (OVERLAPPED**)&pOv, dwTimeout);
                ...
                void* buffer, pj_ssize_t* length, pj_uint32_t flags)
{
    ...
    rc =
      WSARecv((SOCKET)key->hnd, &op_key_rec->overlapped.wsabuf, 1, &bytesRead,
              &dwFlags, &op_key_rec->overlapped.overlapped, NULL);
    if (rc == SOCKET_ERROR) {
                  ...
                  const pj_sockaddr_t* addr, int addrlen)
{
    ...
    if ((flags & PJ_IOQUEUE_ALWAYS_ASYNC) == 0) {
        rc = WSASendTo((SOCKET)key->hnd, &op_key_rec->overlapped.wsabuf, 1,
                       &bytesWritten, dwFlags, addr, addrlen, NULL, NULL);
        if (rc == 0) {
            *length = bytesWritten;
            return PJ_SUCCESS;
        } else {
...
#    endif
    ...
    rc =
      AcceptEx((SOCKET)key->hnd, (SOCKET)op_key_rec->accept.newsock,
               op_key_rec->accept.accept_buf, 0, ACCEPT_ADDR_LEN,
               ACCEPT_ADDR_LEN, &bytesReceived, &op_key_rec->accept.overlapped);
...
pj_ioqueue_is_pending(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key)
{
    ...
    rc = GetOverlappedResult(key->hnd, (LPOVERLAPPED)op_key, &bytesTransferred,
                             FALSE);
                           ...
                           pj_ssize_t bytes_status)
{
    ...
    rc = PostQueuedCompletionStatus(key->ioqueue->iocp, bytes_status, (long)key,
                                    (OVERLAPPED*)op_key);
    if (rc == FALSE) {
        return PJ_RETURN_OS_ERROR(GetLastError());
    }
