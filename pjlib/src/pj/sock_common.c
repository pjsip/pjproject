 ...
 */
PJ_DEF(char*) pj_inet_ntop2(int af, const void* src, char* dst, int size)
{
    pj_status_t status;
                  ...
                  unsigned flags)
{
    ...
    status =
      pj_inet_ntop(h->sa_family, pj_sockaddr_get_addr(addr), txt, sizeof(txt));
    ...
    if (flags & WITH_PORT) {
        pj_ansi_snprintf(port, sizeof(port), ":%d", pj_sockaddr_get_port(addr));
    } else {
        port[0] = '\0';
    }

    pj_ansi_snprintf(buf, size, "%s%s%s%s", bquote, txt, equote, port);
 ...
 */
PJ_DEF(pj_status_t)
pj_sockaddr_in_set_str_addr(pj_sockaddr_in* addr, const pj_str_t* str_addr)
{
    PJ_CHECK_STACK();
...
PJ_DEF(pj_status_t)
pj_sockaddr_set_str_addr(int af, pj_sockaddr* addr, const pj_str_t* str_addr)
{
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pj_sockaddr_in_init(pj_sockaddr_in* addr, const pj_str_t* str_addr,
                    pj_uint16_t port)
{
...
pj_sockaddr_cmp(const pj_sockaddr_t* addr1, const pj_sockaddr_t* addr2)
{
    ...
    /* Compare addresses */
    result = pj_memcmp(pj_sockaddr_get_addr(a1), pj_sockaddr_get_addr(a2),
...
PJ_DEF(void*) pj_sockaddr_get_addr(const pj_sockaddr_t* addr)
{
    ...
    PJ_ASSERT_RETURN(
      a->addr.sa_family == PJ_AF_INET || a->addr.sa_family == PJ_AF_INET6,
      NULL);
...
PJ_DEF(unsigned) pj_sockaddr_get_addr_len(const pj_sockaddr_t* addr)
{
    const pj_sockaddr* a = (const pj_sockaddr*)addr;
    PJ_ASSERT_RETURN(
      a->addr.sa_family == PJ_AF_INET || a->addr.sa_family == PJ_AF_INET6, 0);
    return a->addr.sa_family == PJ_AF_INET6 ? sizeof(pj_in6_addr) :
                                              sizeof(pj_in_addr);
}
...
PJ_DEF(unsigned) pj_sockaddr_get_len(const pj_sockaddr_t* addr)
{
    ...
    PJ_ASSERT_RETURN(
      a->addr.sa_family == PJ_AF_INET || a->addr.sa_family == PJ_AF_INET6, 0);
    return a->addr.sa_family == PJ_AF_INET6 ? sizeof(pj_sockaddr_in6) :
 ...
 */
PJ_DEF(void) pj_sockaddr_copy_addr(pj_sockaddr* dst, const pj_sockaddr* src)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_sockaddr_synthesize(int dst_af, pj_sockaddr_t* dst, const pj_sockaddr_t* src)
{
 ...
 */
PJ_DEF(void) pj_sockaddr_in_set_port(pj_sockaddr_in* addr, pj_uint16_t hostport)
{
 ...
 * Set IPv4 address
 */
PJ_DEF(void) pj_sockaddr_in_set_addr(pj_sockaddr_in* addr, pj_uint32_t hostaddr)
{
...
pj_sockaddr_parse2(int af, unsigned options, const pj_str_t* str,
                   pj_str_t* p_hostpart, pj_uint16_t* p_port, int* raf)
{
    ...
    PJ_ASSERT_RETURN(
      (af == PJ_AF_INET || af == PJ_AF_INET6 || af == PJ_AF_UNSPEC) &&
        options == 0 && str != NULL,
      PJ_EINVAL);
                  ...
                  pj_sockaddr* addr)
{
    ...
    PJ_ASSERT_RETURN(
      af == PJ_AF_UNSPEC || af == PJ_AF_INET || af == PJ_AF_INET6, PJ_EINVAL);
                         ...
                         pj_bool_t auto_retry, unsigned* buf_size)
{
    ...
    PJ_ASSERT_RETURN(sockfd != PJ_INVALID_SOCKET && buf_size && *buf_size > 0 &&
                       (optname == pj_SO_RCVBUF() || optname == pj_SO_SNDBUF()),
                     PJ_EINVAL);

    size_len = sizeof(cur_size);
    status = pj_sock_getsockopt(sockfd, pj_SOL_SOCKET(), optname, &cur_size,
                                &size_len);
    if (status != PJ_SUCCESS)
        return status;
    ...
    for (i = 0; i < (MAX_TRY - 1); ++i) {
        ...
        status = pj_sock_setsockopt(sockfd, pj_SOL_SOCKET(), optname, &try_size,
                                    sizeof(try_size));
        if (status == PJ_SUCCESS) {
                  ...
                  unsigned flag)
{
    ...
    if (flag & WITH_PORT) {
        pj_ansi_snprintf(buf, size, "%s%.*s%s:%d", bquote, (int)host_str->slen,
                         host_str->ptr, equote, port);
    } else {
        pj_ansi_snprintf(buf, size, "%s%.*s%s", bquote, (int)host_str->slen,
                         host_str->ptr, equote);
    }
    return buf;
}
