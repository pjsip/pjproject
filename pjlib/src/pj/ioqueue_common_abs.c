 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_set_lock(pj_ioqueue_t* ioqueue, pj_lock_t* lock,
...
static pj_status_t ioqueue_init_key(pj_pool_t* pool, pj_ioqueue_t* ioqueue,
                                    pj_ioqueue_key_t* key, pj_sock_t sock,
                                    pj_grp_lock_t* grp_lock, void* user_data,
                                    const pj_ioqueue_callback* cb)
{
    ...
    rc = pj_sock_getsockopt(sock, pj_SOL_SOCKET(), pj_SO_TYPE(), &key->fd_type,
                            &optlen);
    if (rc != PJ_SUCCESS)
...
#    if (defined(PJ_HAS_SO_ERROR) && PJ_HAS_SO_ERROR != 0)
         ...
         */
        {
            ...
            int gs_rc =
              pj_sock_getsockopt(h->fd, SOL_SOCKET, SO_ERROR, &value, &vallen);
            if (gs_rc != 0) {
                /* Argh!! What to do now???
...
#    else
         ...
         */
        {
            ...
            status =
              pj_sock_getpeername(h->fd, (struct sockaddr*)&addr, &addrlen);
        }
#    endif
        ...
        if (key_has_pending_write(h)) {
            ...
            if (write_op->op == PJ_IOQUEUE_OP_SEND) {
                send_rc = pj_sock_send(h->fd, write_op->buf + write_op->written,
                                       &sent, write_op->flags);
                /* Can't do this. We only clear "op" after we're finished
                ...
                int retry = 2;
                while (--retry >= 0) {
                    send_rc = pj_sock_sendto(
                      h->fd, write_op->buf + write_op->written, &sent,
                      write_op->flags, &write_op->rmt_addr,
                      write_op->rmt_addrlen);
#if defined(PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT) && \
  PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT != 0
                    /* Special treatment for dead UDP sockets here, see ticket
                     * #1107 */
                        ...
                        h->fd_type == pj_SOCK_DGRAM())
                    {
                        PJ_PERROR(
                          4, (THIS_FILE, send_rc,
                              "Send error for socket %d, retrying", h->fd));
                ...
                h->fd_type == pj_SOCK_DGRAM())
            {
                 ...
                 * hold the mutex while calling the callback) */
                if (h->allow_concurrent) {
                    /* concurrency may be changed while we're in the callback,
                     * so save it to a flag.
                     */
                    has_lock = PJ_FALSE;
                    pj_ioqueue_unlock_key(h);
                    PJ_RACE_ME(5);
                ...
                if (h->cb.on_write_complete && !IS_CLOSING(h)) {
                    (*h->cb.on_write_complete)(
                      h, (pj_ioqueue_op_key_t*)write_op, write_op->written);
                }
                                      ...
                                      pj_ioqueue_key_t* h)
{
    ...
    if (!pj_list_empty(&h->accept_list)) {
        ...
        rc = pj_sock_accept(h->fd, accept_op->accept_fd, accept_op->rmt_addr,
                            accept_op->addrlen);
        if (rc == PJ_SUCCESS && accept_op->local_addr) {
            rc = pj_sock_getsockname(*accept_op->accept_fd,
                                     accept_op->local_addr, accept_op->addrlen);
        }
      ...
      if (key_has_pending_read(h))
    {
        ...
        if (read_op->op == PJ_IOQUEUE_OP_RECV_FROM) {
            ...
            rc =
              pj_sock_recvfrom(h->fd, read_op->buf, &bytes_read, read_op->flags,
                               read_op->rmt_addr, read_op->rmt_addrlen);
        } else if (read_op->op == PJ_IOQUEUE_OP_RECV) {
            read_op->op = PJ_IOQUEUE_OP_NONE;
            rc = pj_sock_recv(h->fd, read_op->buf, &bytes_read, read_op->flags);
        } else {
...
pj_ioqueue_recv(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
                void* buffer, pj_ssize_t* length, unsigned flags)
{
    struct read_operation* read_op;
                    ...
                    pj_sockaddr_t* addr, int* addrlen)
{
    ...
    if ((flags & PJ_IOQUEUE_ALWAYS_ASYNC) == 0) {
        ...
        size = *length;
        status = pj_sock_recvfrom(key->fd, buffer, &size, flags, addr, addrlen);
        if (status == PJ_SUCCESS) {
            /* Yes! Data is available! */
            *length = size;
            return PJ_SUCCESS;
...
pj_ioqueue_send(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
                const void* data, pj_ssize_t* length, unsigned flags)
{
    struct write_operation* write_op;
    pj_status_t status;
    unsigned retry;
  ...
  PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT != 0
                    ...
                    key->fd_type == pj_SOCK_DGRAM())
                {
                    if (!restart_retry) {
                        PJ_PERROR(
                          4, (THIS_FILE, status,
                              "Send error for socket %d, retrying", key->fd));
    ...
    while (op_rec != (void*)&key->accept_list) {
        if (op_rec == (void*)op_key) {
            ...
            if (key->cb.on_accept_complete) {
                (*key->cb.on_accept_complete)(key, op_key, PJ_INVALID_SOCKET,
