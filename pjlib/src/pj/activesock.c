...
static void activesock_create_iphone_os_stream(pj_activesock_t* asock)
{
    if (ios_bg_support && asock->bg_setting && asock->stream_oriented) {
        ...
        {
            PJ_LOG(2, ("",
                       "Failed to configure TCP transport for VoIP "
...
PJ_DEF(void) pj_activesock_set_iphone_os_bg(pj_activesock_t* asock, int val)
{
                     ...
                     pj_activesock_t** p_asock)
{
    ...
    PJ_ASSERT_RETURN(
      sock_type == pj_SOCK_STREAM() || sock_type == pj_SOCK_DGRAM(), PJ_EINVAL);
                         ...
                         pj_activesock_t** p_asock, pj_sockaddr* bound_addr)
{
    ...
    status = pj_sock_socket(addr->addr.sa_family, pj_SOCK_DGRAM(), 0, &sock_fd);
    if (status != PJ_SUCCESS) {
        return status;
    }
    ...
    status = pj_activesock_create(pool, sock_fd, pj_SOCK_DGRAM(), opt, ioqueue,
                                  cb, user_data, p_asock);
    if (status != PJ_SUCCESS) {
        pj_sock_close(sock_fd);
        return status;
    }
                         ...
                         unsigned buff_size, pj_uint32_t flags)
{
    ...
    readbuf = (void**)pj_pool_calloc(pool, asock->async_count, sizeof(void*));
                          ...
                          pj_uint32_t flags)
{
    ...
    asock->read_op = (struct read_op*)pj_pool_calloc(pool, asock->async_count,
                             ...
                             unsigned buff_size, pj_uint32_t flags)
{
    ...
    return pj_activesock_start_recvfrom2(asock, pool, buff_size, readbuf,
                                         flags);
}
                              ...
                              pj_uint32_t flags)
{
    ...
    for (i = 0; i < asock->async_count; ++i) {
        ...
        status = pj_ioqueue_recvfrom(
          asock->key, &r->op_key, r->pkt, &size_to_read,
          PJ_IOQUEUE_ALWAYS_ASYNC | flags, &r->src_addr, &r->src_addr_len);
        PJ_ASSERT_RETURN(status != PJ_SUCCESS, PJ_EBUG);
    ...
    do {
        ...
        if (bytes_read > 0) {
            ...
            if (asock->read_type == TYPE_RECV && asock->cb.on_data_read) {
                ...
                ret = (*asock->cb.on_data_recvfrom)(
                  asock, r->pkt, r->size, &r->src_addr, r->src_addr_len,
                  PJ_SUCCESS);
            }
                    ...
                    -bytes_read != PJ_STATUS_FROM_OS(OSERR_ECONNRESET)))
        {
            ...
            if (asock->read_type == TYPE_RECV && asock->cb.on_data_read) {
                 ...
                 */
                // ret = (*asock->cb.on_data_read)(asock, (r->size?
                // r->pkt:NULL), 				r->size, status, &remainder);
                ret = (*asock->cb.on_data_read)(asock, r->pkt, r->size, status,
                                                &remainder);
                 ...
                 */
                if (status != PJ_SUCCESS) {
                    ret = (*asock->cb.on_data_recvfrom)(asock, NULL, 0, NULL, 0,
                                                        status);
                }
            }
        ...
        if (asock->read_type == TYPE_RECV) {
            status = pj_ioqueue_recv(key, op_key, r->pkt + r->size, &bytes_read,
                                     flags);
        } else {
            r->src_addr_len = sizeof(r->src_addr);
            status =
              pj_ioqueue_recvfrom(key, op_key, r->pkt + r->size, &bytes_read,
                                  flags, &r->src_addr, &r->src_addr_len);
        }
                                  ...
                                  pj_ioqueue_op_key_t* send_key)
{
    ...
    do {
        ...
        size = sd->len - sd->sent;
        status = pj_ioqueue_send(asock->key, send_key, sd->data + sd->sent,
                                 &size, sd->flags);
        if (status != PJ_SUCCESS) {
            /* Pending or error */
            break;
                     ...
                     const pj_sockaddr_t* addr, int addr_len)
{
    ...
    return pj_ioqueue_sendto(asock->key, send_key, data, size, flags, addr,
                             addr_len);
}
...
pj_activesock_start_accept(pj_activesock_t* asock, pj_pool_t* pool)
{
    ...
    asock->accept_op = (struct accept_op*)pj_pool_calloc(
      pool, asock->async_count, sizeof(struct accept_op));
    for (i = 0; i < asock->async_count; ++i) {
    ...
    do {
        if (status == asock->last_err && status != PJ_SUCCESS) {
            ...
            if (asock->err_counter >=
                ...
                PJ_LOG(3, ("",
                           "Received %d consecutive errors: %d for the accept()"
                ...
                {
                    (*asock->cb.on_accept_complete2)(
                      asock, accept_op->new_sock, &accept_op->rem_addr,
                      accept_op->rem_addr_len, PJ_ESOCKETSTOP);
                }
                return;
            }
        } else {
        ...
        if (status == PJ_SUCCESS &&
            (asock->cb.on_accept_complete2 || asock->cb.on_accept_complete))
        {
            pj_bool_t ret;

            /* Notify callback */
            if (asock->cb.on_accept_complete2) {
                ret = (*asock->cb.on_accept_complete2)(
                  asock, accept_op->new_sock, &accept_op->rem_addr,
                  accept_op->rem_addr_len, status);
            } else {
                ret = (*asock->cb.on_accept_complete)(
                  asock, accept_op->new_sock, &accept_op->rem_addr,
...
#    endif
        ...
        status =
          pj_ioqueue_accept(asock->key, op_key, &accept_op->new_sock, NULL,
                            &accept_op->rem_addr, &accept_op->rem_addr_len);
