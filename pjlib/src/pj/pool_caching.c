...
static pj_pool_t* cpool_create_pool(pj_pool_factory* pf, const char* name,
...
PJ_DEF(void)
pj_caching_pool_init(pj_caching_pool* cp, const pj_pool_factory_policy* policy,
                     pj_size_t max_capacity)
{
    ...
    pool =
      pj_pool_create_on_buf("cachingpool", cp->pool_buf, sizeof(cp->pool_buf));
                                    ...
                                    pj_pool_callback* callback)
{
    ...
    if (initial_size <= pool_sizes[START_SIZE]) {
        for (idx = START_SIZE - 1; idx >= 0 && pool_sizes[idx] >= initial_size;
             --idx)
            ;
        ++idx;
    } else {
    ...
    if (detail) {
        ...
        while (pool != (void*)&cp->used_list) {
            ...
            PJ_LOG(3, ("cachpool", "   %16s: %8d of %8d (%d%%) used",
                       pj_pool_getobjname(pool), pj_pool_get_used_size(pool),
                       pool_capacity,
                       pj_pool_get_used_size(pool) * 100 / pool_capacity));
        ...
        if (total_capacity) {
            PJ_LOG(3,
                   ("cachpool", "  Total %9d of %9d (%d %%) used!", total_used,
                    total_capacity, total_used * 100 / total_capacity));
        }
    }
