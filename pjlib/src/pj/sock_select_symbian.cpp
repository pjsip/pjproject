...
PJ_DEF(int)
pj_sock_select(int n, pj_fd_set_t* readfds, pj_fd_set_t* writefds,
               pj_fd_set_t* exceptfds, const pj_time_val* timeout)
{
    ...
    if (readfds) {
        ...
        do {
            ...
            for (i = 0; i < fds->count; ++i) {
                ...
                if (pjsock->Reader()) {
                    if (pjsock->Reader()->HasData() &&
                        !pjsock->Reader()->IsActive()) {
