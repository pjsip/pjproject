...
class PjUwpSocket {
   public:
    ...
    enum PjUwpSocketType GetType()
    {
        return sock_type;
    }
    enum PjUwpSocketState GetState()
    {
        return sock_state;
    }

    pj_sockaddr* GetLocalAddr()
    {
        return &local_addr;
    }
    pj_sockaddr* GetRemoteAddr()
    {
        return &remote_addr;
    }
    ...
    pj_status_t SendTo(const void* buf, pj_ssize_t* len,
   ...
   private:
    PjUwpSocket* CreateAcceptSocket(Windows::Networking::Sockets::StreamSocket ^
                                    stream_sock_);
                                             ...
                                             int* port)
{
    ...
    pj_ansi_to_unicode(tmp, pj_ansi_strlen(tmp), wtmp, PJ_INET6_ADDRSTRLEN);
    hostname =
      ref new Windows::Networking::HostName(ref new Platform::String(wtmp));
...
inline Microsoft::WRL::ComPtr<Windows::Storage::Streams::IBufferByteAccess>
GetBufferByteAccess(Windows::Storage::Streams::IBuffer ^ buffer)
{
    auto pUnk = reinterpret_cast<IUnknown*>(buffer);

    Microsoft::WRL::ComPtr<Windows::Storage::Streams::IBufferByteAccess>
      comBuff;
    pUnk->QueryInterface(__uuidof(Windows::Storage::Streams::IBufferByteAccess),
                         (void**)comBuff.ReleaseAndGetAddressOf());

    return comBuff;
}

inline void GetRawBufferFromIBuffer(Windows::Storage::Streams::IBuffer ^ buffer,
                                    unsigned char** pbuffer)
{
    Platform::Object ^ obj = buffer;
    Microsoft::WRL::ComPtr<IInspectable> insp(
      reinterpret_cast<IInspectable*>(obj));
    Microsoft::WRL::ComPtr<Windows::Storage::Streams::IBufferByteAccess>
      bufferByteAccess;
...
inline void CopyToIBuffer(unsigned char* buffSource, unsigned int copyByteCount,
                          Windows::Storage::Streams::IBuffer ^ buffer,
                          unsigned int writeStartPos = 0)
{
...
inline void CopyFromIBuffer(unsigned char* buffDestination,
                            unsigned int copyByteCount,
                            Windows::Storage::Streams::IBuffer ^ buffer,
                            unsigned int readStartPos = 0)
{
...
inline concurrency::task<void> complete_after(unsigned int timeout)
{
    ...
    // Create a call object that sets the completion event after the timer
    // fires.
    auto callback = new concurrency::call<int>([tce](int) { tce.set(); });
...
// did not complete.
template <typename T>
inline concurrency::task<T> cancel_after_timeout(
  concurrency::task<T> t, concurrency::cancellation_token_source cts,
  unsigned int timeout)
{
    // Create a task that returns true after the specified task completes.
    concurrency::task<bool> success_task = t.then([](T) { return true; });
    // Create a task that returns false after the specified timeout.
    concurrency::task<bool> failure_task =
      complete_after(timeout).then([] { return false; });
