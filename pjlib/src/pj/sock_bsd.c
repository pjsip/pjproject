...
#        endif
    ...
    if (type == PJ_SOCK_DGRAM) {
        ...
        rc =
          WSAIoctl(*sock, SIO_UDP_CONNRESET, &bNewBehavior,
    ...
    *sock = socket(af, type, proto);
    if (*sock == PJ_INVALID_SOCKET)
        ...
        if (type == pj_SOCK_STREAM()) {
            pj_sock_setsockopt(*sock, pj_SOL_SOCKET(), pj_SO_NOSIGPIPE(), &val,
                               sizeof(val));
        }
#    if defined(PJ_SOCK_HAS_IPV6_V6ONLY) && PJ_SOCK_HAS_IPV6_V6ONLY != 0
        if (af == PJ_AF_INET6) {
            pj_sock_setsockopt(*sock, PJ_SOL_IPV6, IPV6_V6ONLY, &val,
                               sizeof(val));
        }
#    endif
                   ...
                   void* optval, int* optlen)
{
    ...
    if (getsockopt(sock, level, optname, (char*)optval, (socklen_t*)optlen) !=
        0)
        return PJ_RETURN_OS_ERROR(pj_get_native_netos_error());
    else
        return PJ_SUCCESS;
