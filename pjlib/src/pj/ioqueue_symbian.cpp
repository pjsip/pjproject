...
class CIoqueueCallback : public CActive {
   public:
    static CIoqueueCallback* NewL(pj_ioqueue_t* ioqueue, pj_ioqueue_key_t* key,
                                  pj_sock_t sock, const pj_ioqueue_callback* cb,
                                  void* user_data);

    //
    // Start asynchronous recv() operation
    //
    pj_status_t StartRead(pj_ioqueue_op_key_t* op_key, void* buf,
                          pj_ssize_t* size, unsigned flags, pj_sockaddr_t* addr,
                          int* addrlen);

    //
    // Start asynchronous accept() operation.
    //
    pj_status_t StartAccept(pj_ioqueue_op_key_t* op_key, pj_sock_t* new_sock,
                            pj_sockaddr_t* local, pj_sockaddr_t* remote,
                            int* addrlen);
   ...
   private:
    ...
    CIoqueueCallback(pj_ioqueue_t* ioqueue, pj_ioqueue_key_t* key,
                     pj_sock_t sock, const pj_ioqueue_callback* cb,
                     void* user_data)
        : CActive(CActive::EPriorityStandard),
          ioqueue_(ioqueue),
          key_(key),
          sock_((CPjSocket*)sock),
          user_data_(user_data),
          aBufferPtr_(NULL, 0),
          type_(TYPE_NONE)
    {
...
CIoqueueCallback* CIoqueueCallback::NewL(pj_ioqueue_t* ioqueue,
                                         ...
                                         void* user_data)
{
    CIoqueueCallback* self =
      new CIoqueueCallback(ioqueue, key, sock, cb, user_data);
...
// Start asynchronous recv() operation
//
pj_status_t CIoqueueCallback::StartRead(pj_ioqueue_op_key_t* op_key, void* buf,
...
pj_status_t CIoqueueCallback::StartAccept(pj_ioqueue_op_key_t* op_key,
                                          pj_sock_t* new_sock,
                                          pj_sockaddr_t* local,
                                          pj_sockaddr_t* remote, int* addrlen)
{
    PJ_ASSERT_RETURN(IsActive() == false, PJ_EBUSY);
    PJ_ASSERT_RETURN(pending_data_.common_.op_key_ == NULL, PJ_EBUSY);

    // addrlen must be specified if local or remote is specified
    PJ_ASSERT_RETURN((!local && !remote) || (addrlen && *addrlen), PJ_EINVAL);
...
{
    CPjSocket* pjNewSock = new CPjSocket(
      get_pj_socket()->GetAf(), get_pj_socket()->GetSockType(), blank_sock_);
    int addrlen = 0;
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_set_lock(pj_ioqueue_t* ioq, pj_lock_t* lock, pj_bool_t auto_delete)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_register_sock(pj_pool_t* pool, pj_ioqueue_t* ioq, pj_sock_t sock,
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_set_user_data(pj_ioqueue_key_t* key, void* user_data,
                         void** old_data)
{
    if (old_data)
 ...
 */
PJ_DEF(void) pj_ioqueue_op_key_init(pj_ioqueue_op_key_t* op_key, pj_size_t size)
{
 ...
 */
PJ_DEF(pj_bool_t)
pj_ioqueue_is_pending(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key)
{
    return key->cbObj->get_op_key() == op_key && key->cbObj->IsActive();
}
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_post_completion(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
                           pj_ssize_t bytes_status)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_accept(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
                  pj_sock_t* new_sock, pj_sockaddr_t* local,
                  pj_sockaddr_t* remote, int* addrlen)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_connect(pj_ioqueue_key_t* key, const pj_sockaddr_t* addr,
                   int addrlen)
{
    pj_status_t status;
    ...
    // Convert address
    status = PjSymbianOS::pj2Addr(*(const pj_sockaddr*)addr, addrlen, inetAddr);
 ...
 */
PJ_DEF(int) pj_ioqueue_poll(pj_ioqueue_t* ioq, const pj_time_val* timeout)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_recv(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
                void* buffer, pj_ssize_t* length, pj_uint32_t flags)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_recvfrom(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
                    void* buffer, pj_ssize_t* length, pj_uint32_t flags,
                    pj_sockaddr_t* addr, int* addrlen)
{
    ...
    if (addr || addrlen) {
        ...
        if (sock->GetAf() == PJ_AF_INET) {
            PJ_ASSERT_RETURN(*addrlen >= (int)sizeof(pj_sockaddr_in),
                             PJ_EINVAL);
        } else if (sock->GetAf() == PJ_AF_INET6) {
            PJ_ASSERT_RETURN(*addrlen >= (int)sizeof(pj_sockaddr_in6),
                             PJ_EINVAL);
        }
    }
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_send(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
                const void* data, pj_ssize_t* length, pj_uint32_t flags)
{
    TRequestStatus reqStatus;
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_sendto(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
                  const void* data, pj_ssize_t* length, pj_uint32_t flags,
                  const pj_sockaddr_t* addr, int addrlen)
{
    TRequestStatus reqStatus;
    TPtrC8 aBuffer;
    TInetAddr inetAddr;
