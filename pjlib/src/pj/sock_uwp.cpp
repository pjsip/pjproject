...
ref class PjUwpSocketDatagramRecvHelper sealed {
    internal : PjUwpSocketDatagramRecvHelper(PjUwpSocket* uwp_sock_)
...
ref class PjUwpSocketListenerHelper sealed {
    internal : PjUwpSocketListenerHelper(PjUwpSocket* uwp_sock_)
...
PjUwpSocket* PjUwpSocket::CreateAcceptSocket(
  Windows::Networking::Sockets::StreamSocket ^ stream_sock_)
{
    ...
    new_sock->socket_reader =
      ref new DataReader(new_sock->stream_sock->InputStream);
    new_sock->socket_writer =
      ref new DataWriter(new_sock->stream_sock->OutputStream);
    ...
    wstr_addr_to_sockaddr(
      stream_sock_->Information->RemoteAddress->CanonicalName->Data(),
      stream_sock_->Information->RemotePort->Data(), &new_sock->remote_addr);
    wstr_addr_to_sockaddr(
      stream_sock_->Information->LocalAddress->CanonicalName->Data(),
      stream_sock_->Information->LocalPort->Data(), &new_sock->local_addr);
...
pj_status_t PjUwpSocket::SendImp(const void* buf, pj_ssize_t* len)
{
    ...
    if (is_blocking) {
        ...
        auto t =
          concurrency::create_task(socket_writer->StoreAsync(), cts_token);
        *len =
                                ...
                                const pj_sockaddr_t* to)
{
    ...
    auto t = concurrency::create_task(
      datagram_sock->GetOutputStreamAsync(hostname, port.ToString()),
      cts_token);
    pj_status_t status = PJ_SUCCESS;
...
pj_status_t PjUwpSocket::Recv(void* buf, pj_ssize_t* len)
{
    ...
    if (is_blocking) {
        ...
        *len = cancel_after_timeout(t, cts, READ_TIMEOUT)
                 .then([this, len, buf, cts_token,
                        &status](concurrency::task<unsigned int> t_) {
                     try {
                         if (cts_token.is_canceled()) {
                             status = PJ_ETIMEDOUT;
                             return 0;
    ...
    auto t =
      concurrency::create_task([this, addr]() {
          ...
          if (sock_type == SOCKTYPE_STREAM)
              return stream_sock->ConnectAsync(
                hostname, port.ToString(), SocketProtectionLevel::PlainSocket);
          else
          ...
          try {
              ...
              if (sock_type == SOCKTYPE_STREAM) {
                  ...
                  socket_reader->InputStreamOptions =
...
pj_status_t PjUwpSocket::Listen()
{
    PJ_ASSERT_RETURN(
      (sock_type == SOCKTYPE_UNKNOWN) ||
        (sock_type == SOCKTYPE_LISTENER && sock_state == SOCKSTATE_INITIALIZED),
      PJ_EINVALIDOP);
...
PJ_DEF(pj_status_t) pj_inet_pton(int af, const pj_str_t* src, void* dst)
{
     ...
     */
    {
        ...
        rc = WSAStringToAddress(
          PJ_STRING_TO_NATIVE(tempaddr, wtempaddr, sizeof(wtempaddr)), af, NULL,
          (LPSOCKADDR)&sock_addr, &addr_len);
        if (rc != 0) {
...
#    if PJ_NATIVE_STRING_IS_UNICODE
        rc = WSAAddressToString((LPSOCKADDR)&sock_addr, addr_len, NULL,
                                wtempaddr, &addr_str_len);
        if (rc == 0) {
            pj_unicode_to_ansi(wtempaddr, wcslen(wtempaddr), dst, size);
        }
#    else
        rc = WSAAddressToString((LPSOCKADDR)&sock_addr, addr_len, NULL, dst,
                                &addr_str_len);
#    endif
...
pj_sock_setsockopt_params(pj_sock_t sockfd, const pj_sockopt_params* params)
{
    ...
    for (; i < params->cnt && i < PJ_MAX_SOCKOPT_PARAMS; ++i) {
        pj_status_t status = pj_sock_setsockopt(
          sockfd, (pj_uint16_t)params->options[i].level,
          (pj_uint16_t)params->options[i].optname, params->options[i].optval,
          params->options[i].optlen);
        if (status != PJ_SUCCESS) {
            retval = status;
            PJ_PERROR(4,
                      (THIS_FILE, status, "Warning: error applying sock opt %d",
