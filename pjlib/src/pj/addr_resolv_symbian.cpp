                                     ...
                                     unsigned* count, pj_addrinfo ai[])
{
    ...
    while (reqStatus == KErrNone && i < *count) {
        ...
        if (1) {
            ...
            if (PjSymbianOS::Addr2pj(inetAddr, a, &namelen, PJ_FALSE) ==
                PJ_SUCCESS) {
                PJ_LOG(
                  5, (THIS_FILE, "resolve %.*s: %s", (int)name->slen, name->ptr,
        ...
        status =
          PjSymbianOS::Addr2pj(inetAddr, ai[i].ai_addr, &addrlen, PJ_TRUE);
        if (status != PJ_SUCCESS)
            return status;
        ...
        pj_unicode_to_ansi((const wchar_t*)nameEntry().iName.Ptr(),
                           nameEntry().iName.Length(), ai[i].ai_canonname,
...
PJ_DEF(pj_status_t)
pj_getaddrinfo(int af, const pj_str_t* nodename, unsigned* count,
               pj_addrinfo ai[])
{
    unsigned start;
    ...
    if (af == PJ_AF_INET6 || af == PJ_AF_UNSPEC) {
        unsigned max = *count;
        status = getaddrinfo_by_af(PJ_AF_INET6, nodename, &max, &ai[start]);
        if (status == PJ_SUCCESS) {
            (*count) -= max;
            start += max;
        }
    ...
    if (af == PJ_AF_INET || af == PJ_AF_UNSPEC) {
        unsigned max = *count;
        status = getaddrinfo_by_af(PJ_AF_INET, nodename, &max, &ai[start]);
        if (status == PJ_SUCCESS) {
            (*count) -= max;
            start += max;
        }
