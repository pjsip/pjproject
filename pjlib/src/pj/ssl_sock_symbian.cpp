...
class CPjSSLSocketReader : public CActive {
   public:
    ...
    {
        CPjSSLSocketReader* self = new (ELeave) CPjSSLSocketReader(sock);
...
class CPjSSLSocket : public CActive {
   public:
    ...
    static CPjSSLSocket* NewL(const TDesC8& ssl_proto, pj_qos_type qos_type,
...
int CPjSSLSocket::Connect(CPjSSLSocket_cb cb, void* key,
                          ...
                          const TInetAddr& rem_addr, const TDesC8& servername,
                          const TDesC8& ciphers)
{
    ...
    // Apply QoS
    status =
      pj_sock_apply_qos2(sock_, qos_type_, &qos_params_, 2, THIS_FILE, NULL);
    ...
    case SSL_STATE_CONNECTING:
        if (iStatus != KErrNone) {
            ...
            if (servername_.Length() > 0)
                securesock_->SetOpt(KSoSSLDomainName, KSolInetSSL, servername_);
                          ...
                          const CX509Certificate* x)
{
    ...
    /* Subject */
    {
        ...
        pj_str_t tmp = get_cert_name(tmp_buf, tmp_buf_len, x->SubjectName());
    ...
    /* Issuer */
    {
        ...
        pj_str_t tmp = get_cert_name(tmp_buf, tmp_buf_len, x->IssuerName());
                   ...
                   pj_ssl_sock_t** p_ssock)
{
    ...
    ssock->write_state.buf =
      (char*)pj_pool_alloc(pool, param->send_buffer_size);
...
pj_ssl_sock_start_accept(pj_ssl_sock_t* ssock, pj_pool_t* pool,
                         const pj_sockaddr_t* local_addr, int addr_len)
{
    PJ_UNUSED_ARG(ssock);
    PJ_UNUSED_ARG(pool);
                          ...
                          const pj_sockaddr_t* remaddr, int addr_len)
{
    ...
    status =
      PjSymbianOS::pj2Addr(*(pj_sockaddr*)localaddr, addr_len, localaddr_);
    if (status != PJ_SUCCESS)
        return status;

    status = PjSymbianOS::pj2Addr(*(pj_sockaddr*)remaddr, addr_len, remaddr_);
    if (status != PJ_SUCCESS)
        return status;
    ...
    /* Init SSL engine */
    TRAPD(err,
          sock = CPjSSLSocket::NewL(proto, ssock->qos_type, ssock->qos_params));
    if (err != KErrNone)
        return PJ_ENOMEM;

    if (ssock->timeout.sec != 0 || ssock->timeout.msec != 0) {
        ssock->connect_timer =
          new CPjTimer(&ssock->timeout, &connect_timer_cb, ssock);
    }

    /* Convert server name to Symbian descriptor */
    TPtrC8 servername_((TUint8*)ssock->servername.ptr, ssock->servername.slen);

    /* Convert cipher list to Symbian descriptor */
    TPtrC8 ciphers_((TUint8*)ssock->ciphers.ptr, ssock->ciphers.slen);
