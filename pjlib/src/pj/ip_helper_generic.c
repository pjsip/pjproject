...
static pj_status_t if_enum_by_af(int af, unsigned* p_cnt, pj_sockaddr ifs[])
{
    ...
    TRACE_(
      (THIS_FILE, "Starting interface enum with getifaddrs() for af=%d", af));
...
#    endif
        ...
        if (ad->sa_family != af) {
            TRACE_((THIS_FILE, "  address %s ignored (af=%d)", get_addr(ad),
                    ad->sa_family));
            continue; /* Skip when interface is down */
        }
...
#elif defined(SIOCGIFCONF) && defined(PJ_HAS_NET_IF_H) && PJ_HAS_NET_IF_H != 0
...
static pj_status_t if_enum_by_af(int af, unsigned* p_cnt, pj_sockaddr ifs[])
{
    ...
    TRACE_(
      (THIS_FILE, "Starting interface enum with SIOCGIFCONF for af=%d", af));
    ...
    *p_cnt = 0;
    for (i = 0; i < count; ++i) {
        ...
        if (ad->sa_family != af) {
            TRACE_((THIS_FILE, "  address %s (af=%d) ignored", get_addr(ad),
                    (int)ad->sa_family));
            continue;
        }
        ...
        if (ioctl(sock, SIOCGIFFLAGS, &iff) != 0) {
            TRACE_(
              (THIS_FILE, "  ioctl(SIOCGIFFLAGS) failed: %s", get_os_errmsg()));
...
#    endif
        ...
        if ((rc = ioctl(sock, SIOCGIFADDR, &ifreq)) != 0) {
            TRACE_(
              (THIS_FILE, "  ioctl(SIOCGIFADDR) failed: %s", get_os_errmsg()));
        ...
        if (ad->sa_family != af) {
            TRACE_((THIS_FILE, "  address %s family %d ignored",
                    get_addr(&ifreq.ifr_addr), ifreq.ifr_addr.sa_family));
                     ...
                     rtattr_ptr = RTA_NEXT(rtattr_ptr, ifaddrmsg_len))
                {
                    ...
                    case IFA_ADDRESS:
                        ...
                        inet_ntop(ifaddrmsg_ptr->ifa_family,
                                  RTA_DATA(rtattr_ptr), deprecatedAddr,
