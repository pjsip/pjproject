...
static int echo_client_thread(void* arg)
{
    ...
    PJ_LOG(3, ("", "...socket connected to %s:%d",
               pj_inet_ntop2(pj_AF_INET(), &addr.sin_addr, addr, sizeof(addr)),
...
int echo_client(int sock_type, const char* server, int port)
{
    ...
    PJ_LOG(3, ("", "  Destination: %s:%d", ECHO_SERVER_ADDRESS,
    ...
    for (;;) {
        ...
        PJ_LOG(3, ("",
                   ...
                   "timeout=%d, invalid=%d",
                   ECHO_CLIENT_MAX_THREADS, bw32 / 1000, timeout, invalid));
    }
