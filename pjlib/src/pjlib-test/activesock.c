                                               ...
                                               int addr_len, pj_status_t status)
{
    ...
    if (srv->echo_enabled) {
        sent = size;
        srv->status = pj_activesock_sendto(asock, &srv->send_key, data, &sent,
                                           0, src_addr, addr_len);
...
static pj_status_t udp_echo_srv_create(pj_pool_t* pool, pj_ioqueue_t* ioqueue,
                                       ...
                                       struct udp_echo_srv** p_srv)
{
    ...
    status = pj_activesock_create_udp(pool, &addr, NULL, ioqueue,
                                      &activesock_cb, srv, &srv->asock, &addr);
    if (status != PJ_SUCCESS) {
...
static int udp_ping_pong_test(void)
{
    ...
    for (count = 0; count < 1000; ++count) {
        ...
        if (need_send) {
            ...
            status = pj_activesock_sendto(srv1->asock, &srv1->send_key, &data,
                                          &sent, 0, &addr, sizeof(addr));
        ...
        for (i = 0;
             i < 10 && last_rx1 == srv1->rx_cnt && last_rx2 == srv2->rx_cnt;
             ++i) {
...
{
    struct tcp_state* st =
      (struct tcp_state*)pj_activesock_get_user_data(asock);
    char* next = (char*)data;
...
static int tcp_perf_test(void)
{
    ...
    status = app_socketpair(pj_AF_INET(), pj_SOCK_STREAM(), 0, &sock1, &sock2);
    if (status != PJ_SUCCESS) {
        status = -100;
        goto on_return;
    }
