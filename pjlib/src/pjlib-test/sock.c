...
static int format_test(void)
{
    ...
    if (p[0] != A[0] || p[1] != A[1] || p[2] != A[2] || p[3] != A[3]) {
        PJ_LOG(3, ("test",
                   "  error: mismatched address. p0=%d, p1=%d, "
                   "p2=%d, p3=%d",
...
#    if defined(PJ_HAS_IPV6) && PJ_HAS_IPV6 != 0
    ...
    /* pj_inet_ntop() */
    {
        ...
        if (p[0] != 0xfe || p[1] != 0x80 || p[2] != 0 || p[3] != 0 ||
            p[4] != 0 || p[5] != 0 || p[6] != 0 || p[7] != 0 || p[8] != 0x02 ||
            p[9] != 0xff || p[10] != 0x83 || p[11] != 0xff || p[12] != 0xfe ||
            p[13] != 0x7c || p[14] != 0x8b || p[15] != 0x42)
        {
            return -27;
        }

        if (pj_inet_ntop(pj_AF_INET(), &ipv4, buf_ipv4, sizeof(buf_ipv4)) !=
            PJ_SUCCESS)
            return -28;
        if (pj_stricmp2(&s_ipv4, buf_ipv4) != 0)
            return -29;

        if (pj_inet_ntop(pj_AF_INET6(), &ipv6, buf_ipv6, sizeof(buf_ipv6)) !=
            PJ_SUCCESS)
            return -30;
...
#    endif /* PJ_HAS_IPV6 */
    ...
    PJ_LOG(
      3, ("test", "....hostname is %.*s", (int)hostname->slen, hostname->ptr));

    /* pj_gethostaddr() */

    /* Various constants */
#    if !defined(PJ_SYMBIAN) || PJ_SYMBIAN == 0
    if (PJ_AF_INET == 0xFFFF)
        return -5500;
    if (PJ_AF_INET6 == 0xFFFF)
        return -5501;
    ...
    if (PJ_SOL_IP == 0xFFFF)
        return -5502;
    if (PJ_SOL_TCP == 0xFFFF)
        return -5510;
    if (PJ_SOL_UDP == 0xFFFF)
        return -5520;
    if (PJ_SOL_IPV6 == 0xFFFF)
        return -5530;

    if (PJ_SO_TYPE == 0xFFFF)
        return -5540;
    if (PJ_SO_RCVBUF == 0xFFFF)
        return -5550;
    if (PJ_SO_SNDBUF == 0xFFFF)
        return -5560;
    if (PJ_TCP_NODELAY == 0xFFFF)
        return -5570;
    if (PJ_SO_REUSEADDR == 0xFFFF)
        return -5580;

    if (PJ_MSG_OOB == 0xFFFF)
        return -5590;
    if (PJ_MSG_PEEK == 0xFFFF)
        return -5600;
#    endif
...
#    endif
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(valid_tests); ++i) {
        ...
        status = pj_sockaddr_parse(
          PJ_AF_UNSPEC, 0, pj_cstr(&input, valid_tests[i].input), &addr);
        ...
        status = pj_sockaddr_init(valid_tests[i].result_af, &result,
        ...
        status =
          pj_sockaddr_parse(valid_tests[i].result_af, 0,
                            pj_cstr(&input, valid_tests[i].input), &addr);
        if (status != PJ_SUCCESS) {
            PJ_LOG(
              1, ("test", ".... failed when parsing %s", valid_tests[i].input));
            return -50;
        }
    ...
    for (i = 0; i < PJ_ARRAY_SIZE(invalid_tests); ++i) {
        ...
        /* Try parsing with PJ_AF_UNSPEC */
        status = pj_sockaddr_parse(
          PJ_AF_UNSPEC, 0, pj_cstr(&input, invalid_tests[i].input), &addr);
        if (status == PJ_SUCCESS) {
...
static int send_recv_test(int sock_type, pj_sock_t ss, pj_sock_t cs,
                          ...
                          int addrlen)
{
    ...
    if (srcaddr) {
        ...
        if (pj_sockaddr_cmp(&addr, srcaddr) != 0) {
            ...
            PJ_LOG(3, ("test",
                       "...error: src address mismatch (original=%s, "
                       "recvfrom addr=%s)",
    ...
    do {
        ...
        if (received <= 0) {
            PJ_LOG(
              3, ("", "...error: socket has closed! (received=%d)", received));
            rc = -173;
            goto on_error;
        }
...
static int udp_test(void)
{
    ...
    rc =
      send_recv_test(pj_SOCK_DGRAM(), ss, cs, &dstaddr, NULL, sizeof(dstaddr));
    if (rc != 0)
        goto on_error;

    /* Test send/recv, with sendto and recvfrom */
    rc = send_recv_test(pj_SOCK_DGRAM(), ss, cs, &dstaddr, &srcaddr,
                        sizeof(dstaddr));
    if (rc != 0)
        goto on_error;
...
#    if !defined(PJ_SYMBIAN) || PJ_SYMBIAN == 0
    ...
    rc =
      send_recv_test(pj_SOCK_DGRAM(), ss, cs, NULL, &srcaddr, sizeof(srcaddr));
    if (rc != 0)
        goto on_error;
#    endif
