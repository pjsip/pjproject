...
static void dump_ssl_info(const pj_ssl_sock_info* si)
{
    ...
    if (si->remote_cert_info && si->remote_cert_info->subject.info.slen) {
        ...
        pj_ssl_cert_get_verify_status_strings(si->verify_status, verif_msgs,
                                              &verif_msg_cnt);
                                         ...
                                         pj_status_t status)
{
    struct test_state* st =
      (struct test_state*)pj_ssl_sock_get_user_data(ssock);
    void* read_buf[1];
                                        ...
                                        pj_status_t accept_status)
{
    ...
    st = (struct test_state*)pj_pool_zalloc(parent_st->pool,
                                            sizeof(struct test_state));
    *st = *parent_st;
                                  ...
                                  pj_size_t* remainder)
{
    ...
    if (size > 0) {
        ...
        *remainder = pj_rand() % 100;
         ...
         * - echo/check_eco set
         */
        if (*remainder > size || status != PJ_SUCCESS || st->echo ||
            st->check_echo)
            *remainder = 0;
        ...
        if (st->echo) {
            ...
            status = pj_ssl_sock_send(
              ssock, (pj_ioqueue_op_key_t*)&st->send_key, data, &size_, 0);
            if (status != PJ_SUCCESS && status != PJ_EPENDING) {
                app_perror("...ERROR pj_ssl_sock_send()", status);
                goto on_return;
            }
        ...
        if (st->check_echo) {
            ...
            if (st->send_str_len == st->recv) {
                ...
                pj_sockaddr_print((pj_sockaddr_t*)&info.local_addr, buf,
                                  sizeof(buf), 1);
                PJ_LOG(3, ("", "...%s successfully recv %d bytes echo", buf,
                           st->recv));
                st->done = PJ_TRUE;
            }
        }
                                  ...
                                  pj_ioqueue_op_key_t* op_key, pj_ssize_t sent)
{
    ...
    if (sent < 0) {
        ...
        while (st->sent < st->send_str_len) {
            ...
            size = st->send_str_len - st->sent;
            status =
              pj_ssl_sock_send(ssock, (pj_ioqueue_op_key_t*)&st->send_key,
                               st->send_str + st->sent, &size, 0);
            if (status != PJ_SUCCESS && status != PJ_EPENDING) {
                app_perror("...ERROR pj_ssl_sock_send()", status);
                st->err = status;
...
static int https_client_test(unsigned ms_timeout)
{
    ...
    pj_sockaddr_init(PJ_AF_INET, &local_addr, pj_strset2(&tmp_st, "0.0.0.0"),
                     0);
    pj_sockaddr_init(PJ_AF_INET, &rem_addr,
                     pj_strset2(&tmp_st, HTTP_SERVER_ADDR), HTTP_SERVER_PORT);
    status = pj_ssl_sock_start_connect(ssock, pool, &local_addr, &rem_addr,
                                       sizeof(rem_addr));
    if (status == PJ_SUCCESS) {
                     ...
                     pj_bool_t req_client_cert, pj_bool_t client_provide_cert)
{
    ...
    {
        pj_str_t tmp_st;
        pj_sockaddr_init(PJ_AF_INET, &addr, pj_strset2(&tmp_st, "127.0.0.1"),
                         0);
    }
...
#    else
        ...
        status = pj_ssl_cert_load_from_buffer(
          pool, &ca_buf, &cert_buf, &privkey_buf, &privkey_pass, &cert);
#    endif
        if (status != PJ_SUCCESS) {
            goto on_return;
        }
    ...
    status = pj_ssl_sock_start_accept(ssock_serv, pool, &addr,
...
#    endif
    ...
    status = pj_ssl_sock_start_connect(ssock_cli, pool, &addr, &listen_addr,
                                       pj_sockaddr_get_len(&addr));
    if (status == PJ_SUCCESS) {
    ...
    while (!state_serv.err && !state_cli.err && !state_serv.done &&
           !state_cli.done) {
#    ifdef PJ_SYMBIAN
        pj_symbianos_poll(-1, 1000);
#    else
...
#    endif
    ...
    PJ_LOG(3, ("", "...Done!"));
    PJ_LOG(3,
           ("", ".....Sent/recv: %d/%d bytes", state_cli.sent, state_cli.recv));
...
#    if (PJ_SSL_SOCK_IMP == PJ_SSL_SOCK_IMP_DARWIN) || \
    ...
    if (status != PJ_SUCCESS) {
        PJ_LOG(3, ("",
                   "Apple SSL requires the private key to be "
...
{
    struct test_state* st =
      (struct test_state*)pj_activesock_get_user_data(asock);
                                           ...
                                           pj_status_t status)
{
    ...
    if (status == PJ_SUCCESS) {
        ...
        status = pj_activesock_start_read2(
          asock, st->pool, sizeof(st->read_buf), (void**)read_buf, 0);
        if (status != PJ_SUCCESS) {
            app_perror("...ERROR pj_ssl_sock_start_read2()", status);
        }
    }
                                          ...
                                          int src_addr_len)
{
    ...
    read_buf[0] = st->read_buf;
    status = pj_activesock_start_read2(
      new_asock, st->pool, sizeof(st->read_buf), (void**)read_buf, 0);
    if (status != PJ_SUCCESS) {
        app_perror("...ERROR pj_ssl_sock_start_read2()", status);
    }
...
#    endif
    ...
    status = pj_ssl_sock_start_accept(ssock_serv, pool, &listen_addr,
    ...
    asock_cb.on_data_read = &asock_on_data_read;
    status = pj_activesock_create(pool, sock, pj_SOCK_STREAM(), NULL, ioqueue,
                                  &asock_cb, &state_cli, &asock_cli);
    if (status != PJ_SUCCESS) {
        goto on_return;
    }

    status =
      pj_activesock_start_connect(asock_cli, pool, (pj_sockaddr_t*)&listen_addr,
                                  pj_sockaddr_get_len(&listen_addr));
    if (status == PJ_SUCCESS) {
...
static int server_non_ssl(unsigned ms_timeout)
{
    ...
    status = pj_activesock_create(pool, sock, pj_SOCK_STREAM(), NULL, ioqueue,
                                  &asock_cb, &state_serv, &asock_serv);
    if (status != PJ_SUCCESS) {
        goto on_return;
    }
    ...
    status = pj_ssl_sock_start_connect(ssock_cli, pool, (pj_sockaddr_t*)&addr,
    ...
    /* Wait until everything has been sent/received or error */
    while ((!state_serv.err && !state_serv.done) ||
...
#    endif
    ...
    ssock_cli =
      (pj_ssl_sock_t**)pj_pool_calloc(pool, clients, sizeof(pj_ssl_sock_t*));
    state_cli = (struct test_state*)pj_pool_calloc(pool, clients,
                                                   sizeof(struct test_state));
    ...
    for (i = 0; i < clients; ++i) {
        ...
        state_cli[i].send_str_len =
          (pj_rand() % 5 + 1) * 1024 + pj_rand() % 1024;
        state_cli[i].send_str =
          (char*)pj_pool_alloc(pool, state_cli[i].send_str_len);
        {
            unsigned j;
        ...
        status = pj_ssl_sock_start_connect(
          ssock_cli[i], pool, &addr, &listen_addr, pj_sockaddr_get_len(&addr));
        if (status == PJ_SUCCESS) {
...
#    endif
    ...
    /* SSL setup and data transfer duration */
    {
        ...
        PJ_LOG(3, ("", ".....Setup & data transfer duration: %d.%03ds",
                   stop.sec, stop.msec));
    }
...
#    ifndef PJ_SYMBIAN
    ...
    PJ_LOG(
      3,
      ("", "..echo test w/ TLSv1 and PJ_TLS_RSA_WITH_AES_256_CBC_SHA cipher"));
    ret = echo_test(PJ_SSL_SOCK_PROTO_TLS1, PJ_SSL_SOCK_PROTO_TLS1,
                    PJ_TLS_RSA_WITH_AES_256_CBC_SHA,
                    PJ_TLS_RSA_WITH_AES_256_CBC_SHA, PJ_FALSE, PJ_FALSE);
    if (ret != 0)
        return ret;

    PJ_LOG(
      3,
      ("", "..echo test w/ SSLv23 and PJ_TLS_RSA_WITH_AES_256_CBC_SHA cipher"));
    ...
    PJ_LOG(
      3, ("",
          "..echo test w/ compatible proto: server TLSv1.2 vs client TLSv1.2"));
    ret = echo_test(PJ_SSL_SOCK_PROTO_TLS1_2, PJ_SSL_SOCK_PROTO_TLS1_2, -1, -1,
                    PJ_FALSE, PJ_FALSE);
    if (ret != 0)
        return ret;

    PJ_LOG(3, ("",
               "..echo test w/ compatible proto: server TLSv1.2+1.3 vs client "
               "TLSv1.3"));
    ret = echo_test(PJ_SSL_SOCK_PROTO_TLS1_2 | PJ_SSL_SOCK_PROTO_TLS1_3,
                    PJ_SSL_SOCK_PROTO_TLS1_3, -1, -1, PJ_FALSE, PJ_FALSE);
    if (ret != 0)
        return ret;
...
#        if (PJ_SSL_SOCK_IMP != PJ_SSL_SOCK_IMP_DARWIN)
    PJ_LOG(
      3,
      ("",
       "..echo test w/ incompatible proto: server TLSv1.2 vs client TLSv1.3"));
    ret = echo_test(PJ_SSL_SOCK_PROTO_TLS1_2, PJ_SSL_SOCK_PROTO_TLS1_3, -1, -1,
                    PJ_FALSE, PJ_FALSE);
    if (ret == 0)
        return PJ_EBUG;
#        endif
...
#        endif
    ...
    ret = echo_test(PJ_SSL_SOCK_PROTO_DEFAULT, PJ_SSL_SOCK_PROTO_DEFAULT,
                    PJ_TLS_RSA_WITH_AES_256_CBC_SHA,
                    PJ_TLS_RSA_WITH_AES_256_CBC_SHA, PJ_TRUE, PJ_FALSE);
    if (ret == 0)
        return PJ_EBUG;
    ...
    ret = echo_test(PJ_SSL_SOCK_PROTO_DEFAULT, PJ_SSL_SOCK_PROTO_DEFAULT,
                    PJ_TLS_RSA_WITH_AES_256_CBC_SHA,
                    PJ_TLS_RSA_WITH_AES_256_CBC_SHA, PJ_TRUE, PJ_TRUE);
    if (ret != 0)
        return ret;
