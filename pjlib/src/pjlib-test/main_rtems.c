...
#define TEST(expr) \
    { \
        ...
        rc = expr; \
        if (rc) \
            my_perror(PJ_STATUS_FROM_OS(rc), #expr); \
    }
 ...
 */
static void send_udp(const char* target)
{
    int sock, rc;
    struct sockaddr_in addr;

    PJ_LOG(3,
           ("main_rtems.c", "IP addr=%s/%s, gw=%s", DEFAULT_IP_ADDRESS_STRING,
            DEFAULT_NETMASK_STRING, DEFAULT_GATEWAY_STRING));
...
static void initialize_network()
{
    ...
    if (0 != rtems_bsdnet_initialize_network())
        ...
        PJ_LOG(3, (THIS_FILE, "IP addr=%s/%s, gw=%s", ip_address_string,
                   netmask_string, gateway_string));
