...
PJ_INLINE(int) cmp(const char* expr, int i, int j)
{
    ...
    if (r) {
        PJ_LOG(3,
               (THIS_FILE, "   error: %s: expecting %d, got %d", expr, j, i));
    }
    return r;
}
#    else
...
#    define STRTEST(res, res2, S1, S2, code) \
        do { \
            ...
            s2.slen = (S2) ? len : 0; \
            pj_get_timestamp(&t1); \
            if (C(pj_stricmp(&s1, &s2), res)) \
                return code; \
            pj_get_timestamp(&t2); \
            pj_sub_timestamp(&t2, &t1); \
            pj_add_timestamp(&e1, &t2); \
            pj_get_timestamp(&t1); \
            if (C(pj_stricmp_alnum(&s1, &s2), res)) \
                return code - 1; \
            pj_get_timestamp(&t2); \
            pj_sub_timestamp(&t2, &t1); \
            pj_add_timestamp(&e2, &t2); \
            if (C(pj_stricmp2(&s1, S2), res2)) \
                return code * 10; \
            if (C(pj_strnicmp(&s1, &s2, len), res)) \
                return code * 100; \
            if (C(pj_strnicmp2(&s1, S2, len), res)) \
                return code * 1000; \
        } while (0)
...
#    define SNULL 0
    ...
    PJ_LOG(3, ("", "  time: stricmp=%u, stricmp_alnum=%u (speedup=%d.%02dx)",
               c1, c2, (c1 * 100 / c2) / 100, (c1 * 100 / c2) % 100));
    return 0;
#    undef STRTEST
}
...
#    define STR_TEST(res, S1, S2, code) \
        do { \
            ...
            s2.ptr = S2; \
            s2.slen = S2 ? len : 0; \
            if (C(pj_strcmp(&s1, &s2), res)) \
                return code; \
            if (C(pj_strcmp2(&s1, S2), res)) \
                return code - 1; \
            if (C(pj_strncmp(&s1, &s2, len), res)) \
                return code - 2; \
            if (C(pj_strncmp2(&s1, S2, len), res)) \
                return code - 3; \
        } while (0)
