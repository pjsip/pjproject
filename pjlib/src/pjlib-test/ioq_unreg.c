...
static int perform_unreg_test(pj_ioqueue_t* ioqueue, pj_pool_t* test_pool,
                              const char* title, pj_bool_t other_socket)
{
    enum
    {
        WORKER_CNT = 1,
    ...
    if (other_socket) {
        ...
        status = pj_ioqueue_register_sock(test_pool, ioqueue, osd.sock, NULL,
                                          &callback, &osd.key);
        if (status != PJ_SUCCESS) {
    ...
    status = app_socketpair(pj_AF_INET(), pj_SOCK_DGRAM(), 0, &sock_data.sock,
                            &sock_data.csock);
    if (status != PJ_SUCCESS) {
    ...
    sock_data.op_key = (pj_ioqueue_op_key_t*)pj_pool_alloc(
      sock_data.pool, sizeof(*sock_data.op_key));
    ...
    for (;;) {
        ...
        if (test_method == UNREGISTER_IN_APP &&
            PJ_TIME_VAL_GTE(now, time_to_unregister) && !sock_data.unregistered)
        {
            sock_data.unregistered = 1;
    ...
    PJ_LOG(3, (THIS_FILE, "....%s: done (%d KB/s)", title,
               sock_data.received * 1000 / MSEC / 1000));
    return 0;
}
...
static int udp_ioqueue_unreg_test_imp(pj_bool_t allow_concur)
{
    ...
    PJ_LOG(3, (THIS_FILE,
               "...ioqueue unregister stress test 0/3, unregister in app (%s)",
               pj_ioqueue_name()));
    for (i = 0; i < LOOP; ++i) {
    ...
    PJ_LOG(3, (THIS_FILE,
               "...ioqueue unregister stress test 1/3, unregister in app (%s)",
               pj_ioqueue_name()));
    for (i = 0; i < LOOP; ++i) {
        pj_ansi_sprintf(title, "repeat %d/%d", i, LOOP);
    ...
    PJ_LOG(3, (THIS_FILE,
               "...ioqueue unregister stress test 2/3, unregister in cb (%s)",
               pj_ioqueue_name()));
    for (i = 0; i < LOOP; ++i) {
        pj_ansi_sprintf(title, "repeat %d/%d", i, LOOP);
        rc = perform_unreg_test(ioqueue, test_pool, title, 0);
    ...
    PJ_LOG(3, (THIS_FILE,
               "...ioqueue unregister stress test 3/3, unregister in cb (%s)",
               pj_ioqueue_name()));
    for (i = 0; i < LOOP; ++i) {
        pj_ansi_sprintf(title, "repeat %d/%d", i, LOOP);
        rc = perform_unreg_test(ioqueue, test_pool, title, 1);
