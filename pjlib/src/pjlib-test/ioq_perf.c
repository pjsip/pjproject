...
{
    pj_sock_t server_fd, client_fd;
    pj_ioqueue_t* ioqueue;
    pj_ioqueue_key_t *server_key, *client_key;
    pj_ioqueue_op_key_t recv_op, send_op;
    int has_pending_send;
    pj_size_t buffer_size;
    char* outgoing_buffer;
    char* incoming_buffer;
    pj_size_t bytes_sent, bytes_recv;
} test_item;
                             ...
                             pj_ssize_t bytes_read)
{
    ...
    do {
        ...
        if (bytes_read < 0) {
            ...
            if (rc != last_error) {
                ...
                if (!IS_ERROR_SILENCED(rc)) {
                    ...
                    PJ_LOG(
                      3, (THIS_FILE, "...error: read error, bytes_read=%d (%s)",
                          bytes_read, errmsg));
                    PJ_LOG(
                      3, (THIS_FILE,
         ...
         */
        if (LIMIT_TRANSFER &&
            item->bytes_recv > item->buffer_size * LIMIT_TRANSFER)
            thread_quit_flag = 1;

        bytes_read = item->buffer_size;
        rc =
          pj_ioqueue_recv(key, op_key, item->incoming_buffer, &bytes_read, 0);
                              ...
                              pj_ssize_t bytes_sent)
{
    ...
    if (bytes_sent <= 0) {
        ...
        rc = pj_ioqueue_send(item->client_key, op_key, item->outgoing_buffer,
                             &bytes_sent, 0);
        if (rc != PJ_SUCCESS && rc != PJ_EPENDING) {
            app_perror("...error: write error", rc);
        } else if (rc == PJ_SUCCESS) {
...
struct thread_arg
{
    int id;
    pj_ioqueue_t* ioqueue;
    unsigned loop_cnt, err_cnt, event_cnt;
};
 ...
 *    period of time.
 */
static int perform_test(pj_bool_t allow_concur, int sock_type,
                        const char* type_name, unsigned thread_cnt,
                        unsigned sockpair_cnt, pj_size_t buffer_size,
                        pj_bool_t display_report, pj_size_t* p_bandwidth)
{
    enum
    {
        MSEC_DURATION = 5000
    ...
    thread =
      (pj_thread_t**)pj_pool_alloc(pool, thread_cnt * sizeof(pj_thread_t*));
    ...
    for (i = 0; i < sockpair_cnt; ++i) {
        ...
        rc = app_socketpair(pj_AF_INET(), sock_type, 0, &items[i].server_fd,
                            &items[i].client_fd);
        if (rc != PJ_SUCCESS) {
        ...
        rc =
          pj_ioqueue_register_sock(pool, ioqueue, items[i].server_fd, &items[i],
                                   &ioqueue_callback, &items[i].server_key);
        if (rc != PJ_SUCCESS) {
        ...
        rc =
          pj_ioqueue_register_sock(pool, ioqueue, items[i].client_fd, &items[i],
                                   &ioqueue_callback, &items[i].client_key);
        if (rc != PJ_SUCCESS) {
            app_perror("...error: registering server socket to ioqueue", rc);
            return -70;
        }
    ...
    for (i = 0; i < thread_cnt; ++i) {
        ...
        rc = pj_thread_create(pool, NULL, &worker_thread, arg,
                              PJ_THREAD_DEFAULT_STACK_SIZE, PJ_THREAD_SUSPENDED,
                              &thread[i]);
        if (rc != PJ_SUCCESS) {
    ...
    *p_bandwidth = (pj_uint32_t)bandwidth;
    ...
    if (display_report) {
        ...
        PJ_LOG(3,
               (THIS_FILE, "  Elapsed  : %u msec", total_elapsed_usec / 1000));
        ...
        for (unsigned i = 0; i < thread_cnt; ++i) {
            struct thread_arg* arg = &args[i];
            PJ_LOG(3, (THIS_FILE, " %6d  %6d  %6d  %6d", arg->id, arg->loop_cnt,
                       arg->event_cnt, arg->err_cnt));
        }
        PJ_LOG(3, (THIS_FILE, "    ============================="));
        ...
        for (unsigned i = 0; i < sockpair_cnt; ++i) {
            ...
            PJ_LOG(3,
                   (THIS_FILE, "    %4d  %5.1f MB  %5.1f MB    %5.1f%%", i,
                    item->bytes_sent / 1000000.0, item->bytes_recv / 1000000.0,
                    item->bytes_recv * 100.0 / total_received));
        }
    } else {
        PJ_LOG(3, (THIS_FILE, "   %.4s    %2d        %2d       %8d KB/s",
                   type_name, thread_cnt, sockpair_cnt, *p_bandwidth));
    }
...
static int ioqueue_perf_test_imp(pj_bool_t allow_concur)
{
    ...
    struct
    {
        ...
        { pj_SOCK_DGRAM(), "udp", 1, 1 },    { pj_SOCK_DGRAM(), "udp", 2, 2 },
        { pj_SOCK_DGRAM(), "udp", 4, 4 },    { pj_SOCK_DGRAM(), "udp", 8, 8 },
        { pj_SOCK_DGRAM(), "udp", 16, 16 },

        { pj_SOCK_STREAM(), "tcp", 1, 1 },   { pj_SOCK_STREAM(), "tcp", 2, 2 },
        { pj_SOCK_STREAM(), "tcp", 4, 4 },   { pj_SOCK_STREAM(), "tcp", 8, 8 },
    ...
    for (i = 0; i < (int)(sizeof(test_param) / sizeof(test_param[0])); ++i) {
        ...
        rc = perform_test(allow_concur, test_param[i].type,
                          test_param[i].type_name, test_param[i].thread_cnt,
                          test_param[i].sockpair_cnt, BUF_SIZE, PJ_FALSE,
                          &bandwidth);
        if (rc != 0)
            return rc;
    ...
    PJ_LOG(3,
           (THIS_FILE,
            ...
            test_param[best_index].type_name, test_param[best_index].thread_cnt,
            test_param[best_index].sockpair_cnt, best_bandwidth));
...
int ioqueue_perf_test(void)
{
    ...
    rc = perform_test(PJ_TRUE, pj_SOCK_DGRAM(), "udp", 8, 8, 512, PJ_TRUE,
                      &bandwidth);

    PJ_LOG(3, (THIS_FILE, " Detailed perf (concurrency=0):"));
    rc = perform_test(PJ_FALSE, pj_SOCK_DGRAM(), "udp", 8, 8, 512, PJ_TRUE,
                      &bandwidth);
