...
static int capacity_test(void)
{
    ...
    if (pj_pool_alloc(pool, freesize) == NULL) {
        PJ_LOG(3, ("test", "...error: wrong freesize %u reported", freesize));
...
#    define IS_ALIGNED(p) \
        ((((unsigned long)(pj_ssize_t)p) & (PJ_POOL_ALIGNMENT - 1)) == 0)
...
{
    pj_pool_t* pool =
      pj_pool_create(mem, NULL, size, increment, &null_callback);
    pj_size_t freesize;
    void* p;
    int status = 0;
