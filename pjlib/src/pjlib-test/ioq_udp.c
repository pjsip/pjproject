...
static pj_ssize_t callback_read_size, callback_write_size,
  callback_accept_status, callback_connect_status;
static pj_ioqueue_key_t *callback_read_key, *callback_write_key,
  *callback_accept_key, *callback_connect_key;
static pj_ioqueue_op_key_t *callback_read_op, *callback_write_op,
  *callback_accept_op;
                            ...
                            pj_ssize_t bytes_read)
{
    ...
    TRACE__(
      (THIS_FILE, "     callback_read_key = %p, bytes=%d", key, bytes_read));
}
...
static int compliance_test(pj_bool_t allow_concur)
{
    ...
    bytes = bufsize;
    rc =
      pj_ioqueue_recvfrom(skey, &read_op, recv_buf, &bytes, 0, &addr, &addrlen);
    if (rc != PJ_SUCCESS && rc != PJ_EPENDING) {
        ...
        recv_pending = 1;
        PJ_LOG(3, (THIS_FILE, "......ok: recvfrom returned pending"));
    } else {
        PJ_LOG(3, (THIS_FILE, "......error: recvfrom returned immediate ok!"));
        status = -29;
        goto on_error;
    }
    ...
    if (rc != PJ_SUCCESS && rc != PJ_EPENDING) {
        ...
        send_pending = 1;
        PJ_LOG(3, (THIS_FILE, "......ok: sendto returned pending"));
    } else {
        send_pending = 0;
        PJ_LOG(3, (THIS_FILE, "......ok: sendto returned immediate success"));
    }

    // reset callback variables.
    callback_read_size = callback_write_size = 0;
    callback_accept_status = callback_connect_status = -2;
    callback_read_key = callback_write_key = callback_accept_key =
...
static int unregister_test(pj_bool_t allow_concur)
{
    ...
    packet_cnt = 0;
    status =
      pj_ioqueue_register_sock(pool, ioqueue, rsock, &packet_cnt, &cb, &key);
    if (status != PJ_SUCCESS) {
    ...
    status = pj_sock_sendto(ssock, sendbuf, &bytes, 0, &addr, sizeof(addr));
...
static int many_handles_test(pj_bool_t allow_concur)
{
    ...
    key =
      (pj_ioqueue_key_t**)pj_pool_alloc(pool, MAX * sizeof(pj_ioqueue_key_t*));
    ...
    for (count = 0; count < MAX; ++count) {
        ...
        key[count] = NULL;
        rc = pj_ioqueue_register_sock(pool, ioqueue, sock[count], NULL,
                                      &test_cb, &key[count]);
                      ...
                      int inactive_sock_count)
{
    ...
    inactive_sock =
      (pj_sock_t*)pj_pool_alloc(pool, inactive_sock_count * sizeof(pj_sock_t));
    inactive_read_op = (pj_ioqueue_op_key_t*)pj_pool_alloc(
      pool, inactive_sock_count * sizeof(pj_ioqueue_op_key_t));
    pj_bzero(&addr, sizeof(addr));
    addr.sin_family = pj_AF_INET();
    for (i = 0; i < inactive_sock_count; ++i) {
        ...
        rc =
          pj_sock_socket(pj_AF_INET(), pj_SOCK_DGRAM(), 0, &inactive_sock[i]);
        ...
        rc = pj_ioqueue_register_sock(pool, ioque, inactive_sock[i], NULL,
                                      &test_cb, &keys[i]);
        if (rc != PJ_SUCCESS) {
            pj_sock_close(inactive_sock[i]);
            inactive_sock[i] = PJ_INVALID_SOCKET;
        ...
        rc =
          pj_ioqueue_recv(keys[i], &inactive_read_op[i], recv_buf, &bytes, 0);
        if (rc != PJ_EPENDING) {
            pj_sock_close(inactive_sock[i]);
            inactive_sock[i] = PJ_INVALID_SOCKET;
    ...
    for (i = 0; i < LOOP; ++i) {
        ...
        if (rc == PJ_SUCCESS) {
            if (bytes < 0) {
                app_perror("...error: pj_ioqueue_sendto()",
                           (pj_status_t)-bytes);
                break;
            }
        }
...
#    endif
    ...
    if (rc == 0) {
        ...
        PJ_LOG(3, (THIS_FILE, "...%10d %15d  % 9d", bufsize,
                   inactive_sock_count, usec_delay));

    } else {
        PJ_LOG(2, (THIS_FILE, "...ERROR rc=%d (buf:%d, fds:%d)", rc, bufsize,
                   inactive_sock_count + 2));
    }
...
static int udp_ioqueue_test_imp(pj_bool_t allow_concur)
{
    ...
    // pass1:
    PJ_LOG(
      3, (THIS_FILE, "...Benchmarking poll times for %s:", pj_ioqueue_name()));
    PJ_LOG(3, (THIS_FILE, "...====================================="));
    ...
    for (bufsize = BUF_MIN_SIZE; bufsize <= BUF_MAX_SIZE; bufsize *= 2) {
        if ((status = bench_test(allow_concur, bufsize, SOCK_INACTIVE_MIN)) !=
            0)
            return status;
    }
    // pass2:
    ...
    for (sock_count = SOCK_INACTIVE_MIN + 2;
         sock_count <= SOCK_INACTIVE_MAX + 2; sock_count *= 2)
    {
