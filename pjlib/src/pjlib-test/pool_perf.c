...
static int pool_test_pool()
{
    int i;
    pj_pool_t* pool =
      pj_pool_create(mem, NULL, total_size + 4 * COUNT, 0, NULL);
    if (!pool)
        return -1;
...
static int pool_test_malloc_free()
{
    ...
    for (i = 0; i < COUNT; ++i) {
        ...
        if (!p[i]) {
            PJ_LOG(3,
                   (THIS_FILE, "   error: malloc failed to allocate %d bytes",
                    sizes[i]));
            --i;
            while (i >= 0)
                free(p[i]), --i;
...
int pool_perf_test()
{
    ...
    for (i = 0; i < COUNT; ++i) {
        ...
        if (aligned_size & (PJ_POOL_ALIGNMENT - 1))
            aligned_size = ((aligned_size + PJ_POOL_ALIGNMENT - 1)) &
                           ~(PJ_POOL_ALIGNMENT - 1);
    ...
    PJ_LOG(4,
           (THIS_FILE, "..pool allocation/deallocation time: %u", pool_time));
    PJ_LOG(4,
           (THIS_FILE, "..malloc/free time:                  %u", malloc_time));
    PJ_LOG(4,
           (THIS_FILE, "..pool again, second invocation:     %u", pool_time2));
    ...
    PJ_LOG(3, (THIS_FILE, "..pool speedup over malloc best=%dx, worst=%dx",
               (int)(malloc_time / best), (int)(malloc_time / worst)));
    return 0;
}
