                              ...
                              int status)
{
    if (sock == PJ_INVALID_SOCKET) {
        if (status != PJ_SUCCESS) {
            ...
            app_perror(
              ".....warning: received error in on_ioqueue_accept() callback",
              status);
        } else {
            callback_accept_status = -61;
            PJ_LOG(3, ("",
                       "..... on_ioqueue_accept() callback was given "
                       "invalid socket and status is %d",
                       status));
        }
    } else {
        pj_sockaddr addr;
...
static int send_recv_test(pj_ioqueue_t* ioque, pj_ioqueue_key_t* skey,
                          pj_ioqueue_key_t* ckey, void* send_buf,
                          void* recv_buf, pj_ssize_t bufsize,
...
static int compliance_test_0(pj_bool_t allow_concur)
{
    ...
    if (rc == PJ_SUCCESS)
        rc =
          pj_ioqueue_register_sock(pool, ioque, csock1, NULL, &test_cb, &ckey1);
    else
        ckey1 = NULL;
    if (rc != PJ_SUCCESS) {
    ...
    status = pj_ioqueue_accept(skey, &accept_op, &csock0, &client_addr,
                               &rmt_addr, &client_addr_len);
    if (status != PJ_EPENDING) {
...
#        endif
    ...
    t_elapsed.u32.lo = 0;
    status = send_recv_test(ioque, ckey0, ckey1, send_buf, recv_buf, bufsize,
                            &t_elapsed);
    if (status != 0) {
        goto on_error;
    }
...
#        endif
    ...
    if ((rc = pj_sock_bind(listener.sock, &listener.addr,
    ...
    for (test_loop = 0; test_loop < TEST_LOOP; ++test_loop) {
        ...
        for (i = 0; i < MAX_PAIR; ++i) {
            rc = pj_sock_socket(pj_AF_INET(), pj_SOCK_STREAM(), 0,
                                &client[i].sock);
            if (rc != PJ_SUCCESS) {
                app_perror("...error creating socket", rc);
                status = -70;
            ...
            status =
              pj_ioqueue_accept(listener.key, &server[i].accept_op,
                                &server[i].sock, &server[i].local_addr,
                                &server[i].rem_addr, &server[i].rem_addr_len);
            if (status != PJ_SUCCESS && status != PJ_EPENDING) {
                app_perror("...ERROR in pj_ioqueue_accept()", rc);
                status = -90;
                goto on_error;
