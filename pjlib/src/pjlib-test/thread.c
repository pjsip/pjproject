...
static int simple_thread(const char* title, unsigned flags)
{
    ...
    rc =
      pj_thread_create(pool, "thread", (pj_thread_proc*)&thread_proc, &counter,
                       PJ_THREAD_DEFAULT_STACK_SIZE, flags, &thread);
...
static int timeslice_test(void)
{
    ...
    for (i = 0; i < NUM_THREADS; ++i) {
        if (counter[i] > i) {
            PJ_LOG(3,
                   (THIS_FILE, "....ERROR! Thread %d-th is not suspended!", i));
            return -30;
        }
    }
    ...
    for (i = 0; i < NUM_THREADS; ++i) {
        TRACE__((THIS_FILE, "    Main thread joining thread %d [%p]..", i,
                 thread[i]));
    ...
    if (diff >= 50) {
        ...
        PJ_LOG(3,
               (THIS_FILE,
                "...info: timeslice diff between lowest & highest=%u%%", diff));
    }
