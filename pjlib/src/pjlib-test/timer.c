...
static int test_timer_heap(void)
{
    ...
    size =
      pj_timer_heap_mem_size(MAX_COUNT) + MAX_COUNT * sizeof(pj_timer_entry);
...
#    endif
        ...
        } while (PJ_TIME_VAL_LTE(now, expire) &&
                 pj_timer_heap_count(timer) > 0);
...
static int stress_worker(void* arg)
{
    ...
    while (!tparam->stopping) {
        ...
        if (job < prob[0])
            task = SCHEDULING;
        else if (job < (prob[0] + prob[1]))
            task = CANCELLING;
        else if (job < (prob[0] + prob[1] + prob[2]))
            task = POLLING;
        else
            task = NOTHING;
        ...
        if (task == SCHEDULING) {
            ...
            if (prev_status == 0 && status != PJ_SUCCESS) {
                ...
                if (pj_atomic_get(tparam->status[idx]) == 1) {
                    ...
                    PJ_LOG(3, ("test", "race schedule-schedule %d: %p", idx,
                               &tparam->entries[idx]));
                } else {
                    if (tparam->err != 0)
                        tparam->err = -210;
                 ...
                 */
                pj_thread_sleep(20);
                PJ_LOG(3, ("test", "race schedule-cancel/poll %d: %p", idx,
                           &tparam->entries[idx]));
            }
            if (status == PJ_SUCCESS) {
            ...
            if (prev_status == 0 && count > 0) {
                ...
                if (pj_atomic_get(tparam->status[idx]) == 1) {
                    ...
                    PJ_LOG(3, ("test", "race cancel-schedule %d: %p", idx,
                               &tparam->entries[idx]));
                } else {
                    if (tparam->err != 0)
                        tparam->err = -220;
                ...
                if (pj_atomic_get(tparam->status[idx]) == 0) {
                    ...
                    PJ_LOG(3, ("test", "race cancel-poll %d: %p", idx,
                               &tparam->entries[idx]));
                } else {
                    if (tparam->err != 0)
                        tparam->err = -230;
...
static int poll_worker(void* arg)
{
    ...
    while (!tparam->stopping) {
        ...
        if (count > 0) {
            ...
            PJ_LOG(5, ("test", "...thread #%d called %d entries", idx, count));
...
static int cancel_worker(void* arg)
{
    ...
    while (!tparam->stopping) {
        ...
        if (count > 0) {
            ...
            PJ_LOG(5,
                   ("test", "...thread #%d cancelled %d entries", idx, count));
            tparam->stat[idx].cnt += count;
...
#    endif
    ...
    if (ST_ENTRY_GROUP_LOCK_COUNT) {
        grp_locks = (pj_grp_lock_t**)pj_pool_calloc(
          pool, ST_ENTRY_GROUP_LOCK_COUNT, sizeof(pj_grp_lock_t*));
    ...
    entries =
      (pj_timer_entry*)pj_pool_calloc(pool, ST_ENTRY_COUNT, sizeof(*entries));
    if (!entries) {
        err = -50;
        goto on_return;
    }
    ...
    if (ST_STRESS_THREAD_COUNT) {
        stress_threads = (pj_thread_t**)pj_pool_calloc(
          pool, ST_STRESS_THREAD_COUNT, sizeof(pj_thread_t*));
    }
    for (i = 0; i < ST_STRESS_THREAD_COUNT; ++i) {
        status = pj_thread_create(pool, "poll", &stress_worker, &tparam, 0, 0,
                                  &stress_threads[i]);
        if (status != PJ_SUCCESS) {
    ...
    if (ST_POLL_THREAD_COUNT) {
        poll_threads = (pj_thread_t**)pj_pool_calloc(pool, ST_POLL_THREAD_COUNT,
                                                     sizeof(pj_thread_t*));
    }
    for (i = 0; i < ST_POLL_THREAD_COUNT; ++i) {
        status = pj_thread_create(pool, "poll", &poll_worker, &tparam, 0, 0,
                                  &poll_threads[i]);
        if (status != PJ_SUCCESS) {
    ...
    if (ST_CANCEL_THREAD_COUNT) {
        cancel_threads = (pj_thread_t**)pj_pool_calloc(
          pool, ST_CANCEL_THREAD_COUNT, sizeof(pj_thread_t*));
    }
    for (i = 0; i < ST_CANCEL_THREAD_COUNT; ++i) {
        status = pj_thread_create(pool, "cancel", &cancel_worker, &tparam, 0, 0,
                                  &cancel_threads[i]);
        if (status != PJ_SUCCESS) {
...
#    if SIMULATE_CRASH
    ...
    PJ_LOG(
      3, ("test", "...Releasing timer entry %p without cancelling it", entry));
...
on_return:
    ...
    for (i = 0; i < ST_POLL_THREAD_COUNT + ST_CANCEL_THREAD_COUNT; ++i) {
        PJ_LOG(3, ("test", "...Thread #%d (%s) executed %d entries", i,
                   (tparam.stat[i].is_poll ? "poll" : "cancel"),
                   tparam.stat[i].cnt));
    }
                        ...
                        pj_timestamp time_start, int start_idx, int end_idx)
{
    ...
    PJ_LOG(3, (THIS_FILE, "    Entries %s-%s: %s %s ent/sec", start_idx_str,
               end_idx_str, get_test_name(test_type), num_req_str));
}

static int bench_test(pj_timer_heap_t* timer, pj_timer_entry* entries,
                      pj_timestamp freq, BENCH_TEST_TYPE test_type)
{
    pj_timestamp t1;
...
static int timer_bench_test(void)
{
    ...
    /* Create and schedule timer entries */
    entries =
      (pj_timer_entry*)pj_pool_calloc(pool, BT_ENTRY_COUNT, sizeof(*entries));
    if (!entries) {
        err = -40;
        goto on_return;
    }
    ...
    for (i = 0; i < BT_REPEAT_RANDOM_TEST; ++i) {
        PJ_LOG(3,
               ("test", "    test %d of %d..", i + 1, BT_REPEAT_RANDOM_TEST));
