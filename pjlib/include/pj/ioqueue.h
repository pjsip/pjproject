...
#include <pj/types.h>
   ...
   * for the handle is inherited from the key concurrency setting of the
   *ioqueue, and the key concurrency setting for the ioqueue can be changed by
   *using #pj_ioqueue_set_default_concurrency(). The default key concurrency
   *setting for ioqueue itself is controlled by compile time setting
   ...
   *
   * \subsection pj_ioq_parallel Parallel Callback Executions for the Same
   *Handle
   *
   ...
   *only perform simultaneous callback executions on the same key when the key
   *has invoked multiple pending operations. This could be done for example by
   ...
   *  - by controlling on per handle/key basis, with
...
typedef struct pj_ioqueue_callback
{
     ...
     */
    void (*on_read_complete)(pj_ioqueue_key_t* key, pj_ioqueue_op_key_t* op_key,
 ...
 */
PJ_DECL(pj_status_t)
pj_ioqueue_create(pj_pool_t* pool, pj_size_t max_fd, pj_ioqueue_t** ioqueue);
 ...
 */
PJ_DECL(pj_status_t)
pj_ioqueue_set_lock(pj_ioqueue_t* ioque, pj_lock_t* lock,
 ...
 */
PJ_DECL(pj_status_t)
pj_ioqueue_register_sock(pj_pool_t* pool, pj_ioqueue_t* ioque, pj_sock_t sock,
 ...
 */
PJ_DECL(pj_status_t)
pj_ioqueue_register_sock2(pj_pool_t* pool, pj_ioqueue_t* ioque, pj_sock_t sock,
 ...
 */
PJ_DECL(void)
pj_ioqueue_op_key_init(pj_ioqueue_op_key_t* op_key, pj_size_t size);
